...
`,Le={init(){p.execMenuOnce("beautifyPage",()=>{this.beautifyPageElement();}),L.isVersion()&&p.execMenuOnce("beautifyHistoryVersionPage",()=>{this.beautifyHistoryVersionPage();}),p.execMenuOnce("beautifyGreasyforkBeautify",()=>{this.beautifyGreasyforkBeautify();}),p.execMenuOnce("beautifyUploadImage",()=>{this.beautifyUploadImage();});},beautifyPageElement(){s.info("美化页面元素"),v(be),v(we),v(xe),v(ve),v(`
        ...
        `;v(Se),v(t),c.ready(function(){let e=document.querySelector("ul.history_versions");if(!e){l.error(r.t("未找到history_versions元素列表"));return}Array.from(e.children).forEach(n=>{var w,g;let o=n.querySelector(".version-number a").href,a=n.querySelector(".version-number a").innerText,i=(w=n.querySelector(".version-date"))==null?void 0:w.getAttribute("datetime"),d=((g=n.querySelector(".version-changelog"))==null?void 0:g.innerHTML)||"",u=c.createElement("span",{className:"script-version-date",innerHTML:m.formatTime(i,r.t("yyyy年MM月dd日 HH:mm:ss"))}),f=c.createElement("div",{className:"script-tag",innerHTML:`
                    ...
                    </div>`}),y=c.createElement("div",{className:"script-note-box-body",innerHTML:d});n.appendChild(u),n.appendChild(f),n.appendChild(y);});});},beautifyGreasyforkBeautify(){s.info("美化 Greasyfork Beautify脚本"),v(Ce),m.isPhone()?v(`
            section#script-info,
            ...
            section#script-info{
                margin-top: 10px;
            }`);},beautifyUploadImage(){s.info("美化上传图片"),v(ke),c.ready(()=>{function t(o){for(;o.nextElementSibling;)o.parentElement.removeChild(o.nextElementSibling);}let e=document.querySelectorAll('input[type="file"]');e.forEach(o=>{o.getAttribute("name")!=="code_upload"&&(o.hasAttribute("accept")&&o.getAttribute("accept").includes("javascript")||c.on(o,["propertychange","input"],function(a){t(a.target);let i=a.currentTarget.files;if(!i||i.length===0)return;s.info(["选择的图片",i]),i.length>5&&c.after(o,c.createElement("p",{textContent:r.t("❌ 最多同时长传5张图片")}));let d=[];Array.from(i).forEach(u=>{(u.size>204800||!u.type.match(/png|jpg|jpeg|gif|apng|webp/i))&&d.push(u);}),d.length!==0&&d.forEach(u=>{c.after(o,c.createElement("p",{textContent:r.t("❌ 图片：{{name}} 大小：{{size}}",{name:u.name,size:u.size})}));});}));}),["textarea#comment_text","textarea.comment-entry"].forEach(o=>{c.on(o,"paste",a=>{s.info(["触发粘贴事件",a]),setTimeout(()=>{c.trigger(e,"input");},100);});});});}},Ee={init(){this.repairCodeLineNumber();},repairCodeLineNumber(){s.info("修复代码的行号显示不够问题"),p.getValue("beautifyGreasyforkBeautify")&&v(`
			.code-container pre code .marker{
				padding-left: 6px;
			}	
			`),m.waitNode("#script-content div.code-container pre.prettyprint ol").then(t=>{t.childElementCount>=1e3&&(s.success(`当前代码行数${t.childElementCount}行，超过1000行，优化行号显示问题`),v(`
                    pre.prettyprint{
                        padding-left: 26px;
                    }
					`));});}},Me={init(){p.execMenu("autoLogin",()=>{this.autoLogin();});},autoLogin(){m.waitNode("span.sign-in-link a[rel=nofollow]").then(async()=>{let t=p.getValue("user"),e=p.getValue("pwd");if(m.isNull(t)){l.error(r.t("请先在菜单中录入账号"));return}if(m.isNull(e)){l.error(r.t("请先在菜单中录入密码"));return}let n=document.querySelector("meta[name='csrf-token']");if(!n){l.error(r.t("获取csrf-token失败"));return}let o=l.loading(r.t("正在登录中...")),a=await U.post("https://greasyfork.org/zh-CN/users/sign_in",{fetch:!0,data:encodeURI(`authenticity_token=${n.getAttribute("content")}&user[email]=${t}&user[password]=${e}&user[remember_me]=1&commit=登录`),headers:{"Content-Type":"application/x-www-form-urlencoded"}});if(o.destroy(),!a.status){s.error(a),l.error(r.t("登录失败，请在控制台查看原因"));return}let i=a.data.responseText;c.parseHTML(i,!0,!0).querySelectorAll(".sign-out-link a[rel=nofollow][data-method='delete']").length?(l.success(r.t("登录成功，1s后自动跳转")),setTimeout(()=>{window.location.reload();},1e3)):(s.error(a),s.error(`当前账号:${t}`),s.error(`当前密码:${e}`),l.error(r.t("登录失败，可能是账号/密码错误，请在控制台查看原因")));});}},Te={init(){s.info("论坛-过滤"),v(`
...
`),y=new Map;document.querySelectorAll(".discussion-list-container").forEach((g,S)=>{var E;if(!g.querySelector("a.script-link"))return;const h={scriptName:g.querySelector("a.script-link").innerText,scriptUrl:g.querySelector("a.script-link").href,scriptId:b.getScriptId(g.querySelector("a.script-link").href),postUserName:g.querySelector("a.user-link").innerText,postUserHomeUrl:g.querySelector("a.user-link").href,postUserId:b.getUserId(g.querySelector("a.user-link").href),postTimeStamp:new Date(g.querySelector("relative-time").getAttribute("datetime")),snippetUrl:g.querySelector("a.discussion-title").href,snippet:g.querySelector("span.discussion-snippet").innerText,replyUserName:void 0,replyUserHomeUrl:void 0,replyUserId:void 0,replyTimeStamp:void 0};if(g.querySelector(".discussion-meta-item .discussion-meta-item")&&(h.replyUserName=g.querySelector(".discussion-meta-item .discussion-meta-item a.user-link").innerText,h.replyUserHomeUrl=g.querySelector(".discussion-meta-item .discussion-meta-item a.user-link").href,h.replyUserId=b.getUserId(h.replyUserHomeUrl),h.replyTimeStamp=new Date((E=g.querySelector(".discussion-meta-item .discussion-meta-item relative-time"))==null?void 0:E.getAttribute("datetime"))),y.has(h.snippet)&&p.getValue("greasyfork-discussions-filter-duplicate-comments")){let k=y.get(h.snippet).querySelector("a.discussion-title");k.setAttribute("data-repeat-tip-show","true");let C=0;k.hasAttribute("data-repeat-count")&&(C=parseInt(k.getAttribute("data-repeat-count"))),C++,k.setAttribute("data-repeat-count",C.toString()),k.setAttribute("data-repeat-tip-show",r.t("已过滤：{{oldCount}}",{oldCount:C})),s.success([`过滤重复内容：${h.snippet}`,h]),g.remove();return}y.set(h.snippet,g);for(const k of d)if(h.scriptId===k){s.success([`过滤脚本id：${h.scriptId}`,h]),g.remove();return}for(const k of u)if(h.postUserId===k){s.success([`过滤发布用户id：${h.postUserId}`,h]),g.remove();return}if(h.replyUserName){for(const k of f)if(h.replyUserId===k){s.success([`过滤回复用户id：${h.replyUserId}`,h]),g.remove();return}}});}},Ue={init(){c.ready(()=>{p.execMenuOnce("greasyfork-discussions-filter-enable",()=>{this.filterEnable();});});},filterEnable(){s.info("启用Greasyfork论坛过滤器"),Te.init();}},_e={init(){s.info("添加收藏按钮"),m.waitNode("ul#script-links li.current span").then(()=>{let t=c.createElement("li",{innerHTML:`
            ...
            <li class="user-collect-item" data-id="${u.id}" data-name="${u.name}">
              ...
              `;});let d=V.alert({title:{text:r.t("收藏集"),position:"center"},content:{html:!0,text:`<ul>${i}</ul>`},mask:{enable:!0,clickEvent:{toClose:!0}},btn:{ok:{enable:!1}},width:V.isPhone()?"92dvw":"500px",height:"auto",drag:!0,only:!0,style:`
            .pops{
            ...
            `});c.on(d.$shadowRoot,"click",".collect-add-script-id",async function(u){let f=u.target.closest(".user-collect-item"),y=f.dataset.id;f.dataset.name;let w=l.loading(r.t("添加中...")),g=await b.getUserCollectionInfo(n,y);if(!g){w.close();return}let S=m.cloneFormData(g),h=m.cloneFormData(g),E=!1;for(const[z,T]of g.entries())if(z==="scripts-included[]"&&JSON.stringify(T)==JSON.stringify(e)){E=!0;break}else h.append(z,T),S.append(z,T);if(E){l.warning(r.t("该脚本已经在该收藏集中")),w.close();return}S.set("add-script",e),S.set("script-action","i"),h.append("scripts-included[]",e),h.set("save","1");let k=new URLSearchParams(S),C=new URLSearchParams(h),q=Array.from(k).map(([z,T])=>`${encodeURIComponent(z)}=${encodeURIComponent(T)}`).join("&"),R=Array.from(C).map(([z,T])=>`${encodeURIComponent(z)}=${encodeURIComponent(T)}`).join("&");s.info(["添加的数据",q]),s.info(["保存的数据",R]);let M=await b.updateUserSetsInfo(n,y,q);if(!M){w.close();return}let I=M.querySelector(".change-script-set");if(!I){l.error(r.t("添加失败，{{selector}}元素不存在",{selector:".change-script-set"})),w.close();return}let P=I.querySelector("section");if(!P){l.error(r.t("添加失败，{{selector}}元素不存在",{selector:"section"})),w.close();return}let G=P.querySelector(".alert");G?V.alert({title:{text:r.t("添加失败"),position:"center"},content:{text:G.innerHTML,html:!0},mask:{enable:!0,clickEvent:{toClose:!0}},style:`
                        .pops-alert-content{
                            font-style: italic;
                        ...
                        `,drag:!0,dragLimit:!0,width:V.isPhone()?"88vw":"400px",height:V.isPhone()?"50vh":"300px"}):(await b.updateUserSetsInfo(n,y,R),l.success(r.t("添加成功"))),w.close();}),c.on(d.$shadowRoot,"click",".collect-delete-script-id",async function(u){let f=u.target.closest(".user-collect-item"),y=f.dataset.id;f.dataset.name;let w=l.loading(r.t("删除中...")),g=await b.getUserCollectionInfo(n,y);if(!g){w.close();return}let S=new FormData,h=new FormData;for(const[M,I]of g.entries())M==="scripts-included[]"&&JSON.stringify(I)==JSON.stringify(e)||(h.append(M,I),S.append(M,I));S.set("remove-scripts-included[]",e),S.set("remove-selected-scripts","i"),S.delete("script-action"),h.set("save","1");let E=new URLSearchParams(S),k=new URLSearchParams(h),C=Array.from(E).map(([M,I])=>`${encodeURIComponent(M)}=${encodeURIComponent(I)}`).join("&"),q=Array.from(k).map(([M,I])=>`${encodeURIComponent(M)}=${encodeURIComponent(I)}`).join("&");if(s.info(["删除的数据",C]),s.info(["保存的数据",q]),!await b.updateUserSetsInfo(n,y,C)){w.close();return}await b.updateUserSetsInfo(n,y,q),l.success(r.t("删除成功")),w.close();});}},N={init(){this.checkPage(),Le.init(),L.isCodeStrict()&&p.execMenuOnce("fullScreenOptimization",()=>{this.fullScreenOptimization();}),L.isCode()&&Ee.init(),L.isDiscuessions()&&Ue.init(),c.ready(()=>{x.initEnv(),Me.init(),L.isScriptList()&&p.execMenuOnce("greasyfork-shield-enable",()=>{Z.init();}),x.handleLocalGotoCallBack(),N.setFindCodeSearchBtn(),_e.init(),N.repairImgShow(),N.scriptHomepageAddedTodaySUpdate(),N.languageSelectorLocale(),p.execMenuOnce("optimizeImageBrowsing",()=>{N.optimizeImageBrowsing();}),p.execMenuOnce("overlayBedImageClickEvent",()=>{N.overlayBedImageClickEvent();}),L.isCodeStrict()&&p.execMenuOnce("addCopyCodeButton",()=>{N.addCopyCodeButton();}),L.isCodeStrict()||N.addMarkdownCopyButton();});},setFindCodeSearchBtn(){s.info("设置代码搜索按钮(对于库)"),m.waitNode("ul#script-links li.current span").then(()=>{let t=c.createElement("li",{innerHTML:`
					<a href="javascript:;">
						<span>${r.t("寻找引用")}</span>
        ...
        `);function t(n=[],o=0){let a="";n.forEach(u=>{a+=`<li><img data-src="${u}" loading="lazy"></li>`;});let i=c.createElement("ul",{innerHTML:a}),d=new ie(i,{inline:!1,url:"data-src",zIndex:m.getMaxZIndex()+100,hidden:()=>{d.destroy();}});o=o<0?0:o,d.view(o),d.zoomTo(1),d.show();}function e(n){return n.getAttribute("data-src")||n.getAttribute("src")||n.getAttribute("alt")}c.on(document,"click","img",function(n){var y;let o=n.target;if(((y=o.parentElement)==null?void 0:y.localName)==="a"&&o.hasAttribute("data-screenshots")||o.closest(".viewer-container")||o.closest(".lum-lightbox-position-helper"))return;let a=o.closest(".user-content"),i=[],d=0,u=[],f=e(o);f!=null&&f.startsWith("https://img.shields.io")||(a?(a.querySelectorAll("img").forEach(w=>{u.push(w);let g=e(w),S=w.parentElement;(S==null?void 0:S.localName)==="a"&&(g=S.getAttribute("data-href")||S.href),i.push(g);}),d=u.indexOf(o),d===-1&&(d=0)):(i.push(f),d=0),s.success(["点击浏览图片👉",i,d]),t(i,d));}),document.querySelectorAll(".user-screenshots").forEach(n=>{let o=n.querySelector("a");if(!o)return;let a=o.getAttribute("data-href")||o.getAttribute("href"),i=n.querySelector("img");i&&(i.setAttribute("data-screenshots","true"),i.setAttribute("data-src",a),o.setAttribute("href","javascript:;"),c.after(o,i),o.remove());});},overlayBedImageClickEvent(){s.info("覆盖图床图片的parentElement的a标签"),document.querySelectorAll(".user-content a>img").forEach(t=>{let e=t.parentElement,n=e.getAttribute("href");e.setAttribute("data-href",n),e.removeAttribute("href"),c.on(e,"click",void 0,function(o){l.warning(`<div style="overflow-wrap: anywhere;">${r.t("拦截跳转：")}<a href="${n}" target="_blank">${n}</a></div>`,{html:!0,timeout:5e3,zIndex:m.getMaxZIndex()});});});},async scriptHomepageAddedTodaySUpdate(){if(!L.isScript()||!document.querySelector("#install-area"))return;s.info("脚本首页新增今日更新");let t=await b.getScriptStats(b.getScriptId());if(!t)return;let e=m.toJSON(t.responseText);s.info(["统计信息",e]);let n=e[m.formatTime(void 0,"yyyy-MM-dd")];if(!n){s.error("今日份的统计信息不存在");return}let o=n.update_checks;s.info(["今日统计信息",n]),c.after("dd.script-show-daily-installs",c.createElement("dt",{className:"script-show-daily-update_checks",innerHTML:`<span>${r.t("今日检查")}</span>`})),c.after("dt.script-show-daily-update_checks",c.createElement("dd",{className:"script-show-daily-update_checks",innerHTML:"<span>"+o+"</span>"}));},addCopyCodeButton(){s.info("添加复制代码按钮"),m.waitNode("div#script-content div.code-container").then(t=>{let e=c.createElement("button",{textContent:r.t("复制代码")},{style:"margin-bottom: 1em;"});c.on(e,"click",async function(){let n=l.loading(r.t("加载文件中...")),o=await U.get(`https://greasyfork.org/zh-CN/scripts/${b.getScriptId()}.json`,{fetch:!0,responseType:"json"});if(!o.status){n.close();return}let i=m.toJSON(o.data.responseText).code_url;s.success(["代码地址：",i]);let d=await U.get(i);if(!d.status){n.close();return}n.close(),m.setClip(d.data.responseText),l.success(r.t("复制成功"));}),c.before(t,e);});},fullScreenOptimization(){s.info("F11全屏，F键代码全屏"),v(`
        .code-wide-screen{
          position: absolute;
          top: 0;
          left: 0;
        ...
        `);let t=!1;c.keydown(window,function(e){if(e.key.toLowerCase()==="f"){let n=document.querySelector("#script-content div.code-container code");e.altKey&&e.shiftKey?(m.preventEvent(e),n.classList.contains("code-wide-screen")?n.classList.remove("code-wide-screen"):n.classList.add("code-wide-screen")):!e.altKey&&!e.ctrlKey&&!e.shiftKey&&!e.metaKey&&(m.preventEvent(e),t?(m.exitFullScreen(n),t=!1):(m.enterFullScreen(n),t=!0));}},{capture:!0});},addMarkdownCopyButton(){s.info("在Markdown右上角添加复制按钮"),v(`
        pre{
          position: relative;
        ...
        `);function t(){let e=c.createElement("div",{className:"zeroclipboard-container",innerHTML:`
            ...
            `}),n=e.querySelector(".js-clipboard-copy"),o=e.querySelector(".octicon-copy"),a=e.querySelector(".octicon-check-copy");return c.on(e,"click",function(){let i=e.parentElement.querySelector("code");if(!i&&e.parentElement.className.includes("prettyprinted")&&(i=e.parentElement),!i){l.error(r.t("未找到{{selector}}元素",{selector:"code"}));return}m.setClip(i.innerText||i.textContent),n.setAttribute("success","true"),o.setAttribute("aria-hidden","true"),a.removeAttribute("aria-hidden");let d=V.tooltip({target:n,content:r.t("✅ 复制成功!"),position:"left",className:"github-tooltip",alwaysShow:!0});setTimeout(()=>{n.removeAttribute("success"),a.setAttribute("aria-hidden","true"),o.removeAttribute("aria-hidden"),d.close();},2e3);}),e}document.querySelectorAll("pre").forEach(e=>{if(e.querySelector("div.zeroclipboard-container"))return;let o=t(),a=c.createElement("div",{className:"snippet-clipboard-content"});c.before(e,a),a.appendChild(e),a.appendChild(o);});},languageSelectorLocale(){let t=p.getValue("language-selector-locale"),e=window.location.pathname.split("/").filter(n=>!!n)[0];if(s.success("选择语言："+t),s.success("当前语言："+e),!m.isNull(t)&&t!==e){let n=null,o=b.getSwitchLanguageUrl(t);b.switchLanguage(o),s.success("新Url："+o),l.loading(r.t("当前语言：{{currentLocaleLanguage}}，，3秒后切换至：{{localeLanguage}}",{currentLocaleLanguage:e,localeLanguage:t}),{timeout:3e3,showClose:!0,onClose(){clearTimeout(n);}}),l.info(r.t("导航至：")+o,{timeout:3e3}),n=setTimeout(()=>{window.location.href=o;},3e3);}},async UIScriptList(t,e,n,o){var w,g,S;if(!x.isLogin){l.error(r.t("请先登录账号！"));return}let i=x.getUserLinkElement().href,d=(S=(g=(w=i==null?void 0:i.split("/"))==null?void 0:w.pop())==null?void 0:g.match(/([0-9]+)/))==null?void 0:S[0],u=V.loading({mask:{enable:!0},parent:o,content:{text:r.t("获取信息中，请稍后...")},addIndexCSS:!1}),f=await b.getUserInfo(d);if(u.close(),!f)return;s.info(f);let y=t==="script-list"?f.scriptList:f.scriptLibraryList;l.success(r.t("获取成功，共 {{count}} 个",{count:y.length}));for(const h of y){let E=c.createElement("li",{className:"w-script-list-item",innerHTML:`
				...
				<div class="w-script-update-time">
					<p>${r.t("更新：")}${m.getDaysDifference(new Date(h.code_updated_at).getTime(),void 0,"auto")}前</p>
				</div>
				</div>
            `}),k=E.querySelector(".w-script-info"),C=c.createElement("div",{className:"pops-panel-button",innerHTML:`
				...
				`});h.deleted&&(E.classList.add("w-script-deleted"),C.querySelector("button").setAttribute("disabled","true")),c.on(C,"click",void 0,async function(){s.success(["同步",h]);let q=C.querySelector("button"),R=C.querySelector("button span"),M=c.createElement("i",{className:"pops-bottom-icon",innerHTML:V.config.iconSVG.loading},{"is-loading":!0});q.setAttribute("disabled","true"),q.setAttribute("data-icon","true"),R.innerText=r.t("同步中..."),c.before(R,M);let I=h==null?void 0:h.id,P=await b.getSourceCodeSyncFormData(I.toString());if(P){const G="script[script_sync_type_id]";if(P.has(G)){let z=P.get(G),T="";z.toString()==="1"?T=r.t("手动"):z.toString()==="2"?T=r.t("自动"):z.toString()==="3"&&(T="webhook");let Y=E.querySelector(".w-script-sync-type");Y?Y.querySelector("p").innerText=r.t("同步方式：{{syncMode}}",{syncMode:T}):c.append(k,`
								<div class="w-script-sync-type">
									<p>${r.t("同步方式：{{syncMode}}",{syncMode:T})}
									</p>
								</div>`),await b.sourceCodeSync(h.id.toString(),P)?l.success(r.t("同步成功")):l.error(r.t("同步失败"));}else l.error(r.t("该脚本未设置同步信息"));}q.removeAttribute("disabled"),q.removeAttribute("data-icon"),R.innerText=r.t("同步代码"),M.remove();}),E.appendChild(C),o.appendChild(E);}},checkPage(){s.info("检测gf页面是否正确加载，有时候会出现"),c.ready(()=>{if(document.body.firstElementChild&&document.body.firstElementChild.localName==="p"&&document.body.firstElementChild.innerText.includes("We're down for maintenance. Check back again soon.")){let t=parseInt(B("greasyfork-check-page-time",0));if(t&&Date.now()-t<5*1e3){l.error(r.t("上次重载时间 {{time}}，5秒内拒绝反复重载",{time:m.formatTime(t,"yyyy-MM-dd HH:mm:ss")}));return}W("greasyfork-check-page-time",Date.now()),window.location.reload();}});}},Ie={id:"greasy-fork-panel-config-script-list",title:r.t("脚本列表"),callback(t,e,n){N.UIScriptList("script-list",t,e,n);},forms:[]},Ae={id:"greasy-fork-panel-config-library",title:r.t("库"),callback(t,e,n){N.UIScriptList("script-library",t,e,n);},forms:[]},ze=`.w-script-list-item {\r
	padding: 10px 0;\r
