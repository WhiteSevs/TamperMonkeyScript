        ...
        bottomElement.forEach((item) => {
            ...
            item
                ...
                .forEach((searchItemEle) => {
                    ...
                    (searchItemEle as HTMLElement).style.setProperty("padding", "0.06rem");
                });
            ...
            DOMUtils.on(item, "click", "div.rw-list-new", function (event) {
                ...
                window.location.href = `https://m.baidu.com/s?word=${(event.target as HTMLElement)?.textContent?.trim()}`;
            });
        });
    },
    /**
