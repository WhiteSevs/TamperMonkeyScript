...
const BaiduResultItem = {
    originURLMap: null as unknown as UtilsDictionaryConstructor<string, string>,
    /**
     * 判断链接是否是百度的中转链接
     * @param url
     * @returns
    ...
    setArticleOriginUrl(targetNode: HTMLElement, articleURL: string) {
        ...
        /* 这个是百度笔记(可能) */
        (Array.from(targetNode
            .querySelectorAll("div[data-aftclk][class*=img-container]")) as HTMLElement[])
            .forEach((item) => {
                let domOriginUrl = BaiduResultItem.parseDOMAttrOriginUrl(item);
                if (
        ...
        /* 对搜索结果中存在的视频进行处理 */
        (Array.from(targetNode
            .querySelectorAll("div.c-video-container div[data-aftclk]")) as HTMLElement[])
            .forEach((item) => {
                let domOriginUrl = BaiduResultItem.parseDOMAttrOriginUrl(item);
                if (
                    !utils.isNull(domOriginUrl) &&
        ...
        /* 对搜索结果中存在的视频进行处理 */
        (Array.from(targetNode
            .querySelectorAll('div[data-module="sc_pc"] div[rl-link-href]')) as HTMLElement[])
            .forEach((item) => {
                let domOriginUrl = BaiduResultItem.parseDOMAttrOriginUrl(item);
                if (
                    !utils.isNull(domOriginUrl) &&
    ...
    addCSDNFlag(targetNode: HTMLElement) {
        ...
        let title_text_element =
            BaiduResultItem.getItemTitleElement(targetNode);
    ...
    removeAds() {
        ...
        document.querySelectorAll(".c-result.result").forEach((item) => {
            ...
            if (
                BaiduSearchRule.handleCustomRule(item as HTMLDivElement, searchArticleOriginal_link)
            ) {
                item.remove();
                return;
            }
            ...
            if (
                ...
                document.querySelectorAll("span").forEach((item) => {
                    let resultParentElement = item.parentElement?.parentElement as HTMLElement;
                    if (
            ...
            /* 底部标识 */
            let bottomLogoElement = Array.from(item.querySelectorAll(".c-color-source")) as HTMLElement[];
    ...
    redirectTopLink() {
        document.querySelectorAll(".se-head-tablink a").forEach((item) => {
            if (
                ...
                BaiduResultItem.isBaiDuTransferStation(item.getAttribute("href") as string) &&
     ...
     */
    async replaceLink() {
        let searchResultList = Array.from(document.querySelectorAll(".c-result.result")) as HTMLDivElement[];
    ...
    replaceVSearchLink() {
        document.querySelectorAll("#realtime-container  div:not([class])").forEach((element) => {
            ...
            if (linkElement.hasAttribute("data-sf-visited")) {
                let dataSfVisited = linkElement.getAttribute("data-sf-visited") as string;
