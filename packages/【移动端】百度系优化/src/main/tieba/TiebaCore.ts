...
const TiebaCore = {
    ...
    getLandlordInfo() {
        ...
        ) as NestedObjectWithToString)?.__vue__?.$props?.author as LandlordInfo;
    },
    /**
     * 获取当前的贴吧名字
     */
    getCurrentForumName(): string {
        let tbMobileViewport = CommonUtil.getVue(document.querySelector(".tb-mobile-viewport"))?.forum?.name;

        let mainPageWrap =
            CommonUtil.getVue(document.querySelector(".main-page-wrap"))?.$children[0]
                ?.$children[0]?.forum?.name;

        let tbForum = CommonUtil.getVue(document.querySelector(".tb-mobile-viewport .tb-forum"))?.forum?.name;

        let appView = CommonUtil.getVue(document.querySelector(".app-view"))?.forum?.name;
