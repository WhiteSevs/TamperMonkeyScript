...
const TiebaHomeData = {
	...
	async getUserDataWithPCDoc(): Promise<PCUserInfo | undefined> {
		...
		Array.from(
			pcDoc.querySelectorAll("ul.new_list > div") as NodeListOf<HTMLDivElement>
		).forEach((listItem) => {
			let postInfo: HomePostsInfo = {
				...
				content: (
					listItem.querySelector("ul.new_list > div .n_txt") as HTMLDivElement
				).innerHTML,
				forumName: (
					listItem.querySelector("ul.new_list > div .n_name") as HTMLSpanElement
