...
const TiebaSearch = {
	...
	postsSearch(isFirstClick = false) {
		...
		let lockFunction = null as unknown as {
			lock: () => void;
			unlock: () => void;
			run: () => Promise<void>;
		};
		...
		async function getSearchResult(qw = "", pn = 0, sm = 1, kw = "") {
			...
			searchDoc
				...
				.forEach((item) => {
					...
					let url = (item.querySelector("span.p_title a") as HTMLAnchorElement)
						.href;
					let title = (
						item.querySelector("span.p_title a") as HTMLAnchorElement
					).innerHTML;
					let content = (item.querySelector(".p_content") as HTMLDivElement)
						.innerHTML;
					/* 来自哪个贴吧 */
					let forum = (
						item.querySelector("a.p_forum font.p_violet") as HTMLElement
					).textContent;
					let author = (
					...
					let authorHomeUrl =
						"https://tieba.baidu.com/home/main?un=" + gbkEncoder.encode(author);
					let time = (item.querySelector(".p_date") as HTMLElement).textContent;
					let imgList: string[] = [];
					(
						Array.from(item.querySelectorAll("img.p_pic")) as HTMLImageElement[]
					).forEach((pictureImg) =>
		...
		function getItemElement(item: SearchResultInfo) {
			...
			content_BDE_Image.forEach((BDE_Image) => {
				/* 高清图片下标 */
				let originalImageIndex = (item["media"] as string[]).findIndex(
					(src) => {
		...
		function setCurrentOrderHTML(
			...
			for (const targetElement of Array.from(
				...
				let flag = false;
				if (
					(targetElementHTML.includes("按时间顺序") && searchModel === 0) ||
					(targetElementHTML.includes("按时间倒序") && searchModel === 1) ||
					(targetElementHTML.includes("按相关性顺序") && searchModel === 2) ||
			...
			if (searchType === 1) {
				DOMUtils.val(
					searchElement.querySelector("#searchtb") as HTMLInputElement,
					true
				);
		...
		async function _click_event_() {
			...
			for (const searchResultItem of searchResult) {
				DOMUtils.append(
					contentElement,
					getItemElement(searchResultItem as SearchResultInfo)
				);
			}
			loadingView.hide();
			if (nextPageUrl) {
