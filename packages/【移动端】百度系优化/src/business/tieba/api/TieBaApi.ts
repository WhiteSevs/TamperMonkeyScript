...
const TieBaApi = {
    ...
    */
    ...
    async getUserPosts(un: string, pn = 1) {
        let getResp = await httpx.get(`https://tieba.baidu.com/home/post?un=${un}&is_ajax=1&lp=&pn=${pn}`,
            {
                fetch: true,
                headers: {
                    "User-Agent": utils.getRandomPCUA(),
        ...
        Array.from(contentDoc.querySelectorAll(".list_item")).forEach(liElement => {
            let postInfo: HomePostsInfo = {
                ...
                forumName: (liElement.querySelector(".post_list_item_info_forum") as HTMLSpanElement).innerText.replace(/吧$/, ""),
            ...
            if (liElement.querySelector(".thread_body_media")) {
                postInfo.mediaList.concat(Array.from(liElement.querySelectorAll(".thread_body_media img") as NodeListOf<HTMLImageElement>).map(item => item.src))
            }
...
const TiebaPageDataApi = {
     ...
     */
    getForumId() {
        let dataBannerInfoStr = document
...
export { TieBaApi, TiebaUrlApi, TiebaPageDataApi, type HomePostsInfo, type PanelUserInfo, type UserJSON };