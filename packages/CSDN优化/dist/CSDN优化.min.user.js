...
`,I={init(){l(se),i.execMenu("csdn-hua-wei-cloud-shieldCloudDeveloperTaskChallengeEvent",()=>{this.shieldCloudDeveloperTaskChallengeEvent();}),i.execMenu("csdn-hua-wei-cloud-autoExpandContent",()=>{this.autoExpandContent();}),i.execMenu("csdn-hua-wei-cloud-shieldLeftFloatingButton",()=>{this.shieldLeftFloatingButton();}),i.execMenu("csdn-hua-wei-cloud-blockRightColumn",()=>{this.blockRightColumn();}),i.execMenu("csdn-hua-wei-cloud-blockRecommendedContentAtTheBottom",()=>{this.blockRecommendedContentAtTheBottom();}),i.execMenu("csdn-hua-wei-cloud-shieldTheBottomForMoreRecommendations",()=>{this.shieldTheBottomForMoreRecommendations();});},autoExpandContent(){o.success("自动展开全文"),l(`
        ...
        `);},shieldCloudDeveloperTaskChallengeEvent(){new d.GM_Cookie().set({name:"show_join_group_index",value:1}),o.success("屏蔽云开发者任务挑战活动");},shieldLeftFloatingButton(){o.success("屏蔽左侧悬浮按钮，包括当前阅读量、点赞按钮、评论按钮、分享按钮"),l(`
        div.toolbar-wrapper.article-interact-bar{
          display: none !important;
        }`);},blockRightColumn(){o.success("屏蔽右侧栏，包括相关产品-活动日历-运营活动-热门标签"),l(`
        div.page-home-right.dp-aside-right{
          display: none !important;
        }
        `);},blockRecommendedContentAtTheBottom(){o.success("屏蔽底部推荐内容"),l(`
        div.recommend-card-box{
          display: none !important;
        }`);},shieldTheBottomForMoreRecommendations(){o.success("屏蔽底部更多推荐"),l(`
        div.more-article{
          display: none !important;
...
`;function y(e=""){d.waitNodeList(e).then(()=>{document.querySelectorAll(e).forEach(t=>{t.remove();});});}const ve={init(){this.addCSS(),i.execMenu("m-csdn-blog-shieldTopToolbar",()=>{this.shieldTopToolbar();}),i.execMenu("m-csdn-blog-notLimitCodePreMaxHeight",()=>{this.notLimitCodePreMaxHeight();}),i.execMenu("m-csdn-blog-notLimitCommentMaxHeight",()=>{this.notLimitCommentMaxHeight();}),i.execMenu("m-csdn-blog-allowSelectText",()=>{this.allowSelectText();}),i.execMenu("m-csdn-blog-autoExpandContent",()=>{this.autoExpandContent();}),i.execMenu("m-csdn-blog-blockBottomArticle",()=>{this.blockBottomArticle();}),i.execMenu("m-csdn-blog-blockComment",()=>{this.blockComment();}),C.ready(()=>{i.execMenu("m-csdn-blog-removeAds",()=>{this.removeAds();}),i.execMenu("m-csdn-blog-refactoringRecommendation",()=>{this.refactoringRecommendation();}),i.execMenu("m-csdn-blog-unBlockCopy",()=>{W.unBlockCopy();});});},addCSS(){l(we),l(Ce);},shieldTopToolbar(){o.success("屏蔽顶部Toolbar"),l(`
        ...
        `);},refactoringRecommendation(){function e(){o.success("重构底部推荐"),document.querySelectorAll(".container-fluid").forEach(n=>{var v,D;let a="",s="",c="",f="",p=!1,g=!1;if(n.hasAttribute("data-url")){if(a=n.getAttribute("data-url"),s=(v=n.querySelector(".recommend_title div.left"))==null?void 0:v.innerHTML,!n.querySelector(".text"))return;c=(D=n.querySelector(".text"))==null?void 0:D.innerHTML,n.querySelectorAll(".recommend-img").length&&n.querySelectorAll(".recommend-img").forEach(q=>{f+=q.innerHTML;});}else o.info("节点上无data-url"),a=n.querySelector("a[data-type]").getAttribute("href"),s=n.querySelector(".recommend_title div.left").innerHTML,c=n.querySelector(".text").innerHTML;var h=new URL(a);h.host==="download.csdn.net"||h.host==="www.iteye.com"&&h.pathname.match(/^\/resource/gi)?(o.info("该链接为csdn资源下载"),p=!0,s='<div class="component-box"><a class="praise" href="javascript:;">CSDN下载</a></div>'+s):h.origin.match(/edu.csdn.net/gi)&&(g=!0,o.info("该链接为csdn学院下载"),s='<div class="component-box"><a class="csdn-edu-title" href="javascript:;">CSDN学院</a></div>'+s),n.setAttribute("class","GM-csdn-dl"),n.setAttribute("data-url",a),n.innerHTML=`<div class="GM-csdn-title"><div class="left">${s}</div></div><div class="GM-csdn-content">${c}</div><div class="GM-csdn-img">${f}</div>`,n.addEventListener("click",function(){i.getValue("m-csdn-blog-openNewTab")?window.open(a,"_blank"):window.location.href=a;}),(p||g)&&i.getValue("m-csdn-blog-removeResourceArticle")&&n.remove();});}let t=new d.LockFunction(e,50);d.waitNode("#recommend").then(n=>{o.success("重构底部推荐"),t.run(),d.mutationObserver(n,{callback:()=>{t.run();},config:{childList:!0,subtree:!0,attributes:!0}});});},blockBottomArticle(){o.success("屏蔽底部文章"),l("#recommend{display:none !important;}");},blockComment(){o.success("屏蔽评论"),l("#comment{display:none !important;}");},removeAds(){o.info("去除广告"),y(".passport-login-container"),y(".btn_open_app_prompt_box.detail-open-removed"),y(".add-firstAd"),y("div.feed-Sign-weixin"),y("div.ios-shadowbox");},notLimitCodePreMaxHeight(){o.success("不限制代码块最大高度"),l(`
        pre{
            max-height: unset !important;
        }
        `);},notLimitCommentMaxHeight(){o.success("不限制评论区最大高度"),l(`
        #comment{
          max-height: none !important;
        }
      `);},allowSelectText(){o.success("允许选择文字"),l(`
        #content_views,
        #content_views pre,
        ...
        `);},autoExpandContent(){o.success("自动展开内容"),l(`
...
`,Se={init(){l(ye),i.execMenu("m-csdn-wenku-shieldBottomToolbar",()=>{this.shieldBottomToolbar();});},shieldBottomToolbar(){l(`
        .page-container > div.btn{
            display: none !important;
        }
        `);}},G={init(){m.isLink()?(o.info("Router: 中转链接"),ge.init()):m.isHuaWeiCloudBlog()?(o.info("Router: 华为云联盟"),xe.init()):m.isBlog()?(o.info("Router: 博客"),ve.init()):m.isWenKu()?(o.info("Router: 文库"),Se.init()):o.error("暂未适配，请反馈开发者："+globalThis.location.href);}};i.init();let j=d.isPhone(),M="change_env_set",b=S(M);H.add({key:M,text:`⚙ 自动: ${j?"移动端":"PC端"}`,autoReload:!1,isStoreValue:!1,showText(e){return b==null?e:e+` 手动: ${b==1?"移动端":b==2?"PC端":"未知"}`},callback:()=>{let e=[0,1,2],t=window.prompt(`请输入当前脚本环境判定
1. 自动判断: 0
2. 移动端: 1
3. PC端: 2`,"0");if(!t)return;let n=parseInt(t);if(isNaN(n)){x.error("输入的不是规范的数字");return}if(!e.includes(n)){x.error("输入的值必须是0或1或2");return}n==0?V(M):k(M,n);}});b!=null?(o.info(`手动判定为${b===1?"移动端":"PC端"}`),b==1?G.init():b==2?A.init():(x.error("意外，手动判定的值不在范围内"),V(M))):j?(o.info("自动判定为移动端"),G.init()):(o.info("自动判定为PC端"),A.init());
