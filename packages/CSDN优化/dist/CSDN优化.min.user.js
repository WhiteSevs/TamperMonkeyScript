...
`,De={init(){i.execMenu("m-csdn-download-removeAds",()=>{u(Re);}),i.execMenuOnce("m-csdn-download-automaticallyExpandResourceIntroduction",()=>{this.automaticallyExpandResourceIntroduction();});},automaticallyExpandResourceIntroduction(){o.info("自动展开资源介绍"),s.addBlockCSS("label.unfold-font"),u(`
		...
		`);}},$={init(){f.isLink()?(o.info("Router: 中转链接"),Se.init()):f.isHuaWeiCloudBlog()?(o.info("Router: 华为云联盟"),ye.init()):f.isBlog()?(o.info("Router: 博客"),ke.init()):f.isWenKu()?(o.info("Router: 文库"),_e.init()):f.isDownload()?(o.info("Router: 资源下载"),De.init()):o.error("暂未适配，请反馈开发者："+globalThis.location.href);}};i.init();let K=c.isPhone(),T="change_env_set",y=M(T);q.add({key:T,text:`⚙ 自动: ${K?"移动端":"PC端"}`,autoReload:!1,isStoreValue:!1,showText(e){return y==null?e:e+` 手动: ${y==1?"移动端":y==2?"PC端":"未知"}`},callback:()=>{let e=[0,1,2],t=window.prompt(`请输入当前脚本环境判定

自动判断: 0
移动端: 1
PC端: 2`,"0");if(!t)return;let n=parseInt(t);if(isNaN(n)){C.error("输入的不是规范的数字");return}if(!e.includes(n)){C.error("输入的值必须是0或1或2");return}n==0?P(T):R(T,n);}});y!=null?(o.info(`手动判定为${y===1?"移动端":"PC端"}`),y==1?$.init():y==2?G.init():(C.error("意外，手动判定的值不在范围内"),P(T))):K?(o.info("自动判定为移动端"),$.init()):(o.info("自动判定为PC端"),G.init());
