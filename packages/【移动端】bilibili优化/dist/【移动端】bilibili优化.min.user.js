...
`,at={$data:{isAddBeautifyCSS:!1},init(){rt.init(),s.execMenu("bili-video-setVideoPlayer",()=>{this.setVideoPlayer();}),s.execMenuOnce("bili-video-repairVideoBottomAreaHeight",()=>{this.repairVideoBottomAreaHeight();}),s.execMenuOnce("bili-video-autoClickContinueToWatchOnTheWebpage",()=>{this.autoClickContinueToWatchOnTheWebpage();}),s.execMenu("bili-video-beautify",()=>{this.beautify();}),s.execMenuOnce("bili-video-cover-bottomRecommendVideo",()=>{this.coverBottomRecommendVideo();}),s.execMenuOnce("bili-video-gestureReturnToCloseCommentArea",()=>{this.gestureReturnToCloseCommentArea();});},beautify(){r.info("美化"),this.$data.isAddBeautifyCSS||(this.$data.isAddBeautifyCSS=!0,C(ot)),a.waitNode(d.className.video+" .bottom-tab .list-view .card-box",1e4).then(t=>{if(!t){r.error("$cardBox is null");return}let e=new a.LockFunction(()=>{document.querySelectorAll(d.className.video+" .bottom-tab .list-view .card-box .v-card-toapp").forEach(i=>{var p,f;let n=i.querySelector(".title"),o=i.querySelector(".count .left"),l=u.getVue(i);if(n&&o&&!i.querySelector(".gm-right-container")){let y=document.createElement("div"),w=(f=(p=l==null?void 0:l.info)==null?void 0:p.owner)==null?void 0:f.name;y.className="gm-up-name",y.innerHTML=`
