...
const BilibiliLive = {
    ...
    preventOpenAppBtn() {
        utils.waitNode("body").then($body => {
            ...
            DOMUtils.on<PointerEvent | MouseEvent>($body, "click", ".open-app-btn", function (event) {
                utils.preventEvent(event);
            }, {
                capture: true
            })
            DOMUtils.on<PointerEvent | MouseEvent>($body, "click", "#web-player-controller-wrap-el", function (event) {
                utils.preventEvent(event);
            }, {
                capture: true
            })
