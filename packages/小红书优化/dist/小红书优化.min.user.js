...
`);c.init();let oe=s.isPhone(),P="change_env_set",k=S(P);ee.add({key:P,text:`⚙ 自动: ${oe?"移动端":"PC端"}`,autoReload:!1,isStoreValue:!1,showText(t){return k==null?t:t+` 手动: ${k==1?"移动端":k==2?"PC端":"未知"}`},callback:()=>{let t=[0,1,2],e=window.prompt(`请输入当前脚本环境判定
1. 自动判断: 0
2. 移动端: 1
3. PC端: 2`,"0");if(!e)return;let o=parseInt(e);if(isNaN(o)){f.error("输入的不是规范的数字");return}if(!t.includes(o)){f.error("输入的值必须是0或1或2");return}o==0?Z(P):I(P,o);}});k!=null?(n.info(`手动判定为${k===1?"移动端":"PC端"}`),k==1?z.init():k==2?N.init():(f.error("意外，手动判定的值不在范围内"),Z(P))):oe?(n.info("自动判定为移动端"),z.init()):(n.info("自动判定为PC端"),N.init());
