    ...
    .author-box .author-desc-wrapper .author-desc .author-desc-trigger{
      display: none !important;
        ...
        *{
        ...
        `);}},xe={getSearchUrl(t){return `https://www.xiaohongshu.com/search_result?keyword=${t}&source=web_explore_feed`}},ve={init(){(u.getValue("pc-xhs-search-open-blank-btn")||u.getValue("pc-xhs-search-open-blank-keyboard-enter"))&&this.optimizationSearch();},optimizationSearch(){function t(e,o=!0){{let r=document.querySelector("#search-input");if(r){let l=r.value,n=xe.getSearchUrl(l);i.info("搜索内容: "+l),window.open(n,o?"_blank":"_self");}else f.error("未找到搜索的输入框");}}s.waitNode("#search-input").then(e=>{e.placeholder="搜索小红书",u.execMenu("pc-xhs-search-open-blank-keyboard-enter",()=>{s.listenKeyboard(e,"keydown",(o,r,l,n)=>{o==="Enter"&&!l.length&&(i.info("按下回车键"),s.preventEvent(n),e.blur(),t());});});}),s.waitNode("#search-input + .input-button .search-icon").then(e=>{u.execMenu("pc-xhs-search-open-blank-btn",()=>{m.on(e,"click",o=>{s.preventEvent(o),i.info("点击搜索按钮"),t();},{capture:!0});});});}},N={init(){u.execMenu("pc-xhs-hook-vue",()=>{X.webPackVue();}),u.execMenu("pc-xhs-allowCopy",()=>{N.allowPCCopy();}),u.execMenu("pc-xhs-open-blank-article",()=>{N.openBlankArticle();}),F.init(),ve.init();},allowPCCopy(){i.success("允许复制文字"),m.on(h,"copy",void 0,function(t){s.preventEvent(t);let e=h.getSelection();return e?s.setClip(e.toString()):i.error("未选中任何内容"),!1},{capture:!0});},openBlankArticle(){i.success("新标签页打开文章"),m.on(document,"click",".feeds-container .note-item",function(t){s.preventEvent(t);let o=t.target.querySelector("a[href]");o&&o.href?(i.info("跳转文章: "+o.href),window.open(o.href,"_blank")):f.error("未找到文章链接");},{capture:!0});}};w(`
.qmsg svg.animate-turn {
    fill: none;
}
`);u.init();let te=s.isPhone(),$="change_env_set",v=V($);Y.add({key:$,text:`⚙ 自动: ${te?"移动端":"PC端"}`,autoReload:!1,isStoreValue:!1,showText(t){return v==null?t:t+` 手动: ${v==1?"移动端":v==2?"PC端":"未知"}`},callback:()=>{let t=[0,1,2],e=window.prompt(`请输入当前脚本环境判定
1. 自动判断: 0
2. 移动端: 1
3. PC端: 2`,"0");if(!e)return;let o=parseInt(e);if(isNaN(o)){f.error("输入的不是规范的数字");return}if(!t.includes(o)){f.error("输入的值必须是0或1或2");return}o==0?Z($):D($,o);}});v!=null?(i.info(`手动判定为${v===1?"移动端":"PC端"}`),v==1?z.init():v==2?N.init():(f.error("意外，手动判定的值不在范围内"),Z($))):te?(i.info("自动判定为移动端"),z.init()):(i.info("自动判定为PC端"),N.init());
