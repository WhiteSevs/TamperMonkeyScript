...
// @require      https://greasyfork.org/scripts/455186-whitesevsutils/code/WhiteSevsUtils.js?version=1137591
  ...
  var baidu = {
    ...
    search() {
      ...
      function replaceLink() {
        /* 替换链接 */
        function replaceElementAttrHref(DOM, url) {
          /* 修改元素的href属性，并返回url */
          Array.from(DOM.getElementsByTagName("a")).forEach((item) => {
            let newUrl = real_link.has(item.href)
        ...
        function getRealLinkJson() {
          ...
          Array.from($("script[id^='atom-data-']")).forEach((item) => {
        ...
        function getDOMAttrOriginUrl(value) {
          /* 解析DOM节点上隐藏在属性中的真正url */
          let url = null;
          let dataLog = value.getAttribute("data-log");
          try {
          ...
          if (!url || url === "") {
            let dataIVK = value.getAttribute("data-ivk");
            try {
        ...
        function addCSDNFlag(DOM) {
          if (DOM.querySelector(".csdn-flag-component-box") != null) {
            return;
          }
        ...
        function removeElement() {
          ...
          Array.from($(".c-result.result")).forEach((items, index) => {
            let dataLog = JSON.parse(items.dataset.log);
            ...
            let bottom_logo_text =
              items.getElementsByClassName("c-color-source"); //底部标识
          ...
          Array.from($("span")).forEach((items, index) => {
            let result_parent = items.parentElement.parentElement;
            if (
              items.outerText.match(/百度APP内打开/) ||
        ...
        function replaceLink() {
          ...
          Array.from(searchWebSitePages).forEach((items, index) => {
            let realLinkUrl = getDOMAttrOriginUrl(items);
            if (!realLinkUrl) {
              return;
            }
            let articleElement = items.getElementsByTagName("article")[0];
            if (!articleElement) {
              return;
            }
            ...
            if (realLinkUrl.match(/http:\/\/www.internal.video.baidu.com/g)) {
              ...
              if (internalVideoMatch) {
                ...
                replaceElementAttrHref(items, newinternalVideo);
              ...
              replaceElementAttrHref(items, realLinkUrl);
              articleElement.setAttribute("rl-link-href", realLinkUrl);
            }

            if (!realLinkUrl.match(/http(s|):\/\/m.baidu.com\/from/g)) {
              if (!GM_getValue("menu_showisdirect", false)) {
                return;
              }
              if ($(items).find(".white-bdsearch-isredirecrt").length == 0) {
        ...
        let real_link = getRealLinkJson();
        removeElement();
        replaceLink();
      }
      function checkHrefTag() {
        /* 检查链接中是否还存在没有修改完毕的链接 */
        let ret = true;
        document.querySelectorAll(".c-result.result").forEach((item, index) => {
          item.querySelectorAll("a").forEach((tagItem, index2) => {
            var url = tagItem.getAttribute("href");
            if (!url) {
              return;
            }
            if (url.match(/http(s|):\/\/m.baidu.com\/from/g)) {
              ...
              /* __console__.log("%c[BaiDu优化%c-%c百度搜索%c]%c 发现存在没有替换成功的链接: %s", 
                                        ...
                                        "color:red",
                                        url); */
              item.querySelector(".white-bdsearch-isredirecrt")?.remove();
            }
          });
        });
        return ret;
