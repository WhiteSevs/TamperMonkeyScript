...
// @require      https://fastly.jsdelivr.net/npm/@whitesev/utils@2.4.6/dist/index.umd.js
...
(function (Qmsg, DOMUtils, Utils, pops) {
  ...
  const parseM3U8Info = (durationText, fileNameText, currentDuration) => {
  ...
  const M3U8Filter = {
     ...
     */
    filterAdsWithFileNameLength(m3u8Text, consoleLog) {
      ...
      if (needFilterSegments.length) {
        ...
        for (let index = 0; index < m3u8Split.length; index++) {
          ...
          if (findIndex != -1) {
            ...
            consoleLog && log.info(
              `通杀1：过滤广告片段 ==> 索引：${index + indexOffset} 文件名：${adsSegmentInfo.data.fileName} 开始：${CommonUtils.duration2Text(
     ...
     * @param consoleLog 是否输出日志
     */
    filterAdsWithFileNameSimilar(m3u8Text, consoleLog) {
      let m3u8Split = m3u8Text.split("\n");
      ...
      for (let index = 0; index < segmentsParseInfoList.length; index++) {
        ...
        for (let iteratorIndex = 0; iteratorIndex < iteratorSegmentsParseInfoList.length; iteratorIndex++) {
          ...
          if (similarCount / iteratorSegmentsParseInfoList.length > includePercent) {
            isAdsSegment = false;
            break;
          }
        }
        if (isAdsSegment) {
          ...
          consoleLog && log.info(
            `通杀2：过滤广告片段 ==> 索引：${segmentInfo.index} 文件名：${segmentInfo.fileName} 开始：${CommonUtils.duration2Text(
        ...
        itemControls: {
          ...
          edit: {
            ...
            getView: (data, isEdit) => {
              ...
              let $data_commonFilterAdsSegmentsFileNameLength = popsPanelContentUtils.createSectionContainerItem_switch(
              ...
              let $data_commonFilterAdsSegmentsFileNameSimilar = popsPanelContentUtils.createSectionContainerItem_switch(
      ...
      for (let index = 0; index < this.$data.matchedRule.length; index++) {
        try {
          ...
          if (RuleOptionData.commonFilterAdsSegmentsFileNameLength) {
            const { filterInfo, m3u8Text: __handler_m3u8_text__ } = M3U8Filter.filterAdsWithFileNameLength(handlerM3U8Text, true);
            handlerM3U8Text = __handler_m3u8_text__;
          }
          if (RuleOptionData.commonFilterAdsSegmentsFileNameSimilar) {
            const { filterInfo, m3u8Text: __handler_m3u8_text__ } = M3U8Filter.filterAdsWithFileNameSimilar(handlerM3U8Text, true);
            handlerM3U8Text = __handler_m3u8_text__;
          }
