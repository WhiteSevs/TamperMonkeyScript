...
import type { ComponentOption } from "artplayer";
import {
...
import { ArtPlayer_PLUGIN_QUALITY_KEY, type ArtPlayerPluginQualityResult } from "./artplayer-plugin-quality";
...
class VideoStatistics {
  ...
  constructor(art: typeof Artplayer.prototype, option: ArtPlayerPluginVideoStatisticsOption) {
    this.art = art;
    this.option = option;
    this.addSetting();
  }
  ...
  addSetting() {
    this.art.setting.add({
      ...
      html: "视频统计信息",
      mounted: ($setting) => {
        let $leftIcon = $setting.querySelector<HTMLElement>(".art-setting-item-left-icon")!;
  ...
  getLayerOption(): ComponentOption {
    let mimeType: Required<ArtPlayerPluginVideoStatisticsOption>["data"][0] | undefined,
      audioHost: Required<ArtPlayerPluginVideoStatisticsOption>["data"][0] | undefined,
      audioTime: Required<ArtPlayerPluginVideoStatisticsOption>["data"][0] | undefined,
      ...
      videoDataRate: Required<ArtPlayerPluginVideoStatisticsOption>["data"][0] | undefined,
      audioDataRate: Required<ArtPlayerPluginVideoStatisticsOption>["data"][0] | undefined,
      audioDuration: Required<ArtPlayerPluginVideoStatisticsOption>["data"][0] | undefined;
    // 画质信息
    let qualityPlugin = this.art.plugins[ArtPlayer_PLUGIN_QUALITY_KEY] as ArtPlayerPluginQualityResult | null;
    if (qualityPlugin) {
    ...
    let data: (Required<ArtPlayerPluginVideoStatisticsOption>["data"][0] | undefined)[] = [
      mimeType,
      {
        key: "Player Type",
    ...
    return {
      ...
      mounted: async ($topWrap) => {
        let $close = $topWrap.querySelector<HTMLDivElement>(".art-player-video-statistics-close svg")!;
...
export const artplayerPluginVideoStatistics = (option: ArtPlayerPluginVideoStatisticsOption) => {
  return (art: typeof Artplayer.prototype): ArtPlayerPluginVideoStatisticsResult => {
