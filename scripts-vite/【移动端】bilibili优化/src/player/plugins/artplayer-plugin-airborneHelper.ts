...
import { ArtPlayer_PLUGIN_TOAST_KEY, type ArtPlayerPluginToastResult } from "./artplayer-plugin-toast";
...
const AirborneHelperEvent = {
  $data: {
    ...
    tipJumpToastInfo: void 0 as undefined | ReturnType<ArtPlayerPluginToastResult["toast"]>,
    successJumpToastInfo: void 0 as undefined | ReturnType<ArtPlayerPluginToastResult["toast"]>,
  },
  $event: {
    "video:timeupdate": () => {
      ...
      if (findIndex !== -1) {
        ...
        let plugin_toast = AirborneHelper.$data.art.plugins[ArtPlayer_PLUGIN_TOAST_KEY] as ArtPlayerPluginToastResult;
  ...
  bind() {
    Object.keys(this.$event).forEach((eventName) => {
      AirborneHelper.$data.art.on(eventName as keyof Events, (this.$event as any)[eventName as keyof Events]);
    });
  },
  unbind() {
    Object.keys(this.$event).forEach((eventName) => {
      AirborneHelper.$data.art.off(eventName as keyof Events, (this.$event as any)[eventName as keyof Events]);
    });
    clearTimeout(AirborneHelperEvent.$data.tipJumpToastTimeoutId);
    AirborneHelperEvent.$data.tipJumpToastTimeoutId = void 0;
    if (AirborneHelperEvent.$data.successJumpToastInfo) {
...
export const artplayerPluginAirborneHelper = (option: ArtPlayerPluginAirborneHelperOption) => {
...
export const ArtPlayer_PLUGIN_AIRBORNE_HELPER_KEY = AirborneHelper.$key.plugin_KEY;
