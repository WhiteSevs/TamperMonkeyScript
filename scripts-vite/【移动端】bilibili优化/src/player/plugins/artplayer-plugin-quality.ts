...
class VideoEncoding {
	...
	getUserChooseVideoCodingCode() {
		// 遍历视频
		let codingCode =
			(this.art.storage.get(this.storageVideoCodingKey) as number) ||
			VideoCodingCodeMap.AV1;

		if (!Object.values(VideoCodingCodeMap).includes(codingCode)) {
			console.error(
				TAG +
					"意外情况，选择的编码格式不是允许的编码，将强制使用默认(av1)，防止过滤掉的视频链接为空：" +
					codingCode
			);
...
class VideoQuality extends VideoEncoding {
	$data = {
		...
		/** 当前选中的画质配置  */
		currentQualityOption: null as any as
			| ArtPlayerPluginQualityOption["qualityList"]["0"]
			| null,
	};
	constructor(art: Artplayer, from: string) {
	...
	getQualityInfo() {
		...
		for (
			let index = 0;
			index < this.$data.qualityOption.qualityList.length;
			index++
		) {
