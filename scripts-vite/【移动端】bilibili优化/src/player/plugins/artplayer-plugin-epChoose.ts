...
import type { Events, Setting, SettingOption } from "artplayer";
 ...
 */
export const GenerateArtPlayerEpTitle = (title: string, title_id?: string | number | null | undefined) => {
 ...
 * 生成播放器的选集菜单配置项
 */
export const GenerateArtPlayerEpSetting = (option: ArtPlayerPluginEpChooseOption): Setting => {
  let tooltip = "";
...
const EpChooseEvent = {
  ...
  bind(art: Artplayer) {
    Object.keys(this.$event).forEach((eventName) => {
      art.on(eventName as any, (this.$event as any)[eventName as keyof typeof this.$event]);
    });
  },
  unbind(art: Artplayer) {
    Object.keys(this.$event).forEach((eventName) => {
      art.off(eventName as any, (this.$event as any)[eventName as keyof typeof this.$event]);
    });
  },
};
 ...
 * @param option
 */
export const artplayerPluginEpChoose = (option: ArtPlayerPluginEpChooseOption) => {
