...
import type { Selector } from "artplayer";
...
export type ArtPlayerPluginM4SAudioSupportResult = {
  ...
  /** 获取当前播放的音频信息 */
  getCurrentPlayConfig(): ArtPlayerPluginM4SAudioSupportOption["audioList"]["0"] | undefined;
};
...
const M4SAudio = {
  ...
  audioEvents: {
    ...
    canplaythrough: (event) => {
      ...
      console.log(TAG + "浏览器估计该音频可以在不停止内容缓冲的情况下播放媒体直到结束");
      // 同步进度
      M4SAudioUtils.intervalHandler(() => {
        M4SAudio.handler.syncTime();
      });
    ...
    error: (event) => {
      ...
      if (M4SAudio.$data.reconnectInfo.count < M4SAudio.$data.reconnectConfig.maxCount) {
        // 还在允许重新连接次数的范围之内
        console.log(TAG + `Audio第${M4SAudio.$data.reconnectInfo.count + 1}次尝试重新连接`);
        // 左上角notice提示
    ...
    [key in keyof HTMLMediaElementEventMap]?: (ev: HTMLMediaElementEventMap[key]) => any;
  },
  /**
   * 音频工具处理
   */
  handler: {
    ...
    syncTime(offset: number = 0.1) {
      ...
      if (Math.abs(videoTime - audioTime) >= Math.abs(offset)) {
        ...
        this.syncMuted();
        if (import.meta.hot) {
          console.log(TAG + `同步进度，视频：${videoTime} 音频：${audioTime} 差距：${videoTime - audioTime}`);
        }
        M4SAudio.$data.art.emit(
          // @ts-ignore
  ...
  update(option: ArtPlayerPluginM4SAudioSupportUpdateOption) {
    ...
    if (option.audioList?.length) {
      ...
      const storageAudioInfo = this.$data.art.storage.get(storageKey) as ArtPlayerPluginM4SAudioSupportStorageOption;
      ...
      if (storageAudioInfo) {
        ...
        if (findAudioIndex !== -1) {
          ...
          console.warn(TAG + "没有找到上次选的音频代码，使用当前默认第一个音频");
        }
      }

      // 调整一下select的default的值
      let selectorList: (Selector & ArtPlayerPluginM4SAudioSupportOption["audioList"]["0"])[] = option.audioList.map(
        (item, index) => {
          return {
      ...
      const settingOption = {
        ...
        onSelect: function (selector) {
          let itemInfo = selector as any as Selector & ArtPlayerPluginM4SAudioSupportOption["audioList"]["0"];
          // 切换音频
  ...
  bind() {
    Object.keys(this.events).forEach((eventName) => {
      this.$data.art.on(eventName as keyof Events, (this.events as any)[eventName as keyof Events]);
    });
  },
  /**
   * 绑定音频事件
  ...
  bindAudio() {
    Object.keys(this.audioEvents).forEach((eventName) => {
      this.$data.audio.addEventListener(eventName, (this.audioEvents as any)[eventName], {
        once: true,
      });
    });
  },
  ...
  unbind() {
    Object.keys(this.events).forEach((eventName) => {
      this.$data.art.off(eventName as keyof Events, (this.events as any)[eventName as keyof Events]);
    });
  },
  /**
   * 取消绑定音频事件
  ...
  unbindAudio() {
    Object.keys(this.audioEvents).forEach((eventName) => {
      this.$data.audio.removeEventListener(eventName, (this.audioEvents as any)[eventName]);
    });
  },
};
if (import.meta.hot) {
 ...
 * m4s视频的音频支持
 */
export const artplayerPluginM4SAudioSupport = (option: ArtPlayerPluginM4SAudioSupportOption) => {
