    ...
    await new Promise((resolve) => {
      DOMUtils.wait(() => {
        if (videoRE.test(global.location.href)) {
          ...
          commentType = global?.__INITIAL_STATE__?.opus?.detail?.basic?.comment_type; // should be '11'
        }
  ...
  function getConvertedMessage(content) {
    ...
    if (Object.keys(content.jump_url).length) {
      ...
      for (const [key, value] of entries) {
        ...
        const linkElementHTML = `<img class="icon normal" src="${value.prefix_icon}" style="${
