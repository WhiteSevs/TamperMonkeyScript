...
export const BilibiliComponentDetection = {
	...
	async queryUserInfo(mid: number | string) {
		...
		while (true) {
			...
			if (
				followingData.list.length === followingData.total &&
				followingPN === 1
			) {
				// 只有一页数据
				// 已获取到的数量是少于等于总数量
		...
		allSpaceContentData.forEach((spaceData) => {
			if (spaceData.orig == null) {
				...
				let contentInfo: (typeof result.space)[0]["contentInfo"] = {
					...
					desc:
						spaceData.modules.module_dynamic.major?.archive?.desc ||
						spaceData.modules.module_dynamic.desc?.text,
				...
				let forwardInfo: (typeof result.space)[0]["forwardInfo"] = {
					...
					title:
						// 转发的内容的标题
						spaceData.orig.modules.module_dynamic?.major?.archive?.title ||
						null,
					desc:
	...
	handleShowLabel(
		...
		BilibiliComponentDetectionRule.$data.ruleData.forEach((ruleData) => {
			if (
				...
				ruleData.data.blacklist.find((it) => it.toString() === mid)
			) {
				// 黑名单中存在符合的mid
