...
import { BilibiliVideoPlayer, GenerateArtPlayerOption, type VideoInfo } from "../video/BilibiliVideoPlayer";
  ...
  updateArtPlayerVideoInfo(videoInfo?: VideoInfo, isEpChoose?: boolean) {
    ...
    VueUtils.waitVuePropToSet(BilibiliData.className.playlist + " .playlist-player", {
      ...
      async set(vueInstance) {
        ...
        let $player = $<HTMLElement>(BilibiliData.className.playlist + " .playlist-player")!;
        let $playerContainer = $<HTMLElement>(BilibiliData.className.playlist)!;
        ...
        if (that.$data.art == null) {
          ...
          that.$data.art.on("video:ended", () => {
            ...
            let $controlPanel = $<HTMLElement>(BilibiliData.className.playlist + " .control-panel");
            if (!$controlPanel) {
            ...
            if (videoIndex >= mediaList.length - 1) {
              ...
              let $currentVideoCard = $<HTMLElement>(`.video-card[index="${videoIndex}"]`)!;
              ...
              if (p >= currentVideoCardVueInstance.video.page) {
                // 切换到下一个视频卡片
                let $nextVideoCard = $<HTMLElement>(`.video-card[index="${videoIndex + 1}"]`)!;
