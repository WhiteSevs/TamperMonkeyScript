...
export const BilibiliExtraSearch = {
  ...
  createSearchResultVideoItem(option: BilibiliSearchBangumiResultEntity) {
    ...
    let $displayInfo = $item.querySelector<HTMLElement>(".gm-card-display-info")!;
    ...
    epsList.forEach((epsItem) => {
      ...
      // 右上角标志
      let $epBadges = $epItem.querySelector<HTMLElement>(".gm-card-ep-badges-container")!;
      // 番剧集数
      let $epInfo = $epItem.querySelector<HTMLElement>(".gm-card-ep-info-container")!;
