...
export const BilibiliHead = {
  ...
  addVideoListUPInfo() {
    ...
    DOMUtils.waitNode<HTMLDivElement>(BilibiliData.className.head + " .video-list .card-box").then(() => {
      let lockFunc = new utils.LockFunction(() => {
        $$<HTMLDivElement>(BilibiliData.className.head + " .video-list .card-box .v-card").forEach(($vcard) => {
          let vueObj = VueUtils.getVue($vcard);
          let upName = (vueObj?.info?.author?.name || vueObj?.info?.owner?.name) as string | undefined;
          let duration = vueObj?.info?.duration as string | number | undefined;
  ...
  async reconfigurationTinyAppSettingButton() {
    ...
    /** 头像元素 */
    let $gmFace = $<HTMLDivElement>(".gm-face")!;
    /** 头像图片元素 */
    ...
    VueUtils.waitVuePropToSet("#app", [
      {
        ...
        set(vueIns) {
          let userInfo = vueIns?.$store?.state?.common?.userInfo as BilibiliUserInfoType;
