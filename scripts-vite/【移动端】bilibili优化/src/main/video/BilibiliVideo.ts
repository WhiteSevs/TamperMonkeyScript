    ...
    DOMUtils.waitNode<HTMLDivElement>(BilibiliData.className.video + " .bottom-tab .list-view .card-box", 10000).then(
      ($cardBox) => {
        ...
        function handleVCardToApp($vCard: HTMLDivElement) {
          ...
          // 左边的播放量信息
          let $originLeft = $vCard.querySelector<HTMLDivElement>(".count .left");
          // 是否已经处理过
          ...
          if ($originTitle && $originLeft && vueObj && !isHandled) {
            ...
            // 视频封面右上角的App文字
            let $isOpenAppWeakened = $vCard.querySelector<HTMLDivElement>(".open-app.weakened");
        ...
        let $descWrapper = DOMUtils.createElement("div", {
          ...
          innerHTML: /*html*/ `
							<div class="video-view-info-wrapper">
								...
								<span class="video-info-text">${utils.formatTime(info.ctime * 1000, "yyyy年MM月dd日 HH:mm:ss")}</span>
							</div>
