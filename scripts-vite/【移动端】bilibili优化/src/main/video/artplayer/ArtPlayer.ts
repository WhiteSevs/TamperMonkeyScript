 ...
 * @param option
 */
const generateVideoSelectSetting = (option: BilibiliVideoArtPlayerOption): ArtPlayerPluginEpChooseOption["EP_LIST"] => {
  // 如果选集只有1集
  // 那么从分p中进行选取
...
export const BilibiliVideoArtPlayer = {
  ...
  updatePluginInfo(art: Artplayer, option: BilibiliVideoArtPlayerOption) {
    // 更新画质
    let plugin_quality = art.plugins[ArtPlayer_PLUGIN_QUALITY_KEY] as ArtPlayerPluginQualityResult;
    ...
    if (Panel.getValue("artplayer-plugin-video-epChoose-enable")) {
      // 更新选集信息
      let plugin_epChoose = art.plugins[ArtPlayer_PLUGIN_EP_CHOOSE_KEY] as ArtPlayerPluginEpChooseResult;
    ...
    if (Panel.getValue("artplayer-plugin-video-toptoolbar-enable")) {
      // 更新顶部标题
      // 更新顶部在线观看人数来源
      let plugin_topToolBar = art.plugins[ArtPlayer_PLUGIN_TOP_TOOLBAR_KEY] as ArtPlayerPluginTopToolBarResult;
      // 更新数据
