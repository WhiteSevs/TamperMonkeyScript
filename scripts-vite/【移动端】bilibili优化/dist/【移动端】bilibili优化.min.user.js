...
(function (w, Et, pe, We, Bt, nt, Dt, lr, Je) {
	'use strict';
...
`)}
      ...
      `,l.appendChild(B),h[b.rpid_str]=true;const E=B.querySelector(".preview-image-container");E&&new lr(E,{title:false,toolbar:false,tooltip:false,keyboard:false});const F=B.querySelector(".sub-reply-list"),T=B.querySelector(".view-more-btn");T&&T.addEventListener("click",()=>X(b.rpid,F,b.rcount,1));}function S(b){const A=new Date(b*1e3),B=A.getFullYear(),E=(A.getMonth()+1).toString().padStart(2,"0"),F=A.getDate().toString().padStart(2,"0"),T=A.getHours().toString().padStart(2,"0"),q=A.getMinutes().toString().padStart(2,"0");return `${B}-${E}-${F} ${T}:${q}`}function D(b){return {0:"#C0C0C0",1:"#BBBBBB",2:"#8BD29B",3:"#7BCDEF",4:"#FEBB8B",5:"#EE672A",6:"#F04C49"}[b]}function O(b){let A=b.message;const B=["https://www.bilibili.com/video/av","https://b23.tv/mall-"];if(b.vote&&b.vote.deleted===false){const E=`<a class="jump-link normal" href="${b.vote.url}" target="_blank" noopener noreferrer>${b.vote.title}</a>`;B.push(E),A=A.replace(`{vote:${b.vote.id}}`,E);}if(b.emote)for(const[E,F]of Object.entries(b.emote)){const T=`<img class="emoji-${["","small","large"][F.meta.size]}" src="${F.url}" alt="${E}">`;B.push(T),A=A.replaceAll(E,T);}if(A=A.replaceAll(/(\d{1,2}[:：]){1,2}\d{1,2}/g,E=>{if(E=E.replaceAll("：",":"),!t.test(e.location.href))return E;const F=E.split(":");if(F.some(K=>parseInt(K)>=60))return E;let T;if(F.length===2?T=parseInt(F[0])*60+parseInt(F[1]):F.length===3&&(T=parseInt(F[0])*3600+parseInt(F[1])*60+parseInt(F[2])),Number.isNaN(T))return E;const q=`<a class="jump-link video-time" onclick="(async () => {
        ...
        <div class="view-more-pagination">
          ...
          ${(()=>{const q=[E-4,E-3,E-2,E-1].filter(ne=>ne>=1),K=[E+1,E+2,E+3,E+4].filter(ne=>ne<=F),_e=[].concat(q,E,K);let Oe;E<=3?Oe=_e.slice(0,5):E>=F-3?Oe=_e.reverse().slice(0,5).reverse():Oe=_e.slice(_e.indexOf(E)-2,_e.indexOf(E)+3);let de=JSON.parse(JSON.stringify(Oe));if(!de.includes(1)){let ne=[1];de.at(0)!==2&&(ne=[1,"..."]),de=[].concat(ne,de);}if(!de.includes(F)){let ne=[F];de.at(-1)!==F-1&&(ne=["...",F]),de=[].concat(de,ne);}return de.reduce((ne,ze)=>ze==="..."?ne+'<span class="pagination-page-dot">...</span>':ze===E?ne+`<span class="pagination-page-number current-page">${ze}</span>`:ne+`<span class="pagination-page-number">${ze}</span>`,"")})()}
...
`).map(P=>Number(P)).filter(P=>!isNaN(P))),n.data[re]=z;}});let ae=e.createSectionContainerItem_textarea(X);return a.append(s,m,x,$,I,S,O,W,ae),a},onsubmit:(n,i,a)=>{let o=n.querySelectorAll(".rule-form-ulist > li"),l=this.getTemplateData();i&&(l.uuid=a.uuid);try{return o.forEach(s=>{let c=Reflect.get(s,"__formConfig__"),m=Reflect.get(c,"attributes"),h=Reflect.get(s,j),x=Reflect.get(m,we),C=Reflect.get(m,Ae),$=h.get(x,C);Reflect.has(l,x)?Reflect.set(l,x,$):Reflect.has(l.data,x)?Reflect.set(l.data,x,$):u.error(`${x}不在数据中`);}),l.name.trim()===""?(w.error("规则名称不能为空"),{success:!1,data:l}):i?{success:this.updateData(l),data:l}:{success:this.addData(l),data:l}}catch(s){return u.error(s),{success:false,data:l}}finally{this.init();}},style:`
                    ...
                    `},delete:{enable:true,deleteCallBack:n=>this.deleteData(n)}}}).showView();},getTemplateData(){return {uuid:p.generateUUID(),enable:true,name:"",data:{isShowDisplayIcon:true,displayIcon:"",isShowDisplayName:true,displayName:"",keywords:[],blacklist:[],followings:[]}}},getData(){return Re(this.$key.STORAGE_KEY,[])},setData(e){Ie(this.$key.STORAGE_KEY,e);},addData(e){let t=this.getData();return t.findIndex(n=>n.uuid==e.uuid)===-1?(t.push(e),Ie(this.$key.STORAGE_KEY,t),true):false},updateData(e){let t=this.getData(),r=t.findIndex(i=>i.uuid==e.uuid),n=false;return r!==-1&&(n=true,t[r]=e),this.setData(t),n},deleteData(e){let t=this.getData(),r=t.findIndex(i=>i.uuid==e.uuid),n=false;return r!==-1&&(n=true,t.splice(r,1)),this.setData(t),n},clearData(){kt(this.$key.STORAGE_KEY);},exportRule(e="rule.json"){let t=this.getData(),r=new Blob([JSON.stringify(t,null,4)]),n=window.URL.createObjectURL(r),i=document.createElement("a");i.href=n,i.download=e,i.click(),setTimeout(()=>{window.URL.revokeObjectURL(n);},1500);},importRule(){let e=Q.alert({title:{text:"请选择导入方式",position:"center"},content:{text:`
