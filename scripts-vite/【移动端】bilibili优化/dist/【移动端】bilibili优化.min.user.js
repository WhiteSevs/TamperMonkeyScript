...
`).map(q=>Number(q)).filter(q=>!isNaN(q))),n.data[G]=I;}});let ot=t.createSectionContainerItem_textarea(Y);return a.append(s,c,g,B,L,$,A,U,ot),a},onsubmit:(n,i,a)=>{let o=n.querySelectorAll(".rule-form-ulist > li"),l=this.getTemplateData();i&&(l.uuid=a.uuid);try{return o.forEach(s=>{let p=Reflect.get(s,"__formConfig__"),c=Reflect.get(p,"attributes"),m=Reflect.get(s,H),g=Reflect.get(c,Dt),C=Reflect.get(c,kt),B=m.get(g,C);Reflect.has(l,g)?Reflect.set(l,g,B):Reflect.has(l.data,g)?Reflect.set(l.data,g,B):u.error(`${g}不在数据中`);}),l.name.trim()===""?(k.error("规则名称不能为空"),{success:!1,data:l}):i?{success:this.updateData(l),data:l}:{success:this.addData(l),data:l}}catch(s){return u.error(s),{success:false,data:l}}finally{this.init();}},style:`
                    ...
                    `},delete:{enable:true,deleteCallBack:n=>this.deleteData(n)}}}).showView();},getTemplateData(){return {uuid:d.generateUUID(),enable:true,name:"",data:{isShowDisplayIcon:true,displayIcon:"",isShowDisplayName:true,displayName:"",keywords:[],blacklist:[],followings:[]}}},getData(){return Et(this.$key.STORAGE_KEY,[])},setData(t){Rt(this.$key.STORAGE_KEY,t);},addData(t){let e=this.getData();return e.findIndex(n=>n.uuid==t.uuid)===-1?(e.push(t),Rt(this.$key.STORAGE_KEY,e),true):false},updateData(t){let e=this.getData(),r=e.findIndex(i=>i.uuid==t.uuid),n=false;return r!==-1&&(n=true,e[r]=t),this.setData(e),n},deleteData(t){let e=this.getData(),r=e.findIndex(i=>i.uuid==t.uuid),n=false;return r!==-1&&(n=true,e.splice(r,1)),this.setData(e),n},clearData(){ur(this.$key.STORAGE_KEY);},exportRule(t="rule.json"){let e=this.getData(),r=new Blob([JSON.stringify(e,null,4)]),n=window.URL.createObjectURL(r),i=document.createElement("a");i.href=n,i.download=t,i.click(),setTimeout(()=>{window.URL.revokeObjectURL(n);},1500);},importRule(){let t=Z.alert({title:{text:"请选择导入方式",position:"center"},content:{text:`
...
`)}
          ...
          <div class="content-warp">
            ...
            <div class="root-reply">
              ...
              <div class="reply-tag-list">
                ${y.card_label?y.card_label.reduce((j,W)=>j+`<span class="reply-tag-item ${W.text_content==="热评"?"reply-tag-hot":""} ${W.text_content==="UP主觉得很赞"?"reply-tag-liked":""}" style="font-size: 12px; background-color: ${W.label_color_day}; color: ${W.text_color_day};">${W.text_content}</span>`,""):""}
              </div>
            </div>
          </div>
        </div>
      ...
      `,l.appendChild(D),m[y.rpid_str]=true;const F=D.querySelector(".preview-image-container");F&&new lr(F,{title:false,toolbar:false,tooltip:false,keyboard:false});const S=D.querySelector(".sub-reply-list"),R=D.querySelector(".view-more-btn");R&&R.addEventListener("click",()=>Y(y.rpid,S,y.rcount,1));}function $(y){const E=new Date(y*1e3),D=E.getFullYear(),F=(E.getMonth()+1).toString().padStart(2,"0"),S=E.getDate().toString().padStart(2,"0"),R=E.getHours().toString().padStart(2,"0"),j=E.getMinutes().toString().padStart(2,"0");return `${D}-${F}-${S} ${R}:${j}`}function v(y){return {0:"#C0C0C0",1:"#BBBBBB",2:"#8BD29B",3:"#7BCDEF",4:"#FEBB8B",5:"#EE672A",6:"#F04C49"}[y]}function A(y){let E=y.message;const D=["https://www.bilibili.com/video/av","https://b23.tv/mall-"];if(y.vote&&y.vote.deleted===false){const F=`<a class="jump-link normal" href="${y.vote.url}" target="_blank" noopener noreferrer>${y.vote.title}</a>`;D.push(F),E=E.replace(`{vote:${y.vote.id}}`,F);}if(y.emote)for(const[F,S]of Object.entries(y.emote)){const R=`<img class="emoji-${["","small","large"][S.meta.size]}" src="${S.url}" alt="${F}">`;D.push(R),E=E.replaceAll(F,R);}if(E=E.replaceAll(/(\d{1,2}[:：]){1,2}\d{1,2}/g,F=>{if(F=F.replaceAll("：",":"),!e.test(t.location.href))return F;const S=F.split(":");if(S.some(W=>parseInt(W)>=60))return F;let R;if(S.length===2?R=parseInt(S[0])*60+parseInt(S[1]):S.length===3&&(R=parseInt(S[0])*3600+parseInt(S[1])*60+parseInt(S[2])),Number.isNaN(R))return F;const j=`<a class="jump-link video-time" onclick="(async () => {
        ...
        })()">${F}</a>`;return D.push(j),j}),y.at_name_to_mid)for(const[F,S]of Object.entries(y.at_name_to_mid)){const R=`<a class="jump-link user" data-user-id="${S}" href="https://space.bilibili.com/${S}" target="_blank" noopener noreferrer>@${F}</a>`;D.push(R),E=E.replaceAll(`@${F}`,R);}if(Object.keys(y.jump_url).length){const F=[].concat(Object.entries(y.jump_url).filter(S=>S[0].startsWith("https://")),Object.entries(y.jump_url).filter(S=>!S[0].startsWith("https://")));for(const[S,R]of F){const j=S.startsWith("BV")||/^av\d+$/.test(S)?`https://www.bilibili.com/video/${S}`:R.pc_url||S;if(j.includes("search.bilibili.com")&&D.join("").includes(S))continue;const W=`<img class="icon normal" src="${R.prefix_icon}" style="${R.extra&&R.extra.is_word_search&&"width: 12px;"}"><a class="jump-link normal" href="${j}" target="_blank" noopener noreferrer>${R.title}</a>`;D.push(W),E=E.replaceAll(S,W);}}return E}function N(y){let E="width: 84px; height: 84px;";y.length===1&&(E="max-width: 260px; max-height: 180px;"),y.length===2&&(E="width: 128px; height: 128px;");let D="";for(const F of y)D+=`<div class="image-item-wrap" style="margin-top: 4px; margin-right: 4px; cursor: zoom-in;"><img src="${F.img_src}" style="border-radius: 4px; ${E}"></div>`;return D}function U(y){if(!(y instanceof Array))return "";let E="";for(const D of y)E+=`
...
`)}
			...
			`)),f.onceExec("listenRouterChange",()=>{this.listenRouterChange();}),f.execMenuOnce("bili-hookSetTimeout_autoOpenApp",()=>{u.info("hook  window.setTimeout autoOpenApp"),Vt.setTimeout("autoOpenApp"),Vt.setTimeout("bilibili://"),Vt.setTimeout("void 0 !== y && document[y]");}),f.execMenuOnce("bili-overrideLaunchAppBtn_Vue_openApp",()=>{u.info("覆盖元素.launch-app-btn上的openApp"),Vt.overRideLaunchAppBtn_Vue_openApp();}),f.execMenuOnce("bili-cover-bili-open-app-open",()=>{u.info("覆盖元素bili-open-app上的opener.open"),Vt.overRideBiliOpenApp();}),f.execMenuOnce("bili-head-beautify",()=>(u.info("添加美化CSS"),P($r))),f.execMenuOnce("bili-componentDetection",()=>{xn.init();}),Q.isVideo()?(u.info("Router: 视频稿件"),Gr.init()):Q.isOpus()?(u.info("Router: 专栏稿件"),un.init()):Cr.isReadMobile()?(u.info("PC-Router: 专栏稿件"),bn.init()):Q.isDynamic()?(u.info("Router: 动态"),sn.init()):Q.isBangumi()?(u.info("Router: 番剧"),tn.init()):Q.isSearch()?(u.info("Router: 搜索"),an.init()):Q.isLive()?(u.info("Router: 直播"),ln.init()):Q.isTopicDetail()?u.info("Router: 话题"):Q.isHead()?(u.info("Router: 首页之类的"),gn.init()):Q.isSpace()?(u.info("Router: 个人空间"),vn.init()):Q.isPlayList()?(u.info("Router: 播放列表"),wn.init()):u.error("该Router暂未适配，可能是首页之类："+window.location.href),h.ready(()=>{f.execMenuOnce("bili-close-wake-app-dialog",()=>this.watchCloseWakeAppDialog());});},listenRouterChange(){T.waitVuePropToSet("#app",{msg:"监听路由变化",check:t=>{var e;return typeof((e=t==null?void 0:t.$router)==null?void 0:e.afterEach)=="function"},set:t=>{u.success("成功设置监听路由变化"),t.$router.beforeHooks.splice(0,0,(e,r,n)=>{if(u.info("路由变化 => 更新前",{to:e,from:r}),e.hash==="#/seeCommentReply"||r.hash==="#/seeCommentReply"){u.info("该路由变化判定为#/seeCommentReply"),n();return}if(f.getValue("bili-repairVueRouter404")&&e.name==="space"){u.info("修复空间跳转404"),window.location.href=e.fullPath;return}if(e.fullPath.startsWith("/video")){if(r.fullPath.startsWith("/video")&&f.getValue("bili-video-forceThisPageToRefreshAndRedirect")){u.info("强制本页刷新"),window.location.href=e.fullPath;return}else if(Q.isHead()&&f.getValue("bili-head-openVideoInNewTab")){u.info("当前是首页，新标签页打开"),window.open(e.fullPath,"_blank");return}}else if(e.fullPath.startsWith("/bangumi")){if(r.fullPath.startsWith("/bangumi")){u.info("番剧 => 番剧"),window.location.href=e.fullPath;return}else if(Q.isHead()&&f.getValue("bili-head-openVideoInNewTab")){u.info("首页 => 番剧"),window.open(e.fullPath,"_blank");return}}n();}),t.$router.afterHooks.splice(0,0,(e,r)=>{if(u.info("路由变化 => 更新后",{to:e,from:r}),e.hash==="#/seeCommentReply"||r.hash==="#/seeCommentReply"){u.info("该路由变化判定为#/seeCommentReply，不重载");return}f.execMenu("bili-listenRouterChange",()=>{nr.init();});});}});},watchCloseWakeAppDialog(){u.info("自动观察并关闭 观看高清流畅视频 的弹窗");let t=new d.LockFunction(()=>{let e=K(".v-dialog");if(e&&h.text(e).includes("观看高清流畅视频")){let r=e.querySelector(".icon-close")||e.querySelector(".btn.cancel");r?(r.click(),u.success("出现【观看高清流畅视频】弹窗，关闭成功！")):(u.error(e),k.error("【观看高清流畅视频】弹窗关闭失败",{consoleLogContent:true}));}});return d.mutationObserver(document,{config:{subtree:true,childList:true},immediate:true,callback:()=>{t.run();}}),gt.addBlockCSS(".open-app-dialog.v-dialog",".v-dialog:has(m-open-app)")}};f.init();nr.init();Z.config.cssText.index+=`
/* bilibili颜色 #FB7299 */
.pops{
    --bili-color: #FB7299;
