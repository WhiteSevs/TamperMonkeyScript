...
(function (A, he, de, Ke, Dt, at, kt, lr, Ze) {
	'use strict';

	const o=new Set;const p = async t=>{o.has(t)||o.add(t);};

	p(' @charset "UTF-8";.m-video2-awaken-btn,.openapp-dialog{display:none!important}.m-head .launch-app-btn.m-nav-openapp,.m-head .launch-app-btn.home-float-openapp,.m-head m-open-app{display:none!important}.m-home .launch-app-btn.home-float-openapp{display:none!important}.m-space .launch-app-btn.m-space-float-openapp,.m-space .launch-app-btn.m-nav-openapp,.m-space m-open-app:has(>.m-fixed-openapp){display:none!important}#app .video .launch-app-btn.m-video-main-launchapp:has([class^=m-video2-awaken]),#app .video .launch-app-btn.m-nav-openapp,#app .video .mplayer-widescreen-callapp,#app .video .launch-app-btn.m-float-openapp,#app .video .m-video-season-panel .launch-app-btn .open-app{display:none!important}#app.LIVE .open-app-btn.bili-btn-warp{display:none!important}#app .m-dynamic .launch-app-btn.m-nav-openapp,#app .m-dynamic .dynamic-float-openapp.dynamic-float-btn,#app .m-dynamic m-open-app:has(>.m-fixed-openapp){display:none!important}#app .m-opus .float-openapp.opus-float-btn,#app .m-opus .v-switcher .launch-app-btn.list-more,#app .m-opus .opus-nav .launch-app-btn.m-nav-openapp,#app .m-opus .m-navbar .m-nav-openapp,#app .m-opus m-open-app.m-open-app.fixed-openapp{display:none!important}#app .topic-detail .launch-app-btn.m-nav-openapp,#app .topic-detail .launch-app-btn.m-topic-float-openapp{display:none!important}#app.main-container bili-open-app.btn-download{display:none!important}#__next m-open-app[class^=TopBar_download],#__next m-open-app:has([class^=GoApp]){display:none!important}#__next m-open-app[class^=MainButton_btnWrap]{visibility:hidden!important}#app .read-app-main bili-open-app{display:none!important}#app .playlist>.open-app-wp{display:none!important}#app .playlist>.open-app-wp+div{padding-top:56.25%}html{--bili-color: #fb7299;--bili-color-rgb: 251, 114, 153} ');
...
`)}
      ...
      `,(document.head||document.documentElement).appendChild(k);}})(),$r=`:root {\r
	--v_xs: 5px;\r
	--v_xsx: 4px;\r
	--v_xxs: 6px;\r
	--v_sm: 10px;\r
...
`,ir={$flag_css:{enableOtherAreaSearchBangumi:false},init(){j(Pr),m.ready(()=>{f.execMenu("bili-search-enableOtherAreaSearchBangumi",()=>{this.enableOtherAreaSearchBangumi();});});},enableOtherAreaSearchBangumi(){this.$flag_css.enableOtherAreaSearchBangumi||(this.$flag_css.enableOtherAreaSearchBangumi=true,j(`
...
`):p},callback(c,p){this.props[J].set(t,p);}};return Reflect.set(s.attributes,De,t),Reflect.set(s.attributes,ke,r),Pe.initComponentsStorageApi("switch",s,{get(c,p){return f.getValue(c,p)},set(c,p){f.setValue(c,p);}}),s};class on{option;constructor(t){this.option=t;}async showView(){let t=Y.confirm({title:{text:this.option.title,position:"center"},content:{text:`
            ...
            `}),r=t.$shadowRoot.querySelector(".filter-container"),n=document.createDocumentFragment();this.option.filterOption.forEach(i=>{let a=m.createElement("button",{innerText:i.name},{type:"button"}),o=async()=>{(await this.option.getAllRuleInfo()).forEach(async s=>{await i.filterCallBack(s.data)?m.show(s.$el,false):m.hide(s.$el,false);}),typeof this.option.execFilterCallBack=="function"&&await this.option.execFilterCallBack(),t.close();};m.on(a,"click",async l=>{d.preventEvent(l),!(typeof i.callback=="function"&&!await i.callback(l,o))&&await o();}),n.appendChild(a);}),r.appendChild(n);}}class un{option;constructor(t){this.option=t;}async showView(t){let r=Y.confirm({title:{text:this.option.title,position:"center"},content:{text:`
            ...
            `}),n=await this.option.data(),i=false;for(let a=0;a<n.length;a++){let o=n[a],l=await this.appendRuleItemElement(r.$shadowRoot,o);(typeof t=="function"?t(o):true)||(i=true,l.forEach(c=>{m.hide(c,false);}));}if(i){let a=r.$shadowRoot.querySelector(".pops-confirm-btn-cancel span");m.text(a,"取消过滤");}}showEditView(t,r,n,i,a,o){let l=async c=>{if(c){if(typeof o=="function"){let p=await this.option.getData(r);o(p);}}else if(t||await this.option.deleteData(r),typeof a=="function"){let p=await this.option.getData(r);a(p);}};new on({title:t?"编辑":"添加",data:()=>r,dialogCloseCallBack:l,getView:async c=>await this.option.itemControls.edit.getView(c,t),btn:{ok:{enable:true,text:t?"修改":"添加"},cancel:{callback:async(c,p)=>{c.close(),await l(false);}},close:{callback:async(c,p)=>{c.close(),await l(false);}}},onsubmit:async(c,p)=>{let h=await this.option.itemControls.edit.onsubmit(c,t,p);return h.success?t?(A.success("修改成功"),n&&await this.updateRuleItemElement(h.data,i,n)):n&&await this.appendRuleItemElement(n,h.data):t&&u.error("修改失败"),h},style:this.option.itemControls.edit.style,width:this.option.itemControls.edit.width,height:this.option.itemControls.edit.height}).showView();}parseViewElement(t){let r=t.querySelector(".rule-view-container"),n=t.querySelector(".pops-confirm-btn button.pops-confirm-btn-other");return {$container:r,$deleteBtn:n}}parseRuleItemElement(t){let r=t.querySelector(".rule-controls-enable"),n=r.querySelector(".pops-panel-switch"),i=r.querySelector(".pops-panel-switch__input"),a=r.querySelector(".pops-panel-switch__core"),o=t.querySelector(".rule-controls-edit"),l=t.querySelector(".rule-controls-delete");return {$enable:r,$enableSwitch:n,$enableSwitchInput:i,$enableSwitchCore:a,$edit:o,$delete:l,data:Reflect.get(t,"data-rule")}}async createRuleItemElement(t,r){let n=await this.option.getDataItemName(t),i=m.createElement("div",{className:"rule-item",innerHTML:`
...
`).map(z=>Number(z)).filter(z=>!isNaN(z))),n.data[P]=V;}});let O=e.createSectionContainerItem_textarea(H);return a.append(s,p,v,_,L,F,R,Q,O),a},onsubmit:(n,i,a)=>{let o=n.querySelectorAll(".rule-form-ulist > li"),l=this.getTemplateData();i&&(l.uuid=a.uuid);try{return o.forEach(s=>{let c=Reflect.get(s,"__formConfig__"),p=Reflect.get(c,"attributes"),h=Reflect.get(s,J),v=Reflect.get(p,De),x=Reflect.get(p,ke),_=h.get(v,x);Reflect.has(l,v)?Reflect.set(l,v,_):Reflect.has(l.data,v)?Reflect.set(l.data,v,_):u.error(`${v}不在数据中`);}),l.name.trim()===""?(A.error("规则名称不能为空"),{success:!1,data:l}):i?{success:this.updateData(l),data:l}:{success:this.addData(l),data:l}}catch(s){return u.error(s),{success:false,data:l}}finally{this.init();}},style:`
                    ...
                    `},delete:{enable:true,deleteCallBack:n=>this.deleteData(n)}}}).showView();},getTemplateData(){return {uuid:d.generateUUID(),enable:true,name:"",data:{isShowDisplayIcon:true,displayIcon:"",isShowDisplayName:true,displayName:"",keywords:[],blacklist:[],followings:[]}}},getData(){return Oe(this.$key.STORAGE_KEY,[])},setData(e){Me(this.$key.STORAGE_KEY,e);},addData(e){let t=this.getData();return t.findIndex(n=>n.uuid==e.uuid)===-1?(t.push(e),Me(this.$key.STORAGE_KEY,t),true):false},updateData(e){let t=this.getData(),r=t.findIndex(i=>i.uuid==e.uuid),n=false;return r!==-1&&(n=true,t[r]=e),this.setData(t),n},deleteData(e){let t=this.getData(),r=t.findIndex(i=>i.uuid==e.uuid),n=false;return r!==-1&&(n=true,t.splice(r,1)),this.setData(t),n},clearData(){Ft(this.$key.STORAGE_KEY);},exportRule(e="rule.json"){let t=this.getData(),r=new Blob([JSON.stringify(t,null,4)]),n=window.URL.createObjectURL(r),i=document.createElement("a");i.href=n,i.download=e,i.click(),setTimeout(()=>{window.URL.revokeObjectURL(n);},1500);},importRule(){let e=Y.alert({title:{text:"请选择导入方式",position:"center"},content:{text:`
