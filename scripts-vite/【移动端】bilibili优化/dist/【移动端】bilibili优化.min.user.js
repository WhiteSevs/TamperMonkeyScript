...
`,De={mergeAidOrBvidSearchParamsData(e,t){if("aid"in t&&t.aid!=null)Reflect.set(e,"aid",t.aid);else if("bvid"in t&&t.bvid!=null)Reflect.set(e,"bvid",t.bvid);else throw new TypeError("avid or bvid must give one")}},we={"240P 极速":6,"360P 流畅":16,"480P 清晰":32,"720P 高清":64,"720P60 高帧率":74,"1080P 高清":80,"1080P+ 高码率":112,"1080P60 高帧率":116,"4K 超清":120,"HDR 真彩色":125,杜比视界:126,"8K 超高清":127},Zt={};Object.keys(we).forEach(e=>{let t=Reflect.get(we,e);Reflect.set(Zt,t,e);});const xe={async playUrl(e,t){let u={cid:e.cid,qn:e.qn??we["1080P60 高帧率"],high_quality:e.high_quality??1,fnval:e.fnval??1,fnver:e.fnver??0,fourk:e.fourk??1};e.setPlatformHTML5&&Reflect.set(u,"platform","html5"),De.mergeAidOrBvidSearchParamsData(u,e),typeof t=="object"&&Object.assign(u,t);let i=await M.get("https://api.bilibili.com/x/player/playurl?"+c.toSearchParamsStr(u),{responseType:"json",fetch:!0});if(!i.status)return;let r=c.toJSON(i.data.responseText);if(r.code===0)return r.data},async onlineTotal(e){let t={cid:e.cid};De.mergeAidOrBvidSearchParamsData(t,e);let u=await M.get(`https://${he.web_host}/x/player/online/total?${c.toSearchParamsStr(t)}`,{responseType:"json",fetch:!0});if(!u.status)return;let i=c.toJSON(u.data.responseText);return H.isWebApiSuccess(i)||o.error(`获取在线观看人数失败: ${JSON.stringify(i)}`),i.data},async like(e){var a;let t={like:e.like,csrf:((a=tt.get("bili_jct"))==null?void 0:a.value)||""};De.mergeAidOrBvidSearchParamsData(t,e);let u=await M.get("https://api.bilibili.com/x/web-interface/archive/like?"+c.toSearchParamsStr(t),{fetch:!0});if(!u.status)return !1;let i=c.toJSON(u.data.responseText);const r=i.code;return r===0?!0:(r===-101?B.error("账号未登录"):r===-111?B.error("csrf校验失败"):r===-400?B.error("请求错误"):r===-403?B.error("账号异常"):r===10003?B.error("不存在该稿件"):r===65004?B.error("取消点赞失败"):r===65006?B.warning("重复点赞"):B.error("未知错误："+i.message),!1)}},Xt={getUserChooseVideoCodingCode(){let e=s.getValue("bili-video-artplayer-videoCodingCode",R.AV1);return Object.values(R).includes(e)||(o.error("意外情况，选择的编码格式不是允许的编码，将强制使用默认，防止过滤掉的视频链接为空："+e),e=R.AV1),e}},at={30216:"64K",30232:"132K",30280:"192K",30250:"杜比全景声",30251:"Hi-Res无损"};class nt{constructor(t){Te(this,"$data",{KEY:"art-player-danmaku-option",localArtDanmakuOption:{}});this.$data.KEY=t;const u=this.getDefaultDanmakuOption();this.$data.localArtDanmakuOption=c.assign(u,Q(this.$data.KEY,{}));}getDefaultDanmakuOption(){return {speed:5,margin:[10,"75%"],opacity:1,modes:[0,1,2],fontSize:18,antiOverlap:!1,synchronousPlayback:!0,visible:!0}}getLocalArtDanmakuOption(){return this.$data.localArtDanmakuOption}repairBrowserNoResponse(t){console.warn("目前尚未知晓导致浏览器卡死的原因是哪里的问题，但是启用该弹幕插件100%复现，复现操作：点击播放，然后重复全屏和退出全屏，拖动进度到弹幕量最多的时间点，过一会卡死");}onConfigChange(t){t.on("artplayerPluginDanmuku:config",u=>{Object.keys(this.$data.localArtDanmakuOption).forEach(i=>{if(Reflect.has(u,i)){let r=Reflect.get(u,i);Reflect.set(this.$data.localArtDanmakuOption,i,r);}}),de(this.$data.KEY,this.$data.localArtDanmakuOption);});}}const j="[artplayer-plugin-bilibiliCCSubTitle]：",ge="setting-bilibili-m4sAudio",ae={$flag:{isIntervaling:!1},intervalHandler(e,t=5,u=555,i=!1){if(i&&(this.$flag.isIntervaling=!1),this.$flag.isIntervaling)return;this.$flag.isIntervaling=!0;let r=1,a=()=>{if(r>t){this.$flag.isIntervaling=!1,clearInterval(n);return}if(typeof e=="function")try{e();}catch(l){console.error(j,l);}r++;};a();let n=setInterval(a,u);}},g={$key:{plugin_KEY:"plugin-bilibili-m4sAudio"},$data:{art:null,audio:new Audio,reconnectConfig:{maxCount:5,delayTime:1e3},reconnectInfo:{url:"",count:0}},userEvent:{onRestart:void 0},events:{play:()=>{g.handler.syncTime(),g.handler.play();},seek:e=>{ae.intervalHandler(()=>{g.handler.syncTime(),g.handler.syncPlayState();},2,800,!0);},pause:()=>{g.handler.syncTime(),g.handler.pause();},restart:e=>{if(typeof g.userEvent.onRestart=="function"){let t=g.userEvent.onRestart(e);g.handler.playUrl(t);}g.handler.syncTime(),g.handler.syncPlayState();},muted:e=>{g.handler.syncVolume(),g.handler.syncMuted();},destroy:()=>{g.handler.pause();},error:(e,t)=>{g.handler.pause();},resize:()=>{ae.intervalHandler(()=>{g.handler.syncTime();});},fullscreen:()=>{ae.intervalHandler(()=>{g.handler.syncTime();});},"video:ended":()=>{g.handler.pause();},"video:ratechange":()=>{g.handler.syncPlayBackRate();},"video:waiting":()=>{g.handler.pause();},"video:playing":()=>{g.handler.syncTime(),g.handler.play();},"video:pause":()=>{g.handler.pause(),g.handler.syncTime();},"video:volumechange":()=>{g.handler.syncTime();},"video:timeupdate":()=>{2<=g.$data.art.currentTime&&g.$data.art.currentTime<=4&&g.handler.syncTime();}},audioEvents:{loadedmetadata:e=>{console.log(j+"Audio预加载完成"),g.$data.reconnectInfo.count=0,g.$data.reconnectInfo.url="",g.handler.syncPlayState(),g.handler.syncMuted(),g.handler.syncPlayBackRate(),g.handler.syncVolume(),ae.intervalHandler(()=>{g.handler.syncTime();});},canplaythrough:e=>{console.log(j+"浏览器估计该音频可以在不停止内容缓冲的情况下播放媒体直到结束"),ae.intervalHandler(()=>{g.handler.syncTime();},void 0,void 0,!0);},error:e=>{console.error(j+"Audio加载失败",e),c.isNull(g.$data.reconnectInfo.url)&&(g.$data.reconnectInfo.url=g.$data.audio.src),g.$data.reconnectInfo.count<g.$data.reconnectConfig.maxCount?(console.log(j+`Audio第${g.$data.reconnectInfo.count+1}次尝试重新连接`),g.$data.art.notice.show=`Audio第${g.$data.reconnectInfo.count+1}次尝试重新连接`,g.$data.reconnectInfo.count++,setTimeout(()=>{g.handler.playUrl(""),g.handler.playUrl(g.$data.reconnectInfo.url),g.$data.audio.load();},g.$data.reconnectConfig.delayTime)):(console.error(j+"Audio已超出重连次数"),g.$data.art.notice.show="Audio已超出重连次数，请尝试切换源");}},handler:{playUrl(e){typeof e=="string"&&(g.$data.audio.src=e,g.unbindAudio(),c.isNotNull(e)&&g.bindAudio());},play(){g.$data.audio.paused&&g.$data.audio.play();},pause(){g.$data.audio.paused||g.$data.audio.pause();},syncPlayState(){g.$data.art.playing?this.play():this.pause();},syncTime(){g.$data.audio.currentTime=g.$data.art.currentTime,this.syncVolume(),this.syncMuted();},syncVolume(){g.$data.audio.volume=g.$data.art.volume;},syncMuted(){let e=g.$data.art.muted,t=g.$data.audio.muted;e!==t&&(g.$data.audio.muted=e);},syncPlayBackRate(){let e=g.$data.art.playbackRate,t=g.$data.audio.playbackRate;e!==t&&(g.$data.audio.playbackRate=e);}},update(e){var u;this.unbind(),this.unbindAudio();const t=this;if((u=e.audioList)!=null&&u.length){let i=e.audioList[0];const r=`artplayer-m4s-audio-${e.from}`,a=this.$data.art.storage.get(r);let n={index:0,html:i.soundQualityCodeText,url:i.url};if(a){const m=e.audioList.findIndex(C=>C.soundQualityCode===a.soundQualityCode);if(m!==-1){const C=e.audioList[m];n.index=m,n.url=C.url,n.html=C.soundQualityCodeText;}else console.warn(j+"没有找到上次选的音频代码，使用当前默认第一个音频");}let l=e.audioList.map((m,C)=>({default:C===n.index,html:m.soundQualityCodeText,url:m.url,soundQualityCode:m.soundQualityCode,soundQualityCodeText:m.soundQualityCodeText}));const d={name:ge,width:200,html:"音频",tooltip:n.html,icon:`
    ...
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 22" width="16" height="16">
        ...
        `));},mutationMPlayerOriginToast(e){let t=this.$el.$originPlayer;t&&(t.hasAttribute("data-mutation")||(o.success("添加观察器，动态更新toast的位置"),t.setAttribute("data-mutation","gm"),c.mutationObserver(t,{config:{subtree:!0,childList:!0},immediate:!0,callback:()=>{this.updatePageToastBottom();}})));},updatePageToastBottom(){let e=Array.from(document.querySelectorAll(`.${this.$config.prefix}`)).concat(Array.from(document.querySelectorAll(".".concat(this.$config.originToast))));e.length&&(e.length-1,e.forEach((t,u)=>{t.setAttribute("data-transition","move"),t.style.bottom=`calc( calc( var(--art-control-height) + var(--art-bottom-gap) ) * ${u+1} + 10px)`;}));},closeToast(e){e.classList.add(this.$config.hideClassName);},getTransitionendEventNameList(){return ["webkitTransitionEnd","mozTransitionEnd","MSTransitionEnd","otransitionend","transitionend"]},setTransitionendEvent(e,t){let u=this,i=this.getTransitionendEventNameList();f.on(e,i,function(r){let a=e.getAttribute("data-transition");if(e.classList.contains(u.$config.hideClassName)){typeof t=="object"&&typeof(t==null?void 0:t.closeCallback)=="function"&&t.closeCallback(),e.remove();return}if(a==="move"){e.removeAttribute("data-transition");return}},{capture:!0});}},vt=e=>t=>(ve.$data.art=t,{name:ve.$key.plugin_KEY,toast(...u){return ve.toast(...u)}}),ru=ve.$key.plugin_KEY,ze=e=>{let t=e.epList||[];if(t.length===1){let u=t[0];return u.pages.map(i=>({isDefault:i.cid===e.cid,title:i.part,aid:e.aid,bvid:e.bvid,cid:i.cid,onSelect(r,a){u.cid=i.cid,X.updateArtPlayerVideoInfo({aid:e.aid,bvid:e.bvid,cid:i.cid,pic:i.first_frame||"",title:i.part,epList:e.epList||[]},!0);}}))}else return t.map(u=>({isDefault:u.aid===e.aid&&u.cid===e.cid,title:gt(u.title),aid:u.aid,bvid:u.bvid,cid:u.cid,onSelect(i,r){X.updateArtPlayerVideoInfo({aid:u.aid,bvid:u.bvid,cid:u.cid,pic:u.arc.pic,title:u.title,epList:e.epList||[]},!0);}}))},ne={$data:{art:null,currentOption:null},resetEnv(e){e&&Reflect.set(this.$data,"art",null),Reflect.set(this.$data,"currentOption",null);},async init(e){this.resetEnv(!0),this.$data.currentOption=e;const t="artplayer-video-danmaku-option",u=new nt(t),i=u.getLocalArtDanmakuOption(),r={...Et(),container:e.container,poster:e.poster,settings:[{name:"video-playback-codeid",html:"播放策略",tooltip:"默认",icon:'<svg t="1727413004405" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3183" width="24" height="24"><path d="M170.666667 256h682.666666c23.466667 0 42.666667 19.2 42.666667 42.666667v170.666666h85.333333V256c0-46.933333-38.4-85.333333-85.333333-85.333333H128c-46.933333 0-85.333333 38.4-85.333333 85.333333v512c0 46.933333 38.4 85.333333 85.333333 85.333333h384v-85.333333H170.666667c-23.466667 0-42.666667-19.2-42.666667-42.666667V298.666667c0-23.466667 19.2-42.666667 42.666667-42.666667z" p-id="3184"></path><path d="M640 512L384 341.333333v341.333334zM968.96 786.346667c1.28-12.373333 1.706667-24.746667 0.426667-36.693334l45.653333-36.266666c4.266667-3.413333 5.12-8.96 2.56-13.653334l-43.946667-76.373333c-2.56-4.693333-8.106667-6.4-13.226666-4.693333l-54.613334 21.333333a146.773333 146.773333 0 0 0-32-17.92l-8.533333-58.026667a10.624 10.624 0 0 0-10.666667-9.386666h-88.32c-5.12 0-9.813333 3.84-10.666666 8.96l-8.533334 58.026666c-11.093333 4.693333-21.76 11.093333-31.573333 17.92l-54.613333-21.333333c-5.12-2.133333-10.666667 0-13.226667 4.693333l-43.946667 76.373334c-2.56 4.693333-1.706667 10.24 2.56 13.653333l45.653334 36.693333c-1.28 12.373333-1.706667 24.746667-0.426667 36.693334l-45.653333 36.266666c-4.266667 3.413333-5.12 8.96-2.56 13.653334l43.946666 76.373333c2.56 4.693333 8.106667 6.4 13.226667 4.693333l54.186667-21.333333c9.813333 7.253333 20.48 13.226667 32 17.92l8.533333 58.026667c0.853333 5.12 5.12 8.96 10.666667 8.96h88.32c5.12 0 9.813333-3.84 10.666666-8.96l8.533334-58.026667c11.093333-4.693333 21.76-11.093333 31.573333-17.92l54.613333 21.333333c5.12 2.133333 10.666667 0 13.226667-4.693333l43.946667-76.373333c2.56-4.693333 1.706667-10.24-2.56-13.653334l-45.226667-36.266666zM810.666667 832c-35.413333 0-64-28.586667-64-64s28.586667-64 64-64 64 28.586667 64 64-28.586667 64-64 64z" p-id="3185"></path></svg>',selector:[{default:!0,html:"默认",value:R.AV1},{html:"AV1",value:R.AV1},{html:"HEVC",value:R.HEVC},{html:"AVC",value:R.AVC}],onSelect:function(a){return s.setValue("bili-bangumi-videoCodingCode",a.value),a.html}}],plugins:[vt(),bt({from:"video",qualityList:e.quality})]};return r.type="mp4",s.getValue("artplayer-plugin-video-danmaku-enable")&&r.plugins.push(Xe({danmuku:e.danmukuUrl,speed:i.speed,margin:i.margin,opacity:i.opacity,color:"#FFFFFF",mode:0,modes:i.modes,fontSize:i.fontSize,antiOverlap:i.antiOverlap,synchronousPlayback:i.synchronousPlayback,mount:void 0,heatmap:!0,width:800,points:[],filter:a=>a.text.length<=100,beforeVisible:()=>!0,visible:i.visible,emitter:!1,maxLength:50,lockTime:3,theme:c.isThemeDark()?"dark":"light",beforeEmit(a){return new Promise(n=>{console.log(a),setTimeout(()=>{n(!0);},1e3);})}})),s.getValue("artplayer-plugin-video-m4sAudioSupport-enable")&&r.plugins.push(ot({from:"video",showSetting:!0,audioList:e.audioList||[]})),s.getValue("artplayer-plugin-video-epChoose-enable")&&r.plugins.push(Ct({EP_LIST:ze(e),automaticBroadcast:!0})),s.getValue("artplayer-plugin-video-cc-subtitle-enable")&&r.plugins.push(mt({from:"video",cid:e.cid,aid:e.aid,bvid:e.bvid})),s.getValue("artplayer-plugin-video-toptoolbar-enable")&&r.plugins.push(st({onlineInfoParams:{aid:e.aid,cid:e.cid,bvid:e.bvid},title:e.videoTitle,showWrap:!0,showTitle:!0,showOnlineTotal:!0})),s.getValue("bili-video-playerAutoPlayVideo")&&(r.muted=!0,r.autoplay=!0),this.$data.art=new Ze(r),s.getValue("artplayer-plugin-video-danmaku-enable")&&u.repairBrowserNoResponse(this.$data.art),u.onConfigChange(this.$data.art),this.$data.art},async update(e,t){this.resetEnv(!1),this.$data.currentOption=t,o.info(["更新新的播放信息",t]),e.pause(),o.info("暂停视频"),e.currentTime=0,o.info("重置播放进度"),this.updatePluginInfo(e,t),e.play(),o.info("播放");},updatePluginInfo(e,t){if(e.plugins[Z].update({from:"video",qualityList:t.quality}),o.info(["更新画质",t.quality]),s.getValue("artplayer-plugin-video-danmaku-enable")&&(e.plugins.artplayerPluginDanmuku.config({danmuku:t.danmukuUrl}),e.plugins.artplayerPluginDanmuku.load(),o.info(["更新弹幕姬",t.danmukuUrl])),s.getValue("artplayer-plugin-video-m4sAudioSupport-enable")&&(e.plugins[lt].update({from:"video",audioList:t.audioList||[]}),o.info(["更新音频",t.audioList])),s.getValue("artplayer-plugin-video-epChoose-enable")&&(e.plugins[At].update({EP_LIST:ze(t),automaticBroadcast:!0}),o.info(["更新选集信息",t.epList])),s.getValue("artplayer-plugin-video-cc-subtitle-enable")){let i=e.plugins[ht];const r={from:"video",aid:t.aid,bvid:t.bvid,cid:t.cid};i.update(r),o.info(["更新字幕",r]);}if(s.getValue("artplayer-plugin-video-toptoolbar-enable")){let i=e.plugins[ct];const r={showRight:!0,showRightFollow:!0,showWrap:!0,showTitle:!0,showOnlineTotal:!0,title:t.videoTitle,onlineInfoParams:{aid:t.aid,cid:t.cid,bvid:t.bvid}};i.update(r),o.info(["更新顶部标题",r]);}}};function au(e){const t={};return e.forEach(i=>{(!t[i.id]||i.size>t[i.id].size)&&(t[i.id]=i);}),Object.values(t)}function Ue(e,t){let u=[];return e.video.forEach(i=>{if(!e.accept_quality.includes(i.id)||t.codecid!=null&&i.codecid!==t.codecid)return;let r=e.support_formats.find(l=>l.quality===i.id),a=L.findBetterCDN(i.base_url,i.baseUrl,i.backup_url,i.backupUrl);a=L.replaceVideoCDN(a);let n=r==null?void 0:r.new_description;u.push({name:n,url:a,type:i.mimeType,id:i.id,quality:i.id,vip:!1});}),u}const nu=async e=>{var a,n;const t=[];let u=[];if(s.getValue("bili-video-playType","mp4")==="mp4"){const l=await xe.playUrl({bvid:e.bvid,cid:e.cid,fnval:1,fnver:0,fourk:1,high_quality:1,qn:127,setPlatformHTML5:!0});if(o.info(l),!l)return;let d=l.durl[0],p=l.support_formats.find(v=>v.quality===l.quality),m=L.findBetterCDN(d.url,d.url||((a=d.backup_url)==null?void 0:a[0])),C=p==null?void 0:p.new_description;u.push({name:C,url:m,type:"audio/mp4",id:l.quality,quality:l.quality,vip:!1});}else {const l=await xe.playUrl({bvid:e.bvid,cid:e.cid,fnval:3088,fnver:0,fourk:1,high_quality:1,qn:127,setPlatformHTML5:!1});if(o.info(l),!l)return;let d=Xt.getUserChooseVideoCodingCode();l.dash.audio.forEach(p=>{let m=L.findBetterCDN(p.baseUrl,p.base_url,p.baseUrl,p.backup_url);s.getValue("bili-video-uposServerSelect-applyAudio")&&(m=L.replaceVideoCDN(m)),t.push({url:m,id:p.id,text:at[p.id]||""});}),t.sort((p,m)=>m.id-p.id),o.info(["ArtPlayer: 获取的音频信息",t]),u=[...Ue({accept_quality:l.accept_quality,support_formats:l.support_formats,video:l.dash.video},{codecid:d})],u.length===0&&l.dash.video.length!==0&&(o.warn(`当前选择的视频编码id为: ${d}，但是过滤出的视频没有一个符合的，所以直接放弃使用自定义选择视频编码`),u=[],u=[...Ue({accept_quality:l.accept_quality,support_formats:l.support_formats,video:l.dash.video},{})]),u=au(u),u.sort((p,m)=>m.quality-p.quality),o.info(["ArtPlayer: 获取的视频画质信息",u]);}const i=u.map((l,d)=>({quality:l.quality,html:l.name,url:l.url})),r={container:null,epList:e.epList,audioUrl:null,url:"",poster:e.pic,aid:e.aid,bvid:e.bvid,cid:e.cid,videoTitle:e.title,danmukuUrl:`https://api.bilibili.com/x/v1/dm/list.so?oid=${e.cid}`,quality:i};return r.url=(n=u==null?void 0:u[0])==null?void 0:n.url,t.length&&(r.audioList=t.map((l,d)=>({isDefault:d===0,url:l.url,soundQualityCode:l.id,soundQualityCodeText:l.text}))),r},X={$data:{art:null},init(){s.execMenu("bili-video-enableArtPlayer",()=>{this.coverVideoPlayer();});},coverVideoPlayer(){if(document.querySelector("#artplayer"))o.warn("已存在播放器，更新播放信息");else {S(`
			...
			`);let e=s.getValue("bili-video-artplayer-controlsPadding-left-right",0);e!=0&&S(`
				...
				`);}this.updateArtPlayerVideoInfo();},updateArtPlayerVideoInfo(e,t){let u=this;F.waitVuePropToSet("#app .video .m-video-player",{msg:"等待m-video-player加载完成",check(i){var r,a,n,l,d,p;return !t&&ne.$data.currentOption!=null?(ne.$data.art.pause(),typeof((r=i==null?void 0:i.info)==null?void 0:r.aid)=="number"&&ne.$data.currentOption.aid!==i.info.aid&&typeof((a=i==null?void 0:i.info)==null?void 0:a.bvid)=="string"&&typeof((n=i==null?void 0:i.info)==null?void 0:n.cid)=="number"):typeof((l=i==null?void 0:i.info)==null?void 0:l.aid)=="number"&&typeof((d=i==null?void 0:i.info)==null?void 0:d.bvid)=="string"&&typeof((p=i==null?void 0:i.info)==null?void 0:p.cid)=="number"},async set(i){var T,_;const r=document.querySelector("#app .video .m-video-player");let{aid:a,bvid:n,cid:l,pic:d,title:p}=i;a=a||i.info.aid,n=n||i.info.bvid,l=l||i.info.cid,d=d||i.info.pic,p=p||i.info.title;let m=[];const C=document.querySelector(".m-video-season-new"),v=document.querySelector(".m-video-part-new");if(C&&F.getVue(C)){let y=F.getVue(C),h=y==null?void 0:y.videoList;Array.isArray(h)&&(m=h);}else if(v&&F.getVue(v)){let y=F.getVue(v),h=y==null?void 0:y.info,O=y==null?void 0:y.p,D=(y==null?void 0:y.pages)||((T=y==null?void 0:y.info)==null?void 0:T.pages);Array.isArray(D)&&m.push({season_id:0,section_id:0,id:0,aid:a||h.aid,bvid:n||h.bvid,cid:l||h.cid,title:p||h.title,attribute:0,arc:{aid:a||h.aid,videos:h==null?void 0:h.videos,type_id:0,type_name:"",copyright:h==null?void 0:h.copyright,pic:h==null?void 0:h.pic,title:h==null?void 0:h.title,pubdate:h==null?void 0:h.pubdate,ctime:h==null?void 0:h.ctime,desc:h==null?void 0:h.desc,state:h==null?void 0:h.state,duration:h==null?void 0:h.duration,rights:h==null?void 0:h.rights,author:h==null?void 0:h.owner,stat:h==null?void 0:h.stat,dynamic:h==null?void 0:h.dynamic,dimension:h==null?void 0:h.dimension,desc_v2:h==null?void 0:h.desc_v2,is_chargeable_season:h==null?void 0:h.is_chargeable_season,is_blooper:h==null?void 0:h.is_blooper,enable_vt:h==null?void 0:h.enable_vt,vt_display:h==null?void 0:h.vt_display},page:(_=h==null?void 0:h.pages)==null?void 0:_[O],pages:h==null?void 0:h.pages});}e==null&&(e={aid:a,bvid:n,cid:l,pic:d,title:p,epList:m}),o.info(`视频播放信息 => aid：${a} bvid：${n} cid：${l}`);const E=await nu(e);if(E==null)return;let b=document.querySelector("#artplayer");if(!b){const y=f.createElement("div",{className:"artplayer-container",innerHTML:`
						<div id="artplayer"></div>
						`});b=y.querySelector("#artplayer"),f.append(r,y);}if(E.container=b,X.$data.art==null){let y=await ne.init(E);if(y)X.$data.art=y;else return;X.$data.art.volume=1,u.$data.art.once("ready",()=>{s.execMenu("bili-video-playerAutoPlayVideoFullScreen",async()=>{o.info("自动进入全屏"),u.$data.art.fullscreen=!0,u.$data.art.once("fullscreenError",()=>{o.warn("未成功进入全屏，需要用户交互操作，使用网页全屏代替"),u.$data.art.fullscreenWeb=!0;});});});}else await ne.update(X.$data.art,E);r.style.paddingTop="";}});}},ou={$data:{isAddBeautifyCSS:!1},init(){X.init(),s.execMenuOnce("bili-video-repairVideoBottomAreaHeight",()=>this.repairVideoBottomAreaHeight()),s.execMenuOnce("bili-video-autoClickContinueToWatchOnTheWebpage",()=>{this.autoClickContinueToWatchOnTheWebpage();}),s.execMenu("bili-video-beautify",()=>{this.beautify();}),s.execMenuOnce("bili-video-cover-bottomRecommendVideo",()=>{this.coverBottomRecommendVideo();}),s.execMenuOnce("bili-video-gestureReturnToCloseCommentArea",()=>{this.gestureReturnToCloseCommentArea();}),s.execMenuOnce("bili-video-cover-seasonNew",()=>{this.coverSeasonNew();}),f.ready(()=>{s.execMenuOnce("bili-video-optimizationScroll",()=>{this.optimizationScroll();}),s.execMenu("bili-video-disableSwipeTab",()=>{this.disableSwipeTab();});});},beautify(){o.info("美化显示"),this.$data.isAddBeautifyCSS||(this.$data.isAddBeautifyCSS=!0,S(Kt)),c.waitNode(x.className.video+" .bottom-tab .list-view .card-box",1e4).then(e=>{if(!e){o.error("$cardBox is null");return}function t(a){var m,C;let n=a.querySelector(".title"),l=a.querySelector(".count .left"),d=!!a.querySelector(".gm-right-container"),p=F.getVue(a);if(n&&l&&p&&!d){let v=(C=(m=p==null?void 0:p.info)==null?void 0:m.owner)==null?void 0:C.name;if(v==null){o.error("美化显示-handleVCardToApp：获取up主名字失败");return}a.querySelector(".count");let E=n.cloneNode(!0),b=l.cloneNode(!0);f.hide(n);let T=a.querySelector(".open-app.weakened");T&&f.hide(T);let _=document.createElement("div");_.className="gm-up-name",_.innerHTML=`
						...
						`;let y=document.createElement("div"),h=document.createElement("div");y.className="gm-right-container",h.className="gm-right-bottom",f.after(n,y),y.appendChild(E),y.appendChild(h),h.appendChild(_),h.appendChild(b);}}function u(a){var m,C,v;let n=a.querySelector(".title"),l=a.querySelector(".count"),d=!!a.querySelector(".gm-right-container"),p=F.getVue(a);if(n&&l&&p&&!d){let E=(m=p==null?void 0:p.info)==null?void 0:m.duration;if(E==null){o.error("美化显示-handleVCard：获取视频时长失败");return}let b=(v=(C=p==null?void 0:p.info)==null?void 0:C.owner)==null?void 0:v.name;if(b==null){o.error("美化显示-handleVCard：获取up主名字失败");return}let T=n.cloneNode(!0),_=l.cloneNode(!0);f.hide(n);let y=document.createElement("div");y.className="duration",y.innerText=k.parseDuration(E),_.className="left";let h=document.createElement("div");l.appendChild(y),h.className="gm-up-name",h.innerHTML=`
						<svg viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg">
							<path fill="#999A9E" d="M896 736v-448c0-54.4-41.6-96-96-96h-576C169.6 192 128 233.6 128 288v448c0 54.4 41.6 96 96 96h576c54.4 0 96-41.6 96-96zM800 128C889.6 128 960 198.4 960 288v448c0 89.6-70.4 160-160 160h-576C134.4 896 64 825.6 64 736v-448C64 198.4 134.4 128 224 128h576zM419.2 544V326.4h60.8v240c0 96-57.6 144-147.2 144S192 665.6 192 569.6V326.4h60.8v217.6c0 51.2 3.2 108.8 83.2 108.8s83.2-57.6 83.2-108.8z m288-38.4c28.8 0 60.8-16 60.8-60.8 0-48-28.8-60.8-60.8-60.8H614.4v121.6h92.8z m3.2-179.2c102.4 0 121.6 70.4 121.6 115.2 0 48-19.2 115.2-121.6 115.2H614.4V704h-60.8V326.4h156.8z">
							</path>
						</svg>
						<span class="gm-up-name-text">${b}</span>
						`;let O=document.createElement("div"),D=document.createElement("div");O.className="gm-right-container",D.className="gm-right-bottom",f.after(n,O),O.appendChild(T),O.appendChild(D),D.appendChild(h),D.appendChild(_);}}let i=new c.LockFunction(()=>{let a=document.querySelectorAll(x.className.video+" .bottom-tab .list-view .card-box .v-card-toapp"),n=document.querySelectorAll(x.className.video+" .bottom-tab .list-view .card-box>a.v-card");a.forEach(l=>{t(l);}),n.forEach(l=>{u(l);});},25),r=document.querySelector(x.className.video);r?c.mutationObserver(r,{config:{subtree:!0,attributes:!0,childList:!0},callback(){i.run();}}):o.error("未找到视频根节点");});},repairVideoBottomAreaHeight(){return o.info("修复视频底部区域高度"),S(`
		${x.className.video} {
			/* 修复视频区域底部的高度 */
	...
	width: 100vw;\r
	height: 35vh;\r
}`,le={getUrl(e){if(e!=null)return e.getAttribute("universallink")},jumpToUrl(e){let u=e.target.querySelector("bili-open-app");if(u){let i=le.getUrl(u);i?k.goToUrl(i):(B.error("获取bili-open-app的Url失败"),o.error("获取bili-open-app的Url失败"));}else B.error("未获取到<bili-open-app>元素"),o.error("未获取到<bili-open-app>元素");}},ce={filteringSensitiveSearchParamData(e){const t=c.assign({},e,!0);return Reflect.deleteProperty(t,"access_key"),Reflect.deleteProperty(t,"access_token"),t},failToast(e){o.error(e),alert(JSON.stringify(e,null,4));}},Ie={async getPlayUrl(e){let t={avid:"",cid:"",ep_id:"",qn:127,fnver:0,fnval:3088,fourk:1};t=c.assign(t,e);let u=Fe.getBangumiProxyHost();o.info("番剧播放地址请求数据");let i=[],r;const a="/pgc/player/web/playurl";o.info(`请求路径：${a}`);for(let n=0;n<u.length;n++){const l=u[n],d=l.host,p={};d!==he.web_host&&(c.assign(p,Fe.getBangumiProxySearchParam({area:l.area}),!0),o.info(`代理服务器数据: ${JSON.stringify(l)}`),o.info(`代理服务器请求参数：${JSON.stringify(ce.filteringSensitiveSearchParamData(p))}`));let m=`https://${d}${a}?${c.toSearchParamsStr(t)}&${c.toSearchParamsStr(p)}`,C=await M.get(m,{responseType:"json",fetch:!1,allowInterceptConfig:!1,headers:{Referer:"https://www.bilibili.com/"}});if(!C.status){o.error(`代理服务器：${d} 请求失败`);continue}let v=c.toJSON(C.data.responseText);if(v.result,!H.isWebApiSuccess(v)||H.isAreaLimit(v)){o.error(`请求失败，当前代理服务器：${d} ${JSON.stringify(v)}`),i.push(v);continue}r=v.result;break}return r==null&&ce.failToast(i),r},async getPlayUrlHTML5(e){let t={avid:"",cid:"",ep_id:"",bsource:""};t=c.assign(t,e),o.info("（原版api）番剧播放地址请求数据");let i=`https://${he.web_host}/pgc/player/web/playurl/html5?${c.toSearchParamsStr(t)}`,r=await M.get(i,{responseType:"json",fetch:!0,headers:{Host:"www.bilibili.com",Referer:"https://www.bilibili.com"}});if(!r.status)return;let a=c.toJSON(r.data.responseText);if(!H.isWebApiSuccess(a)){ce.failToast(a);return}return a.result}},su="[artplayer-plugin-airborneHelper]：",$={$data:{tipJumpToastTimeoutId:void 0,tipJumpToastInfo:void 0,successJumpToastInfo:void 0},$event:{"video:timeupdate":()=>{if($.$data.tipJumpToastTimeoutId!=null||!z.$data.art.playing)return;const e=5;let t=z.$data.art.currentTime,u=z.$data.option.clip_info_list.findIndex(i=>{let r=i.start;return r===0?t<=1:t>=r-e&&t<r});if(u!==-1){let i=function(){var l;clearTimeout($.$data.tipJumpToastTimeoutId),$.$data.tipJumpToastTimeoutId=void 0,(l=$.$data.tipJumpToastInfo)==null||l.close(),$.$data.tipJumpToastInfo=void 0,z.$data.option.clip_info_list.splice(u,1);},r=z.$data.option.clip_info_list[u],a=z.$data.art.plugins[ru],n=(r.start-t)*1e3;$.$data.tipJumpToastTimeoutId=setTimeout(()=>{z.$data.art.currentTime=r.end,$.$data.tipJumpToastTimeoutId=void 0,$.$data.successJumpToastInfo&&($.$data.successJumpToastInfo.close(),$.$data.successJumpToastInfo=void 0),$.$data.successJumpToastInfo=a.toast({text:"空降成功~o(*≧▽≦)ツ┏━┓",closeCallback(){$.$data.successJumpToastInfo=void 0;}});},n),$.$data.tipJumpToastInfo&&($.$data.tipJumpToastInfo.close(),$.$data.tipJumpToastInfo=void 0),$.$data.tipJumpToastInfo=a.toast({text:typeof r.toastText=="string"?r.toastText:"站稳扶好，准备起飞~",timeout:n<2e3?2e3:n,showCloseBtn:!1,jumpText:typeof r.toastText=="string"?"不跳过":"坠机",jumpClickCallback:()=>{i();}}),setTimeout(()=>{$.$data.tipJumpToastInfo&&($.$data.tipJumpToastInfo.close(),$.$data.tipJumpToastInfo=void 0);},(e+3)*1e3);}}},bind(){Object.keys(this.$event).forEach(e=>{z.$data.art.on(e,this.$event[e]);});},unbind(){Object.keys(this.$event).forEach(e=>{z.$data.art.off(e,this.$event[e]);}),clearTimeout($.$data.tipJumpToastTimeoutId),$.$data.tipJumpToastTimeoutId=void 0,$.$data.successJumpToastInfo&&($.$data.successJumpToastInfo.close(),$.$data.successJumpToastInfo=void 0),$.$data.tipJumpToastInfo&&($.$data.tipJumpToastInfo.close(),$.$data.tipJumpToastInfo=void 0);}},z={$key:{plugin_KEY:"plugin-airborne-helper"},$data:{art:null,option:null},init(e,t){this.$data.art=e,this.update(t);},update(e){this.$data.option=e,console.log(su+"更新配置",e),$.unbind(),e.clip_info_list.length&&$.bind();}},cu=e=>t=>(z.init(t,e),{name:z.$key.plugin_KEY,update(u){z.update(u);}}),du=z.$key.plugin_KEY,qe="[flvjs]：",He=e=>e.epList.map(t=>({isDefault:t.ep_id===e.ep_id&&t.aid===e.aid&&t.cid===e.cid,title:gt(t.long_title,t.title),aid:t.aid,bvid:t.bvid,cid:t.cid,ep_id:t.ep_id,onSelect(u,i){Bt.updateArtPlayerVideoInfo(t,e.epList);}})),Ge={$data:{art:null,flv:null,currentOption:null},resetEnv(e){e&&(Reflect.set(this.$data,"art",null),Reflect.set(this.$data,"flv",null)),Reflect.set(this.$data,"currentOption",null);},flvPlayer(){var u,i;if(this.$data.currentOption==null){console.error(qe+"获取当前配置为空");return}let e=this.$data.currentOption.flvInfo;(this.$data.flv!=null||e==null)&&((u=this.$data.flv)==null||u.detachMediaElement(),(i=this.$data.flv)==null||i.destroy());let t=this.$data.currentOption;console.log(qe+"加载视频",e),e.length>1?this.$data.flv=Be.createPlayer({type:"flv",filesize:t.flvTotalSize,duration:t.flvTotalDuration,segments:e.map(r=>({url:r.url,duration:r.duration,filesize:r.size}))},{stashInitialSize:1024*100}):this.$data.flv=Be.createPlayer({type:"flv",url:e[0].url},{stashInitialSize:1024*100}),this.$data.flv.attachMediaElement(this.$data.art.video),this.$data.flv.load();},async init(e){this.resetEnv(!0),this.$data.currentOption=e;const t="artplayer-bangumi-danmaku-option",u=new nt(t),i=u.getLocalArtDanmakuOption(),r={...Et(),container:e.container,settings:[{name:"video-playback-codeid",html:"播放策略",tooltip:"默认",icon:'<svg t="1727413004405" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3183" width="24" height="24"><path d="M170.666667 256h682.666666c23.466667 0 42.666667 19.2 42.666667 42.666667v170.666666h85.333333V256c0-46.933333-38.4-85.333333-85.333333-85.333333H128c-46.933333 0-85.333333 38.4-85.333333 85.333333v512c0 46.933333 38.4 85.333333 85.333333 85.333333h384v-85.333333H170.666667c-23.466667 0-42.666667-19.2-42.666667-42.666667V298.666667c0-23.466667 19.2-42.666667 42.666667-42.666667z" p-id="3184"></path><path d="M640 512L384 341.333333v341.333334zM968.96 786.346667c1.28-12.373333 1.706667-24.746667 0.426667-36.693334l45.653333-36.266666c4.266667-3.413333 5.12-8.96 2.56-13.653334l-43.946667-76.373333c-2.56-4.693333-8.106667-6.4-13.226666-4.693333l-54.613334 21.333333a146.773333 146.773333 0 0 0-32-17.92l-8.533333-58.026667a10.624 10.624 0 0 0-10.666667-9.386666h-88.32c-5.12 0-9.813333 3.84-10.666666 8.96l-8.533334 58.026666c-11.093333 4.693333-21.76 11.093333-31.573333 17.92l-54.613333-21.333333c-5.12-2.133333-10.666667 0-13.226667 4.693333l-43.946667 76.373334c-2.56 4.693333-1.706667 10.24 2.56 13.653333l45.653334 36.693333c-1.28 12.373333-1.706667 24.746667-0.426667 36.693334l-45.653333 36.266666c-4.266667 3.413333-5.12 8.96-2.56 13.653334l43.946666 76.373333c2.56 4.693333 8.106667 6.4 13.226667 4.693333l54.186667-21.333333c9.813333 7.253333 20.48 13.226667 32 17.92l8.533333 58.026667c0.853333 5.12 5.12 8.96 10.666667 8.96h88.32c5.12 0 9.813333-3.84 10.666666-8.96l8.533334-58.026667c11.093333-4.693333 21.76-11.093333 31.573333-17.92l54.613333 21.333333c5.12 2.133333 10.666667 0 13.226667-4.693333l43.946667-76.373333c2.56-4.693333 1.706667-10.24-2.56-13.653334l-45.226667-36.266666zM810.666667 832c-35.413333 0-64-28.586667-64-64s28.586667-64 64-64 64 28.586667 64 64-28.586667 64-64 64z" p-id="3185"></path></svg>',selector:[{default:!0,html:"默认",value:R.AV1},{html:"AV1",value:R.AV1},{html:"HEVC",value:R.HEVC},{html:"AVC",value:R.AVC}],onSelect:function(a){return s.setValue("bili-bangumi-videoCodingCode",a.value),a.html}}],plugins:[vt(),bt({from:"bangumi",qualityList:e.quality})]};if(e.isFlv){if(r.quality=[],r.type="flv",e.flvInfo.length===0){ce.failToast("视频播放地址为空，无法播放！");return}r.url=e.flvInfo[0].url,r.customType={flv:(a,n,l)=>{if(!Be.isSupported()){l.notice.show="Unsupported playback format: flv";return}this.flvPlayer();}};}else r.type="mp4";return s.getValue("artplayer-plugin-bangumi-danmaku-enable")&&r.plugins.push(Xe({danmuku:e.danmukuUrl,speed:i.speed,margin:i.margin,opacity:i.opacity,color:"#FFFFFF",mode:0,modes:i.modes,fontSize:i.fontSize,antiOverlap:i.antiOverlap,synchronousPlayback:i.synchronousPlayback,mount:void 0,heatmap:!0,width:800,points:[],filter:a=>a.text.length<=100,beforeVisible:()=>!0,visible:i.visible,emitter:!1,maxLength:50,lockTime:3,theme:c.isThemeDark()?"dark":"light",beforeEmit(a){return new Promise(n=>{console.log(a),setTimeout(()=>{n(!0);},1e3);})}})),s.getValue("artplayer-plugin-bangumi-m4sAudioSupport-enable")&&r.plugins.push(ot({from:"bangumi",audioList:e.audioList||[],showSetting:!0})),s.getValue("artplayer-plugin-bangumi-epChoose-enable")&&r.plugins.push(Ct({EP_LIST:He(e),automaticBroadcast:!0})),s.getValue("artplayer-plugin-bangumi-cc-subtitle-enable")&&r.plugins.push(mt({from:"bangumi",cid:e.cid,aid:e.aid,bvid:e.bvid,ep_id:e.ep_id})),s.getValue("artplayer-plugin-bangumi-toptoolbar-enable")&&r.plugins.push(st({onlineInfoParams:{aid:e.aid,cid:e.cid,bvid:e.bvid},title:e.videoTitle,showWrap:!0,showTitle:!0,showOnlineTotal:!0})),s.getValue("artplayer-plugin-bangumi-airborneHelper-enable")&&r.plugins.push(cu({clip_info_list:e.clip_info_list})),this.$data.art=new Ze(r),s.getValue("artplayer-plugin-bangumi-danmaku-enable")&&u.repairBrowserNoResponse(this.$data.art),u.onConfigChange(this.$data.art),this.$data.art},async update(e,t){this.resetEnv(!1),this.$data.currentOption=t,o.info("更新新的播放信息",t),e.pause(),o.info("暂停视频"),e.currentTime=0,o.info("重置播放进度"),this.updatePluginInfo(e,t),e.play(),o.info("播放");},updatePluginInfo(e,t){if(e.plugins[Z].update({from:"bangumi",qualityList:t.quality}),o.info("更新画质",t.quality),s.getValue("artplayer-plugin-bangumi-danmaku-enable")&&(e.plugins.artplayerPluginDanmuku.config({danmuku:t.danmukuUrl}),e.plugins.artplayerPluginDanmuku.load(),o.info("更新弹幕姬",t.danmukuUrl)),s.getValue("artplayer-plugin-bangumi-m4sAudioSupport-enable")&&(e.plugins[lt].update({from:"bangumi",audioList:t.audioList||[]}),o.info("更新音频",t.audioList)),s.getValue("artplayer-plugin-bangumi-epChoose-enable")&&(e.plugins[At].update({EP_LIST:He(t),automaticBroadcast:!0}),o.info("更新选集信息",t.epList)),s.getValue("artplayer-plugin-bangumi-cc-subtitle-enable")){let i=e.plugins[ht];const r={from:"bangumi",cid:t.cid,aid:t.aid,ep_id:t.ep_id};i.update(r),o.info("更新字幕",r);}if(s.getValue("artplayer-plugin-bangumi-toptoolbar-enable")){let i=e.plugins[ct];const r={showRight:!0,showRightFollow:!0,showWrap:!0,showTitle:!0,showOnlineTotal:!0,title:t.videoTitle,onlineInfoParams:{aid:t.aid,cid:t.cid,bvid:t.bvid}};i.update(r),o.info("更新顶部标题",r);}s.getValue("artplayer-plugin-bangumi-airborneHelper-enable")&&(e.plugins[du].update({clip_info_list:t.clip_info_list}),o.info("更新空降助手信息",t.clip_info_list));}},pu={getUserChooseVideoCodingCode(){let e=s.getValue("bili-bangumi-videoCodingCode",R.AV1);return Object.values(R).includes(e)||(o.error("意外情况，选择的编码格式不是允许的编码，将强制使用默认，防止过滤掉的视频链接为空："+e),e=R.AV1),e}};function mu(e){const t={};return e.forEach(i=>{(!t[i.id]||i.size>t[i.id].size)&&(t[i.id]=i);}),Object.values(t)}function We(e,t){let u=[];return e.video.forEach(i=>{if(!e.accept_quality.includes(i.id)||t.codecid!=null&&i.codecid!==t.codecid)return;let r=e.support_formats.find(l=>l.quality===i.id),a=L.findBetterCDN(i.base_url,i.baseUrl,i.backup_url,i.backupUrl);a=L.replaceBangumiVideoCDN(a);let n=r==null?void 0:r.new_description;u.push({name:n,url:a,type:i.mimeType,id:i.id,size:i.size,quality:i.id,vip:!!(r!=null&&r.need_vip)});}),u}const hu=(e,t)=>`第${e}话 ${t}`,je=(e,t)=>{var i,r;let u=[];if((r=(i=e==null?void 0:e.dash)==null?void 0:i.video)!=null&&r.length){let a=e;u=[...We({accept_quality:a.accept_quality,support_formats:a.support_formats,video:a.dash.video},{codecid:t})],u.length===0&&a.dash.video.length!==0&&(o.warn(`当前选择的视频编码id为: ${t}，但是过滤出的视频没有一个符合的，所以直接放弃使用自定义选择视频编码`),u=[],u=[...We({accept_quality:a.accept_quality,support_formats:a.support_formats,video:a.dash.video},{})]),u=mu(u),u.sort((n,l)=>l.quality-n.quality);}else {let a=e;a.durls.length===0&&a.durl!=null&&a.durls.push({quality:a.quality,durl:a.durl}),a.durls.forEach(n=>{if(!a.accept_quality.includes(n.quality)||!n.durl.length)return;let l=n.durl[0],d=e.support_formats.find(C=>C.quality===n.quality),p=L.findBetterCDN(l.url,l.backup_url),m=d==null?void 0:d.new_description;u.push({name:m,url:p,type:"audio/mp4",id:n.quality,size:l.size,quality:n.quality,vip:!!(d!=null&&d.need_vip)});});}return u},fu=async(e,t)=>{var h,O;const{aid:u,bvid:i,cid:r,ep_id:a,title:n,long_title:l}=e;o.info(`解析番剧信息 aid:${u} cid:${r} ep_id:${a}`);const d=hu(n,l),p=[];let m=[],C=[],v=!1,E=[],b=0,T=0;if(s.getValue("bili-bangumi-unlockAreaLimit")){const D=await Ie.getPlayUrl({avid:u,cid:r,ep_id:a});if(!D)return;Array.isArray(D==null?void 0:D.clip_info_list)?C=D.clip_info_list:Array.isArray(D==null?void 0:D.clip_info)&&(C=D.clip_info);let fe=pu.getUserChooseVideoCodingCode();if(D.type.toLowerCase()==="flv")v=!0,D.durl.forEach(P=>{let q=L.findBetterCDN(P.url,P.backup_url);q=L.replaceBangumiVideoCDN(q),b+=P.length,T+=P.size,E.push({order:P.order,url:q,duration:P.length,length:P.length,size:P.size});});else if(D.type.toLowerCase()==="dash"||D.type.toLowerCase()==="mp4")(((h=D==null?void 0:D.dash)==null?void 0:h.audio)||[]).forEach(P=>{let q=L.findBetterCDN(P.baseUrl,P.base_url,P.baseUrl,P.backup_url);s.getValue("bili-bangumi-uposServerSelect-applyAudio")&&(q=L.replaceBangumiVideoCDN(q)),p.push({url:q,id:P.id,size:P.size,text:at[P.id]||""});}),p.sort((P,q)=>q.id-P.id),o.info(["ArtPlayer: 获取的音频信息",p]),m=m.concat(je(D,fe)),o.info(["ArtPlayer: 获取的视频画质信息",m]);else {ce.failToast("暂未适配的视频格式："+D.format);return}}else {const D=await Ie.getPlayUrlHTML5({avid:u,cid:r,ep_id:a});if(!D)return;Array.isArray(D==null?void 0:D.clip_info_list)?C=D.clip_info_list:Array.isArray(D==null?void 0:D.clip_info)&&(C=D.clip_info),m=m.concat(je(D));}const _=m.map((D,fe)=>({html:D.name,url:D.url,quality:D.quality})),y={container:null,epList:t,cid:r,aid:u,bvid:i,ep_id:a,videoTitle:d,danmukuUrl:`https://api.bilibili.com/x/v1/dm/list.so?oid=${r}`,quality:_,clip_info_list:C,isFlv:v,flvInfo:E,flvTotalDuration:b,flvTotalSize:T};return y.url=(O=m==null?void 0:m[0])==null?void 0:O.url,p.length&&(y.audioList=p.map((D,fe)=>({isDefault:fe===0,url:D.url,soundQualityCode:D.id,soundQualityCodeText:D.text}))),y},Bt={updateArtPlayerVideoInfo(e,t){F.waitVuePropToSet(".player-wrapper",{msg:"等待player-wrapper加载完成",check(u){var i,r,a;return typeof((i=u==null?void 0:u.EP_INFO)==null?void 0:i.aid)=="number"&&typeof((r=u==null?void 0:u.EP_INFO)==null?void 0:r.cid)=="number"&&typeof((a=u==null?void 0:u.EP_INFO)==null?void 0:a.ep_id)=="number"},async set(u){const i=document.querySelector(".player-wrapper");e==null&&(e=u.EP_INFO),t==null&&(t=u.EP_LIST);const r=await fu(e,t);if(r==null)return;let a=document.querySelector("#artplayer");if(!a){const n=f.createElement("div",{className:"artplayer-container",innerHTML:`
						<div id="artplayer"></div>
						`});a=n.querySelector("#artplayer"),f.after(i,n);}if(r.container=a,se.$data.art==null){let n=await Ge.init(r);if(n)se.$data.art=n;else return;se.$data.art.volume=1;}else Ge.update(se.$data.art,r);}});}},te={loadScript(e){let t=document.createElement("script");return t.src=e,document.head.appendChild(t),new Promise(u=>{t.onload=function(){o.success("script标签加载完毕："+e),setTimeout(()=>{u(!0);},100);};})},addBlockCSS(...e){let t=[];if(e.length!==0&&!(e.length===1&&typeof e[0]=="string"&&e[0].trim()===""))return e.forEach(u=>{Array.isArray(u)?t=t.concat(u):t.push(u);}),S(`${t.join(`,
`)}{display: none !important;}`)},initialScale(){o.info("设置<meta>的viewport固定缩放倍率为1并移除页面原有的<meta>"),f.ready(()=>{let e=f.createElement("meta",{},{name:"viewport",content:"width=device-width,initial-scale=1,user-scalable=no,viewport-fit=cover"});f.remove("meta[name='viewport']"),c.waitNode("head").then(()=>{document.head.appendChild(e);});});}},se={$data:{art:null},init(){s.execMenuOnce("bili-bangumi-initialScale",()=>{te.initialScale();}),s.execMenuOnce("bili-bangumi-hook-callApp",()=>{this.hookCallApp();}),s.execMenu("bili-bangumi-cover-clicl-event-chooseEp",()=>{this.setChooseEpClickEvent();}),s.execMenu("bili-bangumi-cover-clicl-event-other",()=>{this.setClickOtherVideo();}),s.execMenu("bili-bangumi-cover-clicl-event-recommend",()=>{this.setRecommendClickEvent();}),this.coverVideoPlayer();},hookCallApp(){let e=I.setTimeout;I.setTimeout=function(...t){if(t[0].toString().includes("autoOpenApp")){o.success(["阻止唤醒App",t]);return}return e.apply(this,t)};},setPay(){F.waitVuePropToSet("#app",[{msg:"设置参数 $store.state.userStat.pay",check(e){var t,u,i;return typeof typeof((i=(u=(t=e==null?void 0:e.$store)==null?void 0:t.state)==null?void 0:u.userStat)==null?void 0:i.pay)=="number"},set(e){o.success("成功设置参数 $store.state.userStat.pay=1"),e.$store.state.userStat.pay=1;}},{msg:"设置参数 $store.state.mediaInfo.user_status.pay",check(e){var t,u,i,r;return typeof((r=(i=(u=(t=e==null?void 0:e.$store)==null?void 0:t.state)==null?void 0:u.mediaInfo)==null?void 0:i.user_status)==null?void 0:r.pay)=="number"},set(e){o.success("成功设置参数 $store.state.mediaInfo.user_status.pay=1"),e.$store.state.mediaInfo.user_status.pay=1;}}]);},setChooseEpClickEvent(){c.waitNode(x.className.bangumi+" .ep-list-pre-wrapper ul.ep-list-pre-container").then(e=>{o.info("覆盖【选集】的点击事件"),f.on(e,"click","li.episode-item",function(t){c.preventEvent(t),le.jumpToUrl(t);},{capture:!0});}),c.waitNode(x.className.bangumi+" .ep-list-pre-wrapper ul.season-list-wrapper").then(e=>{o.info("覆盖【xx季】的点击事件"),f.on(e,"click","li",function(t){c.preventEvent(t),le.jumpToUrl(t);},{capture:!0});}),c.waitNode(x.className.bangumi+" .ep-list-pre-header").then(e=>{o.info("覆盖【选集】右上角的【全xx话】Arrow的点击事件"),f.on(e,"click",function(t){c.preventEvent(t);},{capture:!0});});},setClickOtherVideo(){c.waitNode(x.className.bangumi+" .section-preview-wrapper ul.ep-list-pre-container").then(e=>{o.info("覆盖【PV&其他】、【预告】、【主题曲】的点击事件"),f.on(e,"click","li.section-preview-item",function(t){c.preventEvent(t),le.jumpToUrl(t);},{capture:!0});}),c.waitNode(x.className.bangumi+" .section-preview-header").then(e=>{o.info("覆盖【PV&其他】、【预告】、【主题曲】右上角的Arrow的点击事件"),f.on(e,"click",function(t){c.preventEvent(t);},{capture:!0});});},setRecommendClickEvent(){c.waitNode(x.className.bangumi+" .recom-wrapper ul.recom-list").then(e=>{o.info("覆盖【更多推荐】番剧的点击事件"),f.on(e,"click","li.recom-item-v2",function(t){c.preventEvent(t),le.jumpToUrl(t);},{capture:!0});});},coverVideoPlayer(){if(document.querySelector("#artplayer"))o.warn("已存在播放器，更新播放信息");else {S(`
			...
			`)),c.waitNode(".m-search-result .tabs:not(:has(.gm-tab-item))").then(e=>{Fe.getSearchProxyHost().forEach(i=>{let r=f.createElement("a",{className:"tab-item gm-tab-item",innerHTML:`番剧（${i.name}）`},{"data-area":i.area,"data-host":i.host});e.appendChild(r);});const u=i=>{e.querySelectorAll(".tab-item").forEach(r=>i!=r&&r.classList.remove("on")),i.classList.add("on");};f.on(e,"click",".tab-item",async i=>{let r=i.target;u(r);let a=document.querySelector(".result-panel"),n=document.querySelector(".gm-result-panel");if(n&&(n.remove(),f.show(a)),!r.classList.contains("gm-tab-item"))return;let l=r.dataset.area,d=r.dataset.host,p=document.querySelector(".m-search-result"),m=F.getVue(p);m.switchTab(233),f.hide(a);let C=m.keyword,v=B.loading("搜索中，请稍后..."),E=await Dt.getBangumiSearchResult({keyword:C,area:l,host:d});if(v.close(),!E)return;if(!E.isSuccess){alert(JSON.stringify(E.data,null,2));return}let b=E.data;o.info(["搜索结果：",b]);let T=f.createElement("div",{className:"gm-result-panel",innerHTML:`
					...
					`}),_=T.querySelector(".gm-card-box");b.forEach(y=>{_.appendChild(this.createSearchResultVideoItem(y));}),p.appendChild(T);});});},createSearchResultVideoItem(e){var d,p;let t=f.createElement("div",{className:"gm-card-item",innerHTML:`
