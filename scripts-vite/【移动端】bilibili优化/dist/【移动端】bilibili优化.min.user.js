...
(function (w, he, Fe, nt, Mt, mt, Rt, fr, it) {
  'use strict';
...
`)}{display: none !important;}`)},setGMResourceCSS(e){const t=typeof gt=="function"?gt(e.keyName):null;return typeof t=="string"&&t?H(t):ce.loadStyleLink(e.url)},async loadStyleLink(e){let t=document.createElement("link");return t.rel="stylesheet",t.type="text/css",t.href=e,new Promise(r=>{he.ready(()=>{document.head.appendChild(t),r(t);});})},async loadScript(e){let t=document.createElement("script");return t.src=e,new Promise(r=>{t.onload=()=>{r(null);},(document.head||document.documentElement).appendChild(t);})},fixUrl(e){return e=e.trim(),e.startsWith("data:")||e.match(/^http(s|):\/\//i)?e:e.startsWith("//")?(e.startsWith("///")||(e=window.location.protocol+e),e):(e.startsWith("/")||(e+="/"),e=window.location.origin+e,e)},fixHttps(e){if(e.startsWith("https://")||!e.startsWith("http://"))return e;try{let t=new URL(e);return t.protocol="https:",t.toString()}catch{return e}},lockScroll(...e){let t=document.createElement("style");t.innerHTML=`
		...
		`;let r=[document.documentElement,document.body].concat(...e||[]);return r.forEach(n=>{n.classList.add("pops-overflow-hidden-important");}),(document.head||document.documentElement).appendChild(t),{recovery(){r.forEach(n=>{n.classList.remove("pops-overflow-hidden-important");}),t.remove();}}},async getClipboardText(){function e(n){navigator.clipboard.readText().then(i=>{n(i);}).catch(i=>{u.error("读取剪贴板内容失败👉",i),n("");});}function t(n){navigator.permissions.query({name:"clipboard-read"}).then(i=>{e(n);}).catch(i=>{u.error("申请剪贴板权限失败，尝试直接读取👉",i.message??i.name??i.stack),e(n);});}function r(){return !(typeof navigator?.clipboard?.readText!="function"||typeof navigator?.permissions?.query!="function")}return new Promise(n=>{if(!r()){n("");return}document.hasFocus()?t(n):window.addEventListener("focus",()=>{t(n);},{once:true});})},escapeHtml(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;").replace(/©/g,"&copy;").replace(/®/g,"&reg;").replace(/™/g,"&trade;").replace(/→/g,"&rarr;").replace(/←/g,"&larr;").replace(/↑/g,"&uarr;").replace(/↓/g,"&darr;").replace(/—/g,"&mdash;").replace(/–/g,"&ndash;").replace(/…/g,"&hellip;").replace(/ /g,"&nbsp;").replace(/\r\n/g,"<br>").replace(/\r/g,"<br>").replace(/\n/g,"<br>").replace(/\t/g,"&nbsp;&nbsp;&nbsp;&nbsp;")},interval(e,t,r=5e3){let n,i=r-t,a=t,o=async l=>{let c=await e(l);if(typeof c=="boolean"&&!c||l){h.workerClearTimeout(n);return}if(a+=t,a>i){o(true);return}n=h.workerSetTimeout(()=>{o(false);},t);};o(false);},findParentNode(e,t,r){if(r){let n=he.closest(e,r);if(n)return n.querySelector(t)}else return he.matches(e,t)?e:he.closest(e,t)},toStr(e){const t="__undefined__placeholder__replaced__str__";return JSON.stringify(e,(n,i)=>i===void 0?t:i,2).replace(new RegExp(`"${t}"`,"g"),"undefined")}},tt={$data:{__contentConfig:null,get contentConfig(){return this.__contentConfig==null&&(this.__contentConfig=new h.Dictionary),this.__contentConfig},__defaultBottomContentConfig:[]},addContentConfig(e){Array.isArray(e)||(e=[e]);let t=this.$data.contentConfig.keys().length;this.$data.contentConfig.set(t,e);},getAllContentConfig(){return this.$data.contentConfig.values().flat()},getConfig(e=0){return this.$data.contentConfig.get(e)??[]},getDefaultBottomContentConfig(){if(this.$data.__defaultBottomContentConfig.length)return this.$data.__defaultBottomContentConfig;let e=false,t;const r=(a,o)=>{typeof o!="string"&&(o=ce.toStr(o));const l=new Blob([o]),c=globalThis.URL.createObjectURL(l);s.createElement("a",{href:c,download:a}).click(),h.workerSetTimeout(()=>{globalThis.URL.revokeObjectURL(c);},500);},n=()=>{const a=m=>{const g=G.alert({title:{text:"请选择导入方式",position:"center"},content:{text:`
          ...
          }`}),y=g.$shadowRoot.querySelector(".btn-control[data-mode='local']"),E=g.$shadowRoot.querySelector(".btn-control[data-mode='network']"),I=g.$shadowRoot.querySelector(".btn-control[data-mode='clipboard']"),S=async D=>{confirm("是否清空脚本存储的配置？（如果点击取消按钮，则仅做配置覆盖处理）")&&(typeof Ge=="function"?typeof Xe=="function"?(Ge().forEach(V=>{Xe(V);}),w.success("已清空脚本存储的配置")):w.error("不支持GM_deleteValue函数，无法执行删除脚本配置"):w.error("不支持GM_listValues函数，无法清空脚本存储的配置")),typeof bt=="function"?bt(D):Object.keys(D).forEach(V=>{const N=D[V];Me(V,N);}),w.success("配置导入完毕");},_=D=>new Promise(async k=>{const O=h.toJSON(D);Object.keys(O).length===0?w.warning("解析为空配置，不导入"):await S(O),k(true);});s.on(y,"click",D=>{s.preventEvent(D),g.close();const k=s.createElement("input",{type:"file",accept:".json"});s.on(k,["propertychange","input"],O=>{if(!k.files?.length)return;const V=k.files[0],N=new FileReader;N.onload=()=>{_(N.result);},N.readAsText(V,"UTF-8");}),k.click();}),s.on(E,"click",D=>{s.preventEvent(D),g.close();const k=G.prompt({title:{text:"网络导入",position:"center"},content:{text:"",placeholder:"请填写URL",focus:true},btn:{close:{enable:true,callback(N,q){N.close();}},ok:{text:"导入",callback:async(N,q)=>{const W=N.text;if(h.isNull(W)){w.error("请填入完整的url");return}const z=w.loading("正在获取配置..."),M=await K.get(W,{allowInterceptConfig:false});if(z.close(),!M.status){u.error(M),w.error("获取配置失败",{consoleLogContent:true});return}await _(M.data.responseText)&&N.close();}},cancel:{enable:false}},drag:true,mask:{enable:true},width:se.info.width,height:"auto"}),O=k.$shadowRoot.querySelector("input"),V=k.$shadowRoot.querySelector(".pops-prompt-btn-ok");s.on(O,["input","propertychange"],N=>{s.val(O)===""?s.attr(V,"disabled","true"):s.removeAttr(V,"disabled");}),s.listenKeyboard(O,"keydown",(N,q,W)=>{N==="Enter"&&W.length===0&&s.val(O)!==""&&s.trigger(V,"click");}),s.trigger(O,"input");}),s.on(I,"click",async D=>{s.preventEvent(D),g.close();let k=await ce.getClipboardText();if(k.trim()===""){w.warning("获取到的剪贴板内容为空");return}await _(k);});},o=(m=`${st}_panel-setting-${h.formatTime(Date.now(),"yyyy_MM_dd_HH_mm_ss")}.json`,g)=>{const y=G.alert({title:{text:"请选择导出方式",position:"center"},content:{text:`
          ...
          .btn-control:hover{
            ...
            border-color: #c6e2ff;
            background-color: #ecf5ff;
          }`}),E=y.$shadowRoot.querySelector(".btn-control[data-mode='export-to-file']"),I=y.$shadowRoot.querySelector(".btn-control[data-mode='export-to-clipboard']");s.on(E,"click",S=>{s.preventEvent(S);try{r(m,g),y.close();}catch(_){w.error(_.toString(),{consoleLogContent:true});}}),s.on(I,"click",async S=>{await h.copy(g)?(w.success("复制成功"),y.close()):w.error("复制失败");});},c=G.confirm({title:{text:"配置",position:"center"},content:{text:`
        ...
        `}).$shadowRoot.querySelector("textarea"),p={};if(typeof Ge=="function")Ge().forEach(g=>{const y=Ve(g);Reflect.set(p,g,y);});else {w.warning("不支持函数GM_listValues，仅导出菜单配置");const m=Ve(et);Reflect.set(p,et,m);}const d=ce.toStr(p);c.value=d;},i=()=>{let a=Oe?.script?.supportURL||Oe?.script?.namespace;typeof a=="string"&&h.isNotNull(a)&&window.open(a,"_blank");};return [{id:"script-version",title:`版本：${Oe?.script?.version||"未知"}`,isBottom:true,views:[],clickFirstCallback(){return  false},afterRender(a){new kr(a.$asideLiElement).on("tap",function(l){clearTimeout(t),t=void 0,e?(e=false,n()):(t=setTimeout(()=>{e=false,i();},200),e=true);});}}]},setDefaultBottomContentConfig(e){this.$data.__defaultBottomContentConfig=e;}},Ot={$data:{__menuOption:[{key:"show_pops_panel_setting",text:"⚙ 设置",autoReload:false,isStoreValue:false,showText(e){return e},callback:()=>{f.showPanel(tt.getConfig(0));}}],get menuOption(){return this.__menuOption}},init(){this.initExtensionsMenu();},initExtensionsMenu(){f.isTopWindow()&&_r.add(this.$data.menuOption);},addMenuOption(e){Array.isArray(e)||(e=[e]),this.$data.menuOption.push(...e);},updateMenuOption(e){Array.isArray(e)||(e=[e]),e.forEach(t=>{let r=this.$data.menuOption.findIndex(n=>n.key===t.key);r!==-1&&(this.$data.menuOption[r]=t);});},getMenuOption(e=0){return this.$data.menuOption[e]},deleteMenuOption(e=0){this.$data.menuOption.splice(e,1);}},f={$data:{__contentConfigInitDefaultValue:null,__onceExecMenuData:null,__urlChangeReloadMenuExecOnce:null,__onceExecData:null,__panelConfig:{},$panel:null,panelContent:[],get contentConfigInitDefaultValue(){return this.__contentConfigInitDefaultValue==null&&(this.__contentConfigInitDefaultValue=new h.Dictionary),this.__contentConfigInitDefaultValue},contentConfigInitDisabledKeys:[],get onceExecMenuData(){return this.__onceExecMenuData==null&&(this.__onceExecMenuData=new h.Dictionary),this.__onceExecMenuData},get urlChangeReloadMenuExecOnce(){return this.__urlChangeReloadMenuExecOnce==null&&(this.__urlChangeReloadMenuExecOnce=new h.Dictionary),this.__urlChangeReloadMenuExecOnce},get onceExecData(){return this.__onceExecData==null&&(this.__onceExecData=new h.Dictionary),this.__onceExecData},get scriptName(){return st},get panelConfig(){return this.__panelConfig},set panelConfig(e){this.__panelConfig=e;},key:et,attributeKeyName:_e,attributeDefaultValueName:$e},init(){this.initContentDefaultValue(),Ot.init();},isTopWindow(){return te.top===te.self},initContentDefaultValue(){const e=n=>{if(!n.attributes||n.type==="button"||n.type==="container"||n.type==="deepMenu")return;const i=n.attributes;let a=i[zt];if(typeof a=="function"){let p=a();if(typeof p=="boolean"&&!p)return}let o=new Map,l=i[_e];if(l!=null){const p=i[$e];o.set(l,p);}let c=i[Ar];if(typeof c=="object"&&c&&Object.keys(c).forEach(p=>{const d=c[p];o.set(p,d);}),!o.size){u.warn(["请先配置键",n]);return}if(n.type==="switch"){let p=typeof n.disabled=="function"?n.disabled():n.disabled;typeof p=="boolean"&&p&&this.$data.contentConfigInitDisabledKeys.push(...o.keys());}for(const[p,d]of o.entries())this.setDefaultValue(p,d);},t=n=>{for(let i=0;i<n.length;i++){let a=n[i];e(a);let o=a.views;o&&Array.isArray(o)&&t(o);}},r=[...tt.getAllContentConfig()];for(let n=0;n<r.length;n++){let i=r[n];if(!i.views)continue;const a=i.views;a&&Array.isArray(a)&&t(a);}this.$data.contentConfigInitDisabledKeys=[...new Set(this.$data.contentConfigInitDisabledKeys)];},setDefaultValue(e,t){this.$data.contentConfigInitDefaultValue.has(e)&&u.warn("请检查该key(已存在): "+e),this.$data.contentConfigInitDefaultValue.set(e,t);},getDefaultValue(e){return this.$data.contentConfigInitDefaultValue.get(e)},setValue(e,t){ye.set(e,t);},getValue(e,t){const r=ye.get(e);return r??(this.$data.contentConfigInitDefaultValue.has(e)?this.$data.contentConfigInitDefaultValue.get(e):t)},deleteValue(e){ye.delete(e);},hasKey(e){return ye.has(e)},addValueChangeListener(e,t){return ye.addValueChangeListener(e,(n,i,a)=>{t(e,a,i);})},removeValueChangeListener(e){ye.removeValueChangeListener(e);},triggerMenuValueChange(e,t,r){ye.triggerValueChangeListener(e,r,t);},async exec(e,t,r,n=true){const i=this;let a;typeof e=="string"||Array.isArray(e)?a=()=>e:a=e;let o=false;const l=a();let c=[];Array.isArray(l)?(o=true,c=l):c.push(l);const p=c.find(V=>!this.$data.contentConfigInitDefaultValue.has(V));if(p){u.warn(`${p} 键不存在`);return}const d=JSON.stringify(c);if(n&&this.$data.onceExecMenuData.has(d))return this.$data.onceExecMenuData.get(d);let m=[];const g=[];let y=[];const E=(V,N)=>{let q=[],W=[],z=[];if(Array.isArray(N))z=z.concat(N);else if(typeof N=="object"&&N!=null)if(N instanceof Element)z.push(N);else {const{$css:M,destory:R}=N;M!=null&&(Array.isArray(M)?z=z.concat(M):z.push(M)),typeof R=="function"&&z.push(R);}else z.push(N);for(const M of z)if(M!=null){if(M instanceof Element){q.push(M);continue}if(typeof M=="function"){y.push(M);continue}}V?(m=m.concat(q),y=y.concat(W)):(S(),_());},I=V=>!!this.getValue(V),S=()=>{for(let V=0;V<m.length;V++)m[V]?.remove(),m.splice(V,1),V--;},_=()=>{for(let V=0;V<y.length;V++){const N=y[V];N(),y.splice(V,1),V--;}},D=()=>{let V=false;return typeof r=="function"?V=r(c):V=c.every(N=>I(N)),V},k=async V=>{if(D()){const q=c.map(z=>this.getValue(z)),W=await t({value:o?q:q[0],addStoreValue:(...z)=>E(true,z)});E(true,W);}else E(false,[]);};n&&c.forEach(V=>{const N=this.addValueChangeListener(V,(q,W,z)=>k());g.push(N);}),await k();const O={reload(){this.clearStoreStyleElements(),this.destory(),k();},clear(){this.clearStoreStyleElements(),this.destory(),this.removeValueChangeListener(),this.clearOnceExecMenuData();},clearStoreStyleElements:()=>S(),destory(){return _()},removeValueChangeListener:()=>{g.forEach(V=>{this.removeValueChangeListener(V);});},clearOnceExecMenuData(){n&&i.$data.onceExecMenuData.delete(d);}};return this.$data.onceExecMenuData.set(d,O),O},async execMenu(e,t,r=false,n=false){return await this.exec(e,async i=>await t(i),i=>i.every(o=>{let l=!!this.getValue(o);return f.$data.contentConfigInitDisabledKeys.includes(o)&&(l=false,u.warn(`.execMenu${n?"Once":""} ${o} 被禁用`)),r&&(l=!l),l}),n)},async execMenuOnce(e,t,r=false,n=false){const i=await this.execMenu(e,t,r,true);if(n&&i){const a=()=>{i.reload();};this.removeUrlChangeWithExecMenuOnceListener(e),this.addUrlChangeWithExecMenuOnceListener(e,a);}return i},deleteExecMenuOnce(e){return e=this.transformKey(e),this.$data.onceExecMenuData.delete(e),this.$data.urlChangeReloadMenuExecOnce.delete(e),ye.removeValueChangeListener(e)},onceExec(e,t){if(e=this.transformKey(e),typeof e!="string")throw new TypeError("key 必须是字符串");this.$data.onceExecData.has(e)||(t(),this.$data.onceExecData.set(e,1));},deleteOnceExec(e){e=this.transformKey(e),this.$data.onceExecData.delete(e);},addUrlChangeWithExecMenuOnceListener(e,t){e=this.transformKey(e),this.$data.urlChangeReloadMenuExecOnce.set(e,t);},removeUrlChangeWithExecMenuOnceListener(e){e=this.transformKey(e),this.$data.urlChangeReloadMenuExecOnce.delete(e);},hasUrlChangeWithExecMenuOnceListener(e){return e=this.transformKey(e),this.$data.urlChangeReloadMenuExecOnce.has(e)},async triggerUrlChangeWithExecMenuOnceEvent(e){const t=this.$data.urlChangeReloadMenuExecOnce.values();for(const r of t)await r(e);},showPanel(e,t=`${st}-设置`,r=false,n=false){this.$data.$panel=null,this.$data.panelContent=[];let i=e.findIndex(o=>(typeof o.isBottom=="function"?o.isBottom():!!o.isBottom)&&o.id==="script-version")!==-1;!r&&!i&&e.push(...tt.getDefaultBottomContentConfig());let a=G.panel({title:{text:t,position:"center",html:false,style:""},content:e,btn:{close:{enable:true,callback:(o,l)=>{o.close(),this.$data.$panel=null;}}},mask:{enable:true,clickEvent:{toClose:true,toHide:false},clickCallBack:(o,l)=>{o(),this.$data.$panel=null;}},width:se.setting.width,height:se.setting.height,drag:true,only:true,...this.$data.panelConfig});this.$data.$panel=a,this.$data.panelContent=e,n||this.registerConfigSearch({$panel:a,content:e});},registerConfigSearch(e){const{$panel:t,content:r}=e,n=async(g,y)=>{if(g==null)return;const E=await y(g);return E&&typeof E.isFind=="boolean"&&E.isFind?E.data:await n(E.data,y)},i=(g,y)=>{const E=new IntersectionObserver(I=>{I.forEach(S=>{S.isIntersecting&&(y?.(),E.disconnect());});},{root:null,threshold:1});E.observe(g),g.scrollIntoView({behavior:"smooth",block:"center"});},a=g=>{const y="pops-flashing";s.animationend(g,()=>{g.classList.remove(y);}),g.classList.add(y);},o=g=>{if(g.type==="dblclick"&&m)return;s.preventEvent(g),c=null;const y=G.alert({title:{text:"搜索配置",position:"center"},content:{text:`
					...
					${e.searchDialogStyle??""}
				`});y.$shadowRoot.querySelector(".search-wrapper");const E=y.$shadowRoot.querySelector(".search-config-text"),I=y.$shadowRoot.querySelector(".search-result-wrapper");E.focus();const S=()=>{s.empty(I);},_=k=>{const O=h.queryProperty(k,q=>q?.next?{isFind:false,data:q.next}:{isFind:true,data:q}),V=s.createElement("div",{className:"search-result-item",innerHTML:`
						...
						`}),N=G.config.PanelHandlerComponents();return s.on(V,"click",q=>{const z=t.$shadowRoot.querySelectorAll("aside.pops-panel-aside .pops-panel-aside-top-container li")[k.index];if(!z){w.error(`左侧项下标${k.index}不存在`);return}z.scrollIntoView({behavior:"smooth",block:"center"}),z.click(),n(k.next,async M=>{if(M?.next){const R=await s.waitNode(()=>Array.from(t.$shadowRoot.querySelectorAll(".pops-panel-deepMenu-nav-item")).find(b=>{const C=Reflect.get(b,N.$data.nodeStoreConfigKey);return typeof C=="object"&&C!=null&&C.text===M.name}),2500);if(R)R.click();else return w.error("未找到对应的二级菜单"),{isFind:true,data:M};return {isFind:false,data:M.next}}else {const R=await s.waitNode(()=>Array.from(t.$shadowRoot.querySelectorAll("li:not(.pops-panel-deepMenu-nav-item)")).find(b=>Reflect.get(b,N.$data.nodeStoreConfigKey)===M.matchedData?.formConfig),2500);if(R){i(R);const b=R.closest(".pops-panel-forms-fold[data-fold-enable]");b&&(b.querySelector(".pops-panel-forms-fold-container").click(),await h.sleep(500)),i(R,()=>{a(R);});}else w.error("未找到对应的菜单项");return {isFind:true,data:M}}});}),V},D=k=>{const O=new RegExp(k,"i"),V=[],N=(W,z)=>{for(let M=0;M<W.length;M++){const R=W[M],b=R.views;if(b&&Array.isArray(b)){const C=h.deepClone(z);if(R.type==="deepMenu"){const B=h.queryProperty(C,F=>F?.next?{isFind:false,data:F.next}:{isFind:true,data:F});B.next={name:R.text};}N(b,C);}else {let C,B;if(R.type==="own"){const $=Reflect.get(R.attributes||{},Er);$&&(typeof $.text=="string"&&(C=$.text),typeof $.desc=="string"&&(B=$.desc));}else C=Reflect.get(R,"text"),B=Reflect.get(R,"description");const F=[C,B],T=F.findIndex($=>{if(typeof $=="string")return $.match(O)});if(T!==-1){const $=h.deepClone(z),Q=h.queryProperty($,ae=>ae?.next?{isFind:false,data:ae.next}:{isFind:true,data:ae});Q.next={name:C,matchedData:{path:"",formConfig:R,matchedText:F[T],description:B}};const Z=[];h.queryProperty($,ae=>{const oe=ae?.name;return typeof oe=="string"&&oe.trim()!==""&&Z.push(oe),ae?.next?{isFind:false,data:ae.next}:{isFind:true,data:ae}});const ve=Z.join(ce.escapeHtml(" - "));Q.next.matchedData.path=ve,V.push($);}}}};for(let W=0;W<r.length;W++){const z=r[W];if(!z.views||z.isBottom&&z.id==="script-version")continue;const M=z.views;if(M&&Array.isArray(M)){let R=z.title;typeof R=="function"&&(R=R()),N(M,{index:W,name:R});}}const q=document.createDocumentFragment();for(const W of V){let z=_(W);q.appendChild(z);}S(),I.append(q);};s.on(E,"input",h.debounce(k=>{s.preventEvent(k);let O=s.val(E).trim();if(O===""){S();return}D(O);},200));};t.$shadowRoot.querySelectorAll("aside.pops-panel-aside .pops-panel-aside-item:not(#script-version)").forEach(g=>{s.on(g,"dblclick",o);});let c=null,p=false,d,m=false;s.on(t.$shadowRoot,"touchend","aside.pops-panel-aside .pops-panel-aside-item:not(#script-version)",(g,y)=>{m=true,clearTimeout(d),d=void 0,p&&c===y?(p=false,c=null,o(g)):(d=setTimeout(()=>{p=false;},200),p=true,c=y);},{capture:true}),t.$shadowRoot.appendChild(s.createElement("style",{type:"text/css",textContent:`
					.pops-flashing{
...
`)}
      ...
      `;}async function E(b){const C=b.querySelector(".comment-container .reply-header .nav-sort"),B=C.querySelector(".hot-sort"),F=C.querySelector(".time-sort");C.classList.add("hot"),C.classList.remove("time"),B.addEventListener("click",T=>{T.stopPropagation(),T.preventDefault(),p!==c.HOT&&(p=c.HOT,C.classList.add("hot"),C.classList.remove("time"),b.scrollTo(0,0),I(b));}),F.addEventListener("click",T=>{T.stopPropagation(),T.preventDefault(),p!==c.LATEST&&(p=c.LATEST,C.classList.add("time"),C.classList.remove("hot"),b.scrollTo(0,0),I(b));});}async function I(b){const{data:C,code:B}=await S();if(a=C.upper.mid,l.innerHTML="",m={},document.querySelector(".comment-container .reply-warp .no-more-replies-info")?.remove(),document.querySelector(".comment-container .reply-warp .anchor-for-loading")?.remove(),B!==0){const $=B===12061?"UP主已关闭评论区":"无法从API获取评论数据";l.innerHTML=`<p style="padding: 100px 0; text-align: center; color: #999;">${$}</p>`;return}const F=b.querySelector(".comment-container .reply-header .total-reply"),T=parseInt(C?.cursor?.all_count)||0;if(F.textContent=T,C?.cursor?.name?.includes("精选")){const $=b.querySelector(".comment-container .reply-header .nav-sort");$.innerHTML='<div class="selected-sort">精选评论</div>';}if(C.top_replies&&C.top_replies.length!==0){const $=C.top_replies[0];_($,true);}for(const $ of C.replies)_($);if(C.replies.length===0||C.cursor.is_end){const $=document.createElement("p");$.classList.add("no-more-replies-info"),$.style="padding-bottom: 100px; text-align: center; color: #999;",$.textContent="没有更多评论",document.querySelector(".comment-container .reply-warp").appendChild($);return}z();}async function S(){const b={pagination_str:JSON.stringify({offset:d||""}),oid:i,type:o,wts:parseInt(Date.now()/1e3),plat:1,web_location:1315875};p===c.HOT?(b.mode=3,d||(b.seek_rpid="")):p===c.LATEST&&(b.mode=2);const C=await K.get(`https://api.bilibili.com/x/v2/reply/wbi/main?${await Pr(b)}`,{fetch:true}),B=h.toJSON(C.data.responseText);return d=B.data.cursor?.pagination_reply?.next_offset||"",B}function _(b,C){if(m[b.rpid_str])return;const B=document.createElement("div");B.classList.add("reply-item"),B.innerHTML=`
      ...
      `,l.appendChild(B),m[b.rpid_str]=true;const F=B.querySelector(".preview-image-container");F&&new fr(F,{title:false,toolbar:false,tooltip:false,keyboard:false});const T=B.querySelector(".sub-reply-list"),$=B.querySelector(".view-more-btn");$&&$.addEventListener("click",()=>q(b.rpid,T,b.rcount,1));}function D(b){const C=new Date(b*1e3),B=C.getFullYear(),F=(C.getMonth()+1).toString().padStart(2,"0"),T=C.getDate().toString().padStart(2,"0"),$=C.getHours().toString().padStart(2,"0"),Q=C.getMinutes().toString().padStart(2,"0");return `${B}-${F}-${T} ${$}:${Q}`}function k(b){return {0:"#C0C0C0",1:"#BBBBBB",2:"#8BD29B",3:"#7BCDEF",4:"#FEBB8B",5:"#EE672A",6:"#F04C49"}[b]}function O(b){let C=b.message;const B=["https://www.bilibili.com/video/av","https://b23.tv/mall-"];if(b.vote&&b.vote.deleted===false){const F=`<a class="jump-link normal" href="${b.vote.url}" target="_blank" noopener noreferrer>${b.vote.title}</a>`;B.push(F),C=C.replace(`{vote:${b.vote.id}}`,F);}if(b.emote)for(const[F,T]of Object.entries(b.emote)){const $=`<img class="emoji-${["","small","large"][T.meta.size]}" src="${T.url}" alt="${F}">`;B.push($),C=C.replaceAll(F,$);}if(C=C.replaceAll(/(\d{1,2}[:：]){1,2}\d{1,2}/g,F=>{if(F=F.replaceAll("：",":"),!t.test(e.location.href))return F;const T=F.split(":");if(T.some(Z=>parseInt(Z)>=60))return F;let $;if(T.length===2?$=parseInt(T[0])*60+parseInt(T[1]):T.length===3&&($=parseInt(T[0])*3600+parseInt(T[1])*60+parseInt(T[2])),Number.isNaN($))return F;const Q=`<a class="jump-link video-time" onclick="(async () => {
          // jump to exact time
        ...
        })()">${F}</a>`;return B.push(Q),Q}),b.at_name_to_mid)for(const[F,T]of Object.entries(b.at_name_to_mid)){const $=`<a class="jump-link user" data-user-id="${T}" href="https://space.bilibili.com/${T}" target="_blank" noopener noreferrer>@${F}</a>`;B.push($),C=C.replaceAll(`@${F}`,$);}if(Object.keys(b.jump_url).length){const F=[].concat(Object.entries(b.jump_url).filter(T=>T[0].startsWith("https://")),Object.entries(b.jump_url).filter(T=>!T[0].startsWith("https://")));for(const[T,$]of F){const Q=T.startsWith("BV")||/^av\d+$/.test(T)?`https://www.bilibili.com/video/${T}`:$.pc_url||T;if(Q.includes("search.bilibili.com")&&B.join("").includes(T))continue;const Z=`<img class="icon normal" src="${$.prefix_icon}" style="${$.extra&&$.extra.is_word_search&&"width: 12px;"}"><a class="jump-link normal" href="${Q}" target="_blank" noopener noreferrer>${$.title}</a>`;B.push(Z),C=C.replaceAll(T,Z);}}return C}function V(b){let C="width: 84px; height: 84px;";b.length===1&&(C="max-width: 260px; max-height: 180px;"),b.length===2&&(C="width: 128px; height: 128px;");let B="";for(const F of b)B+=`<div class="image-item-wrap" style="margin-top: 4px; margin-right: 4px; cursor: zoom-in;"><img src="${F.img_src}" style="border-radius: 4px; ${C}"></div>`;return B}function N(b){if(!(b instanceof Array))return "";let C="";for(const B of b)C+=`
        ...
        `;return C}async function q(b,C,B,F){const T={oid:i,type:o,root:b,ps:10,pn:F,web_location:333.788},$=await K.get(`https://api.bilibili.com/x/v2/reply/reply?${h.toSearchParamsStr(T)}`,{allowInterceptConfig:false,fetch:true});if(!$.status){log.error($),w.error("请求异常，获取评论的回复失败");return}const Q=h.toJSON($.data.responseText);if(Q===-352){w.error("请登录后再进行操作"),console.error("you should login first",$);return}const Z=Q.data;C.innerHTML=N(Z.replies),W(b,C,B,F),C.parentElement.parentElement.scrollIntoView({behavior:"instant"}),e.scrollTo(0,document.documentElement.scrollTop-60);}function W(b,C,B,F){if(B<=10)return;const T=Math.ceil(B/10),$=document.createElement("div");$.classList.add("view-more"),$.innerHTML=`
        <div class="view-more-pagination">
          ...
          ${(()=>{const Q=[F-4,F-3,F-2,F-1].filter(le=>le>=1),Z=[F+1,F+2,F+3,F+4].filter(le=>le<=T),ve=[].concat(Q,F,Z);let ae;F<=3?ae=ve.slice(0,5):F>=T-3?ae=ve.reverse().slice(0,5).reverse():ae=ve.slice(ve.indexOf(F)-2,ve.indexOf(F)+3);let oe=JSON.parse(JSON.stringify(ae));if(!oe.includes(1)){let le=[1];oe.at(0)!==2&&(le=[1,"..."]),oe=[].concat(le,oe);}if(!oe.includes(T)){let le=[T];oe.at(-1)!==T-1&&(le=["...",T]),oe=[].concat(oe,le);}return oe.reduce((le,qe)=>qe==="..."?le+'<span class="pagination-page-dot">...</span>':qe===F?le+`<span class="pagination-page-number current-page">${qe}</span>`:le+`<span class="pagination-page-number">${qe}</span>`,"")})()}
          ${F!==T?'<span class="pagination-btn pagination-to-next-btn">下一页</span>':""}
        </div>
      `,$.querySelector(".pagination-to-prev-btn")?.addEventListener("click",()=>q(b,C,B,F-1)),$.querySelector(".pagination-to-next-btn")?.addEventListener("click",()=>q(b,C,B,F+1)),$.querySelectorAll(".pagination-page-number:not(.current-page)")?.forEach(Q=>{const Z=parseInt(Q.textContent);Q.addEventListener("click",()=>q(b,C,B,Z));}),C.appendChild($);}function z(){const b=document.createElement("div");b.classList.add("anchor-for-loading"),b.textContent="正在加载...",b.style="text-align: center; color: #61666d; transform: translateY(-50px);",document.querySelector(".comment-container .reply-warp").appendChild(b);const C=new IntersectionObserver(async B=>{if(!B[0].isIntersecting)return;const{data:F}=await S();if(!F.replies||F.replies.length===0){b.textContent="所有评论已加载完毕",C.disconnect();return}for(const T of F.replies)_(T);});C.observe(b);}function M(){const b=XMLHttpRequest.prototype.open;XMLHttpRequest.prototype.open=function(){const C=arguments[1];if(typeof C=="string"&&C.includes("reply/wbi/main")){const{searchParams:B}=new URL(`${C.startsWith("//")?"https:":""}${C}`);e.dynamicDetail={oid:B.get("oid"),commentType:B.get("type")};}return b.apply(this,arguments)};}async function R(){await new Promise($=>{const Q=setInterval(()=>{document&&document.createElement&&document.head&&document.head.appendChild&&(clearInterval(Q),$());},100);});const b=document.createElement("style");b.textContent=`
        .reply-header {
          padding: 12px;
      ...
      `,document.head.appendChild(b);const C=document.createElement("style");C.textContent=`
        .reply-list {
      ...
      `,document.head.appendChild(C);const B=document.createElement("style");B.textContent=`
...
`;class qr{isBacking=false;config;constructor(t){this.config=t,this.enterGestureBackMode=this.enterGestureBackMode.bind(this),this.quitGestureBackMode=this.quitGestureBackMode.bind(this),this.popStateEvent=this.popStateEvent.bind(this),(typeof this.config.backDelayTime!="number"||isNaN(this.config.backDelayTime))&&(this.config.backDelayTime=150),this.config.win==null&&(this.config.win=self);}popStateEvent(t){s.preventEvent(t),!this.isBacking&&this.quitGestureBackMode(true);}enterGestureBackMode(){u.success("进入手势模式");let t=this.config.hash;t.startsWith("#")||(t.startsWith("/")||(t="/"+t),t="#"+t),this.config.useUrl&&(t=this.config.win.location.origin+this.config.win.location.pathname+this.config.win.location.search+t),this.config.win.history.pushState({},"",t),u.success("监听popstate事件"),s.on(this.config.win,"popstate",this.popStateEvent,{capture:true});}async quitGestureBackMode(t=false){this.isBacking=true,u.success("退出手势模式"),typeof this.config.beforeHistoryBackCallBack=="function"&&this.config.beforeHistoryBackCallBack(t);let r=Date.now()+1e3*5;for(;;){if(Date.now()>r){u.error("未知情况，history.back()失败，无法退出手势模式");break}if(this.config.win.location.hash.endsWith(this.config.hash))u.info("history.back()"),this.config.win.history.back(),await Fe.sleep(this.config.backDelayTime||150);else break}u.success("移除popstate事件"),s.off(this.config.win,"popstate",this.popStateEvent,{capture:true}),this.isBacking=false,typeof this.config.afterHistoryBackCallBack=="function"&&this.config.afterHistoryBackCallBack(t);}}const Gr={$data:{isAddBeautifyCSS:false,isInitCommentModule:false,isInitDescModule:false},init(){dt.init(),f.execMenuOnce("bili-video-cover-bottomRecommendVideo",()=>{this.coverBottomRecommendVideo();}),f.execMenuOnce("bili-video-cover-UpWrapper",()=>{this.coverUpWrapper();}),f.execMenuOnce("bili-video-cover-seasonNew",()=>{this.coverSeasonNew();}),s.ready(()=>{f.execMenu("bili-video-addCommentModule",()=>{this.addCommentModule();}),f.execMenu("bili-video-addDescModule",()=>{this.addDescModule();});});},beautify(){u.info("美化显示"),this.$data.isAddBeautifyCSS||(this.$data.isAddBeautifyCSS=true,H(`
			...
			`)),s.waitNode(x.className.video+" .bottom-tab .list-view .card-box",1e4).then(e=>{if(!e){u.error("$cardBox is null");return}function t(a){let o=a.querySelector(".title"),l=a.querySelector(".count .left"),c=!!a.querySelector(".gm-right-container"),p=L.getVue(a);if(o&&l&&p&&!c){let d=p?.info?.owner?.name;if(d==null){u.error("美化显示-handleVCardToApp：获取up主名字失败");return}a.querySelector(".count");let m=o.cloneNode(true),g=l.cloneNode(true);s.hide(o);let y=a.querySelector(".open-app.weakened");y&&s.hide(y);let E=document.createElement("div");E.className="gm-up-name",E.innerHTML=`
						...
						`;let I=document.createElement("div"),S=document.createElement("div");I.className="gm-right-container",S.className="gm-right-bottom",s.after(o,I),I.appendChild(m),I.appendChild(S),S.appendChild(E),S.appendChild(g);}}function r(a){let o=a.querySelector(".title"),l=a.querySelector(".count"),c=!!a.querySelector(".gm-right-container"),p=L.getVue(a);if(o&&l&&p&&!c){let d=p?.info?.duration;if(d==null){u.error("美化显示-handleVCard：获取视频时长失败");return}let m=p?.info?.owner?.name;if(m==null){u.error("美化显示-handleVCard：获取up主名字失败");return}let g=o.cloneNode(true),y=l.cloneNode(true);s.hide(o);let E=document.createElement("div");E.className="duration",E.innerText=j.parseDuration(d),y.className="left";let I=document.createElement("div");l.appendChild(E),I.className="gm-up-name",I.innerHTML=`
						<svg viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg">
							<path fill="#999A9E" d="M896 736v-448c0-54.4-41.6-96-96-96h-576C169.6 192 128 233.6 128 288v448c0 54.4 41.6 96 96 96h576c54.4 0 96-41.6 96-96zM800 128C889.6 128 960 198.4 960 288v448c0 89.6-70.4 160-160 160h-576C134.4 896 64 825.6 64 736v-448C64 198.4 134.4 128 224 128h576zM419.2 544V326.4h60.8v240c0 96-57.6 144-147.2 144S192 665.6 192 569.6V326.4h60.8v217.6c0 51.2 3.2 108.8 83.2 108.8s83.2-57.6 83.2-108.8z m288-38.4c28.8 0 60.8-16 60.8-60.8 0-48-28.8-60.8-60.8-60.8H614.4v121.6h92.8z m3.2-179.2c102.4 0 121.6 70.4 121.6 115.2 0 48-19.2 115.2-121.6 115.2H614.4V704h-60.8V326.4h156.8z">
							</path>
						</svg>
						<span class="gm-up-name-text">${m}</span>
						`;let S=document.createElement("div"),_=document.createElement("div");S.className="gm-right-container",_.className="gm-right-bottom",s.after(o,S),S.appendChild(g),S.appendChild(_),_.appendChild(I),_.appendChild(y);}}let n=new h.LockFunction(()=>{let a=document.querySelectorAll(x.className.video+" .bottom-tab .list-view .card-box .v-card-toapp"),o=document.querySelectorAll(x.className.video+" .bottom-tab .list-view .card-box>a.v-card");a.forEach(l=>{t(l);}),o.forEach(l=>{r(l);});},25),i=document.querySelector(x.className.video);i?h.mutationObserver(i,{config:{subtree:true,attributes:true,childList:true},callback(){n.run();}}):u.error("未找到视频根节点");});},repairVideoBottomAreaHeight(){return u.info("修复视频底部区域高度"),H(`
		${x.className.video},
		${x.className.mVideo} {
			/* 修复视频区域底部的高度 */
...
`,dr={$flag_css:{enableOtherAreaSearchBangumi:false},init(){H(tn),s.ready(()=>{f.execMenu("bili-search-enableOtherAreaSearchBangumi",()=>{this.enableOtherAreaSearchBangumi();});});},enableOtherAreaSearchBangumi(){this.$flag_css.enableOtherAreaSearchBangumi||(this.$flag_css.enableOtherAreaSearchBangumi=true,H(`
			...
			`)),s.waitNode(".m-search-result .tabs:not(:has(.gm-tab-item))").then(e=>{pt.getSearchProxyHost().forEach(n=>{let i=s.createElement("a",{className:"tab-item gm-tab-item",innerHTML:`番剧（${n.name}）`},{"data-area":n.area,"data-host":n.host});e.appendChild(i);});const r=n=>{e.querySelectorAll(".tab-item").forEach(i=>n!=i&&i.classList.remove("on")),n.classList.add("on");};s.on(e,"click",".tab-item",async n=>{let i=n.target;r(i);let a=document.querySelector(".result-panel"),o=document.querySelector(".gm-result-panel");if(o&&(o.remove(),s.show(a)),!i.classList.contains("gm-tab-item"))return;let l=i.dataset.area,c=i.dataset.host,p=document.querySelector(".m-search-result"),d=L.getVue(p);d.switchTab(233),s.hide(a);let m=d.keyword,g=w.loading("搜索中，请稍后..."),y=await pr.getBangumiSearchResult({keyword:m,area:l,host:c});if(g.close(),!y)return;if(!y.isSuccess){alert(JSON.stringify(y.data,null,2));return}let E=y.data;u.info("搜索结果：",E);let I=s.createElement("div",{className:"gm-result-panel",innerHTML:`
					...
					`}),S=I.querySelector(".gm-card-box");E.forEach(_=>{S.appendChild(this.createSearchResultVideoItem(_));}),p.appendChild(I);});});},createSearchResultVideoItem(e){let t=s.createElement("div",{className:"gm-card-item",innerHTML:`
