...
`)}
      ...
      `;}async function _(v){const A=v.querySelector(".comment-container .reply-header .nav-sort"),$=A.querySelector(".hot-sort"),F=A.querySelector(".time-sort");A.classList.add("hot"),A.classList.remove("time"),$.addEventListener("click",S=>{S.stopPropagation(),S.preventDefault(),p!==s.HOT&&(p=s.HOT,A.classList.add("hot"),A.classList.remove("time"),v.scrollTo(0,0),k(v));}),F.addEventListener("click",S=>{S.stopPropagation(),S.preventDefault(),p!==s.LATEST&&(p=s.LATEST,A.classList.add("time"),A.classList.remove("hot"),v.scrollTo(0,0),k(v));});}async function k(v){var R,U,G,ge,me;const{data:A,code:$}=await L();if(a=A.upper.mid,l.innerHTML="",m={},(R=document.querySelector(".comment-container .reply-warp .no-more-replies-info"))==null||R.remove(),(U=document.querySelector(".comment-container .reply-warp .anchor-for-loading"))==null||U.remove(),$!==0){const Z=$===12061?"UP主已关闭评论区":"无法从API获取评论数据";l.innerHTML=`<p style="padding: 100px 0; text-align: center; color: #999;">${Z}</p>`;return}const F=v.querySelector(".comment-container .reply-header .total-reply"),S=parseInt((G=A==null?void 0:A.cursor)==null?void 0:G.all_count)||0;if(F.textContent=S,(me=(ge=A==null?void 0:A.cursor)==null?void 0:ge.name)!=null&&me.includes("精选")){const Z=v.querySelector(".comment-container .reply-header .nav-sort");Z.innerHTML='<div class="selected-sort">精选评论</div>';}if(A.top_replies&&A.top_replies.length!==0){const Z=A.top_replies[0];V(Z,true);}for(const Z of A.replies)V(Z);if(A.replies.length===0||A.cursor.is_end){const Z=document.createElement("p");Z.classList.add("no-more-replies-info"),Z.style="padding-bottom: 100px; text-align: center; color: #999;",Z.textContent="没有更多评论",document.querySelector(".comment-container .reply-warp").appendChild(Z);return}Y();}async function L(){var F,S;const v={pagination_str:JSON.stringify({offset:c||""}),oid:i,type:o,wts:parseInt(Date.now()/1e3),plat:1,web_location:1315875};p===s.HOT?(v.mode=3,c||(v.seek_rpid="")):p===s.LATEST&&(v.mode=2);const A=await K.get(`https://api.bilibili.com/x/v2/reply/wbi/main?${await Mr(v)}`,{fetch:true}),$=d.toJSON(A.data.responseText);return c=((S=(F=$.data.cursor)==null?void 0:F.pagination_reply)==null?void 0:S.next_offset)||"",$}function V(v,A){if(m[v.rpid_str])return;const $=document.createElement("div");$.classList.add("reply-item"),$.innerHTML=`
          ...
          <div class="content-warp">
            ...
            <div class="root-reply">
              ...
              <div class="reply-info">
                <span class="reply-time" style="margin-right: 20px;">${D(v.ctime)}</span>
      ...
      `,l.appendChild($),m[v.rpid_str]=true;const F=$.querySelector(".preview-image-container");F&&new hr(F,{title:false,toolbar:false,tooltip:false,keyboard:false});const S=$.querySelector(".sub-reply-list"),R=$.querySelector(".view-more-btn");R&&R.addEventListener("click",()=>j(v.rpid,S,v.rcount,1));}function D(v){const A=new Date(v*1e3),$=A.getFullYear(),F=(A.getMonth()+1).toString().padStart(2,"0"),S=A.getDate().toString().padStart(2,"0"),R=A.getHours().toString().padStart(2,"0"),U=A.getMinutes().toString().padStart(2,"0");return `${$}-${F}-${S} ${R}:${U}`}function f(v){return {0:"#C0C0C0",1:"#BBBBBB",2:"#8BD29B",3:"#7BCDEF",4:"#FEBB8B",5:"#EE672A",6:"#F04C49"}[v]}function C(v){let A=v.message;const $=["https://www.bilibili.com/video/av","https://b23.tv/mall-"];if(v.vote&&v.vote.deleted===false){const F=`<a class="jump-link normal" href="${v.vote.url}" target="_blank" noopener noreferrer>${v.vote.title}</a>`;$.push(F),A=A.replace(`{vote:${v.vote.id}}`,F);}if(v.emote)for(const[F,S]of Object.entries(v.emote)){const R=`<img class="emoji-${["","small","large"][S.meta.size]}" src="${S.url}" alt="${F}">`;$.push(R),A=A.replaceAll(F,R);}if(A=A.replaceAll(/(\d{1,2}[:：]){1,2}\d{1,2}/g,F=>{if(F=F.replaceAll("：",":"),!t.test(e.location.href))return F;const S=F.split(":");if(S.some(G=>parseInt(G)>=60))return F;let R;if(S.length===2?R=parseInt(S[0])*60+parseInt(S[1]):S.length===3&&(R=parseInt(S[0])*3600+parseInt(S[1])*60+parseInt(S[2])),Number.isNaN(R))return F;const U=`<a class="jump-link video-time" onclick="(async () => {
          // jump to exact time
        ...
        })()">${F}</a>`;return $.push(U),U}),v.at_name_to_mid)for(const[F,S]of Object.entries(v.at_name_to_mid)){const R=`<a class="jump-link user" data-user-id="${S}" href="https://space.bilibili.com/${S}" target="_blank" noopener noreferrer>@${F}</a>`;$.push(R),A=A.replaceAll(`@${F}`,R);}if(Object.keys(v.jump_url).length){const F=[].concat(Object.entries(v.jump_url).filter(S=>S[0].startsWith("https://")),Object.entries(v.jump_url).filter(S=>!S[0].startsWith("https://")));for(const[S,R]of F){const U=S.startsWith("BV")||/^av\d+$/.test(S)?`https://www.bilibili.com/video/${S}`:R.pc_url||S;if(U.includes("search.bilibili.com")&&$.join("").includes(S))continue;const G=`<img class="icon normal" src="${R.prefix_icon}" style="${R.extra&&R.extra.is_word_search&&"width: 12px;"}"><a class="jump-link normal" href="${U}" target="_blank" noopener noreferrer>${R.title}</a>`;$.push(G),A=A.replaceAll(S,G);}}return A}function M(v){let A="width: 84px; height: 84px;";v.length===1&&(A="max-width: 260px; max-height: 180px;"),v.length===2&&(A="width: 128px; height: 128px;");let $="";for(const F of v)$+=`<div class="image-item-wrap" style="margin-top: 4px; margin-right: 4px; cursor: zoom-in;"><img src="${F.img_src}" style="border-radius: 4px; ${A}"></div>`;return $}function P(v){if(!(v instanceof Array))return "";let A="";for(const $ of v)A+=`
          <div class="sub-reply-item">
            <div class="sub-user-info">
              ...
              <span style="height: 14px; padding: 0 2px; margin-right: 4px; display: flex; align-items: center; font-size: 10px; color: white; border-radius: 2px; background-color: ${f($.member.level_info.current_level)};">LV${$.member.level_info.current_level}</span>
            ...
            <span class="reply-content-container sub-reply-content">
              <span class="reply-content">${C($.content)}</span>
            </span>
        ...
        `;return A}async function j(v,A,$,F){const S={oid:i,type:o,root:v,ps:10,pn:F,web_location:333.788},R=await K.get(`https://api.bilibili.com/x/v2/reply/reply?${fe.toSearchParamsStr(S)}`,{allowInterceptConfig:false,fetch:true});if(!R.status){log.error(R),E.error("请求异常，获取评论的回复失败");return}const U=d.toJSON(R.data.responseText);if(U===-352){E.error("请登录后再进行操作"),console.error("you should login first",R);return}const G=U.data;A.innerHTML=P(G.replies),J(v,A,$,F),A.parentElement.parentElement.scrollIntoView({behavior:"instant"}),e.scrollTo(0,document.documentElement.scrollTop-60);}function J(v,A,$,F){var U,G,ge;if($<=10)return;const S=Math.ceil($/10),R=document.createElement("div");R.classList.add("view-more"),R.innerHTML=`
        <div class="view-more-pagination">
          ...
          ${(()=>{const me=[F-4,F-3,F-2,F-1].filter(le=>le>=1),Z=[F+1,F+2,F+3,F+4].filter(le=>le<=S),ye=[].concat(me,F,Z);let _e;F<=3?_e=ye.slice(0,5):F>=S-3?_e=ye.reverse().slice(0,5).reverse():_e=ye.slice(ye.indexOf(F)-2,ye.indexOf(F)+3);let ae=JSON.parse(JSON.stringify(_e));if(!ae.includes(1)){let le=[1];ae.at(0)!==2&&(le=[1,"..."]),ae=[].concat(le,ae);}if(!ae.includes(S)){let le=[S];ae.at(-1)!==S-1&&(le=["...",S]),ae=[].concat(ae,le);}return ae.reduce((le,Ue)=>Ue==="..."?le+'<span class="pagination-page-dot">...</span>':Ue===F?le+`<span class="pagination-page-number current-page">${Ue}</span>`:le+`<span class="pagination-page-number">${Ue}</span>`,"")})()}
          ${F!==S?'<span class="pagination-btn pagination-to-next-btn">下一页</span>':""}
        </div>
      `,(U=R.querySelector(".pagination-to-prev-btn"))==null||U.addEventListener("click",()=>j(v,A,$,F-1)),(G=R.querySelector(".pagination-to-next-btn"))==null||G.addEventListener("click",()=>j(v,A,$,F+1)),(ge=R.querySelectorAll(".pagination-page-number:not(.current-page)"))==null||ge.forEach(me=>{const Z=parseInt(me.textContent);me.addEventListener("click",()=>j(v,A,$,Z));}),A.appendChild(R);}function Y(){const v=document.createElement("div");v.classList.add("anchor-for-loading"),v.textContent="正在加载...",v.style="text-align: center; color: #61666d; transform: translateY(-50px);",document.querySelector(".comment-container .reply-warp").appendChild(v);const A=new IntersectionObserver(async $=>{if(!$[0].isIntersecting)return;const{data:F}=await L();if(!F.replies||F.replies.length===0){v.textContent="所有评论已加载完毕",A.disconnect();return}for(const S of F.replies)V(S);});A.observe(v);}function H(){const v=XMLHttpRequest.prototype.open;XMLHttpRequest.prototype.open=function(){const A=arguments[1];if(typeof A=="string"&&A.includes("reply/wbi/main")){const{searchParams:$}=new URL(`${A.startsWith("//")?"https:":""}${A}`);e.dynamicDetail={oid:$.get("oid"),commentType:$.get("type")};}return v.apply(this,arguments)};}async function q(){await new Promise(R=>{const U=setInterval(()=>{document&&document.createElement&&document.head&&document.head.appendChild&&(clearInterval(U),R());},100);});const v=document.createElement("style");v.textContent=`
        .reply-header {
          padding: 12px;
      ...
      `,document.head.appendChild($);const F=document.createElement("style");F.textContent=`
...
`;class zr{constructor(t){te(this,"isBacking",false);te(this,"config");this.config=t,this.enterGestureBackMode=this.enterGestureBackMode.bind(this),this.quitGestureBackMode=this.quitGestureBackMode.bind(this),this.popStateEvent=this.popStateEvent.bind(this),(typeof this.config.backDelayTime!="number"||isNaN(this.config.backDelayTime))&&(this.config.backDelayTime=150),this.config.win==null&&(this.config.win=self);}popStateEvent(t){fe.preventEvent(t),!this.isBacking&&this.quitGestureBackMode(true);}enterGestureBackMode(){u.success("进入手势模式");let t=this.config.hash;t.startsWith("#")||(t.startsWith("/")||(t="/"+t),t="#"+t),this.config.useUrl&&(t=this.config.win.location.origin+this.config.win.location.pathname+this.config.win.location.search+t),this.config.win.history.pushState({},"",t),u.success("监听popstate事件"),h.on(this.config.win,"popstate",this.popStateEvent,{capture:true});}async quitGestureBackMode(t=false){this.isBacking=true,u.success("退出手势模式"),typeof this.config.beforeHistoryBackCallBack=="function"&&this.config.beforeHistoryBackCallBack(t);let r=Date.now()+1e3*5;for(;;){if(Date.now()>r){u.error("未知情况，history.back()失败，无法退出手势模式");break}if(this.config.win.location.hash.endsWith(this.config.hash))u.info("history.back()"),this.config.win.history.back(),await fe.sleep(this.config.backDelayTime||150);else break}u.success("移除popstate事件"),h.off(this.config.win,"popstate",this.popStateEvent,{capture:true}),this.isBacking=false,typeof this.config.afterHistoryBackCallBack=="function"&&this.config.afterHistoryBackCallBack(t);}}const Or={$data:{isAddBeautifyCSS:false,isInitCommentModule:false,isInitDescModule:false},init(){lt.init(),g.execMenuOnce("bili-video-cover-bottomRecommendVideo",()=>{this.coverBottomRecommendVideo();}),g.execMenuOnce("bili-video-cover-UpWrapper",()=>{this.coverUpWrapper();}),g.execMenuOnce("bili-video-cover-seasonNew",()=>{this.coverSeasonNew();}),h.ready(()=>{g.execMenu("bili-video-addCommentModule",()=>{this.addCommentModule();}),g.execMenu("bili-video-addDescModule",()=>{this.addDescModule();});});},beautify(){u.info("美化显示"),this.$data.isAddBeautifyCSS||(this.$data.isAddBeautifyCSS=true,O(`
			...
			`)),d.waitNode(x.className.video+" .bottom-tab .list-view .card-box",1e4).then(e=>{if(!e){u.error("$cardBox is null");return}function t(a){var c,m;let o=a.querySelector(".title"),l=a.querySelector(".count .left"),s=!!a.querySelector(".gm-right-container"),p=T.getVue(a);if(o&&l&&p&&!s){let y=(m=(c=p==null?void 0:p.info)==null?void 0:c.owner)==null?void 0:m.name;if(y==null){u.error("美化显示-handleVCardToApp：获取up主名字失败");return}a.querySelector(".count");let B=o.cloneNode(true),_=l.cloneNode(true);h.hide(o);let k=a.querySelector(".open-app.weakened");k&&h.hide(k);let L=document.createElement("div");L.className="gm-up-name",L.innerHTML=`
						...
						`;let V=document.createElement("div"),D=document.createElement("div");V.className="gm-right-container",D.className="gm-right-bottom",h.after(o,V),V.appendChild(B),V.appendChild(D),D.appendChild(L),D.appendChild(_);}}function r(a){var c,m,y;let o=a.querySelector(".title"),l=a.querySelector(".count"),s=!!a.querySelector(".gm-right-container"),p=T.getVue(a);if(o&&l&&p&&!s){let B=(c=p==null?void 0:p.info)==null?void 0:c.duration;if(B==null){u.error("美化显示-handleVCard：获取视频时长失败");return}let _=(y=(m=p==null?void 0:p.info)==null?void 0:m.owner)==null?void 0:y.name;if(_==null){u.error("美化显示-handleVCard：获取up主名字失败");return}let k=o.cloneNode(true),L=l.cloneNode(true);h.hide(o);let V=document.createElement("div");V.className="duration",V.innerText=z.parseDuration(B),L.className="left";let D=document.createElement("div");l.appendChild(V),D.className="gm-up-name",D.innerHTML=`
						<svg viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg">
							<path fill="#999A9E" d="M896 736v-448c0-54.4-41.6-96-96-96h-576C169.6 192 128 233.6 128 288v448c0 54.4 41.6 96 96 96h576c54.4 0 96-41.6 96-96zM800 128C889.6 128 960 198.4 960 288v448c0 89.6-70.4 160-160 160h-576C134.4 896 64 825.6 64 736v-448C64 198.4 134.4 128 224 128h576zM419.2 544V326.4h60.8v240c0 96-57.6 144-147.2 144S192 665.6 192 569.6V326.4h60.8v217.6c0 51.2 3.2 108.8 83.2 108.8s83.2-57.6 83.2-108.8z m288-38.4c28.8 0 60.8-16 60.8-60.8 0-48-28.8-60.8-60.8-60.8H614.4v121.6h92.8z m3.2-179.2c102.4 0 121.6 70.4 121.6 115.2 0 48-19.2 115.2-121.6 115.2H614.4V704h-60.8V326.4h156.8z">
							</path>
						</svg>
						<span class="gm-up-name-text">${_}</span>
						`;let f=document.createElement("div"),C=document.createElement("div");f.className="gm-right-container",C.className="gm-right-bottom",h.after(o,f),f.appendChild(k),f.appendChild(C),C.appendChild(D),C.appendChild(L);}}let n=new d.LockFunction(()=>{let a=document.querySelectorAll(x.className.video+" .bottom-tab .list-view .card-box .v-card-toapp"),o=document.querySelectorAll(x.className.video+" .bottom-tab .list-view .card-box>a.v-card");a.forEach(l=>{t(l);}),o.forEach(l=>{r(l);});},25),i=document.querySelector(x.className.video);i?d.mutationObserver(i,{config:{subtree:true,attributes:true,childList:true},callback(){n.run();}}):u.error("未找到视频根节点");});},repairVideoBottomAreaHeight(){return u.info("修复视频底部区域高度"),O(`
		${x.className.video},
		${x.className.mVideo} {
			/* 修复视频区域底部的高度 */
...
`,sr={$flag_css:{enableOtherAreaSearchBangumi:false},init(){O(Wr),h.ready(()=>{g.execMenu("bili-search-enableOtherAreaSearchBangumi",()=>{this.enableOtherAreaSearchBangumi();});});},enableOtherAreaSearchBangumi(){this.$flag_css.enableOtherAreaSearchBangumi||(this.$flag_css.enableOtherAreaSearchBangumi=true,O(`
			...
			`)),d.waitNode(".m-search-result .tabs:not(:has(.gm-tab-item))").then(e=>{ot.getSearchProxyHost().forEach(n=>{let i=h.createElement("a",{className:"tab-item gm-tab-item",innerHTML:`番剧（${n.name}）`},{"data-area":n.area,"data-host":n.host});e.appendChild(i);});const r=n=>{e.querySelectorAll(".tab-item").forEach(i=>n!=i&&i.classList.remove("on")),n.classList.add("on");};h.on(e,"click",".tab-item",async n=>{let i=n.target;r(i);let a=document.querySelector(".result-panel"),o=document.querySelector(".gm-result-panel");if(o&&(o.remove(),h.show(a)),!i.classList.contains("gm-tab-item"))return;let l=i.dataset.area,s=i.dataset.host,p=document.querySelector(".m-search-result"),c=T.getVue(p);c.switchTab(233),h.hide(a);let m=c.keyword,y=E.loading("搜索中，请稍后..."),B=await ur.getBangumiSearchResult({keyword:m,area:l,host:s});if(y.close(),!B)return;if(!B.isSuccess){alert(JSON.stringify(B.data,null,2));return}let _=B.data;u.info("搜索结果：",_);let k=h.createElement("div",{className:"gm-result-panel",innerHTML:`
					...
					`}),L=k.querySelector(".gm-card-box");_.forEach(V=>{L.appendChild(this.createSearchResultVideoItem(V));}),p.appendChild(k);});});},createSearchResultVideoItem(e){var s,p;let t=h.createElement("div",{className:"gm-card-item",innerHTML:`
				...
				<div class="gm-card-ep-info-container">
					${m}
				</div>`},{"data-id":c.id,"data-url":y,"data-title":m,"data-long_title":c.long_title}),_=B.querySelector(".gm-card-ep-badges-container");if(B.querySelector(".gm-card-ep-info-container"),Array.isArray(c.badges)&&c.badges.length){let k=c.badges[0],L=h.createElement("span",{className:"gm-card-ep-badge-top-right",innerText:k.text});typeof k.bg_color=="string"&&(L.style.backgroundColor=k.bg_color),typeof k.text_color=="string"&&(L.style.color=k.text_color),h.append(_,L);}h.on(B,"click",k=>{d.preventEvent(k),window.open(y,"_blank");}),o.appendChild(B);}),t},searchBangumi(){}},Jr={$flag:{mutationSearchResult:false},init(){this.mutationSearchResult();},mutationSearchResult(){this.$flag.mutationSearchResult||(this.$flag.mutationSearchResult=true,O(`
        .bangumi-list{
            padding: 0 10px;
        }
...
`):s},callback(s,p){this.props[W].set(t,p);}};return Reflect.set(l.attributes,ke,t),Reflect.set(l.attributes,Fe,r),je.setComponentsStorageApiProperty("switch",l,{get(s,p){return g.getValue(s,p)},set(s,p){g.setValue(s,p);}}),l};class mn{constructor(t){te(this,"option");this.option=t;}async showView(){var o;let t=ee.confirm({title:{text:this.option.title,position:"center"},content:{text:`
            ...
            `}),r=t.$shadowRoot.querySelector(".filter-container"),n=document.createDocumentFragment();this.option.filterOption.forEach(i=>{let a=document.createElement("button");a.innerText=i.name;let o=async()=>{(await this.option.getAllRuleInfo()).forEach(async s=>{await i.filterCallBack(s.data)?h.show(s.$el,false):h.hide(s.$el,false);}),typeof this.option.execFilterCallBack=="function"&&await this.option.execFilterCallBack(),t.close();};h.on(a,"click",async l=>{d.preventEvent(l),!(typeof i.callback=="function"&&!await i.callback(l,o))&&await o();}),n.appendChild(a);}),r.appendChild(n);}}class fn{constructor(t){te(this,"option");this.option=t;}async showView(t){var a,o,l,s,p,c,m,y,B;let r=ee.confirm({title:{text:this.option.title,position:"center"},content:{text:`
                    <div class="rule-view-container">
                    </div>
                    `,html:true},btn:{merge:true,reverse:false,position:"space-between",ok:{enable:((l=(o=(a=this.option)==null?void 0:a.bottomControls)==null?void 0:o.add)==null?void 0:l.enable)||true,type:"primary",text:"添加",callback:async _=>{this.showEditView(false,await this.option.getAddData(),r.$shadowRoot);}},close:{enable:true,callback(_){r.close();}},cancel:{enable:((c=(p=(s=this.option)==null?void 0:s.bottomControls)==null?void 0:p.filter)==null?void 0:c.enable)||false,type:"default",text:"过滤",callback:(_,k)=>{var D,f,C,M,P,j,J;typeof((C=(f=(D=this.option)==null?void 0:D.bottomControls)==null?void 0:f.filter)==null?void 0:C.callback)=="function"&&this.option.bottomControls.filter.callback();let L=()=>Array.from(r.$shadowRoot.querySelectorAll(".rule-view-container .rule-item")),V=k.target.closest(".pops-confirm-btn").querySelector(".pops-confirm-btn-cancel span");h.text(V).includes("取消")?(L().forEach(Y=>{h.show(Y,false);}),h.text(V,"过滤")):new hn({title:((P=(M=this.option.bottomControls)==null?void 0:M.filter)==null?void 0:P.title)??"过滤规则",filterOption:((J=(j=this.option.bottomControls)==null?void 0:j.filter)==null?void 0:J.option)||[],execFilterCallBack(){h.text(V,"取消过滤");},getAllRuleInfo:()=>L().map(H=>({data:this.parseRuleItemElement(H).data,$el:H}))}).showView();}},other:{enable:((B=(y=(m=this.option)==null?void 0:m.bottomControls)==null?void 0:y.clear)==null?void 0:B.enable)||true,type:"xiaomi-primary",text:`清空所有(${(await this.option.data()).length})`,callback:_=>{let k=ee.confirm({title:{text:"提示",position:"center"},content:{text:"确定清空所有的数据？",html:false},btn:{ok:{enable:true,callback:async L=>{var D,f,C;if(u.success("清空所有"),typeof((C=(f=(D=this.option)==null?void 0:D.bottomControls)==null?void 0:f.clear)==null?void 0:C.callback)=="function"&&this.option.bottomControls.clear.callback(),(await this.option.data()).length){E.error("清理失败");return}else E.success("清理成功");await this.updateDeleteAllBtnText(r.$shadowRoot),this.clearContent(r.$shadowRoot),k.close();}},cancel:{text:"取消",enable:true}},mask:{enable:true},width:"300px",height:"200px"});}}},mask:{enable:true},width:window.innerWidth>500?"500px":"88vw",height:window.innerHeight>500?"500px":"80vh",style:`
            ${ee.config.cssText.panelCSS}
            ...
            `}),n=await this.option.data(),i=false;for(let _=0;_<n.length;_++){let k=n[_],L=await this.appendRuleItemElement(r.$shadowRoot,k);(typeof t=="function"?t(k):true)||(i=true,L.forEach(D=>{h.hide(D,false);}));}if(i){let _=r.$shadowRoot.querySelector(".pops-confirm-btn-cancel span");h.text(_,"取消过滤");}}showEditView(t,r,n,i,a,o){let l=async p=>{if(p){if(typeof o=="function"){let c=await this.option.getData(r);o(c);}}else if(t||await this.option.deleteData(r),typeof a=="function"){let c=await this.option.getData(r);a(c);}};new mn({title:t?"编辑":"添加",data:()=>r,dialogCloseCallBack:l,getView:async p=>await this.option.itemControls.edit.getView(p,t),btn:{ok:{enable:true,text:t?"修改":"添加"},cancel:{callback:async(p,c)=>{p.close(),await l(false);}},close:{callback:async(p,c)=>{p.close(),await l(false);}}},onsubmit:async(p,c)=>{let m=await this.option.itemControls.edit.onsubmit(p,t,c);return m.success?t?(E.success("修改成功"),n&&await this.updateRuleItemElement(m.data,i,n)):n&&await this.appendRuleItemElement(n,m.data):t&&u.error("修改失败"),m},style:this.option.itemControls.edit.style,width:this.option.itemControls.edit.width,height:this.option.itemControls.edit.height}).showView();}parseViewElement(t){let r=t.querySelector(".rule-view-container"),n=t.querySelector(".pops-confirm-btn button.pops-confirm-btn-other");return {$container:r,$deleteBtn:n}}parseRuleItemElement(t){let r=t.querySelector(".rule-controls-enable"),n=r.querySelector(".pops-panel-switch"),i=r.querySelector(".pops-panel-switch__input"),a=r.querySelector(".pops-panel-switch__core"),o=t.querySelector(".rule-controls-edit"),l=t.querySelector(".rule-controls-delete");return {$enable:r,$enableSwitch:n,$enableSwitchInput:i,$enableSwitchCore:a,$edit:o,$delete:l,data:Reflect.get(t,"data-rule")}}async createRuleItemElement(t,r){let n=await this.option.getDataItemName(t),i=h.createElement("div",{className:"rule-item",innerHTML:`
...
`).map(v=>Number(v)).filter(v=>!isNaN(v)):q},set(Y,H){typeof H=="string"&&(H=H.split(`
`).map(q=>Number(q)).filter(q=>!isNaN(q))),n.data[Y]=H;}});let J=e.createSectionContainerItem_textarea(j);return a.append(s,c,y,_,L,D,C,P,J),a},onsubmit:(n,i,a)=>{let o=n.querySelectorAll(".rule-form-ulist > li"),l=this.getTemplateData();i&&(l.uuid=a.uuid);try{return o.forEach(s=>{let p=Reflect.get(s,"__formConfig__"),c=Reflect.get(p,"attributes"),m=Reflect.get(s,W),y=Reflect.get(c,ke),B=Reflect.get(c,Fe),_=m.get(y,B);Reflect.has(l,y)?Reflect.set(l,y,_):Reflect.has(l.data,y)?Reflect.set(l.data,y,_):u.error(`${y}不在数据中`);}),l.name.trim()===""?(E.error("规则名称不能为空"),{success:!1,data:l}):i?{success:this.updateData(l),data:l}:{success:this.addData(l),data:l}}catch(s){return u.error(s),{success:false,data:l}}finally{this.init();}},style:`
            ...
            `}),t=e.$shadowRoot.querySelector(".import-mode[data-mode='local']"),r=e.$shadowRoot.querySelector(".import-mode[data-mode='network']");h.on(t,"click",n=>{d.preventEvent(n),e.close();let i=h.createElement("input",{type:"file",accept:".json"});h.on(i,["propertychange","input"],a=>{var s;if(!((s=i.files)!=null&&s.length))return;let o=i.files[0],l=new FileReader;l.onload=()=>{let p=d.toJSON(l.result);if(!Array.isArray(p)){u.error("不是正确的规则文件",p),E.error("不是正确的规则文件");return}this.setData(p),E.success(`成功导入 ${p.length}条规则`);},l.readAsText(o,"UTF-8");}),i.click();}),h.on(r,"click",n=>{d.preventEvent(n),e.close(),ee.prompt({title:{text:"网络导入",position:"center"},content:{text:"",placeholder:"url",focus:true},btn:{ok:{callback:async(i,a)=>{let o=i.text;if(d.isNull(o)){E.error("请填入完整的url");return}let l=await K.get(o);if(!l.status)return;let s=d.toJSON(l.data.responseText);if(!Array.isArray(s)){u.error("不是正确的规则文件",l,s),E.error("不是正确的规则文件");return}this.setData(s),i.close(),E.success(`成功导入 ${s.length}条规则`);}}},width:Te.info.width,height:"auto"});});}},gn={$data:{searchIcon:`
...
`)}
				...
				`});}),t},clearLabel(e){var t;for(;;){let r=h.prev(e);if(!r)break;if((t=r==null?void 0:r.classList)!=null&&t.contains("composition-checked"))r.remove();else break}},handleShowLabel(e,t,r){if(Ee.$data.ruleData.length===0){E.warning("未配置规则，请在设置中进行添加");return}if(e=e.toString(),Ee.$data.whiteList.includes(e))return;let n=[],i=(a,o)=>{let l=n.find(s=>s.rule===a);l?l.matchedInfoList.push(o):n.push({rule:a,matchedInfoList:[o]});};Ee.$data.ruleData.forEach(a=>{if(Array.isArray(a.data.blacklist)&&a.data.blacklist.find(o=>o.toString()===e)){i(a,{reason:"黑名单用户",reasonText:e,reasonLink:De.getUserSpaceUrl(e),reasonTime:null});return}if(Array.isArray(a.data.followings)){let o="关注列表",l="";a.data.followings.some(p=>{let c=t.following.some(m=>m.mid.toString()===p.toString());return c&&(l=p.toString()),c})&&i(a,{reason:o,reasonText:l,reasonLink:De.getUserSpaceUrl(l),reasonTime:null});}Array.isArray(a.data.keywords)&&a.data.keywords.forEach(o=>{var l,s;for(let p=0;p<t.space.length;p++){const c=t.space[p];let m="",y=o,B=`/opus/${c.contentInfo.id_str}`,_=c.contentInfo.pub_ts;c.forwardInfo==null?typeof c.contentInfo.desc=="string"&&c.contentInfo.desc.match(o)?m="投稿视频简介":typeof c.contentInfo.title=="string"&&c.contentInfo.title.match(o)&&(m="投稿视频标题"):typeof c.contentInfo.desc=="string"&&c.contentInfo.desc.match(o)?m="空间动态转发":typeof((l=c.forwardInfo)==null?void 0:l.title)=="string"&&c.forwardInfo.title.match(o)?m="空间动态视频标题":typeof((s=c.forwardInfo)==null?void 0:s.desc)=="string"&&c.forwardInfo.desc.match(o)&&(m="空间动态视频简介"),m!==""&&i(a,{reason:m,reasonText:y,reasonLink:B,reasonTime:_});}});}),d.sortListByProperty(n,a=>a.matchedInfoList.length,true),n.forEach(a=>{let o=this.createLabel(a);h.before(r,o);});}},bn={$flag:{isWatchVideoChange:false},$data:{art:null},init(){},updateArtPlayerVideoInfo(e,t){const r=this;T.waitVuePropToSet(x.className.playlist+" .playlist-player",{msg:"等待覆盖playlist播放器",check(n){return typeof(n==null?void 0:n.aid)=="number"&&typeof(n==null?void 0:n.cid)=="number"&&typeof(n==null?void 0:n.bvid)=="string"},async set(n){var _;(_=X(".playlist-player .player-container"))==null||_.remove();let i=X(x.className.playlist+" .playlist-player"),a=X(x.className.playlist),o=T.getVue(a),{aid:l,cid:s,bvid:p}=n,{title:c,cover:m}=o.video;u.info(`视频播放信息 => aid：${l} bvid：${p} cid：${s}`),e==null&&(e={aid:l,bvid:p,cid:s,pic:m,title:c});const y=await or(e);if(y==null)return;let B=X("#artplayer");if(!B){const k=h.createElement("div",{className:"artplayer-container",innerHTML:`
								<div id="artplayer"></div>
							`});B=k.querySelector("#artplayer"),h.append(i,k);}if(y.container=B,r.$data.art==null){let k=await Se.init(y);if(k)r.$data.art=k;else return;r.$data.art.volume=1,r.$data.art.once("ready",()=>{g.execMenu("bili-video-playerAutoPlayVideoFullScreen",async()=>{u.info("自动进入全屏"),r.$data.art.fullscreen=true,r.$data.art.once("fullscreenError",()=>{u.warn("未成功进入全屏，需要用户交互操作，使用网页全屏代替"),r.$data.art.fullscreenWeb=true;});});}),r.$data.art.on("video:ended",()=>{u.info("视频播放结束，自动下一集");let L=X(x.className.playlist+" .control-panel");if(!L){u.error("未找到播放列表，无法自动播放下一集");return}if(T.getVue(L)==null){u.error("未找到播放列表的Vue实例，无法自动播放下一集");return}let{playMode:D,mediaList:f,videoIndex:C}=n.$store.state.playlist;if(C>=f.length-1)u.info("播放列表已播放完毕");else {let M=X(`.video-card[index="${C}"]`),P=T.getVue(M),j=P.p;if(j>=P.video.page){let J=X(`.video-card[index="${C+1}"]`);T.getVue(J).changeVideo(),u.info(`当前播放列表共：${f.length-1}个，即将播放下一个视频，第${C+2}个`);}else j++,P.changeVideo(j),u.info(`当前播放列表共：${f.length-1}个，即将播放第${C+2}-${j}`);}});}else await Se.update(r.$data.art,y);}}),T.waitVuePropToSet(x.className.playlist+" .playlist-player",{msg:"等待监听playlist播放列表改变",check(n){return typeof n.$watch=="function"},set(n){r.$flag.isWatchVideoChange||(r.$flag.isWatchVideoChange=true,n.$watch("cid",(i,a)=>{u.info("切换播放视频"),r.updateArtPlayerVideoInfo();}));}});}},yn={init(){this.coverVideoPlayer();},coverVideoPlayer(){document.querySelector("#artplayer")?u.warn("已存在播放器，更新播放信息"):O(`
