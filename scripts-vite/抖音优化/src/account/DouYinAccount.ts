...
export const DouYinAccount = {
  ...
  watchLoginDialogToClose() {
    ...
    const lockFn = new utils.LockFunction(() => {
      ...
      const $loginDialog = $<HTMLElement>('div[id^="login-full-panel-"]');
      if ($loginDialog && $loginDialog.children.length) {
        const $loginDialogCloseBtn =
          $loginDialog.querySelector<HTMLElement>(".dy-account-close") ||
          $loginDialog.querySelector<HTMLElement>(
        ...
        if ($loginDialogCloseBtn) {
          ...
          const $logPanelNew = $loginDialog.querySelector<HTMLDivElement>("#login-panel-new > div");
