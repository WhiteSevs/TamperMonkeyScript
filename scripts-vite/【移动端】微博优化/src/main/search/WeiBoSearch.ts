...
export const WeiBoSearch = {
  ...
  autoFocusSearchInput() {
    log.info(`自动聚焦搜索框`);
    DOMUtils.waitNode<HTMLInputElement>(`.ntop-nav input[type="search"]`).then(($input) => {
      if (!$input) {
        log.error("未找到搜索框");
        Qmsg.error("未找到搜索框");
        return;
  ...
  addOpenBlankBtn() {
    utils.mutationObserver(document.documentElement, {
      ...
      callback() {
        ...
        document
          .querySelectorAll<HTMLElement>(".card footer.m-ctrl-box:not(:has(.gm-open-blank))")
          .forEach(($footerCtrl) => {
            ...
            let $diyBtnList = $footerCtrl.querySelectorAll<HTMLElement>(".m-diy-btn");
