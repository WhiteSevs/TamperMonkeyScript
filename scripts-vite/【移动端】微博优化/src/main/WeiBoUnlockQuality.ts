...
export class WeiBoUnlockQuality {
    ...
    VueUtils.waitVuePropToSet(".video-player .mwb-video", [
      {
        ...
        set(vueObj) {
          ...
          let userSetQuality = Panel.getValue("weibo-common-lockVideoQuality") as string;
            ...
            if (name === "qualityButton") {
              ...
              qualityInfo["qualityList"].find((item: any) => {
                ...
                that.$data.videoQualityMap.get(item.src)!.forEach((videoQualityMapInfo) => {
                  ...
                  let findIndex = qualityInfo["qualityList"].findIndex((qualityItem: any) => {
              ...
              if (userSetQualitySign !== -1) {
                // 不是选择的自动
                let findSign = qualityInfo["qualityList"].find((item: any) => item["sign"] === userSetQualitySign);
                if (findSign) {
                ...
                /* 直接锁定sign值最大的一个 */
                let signList = qualityInfo["qualityList"].map((item: any) => item.sign);
  ...
  async unlockVideoHigherQuality() {
    ...
    $$<HTMLDivElement>(".weibo-media-wraps:not([data-unlock-quality])").forEach(($ele) => {
