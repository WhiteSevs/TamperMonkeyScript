...
export const WeiBoHook = {
    ...
    VueUtils.waitVuePropToSet("#app", [
      {
        ...
        set(vueIns) {
          ...
          let beforeEachFn = (to: Vue2Instance["$route"], from: Vue2Instance["$route"], next: Function) => {
            if (to.name === "profile") {
              // 主页
          ...
          let ownHookIndex = vueIns.$router.beforeHooks.findIndex((item: any) => item == beforeEachFn);
          if (ownHookIndex !== -1) {
  ...
  hookServiceWorkerRegister() {
    ...
    unsafeWindow.Object.defineProperty(unsafeWindow.navigator.serviceWorker, "register", {
      get() {
