...
`):d},callback(c,d){this.props[L].set(t,d);}};return Reflect.set(l.attributes,Z,t),Reflect.set(l.attributes,Y,n),me.initComponentsStorageApi("switch",l,{get(c,d){return A.getValue(c,d)},set(c,d){A.setValue(c,d);}}),l};class Le{option;constructor(t){this.option=t;}async showView(){let t=R.confirm({title:{text:this.option.title,position:"center"},content:{text:`
...
`)}
            ...
            `}).showView();},getTemplateData(){return {enable:true,avatarFlag:false,replyFlag:false,viewthreadFlag:false,minLength:5,keywordLength:8,keywords:[],userBlackList:[],userWhiteList:[]}},getData(){return re(this.$key.STORAGE_KEY,this.getTemplateData())},setData(e){G(this.$key.STORAGE_KEY,e);}};class Ke{option;$data={isFilteredData:[]};constructor(t){this.option=t;}showView(){let t=R.alert({title:{text:this.option.title,position:"center"},content:{text:`
...
`,et={$data:{},init(){this.registerMenu();},registerMenu(){se.add({key:"online-user",text:"⚙ 在线用户",autoReload:false,isStoreValue:false,showText(e){return e},callback:()=>{this.showOnlineUser();}});},async showOnlineUser(){let e=x.loading("正在获取在线用户名单中..."),t=await this.getOnlineUserListInfo();if(e.close(),!t)return;let n=R.alert({title:{text:"在线用户",position:"center"},content:{text:`
