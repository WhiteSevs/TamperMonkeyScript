...
(function (x, G, V, K, X, ye) {
	'use strict';

	var ce=typeof GM<"u"?GM:void 0,le=typeof GM_deleteValue<"u"?GM_deleteValue:void 0,ue=typeof GM_getResourceText<"u"?GM_getResourceText:void 0,Y=typeof GM_getValue<"u"?GM_getValue:void 0,j=typeof GM_info<"u"?GM_info:void 0,ve=typeof GM_registerMenuCommand<"u"?GM_registerMenuCommand:void 0,P=typeof GM_setValue<"u"?GM_setValue:void 0,Ae=typeof GM_unregisterMenuCommand<"u"?GM_unregisterMenuCommand:void 0,Ce=typeof GM_xmlhttpRequest<"u"?GM_xmlhttpRequest:void 0,D=typeof unsafeWindow<"u"?unsafeWindow:void 0,ke=window;const oe={waitRemove(...t){t.forEach(e=>{typeof e=="string"&&p.waitNodeList(e).then(a=>{a.forEach(n=>n.remove());});});},addBlockCSS(...t){let e=[];if(t.length!==0&&!(t.length===1&&typeof t[0]=="string"&&t[0].trim()===""))return t.forEach(a=>{Array.isArray(a)?e=e.concat(a):e.push(a);}),N(`${e.join(`,
`)}{display: none !important;}`)},setGMResourceCSS(t){let e=typeof ue=="function"?ue(t.keyName):null;typeof e=="string"&&e?N(e):oe.loadStyleLink(t.url);},async loadStyleLink(t){let e=document.createElement("link");e.rel="stylesheet",e.type="text/css",e.href=t,G.ready(()=>{document.head.appendChild(e);});},async loadScript(t){let e=document.createElement("script");return e.src=t,new Promise(a=>{e.onload=()=>{a(null);},(document.head||document.documentElement).appendChild(e);})},fixUrl(t){return t=t.trim(),t.match(/^http(s|):\/\//i)?t:t.startsWith("//")?(t.startsWith("///")||(t=window.location.protocol+t),t):(t.startsWith("/")||(t+="/"),t=window.location.origin+t,t)},fixHttps(t){if(t.startsWith("https://")||!t.startsWith("http://"))return t;let e=new URL(t);return e.protocol="https:",e.toString()},lockScroll(...t){let e=document.createElement("style");e.innerHTML=`
		...
		`),X.registerLanguage("smali",t);let e=new p.LockFunction(()=>{function a(i,r="java"){i.oldValue||(i.oldValue=i.textContent),i.innerHTML=X.highlight(i.oldValue,{language:r}).value.replace(/\\n$/gi,"");}document.querySelectorAll("em[onclick^=copycode]").forEach(i=>{if(i.nextElementSibling&&typeof i.nextElementSibling.className=="string"&&i.nextElementSibling.className=="code-select-language")return;let r=X.highlightAuto(u.text(i.parentElement.querySelector("div[id^=code]"))).language,l=document.createElement("select"),o=X.listLanguages().sort();o=o.concat("自动检测");let s="";o.forEach(c=>{c.startsWith("自动检测")?s+=`<option data-value="${r}" selected="selected">${c}(${r})</option>`:s+=`<option data-value="${c}">${c}</option>`;}),l.className="code-select-language",l.innerHTML=s,u.on(l,"change",()=>{let c=l.selectedOptions[0].getAttribute("data-value");h.info("切换代码块语言: ",c),u.parent(l).querySelectorAll("li").forEach(m=>{a(m,c);});}),p.preventEvent(l,"click"),p.preventEvent(i,"click"),i.insertAdjacentElement("afterend",l),p.dispatchEvent(l,"change");}),document.querySelectorAll(".blockcode").forEach(i=>i.className="hljs");},this,500);p.mutationObserver(document,{config:{subtree:true,childList:true},callback:()=>{e.run();}});},optimizationImagePreview(){h.info("图片查看优化");let t=[{hostName:"avatar-bbs.mt2.cn",pathName:"*"},{hostName:"cdn-bbs.mt2.cn",pathName:"^(/static(/|//)image|/template)"},{hostName:window.location.hostname,pathName:"^(/static(/|//)image|/template)"},{hostName:window.location.hostname,pathName:"/uc_server/avatar.php"}];function e(i=[],r=0){let l="";i.forEach(c=>{l+=`<li><img data-src="${c}"></li>`;});let o=u.createElement("ul",{innerHTML:l}),s=new ye(o,{inline:false,url:"data-src",zIndex:p.getMaxZIndex()+100,hidden:()=>{s.destroy();}});s.view(r),s.zoomTo(1),s.show();}function a(){O("#postlist .comiis_vrx:not([data-isHandlingViewIMG])").forEach(i=>{i.setAttribute("data-isHandlingViewIMG","true");let r=[];i.querySelectorAll("img").forEach(l=>{let o=l.getAttribute("file")||l.src;if(p.isNull(o))return;let s=new URL(o).hostname,c=new URL(o).pathname,m=l.parentElement;m.nodeName.toLowerCase()==="a"&&m.getAttribute("href")===o&&(m.setAttribute("href","javascript:;"),m.removeAttribute("target"));let g=false;for(let w of t)if(s.indexOf(w.hostName)!=-1&&c.match(w.pathName)){g=true;break}g||(r.push(o),l.removeAttribute("onclick"),l.setAttribute("onclick",""),u.on(l,"click",function(w){p.preventEvent(w),h.info("点击图片",l);let d=r.findIndex(f=>f==o);e(r,d);},{capture:true}));});});}let n=new p.LockFunction(()=>{a();});p.mutationObserver(document,{config:{subtree:true,childList:true},immediate:true,callback:()=>{n.run();}});},setAttachmentsClickTip(){h.info("附件点击提醒");function t(e){if(e.hasAttribute("href")){let a=e.hasAttribute("id")?e.id:e.parentElement.id,n=e.getAttribute("href"),i=e.innerText;if(document.querySelector(`#${a}_menu`).innerText.indexOf("金币")===-1)return;console.log("发现附件",e),console.log("该附件是金币附件，拦截！"),e.setAttribute("data-href",n),e.style.setProperty("cursor","pointer"),e.removeAttribute("href"),e.innerText="【已拦截】"+i,e.onclick=function(){I.confirm({title:{text:"提示",position:"center"},content:{text:`<br />确定花费2金币下载附件 <a style="color: #507daf !important;">${i}</a> ？<br /><br />`,html:true},btn:{ok:{callback:l=>{window.open(n,"_blank"),l.close();}}},width:"400px",height:"200px"});};}}p.mutationObserver(document.documentElement,{callback:()=>{document.querySelectorAll(".attnm a").forEach(e=>{t(e);}),document.querySelectorAll(".comiis_attach a").forEach(e=>{t(e);}),document.querySelectorAll("span[id*=attach_] a").forEach(e=>{t(e);});},immediate:true,config:{childList:true,subtree:true}});},async detectingUserOnlineStatus(){h.info("探测用户在线状态"),b.onceExec("mt-forum-post-detectingUserOnlineStatus",()=>{N(`
				.gm-user-status-icon{
					border: 0 !important;
		...
		`),O(".bm_c table tbody").forEach(t=>{let e=t.querySelector("th.common"),a=u.html(e),n=e.querySelectorAll("a")[0].getAttribute("href"),i=null,l=t.querySelector("td.by>cite>a").getAttribute("href").match(/uid-(\d+)/)[1],o=`
			...
			`;u.html(t,o);});}},ie={$data:{__storeApiFn:null,get storeApiValue(){return this.__storeApiFn||(this.__storeApiFn=new V.Dictionary),this.__storeApiFn}},getStorageApi(t){if(this.hasStorageApi(t))return this.$data.storeApiValue.get(t)},hasStorageApi(t){return this.$data.storeApiValue.has(t)},setStorageApi(t,e){this.$data.storeApiValue.set(t,e);},initComponentsStorageApi(t,e,a){let n;this.hasStorageApi(t)?n=this.getStorageApi(t):n=a,this.setComponentsStorageApiProperty(e,n);},setComponentsStorageApiProperty(t,e){Reflect.set(t.props,k,e);}},ae=function(t,e,a,n,i,r="",l,o,s){let c={text:t,type:"input",isNumber:!!l,isPassword:false,attributes:{},props:{},description:n,afterAddToUListCallBack:s,getValue(){return this.props[k].get(e,a)},callback(m,g,w){this.props[k].set(e,g);},placeholder:r};return Reflect.set(c.attributes,z,e),Reflect.set(c.attributes,$,a),ie.initComponentsStorageApi("input",c,{get(m,g){return b.getValue(m,g)},set(m,g){b.setValue(m,g);}}),c},S=function(t,e,a,n,i,r,l){let o={text:t,type:"switch",description:i,disabled:l,attributes:{},props:{},getValue(){return this.props[k].get(e,a)},callback(s,c){let m=!!c;h.success(`${m?"开启":"关闭"} ${t}`),this.props[k].set(e,m);},afterAddToUListCallBack:r};return Reflect.set(o.attributes,z,e),Reflect.set(o.attributes,$,a),ie.initComponentsStorageApi("switch",o,{get(s,c){return b.getValue(s,c)},set(s,c){b.setValue(s,c);}}),o},re=function(t,e,a,n,i,r="",l){let o={text:t,type:"textarea",attributes:{},props:{},description:n,placeholder:r,disabled:l,getValue(){let c=this.props[k].get(e,a);return Array.isArray(c)?c.join(`
`):c},callback(s,c){this.props[k].set(e,c);}};return Reflect.set(o.attributes,z,e),Reflect.set(o.attributes,$,a),ie.initComponentsStorageApi("switch",o,{get(s,c){return b.getValue(s,c)},set(s,c){b.setValue(s,c);}}),o};class be{option;constructor(e){this.option=e;}async showView(){let e=I.confirm({title:{text:this.option.title,position:"center"},content:{text:`
...
`,Ue={$data:{cid:""},init(){this.registerMenu();},registerMenu(){Z.add({key:"black-home",text:"⚙ 小黑屋",autoReload:false,isStoreValue:false,showText(t){return t},callback:()=>{this.showBlackHome();}});},async showBlackHome(){let t=x.loading("正在获取小黑屋名单中..."),e=await this.getBlackListInfo("");if(t.close(),!e)return;if(e.data.length===0){x.error("获取小黑屋名单为空");return}this.$data.cid=e.next_cid;let a=I.confirm({title:{text:"小黑屋名单",position:"center"},content:{text:`
                ...
                `,html:true},btn:{ok:{text:"下一页",callback:async()=>{let o=x.loading("正在获取小黑屋名单中...");h.info("下一页的cid: ",this.$data.cid);let s=await this.getBlackListInfo(this.$data.cid);o.close(),s&&(this.$data.cid=s.next_cid,s.data.forEach(c=>{let m=this.createListViewItem(c);n.appendChild(m);}),s.data.length===0?x.error("获取小黑屋名单为空"):x.success(`成功获取 ${s.data.length}条数据`),p.dispatchEvent(i,"input"));}},cancel:{text:"关闭"}},width:W.settingBig.width,height:W.settingBig.height,style:Be}),n=a.$shadowRoot.querySelector(".blackhome-user-list"),i=a.$shadowRoot.querySelector(".blackhome-user-filter input");e.data.forEach(o=>{let s=this.createListViewItem(o);n.appendChild(s);}),x.success(`成功获取 ${e.data.length}条数据`);let r=false;u.on(i,["input","propertychange"],p.debounce(()=>{let o=i.value.trim();if(!r){if(r=true,o==""){a.$shadowRoot.querySelectorAll(".blackhome-user-item").forEach(s=>{s.removeAttribute("style");}),r=false;return}a.$shadowRoot.querySelectorAll(".blackhome-user-item").forEach(s=>{s.getAttribute("data-name").match(new RegExp(o,"ig"))||s.getAttribute("data-uid").trim().match(new RegExp(o,"ig"))||s.getAttribute("data-operator").match(new RegExp(o,"ig"))?s.removeAttribute("style"):s.setAttribute("style","display:none;");}),r=false;}}));let l=await this.getBlackListInfo(this.$data.cid);l&&(this.$data.cid=l.next_cid);},async getBlackListInfo(t=""){let e={mod:"misc",action:"showdarkroom",cid:t,ajaxdata:"json"},a=await R.get(`/forum.php?${p.toSearchParamsStr(e)}`,{headers:{"User-Agent":p.getRandomPCUA()},responseType:"json"});if(!a.status)return;let n=p.toJSON(a.data.responseText),i=n.message.split("|"),r=i[i.length-1],l=p.parseObjectToArray(n.data),o=[],s=[];return l.forEach(c=>{let m=c.dateline.match(/([0-9]{4}-[0-9]{1,2}-[0-9]{1,2}[\s]*[0-9]{1,2}:[0-9]{1,2})/g);if(m==null){let g=parseInt((Date.now()/1e3).toString()),w=0,d=c.dateline.match(/([0-9]+|半)[\s\S]*秒前/),f=c.dateline.match(/([0-9]+|半)[\s\S]*分钟前/),y=c.dateline.match(/([0-9]+|半)[\s\S]*小时前/),A=c.dateline.match(/昨天[\s\S]*(\d{2}):(\d{2})/),C=c.dateline.match(/前天[\s\S]*(\d{2}):(\d{2})/),E=c.dateline.match(/([0-9]+|半)[\s\S]*天前/);if(d)d=d[d.length-1],d=d.replace(/半/g,.5),d=parseFloat(d),w=g-d;else if(f)f=f[f.length-1],f=f.replace(/半/g,.5),f=parseFloat(f),w=g-f*60;else if(y)y=y[y.length-1],y=y.replace(/半/g,.5),y=parseFloat(y),w=g-y*60*60;else if(A){let v=A[1],T=A[2];w=g-86400-parseInt(v)*3600-parseInt(T)*60;}else if(C){let v=C[1],T=C[2];w=g-86400*2-parseInt(v)*3600-parseInt(T)*60;}else E&&(E=E[E.length-1],E=E.replace(/半/g,.5),E=parseFloat(E),w=g-E*60*60*24);c.time=parseInt(w.toString())*1e3,o=o.concat(c);return}else m=m[0];c.time=p.formatToTimeStamp(m),o=o.concat(c);}),p.sortListByProperty(o,"time"),p.sortListByProperty(s,"time",false),o=o.concat(s),{next_cid:r,data:o}},createListViewItem(t){let e=u.createElement("div",{className:"blackhome-user-item",innerHTML:`
...
`,Ne={$data:{},init(){this.registerMenu();},registerMenu(){Z.add({key:"online-user",text:"⚙ 在线用户",autoReload:false,isStoreValue:false,showText(t){return t},callback:()=>{this.showOnlineUser();}});},async showOnlineUser(){let t=x.loading("正在获取在线用户名单中..."),e=await this.getOnlineUserListInfo();if(t.close(),!e)return;let a=I.alert({title:{text:"在线用户",position:"center"},content:{text:`
                ...
                `,html:true},btn:{ok:{text:"关闭",type:"default"}},width:W.settingBig.width,height:W.settingBig.height,style:_e}),n=a.$shadowRoot.querySelector(".online-user-list"),i=a.$shadowRoot.querySelector(".online-user-filter input");e.data.forEach(l=>{let o=this.createListViewItem(l);n.appendChild(o);}),x.success(`成功获取 ${e.data.length}条数据`);let r=false;G.on(i,["propertychange","input"],p.debounce(()=>{let l=i.value.trim();if(!r){if(r=true,l==""){a.$shadowRoot.querySelectorAll(".online-user-list .online-item").forEach(o=>{o.removeAttribute("style");}),r=false;return}a.$shadowRoot.querySelectorAll(".online-user-list .online-item").forEach(o=>{o.getAttribute("data-name").match(new RegExp(l,"ig"))||o.getAttribute("data-sf").match(new RegExp(l,"ig"))||o.getAttribute("data-uid").match(new RegExp(l,"ig"))?o.removeAttribute("style"):o.setAttribute("style","display:none;");}),r=false;}}));},async getOnlineUserListInfo(){let t={showoldetails:"yes"},e=await R.get(`/forum.php?${p.toSearchParamsStr(t)}`,{headers:{"User-Agent":p.getRandomPCUA()}});if(!e.status)return;let a=p.parseFromString(e.data.responseText,"text/html"),n={data:[],totalOnline:0,onlineUser:0,noRegisterUser:0,invisibleUser:0};a.querySelectorAll("#onlinelist ul li").forEach(l=>{let o=l.querySelector("a").getAttribute("href").match("uid-(.+?).html")[1],s=M.getAvatar(o,"middle"),c=l.querySelector("a").innerText,m="",g=l.querySelector("a").getAttribute("href"),w=l.querySelector("img").src;w.indexOf("online_member")!=-1?m="会员":w.indexOf("online_moderator")!=-1?m="版主":w.indexOf("online_supermod")!=-1?m="超级版主":w.indexOf("online_admin")!=-1?m="管理员":m="未知身份",n.data.push({uid:o,avatar:s,name:c,sf:m,space:g});});let r=a.querySelector("#online div.bm_h span.xs1").textContent;return n.totalOnline=p.parseInt(r.match(/([0-9]*)\s*人在线/i),0),n.onlineUser=p.parseInt(r.match(/([0-9]*)\s*会员/i),0),n.noRegisterUser=p.parseInt(r.match(/([0-9]*)\s*位游客/i),0),n.invisibleUser=p.parseInt(r.match(/([0-9]*)\s*隐身/i),0),n},createListViewItem(t){let e=G.createElement("div",{className:"online-item",innerHTML:`
		...
		`),window.location.href.includes("/forum.php?mod=guide&view=newthread")&&(u.removeClass("#mn_forum_10","a"),u.css("#latest_publication a","background",'url("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAcAAADICAYAAAAk7PuyAAAAAXNSR0IArs4c6QAAAGFJREFUWEft1zESgCAQQ9HsVfb+d5RRlLHRVotHS5f5+YHKyymXiRAihKMISBDCNOInJHT39iTkcpkIYUcTCUKYkkKCEJBwTaY6cML5eiNGYiRGYrz9pqyDdbAOqxC/q8MApobR97qxnMwAAAAASUVORK5CYII=") repeat-x 50% -50px'));},async extendCookieExpire(){h.info("延长cookie有效期");let t=await ce.cookie.list({}),e=["_auth","_saltkey","_client_created","_client_token"];t.forEach(async a=>{if(a.session)return;let n=a.expirationDate,i=Date.now()/1e3;if(n<i)return;let r=60*60*24*30;n-i>r||!e.find(o=>a.name.endsWith(o))||ce.cookie.set({name:a.name,value:a.value,expirationDate:a.expirationDate+r}).then(()=>{h.info(`延长Cookie +30天成功：${a.name}`);}).catch(()=>{h.error(`延长Cookie +30天失败：${a.name}`);});});}},pe=function(t,e,a,n,i,r){let l=[];typeof n=="function"?l=n():l=n;let o={text:t,type:"select",description:r,attributes:{},props:{},getValue(){return this.props[k].get(e,a)},callback(s,c,m){let g=c;if(h.info(`选择：${m}`),typeof i=="function"&&i(s,g,m))return;this.props[k].set(e,g);},data:l};return Reflect.set(o.attributes,z,e),Reflect.set(o.attributes,$,a),ie.initComponentsStorageApi("select",o,{get(s,c){return b.getValue(s,c)},set(s,c){b.setValue(s,c);}}),o},me=function(t,e,a,n,i,r,l,o,s,c){let m={text:t,type:"button",attributes:{},props:{},description:e,buttonIcon:n,buttonIsRightIcon:i,buttonIconIsLoading:r,buttonType:l,buttonText:a,callback(g){typeof o=="function"&&o(g);},afterAddToUListCallBack:s};return Reflect.set(m.attributes,se,()=>{m.disable=false;}),m},he=function(t,e,a,n){let i={type:"own",attributes:{},props:a,getLiElementCallBack:t,afterAddToUListCallBack:n};return Reflect.set(i.attributes,se,()=>false),i},H={$upload:{small:false,middle:false,big:false},$data:{avatarInfo:{maxSize:2097152,small:{width:48,height:48},middle:{width:120,height:120},big:{width:200,height:250}}},$el:{$smallUpload:null,$middleUpload:null,$bigUpload:null,$smallStatus:null,$middleStatus:null,$bigStatus:null},$avatar:{get small(){return H.$el.$smallUpload.files[0]},get middle(){return H.$el.$middleUpload.files[0]},get big(){return H.$el.$bigUpload.files[0]}},init(){this.showView();},showView(){const t=this;let e=I.confirm({title:{text:"修改头像",position:"center"},content:{text:`
                ...
                `,html:true},btn:{ok:{text:"上传",callback:async()=>{if(!t.$upload.small){x.error("请上传小头像");return}if(!t.$upload.middle){x.error("请上传中头像");return}if(!t.$upload.big){x.error("请上传大头像");return}let a=x.loading("正在处理数据中...");try{let n=await this.getUploadUrl();if(n==null)return;let i=await M.getFormHash();if(i==null){x.error("获取formhash失败");return}let r={big:{base64:await p.parseFileToBase64(this.$avatar.big)},middle:{base64:await p.parseFileToBase64(this.$avatar.middle)},small:{base64:await p.parseFileToBase64(this.$avatar.small)}};Object.keys(r).forEach(s=>{let c=r[s];c.base64=c.base64.substring(c.base64.indexOf(",")+1);});let l=new FormData;l.append("Filedata",this.$avatar.big||""),l.append("confirm","确定"),l.append("avatar1",r.big.base64),l.append("avatar2",r.middle.base64),l.append("avatar3",r.small.base64),l.append("formhash",i),h.info("头像的base64字符串",r);let o=await R.post(n,{data:l,processData:!1,headers:{Accept:"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9","User-Agent":p.getRandomPCUA(),Host:window.location.hostname,Origin:window.location.origin,Referer:`${window.location.origin}/home.php?mod=spacecp&ac=avatar`}});if(!o.status)return;o.data.responseText.indexOf("window.parent.postMessage('success','*')")!=-1?(e.close(),x.success("上传成功")):(h.error("上传失败",o),x.error(o.data.responseText,{timeout:6e3,isHTML:!1,html:!1}));}catch(n){h.error(n);}finally{a.close();}}}},width:window.innerWidth>500?"500px":"88vw",height:window.innerHeight>500?"500px":"80vh",style:`
            .avatar-container{
                display: flex;
