...
export const MTUtils = {
  ...
  getCurrentUID() {
    ...
    let $exit =
      $<HTMLAnchorElement>('.sidenv_exit a[href*="uid-"]') || $<HTMLAnchorElement>('#comiis_key a[href*="uid-"]');
    if ($exit) {
  ...
  async getFormHash() {
    let $inputFormHashList = Array.from(
      (top || globalThis).document.querySelectorAll<HTMLInputElement>("input[name=formhash]")
    );
    ...
    let $anchorFormHashList = Array.from(
      (top || globalThis).document.querySelectorAll<HTMLAnchorElement>('a[href*="formhash="]')
    );
    ...
    if (homeResponse.status) {
      ...
      let $formhash = homeDoc.querySelector<HTMLInputElement>("input[name=formhash]");
      if ($formhash) {
