...
export const MTOnlineUser = {
  ...
  async showOnlineUser() {
    ...
    let $alert = pops.alert({
      ...
      content: {
        text: /*html*/ `
                <div class="online-user-info">${onlineUserInfo.totalOnline} 人在线 - ${onlineUserInfo.onlineUser} 会员${
    ...
    let $list = $alert.$shadowRoot.querySelector<HTMLDivElement>(".online-user-list")!;
    let $filterInput = $alert.$shadowRoot.querySelector<HTMLInputElement>(".online-user-filter input")!;
    ...
    DOMUtils.on(
      ...
      utils.debounce(() => {
        ...
        let isFind = false;
        $alert.$shadowRoot.querySelectorAll<HTMLDivElement>(".online-user-list .online-item").forEach((item) => {
          if (
  ...
  async getOnlineUserListInfo() {
    ...
    let onlineList = pageHTML.querySelectorAll<HTMLLIElement>("#onlinelist ul li");
    onlineList.forEach((item) => {
      /* uid */
      let uid = item.querySelector<HTMLAnchorElement>("a")!.getAttribute("href")!.match("uid-(.+?).html")![1];
      /* 头像 */
      ...
      let sf = "";
      /* 个人空间页 */
      let space = item.querySelector<HTMLAnchorElement>("a")!.getAttribute("href")!;
