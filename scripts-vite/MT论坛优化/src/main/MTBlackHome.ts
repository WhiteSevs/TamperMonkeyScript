...
export const MTBlackHome = {
	...
	async showBlackHome() {
		...
		let $filterInput = $confirm.$shadowRoot.querySelector<HTMLInputElement>(
		...
		DOMUtils.on(
			...
			utils.debounce(() => {
				...
				if (inputText == "") {
					$confirm.$shadowRoot
						.querySelectorAll(".blackhome-user-item")
						.forEach((item) => {
				...
				let isFind = false;
				$confirm.$shadowRoot
					.querySelectorAll<HTMLDivElement>(".blackhome-user-item")
					.forEach((item) => {
						if (
							item
	...
	async getBlackListInfo(cid = "") {
		...
		blackListData.forEach((item) => {
			...
			if (date == null) {
				...
				let sec_data =
					item["dateline"].match(/([0-9]+|半)[\s\S]*秒前/); /* xx|半秒前 */
				let min_data =
					item["dateline"].match(/([0-9]+|半)[\s\S]*分钟前/); /* xx|半分钟前 */
				let hour_data =
					item["dateline"].match(/([0-9]+|半)[\s\S]*小时前/); /* xx|半小时前 */
				...
				let day_data =
					item["dateline"].match(/([0-9]+|半)[\s\S]*天前/); /* xx天前 */
				if (sec_data) {
					// @ts-ignore
		...
		let $item = DOMUtils.createElement(
			"div",
			{
                ...
                <div class="blackhome-user-avatar">
                    ...
                    <div class="blackhome-user-uuid">UID: ${
											userInfo["uid"]
										}</div>
