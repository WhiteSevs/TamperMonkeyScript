...
export const MTGuide = {
	...
	beautifyPage() {
		...
		`);
		$$<HTMLTableRowElement>(".bm_c table tbody").forEach((item) => {
			let tableNode = unsafeWindow.jQuery(item);
			...
			`;
			let newListNode = unsafeWindow.jQuery(newHTML);
			unsafeWindow
				.jQuery(newListNode.find(".byg_th_align")[0].children[0])
				.before(`<em>[${tableNode.find("tr>td.by>a")[0].outerHTML}]</em>`);
