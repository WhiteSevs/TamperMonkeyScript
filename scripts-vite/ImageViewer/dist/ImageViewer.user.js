...
(function (qmsg, _whitesev_domutils, _whitesev_pops, _whitesev_utils, viewerjs) {
  ...
  var WebSiteStorage = new StorageUtils("user-rule-data");
  var WebSiteDefaultRule = () => {
    return {
      url: "*://*/*",
      selector: "img",
  ...
  var ImageUtils = {
    getImageElementUrl($el) {
      function isStringNull(target) {
        return target.trim() === "null" || target.trim() === "undefined" || target.trim() === "";
      }
      if ($el == null) return;
      if (typeof $el === "string" && isStringNull($el)) return;
      let url = "";
      if (isStringNull(url)) {
        if ($el.hasAttribute("data-src")) url = $el.getAttribute("data-src");
      }
      if (isStringNull(url)) {
        if ($el.hasAttribute("src")) url = $el.getAttribute("src");
      }
      if (isStringNull(url)) {
        if ($el.hasAttribute("alt")) url = $el.getAttribute("alt");
      }
      if (isStringNull(url)) return;
      return url;
    },
  };
  var ImageViewer = {
    viewIMG(imageUrlList, index = 0) {
      ...
      let viewerULElement = domUtils.createElement("ul", { innerHTML: viewerULHTML });
      let viewer = new viewerjs.default(viewerULElement, {
        inline: false,
        url: "data-src",
  ...
  var WebSiteRule = {
    $data: {
      ...
      get userRule() {
        return WebSiteStorage.get("user-rule", []);
      },
      currentRule: [],
    },
    execRule() {
      ...
      this.$data.userRule.forEach((rule, index) => {
        ...
        if (!new RegExp(rule.url, "gi").test(window.location.href)) return;
      ...
      this.$data.currentRule.forEach((rule) => {
        if (rule.mode === "handleClickEvent") this.handleClickEvent(rule);
        else if (rule.mode === "handleElement") this.handleElement(rule);
        else log.error(["未知模式的规则", rule]);
      });
    },
    handleClickEvent(rule) {
      function setClickEvent($el) {
        ...
        domUtils.on(
          ...
          rule.selector,
          async (event) => {
            if (rule.isPreventDefault) domUtils.preventEvent(event);
            const targetElement = event.target;
            if (targetElement instanceof HTMLImageElement && targetElement.closest(".viewer-container")) return;
            let imageList = [];
            let imageIndex = 0;
            if (typeof rule.clickEvent === "string") {
              const asyncUserClickEvent = function (event) {
                return new Promise((resolve, reject) => {
                  new Function("event", "resolve", "reject", rule.clickEvent).bind({
                    ImageUtils,
                    utils,
                    DOMUtils: domUtils,
              ...
              if (!Boolean(result.isView)) return;
              if (Array.isArray(result.imageList)) imageList = result.imageList;
              if (typeof result.imageIndex === "number") {
                imageIndex = result.imageIndex;
                if (imageIndex < 0 || imageIndex >= imageList.length) imageIndex = 0;
              }
            }
      ...
      if (typeof rule.context === "string")
        domUtils.waitNode(rule.context).then((contextElement) => {
          if (rule.useSelector === "querySelector") setClickEvent($(rule.context));
          else setClickEvent($$(rule.context));
        });
      else setClickEvent(document);
    },
    handleElement(rule) {},
  };
