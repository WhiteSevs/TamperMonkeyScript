...
import { ExtendJQueryFn, MTQuickUBB, MTUBB_Rainbow } from "../forum-post/MTQuickUBB";
...
import { MTEditorOptimizationNormal, type EditorNormalStorageOption } from "../forum-post/MTEditorOptimization-normal";
...
const MTEditorPreview = {
  ...
  parseVoteText() {
    ...
    let visibilitypoll = unsafeWindow
      ...
      : true;
    /* 公开投票参与人 */
    let overt = unsafeWindow.$("input#overt").parent().find(".comiis_checkbox").hasClass("comiis_checkbox_close")
      ? false
      : true;
    let html = "";
    let choosehtml = "";
    ...
    html = `
                    <div class="comiis_poll cl comiis_input_style b_t postforum_vote">
                            <div class="comiis_poll_top">
                                ...
                                <h2>${
                                  maxchoices > 1
                                    ? "多选投票" + '<em class="f_c"> 最多可选 ' + maxchoices + " 项</em>"
                                    : "单选投票"
                                }</h2>
                                ...
                                ${
                                  polldatas > 1 ? '<em class="f_a">' + (polldatas - 1) + "</em> 天 " : ""
...
export const MTEditorOptimization = {
  ...
  overridePageEditor() {
    ...
    let comiis_post_tab = unsafeWindow.$("#postform .comiis_post_from #comiis_post_tab");
    unsafeWindow.$("#postform .comiis_post_from .comiis_post_ico").append(comiis_post_tab);
    ...
    let before_image_luntan = unsafeWindow.$(".gm_plugin_chartbed .comiis_over_box.comiis_input_style #imglist");
    addStyle(/*css*/ `
  ...
  async initReplyText() {
    ...
    let data = null as any as EditorStorageOption["data"] | EditorNormalStorageOption;
    let save_callback = null as any as null | (() => boolean | Promise<boolean>);
    ...
    if (MTRouter.isPostPublish_newthread()) {
      ...
      if (MTRouter.isPostPublish_voting()) {
        // 投票页面
        log.info(`投票页面`);
        data = GM_getValue<VotingContentOption>(this.$key.noPublishVotingSerializeText);
        delete_callback = () => {
      ...
      if (queryResult.data) {
        ...
        if (findValue) {
          ...
          delete_callback = async () => {
            let deleteQuery = await this.$data.db.get<EditorStorageOption[]>("data");
            if (deleteQuery.data) {
              ...
              if (deleteFindIndex != -1) {
                ...
                await this.$data.db.save<EditorStorageOption[]>("data", deleteQuery.data);
              }
            }
          };
        }
      ...
      if (Panel.getValue("mt-forum-post-editorOptimizationNormal-recordInputText")) {
        let initResult = await MTEditorOptimizationNormal.$data.db.get<EditorNormalStorageOption[]>("data");
        if (initResult.code === 201) {
          // 初始化
          await this.$data.db.save("data", []);
        }
        let queryResult = await MTEditorOptimizationNormal.$data.db.get<EditorNormalStorageOption[]>("data");
        if (queryResult.data) {
          let findValue = queryResult.data.find((item) => {
    ...
    if (MTRouter.isPostPublish_voting()) {
      // 投票页面
      save_callback = () => {
        let $title = that.$el.$form.querySelector<HTMLInputElement>("input[name='subject']")!;
        // 内容
        let $content = that.$el.$form.querySelector<HTMLTextAreaElement>("textarea[name='message']")!;
        // 最多可选xx项
        let $maxchoices = that.$el.$form.querySelector<HTMLInputElement>("input[name='maxchoices']")!;
        // 记票天数
        let $expiration = that.$el.$form.querySelector<HTMLInputElement>("input[name='expiration']")!;
        // 投票后结果可见
        let $visibilitypoll = that.$el.$form.querySelector<HTMLInputElement>("input[name='visibilitypoll']")!;
        // 公开投票参与人
        let $overt = that.$el.$form.querySelector<HTMLInputElement>("input[name='overt']")!;
  ...
  deleteReplyTextStorage() {
    ...
    this.$data.db.get<EditorStorageOption[]>("data").then((result) => {
      ...
      let type: EditorStorageOption["type"] = MTRouter.isPostPublish_voting() ? "post-vote" : "post";
      ...
      if (localDataIndex !== -1) {
        ...
        that.$data.db.save<EditorStorageOption[]>("data", result.data).then((result2) => {});
      }
    });
  },
  /**
    ...
    DOMUtils.on([this.$el.$input, this.$el.$title].filter(Boolean), ["input", "propertychange"], function (event) {
      ...
      if (MTRouter.isPostPublish_newthread()) {
        ...
        Panel.execMenu("mt-forum-post-editorOptimizationNormal-recordInputText", () => {
          MTEditorOptimizationNormal.$data.db.get<EditorNormalStorageOption[]>("data").then((result) => {
   ...
   */
  initSaveBtn() {
    let $save = DOMUtils.selector<HTMLButtonElement>(".comiis_btnbox button#postsubmit:contains('保存')")!;
    if (!$save) {
      return;
    }
   ...
   */
  initPostBtn() {
    let $post = DOMUtils.selector<HTMLButtonElement>(".comiis_btnbox button#postsubmit:contains('发表')");
    if (!$post) {
      return;
    }
    let $header = $<HTMLElement>("#comiis_head .header_y")!;
  ...
  initReplyBtn() {
    const that = this;
    let $reply = DOMUtils.selector<HTMLButtonElement>(".comiis_btnbox button#postsubmit:contains('回复')");
    if (!$reply) {
      return;
    }
    let $header = $<HTMLElement>("#comiis_head .header_y")!;
  ...
  initVotePage() {
    ...
    unsafeWindow.$("#htmlon").parent().append(/*html*/ `
                ...
                `);
    if (unsafeWindow.$(".comiis_scrollTop_box .swiper-slide a:contains('发表帖子')").attr("class") != "f_c") {
      unsafeWindow.$(".comiis_checkbox.comiis_choose_post").removeClass("comiis_checkbox_close");
    } else {
      unsafeWindow.$(".comiis_checkbox.comiis_choose_vote").removeClass("comiis_checkbox_close");
    }
    unsafeWindow.$(".comiis_checkbox.comiis_choose_post").on("click", function (this: any) {
      let obj = unsafeWindow.$(this);
      obj.addClass("comiis_checkbox_close");
      unsafeWindow.$(".comiis_checkbox.comiis_choose_vote").addClass("comiis_checkbox_close");
      obj.removeClass("comiis_checkbox_close");
      window.location.href = window.location.href.replace("&special=1", "");
    });
    unsafeWindow.$(".comiis_checkbox.comiis_choose_vote").on("click", function (this: any) {
      let obj = unsafeWindow.$(this);
      obj.addClass("comiis_checkbox_close");
      unsafeWindow.$(".comiis_checkbox.comiis_choose_post").addClass("comiis_checkbox_close");
      obj.removeClass("comiis_checkbox_close");
  ...
  initSaveDraftBtn() {
    const that = this;
    let $saveDraft = DOMUtils.selector<HTMLEmbedElement>(".comiis_btnbox button#postsubmit em:contains('保存草稿')");
    if (!$saveDraft) {
      return;
    }
    let $header = $<HTMLElement>("#comiis_head .header_y")!;
    ...
    DOMUtils.append($header, $btn);
    let $submitPost = DOMUtils.selector<HTMLElement>(".comiis_scrollTop_box .swiper-slide a:contains('发表帖子')");
  ...
  observerChangeEditorHeight() {
    ...
    DOMUtils.waitNode("#postform > div > div.comiis_post_ico.comiis_minipost_icot").then((element) => {
      utils.mutationObserver(element, {
        callback: (mutations) => {
          var $tar = $<HTMLElement>("#postform > div > div.comiis_post_ico.comiis_minipost_icot")!;
          ...
          let needMessageSeeHeight =
            ...
            $<HTMLElement>("#postform > div > div.comiis_post_ico.comiis_minipost_icot")!.getBoundingClientRect()
              .height -
          ...
          if (needMessageSeeHeight - 5 < 100) {
            ...
            unsafeWindow.$(".gm_plugin_previewpostforum_html.double-preview .comiis_over_box").css("height", "100px");
          } else {
            unsafeWindow.$("#needmessage").css("height", needMessageSeeHeight - 5 + "px");
            unsafeWindow
  ...
  listenResize() {
    DOMUtils.on(window, "resize", function () {
      let needMessageSeeHeight =
        document.documentElement.clientHeight -
        $<HTMLElement>("#postform > div > div.comiis_post_ico.comiis_minipost_icot")!.getBoundingClientRect().height -
        $<HTMLTextAreaElement>("#needmessage")!.getBoundingClientRect().top;
      if (needMessageSeeHeight - 5 < 100) {
  ...
  initSelectPostingSection() {
    ...
    if (MTRouter.isPostPublish_newthread()) {
      ...
      DOMUtils.on($select, "change", function () {
        ...
        let otherSelect = (classifyClassNameDict as any)[(section_dict as any)[fid]] as {
          className: string;
          optionHTML: string;
        } | null;
        if (otherSelect) {
          ...
          Object.keys(classifyClassNameDict).forEach((key) => {
            DOMUtils.remove(".comiis_post_from ." + (classifyClassNameDict as any)[key]["className"]);
          });
        }
  ...
  initUBB() {
    ...
    DOMUtils.on("#comiis_post_tab .comiis_input_style .comiis_post_urlico li", "click", function () {
      ...
      unsafeWindow.$("#comiis_post_qydiv ul li").hide().eq(unsafeWindow.$(this).index()).fadeIn();
    });
    unsafeWindow.$.each(ubbCode, function (key: any, value: any) {
      ...
      DOMUtils.on($ubbs, "click", (event) => {
        ...
        unsafeWindow.$("#comiis_post_qydiv ul li").hide().eq(contentIndex).fadeIn();
      });
  ...
  initSettingImmersionMode() {
    ...
    DOMUtils.on("#immersiveinput", "click", function () {
      let $click = this;
      let code_obj = DOMUtils.parent($click).querySelector<HTMLElement>(".comiis_checkbox")!;
      let elementList = [
        /* 板块、标题 */
