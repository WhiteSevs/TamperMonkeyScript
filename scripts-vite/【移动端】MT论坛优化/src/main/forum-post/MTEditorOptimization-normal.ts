...
export const MTEditorOptimizationNormal = {
	...
	handle_error(text: string) {
		...
		let messagetext: string = DOMUtils.text(
			DOMUtils.parseHTML(text, false, false).querySelector<HTMLElement>("#messagetext")!
		);
		...
		DOMUtils.on(this.$el.$like, "click", async (event) => {
			...
			if (unsafeWindow.comiis_recommend_key == 0) {
				...
				if (resultText.includes("您已评价过本主题")) {
					...
					if ($$(".comiis_recommend_list_t").length > 0) {
						...
						DOMUtils.prepend(
							...
							`<span id="comiis_recommend_list_t${unsafeWindow.uid}">
                          							<a href="home.php?mod=space&uid=${unsafeWindow.uid}" class="f_c">${
					...
					Qmsg.success(
						"点赞成功" +
							(recommendcList["daycount"] ? `, 您今天还能点赞 ${recommendcList["daycount"] - 1} 次` : "")
					);
				} else if (
					resultText.indexOf("window.location.href = 'member.php?mod=logging&action=login&mobile=2'") >= 0
				) {
	...
	setSubmitBtnClickEvent() {
		...
		DOMUtils.on(this.$el.$fastpostsubmit, "click", async (event) => {
			...
			if (DOMUtils.val(that.$el.$fastpostsubmit) == "发表") {
				...
				/* 回复 */
				let data =
					DOMUtils.attr("#comiis_foot_menu_beautify_big .reply_user_content", "data-reply-serialize") +
					message;
		...
		DOMUtils.on(
			...
			async (event) => {
				...
				let reply_content = DOMUtils.val(
					requestDOM.querySelector<HTMLInputElement>("input[name='noticeauthormsg']")!
				);
	...
	deleteReplyTextStorage(isUserReply: boolean = false, replyUrl: string | undefined | null = undefined) {
		...
		this.$data.db.get<EditorNormalStorageOption[]>("data").then((result) => {
			...
			let localDataIndex = result.data.findIndex((item) => {
				if (isUserReply) {
					return (
						item.forumId === that.$data.tid && replyUrl && item.repquote === MTUtils.getRepquote(replyUrl)
					);
				} else {
