...
export const MTEditorOptimizationNormal = {
	...
	overridePageEditor() {
		// 评论图标
		let $old_commentIcon = document.querySelector<HTMLElement>(
			"#comiis_foot_memu .comiis_flex li:nth-child(2)"
		)!;
		// 点赞图标
		let $old_linkIcon = document.querySelector<HTMLElement>(
			"#comiis_foot_memu .comiis_flex li:nth-child(3)"
		)!;
		// 收藏图标
		let $old_collectIcon = document.querySelector<HTMLElement>(
		...
		let forum_url = document.querySelector<HTMLAnchorElement>(
		...
		unsafeWindow
			.$(document)
			.on("click", "#imglist .up_btn a", function (this: any) {
				unsafeWindow.$(this).next().click();
			});
		unsafeWindow
			.$(document)
			.on("click", "#imglist .p_img a", function (this: any) {
	...
	handle_error(text: string) {
		...
		let messagetext: string = DOMUtils.text(
			DOMUtils.parseHTML(text, false, false).querySelector<HTMLElement>(
				"#messagetext"
			)!
		);
		if (
	...
	setInputChangeSaveEvent() {
		...
		DOMUtils.on(this.$el.$input, ["input", "propertychange"], (event) => {
			let inputText = that.$el.$input.value;
			let $reply = that.$el.$input
				.closest(".reply_area")!
				.querySelector<HTMLElement>(".reply_user_content")!;
			...
			that.$data.db.get<EditorNormalStorageOption[]>("data").then((result) => {
				...
				let localDataIndex = result.data.findIndex((item) => {
					return (
						item.forumId === data.forumId && item.repquote === data.repquote
					);
				});
				if (localDataIndex !== -1) {
	...
	async initReplyText(
		...
		replyUrl: string | null | undefined = undefined
	) {
		const that = this;
		let initResult = await this.$data.db.get<EditorNormalStorageOption[]>(
			"data"
		);
		...
		let queryResult = await this.$data.db.get<EditorNormalStorageOption[]>(
			"data"
		);
		...
		DOMUtils.on(this.$el.$like, "click", async (event) => {
			...
			if (unsafeWindow.comiis_recommend_key == 0) {
				...
				if (resultText.includes("您已评价过本主题")) {
					...
					if (
						document.querySelectorAll(".comiis_recommend_list_a").length > 0
					) {
						DOMUtils.remove("#comiis_recommend_list_a" + unsafeWindow.uid);
					}
					if (
						document.querySelectorAll(".comiis_recommend_list_s").length > 0
					) {
						DOMUtils.remove("#comiis_recommend_list_s" + unsafeWindow.uid);
					}
					if (
						document.querySelectorAll(".comiis_recommend_list_t").length > 0
					) {
					...
					if (recommend_num > 1) {
						DOMUtils.text(
							...
							recommend_num - Number(unsafeWindow.allowrecommend)
						);
						DOMUtils.text(
					...
					if (
						...
						if (
							document.querySelectorAll(".comiis_recommend_list_s li").length <
							7
						) {
					...
					resultText.includes(
						"'recommendv':'+" + unsafeWindow.allowrecommend + "'"
					)
				) {
					var recommendcList = {
							recommendc: 0,
					...
					if (
						document.querySelectorAll(".comiis_recommend_new span").length < 1
					) {
						DOMUtils.append(
					...
					if ($$(".comiis_recommend_list_t").length > 0) {
						...
						DOMUtils.prepend(
							...
							`<span id="comiis_recommend_list_t${unsafeWindow.uid}">
								...
								unsafeWindow.username
							}</a>
													${
														$$(".comiis_recommend_list_t a").length > 0
															? '<span class="f_d"> , </span>'
															: ""
													}
                        						</span>`
					...
					DOMUtils.text(
						".comiis_recommend_num",
						comiis_recommend_num + Number(unsafeWindow.allowrecommend)
					);
					DOMUtils.text(
						".comiis_recommend_nums",
						"+" + (comiis_recommend_num + Number(unsafeWindow.allowrecommend))
					...
					Qmsg.success(
						"点赞成功" +
							(recommendcList["daycount"]
								? `, 您今天还能点赞 ${recommendcList["daycount"] - 1} 次`
								: "")
					);
				} else if (
					resultText.indexOf(
		...
		DOMUtils.on(
			...
			async (event) => {
				...
				let reply_url = requestDOM
					.querySelector<HTMLAnchorElement>(".comiis_tip .tip_tit a")
					?.getAttribute("href")!;
				let reply_user = DOMUtils.text(
					requestDOM.querySelector<HTMLSpanElement>(".comiis_tip span.f_0")!
				);
				let reply_content = DOMUtils.val(
					requestDOM.querySelector<HTMLInputElement>(
						"input[name='noticeauthormsg']"
					)!
				);

				let reply_action = DOMUtils.attr(
					requestDOM.querySelector<HTMLFormElement>("#postforms")!,
					"action"
				);
	...
	setGlobalClickCheckEvent() {
		...
		DOMUtils.on(document, "click", function (this: HTMLElement, event) {
			...
			if (
				$(".popups-popmenu") ||
				MTEditorOptimizationNormal.$data.isUBBCodeInsertClick
			) {
				/* 当前存在弹出层，不做处理 */
				...
				window.event &&
				!utils.checkUserClickInNode($("#comiis_foot_menu_beautify_big")!)
			) {
				/* 当前点击的不在底部编辑框区域内 */
				...
				if (
					DOMUtils.attr("#comiis_foot_menu_beautify_big", "data-model") ==
					"reply"
				) {
					/* 当前编辑框模式为回复某人评论 */
					DOMUtils.attr(
	...
	deleteReplyTextStorage(
		isUserReply: boolean = false,
		replyUrl: string | undefined | null = undefined
	) {
		const that = this;
			...
			let localDataIndex = result.data.findIndex((item) => {
				if (isUserReply) {
					return (
						...
						replyUrl &&
						item.repquote === MTUtils.getRepquote(replyUrl)
					);
				} else {
