...
export const MTForumPost = {
	...
	addCommentOnBtn() {
		log.info(`添加【点评】按钮`);
		utils
			.waitNodeList<NodeListOf<HTMLElement>>(
			...
			.then(($bottomZhanList) => {
				$bottomZhanList.forEach(($bottmZhan) => {
					...
					var replyUrl =
						replyNode.getAttribute("datahref") ||
						replyNode.getAttribute("data-href") ||
						replyNode.href ||
						"";
					...
					var reviewsUserName =
						$postli.querySelector(".top_user.f_b")?.textContent || "";
					...
					DOMUtils.on(reviewsNode, "click", function () {
						utils
							.waitNode<HTMLSpanElement>("div[id=ntcmsg_popmenu]>div>span.f_c")
							.then((element) => {
								try {
	...
	loadNextPageComment() {
		...
		function autoLoadNextPageComments(post_comments_list: HTMLElement) {
			...
			let $nextPage = Array.from(
				post_comments_list.querySelectorAll<HTMLAnchorElement>("a[href]")
			).find((item) => {
		...
		if (commentsNum >= 10) {
			utils.waitNode<HTMLElement>(".comiis_page.bg_f").then(($nextPage) => {
	...
	setAttachmentsClickTip() {
		...
		function handleClick($ele: HTMLAnchorElement) {
			if ($ele.hasAttribute("href")) {
				...
				let attachmentNameNode =
					$ele.querySelector<HTMLSpanElement>("span.f_ok")!;
				let attachmentDownloadInfo =
					$ele.querySelector<HTMLElement>(".attach_size")!;
		...
		utils
			.waitNodeList<NodeListOf<HTMLAnchorElement>>(".attnm a")
			.then((nodeList) => {
				nodeList.forEach((item) => {
					handleClick(item);
				});
		...
		utils
			.waitNodeList<NodeListOf<HTMLAnchorElement>>(".comiis_attach a")
			.then((nodeList) => {
				nodeList.forEach((item) => {
					handleClick(item);
				});
