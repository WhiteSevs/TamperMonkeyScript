...
type ImageBedOption<T> = {
   ...
   * + false 不允许触发上传事件
   */
  uploadBtnClickEvent: (event: PointerEvent | MouseEvent) => boolean | Promise<boolean>;
  /**
   * 图片上传的事件
   * @param event
   * @returns
...
export class MTEditorImageBed<T> {
  ...
  constructor(option: ImageBedOption<T>) {
    ...
    DOMUtils.on(`.comiis_post_imglist[data-chartbed="${this.option.name}"] .up_btn a`, "click", async (event) => {
      ...
      if (result) {
        // 允许上传
        $<HTMLInputElement>(`.comiis_post_imglist[data-chartbed="${this.option.name}"] input[type="file"]`)!.click();
      } else {
        // 禁止上传
      }
    });
      ...
      async (event) => {
        ...
        if ($file.files && $file.files.length) {
          ...
          if (Panel.getValue<boolean>("mt-image-bed-watermark-enable")) {
            ...
            await Promise.all(
              Array.from($file.files).map(async (item, index) => {
                if (item.type === "image/gif") {
                  ...
                  watermark.addText({
                    text: [Panel.getValue<string>("mt-image-bed-watermark-text")],
                    color: Panel.getValue<string>("mt-image-bed-watermark-text-color"),
                    fontSize: Panel.getValue<number>("mt-image-bed-watermark-font-size"),
                    globalAlpha: Panel.getValue<number>("mt-image-bed-watermark-font-opacity"),
                    xMoveDistance: Panel.getValue<number>("mt-image-bed-watermark-left-right-margin"),
                    yMoveDistance: Panel.getValue<number>("mt-image-bed-watermark-top-bottom-margin"),
                    rotateAngle: Panel.getValue<number>("mt-image-bed-watermark-rotate"),
                  });
            ...
            if (Panel.getValue<boolean>("mt-image-bed-watermark-autoAddWaterMark")) {
  ...
  addTab() {
    ...
    let $historyBox = Array.from($$<HTMLElement>("#comiis_pictitle_tab .comiis_upbox")).find((item) =>
    ...
    if (!$historyBox) {
      ...
      $historyBox = Array.from($$<HTMLElement>("#comiis_pictitle_tab .comiis_upbox")).find((item) =>
        Boolean(item.querySelector("#imglist_history"))
      );
    }
    DOMUtils.before(
  ...
  initHistoryUploadImageList() {
    let $imglist_history = $<HTMLUListElement>("#comiis_pictitle_tab #imglist_history")!;
   ...
   * 设置图片缩略图的删除事件
   */
  setImageBtnDeleteEvent($ele: HTMLElement, data: ImageBedUploadImageReusltInfo<T>) {
