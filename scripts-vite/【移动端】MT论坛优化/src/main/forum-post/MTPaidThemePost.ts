  ...
  init() {
    ...
    if (MTRouter.isPost()) {
      ...
      if ($kmren) {
        ...
        let $header_y = document.querySelector<HTMLDivElement>(".comiis_head.f_top .header_y")!;
    ...
    if (MTRouter.isMySpace() || MTRouter.isGuide() || MTRouter.isForumList() || MTRouter.isPlate()) {
      /* 当前网页为，底部导航-我的 */
      /* 底部导航-我的-右上角小红点 */
      let redBtn = document.querySelector<HTMLDivElement>(".icon_msgs.bg_del.f_f");
      let tipNums = 0;
      if (redBtn) {
    ...
    /* 当前网页为，全部 */
    let redBtn = document.querySelector<HTMLElement>(".sidenv_num.bg_del.f_f"); /* 侧边栏-头像-右上角小红点 */
    let tipNums = 0;
    if (redBtn) {
      tipNums = parseInt(DOMUtils.text(redBtn));
