...
export const MTSmallWindow = {
  ...
  showSmallWindow(title: string, url: string, imagesList: string[] = []) {
    ...
    let $webSiteIcon = $drawer.$shadowRoot.querySelector<HTMLElement>(".small-window-website-icon")!;
    let $refreshIcon = $drawer.$shadowRoot.querySelector<HTMLElement>(".refresh-icon")!;

    let $imageIcon = $drawer.$shadowRoot.querySelector<HTMLElement>(".small-window-control-image-view")!;
    let $openBlankIcon = $drawer.$shadowRoot.querySelector<HTMLElement>(".small-window-control-open-blank")!;
    let $closeIcon = $drawer.$shadowRoot.querySelector<HTMLElement>(".small-window-control-close")!;

    let $titleText = $drawer.$shadowRoot.querySelector<HTMLElement>(".small-window-title-text")!;
  ...
  async handleForumPost(forumlist: HTMLElement[]) {
    forumlist.forEach(($forum) => {
      ...
      /* 帖子标题 */
      let title = DOMUtils.text($forum.querySelector<HTMLAnchorElement>(".mmlist_li_box h2 a")!);
      // 防止获取的标题是空的
      if (title == "" || title == null) {
        title = DOMUtils.text($forum.querySelector<HTMLAnchorElement>(".mmlist_li_box a")!);
      }
      title = title.trim();
      /* 帖子地址 */
      let url = $forum.querySelector<HTMLAnchorElement>(".mmlist_li_box a")!.href;
      /* 帖子内图片列表 */
      ...
      $forum.querySelectorAll<HTMLImageElement>(".comiis_pyqlist_img img").forEach(($img) => {
        imagesList.push($img.getAttribute("src")!);
      });
      $forum.querySelectorAll<HTMLImageElement>(".comiis_pyqlist_imgs img").forEach(($img) => {
        imagesList.push($img.getAttribute("src")!);
      });
      $forum.querySelectorAll<HTMLAnchorElement>(".mmlist_li_box a").forEach(($anchor) => {
        $anchor.removeAttribute("href");
        $anchor.setAttribute("data-href", url);
      });
      let $mmlist_li_box = $forum.querySelector<HTMLDivElement>(".mmlist_li_box");
