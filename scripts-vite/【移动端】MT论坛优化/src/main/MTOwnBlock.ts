			...
			document
				...
				.forEach((item) => {
					let postForumInfo: BlockOption["data"] = {
						...
						postUrl:
							item
								.querySelector<HTMLAnchorElement>(".mmlist_li_box a")!
								.getAttribute("href") ||
							item
								.querySelector<HTMLAnchorElement>(".mmlist_li_box a")!
						...
						/* 帖子标题 */
						postTitle:
							item.querySelector<HTMLAnchorElement>(".mmlist_li_box h2 a")
								?.innerText || "",
						/* 帖子内容(缩略) */
						postContent: item.querySelector<HTMLElement>(
							".mmlist_li_box .list_body"
						)!.innerText,
						/* 帖子板块 */
						postPlateName: (item.querySelector<HTMLAnchorElement>(
					...
					if (utils.isNull(postForumInfo.postPlateName)) {
						postForumInfo.postPlateName = document.querySelector<HTMLElement>(
							"#comiis_wx_title_box"
						)!.innerText;
					}
					/* console.log(`
				...
				.forEach((item) => {
					let postForumInfo: BlockOption["data"] = {
						...
						userName: item
							.querySelector("h2")!
							.innerText.replace(
								item.querySelector<HTMLSpanElement>("h2 span")!.innerText,
								""
							)
							.replace(/\s*/, ""),
						/* 用户UID */
						...
						/* 帖子内容(缩略) */
						postContent: item
							.querySelector<HTMLElement>("p.f_c")!
							.innerText!.trim(),
						/* 帖子板块 */
						postPlateName: undefined as any,
					};
