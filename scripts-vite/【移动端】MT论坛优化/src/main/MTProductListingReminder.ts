...
export const MTProductListingReminder = {
  ...
  async runRule() {
    ...
    async function getCurrentProduct() {
      ...
      doc.querySelectorAll<HTMLLIElement>(".task-list-wrapper li.col-xs-12").forEach(($taskList) => {
        productInfoList.push({
          name:
            DOMUtils.text($taskList.querySelector<HTMLElement>(".mall-info a > *:first-child")!) ||
            DOMUtils.text($taskList.querySelector<HTMLElement>(".mall-info a")!),
          price: DOMUtils.text($taskList.querySelector<HTMLElement>(".mall-info span.discount-price i")!),
          endTime: DOMUtils.text($taskList.querySelector<HTMLElement>(".mall-info #time_hz span.time")!),
