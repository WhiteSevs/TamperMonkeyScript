...
export const MTAutoSignIn = {
  ...
  checkLogin() {
    if (MTUtils.envIsMobile()) {
      /* 移动端的退出按钮，不登录是不会出现的 */
      let mobile_login_exitBtn = $<HTMLAnchorElement>("a[href*='member.php?mod=logging&action=logout']");
  ...
  async sign() {
    ...
    let unknownSignContentCallback = (content: string) => {
      ...
      let $content = $alert.$shadowRoot.querySelector<HTMLElement>(".pops-alert-content")!;
