...
export const MTSign = {
	...
	async showTodaySignStar() {
		...
		let todayStarParent = document.querySelector<HTMLElement>(
		...
		`;
		let $comiisSpaceBox =
			document.querySelector<HTMLElement>(".comiis_space_box")!;
	...
	showTodayRanking() {
		log.info(`显示【今日最先】`);
		let today_ranking_ele = document.querySelector<HTMLElement>(
		...
		let getMaxPage = async (urlextra: string) => {
			...
			let doc = DOMUtils.parseHTML(response.data.responseText, true, true);
			let last_page = doc.querySelector<HTMLSpanElement>(
		...
		let getPagePeople = async (page: string | number) => {
			...
			let doc = DOMUtils.parseHTML(response.data.responseText, true, true);
			let peoples = doc.querySelectorAll<HTMLTableColElement>(
