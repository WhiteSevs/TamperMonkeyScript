...
`,se={$data:{},init(){this.registerMenu();},registerMenu(){O.registerLeftMenu({name:"在线用户",iconColor:"#2d92ff",icon:"",callback:()=>{this.showOnlineUser();}});},async showOnlineUser(){let e=f.loading("正在获取在线用户名单中..."),t=await this.getOnlineUserListInfo();if(e.close(),!t)return;let i=D.alert({title:{text:"在线用户",position:"center"},content:{text:`
...
`)}    
		...
		.pg_k_misign .comiis_qdinfo{
			height: 110px !important;
					...
					</div>`;o(d,r);}else V.get(`plugin.php?id=k_misign:sign&operation=${urlextra}`,{responseType:"html",fetch:true}).then(l=>{let c=l.data.responseText;c=c.replace("今日排行</a></li>",`今日排行</a></li>
							<li class="flex">
...
`);P.length==1?R=`<li>${q}</li>`:Array.from(P).forEach((Q,at)=>{at==P.length-1?R=`${R}<li>${Q}</li>`:R=`${R}<li>${Q}<br></li>`;}),e=e.replace(w,`
        ...
        `);let t=$("#select-post-section"),i=N.getForumId(window.location.href);T.isPostPublish_newthread()?(u.info("发帖"),a.on(t,"change",function(){let n=a.val(t),s=`forum.php?mod=post&action=newthread&fid=${n}&extra=&topicsubmit=yes&mobile=2`;u.info(`修改发帖板块: ${e[n]} ${s}`);let o={求助问答:{className:"gm_user_select_help",optionHTML:`<option value="0" selected="selected">请选择</option>
        ...
        <a href="javascript:;" class="comiis_pictitle"><i class="comiis_font"><em>图片</em></i></a>`),a.on(".comiis_pictitle","click",function(){this.querySelector("i.comiis_font").classList.contains("f_0")?a.hide(".gm_plugin_chartbed",false):a.show(".gm_plugin_chartbed",false);}),a.append("#comiis_post_tab",`
            ...
            `);let t=$("#imglist"),i=a.parent(t);a.before(".gm_plugin_chartbed .bqbox_t",i),a.on("#imglist .comiis_font","click",n=>{$("#filedata").click();}),a.on("#comiis_pictitle_tab #comiis_pictitle_key","click","li",function(n){let s=n.target;a.removeClass("#comiis_pictitle_tab #comiis_pictitle_key li","bg_f"),a.addClass(s,"bg_f"),b.$("#comiis_pictitle_tab div.comiis_upbox").hide().eq(b.$(s).index()).fadeIn();}),v.execMenuOnce("mt-image-bed-hello-enable",()=>{zt.init();}),v.execMenuOnce("mt-image-bed-mt-enable",()=>{Nt.init();});},initPreview(){const e=this;u.info("添加功能：双列预览"),G(`
