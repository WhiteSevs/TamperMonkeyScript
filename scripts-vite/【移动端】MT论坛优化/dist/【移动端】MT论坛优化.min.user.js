...
`)}
        ...
        `;if(!i){let o=r.createElement("li");i=r.createElement("a",{href:"javascript:;",className:"comiis-picture-tab",innerHTML:"历史图片"},{"data-type":"history"}),r.on(i,"click",()=>{this.initHistoryUploadImageList();}),r.append(o,i),r.append(e,o);}r.before(i.parentElement,n);let s=C("#comiis_pictitle_tab .bqbox_t"),l=Array.from(B("#comiis_pictitle_tab .comiis_upbox")).find(o=>!!o.querySelector("#imglist_history"));l||(l=r.createElement("div",{className:"comiis_upbox bg_f cl",innerHTML:`
					<ul class="comiis_post_imglist cl" id="imglist_history">	
                    </ul>
				`},{style:"display: none;"}),r.before(s,l),l=Array.from(B("#comiis_pictitle_tab .comiis_upbox")).find(o=>!!o.querySelector("#imglist_history"))),r.before(l,`
...
`);U.length==1?z=`<li>${T}</li>`:Array.from(U).forEach((J,at)=>{at==U.length-1?z=`${z}<li>${J}</li>`:z=`${z}<li>${J}<br></li>`;}),t=t.replace(q,`
                ...
                `,y.$(".gm_plugin_previewpostforum_html .postforum_vote").remove(),y.$(".gm_plugin_previewpostforum_html .comiis_messages.comiis_aimg_show").children().eq(0).before(y.$(l));}},ht={$data:{db:new st.indexedDB("mt_full_reply_record","input_text"),get type(){return M.isPostPublish_voting()?"post-vote":"post"},get tid(){return F.getThreadId(window.location.href)},get pid(){return F.getPostId(window.location.href)}},$key:{noPublishSerializeText:"mt-editor-no-publish-serialize-text",noPublishVotingSerializeText:"mt-editor-no-publish-voting-serialize-text"},$el:{$title:null,$input:null,$form:null},init(){b.info("编辑器优化"),W(le),this.overridePageEditor();},overridePageEditor(){const t=this;this.$el.$title=C("#needsubject"),this.$el.$form=C("#postform"),this.$el.$input=C("#needmessage"),r.hide(r.parent(".comiis_scrollTop_box"),false),r.css("#postform .comiis_post_from.mt15",{"margin-top":"0px !important"});let e=y.$("#postform .comiis_post_from #comiis_post_tab");y.$("#postform .comiis_post_from .comiis_post_ico").append(e),e.remove(),y.textarea_scrollHeight=()=>{};let i=y.$.fn.comiis_delete;y.$.fn.extend({comiis_delete:function(...c){let m=i.call(this,...c);return g.dispatchEvent(t.$el.$input,"input"),m}}),r.hide(".comiis_btnbox",false),this.initVotePage(),y.$(".gm_plugin_chartbed .comiis_over_box.comiis_input_style #imglist"),W(`
        ...
        `);let t=Bt(),e=C(".comiis_post_urlico > ul"),i=C("#comiis_post_qydiv > ul"),n=B("#comiis_post_qydiv ul li").length;re(),r.on("#comiis_post_tab .comiis_input_style .comiis_post_urlico li","click",function(){r.removeClass("#comiis_post_tab .comiis_input_style .comiis_post_urlico li a","f_0"),r.addClass("#comiis_post_tab .comiis_input_style .comiis_post_urlico li a","f_d"),r.attr(this.querySelector("a"),"class","comiis_xifont f_0"),y.$("#comiis_post_qydiv ul li").hide().eq(y.$(this).index()).fadeIn();}),y.$.each(t,function(s,l){let o=r.createElement("li",{className:"quickUBBs",innerHTML:`
            ...
            <div class="comiis_styli_m f15 comiis_flex" style="padding-top:0;">
                ...
                <div class="flex"></div>
            </div>`,r.append(i,a),r.on(`.comiis_sendbtn[data-keyI="${s}"]`,"click",()=>{let c=y.$(`#comiis_input_${s}`).val();if(c==""){x.warning("请输入需要插入的内容");return}let m=t[s];m.isFunc&&(c=Ft(m.num,c)),m.hasOwnProperty("L")&&(c=m.L+c+m.R),y.$("#needmessage").insertAtCaret(c),m.hasOwnProperty("cursorL")&&y.$("#needmessage").moveCursorToCenterByTextWithLeft(m.cursorL,m.cursorLength),m.hasOwnProperty("cursorR")&&y.$("#needmessage").moveCursorToCenterByTextWithRight(m.cursorR,m.cursorLength);});});},initImage(){b.info("添加功能：图片"),W(`
        ...
        <a href="javascript:;" class="swiper-slide gm_plugin_previewpostforum">
            ...
            </div>`),r.on(".gm_plugin_previewpostforum","click",function(i){let n=this;if(B("#polldatas").length&&xt.parseVoteText(),n.querySelector("i.comiis_font").classList.contains("f_0"))r.hide(".gm_plugin_previewpostforum_html",false);else {r.show(".gm_plugin_previewpostforum_html",false);let l=xt.parseText(r.val(t.$el.$input));r.html(".gm_plugin_previewpostforum_html .comiis_message_table",l),r.css(".gm_plugin_previewpostforum_html.double-preview .comiis_over_box.comiis_input_style","height",r.css(t.$el.$input,"height"));}});},initSettingImmersionMode(){b.info("初始化设置功能-使用沉浸模式"),r.append(r.parent("#htmlon"),`
            <li class="comiis_styli_m f15 comiis_flex b_b">
...
`,_e={$data:{},init(){this.registerMenu();},registerMenu(){j.registerLeftMenu({name:"在线用户",iconColor:"#2d92ff",icon:"",callback:()=>{this.showOnlineUser();}});},async showOnlineUser(){let t=x.loading("正在获取在线用户名单中..."),e=await this.getOnlineUserListInfo();if(t.close(),!e)return;let i=N.alert({title:{text:"在线用户",position:"center"},content:{text:`
        ...
        }`)},removeFontStyle(){b.info("移除帖子字体效果");let t=C(".comiis_a.comiis_message_table");t&&r.html(t,r.html(t).replace(X.fontSpecial,""));},removeCommentFontStyle(){var i;b.info("移除评论区的字体效果");let t=B("font"),e=((i=C(".comiis_postlist .comiis_postli"))==null?void 0:i.innerHTML)||"";e!==""&&(t.forEach(n=>{e.includes(n.innerHTML)||(n.removeAttribute("color"),n.removeAttribute("style"),n.removeAttribute("size"));}),B(".comiis_message.message").forEach(n=>{if(e.includes(n.innerHTML)){n.innerHTML=n.innerHTML.replace(X.fontSpecial,"");let s=n.nextElementSibling;s&&s.localName==="strike"&&(s.outerHTML=s.outerHTML.replace(/^<strike>(\n|)/g,"").replace(/<\/strike>$/g,""));}})),B(".comiis_postli.comiis_list_readimgs.nfqsqi").forEach(n=>{let s=n.parentElement;s&&s.localName==="strike"&&(s.outerHTML=s.outerHTML.replace(/^<strike>(\n|)/g,"").replace(/<\/strike>$/g,""));});},addCommentOnBtn(){b.info("添加【点评】按钮"),g.waitNodeList(".bottom_zhan:not([data-isaddreviews])").then(t=>{t.forEach(e=>{var u,f,p;e.setAttribute("data-isaddreviews","true");var i=e.querySelector("a");if(i){var n=i.getAttribute("datahref")||i.getAttribute("data-href")||i.href||"",s=n.replace("mod=post&","mod=misc&").replace("action=reply&","action=comment&"),l=(u=n==null?void 0:n.match(/&page=([\w]+)/i))==null?void 0:u[1],o=`${s}&extra=page%3D1&page=${l}`,a=e==null?void 0:e.closest(".comiis_postli[id]"),c=(f=a.getAttribute("id"))==null?void 0:f.replace("pid","&pid=");o=o+c;var m=((p=a.querySelector(".top_user.f_b"))==null?void 0:p.textContent)||"",d=r.parseHTML(`
