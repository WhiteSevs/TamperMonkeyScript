...
`,ge={$data:{cid:""},init(){this.registerMenu();},registerMenu(){j.registerLeftMenu({name:"小黑屋",iconColor:"#000000",icon:"",callback:()=>{this.showBlackHome();}});},async showBlackHome(){let t=x.loading("正在获取小黑屋名单中..."),e=await this.getBlackListInfo("");if(t.close(),!e)return;if(e.data.length===0){x.error("获取小黑屋名单为空");return}this.$data.cid=e.next_cid;let n=B.confirm({title:{text:"小黑屋名单",position:"center"},content:{text:`
                ...
                `,html:!0},btn:{ok:{text:"下一页",callback:async()=>{let a=x.loading("正在获取小黑屋名单中...");b.info("下一页的cid: ",this.$data.cid);let c=await this.getBlackListInfo(this.$data.cid);a.close(),c&&(this.$data.cid=c.next_cid,c.data.forEach(m=>{let d=this.createListViewItem(m);i.appendChild(d);}),c.data.length===0?x.error("获取小黑屋名单为空"):x.success(`成功获取 ${c.data.length}条数据`));}},close:{text:"关闭"}},width:"88vw",height:"82vh",style:he,mask:{enable:!0}}),i=n.$shadowRoot.querySelector(".blackhome-user-list"),s=n.$shadowRoot.querySelector(".blackhome-user-filter input");e.data.forEach(a=>{let c=this.createListViewItem(a);i.appendChild(c);}),x.success(`成功获取 ${e.data.length}条数据`);let l=!1;r.on(s,["input","propertychange"],g.debounce(()=>{let a=s.value.trim();if(!l){if(l=!0,a==""){n.$shadowRoot.querySelectorAll(".blackhome-user-item").forEach(c=>{c.removeAttribute("style");}),l=!1;return}n.$shadowRoot.querySelectorAll(".blackhome-user-item").forEach(c=>{c.getAttribute("data-name").match(new RegExp(a,"ig"))||c.getAttribute("data-uid").trim().match(new RegExp(a,"ig"))||c.getAttribute("data-operator").match(new RegExp(a,"ig"))?c.removeAttribute("style"):c.setAttribute("style","display:none;");}),l=!1;}}));let o=await this.getBlackListInfo(this.$data.cid);o&&(this.$data.cid=o.next_cid);},async getBlackListInfo(t=""){let e={mod:"misc",action:"showdarkroom",cid:t,ajaxdata:"json"},n=await V.get(`/forum.php?${g.toSearchParamsStr(e)}`,{headers:{"User-Agent":g.getRandomPCUA()},responseType:"json"});if(!n.status)return;let i=g.toJSON(n.data.responseText),s=i.message.split("|"),l=s[s.length-1],o=g.parseObjectToArray(i.data),a=[],c=[];return o.forEach(m=>{let d=m.dateline.match(/([0-9]{4}-[0-9]{1,2}-[0-9]{1,2}[\s]*[0-9]{1,2}:[0-9]{1,2})/g);if(d==null){let u=parseInt((Date.now()/1e3).toString()),f=0,p=m.dateline.match(/([0-9]+|半)[\s\S]*秒前/),h=m.dateline.match(/([0-9]+|半)[\s\S]*分钟前/),_=m.dateline.match(/([0-9]+|半)[\s\S]*小时前/),k=m.dateline.match(/昨天[\s\S]*(\d{2}):(\d{2})/),v=m.dateline.match(/前天[\s\S]*(\d{2}):(\d{2})/),T=m.dateline.match(/([0-9]+|半)[\s\S]*天前/);if(p)p=p[p.length-1],p=p.replace(/半/g,.5),p=parseFloat(p),f=u-p;else if(h)h=h[h.length-1],h=h.replace(/半/g,.5),h=parseFloat(h),f=u-h*60;else if(_)_=_[_.length-1],_=_.replace(/半/g,.5),_=parseFloat(_),f=u-_*60*60;else if(k){let R=k[1],C=k[2];f=u-86400-parseInt(R)*3600-parseInt(C)*60;}else if(v){let R=v[1],C=v[2];f=u-86400*2-parseInt(R)*3600-parseInt(C)*60;}else T&&(T=T[T.length-1],T=T.replace(/半/g,.5),T=parseFloat(T),f=u-T*60*60*24);m.time=parseInt(f.toString())*1e3,a=a.concat(m);return}else d=d[0];m.time=g.formatToTimeStamp(d),a=a.concat(m);}),g.sortListByProperty(a,"time"),g.sortListByProperty(c,"time",!1),a=a.concat(c),{next_cid:l,data:a}},createListViewItem(t){let e=r.createElement("div",{className:"blackhome-user-item",innerHTML:`
...
`;class qe{constructor(e){nt(this,"isBacking",!1);nt(this,"config");this.config=e,this.enterGestureBackMode.bind(this),this.quitGestureBackMode.bind(this),this.popStateEvent.bind(this),(typeof this.config.backDelayTime!="number"||isNaN(this.config.backDelayTime))&&(this.config.backDelayTime=150),this.config.win==null&&(this.config.win=self);}popStateEvent(e){g.preventEvent(e),!this.isBacking&&this.quitGestureBackMode(!0);}enterGestureBackMode(){b.success("进入手势模式");let e=this.config.hash;e.startsWith("#")||(e.startsWith("/")||(e="/"+e),e="#"+e),this.config.useUrl&&(e=this.config.win.location.origin+this.config.win.location.pathname+this.config.win.location.search+e),this.config.win.history.pushState({},"",e),b.success("监听popstate事件"),r.on(this.config.win,"popstate",this.popStateEvent.bind(this),{capture:!0});}async quitGestureBackMode(e=!1){this.isBacking=!0,b.success("退出手势模式"),typeof this.config.beforeHistoryBackCallBack=="function"&&this.config.beforeHistoryBackCallBack(e);let n=Date.now()+1e3*5;for(;;){if(Date.now()>n){b.error("未知情况，history.back()失败，无法退出手势模式");break}if(this.config.win.location.hash.endsWith(this.config.hash))b.info("history.back()"),this.config.win.history.back(),await g.sleep(this.config.backDelayTime||150);else break}b.success("移除popstate事件"),r.off(this.config.win,"popstate",this.popStateEvent.bind(this),{capture:!0}),this.isBacking=!1,typeof this.config.afterHistoryBackCallBack=="function"&&this.config.afterHistoryBackCallBack(e);}}const ct={https:`
        ...
        <div class="small-window-title-container">
            ...
            `);var n="";g.sortListByProperty(e,s=>{var l=s.href.match(/thread-(.+?)-/i);return parseInt(l[l.length-1])},!0),b.info("导读内容",e),e.forEach(s=>{n+=`
                ...
                </li>`;});let i=document.querySelector(".comiis_forumlist.comiis_xznlist");X.before(i,`<div class="comiis_mh_kxtxt bg_f comiis_mh_kxtxt_owm"><ul>${n}</ul></div>`);});}};class Ht{constructor(e){nt(this,"option");this.option=e;}showView(){var o;let e=B.confirm({title:{text:this.option.title,position:"center"},content:{text:`
            ...
            `,width:window.innerWidth>500?"500px":"88vw",height:window.innerHeight>500?"500px":"80vh"}),n=e.$shadowRoot.querySelector(".rule-form-container");e.$shadowRoot.querySelector("input[type=submit]");let i=e.$shadowRoot.querySelector(".rule-form-ulist"),s=this.option.getView(this.option.data());i.appendChild(s);const l=()=>{this.option.onsubmit(n,this.option.data()).success&&(e.close(),this.option.dialogCloseCallBack(!0));};}}class ke{constructor(e){nt(this,"option");this.option=e;}showView(){let e=B.alert({title:{text:this.option.title,position:"center"},content:{text:`
            ...
            `});this.option.data().forEach(f=>{this.appendRuleItemElement(e.$shadowRoot,f);});}parseViewElement(e){let n=e.querySelector(".rule-view-container"),i=e.querySelector(".pops-confirm-btn button.pops-confirm-btn-other");return {$container:n,$deleteBtn:i}}parseRuleItemElement(e){let n=e.querySelector(".rule-controls-enable"),i=n.querySelector(".pops-panel-switch"),s=n.querySelector(".pops-panel-switch__input"),l=n.querySelector(".pops-panel-switch__core"),o=e.querySelector(".rule-controls-edit"),a=e.querySelector(".rule-controls-delete");return {$enable:n,$enableSwitch:i,$enableSwitchInput:s,$enableSwitchCore:l,$edit:o,$delete:a,data:Reflect.get(e,"data-rule")}}createRuleItemElement(e,n){let i=this.option.getDataItemName(e),s=r.createElement("div",{className:"rule-item",innerHTML:`
