...
`)}
								...
								`});}else await l(o);}});}addTab(){let e=E("#comiis_pictitle_key"),n=e.querySelector("a[data-type='history']"),i=`
            <li>
                <a href="javascript:;" class="comiis-picture-tab" data-type="image-bed">${this.option.name}</a>
            </li>
        `;if(!n){let o=r.createElement("li");n=r.createElement("a",{href:"javascript:;",className:"comiis-picture-tab",innerHTML:"历史图片"},{"data-type":"history"}),r.on(n,"click",()=>{this.initHistoryUploadImageList();}),r.append(o,n),r.append(e,o);}r.before(n.parentElement,i);let s=E("#comiis_pictitle_tab .bqbox_t"),l=Array.from(O("#comiis_pictitle_tab .comiis_upbox")).find(o=>!!o.querySelector("#imglist_history"));l||(l=r.createElement("div",{className:"comiis_upbox bg_f cl",innerHTML:`
...
`);N.length==1?z=`<li>${T}</li>`:Array.from(N).forEach((Z,ot)=>{ot==N.length-1?z=`${z}<li>${Z}</li>`:z=`${z}<li>${Z}<br></li>`;}),t=t.replace($,`
                <div class="comiis_blockcode comiis_bodybg b_ok f_b"><div class="bg_f b_l"><ol>${z}</ol></div></div>`);});let s=t.match(/\[url\=[\s\S]*?\]([\s\S]*?)\[\/url\]/g);s&&s.forEach($=>{let w=$.match(/\[url=([\s\S]*?)\][\s\S]*\[\/url\]/),T=$.match(/\[url=[\s\S]*?\]([\s\S]*?)\[\/url\]/),z=w?w[w.length-1]:"",N=T?T[T.length-1]:"";t=t.replace($,`<a href="${z}" target="_blank">${N}</a>`);});let l=t.match(/\[color\=[\s\S]*?\]([\s\S]*?)\[\/color\]/g);l&&l.forEach($=>{let w=$.match(/\[color=([\s\S]*?)\][\s\S]*\[\/color\]/),T=$.match(/\[color=[\s\S]*?\]([\s\S]*?)\[\/color\]/),z=w?w[w.length-1]:"",N=T?T[T.length-1]:"";t=t.replace($,`<font color="${z}">${N}</font>`);});let o=t.match(/\[size\=[\s\S]*?\]([\s\S]*?)\[\/size\]/g);o&&o.forEach($=>{let w=$.match(/\[size=([\s\S]*?)\][\s\S]*\[\/size\]/),T=$.match(/\[size=[\s\S]*?\]([\s\S]*?)\[\/size\]/),z=w?w[w.length-1]:"",N=T?T[T.length-1]:"";t=t.replace($,`<font size="${z}">${N}</font>`);});let a=t.match(/\[img(|\=[\s\S]+?)\]([\s\S]*?)\[\/img\]/g);a&&a.forEach($=>{let w=null,T=null,z=$.match(/\[img\=([\s\S]+?)\][\s\S]*?\[\/img\]/);z&&(z=z[z.length-1].split(","),w=z[0],T=z[1]),w=w||"",T=T||"";let N=$.match(/\[img\]([\s\S]*?)\[\/img\]|\[img=[\s\S]*?\]([\s\S]*?)\[\/img\]/),Z="";N&&(N[N.length-1]==null?Z=N[N.length-2]:Z=N[N.length-1]),t=t.replace($,`<img loading="lazy" src="${Z}" border="0" alt="" width="${w}" height="${T}" crossoriginNew="anonymous">`);});let c=t.match(/\[hide\]([\s\S]*?)\[\/hide\]/g);c&&c.forEach($=>{let w=$.match(/\[hide\]([\s\S]*?)\[\/hide\]/),T=w?w[w.length-1]:"";t=t.replace($,`<div class="comiis_quote bg_h f_c"><h2 class="f_a">本帖隐藏的内容: </h2>${T}</div>`);});let m=t.match(/\[hide=[\s\S]*?\]([\s\S]*?)\[\/hide\]/g);m&&m.forEach($=>{let w=$.match(/\[hide=([\s\S]*?)\]([\s\S]*?)\[\/hide\]/),T=w?w[w.length-2]:"";T=T.split(",");let z=T.length==2?T[1]:"";t=t.replace($,`<div class="comiis_quote bg_h f_c">以下内容需要积分高于 ${z} 才可浏览</div>`);});let d=t.match(/\[quote\]([\s\S]*?)\[\/quote\]/g);d&&d.forEach($=>{let w=$.match(/\[quote\]([\s\S]*?)\[\/quote\]/),T=w?w[w.length-1]:"";t=t.replace($,`<div class="comiis_quote bg_h b_dashed f_c"><blockquote><font>回复</font> ${T}</blockquote></div>`);});let u=t.match(/\[free\]([\s\S]*?)\[\/free\]/g);u&&u.forEach($=>{let w=$.match(/\[free\]([\s\S]*?)\[\/free\]/),T=w?w[w.length-1]:"";t=t.replace($,`<div class="comiis_quote bg_h f_c"><blockquote>${T}</blockquote></div>`);});let f=t.match(/\[b\]([\s\S]*?)\[\/b\]/g);f&&f.forEach($=>{let w=$.match(/\[b\]([\s\S]*?)\[\/b\]/i),T=w?w[w.length-1]:"";t=t.replace($,`<strong>${T}</strong>`);});let p=t.match(/\[u\]([\s\S]*?)\[\/u\]/g);p&&p.forEach($=>{let w=$.match(/\[u\]([\s\S]*?)\[\/u\]/),T=w?w[w.length-1]:"";t=t.replace($,`<u>${T}</u>`);});let h=t.match(/\[i\]([\s\S]*?)\[\/i\]/g);h&&h.forEach($=>{let w=$.match(/\[i\]([\s\S]*?)\[\/i\]/),T=w?w[w.length-1]:"";t=t.replace($,`<i>${T}</i>`);});let _=t.match(/\[s\]([\s\S]*?)\[\/s\]/g);_&&_.forEach($=>{let w=$.match(/\[s\]([\s\S]*?)\[\/s\]/),T=w?w[w.length-1]:"";t=t.replace($,`<strike>${T}</strike>`);});let k=t.match(/\[([\s\S]+?)\]/g);k&&k.forEach($=>{let w=e[$];w&&(t=t.replace($,`<img loading="lazy" src="${w}" border="0" alt="" smilieid="">`));});let v=t.match(/\[media=[\s\S]+?\][\s\S]+?\[\/media\]/g);v&&v.forEach($=>{let w=$.match(/\[media=[\s\S]*?\]([\s\S]*?)\[\/media\]/),T=w?w[w.length-1]:"";T&&(t=t.replace($,`<ignore_js_op><span><iframe src="${T}" border="0" scrolling="no" framespacing="0" allowfullscreen="true" style="max-width: 100%" width="100%" height="auto" frameborder="no"></iframe></span></ignore_js_op>`));});let S=t.match(/\[email=[\s\S]+?\][\s\S]+?\[\/email\]/g);S&&S.forEach($=>{let w=$.match(/\[email=([\s\S]*?)\][\s\S]*?\[\/email\]/),T=$.match(/\[email=[\s\S]*?\]([\s\S]*?)\[\/email\]/),z=w.length?w[w.length-1]:"",N=T.length?T[T.length-1]:"";(z||N)&&(t=t.replace($,`<a href="mailto:${z}">${N}</a>`));});let A=t.match(/\[align=[\s\S]+?\][\s\S]+?\[\/align\]/g);A&&A.forEach($=>{let w=$.match(/\[align=([\s\S]*?)\][\s\S]+?\[\/align\]/),T=$.match(/\[align=[\s\S]*?\]([\s\S]+?)\[\/align\]/),z=w.length?w[w.length-1]:"",N=T.length?T[T.length-1]:"";(z||N)&&(t=t.replace($,`<div align="${z}">${N}</div>`));});let C=t.match(/\[qq\][\s\S]*?\[\/qq\]/g);C&&C.forEach($=>{let w=$.match(/\[qq\]([\s\S]*?)\[\/qq\]/),T=w?w[w.length-1]:"";t=t.replace($,`<a href="http://wpa.qq.com/msgrd?v=3&uin=${T}&site=[Discuz!]&from=discuz&menu=yes" target="_blank"><img loading="lazy" src="static/image/common/qq_big.gif" border="0"></a>`);});let D=t.match(/\[td\][\s\S]+?\[\/td\]/g);D&&D.forEach($=>{let w=$.match(/\[td\]([\s\S]*?)\[\/td\]/),T=w?w[w.length-1]:"";t=t.replace($,`<td>${T}</td>`);});let L=t.match(/\[tr\][\s\S]+?\[\/tr\]/g);L&&L.forEach($=>{let w=$.match(/\[tr\]([\s\S]*?)\[\/tr\]/),T=w?w[w.length-1]:"";t=t.replace($,`<tr>${T}</tr>`);});let M=t.match(/\[table\][\s\S]+?\[\/table\]/g);M&&M.forEach($=>{let w=$.match(/\[table\]([\s\S]*?)\[\/table\]/),T=w?w[w.length-1]:"";T=T.replace(/\n/g,""),t=t.replace($,`<table>${T}</table>`);});let H=t.match(/\[list=[\s\S]+?\][\s\S]+?\[\/list\]/g);return H&&H.forEach($=>{let w=$.match(/\[list=([\s\S]*?)\][\s\S]*?\[\/list\]/),T=$.match(/\[list=[\s\S]*?\]([\s\S]*?)\[\/list\]/),z=w?w[w.length-1]:"",N="";z==="a"?N="litype_2":z==="A"?N="litype_3":z.length===1&&z.match(/[0-9]{1}/)&&(N="litype_1");let Z=T?T[T.length-1]:"",ot=Z.split("[*]");if(ot.length>1){let _t="";ot[0].replace(/[\s]*/,"")==""&&(ot=ot.slice(1)),Array.from(ot).forEach(jt=>{_t=`${_t}<li>${jt}</li>`;}),Z=_t;}Z=Z.replace(/\n/g,""),t=t.replace($,`<ul type="${z}" class="${N}">${Z}</ul>`);}),t},parseVoteText(){let t=["rgb(233, 39, 37)","rgb(242, 123, 33)","rgb(242, 166, 31)","rgb(90, 175, 74)","rgb(66, 196, 245)","rgb(0, 153, 204)","rgb(51, 101, 174)","rgb(42, 53, 145)","rgb(89, 45, 142)","rgb(219, 49, 145)","rgb(233, 39, 37)","rgb(242, 123, 33)","rgb(242, 166, 31)","rgb(90, 175, 74)","rgb(66, 196, 245)","rgb(0, 153, 204)","rgb(51, 101, 174)","rgb(42, 53, 145)","rgb(89, 45, 142)","rgb(219, 49, 145)"],e=O(".comiis_polloption_add ul li:first-child div.flex .comiis_input.kmshow[type='text']"),n=parseInt(r.val("input#maxchoices"));n=isNaN(n)?0:n,n=n>0?n:0,n=n>e.length?e.length:n;let i=parseInt(r.val("input#polldatas"));i=isNaN(i)?0:i,y.$("input#visibilitypoll").parent().find(".comiis_checkbox").hasClass("comiis_checkbox_close");let s=!y.$("input#overt").parent().find(".comiis_checkbox").hasClass("comiis_checkbox_close"),l="",o="";e.forEach((a,c)=>{c>=20||(o=o+`
                    <li class="kmnop">
        ...
        `),r.attr("#filedata","multiple",!0),r.remove(".gm_plugin_chartbed .comiis_over_box.comiis_input_style"),r.on(document,"#comiis_pictitle_key li","click",function(){r.removeClass("#comiis_pictitle_key li","bg_f"),r.addClass(this,"bg_f"),y.$(".gm_plugin_chartbed .comiis_upbox").hide().eq(y.$(this).index()).fadeIn();});let i=parseInt(r.css("#comiis_head","height"))||0,s=parseInt(r.css("#comiis_sub","height"))||0,l=E("#pollm_c_1")?60:0,o=parseInt(r.css(".comiis_styli.comiis_flex","height"))||0,a=parseInt(r.css(".comiis_post_ico.comiis_minipost_icot","height"))||0;r.css("#needmessage","height",`${window.screen.height-i-s-48-o-a-10}px`),r.css("#needmessage","marginBottom",l+"px"),I.isPostPublish_edit()&&r.val("#needsubject")===""?r.hide(".comiis_styli.comiis_flex",!1):r.attr("#needsubject","placeholder","请输入完整的帖子标题 (1-80个字)"),r.attr("#needmessage","placeholder","来吧，尽情发挥吧..."),typeof y.comiis_addsmilies=="function"&&(y.comiis_addsmilies=c=>{y.$("#needmessage").comiis_insert(c),y.$("#needmessage")[0].dispatchEvent(new Event("input"));}),(q.getValue("mt-forum-post-editorOptimizationNormal-recordInputText")||q.getValue("mt-forum-post-editorOptimization-recordInputText"))&&(this.setInputChangeEvent(),this.initReplyText()),this.initDeleteBtn(),this.initSaveDraftBtn(),this.initSaveBtn(),this.initPostBtn(),this.initReplyBtn(),this.observerChangeEditorHeight(),this.listenResize(),this.initSelectPostingSection(),this.initUBB(),this.initImage(),this.initPreview(),this.initCharacterCount(),this.initSettingImmersionMode();},async initReplyText(){const t=this;let e=null,n=null,i=null;if(I.isPostPublish_newthread())b.info("新发布帖子的页面"),I.isPostPublish_voting()?(b.info("投票页面"),e=Y(this.$key.noPublishVotingSerializeText),i=()=>{at(t.$key.noPublishVotingSerializeText);}):(b.info("普通帖子页面"),e=Y(this.$key.noPublishSerializeText),i=()=>{at(this.$key.noPublishSerializeText);});else if(I.isPostPublish_edit()){b.info("草稿的页面"),b.info(`type：${this.$data.type} tid：${this.$data.tid} pid：${this.$data.pid}`),(await this.$data.db.get("data")).code===201&&await this.$data.db.save("data",[]);let l=await this.$data.db.get("data");if(l.data){let o=l.data.find(a=>{if(a.type===t.$data.type&&!(a.tid!==t.$data.tid||a.pid!==t.$data.pid))return !0});o&&(e=o.data,i=async()=>{let a=await this.$data.db.get("data");if(a.data){let c=a.data.findIndex(m=>{if(m.type===t.$data.type&&!(m.tid!==t.$data.tid||m.pid!==t.$data.pid))return !0});c!=-1&&(a.data.splice(c,1),await this.$data.db.save("data",a.data));}});}}else if(I.isPostPublish_reply()&&(b.info("回复页面"),q.getValue("mt-forum-post-editorOptimizationNormal-recordInputText"))){(await tt.$data.db.get("data")).code===201&&await this.$data.db.save("data",[]);let l=await tt.$data.db.get("data");if(l.data){let o=l.data.find(a=>a.forumId===t.$data.tid&&a.repquote===F.getRepquote(window.location.href));o&&(e=o);}}e&&(I.isPostPublish_voting()?n=()=>{let s=t.$el.$form.querySelector("input[name='subject']"),l=t.$el.$form.querySelector("textarea[name='message']"),o=t.$el.$form.querySelector("input[name='maxchoices']"),a=t.$el.$form.querySelector("input[name='expiration']"),c=t.$el.$form.querySelector("input[name='visibilitypoll']"),m=t.$el.$form.querySelector("input[name='overt']");return r.val(s,e.title),r.val(l,e.content),r.val(o,e.maxchoices),r.val(a,e.expiration),r.val(c,e.visibilitypoll),r.val(m,e.overt),g.dispatchEvent(s,"input"),g.dispatchEvent(l,"input"),g.dispatchEvent(o,"input"),g.dispatchEvent(a,"input"),g.dispatchEvent(c,"input"),g.dispatchEvent(m,"input"),!0}:I.isPostPublish_reply()?n=()=>{let s=t.$el.$form.querySelector("textarea[name='message']");return r.val(s,e.text),g.dispatchEvent(s,"input"),!0}:n=()=>{let s=t.$el.$form.querySelector("input[name='subject']"),l=t.$el.$form.querySelector("textarea[name='message']");return r.val(s,e.title),r.val(l,e.content),g.dispatchEvent(s,"input"),g.dispatchEvent(l,"input"),!0},I.isPostPublish_newthread()?(b.info("新发布帖子的页面"),typeof n=="function"&&n()):I.isPostPublish_edit()?(b.info("草稿的页面"),typeof n=="function"&&typeof i=="function"&&B.confirm({title:{text:"提示",position:"center"},content:{text:"<p>存在历史写入的数据，是否恢复到编辑器里？</p>",html:!0},btn:{merge:!0,position:"space-between",ok:{text:"恢复",callback:async s=>{await n()&&(x.success("恢复成功"),s.close());}},other:{enable:!0,type:"danger",text:"删除该数据",callback:async s=>{await i(),s.close(),x.success("删除成功");}}},width:"300px",height:"200px"})):I.isPostPublish_reply()&&(b.info("回复页面"),typeof n=="function"&&n()));},async getReplyRecordSize(){var e;let t=await this.$data.db.get("data");return t.success?g.getTextStorageSize((e=t==null?void 0:t.data)!=null&&e.length?JSON.stringify(t.data):""):g.formatByteToSize(0)},async clearAllReplyRecord(){return await this.$data.db.deleteAll()},deleteReplyTextStorage(){const t=this;this.$data.db.get("data").then(e=>{if(!e.success){console.warn(e);return}let n=I.isPostPublish_voting()?"post-vote":"post",i=F.getThreadId(window.location.href),s=F.getPostId(window.location.href),l=e.data.findIndex(o=>{if(o.type===n&&!(o.tid!==i||o.pid!==s))return !0});l!==-1&&(e.data.splice(l,1),t.$data.db.save("data",e.data).then(o=>{}));});},setInputChangeEvent(){const t=this;r.on([this.$el.$input,this.$el.$title].filter(Boolean),["input","propertychange"],function(e){let n=null;if(I.isPostPublish_voting()){let i=t.$el.$form.querySelector("input[name='subject']"),s=t.$el.$form.querySelector("textarea[name='message']"),l=t.$el.$form.querySelector("input[name='maxchoices']"),o=t.$el.$form.querySelector("input[name='expiration']"),a=t.$el.$form.querySelector("input[name='visibilitypoll']"),c=t.$el.$form.querySelector("input[name='overt']");n={title:i.value,maxchoices:l.value,expiration:o.value,visibilitypoll:a.checked,overt:c.checked,content:s.value};}else {let i=t.$el.$form.querySelector("input[name='subject']"),s=t.$el.$form.querySelector("textarea[name='message']");n={title:i==null?void 0:i.value,content:s.value};}I.isPostPublish_newthread()?(b.info("内容改变 ==> 新发布帖子的页面"),I.isPostPublish_voting()?W(t.$key.noPublishVotingSerializeText,n):W(t.$key.noPublishSerializeText,n)):I.isPostPublish_edit()?(b.info("内容改变 ==> 草稿的页面"),t.$data.db.get("data").then(i=>{if(!i.success){console.warn(i);return}let s=i.data.findIndex(l=>{if(l.type===t.$data.type&&!(l.tid!==t.$data.tid||l.pid!==t.$data.pid))return !0});s!==-1&&i.data.splice(s,1),i.data.push({url:window.location.href,data:n,pid:t.$data.pid,tid:t.$data.tid,type:t.$data.type}),t.$data.db.save("data",i.data).then(l=>{});})):I.isPostPublish_reply()&&(b.info("内容改变 ==> 回复页面"),q.execMenu("mt-forum-post-editorOptimizationNormal-recordInputText",()=>{tt.$data.db.get("data").then(i=>{if(!i.success||i.code===201){console.warn(i);return}let s=i.data.findIndex(l=>l.forumId===t.$data.tid&&l.repquote===F.getRepquote(window.location.href));s!==-1?n.content==null||n.content===""?i.data.splice(s,1):i.data[s]=g.assign(i.data[s],{text:n.content}):i.data.push({forumId:t.$data.tid,url:window.location.href,repquote:F.getRepquote(window.location.href),text:n.content}),tt.$data.db.save("data",i.data).then(l=>{});});}));});},initDeleteBtn(){if(!E(".comiis_btnbox .comiis_btn.bg_del"))return;let e=E("#comiis_head .header_y"),n=r.createElement("input",{className:"new_btn_del"},{type:"button",value:"删除"});r.append(e,n),r.on(n,"click",function(){B.confirm({title:{text:"提示",position:"center"},content:{text:"<p>是否删除帖子?</p>",html:!0},btn:{ok:{callback:i=>{i.close(),y.comiis_delthread();}}},width:"300px",height:"200px"});});},initSaveBtn(){let t=r.selector(".comiis_btnbox button#postsubmit:contains('保存')");if(!t||r.text(t).includes("草稿"))return;let e=E("#comiis_head .header_y"),n=r.createElement("input",{className:"new_btn_save"},{type:"button",value:"保存"});r.append(e,n),r.on(n,"click",function(){t.click();});},initPostBtn(){let t=r.selector(".comiis_btnbox button#postsubmit:contains('发表')");if(!t)return;let e=E("#comiis_head .header_y"),n=r.createElement("input",{className:"new_btn_post"},{type:"button",value:"发表"});r.append(e,n),r.on(n,"click",function(){r.val("#postsave",0),t.click();});},initReplyBtn(){const t=this;let e=r.selector(".comiis_btnbox button#postsubmit:contains('回复')");if(!e)return;let n=E("#comiis_head .header_y"),i=r.createElement("input",{className:"new_btn_reply"},{type:"button",value:"回复"});r.append(n,i),r.on(i,"click",function(){e.click(),t.deleteReplyTextStorage();});},initVotePage(){O(".comiis_scrollTop_box").length&&(y.$("#htmlon").parent().append(`
        ...
        `);let e=E("#select-post-section"),n=F.getForumId(window.location.href);I.isPostPublish_newthread()?(b.info("发帖"),r.on(e,"change",function(){let i=r.val(e),s=`forum.php?mod=post&action=newthread&fid=${i}&extra=&topicsubmit=yes&mobile=2`;b.info(`修改发帖板块: ${t[i]} ${s}`);let l={求助问答:{className:"gm_user_select_help",optionHTML:`<option value="0" selected="selected">请选择</option>
        ...
        <a href="javascript:;" class="comiis_pictitle"><i class="comiis_font"><em>图片</em></i></a>`),r.on(".comiis_pictitle","click",function(){this.querySelector("i.comiis_font").classList.contains("f_0")?r.hide(".gm_plugin_chartbed",!1):r.show(".gm_plugin_chartbed",!1);}),r.append("#comiis_post_tab",`
            ...
            `);let e=E("#imglist"),n=r.parent(e);r.before(".gm_plugin_chartbed .bqbox_t",n),r.on("#imglist .comiis_font","click",i=>{E("#filedata").click();}),r.on("#comiis_pictitle_tab #comiis_pictitle_key","click","li",function(i){let s=i.target;r.removeClass("#comiis_pictitle_tab #comiis_pictitle_key li","bg_f"),r.addClass(s,"bg_f"),y.$("#comiis_pictitle_tab div.comiis_upbox").hide().eq(y.$(s).index()).fadeIn();}),q.execMenuOnce("mt-image-bed-hello-enable",()=>{zt.init();}),q.execMenuOnce("mt-image-bed-mt-enable",()=>{Bt.init();});},initPreview(){const t=this;b.info("添加功能：双列预览"),G(`
        ...
        <a href="javascript:;" class="swiper-slide gm_plugin_previewpostforum">
            ...
            </div>`),r.on(".gm_plugin_previewpostforum","click",function(n){let i=this;if(O("#polldatas").length&&xt.parseVoteText(),i.querySelector("i.comiis_font").classList.contains("f_0"))r.hide(".gm_plugin_previewpostforum_html",!1);else {r.show(".gm_plugin_previewpostforum_html",!1);let l=xt.parseText(r.val(t.$el.$input));r.html(".gm_plugin_previewpostforum_html .comiis_message_table",l),r.css(".gm_plugin_previewpostforum_html.double-preview .comiis_over_box.comiis_input_style","height",r.css(t.$el.$input,"height"));}});},initSettingImmersionMode(){b.info("初始化设置功能-使用沉浸模式"),r.append(r.parent("#htmlon"),`
            <li class="comiis_styli_m f15 comiis_flex b_b">
            ...
            `),r.on("#immersiveinput","click",function(){let t=this,e=r.parent(t).querySelector(".comiis_checkbox"),n=[".comiis_wzpost ul li.comiis_flex",".comiis_wzpost ul li.comiis_styli.kmquote",r.parent(r.parent("#pollchecked")),"#pollm_c_1",".comiis_polloption_add+div.f_0",".comiis_wzpost ul li.comiis_thread_content:contains('内容')","div#comiis_head","div#comiis_head+*:not([class])"];r.hasClass(e,"comiis_checkbox_close")?n.forEach(i=>{i&&r.hide(i,!1);}):n.forEach(i=>{i&&r.show(i,!1);}),window.dispatchEvent(new Event("resize"));});}},ce={id:"component-forum-post",title:"帖子",forms:[{text:"",type:"forms",forms:[{text:"功能",type:"deepMenu",forms:[{text:"",type:"forms",forms:[R("自动展开内容","mt-forum-post-autoExpandContent",!0,void 0,"注入CSS展开帖子的内容"),R("修复图片宽度","mt-forum-post-repairImageWidth",!0,void 0,"修复图片宽度超出页面宽度的问题"),R("移除帖子字体效果","mt-forum-post-removeFontStyle",!1,void 0,""),R("移除评论区的字体效果","mt-forum-post-removeCommentFontStyle",!1,void 0,""),R("添加【点评】按钮","mt-forum-post-addCommentOnBtn",!1,void 0,"在评论区的每个评论右下角添加按钮"),R("附件点击提醒","mt-forum-post-setAttachmentsClickTip",!0,void 0,"阻止默认点击附件就会触发附件下载"),R("代码块优化","mt-forum-post-codeQuoteOptimization",!0,void 0,"自动检测代码块语言并设置关键字高亮"),R("图片查看优化","mt-forum-post-optimizationImagePreview",!0,void 0,"使用Viewer查看图片")]}]},{text:"自动加载评论",type:"deepMenu",forms:[{type:"forms",text:"",forms:[R("自动加载下一页评论","mt-forum-post-loadNextPageComment",!0,void 0,""),R("同步加载的地址","mt-forum-post-syncNextPageUrl",!1,void 0,"便于刷新页面会停留在当前查看的评论页面")]}]},{text:"编辑器-简略版",type:"deepMenu",forms:[{type:"forms",text:"",forms:[R("启用","mt-forum-post-editorOptimizationNormal",!0,void 0,"优化样式，插入bbcode代码等"),R("自动保存输入记录","mt-forum-post-editorOptimizationNormal-recordInputText",!0,void 0,"当回复时会自动清空记录"),ft("清空回复记录","当前占用空间大小：计算中","清理",void 0,!1,!1,"default",async t=>{let i=t.target.closest("li").querySelector(".pops-panel-item-left-desc-text"),s=await tt.clearAllReplyRecord();s.success?(x.success("清理成功"),r.text(i,`当前占用空间大小：${await tt.getReplyRecordSize()}`)):x.error("清理失败 "+s.msg);},async(t,e)=>{let n=e.target.querySelector(".pops-panel-item-left-desc-text");r.text(n,`当前占用空间大小：${await tt.getReplyRecordSize()}`);})]}]},{text:"编辑器-完整版",type:"deepMenu",forms:[{type:"forms",text:"",forms:[R("启用","mt-forum-post-publish-editorOptimization",!0,void 0,"优化样式，插入bbcode代码，双列预览等"),R("自动保存输入记录","mt-forum-post-editorOptimization-recordInputText",!0,void 0,"当回复/发表时会自动清空记录"),ft("清空回复记录","当前占用空间大小：计算中","清理",void 0,!1,!1,"default",async t=>{let i=t.target.closest("li").querySelector(".pops-panel-item-left-desc-text"),s=await gt.clearAllReplyRecord();s.success?(x.success("清理成功"),r.text(i,`当前占用空间大小：${await gt.getReplyRecordSize()}`)):x.error("清理失败 "+s.msg);},async(t,e)=>{let n=e.target.querySelector(".pops-panel-item-left-desc-text");r.text(n,`当前占用空间大小：${await gt.getReplyRecordSize()}`);})]}]},{text:"编辑器-图床配置",type:"deepMenu",forms:[{type:"forms",text:'<a href="https://www.helloimg.com/" target="_blank">Hello图床</a>',forms:[R("启用","mt-image-bed-hello-enable",!1,void 0,"启用Hello图床"),U("账号","mt-image-bed-hello-account","","",void 0,"必填"),U("密码","mt-image-bed-hello-password","","",void 0,"必填",!1,!0),U("token","mt-image-bed-hello-token","","",void 0,"必填",!1,!0)]},{type:"forms",text:'<a href="https://img.binmt.cc/" target="_blank">MT图床</a>',forms:[R("启用","mt-image-bed-mt-enable",!0,void 0,"启用MT图床")]},{type:"forms",text:"图片水印",forms:[R("启用","mt-image-bed-watermark-enable",!1,void 0,"开启后会为图床图片添加文字水印"),R("自动添加水印","mt-image-bed-watermark-autoAddWaterMark",!1,void 0,"开启后会自动添加水印，关闭后会有添加水印后的图片预览"),U("水印文字","mt-image-bed-watermark-text","MT论坛"),U("颜色","mt-image-bed-watermark-text-color","#000000",void 0,void 0,"",!1,!1,(t,e)=>{var s,l;let n=(s=e.target)==null?void 0:s.querySelector("input"),i=(l=e.target)==null?void 0:l.querySelector(".pops-panel-input__suffix");r.hide(i,!1),n.setAttribute("type","color"),r.css(n,{margin:"unset",padding:"unset",width:"80px"});}),U("大小","mt-image-bed-watermark-font-size",16,void 0,void 0,void 0,!0),U("透明度","mt-image-bed-watermark-font-opacity",1,void 0,void 0,void 0,!0),U("左右间距","mt-image-bed-watermark-left-right-margin",80,void 0,void 0,void 0,!0),U("上下间距","mt-image-bed-watermark-top-bottom-margin",80,void 0,void 0,void 0,!0),U("旋转角度","mt-image-bed-watermark-rotate",45,void 0,void 0,void 0,!0)]}]}]}]},me={id:"component-search",title:"搜索",forms:[{type:"forms",text:"",forms:[R("显示搜索历史","mt-search-showSearchHistory",!0,void 0,"自动记住搜索历史并显示"),R("修复清空按钮","mt-search-repairClearBtn",!0,void 0,"修复点击清空按钮不清空输入框的问题"),R("搜索框自动获取焦点","mt-search-searchInputAutoFocus",!0,void 0,"")]}]},mt={$key:{signTime:"mt-sign-time"},init(){this.sign();},todayIsSign(){let t=this.getSignTime();return !(t==null||g.formatTime(Date.now(),"yyyyMMdd")!==g.formatTime(t,"yyyyMMdd"))},setSignTime(){W(this.$key.signTime,Date.now());},getSignTime(){return Y(this.$key.signTime)},clearSignTime(){at(this.$key.signTime);},async checkLogin(){return F.envIsMobile()?document.querySelector(".sidenv_exit a[href*='member.php?mod=logging&action=logout']"):document.querySelector("#comiis_key")},getFormHash(){let t=(top||globalThis).document.querySelector("input[name=formhash]"),e=(top||globalThis).document.querySelector("div[class=sidenv_exit]>a"),n=null,i=(top||globalThis).document.querySelector("a.comiis_recommend_addkey"),s=null,l=t?t.value:null;return e&&(n=e.href.match(J.formhash),n=n?n[n.length-1]:null),i&&(s=i.href.match(J.hash),s=s?s[s.length-1]:null),l||n||s},async sign(){let t=this.getFormHash();if(t==null){if(document.querySelector("#comiis_picshowbox")){b.info("当前为评论区的看图模式 ");return}b.error("自动签到：获取账号formhash失败"),at("mt_sign"),x.error({content:"自动签到：获取账号formhash失败"});return}if(this.todayIsSign()){b.info("今日已签到");return}let e={operation:"qiandao",format:"button",formhash:t,inajax:1,ajaxtarget:"midaben_sign"},n=!!q.getValue("mt-auto-sign-useFetch"),i=await V.get(`/k_misign-sign.html?${g.toSearchParamsStr(e)}`,{fetch:n,headers:{"User-Agent":g.getRandomPCUA()},allowInterceptConfig:!1});if(!i.status){b.error("签到：网络异常，请求失败"),x.error("签到：网络异常，请求失败");return}this.setSignTime(),b.info("签到信息：",i);let s=i.data.responseText,l=g.parseCDATA(s),o=r.parseHTML(`<div>${l}</div>`,!0,!1),a=r.text(o);if(a.includes("需要先登录")){x.error("签到：请先登录账号",{timeout:3e3}),this.clearSignTime();return}else if(a.includes("请稍后再试")||a.includes("您已经被列入黑名单")||a.includes("绑定手机号后才可以签到")||a.includes("您所在用户组不允许使用")){x.error("签到："+a,{timeout:5e3});return}else if(a.includes("今日已签")){x.info("签到："+a);return}else if(s.includes("您当前的访问请求当中含有非法字符，已经被系统拒绝")){x.error("签到: 您当前的访问请求当中含有非法字符，已经被系统拒绝",{timeout:6e3});return}else if(n&&"location"in g.toJSON(s)){x.success("签到: 签到成功");return}let c=o.querySelector(".con"),m=o.querySelector(".line");if(c&&m){let f=r.text(c).match(/([0-9]+)金币/),p=r.text(m).match(/([0-9]+)/),h=f[f.length-1],_=p[p.length-1];b.success(`金币${h}，排名${_}`),x.info(`
...
`,be={$data:{},init(){this.registerMenu();},registerMenu(){j.registerLeftMenu({name:"在线用户",iconColor:"#2d92ff",icon:"",callback:()=>{this.showOnlineUser();}});},async showOnlineUser(){let t=x.loading("正在获取在线用户名单中..."),e=await this.getOnlineUserListInfo();if(t.close(),!e)return;let n=B.alert({title:{text:"在线用户",position:"center"},content:{text:`
        ...
        `),X.ready(()=>{q.execMenuOnce("mt-search-showSearchHistory",()=>{this.showSearchHistory();}),q.execMenuOnce("mt-search-repairClearBtn",()=>{this.repairClearBtn();}),q.execMenuOnce("mt-search-searchInputAutoFocus",()=>{this.searchInputAutoFocus();});});},async showSearchHistory(){b.info("显示搜索历史");let t=Y("search_history",[]),e=E("#scform_srchtxt"),n=E("#searchform"),i=B.searchSuggestion({target:e,inputTarget:e,data:t,getItemHTML:function(o){return o},getData(o){return t.filter(a=>a.includes(o))},deleteIcon:{enable:!0,callback(o,a,c){let m=t.findIndex(d=>d===c);m!==-1&&(t.splice(m,1),W("search_history",t)),a.remove();}},itemClickCallBack(o,a,c){e.value=c,n.submit();}});i.init(),i.setAllEvent();function s(){let o=document.querySelector("#scform_submit");X.on(o,"click",function(){let a=e.value;if(a!=""){let c=Y("search_history",[]);c.includes(a)?b.info("已有该搜索历史记录"):(b.info("无该记录，追加"),c.push(a)),W("search_history",c);}});}function l(){let a='<div class="comiis_p12 f14 bg_f f_c b_b cl" style="padding-bottom:10px">搜索记录个数: '+Y("search_history",[]).length+`<button class="btn_clear_search_history" style="
                    border: none;
                    float: right;
                    background: red;
                    color: #fff;
...
`;class qe{constructor(e){nt(this,"isBacking",!1);nt(this,"config");this.config=e,this.enterGestureBackMode=this.enterGestureBackMode.bind(this),this.quitGestureBackMode=this.quitGestureBackMode.bind(this),this.popStateEvent=this.popStateEvent.bind(this),(typeof this.config.backDelayTime!="number"||isNaN(this.config.backDelayTime))&&(this.config.backDelayTime=150),this.config.win==null&&(this.config.win=self);}popStateEvent(e){g.preventEvent(e),!this.isBacking&&this.quitGestureBackMode(!0);}enterGestureBackMode(){b.success("进入手势模式");let e=this.config.hash;e.startsWith("#")||(e.startsWith("/")||(e="/"+e),e="#"+e),this.config.useUrl&&(e=this.config.win.location.origin+this.config.win.location.pathname+this.config.win.location.search+e),this.config.win.history.pushState({},"",e),b.success("监听popstate事件"),r.on(this.config.win,"popstate",this.popStateEvent,{capture:!0});}async quitGestureBackMode(e=!1){this.isBacking=!0,b.success("退出手势模式"),typeof this.config.beforeHistoryBackCallBack=="function"&&this.config.beforeHistoryBackCallBack(e);let n=Date.now()+1e3*5;for(;;){if(Date.now()>n){b.error("未知情况，history.back()失败，无法退出手势模式");break}if(this.config.win.location.hash.endsWith(this.config.hash))b.info("history.back()"),this.config.win.history.back(),await g.sleep(this.config.backDelayTime||150);else break}b.success("移除popstate事件"),r.off(this.config.win,"popstate",this.popStateEvent,{capture:!0}),this.isBacking=!1,typeof this.config.afterHistoryBackCallBack=="function"&&this.config.afterHistoryBackCallBack(e);}}const ct={https:`
        ...
        <div class="small-window-title-container">
            ...
            `);var n="";g.sortListByProperty(e,s=>{var l=s.href.match(/thread-(.+?)-/i);return parseInt(l[l.length-1])},!0),b.info("导读内容",e),e.forEach(s=>{n+=`
                ...
                </li>`;});let i=document.querySelector(".comiis_forumlist.comiis_xznlist");X.before(i,`<div class="comiis_mh_kxtxt bg_f comiis_mh_kxtxt_owm"><ul>${n}</ul></div>`);});}};class Ht{constructor(e){nt(this,"option");this.option=e;}async showView(){var o;let e=B.confirm({title:{text:this.option.title,position:"center"},content:{text:`
            ...
            `}),n=e.$shadowRoot.querySelector(".filter-container"),i=document.createDocumentFragment();this.option.filterOption.forEach(s=>{let l=document.createElement("button");l.innerText=s.name;let o=async()=>{(await this.option.getAllRuleInfo()).forEach(async c=>{await s.filterCallBack(c.data)?r.show(c.$el,!1):r.hide(c.$el,!1);}),typeof this.option.execFilterCallBack=="function"&&await this.option.execFilterCallBack(),e.close();};r.on(l,"click",async a=>{g.preventEvent(a),!(typeof s.callback=="function"&&!await s.callback(a,o))&&await o();}),i.appendChild(l);}),n.appendChild(i);}}class qt{constructor(e){nt(this,"option");this.option=e;}async showView(){var i,s,l,o,a,c,m,d,u;let e=B.confirm({title:{text:this.option.title,position:"center"},content:{text:`
                    ...
                    `,html:!0},btn:{merge:!0,reverse:!1,position:"space-between",ok:{enable:((l=(s=(i=this.option)==null?void 0:i.bottomControls)==null?void 0:s.add)==null?void 0:l.enable)||!0,type:"primary",text:"添加",callback:async f=>{this.showEditView(e.$shadowRoot,!1,await this.option.getAddData());}},close:{enable:!0,callback(f){e.close();}},cancel:{enable:((c=(a=(o=this.option)==null?void 0:o.bottomControls)==null?void 0:a.filter)==null?void 0:c.enable)||!1,type:"default",text:"过滤",callback:(f,p)=>{var k,v,S,A,C,D,L;typeof((S=(v=(k=this.option)==null?void 0:k.bottomControls)==null?void 0:v.filter)==null?void 0:S.callback)=="function"&&this.option.bottomControls.filter.callback();let h=()=>Array.from(e.$shadowRoot.querySelectorAll(".rule-view-container .rule-item")),_=p.target.closest(".pops-confirm-btn").querySelector(".pops-confirm-btn-cancel span");r.text(_).includes("取消")?(h().forEach(M=>{r.show(M,!1);}),r.text(_,"过滤")):new ke({title:((C=(A=this.option.bottomControls)==null?void 0:A.filter)==null?void 0:C.title)??"过滤规则",filterOption:((L=(D=this.option.bottomControls)==null?void 0:D.filter)==null?void 0:L.option)||[],execFilterCallBack(){r.text(_,"取消过滤");},getAllRuleInfo:()=>h().map(H=>({data:this.parseRuleItemElement(H).data,$el:H}))}).showView();}},other:{enable:((u=(d=(m=this.option)==null?void 0:m.bottomControls)==null?void 0:d.clear)==null?void 0:u.enable)||!0,type:"xiaomi-primary",text:`清空所有(${(await this.option.data()).length})`,callback:f=>{let p=B.confirm({title:{text:"提示",position:"center"},content:{text:"确定清空所有的数据？",html:!1},btn:{ok:{enable:!0,callback:async h=>{var k,v,S;if(b.success("清空所有"),typeof((S=(v=(k=this.option)==null?void 0:k.bottomControls)==null?void 0:v.clear)==null?void 0:S.callback)=="function"&&this.option.bottomControls.clear.callback(),(await this.option.data()).length){x.error("清理失败");return}else x.success("清理成功");await this.updateDeleteAllBtnText(e.$shadowRoot),this.clearContent(e.$shadowRoot),p.close();}},cancel:{text:"取消",enable:!0}},mask:{enable:!0},width:"300px",height:"200px"});}}},mask:{enable:!0},width:window.innerWidth>500?"500px":"88vw",height:window.innerHeight>500?"500px":"80vh",style:`
            ${B.config.cssText.panelCSS}
