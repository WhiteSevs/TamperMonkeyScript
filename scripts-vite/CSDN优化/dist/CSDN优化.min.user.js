...
自动判断: 0
移动端: 1
PC端: 2`,"0");if(!t)return;let n=parseInt(t);if(isNaN(n)){v.error("输入的不是规范的数字");return}if(!e.includes(n)){v.error("输入的值必须是0或1或2");return}n==0?P(_):D(_,n);}});M!=null?(o.info(`手动判定为${M===1?"移动端":"PC端"}`),M==1?I.init():M==2?V.init():(v.error("意外，手动判定的值不在范围内"),P(_))):K?(o.info("自动判定为移动端"),I.init()):(o.info("自动判定为PC端"),V.init());
