...
自动判断: 0
移动端: 1
PC端: 2`,"0");if(!t)return;let n=parseInt(t);if(isNaN(n)){S.error("输入的不是规范的数字");return}if(!e.includes(n)){S.error("输入的值必须是0或1或2");return}n==0?se(X):de(X,n);}});q!=null?(r.info(`手动判定为${q===1?"移动端":"PC端"}`),q==1?we.init():q==2?be.init():(S.error("意外，手动判定的值不在范围内"),se(X))):Be?(r.info("自动判定为移动端"),we.init()):(r.info("自动判定为PC端"),be.init());
