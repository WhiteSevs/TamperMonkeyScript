...
#chatgpt-article-detail > div.layout-center > div.main > div.article-box > div.cont.first-show.forbid > div.open {\r
  display: none !important;\r
}`,ve={init(){g(Ce),g(Se),r.execMenuOnce("csdn-wenku-shieldResourceRecommend",()=>this.shieldResourceRecommend()),r.execMenuOnce("csdn-wenku-shieldRightUserInfo",()=>this.shieldRightUserInfo()),r.execMenuOnce("csdn-wenku-shieldRightToolBar",()=>this.shieldRightToolBar());},shieldResourceRecommend(){return o.info("【屏蔽】资源推荐"),d.addBlockCSS("#recommend")},shieldRightUserInfo(){return o.info("【屏蔽】右侧用户信息"),d.addBlockCSS(".layout-right")},shieldRightToolBar(){return o.info("【屏蔽】右侧悬浮工具栏"),d.addBlockCSS(".csdn-side-toolbar")}},F={init(){r.execMenuOnce("csdn-link-jumpRedirect",()=>{this.jumpRedirect();});},jumpRedirect(){try{let e=new URLSearchParams(window.location.search);if(e.has("target")){let t=e.get("target"),n=decodeURIComponent(t);o.success(`跳转链接：${n}`),window.location.href=n;}else o.error("解析跳转的链接失败，原因：搜索参数中没有target参数");}catch(e){S.error("跳转链接失败："+e.message);}}},$={init(){b.isLink()?(o.info("Router: 中转链接"),F.init()):b.isHuaWeiCloudBlog()?(o.info("Router: 华为云联盟"),q.init()):b.isBlog()?(o.info("Router: 博客"),W.init()):b.isWenKu()?(o.info("Router: 文库"),ve.init()):o.error("暂未适配，请反馈开发者："+globalThis.location.href);}},ye={init(){r.execMenuOnce("m-csdn-link-jumpRedirect",()=>{F.jumpRedirect();});}},Me=`/* 右下角的 免费赢华为平板xxxx */\r
...
`,ke={init(){this.addCSS(),r.execMenuOnce("m-csdn-blog-shieldTopToolbar",()=>this.shieldTopToolbar()),r.execMenuOnce("m-csdn-blog-notLimitCodePreMaxHeight",()=>this.notLimitCodePreMaxHeight()),r.execMenuOnce("m-csdn-blog-notLimitCommentMaxHeight",()=>this.notLimitCommentMaxHeight()),r.execMenuOnce("m-csdn-blog-allowSelectText",()=>this.allowSelectText()),r.execMenuOnce("m-csdn-blog-autoExpandContent",()=>this.autoExpandContent()),r.execMenuOnce("m-csdn-blog-bottomArticleEnable",()=>this.blockBottomArticle()),r.execMenuOnce("m-csdn-blog-comment-enable",()=>this.blockComment(),(e,t)=>!t,(e,t)=>!t),v.ready(()=>{r.execMenuOnce("m-csdn-blog-removeAds",()=>this.removeAds()),r.execMenuOnce("m-csdn-blog-refactoringRecommendation",()=>{this.refactoringRecommendation();}),r.execMenuOnce("m-csdn-blog-unBlockCopy",()=>{W.unBlockCopy();});});},addCSS(){g(_e),g(Be);},shieldTopToolbar(){return o.info("屏蔽顶部Toolbar"),[d.addBlockCSS("#csdn-toolbar"),g(`
			/* 内容顶部要归位 */
			body #main,
			.margin_sides{
...
自动判断: 0
移动端: 1
PC端: 2`,"0");if(!t)return;let n=parseInt(t);if(isNaN(n)){S.error("输入的不是规范的数字");return}if(!e.includes(n)){S.error("输入的值必须是0或1或2");return}n==0?P(_):D(_,n);}});M!=null?(o.info(`手动判定为${M===1?"移动端":"PC端"}`),M==1?I.init():M==2?$.init():(S.error("意外，手动判定的值不在范围内"),P(_))):K?(o.info("自动判定为移动端"),I.init()):(o.info("自动判定为PC端"),$.init());
