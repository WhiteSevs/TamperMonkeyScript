...
`,Ee={init(){g(Re),r.execMenu("m-csdn-wenku-shieldBottomToolbar",()=>{this.shieldBottomToolbar();});},shieldBottomToolbar(){o.info("【屏蔽】底部工具栏"),d.addBlockCSS(".page-container > div.btn");}},De=`/* 右下角悬浮图标 买1年送3个月 */\r
...
`,Ae={init(){r.execMenuOnce("m-csdn-download-removeAds",()=>g(De)),r.execMenuOnce("m-csdn-download-automaticallyExpandResourceIntroduction",()=>this.automaticallyExpandResourceIntroduction());},automaticallyExpandResourceIntroduction(){return o.info("自动展开资源介绍"),[d.addBlockCSS("label.unfold-font"),g(`
			.resource-desc{
				max-height: unset !important;
				overflow: unset !important;
			}
			`)]}},I={init(){b.isLink()?(o.info("Router: 中转链接"),ye.init()):b.isHuaWeiCloudBlog()?(o.info("Router: 华为云联盟"),Te.init()):b.isBlog()?(o.info("Router: 博客"),ke.init()):b.isWenKu()?(o.info("Router: 文库"),Ee.init()):b.isDownload()?(o.info("Router: 资源下载"),Ae.init()):o.error("暂未适配，请反馈开发者："+globalThis.location.href);}};r.init();let K=p.isPhone(),_="change_env_set",M=T(_);U.add({key:_,text:`⚙ 自动: ${K?"移动端":"PC端"}`,autoReload:!1,isStoreValue:!1,showText(e){return M==null?e:e+` 手动: ${M==1?"移动端":M==2?"PC端":"未知"}`},callback:()=>{let e=[0,1,2],t=window.prompt(`请输入当前脚本环境判定

自动判断: 0
移动端: 1
PC端: 2`,"0");if(!t)return;let n=parseInt(t);if(isNaN(n)){S.error("输入的不是规范的数字");return}if(!e.includes(n)){S.error("输入的值必须是0或1或2");return}n==0?P(_):E(_,n);}});M!=null?(o.info(`手动判定为${M===1?"移动端":"PC端"}`),M==1?I.init():M==2?$.init():(S.error("意外，手动判定的值不在范围内"),P(_))):K?(o.info("自动判定为移动端"),I.init()):(o.info("自动判定为PC端"),$.init());
