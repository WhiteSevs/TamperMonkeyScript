...
export const M_CSDNBlogArticle = {
  ...
  refactoringRecommendation() {
    function refactoring() {
      /* 反复执行的重构函数 */
      // log.info("重构底部推荐");
      document.querySelectorAll<HTMLDivElement>(".container-fluid").forEach((item) => {
        /* 链接 */
        let url = "";
        /* 标题 */
        let title = "";
    ...
    utils.waitNode<HTMLElement>("#operate .collect-btn", 10000).then(($collectBtn) => {
      ...
      DOMUtils.on(
        ...
        async (event) => {
          ...
          let $collectionContainer = $alert.$shadowRoot.querySelector<HTMLUListElement>(".csdn-collection-items")!;
