...
export const CSDNArticleStatusApi = {
  ...
  async like(articleId: string | number) {
    let response = await httpx.post("https://blog.csdn.net//phoenix/web/v1/article/like", {
      data: {
        articleId: articleId,
      },
      processData: true,
  ...
  async bury(articleId: string | number) {
    let response = await httpx.get(`https://blog.csdn.net//phoenix/web/v1/article/bury`, {
      data: {
        articleId: articleId,
      },
      processData: true,
