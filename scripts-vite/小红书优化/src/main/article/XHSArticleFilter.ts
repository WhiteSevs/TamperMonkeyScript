...
export const XHSArticleFilter = {
	...
	$data: {
		...
		__videoFilterRuleStorage:
			null as null | RuleStorage<XHSArticleFilterOption>,
	...
	execFilter() {
		Panel.execMenuOnce(this.$key.ENABLE_KEY, async () => {
			...
			let checkFilterCallBack = (
				filterTransformInfoResult: ReturnType<
					(typeof XHSArticleFilterBase)["prototype"]["checkInfoIsFilter"]
				>
			) => {
				// 当命中过滤规则，如果开启了仅显示被过滤的视频，则修改isFilter值
				// 并添加记录
				if (this.$data.isReverse) {
					filterTransformInfoResult.isFilter =
						!filterTransformInfoResult.isFilter;
					if (
						typeof filterTransformInfoResult.transformInfo.articleId ===
							"string" &&
						filterTransformInfoResult.matchedFilterOption
					) {
						let filterOptionList: XHSArticleFilterOption[] =
							this.$data.isFilterAwemeInfoList.get(
			...
			let queryScopeFilterOptionList = (
				scopeName: XHSArticleFilterOptionScope
			) => {
						...
						(it.data.scope.includes("all") ||
							Array.from(scopeNameList).findIndex((item) =>
								it.data.scope.includes(
									item as any as XHSArticleFilterOptionScope
								)
							) !== -1)
				);
