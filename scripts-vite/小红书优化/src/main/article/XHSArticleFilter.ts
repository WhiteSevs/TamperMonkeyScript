...
export const XHSArticleFilter = {
  ...
  execFilter() {
    Panel.execMenuOnce(this.$key.ENABLE_KEY, async () => {
      ...
      const checkFilterCallBack = (
        ...
        if (this.$data.isReverse && filterTransformInfoResult?.transformInfo) {
          ...
          if (
            ...
            const filterOptionList: XHSArticleFilterOption[] =
       ...
       * 获取作用域的规则
       */
      const queryScopeFilterOptionList = (scopeName: XHSArticleFilterOptionScope) => {
        ...
        const matchedFilterOptionList = filterOptionList.filter(
          (it) =>
            it.enable &&
       ...
       * 类型1接口结果的hook
       */
      const xhr_hook_callback_1 = (scopeName: XHSArticleFilterOptionScope, request: UtilsAjaxHookRequestOptions) => {
        request.response = (response) => {
          ...
          if (Array.isArray(items)) {
            for (let index = 0; index < items.length; index++) {
              const awemeInfo: XHSArticleInfo = items[index] || {};
  ...
  getRuleViewInstance() {
    ...
    const ruleView = new RuleView({
      ...
      getData: (data) => {
        const allData = this.$data.videoFilterRuleStorage.getAllRule();
      ...
      itemControls: {
        ...
        edit: {
          ...
          getView: (data, isEdit) => {
            ...
            // 作用域
            const scope_template = UISelectMultiple<XHSArticleFilterOptionScope>(
              "作用域",
              "scope",
              [],
              (
                ...
                const result: PopsPanelSelectMultipleConfig<XHSArticleFilterOptionScope>["data"]["0"] = {
                  ...it,
            ...
            // 属性名列表
            const keyNameHandlerInfo = <(keyof XHSArticleHandlerInfo)[]>[
              "display_title",
              "isLike",
              "liked_count",
              "nick_name",
             ...
             */
            const getDynamicProp = (storageData: any) => {
              // 属性名
              const ruleNameDefaultValue = Array.isArray(storageData["ruleName"])
                ? storageData["ruleName"]
                : [storageData["ruleName"]];
              const ruleName_template = UISelectMultiple<keyof XHSArticleHandlerInfo>(
                "属性名",
                "ruleName",
              ...
              const $remarks = panelHandlerComponents.createSectionContainerItem_textarea(remarks_template).$el;
          ...
          onsubmit: ($form, isEdit, editData) => {
            ...
            const data: XHSArticleFilterOption = this.getTemplateData();
            if (isEdit) {
            ...
            $form.querySelectorAll<HTMLLIElement>(".rule-form-ulist-dynamic__inner-container").forEach(($inner) => {
              const dynamicData = {} as XHSArticleFilterDynamicOption;
