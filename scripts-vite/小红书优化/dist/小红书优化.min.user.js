...
自动判断: 0
移动端: 1
PC端: 2`,"0");if(!t)return;let n=parseInt(t);if(isNaN(n)){x.error("输入的不是规范的数字");return}if(!e.includes(n)){x.error("输入的值必须是0或1或2");return}n==0?le(N):U(N,n);}});C!=null?(u.info(`手动判定为${C===1?"移动端":"PC端"}`),C==1?Z.init():C==2?W.init():(x.error("意外，手动判定的值不在范围内"),le(N))):ue?(u.info("自动判定为移动端"),Z.init()):(u.info("自动判定为PC端"),W.init());
