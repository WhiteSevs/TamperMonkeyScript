...
`)}{display: none !important;}`)},setGMResourceCSS(t){const e=typeof Re=="function"?Re(t.keyName):null;return typeof e=="string"&&e?H(e):O.loadStyleLink(t.url)},async loadStyleLink(t){let e=document.createElement("link");return e.rel="stylesheet",e.type="text/css",e.href=t,new Promise(n=>{P.ready(()=>{document.head.appendChild(e),n(e);});})},async loadScript(t){let e=document.createElement("script");return e.src=t,new Promise(n=>{e.onload=()=>{n(null);},(document.head||document.documentElement).appendChild(e);})},fixUrl(t){return t=t.trim(),t.startsWith("data:")||t.match(/^http(s|):\/\//i)?t:t.startsWith("//")?(t.startsWith("///")||(t=window.location.protocol+t),t):(t.startsWith("/")||(t+="/"),t=window.location.origin+t,t)},fixHttps(t){if(t.startsWith("https://")||!t.startsWith("http://"))return t;try{let e=new URL(t);return e.protocol="https:",e.toString()}catch{return t}},lockScroll(...t){let e=document.createElement("style");e.innerHTML=`
		...
		`;let n=[document.documentElement,document.body].concat(...t||[]);return n.forEach(o=>{o.classList.add("pops-overflow-hidden-important");}),(document.head||document.documentElement).appendChild(e),{recovery(){n.forEach(o=>{o.classList.remove("pops-overflow-hidden-important");}),e.remove();}}},async getClipboardText(){function t(o){navigator.clipboard.readText().then(i=>{o(i);}).catch(i=>{h.error("读取剪贴板内容失败👉",i),o("");});}function e(o){navigator.permissions.query({name:"clipboard-read"}).then(i=>{t(o);}).catch(i=>{h.error("申请剪贴板权限失败，尝试直接读取👉",i.message??i.name??i.stack),t(o);});}function n(){return !(typeof navigator?.clipboard?.readText!="function"||typeof navigator?.permissions?.query!="function")}return new Promise(o=>{if(!n()){o("");return}document.hasFocus()?e(o):window.addEventListener("focus",()=>{e(o);},{once:true});})},escapeHtml(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;").replace(/©/g,"&copy;").replace(/®/g,"&reg;").replace(/™/g,"&trade;").replace(/→/g,"&rarr;").replace(/←/g,"&larr;").replace(/↑/g,"&uarr;").replace(/↓/g,"&darr;").replace(/—/g,"&mdash;").replace(/–/g,"&ndash;").replace(/…/g,"&hellip;").replace(/ /g,"&nbsp;").replace(/\r\n/g,"<br>").replace(/\r/g,"<br>").replace(/\n/g,"<br>").replace(/\t/g,"&nbsp;&nbsp;&nbsp;&nbsp;")},interval(t,e,n=5e3){let o,i=n-e,r=e,s=async u=>{let l=await t(u);if(typeof l=="boolean"&&!l||u){y.workerClearTimeout(o);return}if(r+=e,r>i){s(true);return}o=y.workerSetTimeout(()=>{s(false);},e);};s(false);},findParentNode(t,e,n){if(n){let o=P.closest(t,n);if(o)return o.querySelector(e)}else return P.matches(t,e)?t:P.closest(t,e)},toStr(t){const e="__undefined__placeholder__replaced__str__";return JSON.stringify(t,(o,i)=>i===void 0?e:i,2).replace(new RegExp(`"${e}"`,"g"),"undefined")}},ne={$data:{__contentConfig:null,get contentConfig(){return this.__contentConfig==null&&(this.__contentConfig=new y.Dictionary),this.__contentConfig},__defaultBottomContentConfig:[]},addContentConfig(t){Array.isArray(t)||(t=[t]);let e=this.$data.contentConfig.keys().length;this.$data.contentConfig.set(e,t);},getAllContentConfig(){return this.$data.contentConfig.values().flat()},getConfig(t=0){return this.$data.contentConfig.get(t)??[]},getDefaultBottomContentConfig(){if(this.$data.__defaultBottomContentConfig.length)return this.$data.__defaultBottomContentConfig;let t=false,e;const n=(r,s)=>{typeof s!="string"&&(s=O.toStr(s));const u=new Blob([s]),l=globalThis.URL.createObjectURL(u);p.createElement("a",{href:l,download:r}).click(),y.workerSetTimeout(()=>{globalThis.URL.revokeObjectURL(l);},500);},o=()=>{const r=f=>{const d=F.alert({title:{text:"请选择导入方式",position:"center"},content:{text:`
          ...
          }`}),m=d.$shadowRoot.querySelector(".btn-control[data-mode='local']"),g=d.$shadowRoot.querySelector(".btn-control[data-mode='network']"),_=d.$shadowRoot.querySelector(".btn-control[data-mode='clipboard']"),k=async L=>{confirm("是否清空脚本存储的配置？（如果点击取消按钮，则仅做配置覆盖处理）")&&(typeof se=="function"?typeof le=="function"?(se().forEach(v=>{le(v);}),x.success("已清空脚本存储的配置")):x.error("不支持GM_deleteValue函数，无法执行删除脚本配置"):x.error("不支持GM_listValues函数，无法清空脚本存储的配置")),typeof Ae=="function"?Ae(L):Object.keys(L).forEach(v=>{const b=L[v];ae(v,b);}),x.success("配置导入完毕");},M=L=>new Promise(async $=>{const R=y.toJSON(L);Object.keys(R).length===0?x.warning("解析为空配置，不导入"):await k(R),$(true);});p.on(m,"click",L=>{p.preventEvent(L),d.close();const $=p.createElement("input",{type:"file",accept:".json"});p.on($,["propertychange","input"],R=>{if(!$.files?.length)return;const v=$.files[0],b=new FileReader;b.onload=()=>{M(b.result);},b.readAsText(v,"UTF-8");}),$.click();}),p.on(g,"click",L=>{p.preventEvent(L),d.close();const $=F.prompt({title:{text:"网络导入",position:"center"},content:{text:"",placeholder:"请填写URL",focus:true},btn:{close:{enable:true,callback(b,T){b.close();}},ok:{text:"导入",callback:async(b,T)=>{const A=b.text;if(y.isNull(A)){x.error("请填入完整的url");return}const C=x.loading("正在获取配置..."),S=await Q.get(A,{allowInterceptConfig:false});if(C.close(),!S.status){h.error(S),x.error("获取配置失败",{consoleLogContent:true});return}await M(S.data.responseText)&&b.close();}},cancel:{enable:false}},drag:true,mask:{enable:true},width:B.info.width,height:"auto"}),R=$.$shadowRoot.querySelector("input"),v=$.$shadowRoot.querySelector(".pops-prompt-btn-ok");p.on(R,["input","propertychange"],b=>{p.val(R)===""?p.attr(v,"disabled","true"):p.removeAttr(v,"disabled");}),p.listenKeyboard(R,"keydown",(b,T,A)=>{b==="Enter"&&A.length===0&&p.val(R)!==""&&p.trigger(v,"click");}),p.trigger(R,"input");}),p.on(_,"click",async L=>{p.preventEvent(L),d.close();let $=await O.getClipboardText();if($.trim()===""){x.warning("获取到的剪贴板内容为空");return}await M($);});},s=(f=`${fe}_panel-setting-${y.formatTime(Date.now(),"yyyy_MM_dd_HH_mm_ss")}.json`,d)=>{const m=F.alert({title:{text:"请选择导出方式",position:"center"},content:{text:`
            <div class="btn-control" data-mode="export-to-file">导出至文件</div>
            <div class="btn-control" data-mode="export-to-clipboard">导出至剪贴板</div>
            `,html:true},btn:{ok:{enable:false},close:{enable:true,callback(k,M){k.close();}}},drag:true,mask:{enable:true},width:B.info.width,height:B.info.height,style:`
          .btn-control{
              display: inline-block;
              margin: 10px;
              padding: 10px;
          ...
          .btn-control:hover{
            ...
            border-color: #c6e2ff;
            background-color: #ecf5ff;
          }`}),g=m.$shadowRoot.querySelector(".btn-control[data-mode='export-to-file']"),_=m.$shadowRoot.querySelector(".btn-control[data-mode='export-to-clipboard']");p.on(g,"click",k=>{p.preventEvent(k);try{n(f,d),m.close();}catch(M){x.error(M.toString(),{consoleLogContent:true});}}),p.on(_,"click",async k=>{await y.copy(d)?(x.success("复制成功"),m.close()):x.error("复制失败");});},l=F.confirm({title:{text:"配置",position:"center"},content:{text:`
            <textarea name="config-value" id="config" readonly></textarea>
          `,html:true},btn:{ok:{enable:true,type:"primary",text:"导入",callback(f,d){r();}},cancel:{enable:true,text:"导出",callback(f,d){s(void 0,c);}}},width:X.width<450?"90vw":"450px",height:"auto",style:`
        ...
        `}).$shadowRoot.querySelector("textarea"),a={};if(typeof se=="function")se().forEach(d=>{const m=te(d);Reflect.set(a,d,m);});else {x.warning("不支持函数GM_listValues，仅导出菜单配置");const f=te(pe);Reflect.set(a,pe,f);}const c=O.toStr(a);l.value=c;},i=()=>{let r=ie?.script?.supportURL||ie?.script?.namespace;typeof r=="string"&&y.isNotNull(r)&&window.open(r,"_blank");};return [{id:"script-version",title:`版本：${ie?.script?.version||"未知"}`,isBottom:true,forms:[],clickFirstCallback(){return  false},afterRender(r){new Qe(r.$asideLiElement).on("tap",function(u){clearTimeout(e),e=void 0,t?(t=false,o()):(e=setTimeout(()=>{t=false,i();},200),t=true);});}}]},setDefaultBottomContentConfig(t){this.$data.__defaultBottomContentConfig=t;}},be={$data:{__menuOption:[{key:"show_pops_panel_setting",text:"⚙ 设置",autoReload:false,isStoreValue:false,showText(t){return t},callback:()=>{w.showPanel(ne.getConfig(0));}}],get menuOption(){return this.__menuOption}},init(){this.initExtensionsMenu();},initExtensionsMenu(){w.isTopWindow()&&Ne.add(this.$data.menuOption);},addMenuOption(t){Array.isArray(t)||(t=[t]),this.$data.menuOption.push(...t);},updateMenuOption(t){Array.isArray(t)||(t=[t]),t.forEach(e=>{let n=this.$data.menuOption.findIndex(o=>o.key===e.key);n!==-1&&(this.$data.menuOption[n]=e);});},getMenuOption(t=0){return this.$data.menuOption[t]},deleteMenuOption(t=0){this.$data.menuOption.splice(t,1);}},w={$data:{__contentConfigInitDefaultValue:null,__onceExecMenuData:null,__urlChangeReloadMenuExecOnce:null,__onceExecData:null,__panelConfig:{},$panel:null,panelContent:[],get contentConfigInitDefaultValue(){return this.__contentConfigInitDefaultValue==null&&(this.__contentConfigInitDefaultValue=new y.Dictionary),this.__contentConfigInitDefaultValue},contentConfigInitDisabledKeys:[],get onceExecMenuData(){return this.__onceExecMenuData==null&&(this.__onceExecMenuData=new y.Dictionary),this.__onceExecMenuData},get urlChangeReloadMenuExecOnce(){return this.__urlChangeReloadMenuExecOnce==null&&(this.__urlChangeReloadMenuExecOnce=new y.Dictionary),this.__urlChangeReloadMenuExecOnce},get onceExecData(){return this.__onceExecData==null&&(this.__onceExecData=new y.Dictionary),this.__onceExecData},get scriptName(){return fe},get panelConfig(){return this.__panelConfig},set panelConfig(t){this.__panelConfig=t;},key:pe,attributeKeyName:q,attributeDefaultValueName:j},init(){this.initContentDefaultValue(),be.init();},isTopWindow(){return V.top===V.self},initContentDefaultValue(){const t=o=>{if(!o.attributes||o.type==="button"||o.type==="forms"||o.type==="deepMenu")return;const i=o.attributes;let r=i[Fe];if(typeof r=="function"){let a=r();if(typeof a=="boolean"&&!a)return}let s=new Map,u=i[q];if(u!=null){const a=i[j];s.set(u,a);}let l=i[Je];if(typeof l=="object"&&l&&Object.keys(l).forEach(a=>{s.set(a,l[a]);}),!s.size){h.warn(["请先配置键",o]);return}if(o.type==="switch"){let a=typeof o.disabled=="function"?o.disabled():o.disabled;typeof a=="boolean"&&a&&this.$data.contentConfigInitDisabledKeys.push(...s.keys());}for(const[a,c]of s.entries())this.setDefaultValue(a,c);},e=o=>{for(let i=0;i<o.length;i++){let r=o[i];t(r);let s=r.forms;s&&Array.isArray(s)&&e(s);}},n=[...ne.getAllContentConfig()];for(let o=0;o<n.length;o++){let i=n[o];if(!i.forms)continue;const r=i.forms;r&&Array.isArray(r)&&e(r);}this.$data.contentConfigInitDisabledKeys=[...new Set(this.$data.contentConfigInitDisabledKeys)];},setDefaultValue(t,e){this.$data.contentConfigInitDefaultValue.has(t)&&h.warn("请检查该key(已存在): "+t),this.$data.contentConfigInitDefaultValue.set(t,e);},getDefaultValue(t){return this.$data.contentConfigInitDefaultValue.get(t)},setValue(t,e){J.set(t,e);},getValue(t,e){const n=J.get(t);return n??(this.$data.contentConfigInitDefaultValue.has(t)?this.$data.contentConfigInitDefaultValue.get(t):e)},deleteValue(t){J.delete(t);},hasKey(t){return J.has(t)},addValueChangeListener(t,e){return J.addValueChangeListener(t,(o,i,r)=>{e(t,r,i);})},removeValueChangeListener(t){J.removeValueChangeListener(t);},triggerMenuValueChange(t,e,n){J.triggerValueChangeListener(t,n,e);},async exec(t,e,n,o=true){const i=this;let r;typeof t=="string"||Array.isArray(t)?r=()=>t:r=t;let s=false;const u=r();let l=[];Array.isArray(u)?(s=true,l=u):l.push(u);const a=l.find(v=>!this.$data.contentConfigInitDefaultValue.has(v));if(a){h.warn(`${a} 键不存在`);return}const c=JSON.stringify(l);if(o&&this.$data.onceExecMenuData.has(c))return this.$data.onceExecMenuData.get(c);let f=[];const d=[];let m=[];const g=(v,b)=>{let T=[],A=[],C=[];if(Array.isArray(b))C=C.concat(b);else if(typeof b=="object"&&b!=null)if(b instanceof Element)C.push(b);else {const{$css:S,destory:I}=b;S!=null&&(Array.isArray(S)?C=C.concat(S):C.push(S)),typeof I=="function"&&C.push(I);}else C.push(b);for(const S of C)if(S!=null){if(S instanceof Element){T.push(S);continue}if(typeof S=="function"){m.push(S);continue}}v?(f=f.concat(T),m=m.concat(A)):(k(),M());},_=v=>!!this.getValue(v),k=()=>{for(let v=0;v<f.length;v++)f[v]?.remove(),f.splice(v,1),v--;},M=()=>{for(let v=0;v<m.length;v++){const b=m[v];b(),m.splice(v,1),v--;}},L=()=>{let v=false;return typeof n=="function"?v=n(l):v=l.every(b=>_(b)),v},$=async v=>{if(L()){const T=l.map(C=>this.getValue(C)),A=await e({value:s?T:T[0],addStoreValue:(...C)=>g(true,C)});g(true,A);}else g(false,[]);};o&&l.forEach(v=>{const b=this.addValueChangeListener(v,(T,A,C)=>$());d.push(b);}),await $();const R={reload(){this.clearStoreStyleElements(),this.destory(),$();},clear(){this.clearStoreStyleElements(),this.destory(),this.removeValueChangeListener(),this.clearOnceExecMenuData();},clearStoreStyleElements:()=>k(),destory(){return M()},removeValueChangeListener:()=>{d.forEach(v=>{this.removeValueChangeListener(v);});},clearOnceExecMenuData(){o&&i.$data.onceExecMenuData.delete(c);}};return this.$data.onceExecMenuData.set(c,R),R},async execMenu(t,e,n=false,o=false){return await this.exec(t,async i=>await e(i),i=>i.every(s=>{let u=!!this.getValue(s);return w.$data.contentConfigInitDisabledKeys.includes(s)&&(u=false,h.warn(`.execMenu${o?"Once":""} ${s} 被禁用`)),n&&(u=!u),u}),o)},async execMenuOnce(t,e,n=false,o=false){const i=await this.execMenu(t,e,n,true);if(o&&i){const r=()=>{i.reload();};this.removeUrlChangeWithExecMenuOnceListener(t),this.addUrlChangeWithExecMenuOnceListener(t,r);}return i},deleteExecMenuOnce(t){return t=this.transformKey(t),this.$data.onceExecMenuData.delete(t),this.$data.urlChangeReloadMenuExecOnce.delete(t),J.removeValueChangeListener(t)},onceExec(t,e){if(t=this.transformKey(t),typeof t!="string")throw new TypeError("key 必须是字符串");this.$data.onceExecData.has(t)||(e(),this.$data.onceExecData.set(t,1));},deleteOnceExec(t){t=this.transformKey(t),this.$data.onceExecData.delete(t);},addUrlChangeWithExecMenuOnceListener(t,e){t=this.transformKey(t),this.$data.urlChangeReloadMenuExecOnce.set(t,e);},removeUrlChangeWithExecMenuOnceListener(t){t=this.transformKey(t),this.$data.urlChangeReloadMenuExecOnce.delete(t);},hasUrlChangeWithExecMenuOnceListener(t){return t=this.transformKey(t),this.$data.urlChangeReloadMenuExecOnce.has(t)},async triggerUrlChangeWithExecMenuOnceEvent(t){const e=this.$data.urlChangeReloadMenuExecOnce.values();for(const n of e)await n(t);},showPanel(t,e=`${fe}-设置`,n=false,o=false){this.$data.$panel=null,this.$data.panelContent=[];let i=t.findIndex(s=>(typeof s.isBottom=="function"?s.isBottom():!!s.isBottom)&&s.id==="script-version")!==-1;!n&&!i&&t.push(...ne.getDefaultBottomContentConfig());let r=F.panel({title:{text:e,position:"center",html:false,style:""},content:t,btn:{close:{enable:true,callback:(s,u)=>{s.close(),this.$data.$panel=null;}}},mask:{enable:true,clickEvent:{toClose:true,toHide:false},clickCallBack:(s,u)=>{s(),this.$data.$panel=null;}},width:B.setting.width,height:B.setting.height,drag:true,only:true,...this.$data.panelConfig});this.$data.$panel=r,this.$data.panelContent=t,o||this.registerConfigSearch({$panel:r,content:t});},registerConfigSearch(t){const{$panel:e,content:n}=t,o=async(d,m)=>{if(d==null)return;const g=await m(d);return g&&typeof g.isFind=="boolean"&&g.isFind?g.data:await o(g.data,m)},i=(d,m)=>{const g=new IntersectionObserver(_=>{_.forEach(k=>{k.isIntersecting&&(m?.(),g.disconnect());});},{root:null,threshold:1});g.observe(d),d.scrollIntoView({behavior:"smooth",block:"center"});},r=d=>{const m="pops-flashing";p.animationend(d,()=>{d.classList.remove(m);}),d.classList.add(m);},s=d=>{if(d.type==="dblclick"&&f)return;p.preventEvent(d),l=null;const m=F.alert({title:{text:"搜索配置",position:"center"},content:{text:`
				...
				`});m.$shadowRoot.querySelector(".search-wrapper");const g=m.$shadowRoot.querySelector(".search-config-text"),_=m.$shadowRoot.querySelector(".search-result-wrapper");g.focus();const k=()=>{p.empty(_);},M=$=>{const R=y.queryProperty($,b=>b?.next?{isFind:false,data:b.next}:{isFind:true,data:b}),v=p.createElement("div",{className:"search-result-item",innerHTML:`
						...
						`});return p.on(v,"click",b=>{const A=e.$shadowRoot.querySelectorAll("aside.pops-panel-aside .pops-panel-aside-top-container li")[$.index];if(!A){x.error(`左侧项下标${$.index}不存在`);return}A.scrollIntoView({behavior:"smooth",block:"center"}),A.click(),o($.next,async C=>{if(C?.next){const S=await p.waitNode(()=>Array.from(e.$shadowRoot.querySelectorAll(".pops-panel-deepMenu-nav-item")).find(I=>{const N=Reflect.get(I,"__formConfig__");return typeof N=="object"&&N!=null&&N.text===C.name}),2500);if(S)S.click();else return x.error("未找到对应的二级菜单"),{isFind:true,data:C};return {isFind:false,data:C.next}}else {const S=await p.waitNode(()=>Array.from(e.$shadowRoot.querySelectorAll("li:not(.pops-panel-deepMenu-nav-item)")).find(I=>Reflect.get(I,"__formConfig__")===C.matchedData?.formConfig),2500);if(S){i(S);const I=S.closest(".pops-panel-forms-fold[data-fold-enable]");I&&(I.querySelector(".pops-panel-forms-fold-container").click(),await y.sleep(500)),i(S,()=>{r(S);});}else x.error("未找到对应的菜单项");return {isFind:true,data:C}}});}),v},L=$=>{const R=new RegExp($,"i"),v=[],b=(A,C)=>{for(let S=0;S<A.length;S++){const I=A[S],N=I.forms;if(N&&Array.isArray(N)){const W=y.deepClone(C);if(I.type==="deepMenu"){const U=y.queryProperty(W,K=>K?.next?{isFind:false,data:K.next}:{isFind:true,data:K});U.next={name:I.text};}b(N,W);}else {const W=Reflect.get(I,"text"),U=Reflect.get(I,"description"),K=[W,U],$e=K.findIndex(Y=>{if(typeof Y=="string")return Y.match(R)});if($e!==-1){const Y=y.deepClone(C),Ve=y.queryProperty(Y,z=>z?.next?{isFind:false,data:z.next}:{isFind:true,data:z});Ve.next={name:W,matchedData:{path:"",formConfig:I,matchedText:K[$e],description:U}};const Se=[];y.queryProperty(Y,z=>{const xe=z?.name;return typeof xe=="string"&&xe.trim()!==""&&Se.push(xe),z?.next?{isFind:false,data:z.next}:{isFind:true,data:z}});const Ue=Se.join(O.escapeHtml(" - "));Ve.next.matchedData.path=Ue,v.push(Y);}}}};for(let A=0;A<n.length;A++){const C=n[A];if(!C.forms||C.isBottom&&C.id==="script-version")continue;const S=C.forms;if(S&&Array.isArray(S)){let I=C.title;typeof I=="function"&&(I=I()),b(S,{index:A,name:I});}}const T=document.createDocumentFragment();for(const A of v){let C=M(A);T.appendChild(C);}k(),_.append(T);};p.on(g,"input",y.debounce($=>{p.preventEvent($);let R=p.val(g).trim();if(R===""){k();return}L(R);},200));};e.$shadowRoot.querySelectorAll("aside.pops-panel-aside .pops-panel-aside-item:not(#script-version)").forEach(d=>{p.on(d,"dblclick",s);});let l=null,a=false,c,f=false;p.on(e.$shadowRoot,"touchend","aside.pops-panel-aside .pops-panel-aside-item:not(#script-version)",(d,m)=>{f=true,clearTimeout(c),c=void 0,a&&l===m?(a=false,l=null,s(d)):(c=setTimeout(()=>{a=false;},200),a=true,l=m);},{capture:true}),e.$shadowRoot.appendChild(p.createElement("style",{type:"text/css",textContent:`
					.pops-flashing{
...
`,ke={init(){H(ot),(w.getValue("little-red-book-hijack-webpack-mask")||w.getValue("little-red-book-hijack-webpack-scheme"))&&(h.info("劫持webpack"),_e.setTimeout(),_e.call()),w.execMenuOnce("little-red-book-shieldBottomSearchFind",()=>ce.blockBottomSearchFind()),w.execMenuOnce("little-red-book-shieldBottomToorBar",()=>ce.blockBottomToorBar()),w.execMenuOnce("little-red-book-optimizeImageBrowsing",()=>ke.optimizeImageBrowsing()),w.execMenuOnce("little-red-book-optimizeVideoNoteDesc",()=>nt.optimizeVideoNoteDesc()),w.execMenuOnce("little-red-book-shieldAuthorHotNote",()=>ce.blockAuthorHotNote()),w.execMenuOnce("little-red-book-shieldHotRecommendNote",()=>ce.blockHotRecommendNote()),p.ready(function(){w.execMenu("little-red-book-optimizeCommentBrowsing",()=>{ke.optimizeCommentBrowsing();});});},optimizeCommentBrowsing(){h.info("优化评论浏览");const t={QmsgLoading:void 0,scrollFunc:void 0,noteId:"",xsec_token:"",noteData:{},commentData:{},emojiMap:{},emojiNameList:[],currentCursor:void 0,commentContainer:void 0,init(){this.emojiMap=y.toJSON(V.localStorage.getItem("redmoji"))?.redmojiMap||{},this.emojiNameList=Object.keys(this.emojiMap),this.scrollFunc=new y.LockFunction(this.scrollEvent,this);const e=V.__INITIAL_STATE__,n=e.noteData??e.data.noteData;t.noteData=n.data.noteData,t.commentData=n.data.commentData,t.noteId=t.noteData.noteId,t.xsec_token=e.noteData.routeQuery.xsec_token,h.info(["笔记数据",t.noteData]),h.info(["评论数据",t.commentData]);},getCommentHTML(e){return `
        ...
        *{
		...
		`)},transformPublishTime(){h.info("转换笔记发布时间");let t=new y.LockFunction(()=>{et(".note-content:not([data-edit-date])").forEach(n=>{let o=ue.getVue(n);if(!o)return;let i=o?._?.props?.note;if(i==null)return;let r=i.time,s=i.lastUpdateTime,u=i.ipLocation;if(typeof r=="number"){let l=[];l.push(`发布：${y.formatTime(r)}`),typeof s=="number"&&l.push(`修改：${y.formatTime(s)}`),typeof u=="string"&&y.isNotNull(u)&&l.push(u);let a=n.querySelector(".date");p.html(a,l.join("<br>")),n.setAttribute("data-edit-date","");}});});const e=y.mutationObserver(document,{config:{subtree:true,childList:true},immediate:true,callback:()=>{t.run();}});return [()=>{e?.disconnect();}]}},oe={$data:{__storeApiFn:null,get storeApiValue(){return this.__storeApiFn||(this.__storeApiFn=new G.Dictionary),this.__storeApiFn}},getStorageApi(t){if(this.hasStorageApi(t))return this.$data.storeApiValue.get(t)},hasStorageApi(t){return this.$data.storeApiValue.has(t)},setStorageApi(t,e){this.$data.storeApiValue.set(t,e);},initComponentsStorageApi(t,e,n){let o;this.hasStorageApi(t)?o=this.getStorageApi(t):o=n,this.setComponentsStorageApiProperty(e,o);},setComponentsStorageApiProperty(t,e){Reflect.set(t.props,D,e);}},at=function(t,e,n,o,i,r="",s,u,l,a){let c={text:t,type:"input",isNumber:false,isPassword:false,attributes:{},props:{},description:o,afterAddToUListCallBack:l,getValue(){return this.props[D].get(e,n)},callback(f,d,m){this.props[D].set(e,d);},placeholder:r};return Reflect.set(c.attributes,q,e),Reflect.set(c.attributes,j,n),oe.initComponentsStorageApi("input",c,{get(f,d){return w.getValue(f,d)},set(f,d){w.setValue(f,d);}}),c},E=function(t,e,n,o,i,r,s,u){let l={text:t,type:"switch",description:i,disabled:s,attributes:{},props:{},getValue(){return this.props[D].get(e,n)},callback(a,c){let f=!!c;h.success(`${f?"开启":"关闭"} ${t}`),this.props[D].set(e,f);},afterAddToUListCallBack:r};return Reflect.set(l.attributes,q,e),Reflect.set(l.attributes,j,n),oe.initComponentsStorageApi("switch",l,{get(a,c){return w.getValue(a,c)},set(a,c){w.setValue(a,c);}}),l},Te=function(t,e,n,o,i,r,s="请至少选择一个选项",u,l){let a=[];typeof o=="function"?a=o():a=o;let c={text:t,type:"select-multiple",description:r,placeholder:s,attributes:{},props:{},getValue(){return this.props[D].get(e,n)},selectConfirmDialogDetails:u,callback(f){let d=this.props[D],m=[];f.forEach(g=>{m.push(g.value);}),h.info("多选-选择：",m),d.set(e,m);},data:a};return Reflect.set(c.attributes,q,e),Reflect.set(c.attributes,j,n),oe.initComponentsStorageApi("select-multiple",c,{get(f,d){return w.getValue(f,d)},set(f,d){w.setValue(f,d);}}),c},De=function(t,e,n,o,i,r="",s,u){let l={text:t,type:"textarea",attributes:{},props:{},description:o,placeholder:r,disabled:s,getValue(){let c=this.props[D].get(e,n);return Array.isArray(c)?c.join(`
`):c},callback(a,c){this.props[D].set(e,c);}};return Reflect.set(l.attributes,q,e),Reflect.set(l.attributes,j,n),oe.initComponentsStorageApi("switch",l,{get(a,c){return w.getValue(a,c)},set(a,c){w.setValue(a,c);}}),l};class st{option;constructor(e){this.option=e;}getAllRule(){return te(this.option.STORAGE_API_KEY,[])}setAllRule(e){ae(this.option.STORAGE_API_KEY,e);}clearAllRule(){this.setAllRule([]);}getRule(e){const n=this.getAllRule(),o=n.findIndex(i=>i.uuid===e);if(o!==-1)return n[o]}setRule(e){const n=this.getAllRule(),o=n.findIndex(r=>r.uuid===e.uuid);let i=false;return o!==-1&&(n[o]=e,this.setAllRule(n),i=true),i}addRule(e){const n=this.getAllRule(),o=n.findIndex(r=>r.uuid===e.uuid);let i=false;return o!==-1||(n.push(e),this.setAllRule(n),i=true),i}updateRule(e){const n=this.getAllRule(),o=n.findIndex(i=>i.uuid===e.uuid);o!==-1?n[o]=e:n.push(e),this.setAllRule(n);}deleteRule(e){const n=this.getAllRule(),o=typeof e=="string"?e:e.uuid,i=n.findIndex(r=>r.uuid===o);return i!==-1?(n.splice(i,1),this.setAllRule(n),true):false}importRules(e){const n=F.alert({title:{text:"请选择导入方式",position:"center"},content:{text:`
                    <div class="btn-control" data-mode="local">本地导入</div>
                    <div class="btn-control" data-mode="network">网络导入</div>
            ...
            `}),o=n.$shadowRoot.querySelector(".btn-control[data-mode='local']"),i=n.$shadowRoot.querySelector(".btn-control[data-mode='network']"),r=n.$shadowRoot.querySelector(".btn-control[data-mode='clipboard']"),s=async l=>{let a=this.getAllRule();const c=[],f=[];let d=false;for(let g=0;g<l.length;g++){const _=l[g],k=a.findIndex(M=>M.uuid===_.uuid);k!==-1?f.push({index:k,data:_}):c.push(_);}if(f.length&&await new Promise(_=>{F.alert({title:{text:"覆盖规则",position:"center"},content:{text:`存在相同的uuid的规则 ${f.length}条，是否进行覆盖？`,html:true},btn:{close:{callback(k,M){k.close(),_(false);}},ok:{text:"覆盖",callback(k,M){k.close(),_(true);}}},width:B.info.width,height:B.info.height,mask:{enable:true},drag:true});})){for(const _ of f)a[_.index]=_.data;d=true;}c.length&&(a=a.concat(c)),this.setAllRule(a);const m=`共 ${l.length} 条规则，新增 ${c.length} 条，覆盖 ${d?f.length:0} 条`;x.success(m),e?.();},u=l=>new Promise(async a=>{const c=y.toJSON(l);if(!Array.isArray(c)){h.error(c),x.error("导入失败，格式不符合（不是数组）",{consoleLogContent:true}),a(false);return}if(!c.length){x.error("导入失败，解析出的数据为空",{consoleLogContent:true}),a(false);return}await s(c),a(true);});p.on(o,"click",l=>{p.preventEvent(l),n.close();const a=p.createElement("input",{type:"file",accept:".json"});p.on(a,["propertychange","input"],c=>{if(!a.files?.length)return;const f=a.files[0],d=new FileReader;d.onload=()=>{u(d.result);},d.readAsText(f,"UTF-8");}),a.click();}),p.on(i,"click",l=>{p.preventEvent(l),n.close();const a=F.prompt({title:{text:"网络导入",position:"center"},content:{text:"",placeholder:"请填写URL",focus:true},btn:{close:{enable:true,callback(d,m){d.close();}},ok:{text:"导入",callback:async(d,m)=>{const g=d.text;if(y.isNull(g)){x.error("请填入完整的url");return}const _=x.loading("正在获取配置..."),k=await Q.get(g,{allowInterceptConfig:false});if(_.close(),!k.status){h.error(k),x.error("获取配置失败",{consoleLogContent:true});return}await u(k.data.responseText)&&d.close();}},cancel:{enable:false}},mask:{enable:true},drag:true,width:B.info.width,height:"auto"}),c=a.$shadowRoot.querySelector("input"),f=a.$shadowRoot.querySelector(".pops-prompt-btn-ok");p.on(c,["input","propertychange"],d=>{p.val(c)===""?p.attr(f,"disabled","true"):p.removeAttr(f,"disabled");}),p.listenKeyboard(c,"keydown",(d,m,g)=>{d==="Enter"&&g.length===0&&p.val(c)!==""&&p.trigger(f,"click");}),p.trigger(c,"input");}),p.on(r,"click",async l=>{p.preventEvent(l),n.close();const a=await y.getClipboardInfo();if(a.error!=null){x.error(a.error.toString());return}if(a.content.trim()===""){x.warning("获取到的剪贴板内容为空");return}await u(a.content);});}exportRules(e="rule.json"){const n=this.getAllRule(),o=new Blob([JSON.stringify(n,null,4)]),i=globalThis.URL.createObjectURL(o),r=document.createElement("a");r.href=i,r.download=e,r.click(),setTimeout(()=>{globalThis.URL.revokeObjectURL(i);},1500);}}class ct{option;constructor(e){this.option=e;}async showView(){let e=F.confirm({title:{text:this.option.title,position:"center"},content:{text:`
			...
			`});Reflect.set(i,"data-rule",e);let r="pops-panel-switch-is-checked";const{$enable:s,$enableSwitch:u,$enableSwitchCore:l,$enableSwitchInput:a,$delete:c,$edit:f}=this.parseRuleItemElement(i);return this.option.itemControls.enable.enable?(p.on(l,"click",async d=>{let m=false;u.classList.contains(r)?(u.classList.remove(r),m=false):(u.classList.add(r),m=true),a.checked=m,await this.option.itemControls.enable.callback(e,m);}),await this.option.itemControls.enable.getEnable(e)&&u.classList.add(r)):s.remove(),this.option.itemControls.edit.enable?p.on(f,"click",d=>{p.preventEvent(d),this.showEditView(true,e,n,i,m=>{e=null,e=m;});}):f.remove(),this.option.itemControls.delete.enable?p.on(c,"click",d=>{p.preventEvent(d);let m=F.confirm({title:{text:"提示",position:"center"},content:{text:"确定删除该条数据？",html:false},btn:{ok:{enable:true,callback:async g=>{h.success("删除数据"),await this.option.itemControls.delete.deleteCallBack(e)?(x.success("成功删除该数据"),i.remove(),await this.updateDeleteAllBtnText(n),m.close()):x.error("删除该数据失败");}},cancel:{text:"取消",enable:true}},mask:{enable:true},width:"300px",height:"200px"});}):c.remove(),i}async appendRuleItemElement(e,n){let{$container:o}=this.parseViewElement(e),i=[],r=Array.isArray(n)?n:[n];for(let s=0;s<r.length;s++){let u=r[s],l=await this.createRuleItemElement(u,e);o.appendChild(l),i.push(l);}return await this.updateDeleteAllBtnText(e),i}async updateRuleContaienrElement(e){this.clearContent(e);const{$container:n}=this.parseViewElement(e);let o=await this.option.data();await this.appendRuleItemElement(e,o),await this.updateDeleteAllBtnText(e);}async updateRuleItemElement(e,n,o){let i=await this.createRuleItemElement(e,o);n.after(i),n.remove();}clearContent(e){const{$container:n}=this.parseViewElement(e);p.html(n,"");}setDeleteBtnText(e,n,o=false){const{$deleteBtn:i}=this.parseViewElement(e);o?p.html(i,n):p.text(i,n);}async updateDeleteAllBtnText(e){let n=await this.option.data();this.setDeleteBtnText(e,`清空所有(${n.length})`);}}const pt={__ajaxHooker:null,get ajaxHooker(){return this.__ajaxHooker==null&&(this.__ajaxHooker=y.ajaxHooker()),this.__ajaxHooker}};class ft{parseInfoDictData(e,n=false){let o=e?.note_card,i=e.id,r=o.display_title,s=!!o?.interact_info?.liked,u=0,l=e?.note_card?.interact_info?.liked_count;typeof l=="string"&&(u=parseInt(l),isNaN(u)&&(u=0));let a=o?.user?.nick_name||o?.user?.nickname,c=o?.user?.user_id,f=o?.type==="video",d=o?.video?.capa?.duration||0;return {articleId:i,display_title:r,isLike:s,liked_count:u,nick_name:a,user_id:c,isVideo:f,videoDuration:d}}checkFilterWithRule(e){if(e.infoValue==null||e.ruleValue==null)return  false;if(typeof e.infoValue=="string"){if(e.infoValue.match(e.ruleValue))return  true}else if(typeof e.infoValue=="object"){if(Array.isArray(e.infoValue)&&e.infoValue.find(o=>typeof o=="string"&&e.ruleValue!=null?!!o.match(e.ruleValue):false))return  true}else if(typeof e.infoValue=="number"){if(typeof e.ruleValue=="string"){let n=e.ruleValue.trim(),o=n.match(/(\d+)/);if(!o)return h.warn("过滤器-解析比较大小的数字失败: ",e),false;let i=Number(o[1]);if(n.startsWith(">")){if(n.startsWith(">=")){if(e.infoValue>=i)return  true}else if(e.infoValue>i)return  true}else if(n.startsWith("<")){if(n.startsWith("<=")){if(e.infoValue<=i)return  true}else if(e.infoValue<i)return  true}else if(n.startsWith("=")){if(e.infoValue===i)return  true}else return h.warn("视频过滤器-未经允许的比较符号: ",e),false}}else if(typeof e.infoValue=="boolean"&&typeof e.ruleValue=="string"){let n=e.ruleValue.trim();return e.infoValue.toString()===n}return  false}checkInfoIsFilter(e,n){let o=this.parseInfoDictData(n),i=false,r=null;e:for(let s=0;s<e.length;s++){const u=e[s],l=Array.isArray(u.data.ruleName)?u.data.ruleName:[u.data.ruleName];for(let a=0;a<l.length;a++){const c=l[a];if(!Reflect.has(o,c))continue;let f=c,d=o[f],m={infoKey:f,infoValue:d,ruleKey:u.data.ruleName,ruleValue:u.data.ruleValue};if(i=this.checkFilterWithRule(m),i)if(Array.isArray(u.dynamicData)&&u.dynamicData.length){let g=[];for(let _=0;_<u.dynamicData.length;_++){const k=u.dynamicData[_];let M=k.ruleName,L=o[M],$={infoKey:M,infoValue:L,ruleKey:k.ruleName,ruleValue:k.ruleValue};g.push($);let R=this.checkFilterWithRule($);if(i=i&&R,!i)break}i&&h.success([`视频过滤器-多组 ==> ${u.name}`,o,m,g,n,u]);}else h.success([`视频过滤器 ==> ${u.name}`,o,m,n,u]);if(i){r=u;break e}}}return {isFilter:i,matchedFilterOption:r,transformInfo:o,info:n}}removeArticle(...e){if(e.length===1){let n=e[0];n!=null&&n instanceof Element&&n.remove();}else if(e.length===2){let n=e[0],o=e[1];if(typeof o=="number"){let i=n[o];i!=null&&i instanceof Element&&i.remove(),n.splice(o,1);}}}}const de={$key:{ENABLE_KEY:"shieldVideo-exec-network-enable"},$data:{isFilterAwemeInfoList:new G.Dictionary,articleInfoMap:new G.Dictionary,__videoFilterRuleStorage:null,get videoFilterRuleStorage(){return this.__videoFilterRuleStorage==null&&(this.__videoFilterRuleStorage=new st({STORAGE_API_KEY:"xhs-article-filter-rule"})),this.__videoFilterRuleStorage},get isReverse(){return w.getValue("xhs-article-filter-only-show-filtered-video")}},init(){this.execFilter();},execFilter(){w.execMenuOnce(this.$key.ENABLE_KEY,async()=>{h.info("执行笔记过滤器");let t=new ft,e=i=>{if(this.$data.isReverse&&(i.isFilter=!i.isFilter,typeof i.transformInfo.articleId=="string"&&i.matchedFilterOption)){let r=this.$data.isFilterAwemeInfoList.get(i.transformInfo.articleId)||[];r.push(i.matchedFilterOption),this.$data.isFilterAwemeInfoList.set(i.transformInfo.articleId,r);}typeof i.transformInfo.articleId=="string"&&this.$data.articleInfoMap.set(i.transformInfo.articleId,{articleInfo:i.info,transformArticleInfo:i.transformInfo});},n=i=>{if(!w.getValue(this.$key.ENABLE_KEY))return [];let r=this.$data.videoFilterRuleStorage.getAllRule();if(!r.length)return [];let s=Array.isArray(i)?i:[i];return r.filter(l=>l.enable&&(l.data.scope.includes("all")||Array.from(s).findIndex(a=>l.data.scope.includes(a))!==-1))},o=(i,r)=>{r.response=s=>{let u=n(i);if(!u.length)return;let l=y.toJSON(s.responseText),a=l?.data?.items;if(Array.isArray(a)){for(let c=0;c<a.length;c++){let f=a[c]||{},d=t.checkInfoIsFilter(u,f);e(d),d.isFilter&&t.removeArticle(a,c--);}s.responseText=JSON.stringify(l);}};};pt.ajaxHooker.hook(i=>{let r=O.fixUrl(i.url);new URL(r).pathname.startsWith("/api/sns/web/v1/homefeed")&&o("xhr-explore",i);});});},getTemplateData(){return {uuid:y.generateUUID(),enable:true,name:"",data:{scope:[],ruleName:"display_title",ruleValue:"",remarks:""},dynamicData:[]}},showView(){this.getRuleViewInstance().showView();},getRuleViewInstance(){const t=this;let e=F.config.PanelHandlerComponents();function n(i){return {get(r,s){return i[r]??s},set(r,s){i[r]=s;}}}return new dt({title:"笔记过滤器",data:()=>this.$data.videoFilterRuleStorage.getAllRule(),getAddData:()=>this.getTemplateData(),getDataItemName:i=>i.name,updateData:i=>this.$data.videoFilterRuleStorage.setRule(i),deleteData:i=>this.$data.videoFilterRuleStorage.deleteRule(i),getData:i=>this.$data.videoFilterRuleStorage.getAllRule().find(u=>u.uuid===i.uuid)??i,itemControls:{enable:{enable:true,getEnable(i){return i.enable},callback:(i,r)=>{i.enable=r,this.$data.videoFilterRuleStorage.setRule(i);}},edit:{enable:true,getView:(i,r)=>{let s=document.createDocumentFragment();r||(i=this.getTemplateData());let u=E("启用","enable",true);Reflect.set(u.props,D,n(i));let l=e.createSectionContainerItem_switch(u),a=at("规则名称","name","","",void 0,"必填");Reflect.set(a.props,D,n(i));let c=e.createSectionContainerItem_input(a),f=Te("作用域","scope",[],[{text:"所有",value:"all"},{text:"发现",value:"xhr-explore"}].map(b=>({...b,value:b.value})),void 0,"选择需要在xxx上生效的作用域");Reflect.set(f.props,D,n(i.data));let d=e.createSectionContainerItem_select_multiple_new(f),m=["display_title","isLike","liked_count","nick_name","user_id","isVideo","videoDuration"],g=b=>{let T=Array.isArray(b.ruleName)?b.ruleName:[b.ruleName],A=Te("属性名","ruleName",T,m.map(U=>({text:U,value:U})),void 0,"选择需要的属性名 ");Reflect.set(A.props,D,n(b));let C=e.createSectionContainerItem_select_multiple_new(A),S=De("属性值","ruleValue","","如果是字符串，可正则，注意转义");Reflect.set(S.props,D,n(b));let I=e.createSectionContainerItem_textarea(S),N=De("备注","remarks","","");Reflect.set(N.props,D,n(b));let W=e.createSectionContainerItem_textarea(N);return {$ruleName:C,$ruleValue:I,$remarks:W}},_=p.createElement("div",{className:"rule-form-ulist-dynamic",innerHTML:`
							...
							`}),k=_.querySelector(".rule-form-ulist-dynamic__inner"),M=_.querySelector(".pops-panel-button"),L=(b={ruleName:[],ruleValue:"",remarks:""})=>{let T=p.createElement("div",{className:"rule-form-ulist-dynamic__inner-container",innerHTML:`
								...
								`}),A=T.querySelector(".dynamic-control-delete");p.on(A,"click",W=>{if(p.preventEvent(W),T.remove(),Array.isArray(i.dynamicData)){let U=i.dynamicData.findIndex(K=>K==b);U!==-1&&i.dynamicData.splice(U,1);}});let C=T.querySelector(".dynamic-forms"),{$ruleName:S,$ruleValue:I,$remarks:N}=g(b);C.appendChild(S),C.appendChild(I),C.appendChild(N),k.appendChild(T);};if(p.on(M,"click",b=>{p.preventEvent(b),L();}),Array.isArray(i.dynamicData))for(let b=0;b<i.dynamicData.length;b++){const T=i.dynamicData[b];L(T);}let{$ruleName:$,$ruleValue:R,$remarks:v}=g(i.data);return s.append(l,c,d,$,R,v,_),s},onsubmit:(i,r,s)=>{let u=i.querySelectorAll(".rule-form-ulist > li"),l=this.getTemplateData();return r&&(l.uuid=s.uuid),u.forEach(a=>{let c=Reflect.get(a,"__formConfig__");if(!c)return;let f=Reflect.get(c,"attributes");if(!f)return;let d=Reflect.get(a,D),m=Reflect.get(f,q),g=Reflect.get(f,j),_=d.get(m,g);Reflect.has(l,m)?Reflect.set(l,m,_):Reflect.has(l.data,m)?Reflect.set(l.data,m,_):h.error(`${m}不在数据中`);}),i.querySelectorAll(".rule-form-ulist-dynamic__inner-container").forEach(a=>{let c={};a.querySelectorAll(".dynamic-forms > li").forEach(f=>{let d=Reflect.get(f,"__formConfig__");if(!d)return;let m=Reflect.get(d,"attributes");if(!m)return;let g=Reflect.get(f,D),_=Reflect.get(m,q),k=Reflect.get(m,j),M=g.get(_,k);Reflect.set(c,_,M);}),l.dynamicData.push(c);}),l.name.trim()===""?(x.error("规则名称不能为空"),{success:false,data:l}):l.data.scope.length===0?(x.error("请选择作用域"),{success:false,data:l}):l.data.ruleName.length===0?(x.error("请选择属性名"),{success:false,data:l}):l.data.ruleValue.trim()===""?(x.error("属性值不能为空"),{success:false,data:l}):r?{success:t.$data.videoFilterRuleStorage.setRule(l),data:l}:{success:t.$data.videoFilterRuleStorage.addRule(l),data:l}},style:`
                    ...
                    `,width:()=>window.innerWidth>700?"700px":"88vw"},delete:{enable:true,deleteCallBack:i=>t.$data.videoFilterRuleStorage.deleteRule(i)}},bottomControls:{filter:{enable:true,option:[{name:"过滤-已启用",filterCallBack(i){return i.enable}},{name:"过滤-未启用",filterCallBack(i){return !i.enable}}]},clear:{enable:true,callback:()=>{t.$data.videoFilterRuleStorage.clearAllRule();}}}})}},he={init(){de.init(),w.execMenuOnce("pc-xhs-hook-vue",()=>{_e.hookVue();}),w.execMenuOnce("pc-xhs-allowCopy",()=>he.allowPCCopy()),w.execMenuOnce("pc-xhs-open-blank-article",()=>he.openBlankArticle()),rt.init(),w.execMenuOnce("pc-xhs-article-showPubsliushTime",()=>Le.transformPublishTime()),ve.isArticle()&&(h.info("Router: 笔记页面"),Le.init());},allowPCCopy(){h.success("允许复制文字");const t=e=>{p.preventEvent(e);let n=V.getSelection();return n?y.copy(n.toString()):h.error("未选中任何内容"),false};return p.on(V,"copy",t,{capture:true}),{destory(){p.off(V,"copy",t,{capture:true});}}},openBlankArticle(){h.success("新标签页打开文章");let t=(e,n)=>{if(!w.getValue("pc-xhs-open-blank-article"))return;p.preventEvent(e);let i=n.querySelector("a.cover[href]")?.href;if(i){h.info("跳转文章: "+i);let r=new URL(i);r.pathname=r.pathname.replace(/^\/user\/profile\/[a-z0-9A-Z]+\//i,"/discovery/item/"),i=r.toString(),window.open(i,"_blank");}else x.error("未找到文章链接");};return p.on(document,"click",".feeds-container .note-item",t,{capture:true}),{destory(){p.off(document,"click",".feeds-container .note-item",t,{capture:true});}}}},me=function(t,e,n,o,i,r,s){let u=[];typeof o=="function"?u=o():u=o;let l={text:t,type:"select",description:r,attributes:{},props:{},getValue(){return this.props[D].get(e,n)},callback(a,c,f){let d=c;if(h.info(`选择：${f}`),typeof i=="function"&&i(a,d,f))return;this.props[D].set(e,d);},data:u};return Reflect.set(l.attributes,q,e),Reflect.set(l.attributes,j,n),oe.initComponentsStorageApi("select",l,{get(a,c){return w.getValue(a,c)},set(a,c){w.setValue(a,c);}}),l},we=function(t,e,n,o,i,r,s,u,l,a){let c={text:t,type:"button",attributes:{},props:{},description:e,buttonIcon:o,buttonIsRightIcon:i,buttonIconIsLoading:r,buttonType:s,buttonText:n,callback(f){typeof u=="function"&&u(f);},afterAddToUListCallBack:l};return Reflect.set(c.attributes,Fe,()=>{c.disable=false;}),c},ht={id:"xhs-panel-config-common",title:"通用",forms:[{type:"forms",text:"",forms:[{text:"Toast配置",type:"deepMenu",forms:[{text:"",type:"forms",forms:[me("Toast位置","qmsg-config-position","bottom",[{value:"topleft",text:"左上角"},{value:"top",text:"顶部"},{value:"topright",text:"右上角"},{value:"left",text:"左边"},{value:"center",text:"中间"},{value:"right",text:"右边"},{value:"bottomleft",text:"左下角"},{value:"bottom",text:"底部"},{value:"bottomright",text:"右下角"}],(t,e,n)=>{h.info("设置当前Qmsg弹出位置"+n);},"Toast显示在页面九宫格的位置"),me("最多显示的数量","qmsg-config-maxnums",3,[{value:1,text:"1"},{value:2,text:"2"},{value:3,text:"3"},{value:4,text:"4"},{value:5,text:"5"}],void 0,"限制Toast显示的数量"),E("逆序弹出","qmsg-config-showreverse",false,void 0,"修改Toast弹出的顺序")]}]}]},{text:"",type:"forms",forms:[{text:"功能",type:"deepMenu",forms:[{text:"",type:"forms",forms:[E("允许复制","pc-xhs-allowCopy",true,void 0,"可以选择文字并复制"),E("新标签页打开文章","pc-xhs-open-blank-article",false,void 0,"点击文章不会在本页展开，会打开新标签页")]}]},{text:"搜索",type:"deepMenu",forms:[{text:"",type:"forms",forms:[E("新标签页打开-搜索按钮","pc-xhs-search-open-blank-btn",false,void 0,"点击右边的搜索按钮直接新标签页打开搜索内容"),E("新标签页打开-回车键","pc-xhs-search-open-blank-keyboard-enter",false,void 0,"按下回车键直接新标签页打开搜索内容")]}]},{text:"屏蔽",type:"deepMenu",forms:[{text:"",type:"forms",forms:[E("【屏蔽】广告","pc-xhs-shieldAd",true,void 0,"屏蔽元素"),E("【屏蔽】登录弹窗","pc-xhs-shield-login-dialog",true,void 0,"屏蔽会自动弹出的登录弹窗"),E("【屏蔽】选择文字弹出的搜索提示","pc-xhs-shield-select-text-search-position",false,void 0,"屏蔽元素"),E("【屏蔽】顶部工具栏","pc-xhs-shield-topToolbar",false,void 0,"屏蔽元素")]}]},{type:"deepMenu",text:"笔记过滤器",forms:[{text:'<a href="https://greasyfork.org/zh-CN/scripts/483960-%E5%B0%8F%E7%BA%A2%E4%B9%A6%E4%BC%98%E5%8C%96#:~:text=%E5%B1%8F%E8%94%BD%E8%A7%84%E5%88%99" target="_blank">点击查看规则</a>',type:"forms",forms:[E("启用","shieldVideo-exec-network-enable",true,void 0,"开启后以下功能才会生效"),E("仅显示被过滤的笔记","xhs-article-filter-only-show-filtered-video",false,void 0,"只会显示过滤规则命中的笔记"),we("笔记过滤规则","可过滤笔记","自定义",void 0,false,false,"primary",()=>{de.showView();})]},{type:"forms",text:"",forms:[we("数据导入","导入自定义规则数据","导入",void 0,false,false,"primary",()=>{de.$data.videoFilterRuleStorage.importRules();}),we("数据导出","导出自定义规则数据","导出",void 0,false,false,"primary",()=>{de.$data.videoFilterRuleStorage.exportRules(Oe+"-视频过滤规则.json");})]}]},{text:"劫持/拦截",type:"deepMenu",forms:[{text:"",type:"forms",forms:[E("劫持Vue","pc-xhs-hook-vue",true,void 0,"恢复__vue__属性")]}]}]}]},mt=function(t,e,n,o,i,r,s,u,l,a){let c={text:t,type:"slider",description:u,attributes:{},props:{},getValue(){return this.props[D].get(e,n)},getToolTipContent(f){return typeof s=="function"?s(f):`${f}`},callback(f,d){if(typeof r=="function"&&r(f,d))return;this.props[D].set(e,d);},min:o,max:i,step:l};return Reflect.set(c.attributes,q,e),Reflect.set(c.attributes,j,n),oe.initComponentsStorageApi("slider",c,{get(f,d){return w.getValue(f,d)},set(f,d){w.setValue(f,d);}}),c},gt={id:"xhs-panel-config-article",title:"笔记",forms:[{type:"forms",text:"功能",forms:[E("显示发布、修改的绝对时间","pc-xhs-article-showPubsliushTime",false,void 0,"注：需要开启<code>通用</code>-<code>劫持/拦截</code>-<code>劫持Vue</code>")]},{text:"笔记宽屏",type:"forms",forms:[E("启用","pc-xhs-article-fullWidth",false,void 0,`让笔记占据宽屏，当页面可视宽度>=960px时才会触发该功能，当前页面可视宽度: ${window.innerWidth}px`),mt("占据范围","pc-xhs-article-fullWidth-widthSize",90,30,100,(t,e)=>{let n=ye("#noteContainer");if(!n){h.error("未找到笔记容器");return}n.style.width=`${e}vw`;},t=>`${t}%，默认：90%`,"调整笔记页面占据的页面范围")]}]},bt={id:"little-red-book-panel-config-common",title:"通用",forms:[{text:"",type:"forms",forms:[{text:"Toast配置",type:"deepMenu",forms:[{text:"",type:"forms",forms:[me("Toast位置","qmsg-config-position","bottom",[{value:"topleft",text:"左上角"},{value:"top",text:"顶部"},{value:"topright",text:"右上角"},{value:"left",text:"左边"},{value:"center",text:"中间"},{value:"right",text:"右边"},{value:"bottomleft",text:"左下角"},{value:"bottom",text:"底部"},{value:"bottomright",text:"右下角"}],(t,e,n)=>{h.info("设置当前Qmsg弹出位置"+n);},"Toast显示在页面九宫格的位置"),me("最多显示的数量","qmsg-config-maxnums",3,[{value:1,text:"1"},{value:2,text:"2"},{value:3,text:"3"},{value:4,text:"4"},{value:5,text:"5"}],void 0,"限制Toast显示的数量"),E("逆序弹出","qmsg-config-showreverse",false,void 0,"修改Toast弹出的顺序")]}]}]},{text:"",type:"forms",forms:[{text:"屏蔽",type:"deepMenu",forms:[{text:"",type:"forms",forms:[E("【屏蔽】广告","little-red-book-shieldAd",true,void 0,"如：App内打开"),E("【屏蔽】底部搜索发现","little-red-book-shieldBottomSearchFind",true,void 0,"建议开启"),E("【屏蔽】底部工具栏","little-red-book-shieldBottomToorBar",true,void 0,"建议开启")]}]}]}]},yt={id:"little-red-book-panel-config-home",title:"主页",forms:[{text:"",type:"forms",forms:[{text:"劫持/拦截",type:"deepMenu",forms:[{text:"",type:"forms",forms:[E("劫持点击事件","little-red-book-repariClick",true,void 0,"可阻止点击跳转至下载页面")]}]}]}]},xt={id:"little-red-book-panel-config-note",title:"笔记",forms:[{text:"",type:"forms",forms:[{text:"视频笔记",type:"deepMenu",forms:[{text:"",type:"forms",forms:[E("优化视频描述","little-red-book-optimizeVideoNoteDesc",true,void 0,"让视频描述可以滚动显示更多"),E("【屏蔽】作者热门笔记","little-red-book-shieldAuthorHotNote",true,void 0,"建议开启"),E("【屏蔽】热门推荐","little-red-book-shieldHotRecommendNote",true,void 0,"建议开启")]}]}]},{text:"",type:"forms",forms:[{text:"功能",type:"deepMenu",forms:[{text:"",type:"forms",forms:[E("优化评论浏览","little-red-book-optimizeCommentBrowsing",true,void 0,"目前仅可加载部分评论"),E("优化图片浏览","little-red-book-optimizeImageBrowsing",true,void 0,"更方便的浏览图片"),E("允许复制","little-red-book-allowCopy",true,void 0,"可以复制笔记的内容")]}]},{text:"劫持/拦截",type:"deepMenu",forms:[{text:"",type:"forms",forms:[E("劫持webpack-弹窗","little-red-book-hijack-webpack-mask",true,void 0,"如：打开App弹窗、登录弹窗"),E("劫持webpack-唤醒App","little-red-book-hijack-webpack-scheme",true,void 0,"禁止跳转商店小红书详情页/小红书")]}]}]}]};H(`
.qmsg svg.animate-turn {
    fill: none;
}
`);ne.addContentConfig([ht,gt]);ne.addContentConfig([bt,yt,xt]);const Pe=be.getMenuOption();Pe.text="⚙ PC-设置";be.updateMenuOption(Pe);be.addMenuOption({key:"show_mobile_setting",text:"⚙ 移动端-设置",autoReload:false,isStoreValue:false,showText(t){return t},callback:()=>{w.showPanel(ne.getConfig(1),`${Oe}-移动端设置`);}});w.init();let He=y.isPhone(),re="change_env_set",Z=te(re);Ne.add({key:re,text:`⚙ 自动: ${He?"移动端":"PC端"}`,autoReload:false,isStoreValue:false,showText(t){return Z==null?t:t+` 手动: ${Z==1?"移动端":Z==2?"PC端":"未知"}`},callback:()=>{let t=[0,1,2],e=window.prompt(`请输入当前脚本环境判定

自动判断: 0
移动端: 1
PC端: 2`,"0");if(!e)return;let n=parseInt(e);if(isNaN(n)){x.error("输入的不是规范的数字");return}if(!t.includes(n)){x.error("输入的值必须是0或1或2");return}n==0?le(re):ae(re,n);}});Z!=null?(h.info(`手动判定为${Z===1?"移动端":"PC端"}`),Z==1?Ce.init():Z==2?he.init():(x.error("意外，手动判定的值不在范围内"),le(re))):He?(h.info("自动判定为移动端"),Ce.init()):(h.info("自动判定为PC端"),he.init());
