...
`);ne.addContentConfig([vt,wt]);ne.addContentConfig([yt,bt,xt]);const qe=ye.getMenuOption();qe.text="⚙ PC-设置";ye.updateMenuOption(qe);ye.addMenuOption({key:"show_mobile_setting",text:"⚙ 移动端-设置",autoReload:false,isStoreValue:false,showText(t){return t},callback:()=>{x.showPanel(ne.getConfig(1),`${He}-移动端设置`);}});x.init();let je=y.isPhone(),re="change_env_set",Z=te(re);Be.add({key:re,text:`⚙ 自动: ${je?"移动端":"PC端"}`,autoReload:false,isStoreValue:false,showText(t){return Z==null?t:t+` 手动: ${Z==1?"移动端":Z==2?"PC端":"未知"}`},callback:()=>{let t=[0,1,2],e=window.prompt(`请输入当前脚本环境判定

自动判断: 0
移动端: 1
PC端: 2`,"0");if(!e)return;let n=parseInt(e);if(isNaN(n)){w.error("输入的不是规范的数字");return}if(!t.includes(n)){w.error("输入的值必须是0或1或2");return}n==0?ae(re):se(re,n);}});Z!=null?(h.info(`手动判定为${Z===1?"移动端":"PC端"}`),Z==1?ke.init():Z==2?ge.init():(w.error("意外，手动判定的值不在范围内"),ae(re))):je?(h.info("自动判定为移动端"),ke.init()):(h.info("自动判定为PC端"),ge.init());
