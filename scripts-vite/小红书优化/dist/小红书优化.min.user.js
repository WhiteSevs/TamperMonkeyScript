...
`)}{display: none !important;}`)},setGMResourceCSS(t){const e=typeof Re=="function"?Re(t.keyName):null;return typeof e=="string"&&e?j(e):B.loadStyleLink(t.url)},async loadStyleLink(t){let e=document.createElement("link");return e.rel="stylesheet",e.type="text/css",e.href=t,new Promise(n=>{q.ready(()=>{document.head.appendChild(e),n(e);});})},async loadScript(t){let e=document.createElement("script");return e.src=t,new Promise(n=>{e.onload=()=>{n(null);},(document.head||document.documentElement).appendChild(e);})},fixUrl(t){return t=t.trim(),t.startsWith("data:")||t.match(/^http(s|):\/\//i)?t:t.startsWith("//")?(t.startsWith("///")||(t=window.location.protocol+t),t):(t.startsWith("/")||(t+="/"),t=window.location.origin+t,t)},fixHttps(t){if(t.startsWith("https://")||!t.startsWith("http://"))return t;try{let e=new URL(t);return e.protocol="https:",e.toString()}catch{return t}},lockScroll(...t){let e=document.createElement("style");e.innerHTML=`
		...
		`;let n=[document.documentElement,document.body].concat(...t||[]);return n.forEach(o=>{o.classList.add("pops-overflow-hidden-important");}),(document.head||document.documentElement).appendChild(e),{recovery(){n.forEach(o=>{o.classList.remove("pops-overflow-hidden-important");}),e.remove();}}},async getClipboardText(){function t(o){navigator.clipboard.readText().then(i=>{o(i);}).catch(i=>{h.error("读取剪贴板内容失败👉",i),o("");});}function e(o){navigator.permissions.query({name:"clipboard-read"}).then(i=>{t(o);}).catch(i=>{h.error("申请剪贴板权限失败，尝试直接读取👉",i.message??i.name??i.stack),t(o);});}function n(){return !(typeof navigator?.clipboard?.readText!="function"||typeof navigator?.permissions?.query!="function")}return new Promise(o=>{if(!n()){o("");return}document.hasFocus()?e(o):window.addEventListener("focus",()=>{e(o);},{once:true});})},escapeHtml(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;").replace(/©/g,"&copy;").replace(/®/g,"&reg;").replace(/™/g,"&trade;").replace(/→/g,"&rarr;").replace(/←/g,"&larr;").replace(/↑/g,"&uarr;").replace(/↓/g,"&darr;").replace(/—/g,"&mdash;").replace(/–/g,"&ndash;").replace(/…/g,"&hellip;").replace(/ /g,"&nbsp;").replace(/\r\n/g,"<br>").replace(/\r/g,"<br>").replace(/\n/g,"<br>").replace(/\t/g,"&nbsp;&nbsp;&nbsp;&nbsp;")},interval(t,e,n=5e3){let o,i=n-e,r=e,s=async u=>{let l=await t(u);if(typeof l=="boolean"&&!l||u){b.workerClearTimeout(o);return}if(r+=e,r>i){s(true);return}o=b.workerSetTimeout(()=>{s(false);},e);};s(false);},findParentNode(t,e,n){if(n){let o=q.closest(t,n);if(o)return o.querySelector(e)}else return q.matches(t,e)?t:q.closest(t,e)},toStr(t){const e="__undefined__placeholder__replaced__str__";return JSON.stringify(t,(o,i)=>i===void 0?e:i,2).replace(new RegExp(`"${e}"`,"g"),"undefined")}},ne={$data:{__contentConfig:null,get contentConfig(){return this.__contentConfig==null&&(this.__contentConfig=new b.Dictionary),this.__contentConfig},__defaultBottomContentConfig:[]},addContentConfig(t){Array.isArray(t)||(t=[t]);let e=this.$data.contentConfig.keys().length;this.$data.contentConfig.set(e,t);},getAllContentConfig(){return this.$data.contentConfig.values().flat()},getConfig(t=0){return this.$data.contentConfig.get(t)??[]},getDefaultBottomContentConfig(){if(this.$data.__defaultBottomContentConfig.length)return this.$data.__defaultBottomContentConfig;let t=false,e;const n=(r,s)=>{typeof s!="string"&&(s=B.toStr(s));const u=new Blob([s]),l=globalThis.URL.createObjectURL(u);p.createElement("a",{href:l,download:r}).click(),b.workerSetTimeout(()=>{globalThis.URL.revokeObjectURL(l);},500);},o=()=>{const r=f=>{const d=D.alert({title:{text:"请选择导入方式",position:"center"},content:{text:`
          ...
          }`}),m=d.$shadowRoot.querySelector(".btn-control[data-mode='local']"),g=d.$shadowRoot.querySelector(".btn-control[data-mode='network']"),_=d.$shadowRoot.querySelector(".btn-control[data-mode='clipboard']"),k=async T=>{confirm("是否清空脚本存储的配置？（如果点击取消按钮，则仅做配置覆盖处理）")&&(typeof se=="function"?typeof le=="function"?(se().forEach(v=>{le(v);}),x.success("已清空脚本存储的配置")):x.error("不支持GM_deleteValue函数，无法执行删除脚本配置"):x.error("不支持GM_listValues函数，无法清空脚本存储的配置")),typeof Ae=="function"?Ae(T):Object.keys(T).forEach(v=>{const y=T[v];ae(v,y);}),x.success("配置导入完毕");},I=T=>new Promise(async C=>{const S=b.toJSON(T);Object.keys(S).length===0?x.warning("解析为空配置，不导入"):await k(S),C(true);});p.on(m,"click",T=>{p.preventEvent(T),d.close();const C=p.createElement("input",{type:"file",accept:".json"});p.on(C,["propertychange","input"],S=>{if(!C.files?.length)return;const v=C.files[0],y=new FileReader;y.onload=()=>{I(y.result);},y.readAsText(v,"UTF-8");}),C.click();}),p.on(g,"click",T=>{p.preventEvent(T),d.close();const C=D.prompt({title:{text:"网络导入",position:"center"},content:{text:"",placeholder:"请填写URL",focus:true},btn:{close:{enable:true,callback(y,E){y.close();}},ok:{text:"导入",callback:async(y,E)=>{const L=y.text;if(b.isNull(L)){x.error("请填入完整的url");return}const $=x.loading("正在获取配置..."),R=await Y.get(L,{allowInterceptConfig:false});if($.close(),!R.status){h.error(R),x.error("获取配置失败",{consoleLogContent:true});return}await I(R.data.responseText)&&y.close();}},cancel:{enable:false}},drag:true,mask:{enable:true},width:P.info.width,height:"auto"}),S=C.$shadowRoot.querySelector("input"),v=C.$shadowRoot.querySelector(".pops-prompt-btn-ok");p.on(S,["input","propertychange"],y=>{p.val(S)===""?p.attr(v,"disabled","true"):p.removeAttr(v,"disabled");}),p.listenKeyboard(S,"keydown",(y,E,L)=>{y==="Enter"&&L.length===0&&p.val(S)!==""&&p.trigger(v,"click");}),p.trigger(S,"input");}),p.on(_,"click",async T=>{p.preventEvent(T),d.close();let C=await B.getClipboardText();if(C.trim()===""){x.warning("获取到的剪贴板内容为空");return}await I(C);});},s=(f=`${fe}_panel-setting-${b.formatTime(Date.now(),"yyyy_MM_dd_HH_mm_ss")}.json`,d)=>{const m=D.alert({title:{text:"请选择导出方式",position:"center"},content:{text:`
          ...
          .btn-control:hover{
            ...
            border-color: #c6e2ff;
            background-color: #ecf5ff;
          }`}),g=m.$shadowRoot.querySelector(".btn-control[data-mode='export-to-file']"),_=m.$shadowRoot.querySelector(".btn-control[data-mode='export-to-clipboard']");p.on(g,"click",k=>{p.preventEvent(k);try{n(f,d),m.close();}catch(I){x.error(I.toString(),{consoleLogContent:true});}}),p.on(_,"click",async k=>{await b.copy(d)?(x.success("复制成功"),m.close()):x.error("复制失败");});},l=D.confirm({title:{text:"配置",position:"center"},content:{text:`
        ...
        `}).$shadowRoot.querySelector("textarea"),a={};if(typeof se=="function")se().forEach(d=>{const m=te(d);Reflect.set(a,d,m);});else {x.warning("不支持函数GM_listValues，仅导出菜单配置");const f=te(pe);Reflect.set(a,pe,f);}const c=B.toStr(a);l.value=c;},i=()=>{let r=ie?.script?.supportURL||ie?.script?.namespace;typeof r=="string"&&b.isNotNull(r)&&window.open(r,"_blank");};return [{id:"script-version",title:`版本：${ie?.script?.version||"未知"}`,isBottom:true,views:[],clickFirstCallback(){return  false},afterRender(r){new Ye(r.$asideLiElement).on("tap",function(u){clearTimeout(e),e=void 0,t?(t=false,o()):(e=setTimeout(()=>{t=false,i();},200),t=true);});}}]},setDefaultBottomContentConfig(t){this.$data.__defaultBottomContentConfig=t;}},be={$data:{__menuOption:[{key:"show_pops_panel_setting",text:"⚙ 设置",autoReload:false,isStoreValue:false,showText(t){return t},callback:()=>{w.showPanel(ne.getConfig(0));}}],get menuOption(){return this.__menuOption}},init(){this.initExtensionsMenu();},initExtensionsMenu(){w.isTopWindow()&&Ne.add(this.$data.menuOption);},addMenuOption(t){Array.isArray(t)||(t=[t]),this.$data.menuOption.push(...t);},updateMenuOption(t){Array.isArray(t)||(t=[t]),t.forEach(e=>{let n=this.$data.menuOption.findIndex(o=>o.key===e.key);n!==-1&&(this.$data.menuOption[n]=e);});},getMenuOption(t=0){return this.$data.menuOption[t]},deleteMenuOption(t=0){this.$data.menuOption.splice(t,1);}},w={$data:{__contentConfigInitDefaultValue:null,__onceExecMenuData:null,__urlChangeReloadMenuExecOnce:null,__onceExecData:null,__panelConfig:{},$panel:null,panelContent:[],get contentConfigInitDefaultValue(){return this.__contentConfigInitDefaultValue==null&&(this.__contentConfigInitDefaultValue=new b.Dictionary),this.__contentConfigInitDefaultValue},contentConfigInitDisabledKeys:[],get onceExecMenuData(){return this.__onceExecMenuData==null&&(this.__onceExecMenuData=new b.Dictionary),this.__onceExecMenuData},get urlChangeReloadMenuExecOnce(){return this.__urlChangeReloadMenuExecOnce==null&&(this.__urlChangeReloadMenuExecOnce=new b.Dictionary),this.__urlChangeReloadMenuExecOnce},get onceExecData(){return this.__onceExecData==null&&(this.__onceExecData=new b.Dictionary),this.__onceExecData},get scriptName(){return fe},get panelConfig(){return this.__panelConfig},set panelConfig(t){this.__panelConfig=t;},key:pe,attributeKeyName:W,attributeDefaultValueName:G},init(){this.initContentDefaultValue(),be.init();},isTopWindow(){return V.top===V.self},initContentDefaultValue(){const t=o=>{if(!o.attributes||o.type==="button"||o.type==="container"||o.type==="deepMenu")return;const i=o.attributes;let r=i[Fe];if(typeof r=="function"){let a=r();if(typeof a=="boolean"&&!a)return}let s=new Map,u=i[W];if(u!=null){const a=i[G];s.set(u,a);}let l=i[Je];if(typeof l=="object"&&l&&Object.keys(l).forEach(a=>{const c=l[a];s.set(a,c);}),!s.size){h.warn(["请先配置键",o]);return}if(o.type==="switch"){let a=typeof o.disabled=="function"?o.disabled():o.disabled;typeof a=="boolean"&&a&&this.$data.contentConfigInitDisabledKeys.push(...s.keys());}for(const[a,c]of s.entries())this.setDefaultValue(a,c);},e=o=>{for(let i=0;i<o.length;i++){let r=o[i];t(r);let s=r.views;s&&Array.isArray(s)&&e(s);}},n=[...ne.getAllContentConfig()];for(let o=0;o<n.length;o++){let i=n[o];if(!i.views)continue;const r=i.views;r&&Array.isArray(r)&&e(r);}this.$data.contentConfigInitDisabledKeys=[...new Set(this.$data.contentConfigInitDisabledKeys)];},setDefaultValue(t,e){this.$data.contentConfigInitDefaultValue.has(t)&&h.warn("请检查该key(已存在): "+t),this.$data.contentConfigInitDefaultValue.set(t,e);},getDefaultValue(t){return this.$data.contentConfigInitDefaultValue.get(t)},setValue(t,e){X.set(t,e);},getValue(t,e){const n=X.get(t);return n??(this.$data.contentConfigInitDefaultValue.has(t)?this.$data.contentConfigInitDefaultValue.get(t):e)},deleteValue(t){X.delete(t);},hasKey(t){return X.has(t)},addValueChangeListener(t,e){return X.addValueChangeListener(t,(o,i,r)=>{e(t,r,i);})},removeValueChangeListener(t){X.removeValueChangeListener(t);},triggerMenuValueChange(t,e,n){X.triggerValueChangeListener(t,n,e);},async exec(t,e,n,o=true){const i=this;let r;typeof t=="string"||Array.isArray(t)?r=()=>t:r=t;let s=false;const u=r();let l=[];Array.isArray(u)?(s=true,l=u):l.push(u);const a=l.find(v=>!this.$data.contentConfigInitDefaultValue.has(v));if(a){h.warn(`${a} 键不存在`);return}const c=JSON.stringify(l);if(o&&this.$data.onceExecMenuData.has(c))return this.$data.onceExecMenuData.get(c);let f=[];const d=[];let m=[];const g=(v,y)=>{let E=[],L=[],$=[];if(Array.isArray(y))$=$.concat(y);else if(typeof y=="object"&&y!=null)if(y instanceof Element)$.push(y);else {const{$css:R,destory:A}=y;R!=null&&(Array.isArray(R)?$=$.concat(R):$.push(R)),typeof A=="function"&&$.push(A);}else $.push(y);for(const R of $)if(R!=null){if(R instanceof Element){E.push(R);continue}if(typeof R=="function"){m.push(R);continue}}v?(f=f.concat(E),m=m.concat(L)):(k(),I());},_=v=>!!this.getValue(v),k=()=>{for(let v=0;v<f.length;v++)f[v]?.remove(),f.splice(v,1),v--;},I=()=>{for(let v=0;v<m.length;v++){const y=m[v];y(),m.splice(v,1),v--;}},T=()=>{let v=false;return typeof n=="function"?v=n(l):v=l.every(y=>_(y)),v},C=async v=>{if(T()){const E=l.map($=>this.getValue($)),L=await e({value:s?E:E[0],addStoreValue:(...$)=>g(true,$)});g(true,L);}else g(false,[]);};o&&l.forEach(v=>{const y=this.addValueChangeListener(v,(E,L,$)=>C());d.push(y);}),await C();const S={reload(){this.clearStoreStyleElements(),this.destory(),C();},clear(){this.clearStoreStyleElements(),this.destory(),this.removeValueChangeListener(),this.clearOnceExecMenuData();},clearStoreStyleElements:()=>k(),destory(){return I()},removeValueChangeListener:()=>{d.forEach(v=>{this.removeValueChangeListener(v);});},clearOnceExecMenuData(){o&&i.$data.onceExecMenuData.delete(c);}};return this.$data.onceExecMenuData.set(c,S),S},async execMenu(t,e,n=false,o=false){return await this.exec(t,async i=>await e(i),i=>i.every(s=>{let u=!!this.getValue(s);return w.$data.contentConfigInitDisabledKeys.includes(s)&&(u=false,h.warn(`.execMenu${o?"Once":""} ${s} 被禁用`)),n&&(u=!u),u}),o)},async execMenuOnce(t,e,n=false,o=false){const i=await this.execMenu(t,e,n,true);if(o&&i){const r=()=>{i.reload();};this.removeUrlChangeWithExecMenuOnceListener(t),this.addUrlChangeWithExecMenuOnceListener(t,r);}return i},deleteExecMenuOnce(t){return t=this.transformKey(t),this.$data.onceExecMenuData.delete(t),this.$data.urlChangeReloadMenuExecOnce.delete(t),X.removeValueChangeListener(t)},onceExec(t,e){if(t=this.transformKey(t),typeof t!="string")throw new TypeError("key 必须是字符串");this.$data.onceExecData.has(t)||(e(),this.$data.onceExecData.set(t,1));},deleteOnceExec(t){t=this.transformKey(t),this.$data.onceExecData.delete(t);},addUrlChangeWithExecMenuOnceListener(t,e){t=this.transformKey(t),this.$data.urlChangeReloadMenuExecOnce.set(t,e);},removeUrlChangeWithExecMenuOnceListener(t){t=this.transformKey(t),this.$data.urlChangeReloadMenuExecOnce.delete(t);},hasUrlChangeWithExecMenuOnceListener(t){return t=this.transformKey(t),this.$data.urlChangeReloadMenuExecOnce.has(t)},async triggerUrlChangeWithExecMenuOnceEvent(t){const e=this.$data.urlChangeReloadMenuExecOnce.values();for(const n of e)await n(t);},showPanel(t,e=`${fe}-设置`,n=false,o=false){this.$data.$panel=null,this.$data.panelContent=[];let i=t.findIndex(s=>(typeof s.isBottom=="function"?s.isBottom():!!s.isBottom)&&s.id==="script-version")!==-1;!n&&!i&&t.push(...ne.getDefaultBottomContentConfig());let r=D.panel({title:{text:e,position:"center",html:false,style:""},content:t,btn:{close:{enable:true,callback:(s,u)=>{s.close(),this.$data.$panel=null;}}},mask:{enable:true,clickEvent:{toClose:true,toHide:false},clickCallBack:(s,u)=>{s(),this.$data.$panel=null;}},width:P.setting.width,height:P.setting.height,drag:true,only:true,...this.$data.panelConfig});this.$data.$panel=r,this.$data.panelContent=t,o||this.registerConfigSearch({$panel:r,content:t});},registerConfigSearch(t){const{$panel:e,content:n}=t,o=async(d,m)=>{if(d==null)return;const g=await m(d);return g&&typeof g.isFind=="boolean"&&g.isFind?g.data:await o(g.data,m)},i=(d,m)=>{const g=new IntersectionObserver(_=>{_.forEach(k=>{k.isIntersecting&&(m?.(),g.disconnect());});},{root:null,threshold:1});g.observe(d),d.scrollIntoView({behavior:"smooth",block:"center"});},r=d=>{const m="pops-flashing";p.animationend(d,()=>{d.classList.remove(m);}),d.classList.add(m);},s=d=>{if(d.type==="dblclick"&&f)return;p.preventEvent(d),l=null;const m=D.alert({title:{text:"搜索配置",position:"center"},content:{text:`
				...
				`});m.$shadowRoot.querySelector(".search-wrapper");const g=m.$shadowRoot.querySelector(".search-config-text"),_=m.$shadowRoot.querySelector(".search-result-wrapper");g.focus();const k=()=>{p.empty(_);},I=C=>{const S=b.queryProperty(C,E=>E?.next?{isFind:false,data:E.next}:{isFind:true,data:E}),v=p.createElement("div",{className:"search-result-item",innerHTML:`
						...
						`}),y=D.config.PanelHandlerComponents();return p.on(v,"click",E=>{const $=e.$shadowRoot.querySelectorAll("aside.pops-panel-aside .pops-panel-aside-top-container li")[C.index];if(!$){x.error(`左侧项下标${C.index}不存在`);return}$.scrollIntoView({behavior:"smooth",block:"center"}),$.click(),o(C.next,async R=>{if(R?.next){const A=await p.waitNode(()=>Array.from(e.$shadowRoot.querySelectorAll(".pops-panel-deepMenu-nav-item")).find(O=>{const N=Reflect.get(O,y.$data.nodeStoreConfigKey);return typeof N=="object"&&N!=null&&N.text===R.name}),2500);if(A)A.click();else return x.error("未找到对应的二级菜单"),{isFind:true,data:R};return {isFind:false,data:R.next}}else {const A=await p.waitNode(()=>Array.from(e.$shadowRoot.querySelectorAll("li:not(.pops-panel-deepMenu-nav-item)")).find(O=>Reflect.get(O,y.$data.nodeStoreConfigKey)===R.matchedData?.formConfig),2500);if(A){i(A);const O=A.closest(".pops-panel-forms-fold[data-fold-enable]");O&&(O.querySelector(".pops-panel-forms-fold-container").click(),await b.sleep(500)),i(A,()=>{r(A);});}else x.error("未找到对应的菜单项");return {isFind:true,data:R}}});}),v},T=C=>{const S=new RegExp(C,"i"),v=[],y=(L,$)=>{for(let R=0;R<L.length;R++){const A=L[R],O=A.views;if(O&&Array.isArray(O)){const N=b.deepClone($);if(A.type==="deepMenu"){const H=b.queryProperty(N,z=>z?.next?{isFind:false,data:z.next}:{isFind:true,data:z});H.next={name:A.text};}y(O,N);}else {let N,H;if(A.type==="own"){const U=Reflect.get(A.attributes||{},Xe);U&&(typeof U.text=="string"&&(N=U.text),typeof U.desc=="string"&&(H=U.desc));}else N=Reflect.get(A,"text"),H=Reflect.get(A,"description");const z=[N,H],$e=z.findIndex(U=>{if(typeof U=="string")return U.match(S)});if($e!==-1){const U=b.deepClone($),Ve=b.queryProperty(U,J=>J?.next?{isFind:false,data:J.next}:{isFind:true,data:J});Ve.next={name:N,matchedData:{path:"",formConfig:A,matchedText:z[$e],description:H}};const Se=[];b.queryProperty(U,J=>{const xe=J?.name;return typeof xe=="string"&&xe.trim()!==""&&Se.push(xe),J?.next?{isFind:false,data:J.next}:{isFind:true,data:J}});const Ue=Se.join(B.escapeHtml(" - "));Ve.next.matchedData.path=Ue,v.push(U);}}}};for(let L=0;L<n.length;L++){const $=n[L];if(!$.views||$.isBottom&&$.id==="script-version")continue;const R=$.views;if(R&&Array.isArray(R)){let A=$.title;typeof A=="function"&&(A=A()),y(R,{index:L,name:A});}}const E=document.createDocumentFragment();for(const L of v){let $=I(L);E.appendChild($);}k(),_.append(E);};p.on(g,"input",b.debounce(C=>{p.preventEvent(C);let S=p.val(g).trim();if(S===""){k();return}T(S);},200));};e.$shadowRoot.querySelectorAll("aside.pops-panel-aside .pops-panel-aside-item:not(#script-version)").forEach(d=>{p.on(d,"dblclick",s);});let l=null,a=false,c,f=false;p.on(e.$shadowRoot,"touchend","aside.pops-panel-aside .pops-panel-aside-item:not(#script-version)",(d,m)=>{f=true,clearTimeout(c),c=void 0,a&&l===m?(a=false,l=null,s(d)):(c=setTimeout(()=>{a=false;},200),a=true,l=m);},{capture:true}),e.$shadowRoot.appendChild(p.createElement("style",{type:"text/css",textContent:`
					.pops-flashing{
...
`):c},callback(a,c){this.props[F].set(e,c);}};return Reflect.set(l.attributes,W,e),Reflect.set(l.attributes,G,n),oe.initComponentsStorageApi("switch",l,{get(a,c){return w.getValue(a,c)},set(a,c){w.setValue(a,c);}}),l};class ct{option;constructor(e){this.option=e;}getAllRule(){return te(this.option.STORAGE_API_KEY,[])}setAllRule(e){ae(this.option.STORAGE_API_KEY,e);}clearAllRule(){this.setAllRule([]);}getRule(e){const n=this.getAllRule(),o=n.findIndex(i=>i.uuid===e);if(o!==-1)return n[o]}setRule(e){const n=this.getAllRule(),o=n.findIndex(r=>r.uuid===e.uuid);let i=false;return o!==-1&&(n[o]=e,this.setAllRule(n),i=true),i}addRule(e){const n=this.getAllRule(),o=n.findIndex(r=>r.uuid===e.uuid);let i=false;return o!==-1||(n.push(e),this.setAllRule(n),i=true),i}updateRule(e){const n=this.getAllRule(),o=n.findIndex(i=>i.uuid===e.uuid);o!==-1?n[o]=e:n.push(e),this.setAllRule(n);}deleteRule(e){const n=this.getAllRule(),o=typeof e=="string"?e:e.uuid,i=n.findIndex(r=>r.uuid===o);return i!==-1?(n.splice(i,1),this.setAllRule(n),true):false}importRules(e){const n=D.alert({title:{text:"请选择导入方式",position:"center"},content:{text:`
            ...
            `}),o=n.$shadowRoot.querySelector(".btn-control[data-mode='local']"),i=n.$shadowRoot.querySelector(".btn-control[data-mode='network']"),r=n.$shadowRoot.querySelector(".btn-control[data-mode='clipboard']"),s=async l=>{let a=this.getAllRule();const c=[],f=[];let d=false;for(let g=0;g<l.length;g++){const _=l[g],k=a.findIndex(I=>I.uuid===_.uuid);k!==-1?f.push({index:k,data:_}):c.push(_);}if(f.length&&await new Promise(_=>{D.alert({title:{text:"覆盖规则",position:"center"},content:{text:`存在相同的uuid的规则 ${f.length}条，是否进行覆盖？`,html:true},btn:{close:{callback(k,I){k.close(),_(false);}},ok:{text:"覆盖",callback(k,I){k.close(),_(true);}}},width:P.info.width,height:P.info.height,mask:{enable:true},drag:true});})){for(const _ of f)a[_.index]=_.data;d=true;}c.length&&(a=a.concat(c)),this.setAllRule(a);const m=`共 ${l.length} 条规则，新增 ${c.length} 条，覆盖 ${d?f.length:0} 条`;x.success(m),e?.();},u=l=>new Promise(async a=>{const c=b.toJSON(l);if(!Array.isArray(c)){h.error(c),x.error("导入失败，格式不符合（不是数组）",{consoleLogContent:true}),a(false);return}if(!c.length){x.error("导入失败，解析出的数据为空",{consoleLogContent:true}),a(false);return}await s(c),a(true);});p.on(o,"click",l=>{p.preventEvent(l),n.close();const a=p.createElement("input",{type:"file",accept:".json"});p.on(a,["propertychange","input"],c=>{if(!a.files?.length)return;const f=a.files[0],d=new FileReader;d.onload=()=>{u(d.result);},d.readAsText(f,"UTF-8");}),a.click();}),p.on(i,"click",l=>{p.preventEvent(l),n.close();const a=D.prompt({title:{text:"网络导入",position:"center"},content:{text:"",placeholder:"请填写URL",focus:true},btn:{close:{enable:true,callback(d,m){d.close();}},ok:{text:"导入",callback:async(d,m)=>{const g=d.text;if(b.isNull(g)){x.error("请填入完整的url");return}const _=x.loading("正在获取配置..."),k=await Y.get(g,{allowInterceptConfig:false});if(_.close(),!k.status){h.error(k),x.error("获取配置失败",{consoleLogContent:true});return}await u(k.data.responseText)&&d.close();}},cancel:{enable:false}},mask:{enable:true},drag:true,width:P.info.width,height:"auto"}),c=a.$shadowRoot.querySelector("input"),f=a.$shadowRoot.querySelector(".pops-prompt-btn-ok");p.on(c,["input","propertychange"],d=>{p.val(c)===""?p.attr(f,"disabled","true"):p.removeAttr(f,"disabled");}),p.listenKeyboard(c,"keydown",(d,m,g)=>{d==="Enter"&&g.length===0&&p.val(c)!==""&&p.trigger(f,"click");}),p.trigger(c,"input");}),p.on(r,"click",async l=>{p.preventEvent(l),n.close();const a=await b.getClipboardInfo();if(a.error!=null){x.error(a.error.toString());return}if(a.content.trim()===""){x.warning("获取到的剪贴板内容为空");return}await u(a.content);});}exportRules(e="rule.json"){const n=this.getAllRule(),o=new Blob([JSON.stringify(n,null,4)]),i=globalThis.URL.createObjectURL(o),r=document.createElement("a");r.href=i,r.download=e,r.click(),setTimeout(()=>{globalThis.URL.revokeObjectURL(i);},1500);}}class ut{option;constructor(e){this.option=e;}async showView(){let e=D.confirm({title:{text:this.option.title,position:"center"},content:{text:`
			...
			`});Reflect.set(i,"data-rule",e);let r="pops-panel-switch-is-checked";const{$enable:s,$enableSwitch:u,$enableSwitchCore:l,$enableSwitchInput:a,$delete:c,$edit:f}=this.parseRuleItemElement(i);return this.option.itemControls.enable.enable?(p.on(l,"click",async d=>{let m=false;u.classList.contains(r)?(u.classList.remove(r),m=false):(u.classList.add(r),m=true),a.checked=m,await this.option.itemControls.enable.callback(e,m);}),await this.option.itemControls.enable.getEnable(e)&&u.classList.add(r)):s.remove(),this.option.itemControls.edit.enable?p.on(f,"click",d=>{p.preventEvent(d),this.showEditView(true,e,n,i,m=>{e=null,e=m;});}):f.remove(),this.option.itemControls.delete.enable?p.on(c,"click",d=>{p.preventEvent(d);let m=D.confirm({title:{text:"提示",position:"center"},content:{text:"确定删除该条数据？",html:false},btn:{ok:{enable:true,callback:async g=>{h.success("删除数据"),await this.option.itemControls.delete.deleteCallBack(e)?(x.success("成功删除该数据"),i.remove(),await this.updateDeleteAllBtnText(n),m.close()):x.error("删除该数据失败");}},cancel:{text:"取消",enable:true}},mask:{enable:true},width:"300px",height:"200px"});}):c.remove(),i}async appendRuleItemElement(e,n){let{$container:o}=this.parseViewElement(e),i=[],r=Array.isArray(n)?n:[n];for(let s=0;s<r.length;s++){let u=r[s],l=await this.createRuleItemElement(u,e);o.appendChild(l),i.push(l);}return await this.updateDeleteAllBtnText(e),i}async updateRuleContaienrElement(e){this.clearContent(e);const{$container:n}=this.parseViewElement(e);let o=await this.option.data();await this.appendRuleItemElement(e,o),await this.updateDeleteAllBtnText(e);}async updateRuleItemElement(e,n,o){let i=await this.createRuleItemElement(e,o);n.after(i),n.remove();}clearContent(e){const{$container:n}=this.parseViewElement(e);p.html(n,"");}setDeleteBtnText(e,n,o=false){const{$deleteBtn:i}=this.parseViewElement(e);o?p.html(i,n):p.text(i,n);}async updateDeleteAllBtnText(e){let n=await this.option.data();this.setDeleteBtnText(e,`清空所有(${n.length})`);}}const ft={__ajaxHooker:null,get ajaxHooker(){return this.__ajaxHooker==null&&(this.__ajaxHooker=b.ajaxHooker()),this.__ajaxHooker}};class ht{parseInfoDictData(e,n=false){let o=e?.note_card,i=e.id,r=o.display_title,s=!!o?.interact_info?.liked,u=0,l=e?.note_card?.interact_info?.liked_count;typeof l=="string"&&(u=parseInt(l),isNaN(u)&&(u=0));let a=o?.user?.nick_name||o?.user?.nickname,c=o?.user?.user_id,f=o?.type==="video",d=o?.video?.capa?.duration||0;return {articleId:i,display_title:r,isLike:s,liked_count:u,nick_name:a,user_id:c,isVideo:f,videoDuration:d}}checkFilterWithRule(e){if(e.infoValue==null||e.ruleValue==null)return  false;if(typeof e.infoValue=="string"){if(e.infoValue.match(e.ruleValue))return  true}else if(typeof e.infoValue=="object"){if(Array.isArray(e.infoValue)&&e.infoValue.find(o=>typeof o=="string"&&e.ruleValue!=null?!!o.match(e.ruleValue):false))return  true}else if(typeof e.infoValue=="number"){if(typeof e.ruleValue=="string"){let n=e.ruleValue.trim(),o=n.match(/(\d+)/);if(!o)return h.warn("过滤器-解析比较大小的数字失败: ",e),false;let i=Number(o[1]);if(n.startsWith(">")){if(n.startsWith(">=")){if(e.infoValue>=i)return  true}else if(e.infoValue>i)return  true}else if(n.startsWith("<")){if(n.startsWith("<=")){if(e.infoValue<=i)return  true}else if(e.infoValue<i)return  true}else if(n.startsWith("=")){if(e.infoValue===i)return  true}else return h.warn("视频过滤器-未经允许的比较符号: ",e),false}}else if(typeof e.infoValue=="boolean"&&typeof e.ruleValue=="string"){let n=e.ruleValue.trim();return e.infoValue.toString()===n}return  false}checkInfoIsFilter(e,n){let o=this.parseInfoDictData(n),i=false,r=null;e:for(let s=0;s<e.length;s++){const u=e[s],l=Array.isArray(u.data.ruleName)?u.data.ruleName:[u.data.ruleName];for(let a=0;a<l.length;a++){const c=l[a];if(!Reflect.has(o,c))continue;let f=c,d=o[f],m={infoKey:f,infoValue:d,ruleKey:u.data.ruleName,ruleValue:u.data.ruleValue};if(i=this.checkFilterWithRule(m),i)if(Array.isArray(u.dynamicData)&&u.dynamicData.length){let g=[];for(let _=0;_<u.dynamicData.length;_++){const k=u.dynamicData[_];let I=k.ruleName,T=o[I],C={infoKey:I,infoValue:T,ruleKey:k.ruleName,ruleValue:k.ruleValue};g.push(C);let S=this.checkFilterWithRule(C);if(i=i&&S,!i)break}i&&h.success([`视频过滤器-多组 ==> ${u.name}`,o,m,g,n,u]);}else h.success([`视频过滤器 ==> ${u.name}`,o,m,n,u]);if(i){r=u;break e}}}return {isFilter:i,matchedFilterOption:r,transformInfo:o,info:n}}removeArticle(...e){if(e.length===1){let n=e[0];n!=null&&n instanceof Element&&n.remove();}else if(e.length===2){let n=e[0],o=e[1];if(typeof o=="number"){let i=n[o];i!=null&&i instanceof Element&&i.remove(),n.splice(o,1);}}}}const de={$key:{ENABLE_KEY:"shieldVideo-exec-network-enable"},$data:{isFilterAwemeInfoList:new K.Dictionary,articleInfoMap:new K.Dictionary,__videoFilterRuleStorage:null,get videoFilterRuleStorage(){return this.__videoFilterRuleStorage==null&&(this.__videoFilterRuleStorage=new ct({STORAGE_API_KEY:"xhs-article-filter-rule"})),this.__videoFilterRuleStorage},get isReverse(){return w.getValue("xhs-article-filter-only-show-filtered-video")}},init(){this.execFilter();},execFilter(){w.execMenuOnce(this.$key.ENABLE_KEY,async()=>{h.info("执行笔记过滤器");let t=new ht,e=i=>{if(this.$data.isReverse&&(i.isFilter=!i.isFilter,typeof i.transformInfo.articleId=="string"&&i.matchedFilterOption)){let r=this.$data.isFilterAwemeInfoList.get(i.transformInfo.articleId)||[];r.push(i.matchedFilterOption),this.$data.isFilterAwemeInfoList.set(i.transformInfo.articleId,r);}typeof i.transformInfo.articleId=="string"&&this.$data.articleInfoMap.set(i.transformInfo.articleId,{articleInfo:i.info,transformArticleInfo:i.transformInfo});},n=i=>{if(!w.getValue(this.$key.ENABLE_KEY))return [];let r=this.$data.videoFilterRuleStorage.getAllRule();if(!r.length)return [];let s=Array.isArray(i)?i:[i];return r.filter(l=>l.enable&&(l.data.scope.includes("all")||Array.from(s).findIndex(a=>l.data.scope.includes(a))!==-1))},o=(i,r)=>{r.response=s=>{let u=n(i);if(!u.length)return;let l=b.toJSON(s.responseText),a=l?.data?.items;if(Array.isArray(a)){for(let c=0;c<a.length;c++){let f=a[c]||{},d=t.checkInfoIsFilter(u,f);e(d),d.isFilter&&t.removeArticle(a,c--);}s.responseText=JSON.stringify(l);}};};ft.ajaxHooker.hook(i=>{let r=B.fixUrl(i.url);new URL(r).pathname.startsWith("/api/sns/web/v1/homefeed")&&o("xhr-explore",i);});});},getTemplateData(){return {uuid:b.generateUUID(),enable:true,name:"",data:{scope:[],ruleName:"display_title",ruleValue:"",remarks:""},dynamicData:[]}},showView(){this.getRuleViewInstance().showView();},getRuleViewInstance(){const t=this;let e=D.config.PanelHandlerComponents();function n(i){return {get(r,s){return i[r]??s},set(r,s){i[r]=s;}}}return new pt({title:"笔记过滤器",data:()=>this.$data.videoFilterRuleStorage.getAllRule(),getAddData:()=>this.getTemplateData(),getDataItemName:i=>i.name,updateData:i=>this.$data.videoFilterRuleStorage.setRule(i),deleteData:i=>this.$data.videoFilterRuleStorage.deleteRule(i),getData:i=>this.$data.videoFilterRuleStorage.getAllRule().find(u=>u.uuid===i.uuid)??i,itemControls:{enable:{enable:true,getEnable(i){return i.enable},callback:(i,r)=>{i.enable=r,this.$data.videoFilterRuleStorage.setRule(i);}},edit:{enable:true,getView:(i,r)=>{let s=document.createDocumentFragment();r||(i=this.getTemplateData());let u=M("启用","enable",true);Reflect.set(u.props,F,n(i));let l=e.createSectionContainerItem_switch(u),a=st("规则名称","name","","",void 0,"必填");Reflect.set(a.props,F,n(i));let c=e.createSectionContainerItem_input(a),f=Te("作用域","scope",[],[{text:"所有",value:"all"},{text:"发现",value:"xhr-explore"}].map(y=>({...y,value:y.value})),void 0,"选择需要在xxx上生效的作用域");Reflect.set(f.props,F,n(i.data));let d=e.createSectionContainerItem_select_multiple(f),m=["display_title","isLike","liked_count","nick_name","user_id","isVideo","videoDuration"],g=y=>{let E=Array.isArray(y.ruleName)?y.ruleName:[y.ruleName],L=Te("属性名","ruleName",E,m.map(H=>({text:H,value:H})),void 0,"选择需要的属性名 ");Reflect.set(L.props,F,n(y));let $=e.createSectionContainerItem_select_multiple(L),R=De("属性值","ruleValue","","如果是字符串，可正则，注意转义");Reflect.set(R.props,F,n(y));let A=e.createSectionContainerItem_textarea(R),O=De("备注","remarks","","");Reflect.set(O.props,F,n(y));let N=e.createSectionContainerItem_textarea(O);return {$ruleName:$,$ruleValue:A,$remarks:N}},_=p.createElement("div",{className:"rule-form-ulist-dynamic",innerHTML:`
							...
							`}),k=_.querySelector(".rule-form-ulist-dynamic__inner"),I=_.querySelector(".pops-panel-button"),T=(y={ruleName:[],ruleValue:"",remarks:""})=>{let E=p.createElement("div",{className:"rule-form-ulist-dynamic__inner-container",innerHTML:`
								...
								`}),L=E.querySelector(".dynamic-control-delete");p.on(L,"click",N=>{if(p.preventEvent(N),E.remove(),Array.isArray(i.dynamicData)){let H=i.dynamicData.findIndex(z=>z==y);H!==-1&&i.dynamicData.splice(H,1);}});let $=E.querySelector(".dynamic-forms"),{$ruleName:R,$ruleValue:A,$remarks:O}=g(y);$.appendChild(R),$.appendChild(A),$.appendChild(O),k.appendChild(E);};if(p.on(I,"click",y=>{p.preventEvent(y),T();}),Array.isArray(i.dynamicData))for(let y=0;y<i.dynamicData.length;y++){const E=i.dynamicData[y];T(E);}let{$ruleName:C,$ruleValue:S,$remarks:v}=g(i.data);return s.append(l,c,d,C,S,v,_),s},onsubmit:(i,r,s)=>{let u=i.querySelectorAll(".rule-form-ulist > li"),l=this.getTemplateData();return r&&(l.uuid=s.uuid),u.forEach(a=>{let c=Reflect.get(a,e.$data.nodeStoreConfigKey);if(!c)return;let f=Reflect.get(c,"attributes");if(!f)return;let d=Reflect.get(a,F),m=Reflect.get(f,W),g=Reflect.get(f,G),_=d.get(m,g);Reflect.has(l,m)?Reflect.set(l,m,_):Reflect.has(l.data,m)?Reflect.set(l.data,m,_):h.error(`${m}不在数据中`);}),i.querySelectorAll(".rule-form-ulist-dynamic__inner-container").forEach(a=>{let c={};a.querySelectorAll(".dynamic-forms > li").forEach(f=>{let d=Reflect.get(f,e.$data.nodeStoreConfigKey);if(!d)return;let m=Reflect.get(d,"attributes");if(!m)return;let g=Reflect.get(f,F),_=Reflect.get(m,W),k=Reflect.get(m,G),I=g.get(_,k);Reflect.set(c,_,I);}),l.dynamicData.push(c);}),l.name.trim()===""?(x.error("规则名称不能为空"),{success:false,data:l}):l.data.scope.length===0?(x.error("请选择作用域"),{success:false,data:l}):l.data.ruleName.length===0?(x.error("请选择属性名"),{success:false,data:l}):l.data.ruleValue.trim()===""?(x.error("属性值不能为空"),{success:false,data:l}):r?{success:t.$data.videoFilterRuleStorage.setRule(l),data:l}:{success:t.$data.videoFilterRuleStorage.addRule(l),data:l}},style:`
