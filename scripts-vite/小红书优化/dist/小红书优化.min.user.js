    ...
    .author-box .author-desc-wrapper .author-desc .author-desc-trigger{
      display: none !important;
        ...
        *{
		...
		`)}},R={init(){s.execMenu("pc-xhs-hook-vue",()=>{X.webPackVue();}),s.execMenu("pc-xhs-allowCopy",()=>{R.allowPCCopy();}),s.execMenu("pc-xhs-open-blank-article",()=>{R.openBlankArticle();}),Ce.init(),Ee.init();},allowPCCopy(){l.success("允许复制文字"),g.on(_,"copy",void 0,function(e){m.preventEvent(e);let t=_.getSelection();return t?m.setClip(t.toString()):l.error("未选中任何内容"),!1},{capture:!0});},openBlankArticle(){l.success("新标签页打开文章"),g.on(document,"click",".feeds-container .note-item",function(e){m.preventEvent(e);let o=e.target.querySelector("a[href]");o&&o.href?(l.info("跳转文章: "+o.href),window.open(o.href,"_blank")):b.error("未找到文章链接");},{capture:!0});}};w(`
.qmsg svg.animate-turn {
    fill: none;
}
`);s.init();let le=m.isPhone(),P="change_env_set",S=$(P);ie.add({key:P,text:`⚙ 自动: ${le?"移动端":"PC端"}`,autoReload:!1,isStoreValue:!1,showText(e){return S==null?e:e+` 手动: ${S==1?"移动端":S==2?"PC端":"未知"}`},callback:()=>{let e=[0,1,2],t=window.prompt(`请输入当前脚本环境判定

自动判断: 0
移动端: 1
PC端: 2`,"0");if(!t)return;let o=parseInt(t);if(isNaN(o)){b.error("输入的不是规范的数字");return}if(!e.includes(o)){b.error("输入的值必须是0或1或2");return}o==0?te(P):j(P,o);}});S!=null?(l.info(`手动判定为${S===1?"移动端":"PC端"}`),S==1?q.init():S==2?R.init():(b.error("意外，手动判定的值不在范围内"),te(P))):le?(l.info("自动判定为移动端"),q.init()):(l.info("自动判定为PC端"),R.init());
