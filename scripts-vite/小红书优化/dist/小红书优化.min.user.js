...
自动判断: 0
移动端: 1
PC端: 2`,`0`);if(!n)return;let r=parseInt(n);if(isNaN(r)){e.default.error(`输入的不是规范的数字`);return}if(!t.includes(r)){e.default.error(`输入的值必须是0或1或2`);return}r==0?m(Q):y(Q,r)}});var Xe=()=>{Z?(E.info(`自动判定为移动端`),O.delete(`pc_setting`),Oe.init()):(E.info(`自动判定为PC端`),O.delete(`m_setting`),Ue.init())};Xe(),window.addEventListener(`urlchange`,()=>{E.info(`Router change!!!`),Xe()})})(Qmsg,DOMUtils,pops,Utils,Viewer);
