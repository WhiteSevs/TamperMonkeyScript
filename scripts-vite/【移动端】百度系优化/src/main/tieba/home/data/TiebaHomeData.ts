...
const TiebaHomeData = {
   ...
   * 获取PC网页中的用户数据
   */
  async getUserDataWithPCDoc(url: string = window.location.href): Promise<UserPCHomeInfo | undefined> {
    ...
    // 解析吧龄、IP地址、发帖数量
    $doc.querySelectorAll<HTMLSpanElement>(".userinfo_userdata span").forEach((spanItem: HTMLSpanElement) => {
    ...
    let follow =
      parseInt(
        $doc.querySelector(".ihome_aside_section:has(#concern_wrap_concern) .ihome_aside_title .concern_num a")
          ?.textContent || ""
      ) || 0;
    // 关注的贴吧数
    let forum = $doc.querySelectorAll<HTMLAnchorElement>("#forum_group_wrap .u-f-item").length;
    // 粉丝数量
    let fans =
      parseInt(
        $doc.querySelector(".ihome_aside_section:has(#concern_wrap_fans) .ihome_aside_title .concern_num a")
          ?.textContent || ""
      ) || 0;
    // 是否关注
    ...
    // 用户留下的印记图片数组（历史记录、足迹等）
    let imprint: string[] = [];
    $doc.querySelectorAll<HTMLAnchorElement>(".userinfo_honor a").forEach(($honorIcon) => {
