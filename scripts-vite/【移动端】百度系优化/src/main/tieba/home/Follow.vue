  ...
  <el-container class="disable-html-body-scroll">
    ...
    <el-main class="user-main">
      <el-scrollbar class="user-container">
        <div
          class="user-item"
          v-for="followInfo in followInfoList"
          @click="jumpToUserHome(TiebaUrlHandler.getUserHome(followInfo.portrait))">
          <div class="user-item-row">
            <div class="user-item-row-left">
              <div class="user-avatar">
                <el-avatar :src="TiebaUrlHandler.getUserAvatar(followInfo.portrait)" :size="35" />
              </div>
              <div class="user-item-row-center">
                <div class="user-info">
                  <el-text class="user-name" truncated>{{ followInfo.name_show ?? followInfo.name }}</el-text>
                  <el-text class="user-sign-text" :size="'small'" truncated>{{
                    [followInfo.follow_from, followInfo.intro].join(" ")
                  }}</el-text>
                </div>
              </div>
            </div>
...
<script lang="ts" setup>
  ...
  type UserFollowInfo = NonNullable<Awaited<ReturnType<typeof TiebaNewPCApi.followList_pc>>>["follow_list"]["0"];
  ...
  const followInfoList = ref<UserFollowInfo[]>([]);
  ...
  const loadMore = new utils.LockFunction(async () => {
    ...
    let getFollowInfoList = await TiebaNewPCApi.followList_pc(
    ...
    if (getFollowInfoList) {
      if (getFollowInfoList.follow_list) {
        followInfoList.value = followInfoList.value.concat(getFollowInfoList.follow_list);
        pageNumber.value++;
      }
      if (!getFollowInfoList.has_more) {
        isCanceled = true;
