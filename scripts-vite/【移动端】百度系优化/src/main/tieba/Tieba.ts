...
export const BaiduTieBa = {
    ...
    if (BaiduRouter.isTieBaIndex()) {
      ...
      Panel.execMenu("baidu_tieba_index_add_msgtab", () => {
        if (window.location.pathname.startsWith(TiebaMsgTab.pathname)) {
          ...
          DOMUtils.waitNode<HTMLElement>(".tb-index-navbar .navbar-box li:nth-child(2)", 10000).then(($navbarBox) => {
            if (!$navbarBox) {
              return;
            }
   ...
   */
  initTiebaData() {
    DOMUtils.waitAnyNode<HTMLDivElement>([".tb-mobile-viewport", ".main-page-wrap", ".forum-name .name"]).then(
      async () => {
