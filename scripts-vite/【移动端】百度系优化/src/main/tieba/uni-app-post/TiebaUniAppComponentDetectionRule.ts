...
export type TiebaUserLabelRule = {
	/** 唯一uuid */
	uuid: string;
	/** 是否启用 */
	enable: boolean;
...
export const TiebaUniAppComponentDetectionRule = {
	$data: {
		/** 白名单用户id */
		whiteList: [] as string[],
		/** 规则数据 */
		ruleData: <TiebaUserLabelRule[]>[],
	},
	$key: {
			...
			itemControls: {
				...
				edit: {
					...
					onsubmit: ($form, isEdit, editData) => {
						...
						let data: TiebaUserLabelRule = this.getTemplateData();
						if (isEdit) {
	 ...
	 * 获取模板数据
	 */
	getTemplateData(): TiebaUserLabelRule {
		return {
	 ...
	 */
	getData() {
		return GM_getValue<TiebaUserLabelRule[]>(this.$key.STORAGE_KEY, []);
	},
	/**
	 * 设置数据
	 * @param data
	 */
	setData(data: TiebaUserLabelRule[]) {
	 ...
	 * @param data
	 */
	addData(data: TiebaUserLabelRule) {
	 ...
	 * @param data
	 */
	updateData(data: TiebaUserLabelRule) {
		let localData = this.getData();
	 ...
	 * @param data
	 */
	deleteData(data: TiebaUserLabelRule) {
		let localData = this.getData();
		let index = localData.findIndex((item) => item.uuid == data.uuid);
