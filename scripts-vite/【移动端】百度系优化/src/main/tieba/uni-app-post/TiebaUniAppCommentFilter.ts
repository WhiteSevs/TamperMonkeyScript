...
export const TiebaUniAppCommentFilter = {
  ...
  execFilter() {
    ...
    let lockFn = new utils.LockFunction(() => {
      let $commentGroupList = Array.from($$<HTMLElement>("uni-app .comment-group"));
      $commentGroupList.forEach(($commentGroup) => {
        ...
        for (let sectionDataIndex = 0; sectionDataIndex < sectionData.length; sectionDataIndex++) {
          ...
          let findRule = filterRule.find((rule) => {
            ...
            if (rule.author_nameShow.trim() !== "") {
              flag =
                ...
                Boolean(commentData.nameShow.match(new RegExp(rule.author_nameShow)));
            }
          ...
          if (findRule) {
            // 隐藏
            Array.from<HTMLElement>($commentGroup.querySelectorAll(".comment-item")).find(($commentItem, elIndex) => {
