...
import { TiebaUniAppComponentDetectionRule, type TiebaUserLabelRule } from "./TiebaUniAppComponentDetectionRule";
          ...
          for (let spaceIndex = 0; spaceIndex < data.postInfo.length; spaceIndex++) {
            ...
            if (spaceData.replyContent == null) {
              ...
              if (typeof spaceData.title === "string" && spaceData.title.match(keyword)) {
                ...
                ruleData.data.followingForums.findIndex((it) => it.toString() === spaceData.forumName.toString()) !== -1
              ) {
                reason = "发布的帖子所在的吧";
