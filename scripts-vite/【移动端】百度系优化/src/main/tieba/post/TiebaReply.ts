...
export const TiebaReply = {
  ...
  setCurrentReplyMainUser() {
    ...
    let replyInfo = commentBoxVueObj.replyInfo as Partial<TiebaVueCurrentReplyObj>;
    let currentReplyObj = commentBoxVueObj.currentReplyObj as Partial<TiebaVueCurrentReplyObj>;
    log.info([
      "设置本帖发帖人回复参数",
      {
   ...
   * 设置当前为回复楼中楼用户
   */
  setCurrentReplyLzlUser(pid: number, authorName: string, lzlId: number, floor: number, portrait: string) {
    let commentBoxVueObj = this.getCommentBoxWrapVue();
    if (!commentBoxVueObj) {
      return;
    }
    ...
    VueUtils.waitVuePropToSet(".comment-box-wrap", [
      {
        ...
        set(vueObj) {
          ...
          let old_handlerCommentSuccess = vueObj._events["comment-success"][0].fns;
          vueObj._events["comment-success"][0].fns = function (this: any, ...args: any[]) {
    ...
    utils.waitNode<HTMLDivElement>(".comment-box-wrap .avatar", 10000).then(($avatar) => {
      ...
      DOMUtils.on($avatar, "click", (event) => {
        ...
        if (vueObj.isLogin) {
          ...
          $commentBoxWrap.classList.remove("comment-box-wrap-lzl");
          let currentReplyObj = vueObj.currentReplyObj as Partial<TiebaVueCurrentReplyObj>;
