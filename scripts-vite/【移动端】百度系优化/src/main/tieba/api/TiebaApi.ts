		...
		Array.from(contentDoc.querySelectorAll(".list_item")).forEach(
			(liElement) => {
				let postInfo: HomePostsInfo = {
					url: (
						liElement.querySelector("a.list_item_link") as HTMLAnchorElement
					).href,
					title: (
						liElement.querySelector(".post_list_item_title") as HTMLSpanElement
					).innerHTML,
					content: (
						liElement.querySelector(".post_abstract_text") as HTMLDivElement
					).innerHTML,
					forumName: (
...
const TiebaPageDataApi = {
	...
	getForumId() {
		...
		if (dataBannerInfo["forum_id"]) {
			...
			return VueUtils.getVue(document.querySelector(".app-view"))?.forum?.id;
		}
	},
};
if (import.meta.hot) {
