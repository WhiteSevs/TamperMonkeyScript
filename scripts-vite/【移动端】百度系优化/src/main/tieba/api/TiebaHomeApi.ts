...
const TiebaHomeApi = {
		...
		$doc
			...
			.forEach(($li) => {
				...
				let intro = DOMUtils.text(
					$li.querySelector<HTMLDivElement>(".post_abstract_text")!
				);
				resultDataList.push({
					url,
					forumName,
	...
	async getPost(un: string, pn = 1) {
		...
		$doc.querySelectorAll(".list_item").forEach(($listItem) => {
			let url =
				$listItem.querySelector<HTMLAnchorElement>(".list_item_link")!.href;
			let replyContent: string | null = DOMUtils.text(
				$listItem.querySelector<HTMLSpanElement>(".reply_body_text")!
			);
			let title = DOMUtils.text(
				$listItem.querySelector<HTMLSpanElement>(".post_list_item_title")!
			);
			let content = DOMUtils.text(
				$listItem.querySelector<HTMLDivElement>(".post_abstract_text")!
			);
			let forumName = DOMUtils.text(
				$listItem.querySelector<HTMLSpanElement>(".post_list_item_info_forum")!
			);
			forumName = forumName.replace(/吧$/, "");
			let time = DOMUtils.text(
				$listItem.querySelector<HTMLSpanElement>(".post_list_item_info_time")!
			);
			resultDataList.push({
				url,
				replyContent,
