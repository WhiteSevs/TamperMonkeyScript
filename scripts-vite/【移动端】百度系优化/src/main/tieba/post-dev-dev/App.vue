<template>
	...
	<FullToolbar v-show="toolbarStateStore.isShowFullToolbar" @showSmallToolbar="Main.showSmallToolbar"
		@updateContentState="Main.updateContentState" ref="FullToolbarRef" />
</template>
...
const ElementCheckIn = {
	...
	isContainsInSmallToolbar($ele: Node | null) {
		...
		let $smallToolbar =
			document.querySelector<HTMLDivElement>("#small-toolbar");
...
watch(
	...
	(newValue) => {
		...
		toolbarStore.showCommentCount =
			newValue.value > 999 ? "999+" : newValue.value.toString();
	},
	{
		deep: true,
		immediate: true,
...
/* 监听回复编辑器是否登录，从而触发登录框 */
watch(() => TiebaReply.$data.isLogin.value, (newValue) => {
