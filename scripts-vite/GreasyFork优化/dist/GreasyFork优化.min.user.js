...
`,re="",k={isHome(){return window.location.pathname.split("/").length<=2&&window.location.search===""},isCode(){return !!window.location.pathname.split("/")?.includes("code")},isCodeStrict(){return !!window.location.pathname.match(/\/code(\/|)$/)},isVersion(){return !!window.location.pathname.match(/\/versions(\/|)$/)},isUsers(){return !!window.location.pathname.match(/\/.+\/users\/.+/gi)},isUsersConversations(){return this.isUsers()&&!!window.location.pathname.includes("/conversations")},isUsersConversationsWithSomeUser(){return this.isUsersConversations()&&!!window.location.pathname.match(/\/conversations\/[\d]+/)},isScript(){return !!window.location.pathname.match(/\/scripts\/[\d+]/)},isScriptAdmin(){return !!window.location.pathname.endsWith("/admin")},isScriptList(){return !!window.location.pathname.match(/\/scripts(\/|)$/)},isScriptsBySite(){return !!window.location.pathname.match("/scripts/by-site")},isScriptsFeedback(){return this.isScript()&&window.location.pathname.match(/\/feedback(\/|)$/i)},isScriptLibraryList(){return !!window.location.pathname.match(/\/libraries(\/|)$/)},isScriptSearch(){let t=new URLSearchParams(window.location.search);return this.isScriptList()&&t.has("q")},isScriptCodeSearch(){return !!window.location.pathname.match(/\/code-search(\/|)$/)},isScriptLibraryListSearch(){let t=new URLSearchParams(window.location.search);return this.isScriptLibraryList()&&t.has("q")},isDiscuessions(){return !!window.location.pathname.match(/\/discussions(\/|)$/)},isImageSource(){return window.location.pathname.startsWith("/vite/assets")}},ne={init(){h.execMenuOnce("beautifyPage",()=>this.beautifyPageElement()),h.execMenuOnce("beautifyGreasyforkBeautify",()=>this.beautifyGreasyforkBeautify()),h.execMenuOnce("beautifyUploadImage",()=>this.beautifyUploadImage()),h.execMenuOnce("beautifyTopNavigationBar",()=>this.beautifyTopNavigationBar());},beautifyPageElement(){u.info("美化页面元素");let t=[];return t.push(x(Yt)),t.push(x(Zt)),t.push(x(Wt)),t.push(x(Jt)),t.push(x(_t)),t.push(x(`
			...
			`)),d.ready(()=>{let e=M('a[target="markup_choice"][href*="daringfireball.net"]');e&&e.parentElement.replaceChild(d.createElement("span",{textContent:"Markdown"}),e),O(".user-content ul li").forEach(o=>{let a=o.firstChild;a?.nodeName==="#text"&&(a.textContent?.startsWith("[x] ")?(a.textContent=a.textContent.replace("[x] ",""),d.prepend(o,`
				...
				`},n=Object.keys(A);if(O(".user-content blockquote").forEach(o=>{Array.from(o.children).forEach(a=>{let i=d.html(a);if(a.nodeName==="P"){let s=n.find(l=>i.toLowerCase().startsWith(l.toLowerCase()));s&&(d.html(a,i.replace(s,"")),d.before(a,A[s]),d.html(a)===""&&a.remove());}});}),O(".user-content > blockquote").forEach(o=>{Array.from(o.querySelectorAll(".markdown-alert-title")).forEach(a=>{let i=d.createElement("div",{className:"markdown-alert markdown-alert-tip"},{"data-type":a.getAttribute("data-type")});for(;a.nextSibling&&!a.nextSibling?.classList?.contains("markdown-alert-title");)d.append(i,a.nextSibling);d.prepend(i,a),d.before(o,i);});}),k.isHome())t.push(x(Ae));else if(k.isScriptsFeedback()){t.push(x(re));let o=M('.radio-label[for*="discussion_rating_0"]');o&&(o.innerHTML=o.innerHTML.replace("不评分",'<span class="rating-icon rating-icon-none">不评分</span>'));let a=M('.radio-label[for*="discussion_rating_2"]');a&&(a.innerHTML=a.innerHTML.replace("差评",'<span class="rating-icon rating-icon-bad">差评</span>'));let i=M('.radio-label[for*="discussion_rating_3"]');i&&(i.innerHTML=i.innerHTML.replace("一般",'<span class="rating-icon rating-icon-ok">一般</span>'));let s=M('.radio-label[for*="discussion_rating_4"]');s&&(s.innerHTML=s.innerHTML.replace("好评",'<span class="rating-icon rating-icon-good">好评</span>'));}else k.isScriptAdmin()&&(M('input[type="submit"][name="update-only"]')||t.push(x(`
					.indented{
						padding-left: unset;
					}
				...
				section#script-info{
					margin-top: 10px;
				}`)),t},beautifyUploadImage(){u.info("美化上传图片");let t=[];return t.push(x($t)),d.ready(()=>{function e(o){for(;o.nextElementSibling;)o.parentElement.removeChild(o.nextElementSibling);}let A=O('input[type="file"]');A.forEach(o=>{o.getAttribute("name")!=="code_upload"&&(o.hasAttribute("accept")&&o.getAttribute("accept").includes("javascript")||d.on(o,["propertychange","input"],function(a){e(a.target);let i=a.currentTarget.files;if(!i||i.length===0)return;u.info("选择的图片",i),i.length>5&&d.after(o,d.createElement("p",{textContent:r.t("❌ 最多同时长传5张图片")}));let s=[];Array.from(i).forEach(l=>{(l.size>204800||!l.type.match(/png|jpg|jpeg|gif|apng|webp/i))&&s.push(l);}),s.length!==0&&s.forEach(l=>{d.after(o,d.createElement("p",{textContent:r.t("❌ 图片：{{name}} 大小：{{size}}",{name:l.name,size:l.size})}));});}));}),["textarea#comment_text","textarea.comment-entry"].forEach(o=>{d.on(o,"paste",a=>{u.info("触发粘贴事件",a),setTimeout(()=>{d.trigger(A,"input");},100);});});}),t},beautifyTopNavigationBar(){u.info("美化顶部导航栏");let t=[];return t.push(x(ee)),window.outerWidth>550&&(t.push(tt.addBlockCSS(".with-submenu")),d.ready(()=>{let A=M("#site-nav").querySelector("nav");O(".with-submenu nav li").forEach(n=>{A.appendChild(n);});})),t}},E={menu:Et,isLogin:false,initEnv(){let t=this.getUserLinkElement();this.isLogin=!!t;},getUserLinkElement(){return M("#nav-user-info span.user-profile-link a")},async updateScript(t){let e=function(A,n=1){return `
        ...
        `);let t=new g.LockFunction(()=>{this.filter();},50);g.mutationObserver(document.body,{config:{subtree:true,childList:true},immediate:true,callback:()=>{t.run();}}),t.run();},getElementList(){let t=[];return t=t.concat(Array.from(O(".discussion-list-container"))),t},filter(){this.transformOldRule();const t=new Map;this.getElementList().forEach((e,A)=>{const n=this.parseDiscuessionListContainerInfo(e);let o=this.getValue().split(`
`);if(t.has(n.snippet)&&h.getValue("greasyfork-discussions-filter-duplicate-comments")){let a=t.get(n.snippet).querySelector("a.discussion-title");a.setAttribute("data-repeat-tip-show","true");let i=0;a.hasAttribute("data-repeat-count")&&(i=parseInt(a.getAttribute("data-repeat-count"))),i++,a.setAttribute("data-repeat-count",i.toString()),a.setAttribute("data-repeat-tip-show",r.t("已过滤：{{oldCount}}",{oldCount:i})),u.success([`过滤重复内容：${n.snippet}`,n]),e.remove();return}t.set(n.snippet,e);for(let a=0;a<o.length;a++){let i=o[a],s=i.split("##"),l=s[0],c=s[1];if(l in n){let p=new RegExp(c,"ig");if(n[l]!=null&&String(n[l]).match(p)){u.info("触发论坛过滤规则",i,n),e.remove();return}}}});},parseDiscuessionListContainerInfo(t){let e=t.querySelector("a.discussion-title").href,A=e.match(/\/discussions(|\/greasyfork)\/([\d]+)/),n=A[A.length-1];const o={scriptName:t.querySelector(".discussion-meta-item-script-name").innerText,scriptUrl:t.querySelector(".discussion-meta-item-script-name a")?.href,scriptId:C.getScriptId(t.querySelector(".discussion-meta-item-script-name a")?.href),postUserName:t.querySelector("a.user-link").innerText,postUserHomeUrl:t.querySelector("a.user-link").href,postUserId:C.getUserId(t.querySelector("a.user-link").href),postTimeStamp:new Date(t.querySelector("relative-time").getAttribute("datetime")),snippetId:n,snippetUrl:e,snippet:t.querySelector("span.discussion-snippet")?.innerText||"",replyUserName:void 0,replyUserHomeUrl:void 0,replyUserId:void 0,replyTimeStamp:void 0};return t.querySelector(".discussion-meta-item .discussion-meta-item")&&(o.replyUserName=t.querySelector(".discussion-meta-item .discussion-meta-item a.user-link").innerText,o.replyUserHomeUrl=t.querySelector(".discussion-meta-item .discussion-meta-item a.user-link").href,o.replyUserId=C.getUserId(o.replyUserHomeUrl),o.replyTimeStamp=new Date(t.querySelector(".discussion-meta-item .discussion-meta-item relative-time")?.getAttribute("datetime"))),o},transformOldRule(){if(Date.now()>new Date("2024-8-19").getTime())return;const t="greasyfork-discussions-filter-script",e="greasyfork-discussions-filter-post-user",A="greasyfork-discussions-filter-reply-user",n=h.getValue(t,""),o=h.getValue(e,""),a=h.getValue(A,""),i=n.trim()===""?[]:n.split(`
`),s=o.trim()===""?[]:o.split(`
`),l=a.trim()===""?[]:a.split(`
`);i.forEach(c=>{this.addValue("scriptId",g.parseStringToRegExpString("^"+c+"$"));}),s.forEach(c=>{this.addValue("postUserId",g.parseStringToRegExpString("^"+c+"$"));}),l.forEach(c=>{this.addValue("replyUserId",g.parseStringToRegExpString("^"+c+"$"));}),h.deleteValue(t),h.deleteValue(e),h.deleteValue(A);},setValue(t){h.setValue(this.key,t);},addValue(t,e){let A=this.getValue();if(A.trim()!==""&&(A+=`
`),g.isNull(t))return;t=t.toString().trim();let n=t+"##"+e;A+=n,this.setValue(A);},getValue(){return h.getValue(this.key,"")}},ie={init(){this.readBgColor(),d.ready(()=>{h.execMenuOnce("greasyfork-discussions-filter-enable",()=>{this.filterEnable();});let t=new g.LockFunction(()=>{h.execMenu("discussions-addShortcutOperationButton",()=>{this.addShortcutOperationButton();}),h.execMenu("discussions-addReportButton",()=>{this.addReportButton();});});g.mutationObserver(document.body,{config:{subtree:true,childList:true},immediate:true,callback:()=>{t.run();}});});},filterEnable(){u.info("启用Greasyfork论坛过滤器"),V.init();},readBgColor(){u.info("设置已读背景颜色");let t=h.getValue("discussions-readBgColor");return x(`
        ...
        `)},addShortcutOperationButton(){u.info("添加【过滤】按钮"),V.getElementList().forEach(t=>{if(t.querySelector(".discussion-filter-button"))return;let A=t.querySelector(".discussion-list-item").querySelector(".discussion-meta"),n=d.createElement("div",{className:"discussion-meta-item",innerHTML:`
					...
					`},{style:"flex: 0;"}),o=n.querySelector(".discussion-filter-button");A.appendChild(n),d.on(o,"click",a=>{g.preventEvent(a);const i=V.parseDiscuessionListContainerInfo(t);let s="data-filter-key",l="data-filter-value",c=j.alert({title:{text:r.t("选择需要过滤的选项"),position:"center",html:false},content:{text:`
								<button ${s}="scriptId" ${l}="^${i.scriptId}$">${r.t("脚本id：{{text}}",{text:i.scriptId})}</button>
								<button ${s}="scriptName" ${l}="^${g.parseStringToRegExpString(i.scriptName)}$">${r.t("脚本名：{{text}}",{text:i.scriptName})}</button>
								<button ${s}="postUserId" ${l}="^${g.parseStringToRegExpString(i.postUserId)}$">${r.t("发布的用户id：{{text}}",{text:i.postUserId})}</button>
						...
						`}),p=c.$shadowRoot.querySelector(".pops-alert-content");if(i.scriptId==null&&p.querySelector(`button[${s}="scriptId"]`)?.remove(),i.scriptName==null&&p.querySelector(`button[${s}="scriptName"]`)?.remove(),i.postUserId==null&&p.querySelector(`button[${s}="postUserId"]`)?.remove(),i.replyUserId!=null){let f=d.createElement("button",{innerHTML:r.t("作者id：{{text}}",{text:i.replyUserId})});f.setAttribute(s,"scriptAuthorId"),f.setAttribute(l,"^"+i.replyUserId+"$"),p.appendChild(f);}d.on(c.$shadowRoot,"click",`button[${s}]`,f=>{g.preventEvent(f);let m=f.target,y=m.getAttribute(s),S=m.getAttribute(l);V.addValue(y,S),c.close(),V.filter(),v.success(r.t("添加成功"));});});});},addReportButton(){u.info("添加【举报】按钮"),V.getElementList().forEach(t=>{if(t.querySelector(".discussion-report-button"))return;let A=t.querySelector(".discussion-list-item").querySelector(".discussion-meta"),n=d.createElement("div",{className:"discussion-meta-item",innerHTML:`
					<button class="discussion-report-button pops-button--danger">${r.t("举报")}</button>
					`},{style:"flex: 0;"}),o=n.querySelector(".discussion-report-button");A.appendChild(n),d.on(o,"click",a=>{g.preventEvent(a);const i=V.parseDiscuessionListContainerInfo(t);j.alert({title:{text:r.t("举报"),position:"center",html:false},content:{text:`
						<div class="report-item">
							${r.t("举报讨论：")}
							<a href="${C.getReportUrl("discussion",i.snippetId)}" target="_blank">${i.snippet}</a>
						</div>
						${i.scriptId?`
							<div class="report-item">
							${r.t("举报脚本：")}
							<a href="${C.getReportUrl("script",i.scriptId)}" target="_blank">${i.scriptName}</a>
						</div>
						`:""}
						
						<div class="report-item">
							${r.t("举报用户：")}
							<a href="${C.getReportUrl("user",i.postUserId)}" target="_blank">${i.postUserName}</a>
						</div>
						${i.replyUserId&&i.replyUserId!=i.postUserId?`
								<div class="report-item">
									${r.t("举报用户：")}
									<a href="${C.getReportUrl("user",i.replyUserId)}" target="_blank">${i.replyUserName}</a>
								</div>
								`:""}
							...
							`});});});}},Pt={isCurrentLoginUserHome(){let t=At.getCurrentLoginUserId();return !!(t!=null&&k.isUsers()&&window.location.pathname.includes("/"+t))},monacoEditor(){return new Promise(t=>{const e="0.52.2",A="monaco-editor-ready";if(Q.monaco){t(Q.monaco);return}let n=v.loading(r.t("monaco-editor加载中...")),o=d.createElement("script",{type:"module",defer:true,innerHTML:`
		...
		`),tt.addBlockCSS("#script-content .code-container > pre");let t=C.getScriptId(window.location.href);if(!t){v.error("未解析出当前脚本ID",{consoleLogContent:true});return}Promise.all([Pt.monacoEditor(),K.getScriptInfo(t)]).then(async([e,A])=>{let n=A?.code_url;if(!n){v.error("请求结果中未解析出脚本代码URL",{consoleLogContent:true});return}let o=new URLSearchParams(window.location.search);if(o.has("version")){let s=o.get("version");n=n.replace(new RegExp(`/${t}(/[\\d]+|)`),`/${t}/${s}`),u.info(`当前是历史代码页面，请求的脚本代码URL为${n}`);}let a=await U.get(n,{timeout:2e4});if(!a.status)return;let i=a.data.responseText;d.ready(async()=>{let s=await g.waitNode("#script-content .code-container > pre",1e4);if(!s)return;let l=d.createElement("div",{className:"monaco-editor"});d.after(s,l),e.editor.create(l,{value:i,minimap:{enabled:true},automaticLayout:true,codeLens:true,colorDecorators:true,contextmenu:true,readOnly:true,formatOnPaste:true,overviewRulerBorder:true,scrollBeyondLastLine:false,theme:"vs-dark",fontSize:window.innerWidth>600?14:12,wordWrap:"off",language:"javascript",folding:true,foldingStrategy:"indentation"});});});}},se=`ul.history_versions,\r
...
`,le={init(){h.execMenuOnce("beautifyHistoryVersionPage",()=>this.beautifyHistoryVersionPage()),h.execMenuOnce("scripts-versions-addExtraTagButton",()=>{this.addExtraTagButton();}),h.execMenuOnce("scripts-versions-addCompareCodeButton",()=>{this.sourceDiffMonacoEditor();});},beautifyHistoryVersionPage(){u.info("美化 历史版本 页面");let t=[];return t.push(x(se)),t.push(tt.addBlockCSS(".version-number",".version-date",".version-changelog")),d.ready(function(){let e=M("ul.history_versions");if(!e){v.error(r.t("未找到history_versions元素列表"));return}Array.from(e.children).forEach(A=>{let n=A.querySelector(".version-number a").href,o=A.querySelector(".version-number a").innerText,a=A.querySelector(".version-date")?.getAttribute("datetime"),i=A.querySelector(".version-changelog")?.innerHTML||"",s=d.createElement("span",{className:"script-version-date",innerHTML:g.formatTime(a,r.t("yyyy年MM月dd日 HH:mm:ss"))}),l=d.createElement("div",{className:"script-tag",innerHTML:`
                    <div class="script-tag-version">
                    ...
                    </div>`}),c=d.createElement("div",{className:"script-note-box-body",innerHTML:i});A.appendChild(s),A.appendChild(l),A.appendChild(c);});}),t},addExtraTagButton(){u.info("添加额外的标签按钮"),d.ready(()=>{O(".script-tag-version").forEach(t=>{let e=t.querySelector("a");if(!e)return;let A=new URL(e.href),n=A.pathname.match(/\/scripts\/([\d]+)/)?.[1],o=A.searchParams.get("version"),a=A.pathname.match(/\/scripts\/[\d]+-(.+)/)?.[1],i=C.getInstallUrl(n,o,a),s=C.getCodeUrl(n,o),l=d.createElement("div",{className:"scripts-tag-install",innerHTML:`
						...
						`});d.after(t,l);});});},sourceDiffMonacoEditor(){u.info("源码对比（monacoEditor）"),Pt.monacoEditor().then(t=>{d.ready(()=>{O('#script-content form[action*="/diff"] input[type="submit"]').forEach(e=>{let A=d.createElement("input",{type:"button",value:r.t("对比选中版本差异（monacoEditor）")},{style:"margin-left: 10px;"});d.after(e,A),d.on(A,"click",async n=>{g.preventEvent(n);let o=e.closest("form"),a=new FormData(o),i=a.get("v1"),s=a.get("v2");if(i===s){v.warning(r.t("版本号相同，不需要比较源码"));return}let l=v.loading(r.t("正在获取对比文本中...")),c=C.getScriptId(),p=await K.getScriptInfo(c);if(!p){l.close();return}let f=p.code_url,m=f.replace(new RegExp(`/${c}(/[\\d]+|)`),`/${c}/${i}`),y=f.replace(new RegExp(`/${c}(/[\\d]+|)`),`/${c}/${s}`),S="",b="";const[I,P]=await Promise.all([U.get(m,{timeout:2e4}),U.get(y,{timeout:2e4})]);if(l.close(),!I.status||!P.status)return;S=I.data.responseText,b=P.data.responseText;let{recovery:R}=tt.lockScroll(),L=j.alert({title:{text:r.t("代码对比"),html:false,position:"center"},content:{html:true,text:`
						...
						`});L.$shadowRoot.querySelector(".monaco-editor-diff-container");let z=L.$shadowRoot.querySelector(".monaco-editor-diff"),D=t.editor.createDiffEditor(z,{hideUnchangedRegions:{enabled:true},minimap:{enabled:true},automaticLayout:true,codeLens:true,colorDecorators:true,contextmenu:false,readOnly:true,formatOnPaste:true,overviewRulerBorder:true,scrollBeyondLastLine:false,theme:"vs-dark",fontSize:window.innerWidth>600?14:12,wordWrap:"off",language:"javascript"});const T=t.editor.createModel(b,"javascript"),X=t.editor.createModel(S,"javascript");D.setModel({original:T,modified:X});});});});});}};let ut=[];const Ot=async function(t){if(u.info("当前脚本id："+t),!E.isLogin){u.error("请先登录账号"),v.error(r.t("请先登录账号"));return}let e=C.getUserId(E.getUserLinkElement().href);if(e==null){u.error("获取用户id失败"),v.error(r.t("获取用户id失败"));return}if(!ut.length){let a=v.loading(r.t("获取收藏夹中..."));if(ut=await K.getUserCollection(e)||[],a.close(),!ut.length)return}let A="";const n=(a,i)=>{let s=false;i=i.toString().trim();for(const[l,c]of a.entries())if(l==="scripts-included[]"&&c.toString().trim()===i){s=true;break}return s};ut.forEach(a=>{A+=`
		<li class="user-collect-item" data-id="${a.id}" data-name="${a.name}">
		  ...
		  `;});let o=j.alert({title:{text:r.t("收藏集"),position:"center"},content:{html:true,text:`<ul>${A}</ul>`},mask:{enable:true,clickEvent:{toClose:true}},btn:{ok:{enable:false}},width:j.isPhone()?"92vw":"500px",height:"auto",drag:true,only:true,style:`
		.pops{
...
`,vt={getTampermonkey:()=>Q.external?.Tampermonkey,getViolentmonkey:()=>Q.external?.Violentmonkey,getScriptCat:()=>Q.external?.Scriptcat,getScriptContainerStatus(){let t={Tampermonkey:false,Violentmonkey:false,ScriptCat:false};return Q.external?.Tampermonkey&&(t.Tampermonkey=true),Q.external?.Violentmonkey&&(t.Violentmonkey=true),Q.external?.Scriptcat&&(t.ScriptCat=true),t},getRegisterScriptContainerNameList(){let e=this.getScriptContainerStatus(),A=[];return Object.keys(e).forEach(n=>{Reflect.get(e,n)&&A.push(n);}),A},getInstalledVersion(t,e){return new Promise((A,n)=>{const o=this.getTampermonkey();if(o){o.isInstalled(t,e,function(s){s.installed?A(s.version):A(null);});return}const a=this.getViolentmonkey();if(a){a.isInstalled(t,e).then(A);return}const i=this.getScriptCat();if(i){i.isInstalled(t,e,function(s){s.installed?A(s.version):A(null);});return}n(new TypeError("获取脚本容器暴露的external信息失败"));})},compareVersions(t,e){if(t===e)return 0;const A=t.split("."),n=e.split(".");for(let o=0;o<A.length;o++){const a=this.compareVersionPart(A[o],n[o]);if(a!==0)return a}return 0},compareVersionPart(t,e){const A=this.parseVersionPart(t),n=this.parseVersionPart(e);for(let o=0;o<A.length;o++){if(A[o].length>0&&n[o].length===0)return  -1;if(A[o].length===0&&n[o].length>0||A[o]>n[o])return 1;if(A[o]<n[o])return  -1}return 0},parseVersionPart(t){if(!t)return [0,"",0,""];const e=/([0-9]*)([^0-9]*)([0-9]*)([^0-9]*)/.exec(t);return [e[1]?parseInt(e[1]):0,e[2],e[3]?parseInt(e[3]):0,e[4]]},handleInstallResult(t,e,A){if(e!=null)switch(t.removeAttribute("data-ping-url"),this.compareVersions(e,A)){case  -1:t.textContent=t.getAttribute("data-update-label");break;case 1:t.textContent=t.getAttribute("data-downgrade-label");break;case 0:t.textContent=t.getAttribute("data-reinstall-label");break}},async checkForUpdatesJS(t,e){const A=t.getAttribute("data-script-name"),n=t.getAttribute("data-script-namespace"),o=t.getAttribute("data-script-version");try{let a=await this.getInstalledVersion(A,n);return a==null?!1:(this.handleInstallResult(t,a,o),!0)}catch{if(e){await g.sleep(1e3);try{return await this.checkForUpdatesJS(t,!1)}catch{}}return  false}},checkForUpdatesCSS(t){const e=t.getAttribute("data-script-name"),A=t.getAttribute("data-script-namespace");postMessage({type:"style-version-query",name:e,namespace:A,url:location.href},location.origin);}},Tt={init(){h.execMenuOnce("gf-scripts-filter-enable",()=>{W.init();}),h.execMenuOnce("beautifyCenterContent",()=>this.beautifyCenterContent());},beautifyCenterContent(){u.info("美化脚本列表-双列");let t=[];t.push(x(de));const e="lum-lightbox-loader",A=r.t("安装此脚本");return B.ready(()=>{let n=vt.getScriptContainerStatus(),o=Object.values(n).find(c=>c),a=vt.getRegisterScriptContainerNameList(),i=h.getValue("beautifyCenterContent-queryNameSpace");o?u.info("当前暴露的external信息："+a.map(c=>`【${c}】`).join("、")):u.error("脚本容器未暴露external信息",window.external);let s=new g.LockFunction(()=>{W.getElementList().forEach(p=>{if(p.querySelector(".script-list-operation"))return;let f=At.parseScriptListInfo(p),m=p.querySelector(".inline-script-stats");if(!m)return;let y=f.codeUrl,S=B.createElement("dt",{className:"script-list-rating-score",innerHTML:`<span>${r.t("评分")}</span>`}),b=B.createElement("dd",{className:"script-list-rating-score",innerHTML:`<span>${f.scriptRatingScore}</span>`},{"data-position":"right"}),I=p.querySelector("dd.script-list-ratings .good-rating-count"),P=p.querySelector("dd.script-list-ratings .ok-rating-count"),R=p.querySelector("dd.script-list-ratings .bad-rating-count");if(I&&P&&R){let H=parseInt(I.innerText),G=parseInt(P.innerText),Y=parseInt(R.innerText),st=H+G+Y;st>=10?H/st>=.6?b.classList.add("good-rating-count"):b.classList.add("bad-rating-count"):st==0||H>G+Y?b.classList.add("good-rating-count"):b.classList.add("bad-rating-count");}let L=B.createElement("dt",{className:"script-list-version",innerHTML:`<span>${r.t("版本")}</span>`}),z=B.createElement("dd",{className:"script-list-version",innerHTML:`<span>${f.scriptVersion}</span>`},{"data-position":"right"}),D=B.createElement("dt",{className:"script-list-operation",innerHTML:`<span>${r.t("操作")}</span>`}),T=B.createElement("dd",{className:"script-list-operation",innerHTML:`
						...
						`},{"data-position":"right",style:"gap:10px;display: flex;flex-wrap: wrap;align-items: center;"}),X=T.querySelector(".script-collect-btn"),N=T.querySelector(".install-link");if(N["data-script-info"]=f,B.addClass(N,e),f.scriptType==="library"&&N.remove(),B.on(X,"click",H=>{g.preventEvent(H),Ot(f.scriptId);}),h.getValue("gf-scripts-filter-enable")){let H=B.createElement("button",{className:"script-filter-btn",innerHTML:r.t("过滤")}),G="data-filter-key",Y="data-filter-value";B.on(H,"click",st=>{g.preventEvent(st);let gt=j.alert({title:{text:r.t("选择需要过滤的选项"),position:"center"},content:{text:`
								...
								`}),zt=gt.$shadowRoot.querySelector(".pops-alert-content");f.scriptAuthors.forEach(_=>{let rt=B.createElement("button",{innerHTML:r.t("作者id：{{text}}",{text:_.authorId})});rt.setAttribute(G,"scriptAuthorId"),rt.setAttribute(Y,"^"+_.authorId+"$");let lt=B.createElement("button",{innerHTML:r.t("作者名：{{text}}",{text:_.authorName})});lt.setAttribute(G,"scriptAuthorName"),lt.setAttribute(Y,"^"+g.parseStringToRegExpString(_.authorName)+"$"),zt.appendChild(rt),zt.appendChild(lt);}),B.on(gt.$shadowRoot,"click",`button[${G}]`,_=>{g.preventEvent(_);let rt=_.target,lt=rt.getAttribute(G),jt=rt.getAttribute(Y);W.addValue(lt,jt),gt.close(),W.filter(),v.success(r.t("添加成功"));});}),T.appendChild(H);}m.appendChild(S),m.appendChild(b),m.appendChild(L),m.appendChild(z),m.appendChild(D),m.appendChild(T);});},100),l=new g.LockFunction(async()=>{let c=Array.from(O(".install-link[data-install-format=js]:not([gm-is-check-install-status])"));for(let p=0;p<c.length;p++){const f=c[p];f.setAttribute("gm-is-check-install-status","");let m=Reflect.get(f,"data-script-info");if(o){if(i){let y=await K.getScriptInfo(m.scriptId);y&&f.setAttribute("data-script-namespace",y.namespace);}vt.checkForUpdatesJS(f,true).then(y=>{B.removeClass(f,e),y||B.text(f,A);});}else B.removeClass(f,e),B.text(f,A);}});g.mutationObserver(document,{config:{subtree:true,childList:true},immediate:true,callback:()=>{s.run(),l.run();}});}),t}},ue={init(){h.execMenuOnce("users-changeConsoleToTopNavigator",()=>{this.changeConsoleToTopNavigator();}),h.execMenuOnce("gf-scripts-filter-enable",()=>{W.init();}),h.execMenuOnce("beautifyCenterContent",()=>Tt.beautifyCenterContent());},changeConsoleToTopNavigator(){u.info("迁移【控制台】到顶部导航栏"),tt.addBlockCSS("#about-user"),d.ready(()=>{let t=M("#about-user");if(!t){u.error("#about-user元素不存在");return}t=t.cloneNode(true),At.registerTopNavMenu({name:r.t("控制台"),className:"scripts-console",clickEvent(e){j.alert({title:{text:r.t("控制台"),position:"center"},content:{text:"",html:true},btn:{ok:{enable:false}},mask:{enable:true,clickEvent:{toClose:true}},drag:true,useShadowRoot:true,width:J.setting.width,height:J.setting.height,zIndex:g.getMaxZIndex(100),style:`
						#about-user{
							border: 0;
							box-shadow: none;
						}
						...
						`}).$shadowRoot.querySelector(".pops-alert-content").appendChild(t);}});});}};class fe{key="short-cut";$data;isWaitPress=false;currentWaitEnterPressInstanceHandler=null;constructor(e){typeof e=="string"&&(this.key=e),this.$data={otherShortCutOptions:[]};}initConfig(e,A){this.hasOption(e)||this.setOption(e,A);}getStorageKey(){return this.key}getLocalAllOptions(){return F(this.key,[])}hasOption(e){return !!this.getLocalAllOptions().find(o=>o.key===e)}hasOptionValue(e){return this.hasOption(e)?this.getOption(e)?.value!=null:false}getOption(e,A){return this.getLocalAllOptions().find(a=>a.key===e)??A}setOption(e,A){let n=this.getLocalAllOptions(),o=n.findIndex(a=>a.key===e);o==-1?n.push({key:e,value:A}):Reflect.set(n[o],"value",A),q(this.key,n);}emptyOption(e){let A=false,n=this.getLocalAllOptions(),o=n.findIndex(a=>a.key===e);return o!==-1&&(n[o].value=null,A=true),q(this.key,n),A}deleteOption(e){let A=false,n=this.getLocalAllOptions(),o=n.findIndex(a=>a.key===e);return o!==-1&&(n.splice(o,1),A=true),q(this.key,n),A}translateKeyboardValueToButtonText(e){let A="";return e.ohterCodeList.forEach(n=>{A+=g.stringTitleToUpperCase(n,true)+" + ";}),A+=g.stringTitleToUpperCase(e.keyName),A}getShowText(e,A){if(this.hasOption(e)){let n=this.getOption(e);return n.value==null?A:this.translateKeyboardValueToButtonText(n.value)}else return A}async enterShortcutKeys(e){const A=this;return new Promise(n=>{this.isWaitPress=true;let o=d.listenKeyboard(window,"keyup",(a,i,s)=>{const l={keyName:a,keyValue:i,ohterCodeList:s};let c={};try{const p=JSON.stringify(l),f=this.getLocalAllOptions();Array.isArray(this.$data.otherShortCutOptions)&&f.push(...this.$data.otherShortCutOptions);for(let m=0;m<f.length;m++){let y=f[m];if(y.key===e)continue;const S=JSON.stringify(y.value);let b=!1;if(y.value!=null&&p===S&&(b=!0),b){c={status:!1,key:y.key,option:l};return}}this.setOption(e,l),c={status:!0,key:e,option:l};}catch(p){console.log(p),c={status:false,key:e,option:l};}finally{A.isWaitPress=false,o.removeListen(),A.currentWaitEnterPressInstanceHandler=null,n(c);}});A.currentWaitEnterPressInstanceHandler=null,A.currentWaitEnterPressInstanceHandler=()=>{A.isWaitPress=false,o.removeListen();};})}cancelEnterShortcutKeys(){typeof this.currentWaitEnterPressInstanceHandler=="function"&&this.currentWaitEnterPressInstanceHandler();}initGlobalKeyboardListener(e,A){let n=this.getLocalAllOptions();if(!n.length){u.warn("没有设置快捷键");return}const o=this;function a(l,c){d.listenKeyboard(l,"keydown",(p,f,m,y)=>{if(o.isWaitPress)return;A?.isPrevent&&g.preventEvent(y),n=o.getLocalAllOptions();let S=n.findIndex(b=>{let I=b.value,P={keyName:p,keyValue:f,ohterCodeList:m};if(JSON.stringify(I)===JSON.stringify(P))return b});if(S!=-1){let b=n[S];b.key in c&&(u.info(["调用快捷键",b]),c[b.key].callback());}},{capture:!!A?.capture});}let i={},s={};Object.keys(e).forEach(l=>{let c=e[l];(c.target==null||typeof c.target=="string"&&c.target==="")&&(c.target="window"),c.target==="window"?Reflect.set(i,l,c):Reflect.set(s,l,c);}),a(window,i),d.ready(()=>{Object.keys(s).forEach(async l=>{let c=s[l];if(typeof c.target=="string")g.waitNode(c.target,1e4).then(p=>{if(!p)return;let f={};Reflect.set(f,l,c),a(p,f);});else if(typeof c.target=="function"){let p=await c.target();if(p==null)return;let f={};Reflect.set(f,l,c),a(p,f);}else {let p={};Reflect.set(p,l,c),a(c.target,p);}});});}}const Bt={shortCut:new fe,shortOption:{"gf-quickReply":{target:()=>{let t=M("form textarea"),e=M('input[name="commit"][type="submit"]');if(t){if(!e){u.error("页面不存在【发表回复】按钮");return}}else {u.error("页面不存在输入框");return}return u.success("监听快捷键：gf-quickReply"),t},callback(){if(document.activeElement){let t=document.activeElement.closest("form");if(!t){u.error("当前activeElement不在表单内，无法触发快捷键");return}let e=t.querySelectorAll('input[name="commit"][type="submit"]');if(!e.length){u.error("表单内不存在【发表回复】按钮");return}e.length>1&&u.warn("表单内存在多个【发表回复】按钮，只触发第一个"),e[0].click();}else u.error("当前页面没有激活元素，无法触发快捷键");}}},init(){this.shortCut.initGlobalKeyboardListener(this.shortOption);}},pe=`section.text-content {\r
...
.comment:not(:has(.report-link)) .comment-meta-item {\r
	...
	margin-right: 15px;\r
		...
		`),O("section.text-content ul li").forEach(t=>{let e=t.querySelector('a[href*="conversations"]'),A=e.href,n=e.textContent?.split(" ")[1],o=t.querySelector("a.user-link"),a=null,i=null,s=null;if(o){a=o.textContent,i=o.href;let l=t.querySelector("relative-time");new Date(l.getAttribute("datetime")),s=l.shadowRoot.lastChild.textContent;}t.classList.add("user-conversations-item"),C.getUserId(),t.innerHTML=`
			...
			`}),i=a.querySelector(".pops-filter-controls_inner"),s=a.querySelector(".pops-filter-search-container");return o.appendChild(a),d.before(t,n),{$filterControl:i,$search:s}}},ve={init(){d.ready(()=>{Mt.waitScritList().then(t=>{if(!t){u.error("未找到脚本列表节点，无法继续执行");return}h.onceExec("GreasyforkScriptsSearch",()=>{let{$filterControl:e,$search:A}=Mt.addFilterControls(t);this.addFilterControlsItem(A,e);});});});},addFilterControlsItem(t,e){let A=j.config.PanelHandlerComponents(),n=[{name:r.t("名称"),desc:r.t("全词匹配"),ENABLE_KEY:"gf-script-search-filterScriptTitleWholeWordMatching",STORAGE_KEY:"gf-script-search-filterScriptTitleWholeWordMatching-enable",callback:(i,s)=>{try{return !s.scriptName.match(new RegExp(i,"i"))}catch(l){return u.error(l),!s.scriptName.match(i)}},mutualExclusionSwitch:['.gm-script-control-item[data-storage-key="gf-script-search-filterScriptTitleOrDescWholeWordMatching-enable"]']},{name:r.t("描述"),desc:r.t("全词匹配"),ENABLE_KEY:"gf-script-search-filterScriptDescWholeWordMatching",STORAGE_KEY:"gf-script-search-filterScriptDescWholeWordMatching-enable",callback:(i,s)=>{try{return !s.scriptDescription.match(new RegExp(i,"i"))}catch{return !s.scriptDescription.match(i)}},mutualExclusionSwitch:['.gm-script-control-item[data-storage-key="gf-script-search-filterScriptTitleOrDescWholeWordMatching-enable"]']},{name:r.t("名称/描述"),desc:r.t("任一全词匹配"),ENABLE_KEY:"gf-script-search-filterScriptTitleOrDescWholeWordMatching",STORAGE_KEY:"gf-script-search-filterScriptTitleOrDescWholeWordMatching-enable",callback:(i,s)=>{let l=n.find(p=>p.ENABLE_KEY==="gf-script-search-filterScriptTitleWholeWordMatching").callback(i,s),c=n.find(p=>p.ENABLE_KEY==="gf-script-search-filterScriptDescWholeWordMatching").callback(i,s);return l&&c},mutualExclusionSwitch:['.gm-script-control-item[data-storage-key="gf-script-search-filterScriptTitleWholeWordMatching-enable"]','.gm-script-control-item[data-storage-key="gf-script-search-filterScriptDescWholeWordMatching-enable"]']},{name:r.t("作者名称"),desc:r.t("全词匹配"),ENABLE_KEY:"gf-script-search-filterScriptAuthorNameWholeWordMatching",STORAGE_KEY:"gf-script-search-filterScriptAuthorNameWholeWordMatching-enable",callback:(i,s)=>{try{return !s.scriptAuthorName.match(new RegExp(i,"i"))}catch{return !s.scriptAuthorName.match(i)}}}],o=()=>{let i=t.querySelector("input");if(i)return i.value;let s=new URLSearchParams(window.location.search);return s.get("q")?.trim()||s.get("c")?.trim()||""},a=()=>{let i=o();W.getElementList().forEach(l=>{let c=At.parseScriptListInfo(l),p=n.map(f=>{if(i=="")return  false;if(F(f.STORAGE_KEY))return f.callback(i,c)}).filter(f=>typeof f=="boolean");if(p.length!==0){let f=false;p.forEach(m=>{f=f||m;}),f?d.hide(l,false):d.show(l,false);}else d.show(l,false);});};if(h.getValue("gf-script-search-addFilterSearchInput")){let i=A.createSectionContainerItem_input({type:"input",className:"gm-script-search-input",getValue(){let s=o();return h.getValue("gf-script-search-addFilterSearchInput-text")||s||""},description:r.t("自动执行过滤"),placeholder:r.t("过滤的关键词"),text:r.t("关键词"),callback:g.debounce((s,l)=>{h.setValue("gf-script-search-addFilterSearchInput-text",l.toString()),a();},500)});d.append(t,i);}n.forEach(i=>{if(!h.getValue(i.ENABLE_KEY))return;u.info(`添加按钮${i.name}`);let s=A.createSectionContainerItem_switch({type:"switch",className:"gm-script-control-item",text:i.name,description:i.desc,attributes:{"data-storage-key":i.STORAGE_KEY},getValue(){let l=F(i.STORAGE_KEY,false);return l=!!l,a(),l},callback(l,c){if(q(i.STORAGE_KEY,c),a(),c&&Array.isArray(i.mutualExclusionSwitch)){let p=s.closest(".pops");i.mutualExclusionSwitch.forEach(f=>{let m=p.querySelector(f);m&&m.querySelector("input")?.checked&&m.querySelector(".pops-panel-switch__core")?.click();});}}});d.append(e,s);});}},it={init(){if(k.isImageSource()){u.info("Router: 资源界面，不执行脚本功能");return}h.execMenu("checkPage",()=>{this.checkPage();}),ne.init(),Bt.init(),k.isScript()&&ce.init(),(k.isScriptList()||k.isScriptLibraryList()||k.isScriptCodeSearch()||k.isScriptsBySite())&&Tt.init(),k.isDiscuessions()?(u.info("Router: 讨论页面"),ie.init()):k.isUsers()?(u.info("Router: 用户页面"),ue.init(),k.isUsersConversations()&&(u.info("Router-next: 私聊用户页面"),ge.init())):(k.isScriptSearch()||k.isScriptsBySite()||k.isScriptCodeSearch()||k.isScriptLibraryListSearch())&&(u.info("Router: 脚本搜索页面"),ve.init()),h.execMenuOnce("scripts-addOperationPanelBtnWithNavigator",()=>{this.addOperationPanelBtnWithNavigator();}),d.ready(()=>{E.initEnv(),oe.init(),wt.init(),E.handleLocalGotoCallBack(),h.execMenuOnce("fixImageWidth",()=>{it.fixImageWidth();}),it.languageSelectorLocale(),h.execMenuOnce("optimizeImageBrowsing",()=>{it.optimizeImageBrowsing();}),h.execMenuOnce("overlayBedImageClickEvent",()=>{it.overlayBedImageClickEvent();}),k.isCodeStrict()||h.execMenuOnce("addMarkdownCopyButton",()=>{it.addMarkdownCopyButton();});});},fixImageWidth(){window.innerWidth<window.innerHeight&&(u.info("修复图片显示问题"),x(`
            img.lum-img{
                width: 100% !important;
                height: 100% !important;
            }
        ...
        `);function t(A=[],n=0){let o="";A.forEach(s=>{o+=`<li><img data-src="${s}" loading="lazy"></li>`;});let a=d.createElement("ul",{innerHTML:o}),i=new Qt(a,{inline:false,url:"data-src",zIndex:g.getMaxZIndex()+100,hidden:()=>{i.destroy();}});n=n<0?0:n,i.view(n),i.zoomTo(1),i.show();}function e(A){return A.getAttribute("data-src")||A.getAttribute("src")||A.getAttribute("alt")}d.on(document,"click","img",function(A){let n=A.target;if(n.parentElement?.localName==="a"&&n.hasAttribute("data-screenshots")||n.closest(".viewer-container")||n.closest(".lum-lightbox-position-helper"))return;let o=n.closest(".user-content"),a=[],i=0,s=[],l=e(n);if(l){if(l.startsWith("https://img.shields.io"))return;if(l.startsWith("/vite/assets/"))return}o?(o.querySelectorAll("img").forEach(c=>{s.push(c);let p=e(c),f=c.parentElement;f?.localName==="a"&&(p=f.getAttribute("data-href")||f.href),a.push(p);}),i=s.indexOf(n),i===-1&&(i=0)):(a.push(l),i=0),u.success("点击浏览图片👉",a,i),t(a,i);}),O(".user-screenshots").forEach(A=>{let n=A.querySelector("a");if(!n)return;let o=n.getAttribute("data-href")||n.getAttribute("href"),a=A.querySelector("img");a&&(a.setAttribute("data-screenshots","true"),a.setAttribute("data-src",o),n.setAttribute("href","javascript:;"),d.after(n,a),n.remove());});},overlayBedImageClickEvent(){u.info("覆盖图床图片的parentElement的a标签"),O(".user-content a>img").forEach(t=>{let e=t.parentElement,A=e.getAttribute("href");if(e.setAttribute("data-href",A),e.removeAttribute("href"),A.startsWith("/rails/active_storage/blobs/redirect")){u.info("该图片是上传到Greasyfork的图片，拦截默认行为，不做提示");return}d.on(e,"click",()=>{v.warning(`<div style="overflow-wrap: anywhere;">${r.t("拦截跳转：")}<a href="${A}" target="_blank">${A}</a></div>`,{html:true,zIndex:g.getMaxZIndex()+105});});});},addMarkdownCopyButton(){u.info("在Markdown右上角添加复制按钮"),x(`
        pre{
          position: relative;
        ...
        `);function t(){let e=d.createElement("div",{className:"zeroclipboard-container",innerHTML:`
            ...
            `}),A=e.querySelector(".js-clipboard-copy"),n=e.querySelector(".octicon-copy"),o=e.querySelector(".octicon-check-copy");return d.on(e,"click",()=>{let a=d.parent(e),i=a.querySelector("code");if(!i&&a.className.includes("prettyprinted")&&(i=e.parentElement),i||(i=a.querySelector("pre")),!i){v.error(r.t("未找到{{selector}}元素",{selector:"code"}));return}g.setClip(d.text(i)),A.setAttribute("success","true"),n.setAttribute("aria-hidden","true"),o.removeAttribute("aria-hidden");let s=j.tooltip({target:A,content:r.t("✅ 复制成功!"),position:"left",className:"github-tooltip",alwaysShow:true});s.toolTip.onToolTipAnimationFinishEvent(),setTimeout(()=>{A.removeAttribute("success"),o.setAttribute("aria-hidden","true"),n.removeAttribute("aria-hidden"),s.toolTip.close();},2e3);}),e}O("pre").forEach(e=>{if(e.querySelector("div.zeroclipboard-container"))return;let n=t(),o=d.createElement("div",{className:"snippet-clipboard-content"});d.before(e,o),o.appendChild(e),o.appendChild(n);});},languageSelectorLocale(){let t=h.getValue("language-selector-locale"),e=window.location.pathname.split("/").filter(A=>!!A)[0];if(u.success("选择语言："+t),u.success("当前语言："+e),!g.isNull(t)&&t!==e){let A=null,n=C.getSwitchLanguageUrl(t);K.switchLanguage(n),u.success("新Url："+n),v.loading(r.t("当前语言：{{currentLocaleLanguage}}，，3秒后切换至：{{localeLanguage}}",{currentLocaleLanguage:e,localeLanguage:t}),{timeout:3e3,showClose:true,onClose(){clearTimeout(A);}}),v.info(r.t("导航至：")+n,{timeout:3e3}),A=setTimeout(()=>{window.location.href=n;},3e3);}},checkPage(){u.info("检测gf页面是否正确加载，有时候会出现"),d.ready(()=>{if(document.body.firstElementChild&&document.body.firstElementChild.localName==="p"&&document.body.firstElementChild.innerText.includes("We're down for maintenance. Check back again soon.")){let t=parseInt(F("greasyfork-check-page-time",0)),e=h.getValue("greasyfork-check-page-timeout",5),A=e*1e3;if(t&&Date.now()-t<A){v.warning(r.t("上次重载时间 {{time}}，{{timeout}}秒内拒绝反复重载",{time:g.formatTime(t,"yyyy-MM-dd HH:mm:ss"),timeout:e}));return}q("greasyfork-check-page-time",Date.now()),window.location.reload();}});},addOperationPanelBtnWithNavigator(){u.info("添加【操作面板】按钮"),tt.addBlockCSS(".sidebarred .sidebar",".sidebarred-main-content .open-sidebar"),x(`
		...
		`),d.ready(()=>{M("#site-nav nav"),M("#site-nav .with-submenu nav");let t=M("#script-list-option-groups")||M(".list-option-groups");if(!t){u.warn("不存在右侧面板元素#script-list-option-groups");return}t=t.cloneNode(true),t.classList.add("option-panel-groups"),At.registerTopNavMenu({name:r.t("操作面板"),className:"filter-scripts",clickEvent(e){j.alert({title:{text:r.t("操作面板"),position:"center"},content:{text:"",html:true},btn:{ok:{enable:false}},mask:{enable:true,clickEvent:{toClose:true}},drag:true,useShadowRoot:true,width:J.setting.width,height:J.setting.height,zIndex:g.getMaxZIndex(100),style:`
            ...
            `}),e=t.$shadowRoot.querySelector("#github"),A=t.$shadowRoot.querySelector("#webhook");d.on(e,["input","propertychange"],n=>{let o=e.value.trim(),a=[];o.split(`
`).forEach(i=>{try{if(i=i.trim(),g.isNull(i))return;let l=new URL(i).pathname.split("/"),{1:c,2:p,3:f,4:m}=l,y="";if(l.length>=6&&f==="blob")y=l.slice(5,l.length).join("/");else if(l.length>=8&&f==="raw"&&m==="refs")m=l[6],y=l.slice(7,l.length).join("/");else return;if(y==="")return;a.push(`https://raw.githubusercontent.com/${c}/${p}/${m}/${y}`);}catch{}}),A.value=a.join(`
`);});}},at=function(t,e,A,n,o,a,i,s,l,c){let p={text:t,type:"button",attributes:{},props:{},description:e,buttonIcon:n,buttonIsRightIcon:o,buttonIconIsLoading:a,buttonType:i,buttonText:A,callback(f){typeof s=="function"&&s(f);},afterAddToUListCallBack:l};return Reflect.set(p.attributes,xt,()=>{p.disable=false;}),p},Ct={$data:{__storeApiFn:null,get storeApiValue(){return this.__storeApiFn||(this.__storeApiFn=new $.Dictionary),this.__storeApiFn}},getStorageApi(t){if(this.hasStorageApi(t))return this.$data.storeApiValue.get(t)},hasStorageApi(t){return this.$data.storeApiValue.has(t)},setStorageApi(t,e){this.$data.storeApiValue.set(t,e);},initComponentsStorageApi(t,e,A){let n;this.hasStorageApi(t)?n=this.getStorageApi(t):n=A,this.setComponentsStorageApiProperty(e,n);},setComponentsStorageApiProperty(t,e){Reflect.set(t.props,et,e);}},ht=function(t,e,A,n,o,a="",i,s,l){let c={text:t,type:"input",isNumber:!!i,isPassword:!!s,attributes:{},props:{},description:n,afterAddToUListCallBack:l,getValue(){return this.props[et].get(e,A)},callback(p,f,m){this.props[et].set(e,f);},placeholder:a};return Reflect.set(c.attributes,ct,e),Reflect.set(c.attributes,dt,A),Ct.initComponentsStorageApi("input",c,{get(p,f){return h.getValue(p,f)},set(p,f){h.setValue(p,f);}}),c},w=function(t,e,A,n,o,a){let i={text:t,type:"switch",description:o,attributes:{},props:{},getValue(){return !!this.props[et].get(e,A)},callback(s,l){let c=!!l;u.success(`${c?"开启":"关闭"} ${t}`),this.props[et].set(e,c);},afterAddToUListCallBack:a};return Reflect.set(i.attributes,ct,e),Reflect.set(i.attributes,dt,A),Ct.initComponentsStorageApi("switch",i,{get(s,l){return h.getValue(s,l)},set(s,l){h.setValue(s,l);}}),i},ot=function(t,e,A,n,o,a){let i=[];typeof n=="function"?i=n():i=n;let s={text:t,type:"select",description:a,attributes:{},props:{},getValue(){return this.props[et].get(e,A)},callback(l,c,p){let f=c;if(u.info(`选择：${p}`),typeof o=="function"&&o(l,f,p))return;this.props[et].set(e,f);},data:i};return Reflect.set(s.attributes,ct,e),Reflect.set(s.attributes,dt,A),Ct.initComponentsStorageApi("select",s,{get(l,c){return h.getValue(l,c)},set(l,c){h.setValue(l,c);}}),s},me=function(t,e,A,n,o,a="default",i){let s=typeof o=="function"?o():o;typeof n=="object"&&i.initConfig(A,n);let l=()=>i.getShowText(A,s),c=at(t,e,l,"keyboard",false,false,a,async p=>{let m=p.target.closest(".pops-panel-button")?.querySelector("span");if(i.isWaitPress){v.warning("请先执行当前的录入操作");return}if(i.hasOptionValue(A))i.emptyOption(A),v.success("清空快捷键");else {let y=v.loading("请按下快捷键...",{showClose:true,onClose(){i.cancelEnterShortcutKeys();}}),{status:S,option:b,key:I}=await i.enterShortcutKeys(A);y.close(),S?(u.success(["成功录入快捷键",b]),v.success("成功录入")):v.error(`快捷键 ${i.translateKeyboardValueToButtonText(b)} 已被 ${I} 占用`);}m.innerHTML=l();});return c.attributes={},Reflect.set(c.attributes,xt,()=>false),c},St={async UIScriptList(t,e){if(!E.isLogin){v.error(r.t("请先登录账号！"));return}let o=E.getUserLinkElement().href?.split("/")?.pop()?.match(/([0-9]+)/)?.[0],a=j.loading({mask:{enable:true},parent:e,content:{text:r.t("获取信息中，请稍后...")},addIndexCSS:false}),i=await K.getUserInfo(o);if(a.close(),!i)return;u.info(i);let s=t==="script-list"?i.scriptList:i.scriptLibraryList;v.success(r.t("获取成功，共 {{count}} 个",{count:s.length}));for(const l of s){let c=d.createElement("li",{className:"w-script-list-item",innerHTML:`
            ...
            `}),p=c.querySelector(".w-script-info"),f=d.createElement("div",{className:"pops-panel-button",innerHTML:`
				...
				`});l.deleted&&(c.classList.add("w-script-deleted"),f.querySelector("button").setAttribute("disabled","true")),d.on(f,"click",async function(){u.success("同步",l);let m=f.querySelector("button"),y=f.querySelector("button span"),S=d.createElement("i",{className:"pops-bottom-icon",innerHTML:j.config.iconSVG.loading},{"is-loading":true});m.setAttribute("disabled","true"),m.setAttribute("data-icon","true"),y.innerText=r.t("同步中..."),d.before(y,S);let b=l?.id,I=await K.getSourceCodeSyncFormDataInfo(b.toString());if(I){const{formData:P,url:R}=I,L="script[script_sync_type_id]",z="script[sync_type]";if(P.has(L)||P.has(z)){let D="";if(P.has(L)){let N=P.get(L);N.toString()==="1"?D=r.t("手动"):N.toString()==="2"?D=r.t("自动"):N.toString()==="3"&&(D="webhook");}else P.has(z)&&(D=P.get(z));let T=c.querySelector(".w-script-sync-type");T?T.querySelector("p").innerText=r.t("同步方式：{{syncMode}}",{syncMode:D}):d.append(p,`
								<div class="w-script-sync-type">
									<p>${r.t("同步方式：{{syncMode}}",{syncMode:D})}
									</p>
								</div>`),await K.sourceCodeSync(l.id.toString(),P,R)?v.success(r.t("同步成功")):v.error(r.t("同步失败"));}else v.error(r.t("该脚本未设置同步信息"));}m.removeAttribute("disabled"),m.removeAttribute("data-icon"),y.innerText=r.t("同步代码"),S.remove();}),c.appendChild(f),e.appendChild(c);}}},ye={id:"greasy-fork-panel-config-account",title:r.t("通用"),forms:[{text:"",type:"forms",forms:[{text:r.t("Toast配置"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[ot(r.t("Toast位置"),"qmsg-config-position","bottom",[{value:"topleft",text:r.t("左上角")},{value:"top",text:r.t("顶部")},{value:"topright",text:r.t("右上角")},{value:"left",text:r.t("左边")},{value:"center",text:r.t("中间")},{value:"right",text:r.t("右边")},{value:"bottomleft",text:r.t("左下角")},{value:"bottom",text:r.t("底部")},{value:"bottomright",text:r.t("右下角")}],(t,e,A)=>{u.info("设置当前Qmsg弹出位置"+A);},r.t("Toast显示在页面九宫格的位置")),ot(r.t("最多显示的数量"),"qmsg-config-maxnums",3,[{value:1,text:"1"},{value:2,text:"2"},{value:3,text:"3"},{value:4,text:"4"},{value:5,text:"5"}],void 0,r.t("限制Toast显示的数量")),w(r.t("逆序弹出"),"qmsg-config-showreverse",false,void 0,r.t("修改Toast弹出的顺序"))]}]},ot(r.t("语言"),"setting-language","zh-CN",[{value:"zh-CN",text:"中文"},{value:"en-US",text:"English"}],(t,e,A)=>{u.info("改变语言："+A),r.changeLanguage(e);})]},{text:"",type:"forms",forms:[{text:r.t("账号/密码"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[ht(r.t("账号"),"user","",void 0,void 0,r.t("请输入账号")),ht(r.t("密码"),"pwd","",void 0,void 0,r.t("请输入密码"),false,true),ht(r.t("secret"),"secret","","两步验证（2FA）",void 0,r.t("请输入secret"),false,true)]},{text:"",type:"forms",forms:[w(r.t("自动登录"),"autoLogin",true,void 0,r.t("自动登录当前保存的账号")),at(r.t("清空账号/密码"),void 0,r.t("点击清空"),void 0,void 0,false,"default",t=>{if(confirm(r.t("确定清空账号和密码？"))){h.deleteValue("user"),h.deleteValue("pwd"),v.success(r.t("已清空账号/密码"));let e=t.target.getRootNode();e.querySelector('li[data-key="user"] .pops-panel-input input').value="",e.querySelector('li[data-key="pwd"] .pops-panel-input input').value="";}})]}]},{text:r.t("功能"),type:"deepMenu",forms:[{text:r.t("功能"),type:"forms",forms:[ot(r.t("固定当前语言"),"language-selector-locale","",function(){let t=[{value:"",text:r.t("无")}];return d.ready(()=>{O("select.language-selector-locale option").forEach(e=>{let A=e.getAttribute("value");if(A==="help")return;let n=d.text(e).trim();t.push({value:A,text:n});});}),t}()),w(r.t("修复图片宽度显示问题"),"fixImageWidth",true,void 0,r.t("修复图片在移动端宽度超出浏览器宽度问题")),w(r.t("优化图片浏览"),"optimizeImageBrowsing",true,void 0,r.t("使用Viewer浏览图片")),w(r.t("覆盖图床图片跳转"),"overlayBedImageClickEvent",true,void 0,r.t("配合上面的【优化图片浏览】更优雅浏览图片")),w(r.t("添加【操作面板】按钮"),"scripts-addOperationPanelBtnWithNavigator",true,void 0,r.t("在脚本列表页面时为顶部导航栏添加【操作面板】按钮")),w(r.t("给Markdown添加【复制】按钮"),"addMarkdownCopyButton",true,void 0,r.t("在Markdown内容右上角添加【复制】按钮，点击一键复制Markdown内容"))]},{text:r.t("检测页面加载"),type:"forms",forms:[w(r.t("启用"),"checkPage",true,void 0,r.t("检测Greasyfork页面是否正常加载，如加载失败则自动刷新页面")),ot(r.t("检测间隔"),"greasyfork-check-page-timeout",5,(()=>{let t=[];for(let e=0;e<5;e++)t.push({value:e+1,text:e+1+"s"});return t})(),void 0,r.t("设置检测上次刷新页面的间隔时间，当距离上次刷新页面的时间超过设置的值，将不再刷新页面"))]}]},{type:"deepMenu",text:r.t("表单"),forms:[{type:"forms",text:"",forms:[w(r.t("记住回复内容"),"rememberReplyContent",true,void 0,r.t("监听表单内的textarea内容改变并存储到indexDB中，提交表单将清除保存的数据，误刷新页面时可动态恢复")),ot(r.t("自动清理空间"),"gf-autoClearRememberReplayContent",7,[{text:r.t("不清理"),value:-1},{text:r.t("{{value}} 天",{value:1}),value:1},{text:r.t("{{value}} 周",{value:1}),value:7},{text:r.t("{{value}} 个月",{value:1}),value:30},{text:r.t("{{value}} 个月",{value:2}),value:60},{text:r.t("{{value}} 个月",{value:3}),value:90},{text:r.t("半年"),value:180}],void 0,r.t("根据设置的间隔时间自动清理保存的回复内容")),at(r.t("数据占用空间：{{size}}",{size:r.t("计算中")}),r.t("当前存储的数据所占用的空间大小"),r.t("清空"),void 0,void 0,void 0,"default",async()=>{await wt.clearAllRememberReplyContent()?v.success(r.t("清理成功")):v.error(r.t("清理失败"));},async(t,e)=>{let A=e.ulElement.querySelector('li[data-key="gf-autoClearRememberReplayContent"]+li .pops-panel-item-left-main-text'),n=await wt.getAllRememberReplyContent(),o="";n.length?o=g.getTextStorageSize(JSON.stringify(n)):o=g.getTextStorageSize(""),A.innerText=r.t("数据占用空间：{{size}}",{size:o});})]}]},{text:r.t("美化"),type:"deepMenu",forms:[{text:r.t("全局"),type:"forms",forms:[w(r.t("美化页面元素"),"beautifyPage",true,void 0,r.t("如button、input、textarea")),w(r.t("美化上传图片按钮"),"beautifyUploadImage",true,void 0,r.t("放大上传区域")),w(r.t("美化顶部导航栏"),"beautifyTopNavigationBar",true,void 0,r.t("可能会跟Greasyfork Beautify脚本有冲突")),w(r.t("美化Greasyfork Beautify脚本"),"beautifyGreasyforkBeautify",false,void 0,r.t('需安装Greasyfork Beautify脚本，<a href="https://greasyfork.org/zh-CN/scripts/446849-greasyfork-beautify" target="_blank">🖐点我安装</a>'))]},{type:"forms",text:r.t("脚本列表"),forms:[w(r.t("美化脚本列表"),"beautifyCenterContent",true,void 0,r.t("双列显示且添加脚本卡片操作项（安装、收藏）")),w("↑"+r.t("使用namespace查询脚本信息"),"beautifyCenterContent-queryNameSpace",true,void 0,r.t("开启后检测已安装的脚本信息更准确，但是速度会更慢"))]}]},{type:"deepMenu",text:r.t("自定义快捷键"),forms:[{type:"forms",text:"",forms:[me(r.t("快捷键发表回复"),r.t("在输入框内按下快捷发表回复，例如：{{key}}",{key:"Ctrl + Enter"}),"gf-quickReply",{keyName:"Enter",keyValue:"13",ohterCodeList:["ctrl"]},r.t("点击录入快捷键"),void 0,Bt.shortCut)]}]},{text:r.t("过滤"),type:"deepMenu",forms:[{text:`<a target="_blank" href="https://greasyfork.org/zh-CN/scripts/475722-greasyfork%E4%BC%98%E5%8C%96#:~:text=%E8%84%9A%E6%9C%AC%E8%BF%87%E6%BB%A4%E8%A7%84%E5%88%99">${r.t("帮助文档")}</a>`,type:"forms",forms:[w(r.t("启用"),"gf-scripts-filter-enable",true,void 0,r.t("作用域：脚本、脚本搜索、用户主页")),{type:"own",getLiElementCallBack(t){let e=d.createElement("div",{className:"pops-panel-textarea",innerHTML:`
												<textarea placeholder="${r.t("请输入规则，每行一个")}" style="height:200px;"></textarea>`},{style:"width: 100%;"}),A=e.querySelector("textarea");return A.value=W.getValue(),d.on(A,["input","propertychange"],void 0,g.debounce(function(n){W.setValue(A.value);},200)),t.appendChild(e),t}}]}]}]},{type:"forms",text:r.t("脚本管理"),forms:[{type:"deepMenu",text:r.t("代码同步"),forms:[{text:r.t("代码同步"),type:"forms",forms:[at(r.t("源代码同步【脚本列表】"),void 0,r.t("一键同步"),void 0,void 0,false,"primary",t=>{if(!k.isUsers()){h.setValue("goto_updateSettingsAndSynchronize_scriptList",true),E.getUserLinkElement()?(v.success(r.t("前往用户主页")),window.location.href=E.getUserLinkElement().href):v.error(r.t("获取当前已登录的用户主页失败"));return}let e=[];O("#user-script-list-section li a.script-link").forEach(A=>{e=e.concat(C.getAdminUrl(A.href));}),E.updateScript(e);}),at(r.t("源代码同步【未上架的脚本】"),void 0,r.t("一键同步"),void 0,void 0,false,"primary",t=>{if(!k.isUsers()){h.setValue("goto_updateSettingsAndSynchronize_unlistedScriptList",true),E.getUserLinkElement()?(v.success(r.t("前往用户主页")),window.location.href=E.getUserLinkElement().href):v.error(r.t("获取当前已登录的用户主页失败"));return}let e=[];O("#user-unlisted-script-list li a.script-link").forEach(A=>{e=e.concat(C.getAdminUrl(A.href));}),E.updateScript(e);}),at(r.t("源代码同步【库】"),void 0,r.t("一键同步"),void 0,void 0,false,"primary",t=>{if(!k.isUsers()){h.setValue("goto_updateSettingsAndSynchronize_libraryScriptList",true),E.getUserLinkElement()?(v.success(r.t("前往用户主页")),window.location.href=E.getUserLinkElement().href):v.error(r.t("获取当前已登录的用户主页失败"));return}let e=[];O("#user-library-script-list li a.script-link").forEach(A=>{e=e.concat(C.getAdminUrl(A.href));}),E.updateScript(e);})]}]},{type:"deepMenu",text:r.t("脚本列表"),forms:[],afterEnterDeepMenuCallBack(t,e){St.UIScriptList("script-list",e.sectionBodyContainer);}},{type:"deepMenu",text:r.t("库"),forms:[],afterEnterDeepMenuCallBack(t,e){St.UIScriptList("script-library",e.sectionBodyContainer);}}]}]},we={id:"greasy-fork-panel-config-scripts",title:r.t("脚本"),forms:[{text:"",type:"forms",forms:[{text:r.t("代码"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[w(r.t("添加复制代码按钮"),"addCopyCodeButton",true,void 0,r.t("更优雅的复制")),w(r.t("快捷键"),"fullScreenOptimization",true,void 0,r.t("【F】键全屏、【Alt+Shift+F】键宽屏")),w(r.t("修复代码行号显示"),"code-repairCodeLineNumber",false,void 0,r.t("修复代码行数超过1k行号显示不全问题")),w("monacoEditor","code-use-monaco-editor",true,void 0,r.t("使用Monaco编辑器"))]}]},{text:r.t("历史版本"),type:"deepMenu",forms:[{text:r.t("功能"),type:"forms",forms:[w(r.t("添加额外的标签按钮"),"scripts-versions-addExtraTagButton",true,void 0,r.t("在版本下面添加【安装】、【查看代码】按钮")),w(r.t("添加代码对比按钮"),"scripts-versions-addCompareCodeButton",true,void 0,r.t("monacoEditor"))]},{text:r.t("美化"),type:"forms",forms:[w(r.t("美化历史版本页面"),"beautifyHistoryVersionPage",true,void 0,r.t("更直观的查看版本迭代"))]}]}]},{text:"",type:"forms",forms:[{text:r.t("功能"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[w(r.t("添加【寻找引用】按钮"),"addFindReferenceButton",true,void 0,r.t("在脚本栏添加按钮，一般用于搜索引用该库的相关脚本")),w(r.t("添加【收藏】按钮"),"addCollectionButton",true,void 0,r.t("在脚本栏添加按钮，一般用于快捷收藏该脚本/库")),w(r.t("添加【今日检查】信息块"),"scriptHomepageAddedTodaySUpdate",true,void 0,r.t("在脚本信息栏添加【今日检查】信息块"))]}]}]}]},be={id:"greasy-fork-panel-config-script-search",title:r.t("搜索"),forms:[{type:"forms",text:"搜素结果过滤",forms:[w(r.t("新增【关键词】搜索框"),"gf-script-search-addFilterSearchInput",true,void 0,r.t("输入自定义关键词后自动执行过滤")),w(r.t("新增【{{buttonText}}】按钮",{buttonText:r.t("名称")}),"gf-script-search-filterScriptTitleWholeWordMatching",true,void 0,r.t("该Checkbox按钮开启后，自动过滤出包含搜索关键词的脚本")),w(r.t("新增【{{buttonText}}】按钮",{buttonText:r.t("描述")}),"gf-script-search-filterScriptDescWholeWordMatching",true,void 0,r.t("该Checkbox按钮开启后，自动过滤出包含搜索关键词的脚本")),w(r.t("新增【{{buttonText}}】按钮",{buttonText:r.t("名称/描述")}),"gf-script-search-filterScriptTitleOrDescWholeWordMatching",true,void 0,r.t("该Checkbox按钮开启后，自动过滤出包含搜索关键词的脚本")),w(r.t("新增【{{buttonText}}】按钮",{buttonText:r.t("作者名称")}),"gf-script-search-filterScriptAuthorNameWholeWordMatching",true,void 0,r.t("该Checkbox按钮开启后，自动过滤出包含搜索关键词的脚本"))]}]},xe={id:"greasy-fork-panel-config-discussions",title:r.t("论坛"),forms:[{text:"",type:"forms",forms:[{text:r.t("功能"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[{type:"own",attributes:{"data-key":"discussions-readBgColor","data-default-value":"#e5e5e5"},getLiElementCallBack(t){let e="discussions-readBgColor",A=d.createElement("div",{className:"pops-panel-item-left-text",innerHTML:`
											...
											`}),o=n.querySelector(".pops-color-choose");o.value=h.getValue(e);let a=d.createElement("style");return d.append(document.head,a),d.on(o,["input","propertychange"],i=>{u.info("选择颜色："+o.value),a.innerHTML=`
												.discussion-read{
													background: ${o.value} !important;
												}
												`,h.setValue(e,o.value);}),t.appendChild(A),t.appendChild(n),t}},w(r.t("添加【过滤】按钮"),"discussions-addShortcutOperationButton",true,void 0,r.t("在每一行讨论的最后面添加【过滤】按钮，需开启过滤功能才会生效")),w(r.t("添加【举报】按钮"),"discussions-addReportButton",true,void 0,r.t("在每一行讨论的最后面添加【举报】按钮"))]}]},{text:r.t("过滤"),type:"deepMenu",forms:[{text:`<a target="_blank" href="https://greasyfork.org/zh-CN/scripts/475722-greasyfork%E4%BC%98%E5%8C%96#:~:text=%E8%AE%BA%E5%9D%9B%E8%BF%87%E6%BB%A4%E8%A7%84%E5%88%99">${r.t("帮助文档")}</a>`,type:"forms",forms:[w(r.t("启用"),"greasyfork-discussions-filter-enable",true,void 0,r.t("开启后下面的过滤功能才会生效")),w(r.t("过滤重复的评论"),"greasyfork-discussions-filter-duplicate-comments",true,void 0,r.t("过滤掉重复的评论数量(≥2)")),{type:"own",getLiElementCallBack(t){let e=d.createElement("div",{className:"pops-panel-textarea",innerHTML:`
										<textarea placeholder="${r.t("请输入规则，每行一个")}" style="height:200px;"></textarea>`},{style:"width: 100%;"}),A=e.querySelector("textarea");return A.value=V.getValue(),d.on(A,["input","propertychange"],void 0,g.debounce(function(n){V.setValue(A.value);},200)),t.appendChild(e),t}}]}]}]}]},Le={id:"greasy-fork-panel-config-account",title:r.t("用户"),forms:[{text:"",type:"forms",forms:[{text:r.t("功能"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[w(r.t("迁移【控制台】到顶部导航栏"),"users-changeConsoleToTopNavigator",true,void 0,r.t("将【控制台】按钮移动到顶部导航栏，节省空间"))]}]},{text:r.t("美化"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[w(r.t("美化私信页面"),"conversations-beautifyDialogBox",true,void 0,r.t("美化为左右对话模式")),w(r.t("美化私信列表"),"conversations-beautifyPrivateMessageList",true)]}]}]}]},Pe=`.w-script-list-item {\r
	padding: 10px;\r
...
`;It.addMenuOption({key:"githubUrl2webhookUrl",text:"⚙ "+r.t("Url To WebhookUrl"),autoReload:false,isStoreValue:false,showText(t){return t},callback:()=>{he.showView();}});h.$data.panelConfig={style:Pe};Lt.addContentConfig([ye,we,be,xe,Le]);h.init();it.init();
