...
`;var ve=typeof GM_deleteValue<"u"?GM_deleteValue:void 0,ke=typeof GM_getResourceText<"u"?GM_getResourceText:void 0,ee=typeof GM_getValue<"u"?GM_getValue:void 0,ge=typeof GM_info<"u"?GM_info:void 0,ye=typeof GM_listValues<"u"?GM_listValues:void 0,We=typeof GM_registerMenuCommand<"u"?GM_registerMenuCommand:void 0,$=typeof GM_setValue<"u"?GM_setValue:void 0,Oe=typeof GM_setValues<"u"?GM_setValues:void 0,Ze=typeof GM_unregisterMenuCommand<"u"?GM_unregisterMenuCommand:void 0,Je=typeof GM_xmlhttpRequest<"u"?GM_xmlhttpRequest:void 0,R=typeof unsafeWindow<"u"?unsafeWindow:void 0,$e=window;const ce={qmsg_config_position:{key:"qmsg-config-position",defaultValue:"bottom"},qmsg_config_maxnums:{key:"qmsg-config-maxnums",defaultValue:3},qmsg_config_showreverse:{key:"qmsg-config-showreverse",defaultValue:false}},_={waitRemove(...e){e.forEach(t=>{typeof t=="string"&&j.waitNodeList(t).then(A=>{A.forEach(n=>n.remove());});});},createBlockCSSNode(...e){let t=[];if(e.length!==0&&!(e.length===1&&typeof e[0]=="string"&&e[0].trim()===""))return e.forEach(A=>{Array.isArray(A)?t=t.concat(A):t.push(A);}),j.createElement("style",{type:"text/css",innerHTML:`${t.join(`,
`)}{display: none !important;}`})},addBlockCSS(...e){let t=[];if(e.length!==0&&!(e.length===1&&typeof e[0]=="string"&&e[0].trim()===""))return e.forEach(A=>{Array.isArray(A)?t=t.concat(A):t.push(A);}),D(`${t.join(`,
`)}{display: none !important;}`)},setGMResourceCSS(e){const t=typeof ke=="function"?ke(e.keyName):null;return typeof t=="string"&&t?D(t):_.loadStyleLink(e.url)},async loadStyleLink(e){let t=document.createElement("link");return t.rel="stylesheet",t.type="text/css",t.href=e,new Promise(A=>{j.onReady(()=>{document.head.appendChild(t),A(t);});})},async loadScript(e){let t=document.createElement("script");return t.src=e,new Promise(A=>{t.onload=()=>{A(null);},(document.head||document.documentElement).appendChild(t);})},fixUrl(e){return e=e.trim(),e.startsWith("data:")||e.match(/^http(s|):\/\//i)?e:e.startsWith("//")?(e.startsWith("///")||(e=window.location.protocol+e),e):(e.startsWith("/")||(e+="/"),e=window.location.origin+e,e)},fixHttps(e){if(e.startsWith("https://")||!e.startsWith("http://"))return e;try{let t=new URL(e);return t.protocol="https:",t.toString()}catch{return e}},lockScroll(...e){let t=document.createElement("style");t.innerHTML=`
			.pops-overflow-hidden-important {
				overflow: hidden !important;
			}
		`;let A=[document.documentElement,document.body].concat(...e||[]);return A.forEach(n=>{n.classList.add("pops-overflow-hidden-important");}),(document.head||document.documentElement).appendChild(t),{recovery(){A.forEach(n=>{n.classList.remove("pops-overflow-hidden-important");}),t.remove();}}},async getClipboardText(){function e(n){navigator.clipboard.readText().then(i=>{n(i);}).catch(i=>{p.error("读取剪贴板内容失败👉",i),n("");});}function t(n){navigator.permissions.query({name:"clipboard-read"}).then(i=>{e(n);}).catch(i=>{p.error("申请剪贴板权限失败，尝试直接读取👉",i.message??i.name??i.stack),e(n);});}function A(){return !(typeof navigator?.clipboard?.readText!="function"||typeof navigator?.permissions?.query!="function")}return new Promise(n=>{if(!A()){n("");return}document.hasFocus()?t(n):window.addEventListener("focus",()=>{t(n);},{once:true});})},escapeHtml(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;").replace(/©/g,"&copy;").replace(/®/g,"&reg;").replace(/™/g,"&trade;").replace(/→/g,"&rarr;").replace(/←/g,"&larr;").replace(/↑/g,"&uarr;").replace(/↓/g,"&darr;").replace(/—/g,"&mdash;").replace(/–/g,"&ndash;").replace(/…/g,"&hellip;").replace(/ /g,"&nbsp;").replace(/\r\n/g,"<br>").replace(/\r/g,"<br>").replace(/\n/g,"<br>").replace(/\t/g,"&nbsp;&nbsp;&nbsp;&nbsp;")},interval(e,t,A=5e3){let n,i=A-t,s=t,o=async a=>{const l=await e(a);if(typeof l=="boolean"&&l||a){h.workerClearTimeout(n);return}if(s+=t,s>i){o(true);return}n=h.workerSetTimeout(()=>{o(false);},t);};o(false);},findParentNode(e,t,A){if(A){let n=j.closest(e,A);if(n)return n.querySelector(t)}else return j.matches(e,t)?e:j.closest(e,t)},toStr(e){const t="__undefined__placeholder__replaced__str__";return JSON.stringify(e,(n,i)=>i===void 0?t:i,2).replace(new RegExp(`"${t}"`,"g"),"undefined")}},h=se.noConflict(),c=j.noConflict(),Q=Ce,p=new h.Log(ge,R.console||$e.console),be=ge?.script?.name||void 0,_e=Ce.config.Utils.AnyTouch(),et=false;p.config({debug:false,logMaxCount:250,autoClearConsole:true,tag:true});g.config({isHTML:true,autoClose:true,showClose:false,consoleLogContent(e){const t=e.setting.type;if(t==="loading")return  false;const A=e.setting.content;return t==="warning"?p.warn(A):t==="error"?p.error(A):p.info(A),true},get position(){return m.getValue(ce.qmsg_config_position.key,ce.qmsg_config_position.defaultValue)},get maxNums(){return m.getValue(ce.qmsg_config_maxnums.key,ce.qmsg_config_maxnums.defaultValue)},get showReverse(){return m.getValue(ce.qmsg_config_showreverse.key,ce.qmsg_config_showreverse.defaultValue)},get zIndex(){let e=se.getMaxZIndex(),t=Ce.config.InstanceUtils.getPopsMaxZIndex().zIndex;return se.getMaxValue(e,t)+100}});Q.GlobalConfig.setGlobalConfig({zIndex:()=>{const e=se.getMaxZIndex(void 0,void 0,A=>{if(A?.classList?.contains("qmsg-shadow-container")||A?.closest("qmsg")&&A.getRootNode()instanceof ShadowRoot)return  false}),t=Ce.config.InstanceUtils.getPopsMaxZIndex().zIndex;return se.getMaxValue(e,t)+100},mask:{enable:true,clickEvent:{toClose:false,toHide:false}},drag:true});const Ne=new h.GM_Menu({GM_getValue:ee,GM_setValue:$,GM_registerMenuCommand:We,GM_unregisterMenuCommand:Ze}),V=new h.Httpx({xmlHttpRequest:Je,logDetails:et});V.interceptors.request.use(e=>e);V.interceptors.response.use(void 0,e=>(p.error("拦截器-请求错误",e),e.type==="onabort"?g.warning("请求取消",{consoleLogContent:true}):e.type==="onerror"?g.error("请求异常",{consoleLogContent:true}):e.type==="ontimeout"?g.error("请求超时",{consoleLogContent:true}):g.error("其它错误",{consoleLogContent:true}),e));R.Object.defineProperty,R.Function.prototype.apply,R.Function.prototype.call,R.Element.prototype.appendChild,R.setTimeout.bind(R),R.clearTimeout.bind(R),R.setInterval.bind(R),R.clearInterval.bind(R);const D=c.addStyle.bind(c),N=j.selector.bind(j),X=j.selectorAll.bind(j);new h.GM_Cookie;const he="GM_Panel",Le="data-init",pe="data-key",fe="data-default-value",Qe="data-init-more-value",Ue="data-plugin-search-config",ne="data-storage-api",J={get width(){return globalThis.innerWidth},get height(){return globalThis.innerHeight}},Z={setting:{get width(){return J.width<550?"88vw":J.width<700?"550px":"700px"},get height(){return J.height<450?"70vh":J.height<550?"450px":"550px"}},settingMiddle:{get width(){return J.width<350?"88vw":"350px"},get height(){return J.height<450?"88vh":"450px"}},settingBig:{get width(){return J.width<800?"92vw":"800px"},get height(){return J.height<600?"80vh":"600px"}},info:{get width(){return J.width<350?"88vw":"350px"},get height(){return J.height<250?"88vh":"250px"}}},xe={$data:{__contentConfig:null,get contentConfig(){return this.__contentConfig==null&&(this.__contentConfig=new h.Dictionary),this.__contentConfig},__defaultBottomContentConfig:[]},addContentConfig(e){Array.isArray(e)||(e=[e]);let t=this.$data.contentConfig.keys().length;this.$data.contentConfig.set(t,e);},getAllContentConfig(){return this.$data.contentConfig.values().flat()},getConfig(e=0){return this.$data.contentConfig.get(e)??[]},getDefaultBottomContentConfig(){if(this.$data.__defaultBottomContentConfig.length)return this.$data.__defaultBottomContentConfig;let e=false,t;const A=(s,o)=>{typeof o!="string"&&(o=_.toStr(o));const a=new Blob([o]),l=globalThis.URL.createObjectURL(a);c.createElement("a",{href:l,download:s}).click(),h.workerSetTimeout(()=>{globalThis.URL.revokeObjectURL(l);},500);},n=()=>{const s=f=>{const v=Q.alert({title:{text:"请选择导入方式",position:"center"},content:{text:`
            <div class="btn-control" data-mode="local">本地导入</div>
            <div class="btn-control" data-mode="network">网络导入</div>
            <div class="btn-control" data-mode="clipboard">剪贴板导入</div>`,html:true},btn:{ok:{enable:false},close:{enable:true,callback(T,z){T.close();}}},drag:true,mask:{enable:true},width:Z.info.width,height:Z.info.height,style:`
          .btn-control{
          ...
          }`}),y=v.$shadowRoot.querySelector(".btn-control[data-mode='local']"),x=v.$shadowRoot.querySelector(".btn-control[data-mode='network']"),P=v.$shadowRoot.querySelector(".btn-control[data-mode='clipboard']"),I=async T=>{confirm("是否清空脚本存储的配置？（如果点击取消按钮，则仅做配置覆盖处理）")&&(typeof ye=="function"?typeof ve=="function"?(ye().forEach(w=>{ve(w);}),g.success("已清空脚本存储的配置")):g.error("不支持GM_deleteValue函数，无法执行删除脚本配置"):g.error("不支持GM_listValues函数，无法清空脚本存储的配置")),typeof Oe=="function"?Oe(T):Object.keys(T).forEach(w=>{const b=T[w];$(w,b);}),g.success("配置导入完毕");},M=T=>new Promise(async z=>{const k=h.toJSON(T);Object.keys(k).length===0?g.warning("解析为空配置，不导入"):await I(k),z(true);});c.on(y,"click",T=>{c.preventEvent(T),v.close();const z=c.createElement("input",{type:"file",accept:".json"});c.on(z,["propertychange","input"],k=>{if(!z.files?.length)return;const w=z.files[0],b=new FileReader;b.onload=()=>{M(b.result);},b.readAsText(w,"UTF-8");}),z.click();}),c.on(x,"click",T=>{c.preventEvent(T),v.close();const z=Q.prompt({title:{text:"网络导入",position:"center"},content:{text:"",placeholder:"请填写URL",focus:true},btn:{close:{enable:true,callback(b,U){b.close();}},ok:{text:"导入",callback:async(b,U)=>{const B=b.text;if(h.isNull(B)){g.error("请填入完整的url");return}const C=g.loading("正在获取配置..."),E=await V.get(B,{allowInterceptConfig:false});if(C.close(),!E.status){p.error(E),g.error("获取配置失败",{consoleLogContent:true});return}await M(E.data.responseText)&&b.close();}},cancel:{enable:false}},drag:true,mask:{enable:true},width:Z.info.width,height:"auto"}),k=z.$shadowRoot.querySelector("input"),w=z.$shadowRoot.querySelector(".pops-prompt-btn-ok");c.on(k,["input","propertychange"],b=>{c.val(k)===""?c.attr(w,"disabled","true"):c.removeAttr(w,"disabled");}),c.onKeyboard(k,"keydown",(b,U,B)=>{b==="Enter"&&B.length===0&&c.val(k)!==""&&c.emit(w,"click");}),c.emit(k,"input");}),c.on(P,"click",async T=>{c.preventEvent(T),v.close();let z=await _.getClipboardText();if(z.trim()===""){g.warning("获取到的剪贴板内容为空");return}await M(z);});},o=(f=`${be}_panel-setting-${h.formatTime(Date.now(),"yyyy_MM_dd_HH_mm_ss")}.json`,v)=>{const y=Q.alert({title:{text:"请选择导出方式",position:"center"},content:{text:`
            <div class="btn-control" data-mode="export-to-file">导出至文件</div>
            <div class="btn-control" data-mode="export-to-clipboard">导出至剪贴板</div>
            `,html:true},btn:{ok:{enable:false},close:{enable:true,callback(I,M){I.close();}}},drag:true,mask:{enable:true},width:Z.info.width,height:Z.info.height,style:`
          .btn-control{
              display: inline-block;
              margin: 10px;
              padding: 10px;
          ...
          .btn-control:hover{
            ...
            border-color: #c6e2ff;
            background-color: #ecf5ff;
          }`}),x=y.$shadowRoot.querySelector(".btn-control[data-mode='export-to-file']"),P=y.$shadowRoot.querySelector(".btn-control[data-mode='export-to-clipboard']");c.on(x,"click",I=>{c.preventEvent(I);try{A(f,v),y.close();}catch(M){g.error(M.toString(),{consoleLogContent:true});}}),c.on(P,"click",async I=>{await h.copy(v)?(g.success("复制成功"),y.close()):g.error("复制失败");});},l=Q.confirm({title:{text:"配置",position:"center"},content:{text:`
            <textarea name="config-value" id="config" readonly></textarea>
          `,html:true},btn:{ok:{enable:true,type:"primary",text:"导入",callback(f,v){s();}},cancel:{enable:true,text:"导出",callback(f,v){o(void 0,u);}}},width:J.width<450?"90vw":"450px",height:"auto",style:`
        ...
        `}).$shadowRoot.querySelector("textarea"),d={};if(typeof ye=="function")ye().forEach(v=>{const y=ee(v);Reflect.set(d,v,y);});else {g.warning("不支持函数GM_listValues，仅导出菜单配置");const f=ee(he);Reflect.set(d,he,f);}const u=_.toStr(d);l.value=u;},i=()=>{let s=ge?.script?.supportURL||ge?.script?.namespace;typeof s=="string"&&h.isNotNull(s)&&window.open(s,"_blank");};return [{id:"script-version",title:`版本：${ge?.script?.version||"未知"}`,isBottom:true,views:[],clickFirstCallback(){return  false},afterRender(s){new _e(s.$asideLiElement).on("tap",function(a){clearTimeout(t),t=void 0,e?(e=false,n()):(t=setTimeout(()=>{e=false,i();},200),e=true);});}}]},setDefaultBottomContentConfig(e){this.$data.__defaultBottomContentConfig=e;}},Ke={$data:{__menuOption:[{key:"show_pops_panel_setting",text:"⚙ 设置",autoReload:false,isStoreValue:false,showText(e){return e},callback:()=>{m.showPanel(xe.getConfig(0));}}],get menuOption(){return this.__menuOption}},init(){this.initExtensionsMenu();},initExtensionsMenu(){m.isTopWindow()&&Ne.add(this.$data.menuOption);},addMenuOption(e){Array.isArray(e)||(e=[e]),this.$data.menuOption.push(...e);},updateMenuOption(e){Array.isArray(e)||(e=[e]),e.forEach(t=>{let A=this.$data.menuOption.findIndex(n=>n.key===t.key);A!==-1&&(this.$data.menuOption[A]=t);});},getMenuOption(e=0){return this.$data.menuOption[e]},deleteMenuOption(e=0){this.$data.menuOption.splice(e,1);}};class tt{storageKey;listenerData;constructor(t){if(typeof t=="string"){const A=t.trim();if(A=="")throw new Error("key参数不能为空字符串");this.storageKey=A;}else throw new Error("key参数类型错误，必须是字符串");this.listenerData=new se.Dictionary,this.getLocalValue=this.getLocalValue.bind(this),this.set=this.set.bind(this),this.get=this.get.bind(this),this.getAll=this.getAll.bind(this),this.delete=this.delete.bind(this),this.has=this.has.bind(this),this.keys=this.keys.bind(this),this.values=this.values.bind(this),this.clear=this.clear.bind(this),this.addValueChangeListener=this.addValueChangeListener.bind(this),this.removeValueChangeListener=this.removeValueChangeListener.bind(this),this.emitValueChangeListener=this.emitValueChangeListener.bind(this);}getLocalValue(){let t=ee(this.storageKey);return t==null&&(t={},this.setLocalValue(t)),t}setLocalValue(t){$(this.storageKey,t);}set(t,A){const n=this.get(t),i=this.getLocalValue();Reflect.set(i,t,A),this.setLocalValue(i),this.emitValueChangeListener(t,A,n);}get(t,A){const n=this.getLocalValue();return Reflect.get(n,t)??A}getAll(){return this.getLocalValue()}delete(t){const A=this.get(t),n=this.getLocalValue();Reflect.deleteProperty(n,t),this.setLocalValue(n),this.emitValueChangeListener(t,void 0,A);}has(t){const A=this.getLocalValue();return Reflect.has(A,t)}keys(){const t=this.getLocalValue();return Reflect.ownKeys(t)}values(){const t=this.getLocalValue();return Reflect.ownKeys(t).map(A=>Reflect.get(t,A))}clear(){ve(this.storageKey);}addValueChangeListener(t,A){const n=Math.random(),i=this.listenerData.get(t)||[];return i.push({id:n,key:t,callback:A}),this.listenerData.set(t,i),n}removeValueChangeListener(t){let A=false;for(const[n,i]of this.listenerData.entries()){for(let s=0;s<i.length;s++){const o=i[s];(typeof t=="string"&&o.key===t||typeof t=="number"&&o.id===t)&&(i.splice(s,1),s--,A=true);}this.listenerData.set(n,i);}return A}async emitValueChangeListener(...t){const[A,n,i]=t;if(!this.listenerData.has(A))return;let s=this.listenerData.get(A);for(let o=0;o<s.length;o++){const a=s[o];if(typeof a.callback=="function"){let l=this.get(A),d,u;typeof i<"u"&&t.length>=2?u=i:u=l,typeof n<"u"&&t.length>2?d=n:d=l,await a.callback(A,d,u);}}}}const ie=new tt(he),m={$data:{__contentConfigInitDefaultValue:null,__onceExecMenuData:null,__urlChangeReloadMenuExecOnce:null,__onceExecData:null,__panelConfig:{},$panel:null,panelContent:[],get contentConfigInitDefaultValue(){return this.__contentConfigInitDefaultValue==null&&(this.__contentConfigInitDefaultValue=new h.Dictionary),this.__contentConfigInitDefaultValue},contentConfigInitDisabledKeys:[],get onceExecMenuData(){return this.__onceExecMenuData==null&&(this.__onceExecMenuData=new h.Dictionary),this.__onceExecMenuData},get urlChangeReloadMenuExecOnce(){return this.__urlChangeReloadMenuExecOnce==null&&(this.__urlChangeReloadMenuExecOnce=new h.Dictionary),this.__urlChangeReloadMenuExecOnce},get onceExecData(){return this.__onceExecData==null&&(this.__onceExecData=new h.Dictionary),this.__onceExecData},get scriptName(){return be},get panelConfig(){return this.__panelConfig},set panelConfig(e){this.__panelConfig=e;},key:he,attributeKeyName:pe,attributeDefaultValueName:fe},init(){this.initContentDefaultValue(),Ke.init();},isTopWindow(){return R.top===R.self},initContentDefaultValue(){const e=n=>{if(!n.attributes||n.type==="button"||n.type==="container"||n.type==="deepMenu")return;const i=n.attributes;let s=i[Le];if(typeof s=="function"){let d=s();if(typeof d=="boolean"&&!d)return}let o=new Map,a=i[pe];if(a!=null){const d=i[fe];o.set(a,d);}let l=i[Qe];if(typeof l=="object"&&l&&Object.keys(l).forEach(d=>{const u=l[d];o.set(d,u);}),!o.size){p.warn("请先配置键",n);return}if(n.type==="switch"){let d=typeof n.disabled=="function"?n.disabled():n.disabled;typeof d=="boolean"&&d&&this.$data.contentConfigInitDisabledKeys.push(...o.keys());}for(const[d,u]of o.entries())this.setDefaultValue(d,u);},t=n=>{for(let i=0;i<n.length;i++){let s=n[i];e(s);let o=s.views;o&&Array.isArray(o)&&t(o);}},A=[...xe.getAllContentConfig()];for(let n=0;n<A.length;n++){let i=A[n];if(!i.views)continue;const s=i.views;s&&Array.isArray(s)&&t(s);}this.$data.contentConfigInitDisabledKeys=[...new Set(this.$data.contentConfigInitDisabledKeys)];},setDefaultValue(e,t){this.$data.contentConfigInitDefaultValue.has(e)&&p.warn("请检查该key(已存在): "+e),this.$data.contentConfigInitDefaultValue.set(e,t);},getDefaultValue(e){return this.$data.contentConfigInitDefaultValue.get(e)},setValue(e,t){ie.set(e,t);},getValue(e,t){const A=ie.get(e);return A??(this.$data.contentConfigInitDefaultValue.has(e)?this.$data.contentConfigInitDefaultValue.get(e):t)},deleteValue(e){ie.delete(e);},hasKey(e){return ie.has(e)},addValueChangeListener(e,t){return ie.addValueChangeListener(e,t)},removeValueChangeListener(e){ie.removeValueChangeListener(e);},emitMenuValueChange(e,t,A){ie.emitValueChangeListener(e,t,A);},async exec(e,t,A,n=true){const i=this;let s;typeof e=="string"||Array.isArray(e)?s=()=>e:s=e;let o=false;const a=s();let l=[];Array.isArray(a)?(o=true,l=a):l.push(a);const d=l.find(w=>!this.$data.contentConfigInitDefaultValue.has(w));if(d){p.warn(`${d} 键不存在`);return}const u=JSON.stringify(l);if(n&&this.$data.onceExecMenuData.has(u))return this.$data.onceExecMenuData.get(u);let f=[];const v=[];let y=[];const x=(w,b)=>{let U=[],B=[],C=[];if(Array.isArray(b))C=C.concat(b);else {const E=L=>{if(typeof L=="object"&&L!=null)if(L instanceof Element)C.push(L);else {const{$css:H,destory:G}=L;H!=null&&(Array.isArray(H)?C=C.concat(H):C.push(H)),typeof G=="function"&&C.push(G);}else C.push(L);};if(b!=null&&Array.isArray(b))for(const L of b)E(L);else E(b);}for(const E of C)if(E!=null){if(E instanceof Element){U.push(E);continue}if(typeof E=="function"){y.push(E);continue}}w?(f=f.concat(U),y=y.concat(B)):(I(),M());},P=w=>!!this.getValue(w),I=()=>{for(let w=0;w<f.length;w++)f[w]?.remove(),f.splice(w,1),w--;},M=()=>{for(let w=0;w<y.length;w++){const b=y[w];b(),y.splice(w,1),w--;}},T=()=>{let w=false;return typeof A=="function"?w=A(l):w=l.every(b=>P(b)),w},z=async w=>{if(T()){const U=l.map(C=>this.getValue(C)),B=await t({value:o?U:U[0],addStoreValue:(...C)=>x(true,C)});x(true,B);}else x(false,[]);};n&&l.forEach(w=>{const b=this.addValueChangeListener(w,(U,B,C)=>z());v.push(b);}),await z();const k={reload(){this.clearStoreStyleElements(),this.destory(),z();},clear(){this.clearStoreStyleElements(),this.destory(),this.removeValueChangeListener(),this.clearOnceExecMenuData();},clearStoreStyleElements:()=>I(),destory(){return M()},removeValueChangeListener:()=>{v.forEach(w=>{this.removeValueChangeListener(w);});},clearOnceExecMenuData(){n&&i.$data.onceExecMenuData.delete(u);}};return this.$data.onceExecMenuData.set(u,k),k},async execMenu(e,t,A=false,n=false){return await this.exec(e,async i=>await t(i),i=>i.every(o=>{let a=!!this.getValue(o);return m.$data.contentConfigInitDisabledKeys.includes(o)&&(a=false,p.warn(`.execMenu${n?"Once":""} ${o} 被禁用`)),A&&(a=!a),a}),n)},async execMenuOnce(e,t,A=false,n=false){const i=await this.execMenu(e,t,A,true);if(n&&i){const s=()=>{i.reload();};this.removeUrlChangeWithExecMenuOnceListener(e),this.addUrlChangeWithExecMenuOnceListener(e,s);}return i},deleteExecMenuOnce(e){return e=this.transformKey(e),this.$data.onceExecMenuData.delete(e),this.$data.urlChangeReloadMenuExecOnce.delete(e),ie.removeValueChangeListener(e)},onceExec(e,t){if(e=this.transformKey(e),typeof e!="string")throw new TypeError("key 必须是字符串");this.$data.onceExecData.has(e)||(t(),this.$data.onceExecData.set(e,1));},deleteOnceExec(e){e=this.transformKey(e),this.$data.onceExecData.delete(e);},addUrlChangeWithExecMenuOnceListener(e,t){e=this.transformKey(e),this.$data.urlChangeReloadMenuExecOnce.set(e,t);},removeUrlChangeWithExecMenuOnceListener(e){e=this.transformKey(e),this.$data.urlChangeReloadMenuExecOnce.delete(e);},hasUrlChangeWithExecMenuOnceListener(e){return e=this.transformKey(e),this.$data.urlChangeReloadMenuExecOnce.has(e)},async emitUrlChangeWithExecMenuOnceEvent(e){const t=this.$data.urlChangeReloadMenuExecOnce.values();for(const A of t)await A(e);},showPanel(e,t=`${be}-设置`,A=false,n=false){this.$data.$panel=null,this.$data.panelContent=[];const i=e.findIndex(o=>(typeof o.isBottom=="function"?o.isBottom():!!o.isBottom)&&o.id==="script-version")!==-1;!A&&!i&&e.push(...xe.getDefaultBottomContentConfig());const s=Q.panel({title:{text:t,position:"center",html:false,style:""},content:e,btn:{close:{enable:true,callback:(o,a)=>{o.close(),this.$data.$panel=null;}}},mask:{enable:true,clickEvent:{toClose:true,toHide:false},clickCallBack:(o,a)=>{o(),this.$data.$panel=null;}},width:Z.setting.width,height:Z.setting.height,drag:true,only:true,style:`
        ...
        `,...this.$data.panelConfig});this.$data.$panel=s,this.$data.panelContent=e,n||this.registerConfigSearch({$panel:s,content:e});},registerConfigSearch(e){const{$panel:t,content:A}=e,n=async(v,y)=>{if(v==null)return;const x=await y(v);return x&&typeof x.isFind=="boolean"&&x.isFind?x.data:await n(x.data,y)},i=(v,y)=>{const x=new IntersectionObserver(P=>{P.forEach(I=>{I.isIntersecting&&(y?.(),x.disconnect());});},{root:null,threshold:1});x.observe(v),v.scrollIntoView({behavior:"smooth",block:"center"});},s=v=>{const y="pops-flashing";c.onAnimationend(v,()=>{v.classList.remove(y);}),v.classList.add(y);},o=v=>{if(v.type==="dblclick"&&f)return;c.preventEvent(v),l=null;const y=Q.alert({title:{text:"搜索配置",position:"center"},content:{text:`
					...
					${e.searchDialogStyle??""}
				`});y.$shadowRoot.querySelector(".search-wrapper");const x=y.$shadowRoot.querySelector(".search-config-text"),P=y.$shadowRoot.querySelector(".search-result-wrapper");x.focus();const I=()=>{c.empty(P);},M=z=>{const k=h.queryProperty(z,U=>U?.next?{isFind:false,data:U.next}:{isFind:true,data:U}),w=c.createElement("div",{className:"search-result-item",innerHTML:`
						...
						`}),b=Q.config.PanelHandlerComponents();return c.on(w,"click",U=>{const C=t.$shadowRoot.querySelectorAll("aside.pops-panel-aside .pops-panel-aside-top-container li")[z.index];if(!C){g.error(`左侧项下标${z.index}不存在`);return}C.scrollIntoView({behavior:"smooth",block:"center"}),C.click(),n(z.next,async E=>{if(E?.next){const L=await c.waitNode(()=>Array.from(t.$shadowRoot.querySelectorAll(".pops-panel-deepMenu-nav-item")).find(H=>{const G=Reflect.get(H,b.$data.nodeStoreConfigKey);return typeof G=="object"&&G!=null&&G.text===E.name}),2500);if(L)L.click();else return g.error("未找到对应的二级菜单"),{isFind:true,data:E};return {isFind:false,data:E.next}}else {const L=await c.waitNode(()=>Array.from(t.$shadowRoot.querySelectorAll("li:not(.pops-panel-deepMenu-nav-item)")).find(H=>Reflect.get(H,b.$data.nodeStoreConfigKey)===E.matchedData?.formConfig),2500);if(L){i(L);const H=L.closest(".pops-panel-forms-fold[data-fold-enable]");H&&(H.querySelector(".pops-panel-forms-fold-container").click(),await h.sleep(500)),i(L,()=>{s(L);});}else g.error("未找到对应的菜单项");return {isFind:true,data:E}}});}),w},T=z=>{const k=new RegExp(z,"i"),w=[],b=(B,C)=>{for(let E=0;E<B.length;E++){const L=B[E],H=L.views;if(H&&Array.isArray(H)){const G=h.deepClone(C);if(L.type==="deepMenu"){const Ae=h.queryProperty(G,W=>W?.next?{isFind:false,data:W.next}:{isFind:true,data:W});Ae.next={name:L.text};}b(H,G);}else {let G,Ae;if(L.type==="own"){const Y=Reflect.get(L.attributes||{},Ue);Y&&(typeof Y.text=="string"&&(G=Y.text),typeof Y.desc=="string"&&(Ae=Y.desc));}else G=L.text,Ae=Reflect.get(L,"description");const W=[G,Ae],re=W.findIndex(Y=>{if(typeof Y=="string")return Y.match(k)});if(re!==-1){const Y=h.deepClone(C),me=h.queryProperty(Y,oe=>oe?.next?{isFind:false,data:oe.next}:{isFind:true,data:oe});me.next={name:G,matchedData:{path:"",formConfig:L,matchedText:W[re],description:Ae}};const Ie=[];h.queryProperty(Y,oe=>{const ze=oe?.name;return typeof ze=="string"&&ze.trim()!==""&&Ie.push(ze),oe?.next?{isFind:false,data:oe.next}:{isFind:true,data:oe}});const He=Ie.join(_.escapeHtml(" - "));me.next.matchedData.path=He,w.push(Y);}}}};for(let B=0;B<A.length;B++){const C=A[B];if(!C.views||C.isBottom&&C.id==="script-version")continue;const E=C.views;if(E&&Array.isArray(E)){let L=C.title;typeof L=="function"&&(L=L()),b(E,{index:B,name:L});}}const U=document.createDocumentFragment();for(const B of w){let C=M(B);U.appendChild(C);}I(),P.append(U);};c.on(x,"input",h.debounce(z=>{c.preventEvent(z);let k=c.val(x).trim();if(k===""){I();return}T(k);},200));};t.$shadowRoot.querySelectorAll("aside.pops-panel-aside .pops-panel-aside-item:not(#script-version)").forEach(v=>{c.on(v,"dblclick",o);});let l=null,d=false,u,f=false;c.on(t.$shadowRoot,"touchend","aside.pops-panel-aside .pops-panel-aside-item:not(#script-version)",(v,y)=>{f=true,clearTimeout(u),u=void 0,d&&l===y?(d=false,l=null,o(v)):(u=setTimeout(()=>{d=false;},200),d=true,l=y);},{capture:true}),t.$shadowRoot.appendChild(c.createElement("style",{type:"text/css",textContent:`
					.pops-flashing{
            ...
            `}),t=e.$shadowRoot.querySelector("#github"),A=e.$shadowRoot.querySelector("#webhook");c.on(t,["input","propertychange"],n=>{let i=t.value.trim(),s=[];i.split(`
`).forEach(o=>{try{if(o=o.trim(),h.isNull(o))return;let l=new URL(o).pathname.split("/"),{1:d,2:u,3:f,4:v}=l,y="";if(l.length>=6&&f==="blob")y=l.slice(5,l.length).join("/");else if(l.length>=8&&f==="raw"&&v==="refs")v=l[6],y=l.slice(7,l.length).join("/");else return;if(y==="")return;s.push(`https://raw.githubusercontent.com/${d}/${u}/${v}/${y}`);}catch{}}),A.value=s.join(`
`);});}},O={getInstallUrl(e,t,A){return h.isNotNull(A)?A="/"+A:A="",`https://update.${window.location.hostname}/scripts/${e}/${t}${A}.user.js`},getCodeUrl(e,t){return h.isNull(t)&&(t=""),`/scripts/${e}/code?version=${t}`},getCodeSearchUrl(e){return "/zh-CN/scripts/code-search?c="+encodeURIComponent(e)},getScriptInfoUrl(e){return `/scripts/${e}.json`},getScriptHomeUrl(e){return `/scripts/${e}`},getAdminUrl(e){return e+"/admin"},getScriptId(e=window.location.pathname){return e.match(/\/scripts\/([\d]+)/i)?.[1]},getUserId(e=window.location.pathname){return e.match(/\/users\/([\d]+)/i)?.[1]},getSetsId(e=window.location.pathname){const t=e.match(/\/sets\/([\d]+)\//)?.[1],A=e.match(/\?set=([\d]+)/)?.[1];return t??A},getReportUrl(e,t){return `${window.location.origin}/reports/new?item_class=${e}&item_id=${t}`},getScriptName(e){let t=window.location.pathname;e!=null&&(t=new URL(e).pathname),t=decodeURIComponent(t);const A=t.split("/");for(const n of A){const i=n.match(/[\d]+/);if(i&&i.length)return i[1]}},getSwitchLanguageUrl(e="zh-CN"){let t=window.location.origin;const A=window.location.pathname.split("/");return A[1]=e,t=t+A.join("/"),t+=window.location.search,window.location.search===""?t+="?locale_override=1":window.location.search.includes("locale_override=1")||(t+="&locale_override=1"),t}},le={getCurrentLoginUserId(){const e=N("#nav-user-info .user-profile-link a");if(!e)return;const t=O.getUserId(e.href);if(t!=null)return t},parseScriptListInfo(e){const t=e.dataset,A={scriptId:parseInt(t.scriptId),scriptName:t.scriptName,scriptAuthors:[],scriptDailyInstalls:parseInt(t.scriptDailyInstalls),scriptTotalInstalls:parseInt(t.scriptTotalInstalls),scriptRatingScore:parseFloat(t.scriptRatingScore),scriptCreatedDate:new Date(t.scriptCreatedDate),scriptUpdatedDate:new Date(t.scriptUpdatedDate),scriptType:t.scriptType,scriptVersion:t.scriptVersion,sensitive:t.sensitive==="true",scriptLanguage:t.scriptLanguage,cssAvailableAsJs:t.cssAvailableAsJs==="true",codeUrl:t.codeUrl,scriptDescription:t.scriptDescription,scriptAuthorId:parseInt(t.scriptAuthorId),scriptAuthorName:t.scriptAuthorName},n=h.toJSON(t.scriptAuthors);if(Object.keys(n).forEach(i=>{const s=n[i];A.scriptAuthors.push({authorId:parseInt(i),authorName:s});}),(A.scriptAuthorName==null||isNaN(A.scriptAuthorId))&&A.scriptAuthors.length&&(A.scriptAuthorName=A.scriptAuthors[0].authorName,A.scriptAuthorId=A.scriptAuthors[0].authorId),A.scriptDescription==null){let i=e.querySelector(".script-description")||e.querySelector(".description");i&&(A.scriptDescription=i.innerText||i.textContent);}return A},registerTopNavMenu(e){c.onReady(()=>{const t=N("#site-nav nav"),A=N("#site-nav .with-submenu nav");if(!t){p.error("元素#site-nav nav不存在");return}const n=c.createElement("li",{className:e.className,innerHTML:`<a href="javascript:;">${e.name}</a>`});c.on(n,"click",a=>{c.preventEvent(a),e.clickEvent(a);}),A&&A.children.length?A.appendChild(n):t.appendChild(n);const i=c.createElement("li",{className:e.className,innerHTML:`<a href="javascript:;">${e.name}</a>`});c.on(i,"click",a=>{c.preventEvent(a),e.clickEvent(a);});const s=N("#mobile-nav nav"),o=N("#mobile-nav nav .multi-link-nav");o?c.before(o,i):s?c.append(s,i):p.error("元素#mobile-nav nav不存在");});}},F={async getScriptInfo(e){const t=O.getScriptInfoUrl(e),A=await V.get(t,{allowInterceptConfig:false,responseType:"json"});if(!A.status){const i=O.getScriptHomeUrl(e),s=await V.get(i,{fetch:true});if(!s.status)return;const o=c.toElement(s.data.responseText,true,true),a=o.querySelector(".install-link"),l=o.querySelector("dd.script-show-created-date relative-time[datetime]"),d=o.querySelector("dd.script-show-daily-installs"),u=o.querySelector("dd.script-show-total-installs"),f=o.querySelector("dd.script-show-updated-date relative-time[datetime]"),v=o.querySelector("#script-description"),y=o.querySelector(".good-rating-count"),x=o.querySelector(".ok-rating-count"),P=o.querySelector(".bad-rating-count"),I=o.querySelector("dd.script-show-license"),M={id:Number(e),created_at:l?.getAttribute("datetime")||"",daily_installs:Number(c.text(d)||"0"),total_installs:Number(c.text(u)||"0"),code_updated_at:f?.getAttribute("datetime")||"",support_url:"",fan_score:"",namespace:a?.getAttribute("data-script-namespace")||"",contribution_url:null,contribution_amount:null,good_ratings:Number(c.text(y)||"0"),ok_ratings:Number(c.text(x)||"0"),bad_ratings:Number(c.text(P)||"0"),users:[],name:a?.getAttribute("data-script-name")||c.text(o.querySelector("title")),description:c.text(v),url:t,code_url:a?.getAttribute("href")||"",license:c.text(I)||null,version:a?.getAttribute("data-script-version")||o.querySelector("dd.script-show-version")?.innerText||"",locale:"",deleted:false};if(h.isNull(M.code_url)&&!a){let T=o.querySelector("#script-content code");T&&(M.code_url=c.text(T).trim().replace(/^\/\/[\s]*@require[\s]*/gi,"").trim());}return M}return h.toJSON(A.data.responseText)},async getScriptStats(e){const t=await V.get(`/scripts/${e}/stats.json`,{allowInterceptConfig:false});if(p.info(t),!t.status){p.error(r.t("获取脚本统计数据失败"));return}return h.toJSON(t.data.responseText)},async getSourceCodeSyncFormDataInfo(e){const t=await V.get(`/scripts/${e}/admin`,{fetch:true,allowInterceptConfig:false});if(p.info(t),!t.status){g.error(r.t("请求admin内容失败"));return}const A=t.data.responseText,i=c.toElement(A,false,true).querySelector("form.edit_script[action*='sync_update']");if(!i){g.error(r.t("解析admin的源代码同步表单失败"));return}const s=new FormData(i),o=i.querySelector("input[type='submit'][name='update-and-sync']");return o&&s.append(o.name,o.value),{url:i.action,formData:s}},async sourceCodeSync(e,t,A){const n=await V.post(A||`/scripts/${e}/sync_update`,{fetch:true,data:t,allowInterceptConfig:false,headers:{Accept:"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7","Content-Type":"application/x-www-form-urlencoded",Origin:window.location.origin,Referer:window.location.href,"User-Agent":h.getRandomPCUA()}});if(p.info(n),!n.status){g.error(r.t("源代码同步失败"));return}return n},async getUserInfo(e){const t=await V.get(`/users/${e}.json`,{allowInterceptConfig:false});if(p.success(t),!t.status){g.error(r.t("获取用户信息失败"));return}const A=h.toJSON(t.data.responseText);return A.scriptList=[],A.scriptLibraryList=[],A.scripts.forEach(n=>{n.code_url.endsWith(".user.js")?A.scriptList.push(n):A.scriptLibraryList.push(n);}),A},async getUserAllInfo(e){const t=await this.getUserInfo(e);if(t){if(!t.scriptLibraryList.length){const A=await V.get(`/users/${e}`,{fetch:true,allowInterceptConfig:false});if(!A.status){g.error(r.t("获取用户主页信息失败"));return}const n=A.data.responseText,i=c.toElement(n,true,true),s=i.querySelector("#user-library-script-list");s?s.querySelectorAll("li").forEach(o=>{const a=le.parseScriptListInfo(o),l=o.querySelector("a.script-link").href;t.scriptLibraryList.push({id:a.scriptId,created_at:a.scriptCreatedDate.toISOString(),daily_installs:a.scriptDailyInstalls,total_installs:a.scriptTotalInstalls,code_updated_at:a.scriptUpdatedDate.toISOString(),support_url:null,fan_score:a.scriptRatingScore.toString(),namespace:null,contribution_url:null,contribution_amount:null,good_ratings:0,ok_ratings:0,bad_ratings:0,name:a.scriptName,description:a.scriptDescription,url:l,code_url:a.codeUrl,license:null,version:a.scriptVersion,locale:a.scriptLanguage,deleted:false});}):p.error("解析用户主页的库列表失败",i);}return t}},async getUserCollection(e){const t=await V.get(`/users/${e}`,{fetch:true,allowInterceptConfig:false});if(p.info("获取用户的收藏集",t),!t.status){g.error(r.t("获取用户的收藏集失败"));return}const A=t.data.responseText,i=c.toElement(A,true,true).querySelector("#user-script-sets");if(!i){p.error("解析Script Sets失败");return}const s=[];return i.querySelectorAll("li").forEach(o=>{const a=o.querySelector("a:last-child");if(!a)return;const l=a.href;if(l.includes("?fav=1"))return;const d=o.querySelector("a").innerText,u=O.getSetsId(l);s.push({id:u,name:d,url:`/scripts?set=${u}`});}),s},async getUserCollectionInfo(e,t){const A=await V.get(`/users/${e}/sets/${t}/edit`,{fetch:true,allowInterceptConfig:false});if(p.info(A),!A.status){g.error(r.t("获取收藏集{{setsId}}失败",{setsId:t}));return}const n=A.data.responseText,i=c.toElement(n,true,true),s=i.querySelector('form[id^="edit_script_set"]');if(!s){g.error(r.t("获取表单元素#edit_script_set失败"));return}const o=new FormData(s),a=i.querySelector('meta[name="csrf-token"]');if(!a){g.error(r.t("获取表单csrfToken失败"));return}if(a.hasAttribute("content")){const l=a.getAttribute("content");l&&o.set("authenticity_token",l);}return o},async updateUserSetsInfo(e,t,A){const n=await V.post(`/users/${e}/sets/${t}`,{fetch:true,allowInterceptConfig:false,headers:{Accept:"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7","Accept-Language":"zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6","Cache-Control":"no-cache","Content-Type":"application/x-www-form-urlencoded",Pragma:"no-cache"},fetchInit:{referrerPolicy:"strict-origin-when-cross-origin"},data:A});if(p.info(n),!n.status){g.error(r.t("更新收藏集表单请求失败"));return}const i=n.data.responseText;return c.toElement(i,true,true)},async switchLanguage(e){const t=await V.get(e,{fetch:true,headers:{"Upgrade-Insecure-Requests":"1"}});p.info(t),t.status;}},K={isHome(){return window.location.pathname.split("/").length<=2&&window.location.search===""},isCode(){return !!window.location.pathname.split("/")?.includes("code")},isCodeStrict(){return !!window.location.pathname.match(/\/code(\/|)$/)},isVersion(){return !!window.location.pathname.match(/\/versions(\/|)$/)},isUsers(){return !!window.location.pathname.match(/\/.+\/users\/.+/gi)},isUsersConversations(){return this.isUsers()&&!!window.location.pathname.includes("/conversations")},isUsersConversationsWithSomeUser(){return this.isUsersConversations()&&!!window.location.pathname.match(/\/conversations\/[\d]+/)},isScript(){return !!window.location.pathname.match(/\/scripts\/[\d+]/)},isScriptAdmin(){return !!window.location.pathname.endsWith("/admin")},isScriptList(){return !!window.location.pathname.match(/\/scripts(\/|)$/)},isScriptsBySite(){return !!window.location.pathname.match("/scripts/by-site")},isScriptsFeedback(){return this.isScript()&&window.location.pathname.match(/\/feedback(\/|)$/i)},isScriptLibraryList(){return !!window.location.pathname.match(/\/libraries(\/|)$/)},isScriptSearchIndex(){return window.location.pathname.endsWith("/search")},isScriptSearch(){let e=new URLSearchParams(window.location.search);return this.isScriptList()&&e.has("q")},isScriptCodeSearch(){return !!window.location.pathname.match(/\/code-search(\/|)$/)},isScriptLibraryListSearch(){let e=new URLSearchParams(window.location.search);return this.isScriptLibraryList()&&e.has("q")},isDiscuessions(){return !!window.location.pathname.match(/\/discussions(\/|)$/)},isImageSource(){return window.location.pathname.startsWith("/vite/assets")}},it=`:root {\r
...
`,lt=`input[type="search"],\r
input[type="text"],\r
input[type="password"],\r
/* 高级搜索↓ */\r
input#search-total_installs[type="number"],\r
input#search-daily_installs[type="number"],\r
input#search-ratings[type="number"],\r
input#search-created[type="datetime-local"],\r
input#search-updated[type="datetime-local"] {\r
  justify-content: center;\r
  align-items: center;\r
  /* line-height: 1; */\r
  /* height: 32px; */\r
...
\r
/* 首页搜索框右边的搜索按钮 */\r
input.search-submit[type="submit"] {\r
  height: 100%;\r
}\r
\r
@media (prefers-color-scheme: dark) {\r
...
`,gt="",vt={init(){m.execMenuOnce("beautifyPage",()=>this.beautifyPageElement()),m.execMenuOnce("beautifyGreasyforkBeautify",()=>this.beautifyGreasyforkBeautify()),m.execMenuOnce("beautifyUploadImage",()=>this.beautifyUploadImage()),m.execMenuOnce("beautifyTopNavigationBar",()=>this.beautifyTopNavigationBar());},beautifyPageElement(){p.info("美化页面元素");const e=[];return e.push(D(it)),e.push(D(at)),e.push(D(st)),e.push(D(lt)),e.push(D(ct)),e.push(D(`
			p:has(input[type="submit"][name="update-and-sync"]){
			  margin-top: 10px;
			}
			`)),c.onReady(()=>{let t=N('a[target="markup_choice"][href*="daringfireball.net"]');t&&t.parentElement.replaceChild(c.createElement("span",{textContent:"Markdown"}),t),X(".user-content ul li").forEach(i=>{let s=i.firstChild;s?.nodeName==="#text"&&(s.textContent?.startsWith("[x] ")?(s.textContent=s.textContent.replace("[x] ",""),c.prepend(i,`
							<input type="checkbox" disabled="" class="task-list-item-checkbox" checked="">
						`)):s.textContent?.startsWith("[ ] ")&&(s.textContent=s.textContent.replace("[ ] ",""),c.prepend(i,`
				...
				`},n=Object.keys(A);if(X(".user-content blockquote").forEach(i=>{Array.from(i.children).forEach(s=>{let o=c.html(s);if(s.nodeName==="P"){let a=n.find(l=>o.toLowerCase().startsWith(l.toLowerCase()));a&&(c.html(s,o.replace(a,"")),c.before(s,A[a]),c.html(s)===""&&s.remove());}});}),X(".user-content > blockquote").forEach(i=>{Array.from(i.querySelectorAll(".markdown-alert-title")).forEach(s=>{let o=c.createElement("div",{className:"markdown-alert markdown-alert-tip"},{"data-type":s.getAttribute("data-type")});for(;s.nextSibling&&!s.nextSibling?.classList?.contains("markdown-alert-title");)c.append(o,s.nextSibling);c.prepend(o,s),c.before(i,o);});}),K.isHome())e.push(D(ft));else if(K.isScriptsFeedback()){e.push(D(gt));let i=N('.radio-label[for*="discussion_rating_0"]');i&&(i.innerHTML=i.innerHTML.replace("不评分",'<span class="rating-icon rating-icon-none">不评分</span>'));let s=N('.radio-label[for*="discussion_rating_2"]');s&&(s.innerHTML=s.innerHTML.replace("差评",'<span class="rating-icon rating-icon-bad">差评</span>'));let o=N('.radio-label[for*="discussion_rating_3"]');o&&(o.innerHTML=o.innerHTML.replace("一般",'<span class="rating-icon rating-icon-ok">一般</span>'));let a=N('.radio-label[for*="discussion_rating_4"]');a&&(a.innerHTML=a.innerHTML.replace("好评",'<span class="rating-icon rating-icon-good">好评</span>'));}else K.isScriptAdmin()&&(N('input[type="submit"][name="update-only"]')||e.push(D(`
					.indented{
						padding-left: unset;
					}
					`)));}),e},beautifyGreasyforkBeautify(){p.info("美化 Greasyfork Beautify脚本");const e=[];return e.push(D(ut)),h.isPhone()?e.push(D(`
				section#script-info,
				...
				section#script-info{
					margin-top: 10px;
				}`)),e},beautifyUploadImage(){p.info("美化上传图片");const e=[];return e.push(D(dt)),c.onReady(()=>{function t(i){for(;i.nextElementSibling;)i.parentElement.removeChild(i.nextElementSibling);}const A=X('input[type="file"]');A.forEach(i=>{i.getAttribute("name")!=="code_upload"&&(i.hasAttribute("accept")&&i.getAttribute("accept").includes("javascript")||c.on(i,["propertychange","input"],function(s){t(s.target);const o=s.currentTarget.files;if(!o||o.length===0)return;p.info("选择的图片",o),o.length>5&&c.after(i,c.createElement("p",{textContent:r.t("❌ 最多同时长传5张图片")}));const a=[];Array.from(o).forEach(l=>{(l.size>204800||!l.type.match(/png|jpg|jpeg|gif|apng|webp/i))&&a.push(l);}),a.length!==0&&a.forEach(l=>{c.after(i,c.createElement("p",{textContent:r.t("❌ 图片：{{name}} 大小：{{size}}",{name:l.name,size:l.size})}));});}));}),["textarea#comment_text","textarea.comment-entry"].forEach(i=>{c.on(i,"paste",s=>{p.info("触发粘贴事件",s),setTimeout(()=>{c.emit(A,"input");},100);});});}),e},beautifyTopNavigationBar(){p.info("美化顶部导航栏");const e=[];return e.push(D(pt)),window.outerWidth>550&&(e.push(_.addBlockCSS(".with-submenu")),c.onReady(()=>{const A=N("#site-nav").querySelector("nav");X(".with-submenu nav li").forEach(n=>{A.appendChild(n);});})),e}},ht=".whitesev-hide{display:none}.whitesev-hide-important,.ad,.ad-entry{display:none!important}";Ge(ht);const mt={init(){m.execMenu("autoLogin",()=>{this.autoLogin();});},autoLogin(){c.waitNode("span.sign-in-link a[rel=nofollow]").then(async()=>{let e=m.getValue("user"),t=m.getValue("pwd"),A=m.getValue("secret");if(h.isNull(e)){g.error(r.t("请先在菜单中录入账号"));return}if(h.isNull(t)){g.error(r.t("请先在菜单中录入密码"));return}if(h.isNull(A)){g.error(r.t("请先在菜单中录入secret"));return}let n=N("meta[name='csrf-token']");if(!n){g.error(r.t("获取csrf-token失败"));return}let s=new Fe__namespace.TOTP({secret:A}).generate(),o=g.loading(r.t("正在登录中...")),a=await V.post("/zh-CN/users/sign_in",{fetch:true,data:encodeURI(`authenticity_token=${n.getAttribute("content")}&user[email]=${e}&user[password]=${t}&user[remember_me]=1&user[otp_attempt]=${s}&commit=登录`),headers:{"Content-Type":"application/x-www-form-urlencoded"}});if(o.destroy(),!a.status){p.error(a),g.error(r.t("登录失败，请求异常"));return}let l=a.data.responseText,d=c.toElement(l,true,true),u=d.querySelector(".width-constraint .alert");d.querySelectorAll(".sign-out-link a[rel=nofollow][data-method='delete']").length?(g.success(r.t("登录成功，1s后自动跳转")),setTimeout(()=>{window.location.reload();},1e3)):u?(p.error(u),g.error(c.text(u))):(g.error(r.t("登录失败，可能是账号/密码错误，请在控制台查看原因"),{consoleLogContent:true}),p.error(a),p.error(`当前账号:${e}`),p.error(`当前密码:${t}`));});}},q={menu:Ne,isLogin:false,initEnv(){let e=this.getUserLinkElement();this.isLogin=!!e;},getUserLinkElement(){return N("#nav-user-info span.user-profile-link a")},async updateScript(e){let t=function(A,n=1){return `
			...
			</div>`};if(h.isNull(e))g.error(r.t("未获取到【脚本列表】"));else {let A=g.loading(t(O.getScriptName(e[0])),{isHTML:true}),n=0,i=0;for(let s=0;s<e.length;s++){let o=e[s],a=O.getScriptId(o);p.success("更新："+o);let l=O.getScriptName(o);A.setHTML(t(l,s+1));let d=await F.getSourceCodeSyncFormDataInfo(a);if(d){const{formData:u,url:f}=d;await F.sourceCodeSync(a,u,f)?(g.success(r.t("源代码同步成功，3秒后更新下一个")),await h.sleep(3e3),n++):(g.error(r.t("源代码同步失败")),i++);}else g.error(r.t("源代码同步失败")),i++;}A.close(),n===0?g.error(r.t("全部更新失败")):g.success(r.t("全部更新完毕<br >成功：{{successNums}}<br >失败：{{failedNums}}<br >总计：{{scriptUrlListLength}}",{successNums:n,failedNums:i,scriptUrlListLength:e.length}),{isHTML:true});}},handleLocalGotoCallBack(){if(m.getValue("goto_updateSettingsAndSynchronize_scriptList")){if(m.deleteValue("goto_updateSettingsAndSynchronize_scriptList"),!K.isUsers()){m.setValue("goto_updateSettingsAndSynchronize_scriptList",true),q.getUserLinkElement()?(g.success(r.t("前往用户主页")),window.location.href=q.getUserLinkElement().href):g.error(r.t("获取当前已登录的用户主页失败"));return}let e=[];X("#user-script-list-section li a.script-link").forEach(t=>{e=e.concat(O.getAdminUrl(t.href));}),q.updateScript(e);}else if(m.getValue("goto_updateSettingsAndSynchronize_unlistedScriptList")){if(m.deleteValue("goto_updateSettingsAndSynchronize_unlistedScriptList"),!K.isUsers()){m.setValue("goto_updateSettingsAndSynchronize_unlistedScriptList",true),q.getUserLinkElement()?(g.success(r.t("前往用户主页")),window.location.href=q.getUserLinkElement().href):g.error(r.t("获取当前已登录的用户主页失败"));return}let e=[];X("#user-unlisted-script-list li a.script-link").forEach(t=>{e=e.concat(O.getAdminUrl(t.href));}),q.updateScript(e);}else if(m.getValue("goto_updateSettingsAndSynchronize_libraryScriptList")){if(m.deleteValue("goto_updateSettingsAndSynchronize_libraryScriptList"),!K.isUsers()){m.setValue("goto_updateSettingsAndSynchronize_libraryScriptList",true),q.getUserLinkElement()?(g.success(r.t("前往用户主页")),window.location.href=q.getUserLinkElement().href):g.error(r.t("获取当前已登录的用户主页失败"));return}let e=[];X("#user-library-script-list li a.script-link").forEach(t=>{e=e.concat(O.getAdminUrl(t.href));}),q.updateScript(e);}}},Se={$key:{DB_KEY:"data"},$data:{db:null},init(){this.$data.db=this.getDB(),m.execMenuOnce("rememberReplyContent",()=>{this.rememberReplyContent();}),m.execMenu("gf-autoClearRememberReplayContent",e=>{this.autoClearRememberReplayContent(e.value);});},getDB(){const e="reply_record",t="textarea_text";return new h.indexedDB(e,t,2)},async rememberReplyContent(){const e="记住回复内容 -- ";let t=X("form");if(!t.length){p.warn(e+"不存在表单");return}try{await this.clearRelayHistoryRememberContentText();}catch(A){p.error(A);}t.forEach(async A=>{let n=A.querySelector("textarea"),i=A.querySelector('input[type="submit"]');n&&i&&(p.success("开始监听form --- 记住回复内容",A),this.$data.db.get(this.$key.DB_KEY).then(s=>{if(!s.data)return;let o=s.data.findIndex(l=>this.checkUrlIsSame(window.location.href,l.url));if(o==-1)return;let a=s.data[o].text;p.success("填入历史输入内容："+a),n.value=a;}),c.on(n,["propertychange","input"],h.debounce(s=>{let o={url:window.location.href,text:n.value,time:Date.now()};this.$data.db.get(this.$key.DB_KEY).then(a=>{if(!a.success&&a.event&&a.event.type!=="success"){p.warn(a);return}a.data==null&&(a.data=[]);let l=a.data.findIndex(d=>this.checkUrlIsSame(window.location.href,d.url));l!==-1?h.isNull(o.text)?a.data.splice(l,1):a.data[l]=h.assign(a.data[l],o):a.data=a.data.concat(o),this.$data.db.save(this.$key.DB_KEY,a.data).then(d=>{d.success||p.error("保存失败",d);});});},25)),c.on(A,"submit",s=>{p.info("表单提交，刷新页面后清理内容："+window.location.href),$("delyClear_rememberReplyContent_url",window.location.href);},{capture:true}));});},async clearRelayHistoryRememberContentText(){const e="delyClear_rememberReplyContent_url";let t=ee(e);if(t){let A=await this.$data.db.get(this.$key.DB_KEY);if(!A.data){p.info("表单记录：数据库是空的");return}let n=A.data.findIndex(s=>this.checkUrlIsSame(t,s.url));if(n==-1){p.info("表单记录：已不存在该数据"),ve(e);return}A.data.splice(n,1),(await this.$data.db.save(this.$key.DB_KEY,A.data)).success?(p.success("表单记录：成功清除"),ve(e)):p.error("表单记录：清除失败",A);}},checkUrlIsSame(e,t){let A=new URL(e),n=new URL(t);return A.origin===n.origin&&A.pathname===n.pathname},autoClearRememberReplayContent(e){const t="gf-last-time-autoClearRememberReplayContent";let A=ee(t),n=e*24*60*60*1e3;if(A)if(Date.now()-A>n)$(t,Date.now());else return;$(t,Date.now());},async getAllRememberReplyContent(){return (await this.$data.db.get(this.$key.DB_KEY)).data??[]},async clearAllRememberReplyContent(){return (await this.$data.db.delete(this.$key.DB_KEY)).success}};class yt{KEY="short-cut";#e={otherShortCutOptions:[],localOptions:[],currentWaitEnterPressInstanceHandler:null};#t={isWaitPress:false};constructor(t){typeof t=="string"&&(this.KEY=t),this.initData();}initConfig(t,A){this.hasOption(t)||this.setOption(t,A);}initData(t){this.#e.localOptions.length=0,this.#e.localOptions=t??this.getLocalAllOptions();}initGlobalKeyboardListener(t,A){if(!this.#e.localOptions.length){p.warn("快捷键配置为空");return}const n=this,i=function(a,l){c.onKeyboard(a,"keydown",(d,u,f,v)=>{if(n.#t.isWaitPress)return;A?.isPrevent&&c.preventEvent(v);const x=JSON.stringify({keyName:d,keyValue:u,ohterCodeList:f}),P=n.#e.localOptions.find(I=>{const M=I.value;if(JSON.stringify(M)===x)return  true});P&&P.key in l&&(p.info("调用快捷键",P),l[P.key].callback());},{capture:!!A?.capture});},s={},o={};Object.keys(t).forEach(a=>{const l=t[a];(l.target==null||typeof l.target=="string"&&l.target==="")&&(l.target="window"),l.target==="window"?Reflect.set(s,a,l):Reflect.set(o,a,l);}),i(window,s),c.onReady(()=>{Object.keys(o).forEach(async a=>{const l=o[a],d={};let u=null;typeof l.target=="string"?u=await c.waitNode(l.target,1e4):typeof l.target=="function"?u=await l.target():u=l.target,u&&(Reflect.set(d,a,l),i(u,d));});});}isWaitKeyboardPress(){return this.#t.isWaitPress}getStorageKey(){return this.KEY}getLocalAllOptions(){return ee(this.KEY,[])}hasOption(t){return !!this.getLocalAllOptions().find(i=>i.key===t)}hasOptionValue(t){return this.hasOption(t)?this.getOption(t)?.value!=null:false}getOption(t,A){return this.getLocalAllOptions().find(s=>s.key===t)??A}setOption(t,A){const n=this.getLocalAllOptions(),i=n.findIndex(s=>s.key===t);i==-1?n.push({key:t,value:A}):Reflect.set(n[i],"value",A),this.initData(n),$(this.KEY,n);}emptyOption(t){let A=false;const n=this.getLocalAllOptions(),i=n.findIndex(s=>s.key===t);return i!==-1&&(n[i].value=null,A=true),this.initData(n),$(this.KEY,n),A}deleteOption(t){let A=false;const n=this.getLocalAllOptions(),i=n.findIndex(s=>s.key===t);return i!==-1&&(n.splice(i,1),A=true),this.initData(n),$(this.KEY,n),A}translateKeyboardValueToButtonText(t){let A="";return t.ohterCodeList.forEach(n=>{A+=h.stringTitleToUpperCase(n,true)+" + ";}),A+=h.stringTitleToUpperCase(t.keyName),A}getShowText(t,A){if(this.hasOption(t)){const n=this.getOption(t);return n.value==null?A:this.translateKeyboardValueToButtonText(n.value)}else return A}async enterShortcutKeys(t){return new Promise(A=>{this.#t.isWaitPress=true;const n=c.onKeyboard(window,"keyup",(i,s,o)=>{const a={keyName:i,keyValue:s,ohterCodeList:o};let l={};try{const d=JSON.stringify(a);let u=this.getLocalAllOptions();Array.isArray(this.#e.otherShortCutOptions)&&(u=u.concat(this.#e.otherShortCutOptions));for(let f=0;f<u.length;f++){let v=u[f];if(v.key===t)continue;const y=JSON.stringify(v.value);let x=!1;if(v.value!=null&&d===y&&(x=!0),x){l={status:!1,key:v.key,option:a};return}}this.setOption(t,a),l={status:!0,key:t,option:a};}catch(d){p.error(d),l={status:false,key:t,option:a};}finally{this.#t.isWaitPress=false,n.removeListen(),this.#e.currentWaitEnterPressInstanceHandler=null,A(l);}});this.#e.currentWaitEnterPressInstanceHandler=null,this.#e.currentWaitEnterPressInstanceHandler=()=>{this.#t.isWaitPress=false,n.removeListen();};})}cancelEnterShortcutKeys(){typeof this.#e.currentWaitEnterPressInstanceHandler=="function"&&this.#e.currentWaitEnterPressInstanceHandler();}}const Re={shortCut:new yt,shortOption:{"gf-quickReply":{target:()=>{let e=N("form textarea"),t=N('input[name="commit"][type="submit"]');if(e){if(!t){p.error("页面不存在【发表回复】按钮");return}}else {p.error("页面不存在输入框");return}return p.success("监听快捷键：gf-quickReply"),e},callback(){if(document.activeElement){let e=document.activeElement.closest("form");if(!e){p.error("当前activeElement不在表单内，无法触发快捷键");return}let t=e.querySelectorAll('input[name="commit"][type="submit"]');if(!t.length){p.error("表单内不存在【发表回复】按钮");return}t.length>1&&p.warn("表单内存在多个【发表回复】按钮，只触发第一个"),t[0].click();}else p.error("当前页面没有激活元素，无法触发快捷键");}}},init(){this.shortCut.initGlobalKeyboardListener(this.shortOption);}},te={key:"gf-discuessions-filter-rule",$data:{FILTER_SCRIPT_KEY:"greasyfork-discussions-filter-script",FILTER_POST_USER_KEY:"greasyfork-discussions-filter-post-user",FILTER_REPLY_USER_KEY:"greasyfork-discussions-filter-reply-user"},init(){p.info("论坛-过滤"),D(`
        ...
        `);let e=new h.LockFunction(()=>{this.filter();},50);h.mutationObserver(document.body,{config:{subtree:true,childList:true},immediate:true,callback:()=>{e.run();}}),e.run();},getElementList(){let e=[];return e=e.concat(Array.from(X(".discussion-list-container"))),e},filter(){const e=new Map;this.getElementList().forEach((t,A)=>{const n=this.parseDiscuessionListContainerInfo(t);if(!n)return;let i=this.getValue().split(`
`);if(e.has(n.snippet)&&m.getValue("greasyfork-discussions-filter-duplicate-comments")){let s=e.get(n.snippet).querySelector("a.discussion-title");s.setAttribute("data-repeat-tip-show","true");let o=0;s.hasAttribute("data-repeat-count")&&(o=parseInt(s.getAttribute("data-repeat-count"))),o++,s.setAttribute("data-repeat-count",o.toString()),s.setAttribute("data-repeat-tip-show",r.t("已过滤：{{oldCount}}",{oldCount:o})),p.success([`过滤重复内容：${n.snippet}`,n]),t.remove();return}e.set(n.snippet,t);for(let s=0;s<i.length;s++){let o=i[s],a=o.split("##"),l=a[0],d=a[1];if(l in n){let u=new RegExp(d,"ig");if(n[l]!=null&&String(n[l]).match(u)){p.info("触发论坛过滤规则",o,n),t.remove();return}}}});},parseDiscuessionListContainerInfo(e){const t=e.querySelector("a.discussion-title");if(!t)return;const A=t.href,n=A.match(/\/discussions(|\/greasyfork)\/([\d]+)/);if(!n)return;const i=n[n.length-1],s=e.querySelector(".discussion-meta-item-script-name"),o=s?.querySelector("a"),a=e.querySelector("a.user-link"),l={scriptName:s.innerText,scriptUrl:o?.href,scriptId:O.getScriptId(o?.href),postUserName:a?.innerText,postUserHomeUrl:a?.href,postUserId:O.getUserId(a?.href),postTimeStamp:new Date(e.querySelector("relative-time").getAttribute("datetime")),snippetId:i,snippetUrl:A,snippet:e.querySelector("span.discussion-snippet")?.innerText||"",replyUserName:void 0,replyUserHomeUrl:void 0,replyUserId:void 0,replyTimeStamp:void 0};let d=e.querySelector(".discussion-meta-item .discussion-meta-item");if(d){const u=d.querySelector("a.user-link");if(!u)return l;l.replyUserName=u.innerText,l.replyUserHomeUrl=u.href,l.replyUserId=O.getUserId(l.replyUserHomeUrl),l.replyTimeStamp=new Date(d.querySelector("relative-time").getAttribute("datetime"));}return l},setValue(e){m.setValue(this.key,e);},addValue(e,t){let A=this.getValue();if(A.trim()!==""&&(A+=`
`),h.isNull(e))return;e=e.toString().trim();let n=e+"##"+t;A+=n,this.setValue(A);},getValue(){return m.getValue(this.key,"")}},wt={init(){this.readBgColor(),c.onReady(()=>{m.execMenuOnce("greasyfork-discussions-filter-enable",()=>{this.filterEnable();}),D(`
      ...
      `);const e=new h.LockFunction(()=>{const t=m.getValue("discussions-addShortcutOperationButton"),A=m.getValue("discussions-addReportButton");t&&this.addFilterButton(),A&&this.addReportButton(),(t||A)&&this.addMoreButton({addFilterButton:t,addReportButton:A});});h.mutationObserver(document.body,{config:{subtree:true,childList:true},immediate:true,callback:()=>{e.run();}});});},filterEnable(){p.info("启用Greasyfork论坛过滤器"),te.init();},readBgColor(){p.info("设置已读背景颜色");const e=m.getValue("discussions-readBgColor"),A=new h.ColorConversion().getDarkColor(e,.8);return D(`
        .discussion-read{
        ...
        `)},addFilterButton(){const e="discussion-filter-button";te.getElementList().forEach(t=>{if(t.querySelector(`.${e}`))return;const n=t.querySelector(".discussion-list-item").querySelector(".discussion-meta"),i=c.createElement("div",{className:"discussion-meta-item",innerHTML:`
					<button class="${e}">${r.t("过滤")}</button>
					`},{style:"flex: 0;","data-type":"filter"}),s=i.querySelector(`.${e}`);n.appendChild(i),c.on(s,"click",o=>{c.preventEvent(o);const a=te.parseDiscuessionListContainerInfo(t);if(!a)return;const l="data-filter-key",d="data-filter-value",u=Q.alert({title:{text:r.t("选择需要过滤的选项"),position:"center",html:false},content:{text:`
								<button ${l}="scriptId" ${d}="^${a.scriptId}$">${r.t("脚本id：{{text}}",{text:a.scriptId})}</button>
								<button ${l}="scriptName" ${d}="^${h.toRegExpStr(a.scriptName)}$">${r.t("脚本名：{{text}}",{text:a.scriptName})}</button>
                ${a.postUserId!=null?`
                  <button ${l}="postUserId" ${d}="^${h.toRegExpStr(a.postUserId)}$">${r.t("发布的用户id：{{text}}",{text:a.postUserId})}</button>
                `:""}
						...
						`}),f=u.$shadowRoot.querySelector(".pops-alert-content");if(a.scriptId==null&&f.querySelector(`button[${l}="scriptId"]`)?.remove(),a.scriptName==null&&f.querySelector(`button[${l}="scriptName"]`)?.remove(),a.postUserId==null&&f.querySelector(`button[${l}="postUserId"]`)?.remove(),a.replyUserId!=null){const v=c.createElement("button",{innerHTML:r.t("作者id：{{text}}",{text:a.replyUserId})});v.setAttribute(l,"scriptAuthorId"),v.setAttribute(d,"^"+a.replyUserId+"$"),f.appendChild(v);}c.on(u.$shadowRoot,"click",`button[${l}]`,v=>{c.preventEvent(v);const y=v.target,x=y.getAttribute(l),P=y.getAttribute(d);te.addValue(x,P),u.close(),te.filter(),g.success(r.t("添加成功"));});});});},addReportButton(){const e="discussion-report-button";te.getElementList().forEach(t=>{if(t.querySelector(`.${e}`))return;let n=t.querySelector(".discussion-list-item").querySelector(".discussion-meta"),i=c.createElement("div",{className:"discussion-meta-item",innerHTML:`
					<button class="${e} pops-button--danger">${r.t("举报")}</button>
					`},{style:"flex: 0;","data-type":"report"}),s=i.querySelector(`.${e}`);n.appendChild(i),c.on(s,"click",o=>{c.preventEvent(o);const a=te.parseDiscuessionListContainerInfo(t);a&&Q.alert({title:{text:r.t("举报"),position:"center",html:false},content:{text:`
						<div class="report-item">
							${r.t("举报讨论：")}
							<a href="${O.getReportUrl("discussion",a.snippetId)}" target="_blank">${a.snippet}</a>
						</div>
						${a.scriptId?`
							<div class="report-item">
							${r.t("举报脚本：")}
							<a href="${O.getReportUrl("script",a.scriptId)}" target="_blank">${a.scriptName}</a>
						</div>
						`:""}
						
						<div class="report-item">
							${r.t("举报用户：")}
							<a href="${O.getReportUrl("user",a.postUserId)}" target="_blank">${a.postUserName}</a>
						</div>
						${a.replyUserId&&a.replyUserId!=a.postUserId?`
								<div class="report-item">
									${r.t("举报用户：")}
									<a href="${O.getReportUrl("user",a.replyUserId)}" target="_blank">${a.replyUserName}</a>
								</div>
								`:""}
							...
							`});});});},addMoreButton(e){const t="discussion-more-button";te.getElementList().forEach(A=>{if(A.querySelector(`.${t}`))return;let n=A.querySelector(".discussion-list-item"),i=n.querySelector(".discussion-meta"),s=c.createElement("div",{className:"discussion-meta-item",innerHTML:`
					<button class="${t}">${r.t("...")}</button>
					`},{style:"flex: 0;","data-type":"more"});const o=s.querySelector(`.${t}`),a=[];if(e.addFilterButton&&a.push({text:r.t("过滤"),callback:()=>{const u=c.parent(n).querySelector('.discussion-meta-item[data-type="filter"] button');if(!u){g.error(r.t("未找到【过滤】按钮"));return}u.click();}}),e.addReportButton&&a.push({text:r.t("举报"),callback:()=>{const u=c.parent(n).querySelector('.discussion-meta-item[data-type="report"] button');if(!u){g.error(r.t("未找到【举报】按钮"));return}u.click();}}),!a.length)return;const l=Q.rightClickMenu({position:"absolute",limitPositionYInView:false,data:a});l.removeContextMenuEvent(document.documentElement),c.on(o,"click",d=>{c.preventEvent(d);const u=o.getBoundingClientRect();l.PopsContextMenu.contextMenuEvent(new PointerEvent("contextmenu",{clientX:u.left+window.scrollX,clientY:u.top+u.height+window.scrollY}),o);}),i.appendChild(s);});}},De={isCurrentLoginUserHome(){const e=le.getCurrentLoginUserId();return !!(e!=null&&K.isUsers()&&window.location.pathname.includes("/"+e))},monacoEditor(){return new Promise(e=>{const t="0.52.2",A="monaco-editor-ready";if(R.monaco){e(R.monaco);return}const n=g.loading(r.t("monaco-editor加载中...")),i=c.createElement("script",{type:"module",defer:true,innerHTML:`
				...
				`});c.append(document.head||document.documentElement,i),c.on(R,A,()=>{const s=R.monaco;p.success("网络加载monaco编辑器成功"),n.close(),e(s);},{once:true});})}},bt={init(){m.execMenuOnce("code-repairCodeLineNumber",()=>{this.repairCodeLineNumber();}),m.execMenuOnce("code-use-monaco-editor",()=>{this.coverEditorWithMonaco();});},repairCodeLineNumber(){p.info("修复代码的行号显示不够问题"),m.execMenuOnce("beautifyGreasyforkBeautify",()=>{D(`
				.code-container pre code .marker{
					padding-left: 6px;
				}	
				`);}),c.waitNode("#script-content div.code-container pre.prettyprint ol").then(e=>{e.childElementCount>=1e3&&(p.success(`当前代码行数${e.childElementCount}行，超过1000行，优化行号显示问题`),D(`
                    pre.prettyprint{
                        padding-left: 26px;
                    }
					`));});},coverEditorWithMonaco(){p.info("使用monacoEditor替换编辑器"),D(`
			.monaco-editor{
		...
		`),_.addBlockCSS("#script-content .code-container > pre");let e=O.getScriptId(window.location.href);if(!e){g.error("未解析出当前脚本ID",{consoleLogContent:true});return}Promise.all([De.monacoEditor(),F.getScriptInfo(e)]).then(async([t,A])=>{let n=A?.code_url;if(!n){g.error("请求结果中未解析出脚本代码URL",{consoleLogContent:true});return}let i=new URLSearchParams(window.location.search);if(i.has("version")){let a=i.get("version");n=n.replace(new RegExp(`/${e}(/[\\d]+|)`),`/${e}/${a}`),p.info(`当前是历史代码页面，请求的脚本代码URL为${n}`);}let s=await V.get(n,{timeout:2e4});if(!s.status)return;let o=s.data.responseText;c.onReady(async()=>{let a=await c.waitNode("#script-content .code-container > pre",1e4);if(!a)return;let l=c.createElement("div",{className:"monaco-editor"});c.after(a,l),t.editor.create(l,{value:o,minimap:{enabled:true},automaticLayout:true,codeLens:true,colorDecorators:true,contextmenu:true,readOnly:true,formatOnPaste:true,overviewRulerBorder:true,scrollBeyondLastLine:false,theme:"vs-dark",fontSize:window.innerWidth>600?14:12,wordWrap:"off",language:"javascript",folding:true,foldingStrategy:"indentation"});});});}},xt=`ul.history_versions,\r
...
`,Ct={init(){m.execMenuOnce("beautifyHistoryVersionPage",()=>this.beautifyHistoryVersionPage()),m.execMenuOnce("scripts-versions-addCompareCodeButton",()=>{this.sourceDiffMonacoEditor();}),c.onReady(()=>{m.execMenuOnce("scripts-versions-addExtraTagButton",()=>{this.addExtraTagButton();});});},beautifyHistoryVersionPage(){p.info("美化 历史版本 页面");const e=[];return e.push(D(xt)),e.push(_.addBlockCSS(".version-number",".version-date",".version-changelog")),c.onReady(function(){const t=N("ul.history_versions");if(!t){g.error(r.t("未找到history_versions元素列表"));return}Array.from(t.children).forEach(A=>{const n=A.querySelector(".version-number a").href,i=A.querySelector(".version-number a").innerText,s=A.querySelector(".version-date")?.getAttribute("datetime"),o=A.querySelector(".version-changelog")?.innerHTML||"",a=c.createElement("span",{className:"script-version-date",innerHTML:h.formatTime(s,r.t("yyyy年MM月dd日 HH:mm:ss"))}),l=c.createElement("div",{className:"script-tag",innerHTML:`
          ...
          </div>`}),d=c.createElement("div",{className:"script-note-box-body",innerHTML:o});c.append(A,[a,l,d]);});}),e},addExtraTagButton(){p.info("添加额外的标签按钮"),X(".script-tag-version:has(a[href])").forEach(e=>{const t=e.querySelector("a");if(!t)return;const A=new URL(t.href),n=O.getScriptId(A.pathname),i=A.searchParams.get("version"),s=O.getScriptName(A.toString()),o=O.getInstallUrl(n,i,s),a=O.getCodeUrl(n,i),l=c.createElement("div",{className:"scripts-tag-install",innerHTML:`
        <a class="script-btn-install install-link" data-install-format="js" target="_blank" href="${o}">${r.t("安装此脚本")}</a>
        <a class="script-btn-see-code" target="_blank" href="${a}">${r.t("查看代码")}</a>
        `});c.after(e,l);});},sourceDiffMonacoEditor(){p.info("源码对比（monacoEditor）"),De.monacoEditor().then(e=>{c.onReady(()=>{X('#script-content form[action*="/diff"] input[type="submit"]').forEach(t=>{let A=c.createElement("input",{type:"button",value:r.t("对比选中版本差异（monacoEditor）")},{style:"margin-left: 10px;"});c.after(t,A),c.on(A,"click",async n=>{c.preventEvent(n);const i=t.closest("form"),s=new FormData(i),o=s.get("v1"),a=s.get("v2");if(o===a){g.warning(r.t("版本号相同，不需要比较源码"));return}p.info(`对比版本：${o} vs ${a}`);const l=g.loading(r.t("正在获取对比文本中...")),d=O.getScriptId(),u=await F.getScriptInfo(d);if(!u){l.close();return}p.info("脚本信息：",u);const f=u.code_url,v=f.replace(new RegExp(`/${d}/`),`/${d}/${o}/`),y=f.replace(new RegExp(`/${d}/`),`/${d}/${a}/`);p.info(`请求版本代码：${v}`),p.info(`请求版本代码：${y}`);let x="",P="";const[I,M]=await Promise.all([V.get(v,{timeout:2e4}),V.get(y,{timeout:2e4})]);if(l.close(),!I.status||!M.status)return;x=I.data.responseText,P=M.data.responseText;let{recovery:T}=_.lockScroll(),z=Q.alert({title:{text:r.t("代码对比"),html:false,position:"center"},content:{html:true,text:`
								<div class="monaco-editor-diff-container">
									<div class="monaco-editor-diff"></div>
								</div>
								`},mask:{enable:true,clickEvent:{toClose:false,toHide:false}},btn:{ok:{enable:false},close:{callback(B,C){B.close(),T();}}},zIndex(){let B=h.getMaxZIndex(),C=Q.config.InstanceUtils.getPopsMaxZIndex().zIndex;return h.getMaxValue(B,C)+100},useShadowRoot:false,width:"90vw",height:"90vh",drag:true,style:`
						...
						`});z.$shadowRoot.querySelector(".monaco-editor-diff-container");let k=z.$shadowRoot.querySelector(".monaco-editor-diff"),w=e.editor.createDiffEditor(k,{hideUnchangedRegions:{enabled:true},minimap:{enabled:true},automaticLayout:true,codeLens:true,colorDecorators:true,contextmenu:false,readOnly:true,formatOnPaste:true,overviewRulerBorder:true,scrollBeyondLastLine:false,theme:"vs-dark",fontSize:window.innerWidth>600?14:12,wordWrap:"off",language:"javascript"});const b=e.editor.createModel(P,"javascript"),U=e.editor.createModel(x,"javascript");w.setModel({original:b,modified:U});});});});});}};let we=[];const qe=async function(e){if(p.info("当前脚本id："+e),!q.isLogin){p.error("请先登录账号"),g.error(r.t("请先登录账号"));return}const t=O.getUserId(q.getUserLinkElement().href);if(t==null){p.error("获取用户id失败"),g.error(r.t("获取用户id失败"));return}if(!we.length){const s=g.loading(r.t("获取收藏夹中..."));if(we=await F.getUserCollection(t)||[],s.close(),!we.length)return}let A="";const n=(s,o)=>{let a=false;o=o.toString().trim();for(const[l,d]of s.entries())if(l==="scripts-included[]"&&d.toString().trim()===o){a=true;break}return a};we.forEach(s=>{A+=`
		<li class="user-collect-item" data-id="${s.id}" data-name="${s.name}">
			<a class="user-collect-name" href="${s.url}" target="_blank">${s.name}</a>
		  ...
		  `;});const i=Q.alert({title:{text:r.t("收藏集"),position:"center"},content:{html:true,text:`<ul>${A}</ul>`},mask:{enable:true,clickEvent:{toClose:true}},btn:{ok:{enable:false}},width:Q.isPhone()?"92vw":"500px",height:"auto",drag:true,only:true,style:`
		.pops{
		...
		`});c.on(i.$shadowRoot,"click",".collect-add-script-id",async function(s){const o=s.target.closest(".user-collect-item"),a=o.dataset.id;if(h.isNull(a)){g.error(r.t("获取收藏集id失败"));return}o.dataset.name;const l=g.loading(r.t("添加中..."));try{const d=await F.getUserCollectionInfo(t,a);if(!d)return;if(n(d,e)){g.warning(r.t("该脚本已经在该收藏集中"));return}const u=h.cloneFormData(d),f=h.cloneFormData(d);u.set("add-script",e.toString()),u.set("script-action","i"),f.append("scripts-included[]",e.toString()),f.set("save","1");const v=new URLSearchParams(u),y=new URLSearchParams(f),x=Array.from(v).map(([b,U])=>`${encodeURIComponent(b)}=${encodeURIComponent(U)}`).join("&"),P=Array.from(y).map(([b,U])=>`${encodeURIComponent(b)}=${encodeURIComponent(U)}`).join("&");p.info("添加的数据",x),p.info("保存的数据",P);const I=await F.updateUserSetsInfo(t,a,x);if(!I)return;const M=I.querySelector(".change-script-set");if(!M){g.error(r.t("添加失败，{{selector}}元素不存在",{selector:".change-script-set"}));return}const T=M.querySelector("section");if(!T){g.error(r.t("添加失败，{{selector}}元素不存在",{selector:"section"}));return}const z=T.querySelector(".alert");if(z){Q.alert({title:{text:r.t("添加失败"),position:"center"},content:{text:z.innerHTML,html:!0},mask:{enable:!0,clickEvent:{toClose:!0}},style:`
						.pops-alert-content{
							font-style: italic;
						...
						`,drag:!0,dragLimit:!0,width:Z.info.width,height:Z.info.height});return}const k=new FormData(M);if(!n(k,e)){p.error("添加失败，提交的添加请求中不包含该脚本id"),g.error(r.t("添加失败，表单数据中不包含该脚本"));return}await F.updateUserSetsInfo(t,a,P),g.success(r.t("添加成功"));}catch(d){console.error(d);}finally{l.close();}}),c.on(i.$shadowRoot,"click",".collect-delete-script-id",async function(s){const o=s.target.closest(".user-collect-item"),a=o.dataset.id;if(h.isNull(a)){g.error(r.t("获取收藏集id失败"));return}o.dataset.name;const l=g.loading(r.t("删除中..."));try{const d=await F.getUserCollectionInfo(t,a);if(!d)return;if(!n(d,e)){g.info(r.t("已删除：{{scriptId}}",{scriptId:e}));return}const u=h.cloneFormData(d,(k,w)=>k==="scripts-included[]"&&typeof w=="string"&&w.toString().trim()===e.toString().trim()),f=h.cloneFormData(u);u.set("remove-scripts-included[]",e.toString()),u.set("remove-selected-scripts","i"),u.delete("script-action"),f.set("save","1");const v=new URLSearchParams(u),y=new URLSearchParams(f),x=Array.from(v).map(([k,w])=>`${encodeURIComponent(k)}=${encodeURIComponent(w)}`).join("&"),P=Array.from(y).map(([k,w])=>`${encodeURIComponent(k)}=${encodeURIComponent(w)}`).join("&");p.info("删除的数据",x),p.info("保存的数据",P);const I=await F.updateUserSetsInfo(t,a,x);if(!I)return;const M=I.querySelector(".change-script-set");if(!M){g.error(r.t("删除失败，{{selector}}元素不存在",{selector:".change-script-set"}));return}const T=new FormData(M);if(n(T,e)){p.error("删除失败，提交的删除请求中包含该脚本id"),g.error(r.t("删除失败，表单数据中仍包含该脚本"));return}await F.updateUserSetsInfo(t,a,P),g.success(r.t("删除成功"));}catch(d){console.error(d);}finally{l.close();}});},Lt={init(){K.isCode()?bt.init():K.isVersion()&&Ct.init(),K.isCodeStrict()&&(m.execMenuOnce("fullScreenOptimization",()=>{this.fullScreenOptimization();}),m.execMenuOnce("addCopyCodeButton",()=>{this.addCopyCodeButton();})),m.execMenuOnce("addCollectionButton",()=>{this.addCollectionButton();}),m.execMenuOnce("addFindReferenceButton",()=>{this.setFindCodeSearchBtn();}),c.onReady(()=>{m.execMenuOnce("scriptHomepageAddedTodaySUpdate",()=>{this.scriptHomepageAddedTodaySUpdate();});});},addCollectionButton(){p.info("添加收藏按钮"),c.waitNode("ul#script-links li.current span").then(()=>{const e=c.createElement("li",{innerHTML:`
					<a href="javascript:;">
						<span>${r.t("收藏")}</span>
					</a>`});c.append(N("ul#script-links"),e),c.on(e,"click",()=>{const t=O.getScriptId(window.location.pathname);if(!t){p.error(window.location.pathname),g.error(r.t("获取脚本id失败"));return}qe(t);});});},fullScreenOptimization(){p.info("F11全屏，F键代码全屏"),D(`
    ...
    .code-wide-screen{
        ...
        `);let e=false;c.onKeydown(R,function(t){if(t.key.toLowerCase()==="f"){const A=N(".monaco-editor")||N("#script-content div.code-container code");t.altKey&&t.shiftKey?(p.info("宽屏"),c.preventEvent(t),A.classList.contains("code-wide-screen")?A.classList.remove("code-wide-screen"):A.classList.add("code-wide-screen")):!t.altKey&&!t.ctrlKey&&!t.shiftKey&&!t.metaKey&&(p.info("全屏"),c.preventEvent(t),e?(h.exitFullScreen(A),e=false):(h.enterFullScreen(A),e=true));}},{capture:true});},setFindCodeSearchBtn(){p.info("设置代码搜索按钮(对于库)"),c.waitNode("ul#script-links li.current span").then(()=>{const e=c.createElement("li",{innerHTML:`
					<a href="javascript:;">
						<span>${r.t("寻找引用")}</span>
					</a>`});c.append(N("ul#script-links"),e),c.on(e,"click",async function(){let t=O.getScriptId(window.location.pathname);if(!t){p.error(window.location.pathname),g.error(r.t("获取脚本id失败"));return}window.location.href=O.getCodeSearchUrl(`/scripts/${t}`);});});},async scriptHomepageAddedTodaySUpdate(){if(!N("#install-area"))return;p.info("脚本首页新增【今日检查】");const e=await F.getScriptStats(O.getScriptId());if(!e)return;p.info("统计信息",e);const t=e[h.formatTime(void 0,"yyyy-MM-dd")];if(!t){p.error("今日份的统计信息不存在");return}const A=t.update_checks;p.info("今日统计信息",t),c.after("dd.script-show-daily-installs",c.createElement("dt",{className:"script-show-daily-update_checks",innerHTML:`<span>${r.t("今日检查")}</span>`})),c.after("dt.script-show-daily-update_checks",c.createElement("dd",{className:"script-show-daily-update_checks",innerHTML:"<span>"+A+"</span>"}));},addCopyCodeButton(){p.info("添加复制代码按钮"),c.waitNode("div#script-content div.code-container").then(e=>{const t=c.createElement("button",{textContent:r.t("复制代码")},{style:"margin-bottom: 1em;"});c.on(t,"click",async function(){const A=g.loading(r.t("加载文件中...")),n=O.getScriptId(),i=await F.getScriptInfo(n);if(!i){A.close();return}const s=i.code_url;p.success("代码地址：",s);const o=await V.get(s);if(!o.status){A.close();return}A.close(),h.copy(o.data.responseText),g.success(r.t("复制成功"));}),c.before(e,t);});}},ae={key:"gf-shield-rule",init(){p.info("脚本过滤");let e=new h.LockFunction(()=>{this.filter();},50);c.onReady(()=>{if(De.isCurrentLoginUserHome()){p.warn("当前在已登录的账户主页下，禁用脚本过滤");return}h.mutationObserver(document.body,{config:{subtree:true,childList:true},immediate:true,callback:()=>{e.run();}}),e.run();});},getElementList(){return [...Array.from(X("ol.script-list li[data-script-name]"))]},filter(){this.getElementList().forEach(e=>{let t=le.parseScriptListInfo(e),A=this.getValue().split(`
`);for(let n=0;n<A.length;n++){let i=A[n],s=i.split("##"),o=s[0],a=s[1];if(o==="scriptRatingScore"){let l=parseFloat(a.slice(1));if(a.startsWith(">")){if(t.scriptRatingScore>l){p.info("触发脚本过滤规则",[i,t]),e.remove();break}}else if(a.startsWith("<")&&t.scriptRatingScore<l){p.info("触发脚本过滤规则",[i,t]),e.remove();break}}else if(o in t||o==="scriptDescription"){if(typeof a!="string")continue;let l=new RegExp(a,"ig");if(String(t[o]).match(l)){p.info("触发脚本过滤规则",i,t),e.remove();break}}}});},setValue(e){m.setValue(this.key,e);},addValue(e,t){let A=this.getValue();if(A.trim()!==""&&(A+=`
`),h.isNull(e))return;e=e.toString().trim();let n=e+"##"+t;A+=n,this.setValue(A);},getValue(){return m.getValue(this.key,"")}},Pt=`.sidebarred-main-content {\r
  max-width: unset;\r
  flex: unset;\r
}\r
ol.script-list {\r
...
ol.script-list li:only-child {\r
  width: fit-content;\r
  width: -moz-fit-content;\r
}\r
...
@media screen and (min-width: 768px) {\r
  ol.script-list {\r
    display: grid;\r
    grid-template-columns: repeat(auto-fill, minmax(45%, 1fr));\r
  }\r
}\r
`,Me={getTampermonkey:()=>R.external?.Tampermonkey,getViolentmonkey:()=>R.external?.Violentmonkey,getScriptCat:()=>R.external?.Scriptcat,getScriptContainerStatus(){const e={Tampermonkey:false,Violentmonkey:false,ScriptCat:false};return R.external?.Tampermonkey&&(e.Tampermonkey=true),R.external?.Violentmonkey&&(e.Violentmonkey=true),R.external?.Scriptcat&&(e.ScriptCat=true),e},getRegisterScriptContainerNameList(){const t=this.getScriptContainerStatus(),A=[];return Object.keys(t).forEach(n=>{Reflect.get(t,n)&&A.push(n);}),A},getInstalledVersion(e,t){return new Promise((A,n)=>{const i=this.getTampermonkey();if(i){i.isInstalled(e,t,function(a){a.installed?A(a.version):A(null);});return}const s=this.getViolentmonkey();if(s){s.isInstalled(e,t).then(A);return}const o=this.getScriptCat();if(o){o.isInstalled(e,t,function(a){a.installed?A(a.version):A(null);});return}n(new TypeError("获取脚本容器暴露的external信息失败"));})},compareVersions(e,t){if(e===t)return 0;const A=e.split("."),n=t.split(".");for(let i=0;i<A.length;i++){const s=this.compareVersionPart(A[i],n[i]);if(s!==0)return s}return 0},compareVersionPart(e,t){const A=this.parseVersionPart(e),n=this.parseVersionPart(t);for(let i=0;i<A.length;i++){if(A[i].length>0&&n[i].length===0)return  -1;if(A[i].length===0&&n[i].length>0||A[i]>n[i])return 1;if(A[i]<n[i])return  -1}return 0},parseVersionPart(e){if(!e)return [0,"",0,""];const t=/([0-9]*)([^0-9]*)([0-9]*)([^0-9]*)/.exec(e);return [t[1]?parseInt(t[1]):0,t[2],t[3]?parseInt(t[3]):0,t[4]]},handleInstallResult(e,t,A){if(t!=null)switch(e.removeAttribute("data-ping-url"),this.compareVersions(t,A)){case  -1:e.textContent=e.getAttribute("data-update-label");break;case 1:e.textContent=e.getAttribute("data-downgrade-label");break;case 0:e.textContent=e.getAttribute("data-reinstall-label");break}},async checkForUpdatesJS(e,t){const A=e.getAttribute("data-script-name"),n=e.getAttribute("data-script-namespace"),i=e.getAttribute("data-script-version");try{let s=await this.getInstalledVersion(A,n);return s==null?!1:(this.handleInstallResult(e,s,i),!0)}catch{if(t){await h.sleep(1e3);try{return await this.checkForUpdatesJS(e,!1)}catch{}}return  false}},checkForUpdatesCSS(e){const t=e.getAttribute("data-script-name"),A=e.getAttribute("data-script-namespace");postMessage({type:"style-version-query",name:t,namespace:A,url:location.href},location.origin);}},Xe={init(){m.execMenuOnce("gf-scripts-filter-enable",()=>{ae.init();}),m.execMenuOnce("beautifyCenterContent",()=>this.beautifyCenterContent());},beautifyCenterContent(){p.info("美化脚本列表-双列");let e=[];e.push(D(Pt));const t="lum-lightbox-loader",A=r.t("安装此脚本");return j.onReady(()=>{let n=Me.getScriptContainerStatus(),i=Object.values(n).find(d=>d),s=Me.getRegisterScriptContainerNameList(),o=m.getValue("beautifyCenterContent-queryNameSpace");i?p.info("当前暴露的external信息："+s.map(d=>`【${d}】`).join("、")):p.error("脚本容器未暴露external信息",window.external);let a=new h.LockFunction(()=>{ae.getElementList().forEach(u=>{if(u.querySelector(".script-list-operation"))return;let f=le.parseScriptListInfo(u),v=u.querySelector(".inline-script-stats");if(!v)return;let y=f.codeUrl,x=j.createElement("dt",{className:"script-list-rating-score",innerHTML:`<span>${r.t("评分")}</span>`}),P=j.createElement("dd",{className:"script-list-rating-score",innerHTML:`<span>${f.scriptRatingScore}</span>`},{"data-position":"right"}),I=u.querySelector("dd.script-list-ratings .good-rating-count"),M=u.querySelector("dd.script-list-ratings .ok-rating-count"),T=u.querySelector("dd.script-list-ratings .bad-rating-count");if(I&&M&&T){let C=parseInt(I.innerText),E=parseInt(M.innerText),L=parseInt(T.innerText),H=C+E+L;H>=10?C/H>=.6?P.classList.add("good-rating-count"):P.classList.add("bad-rating-count"):H==0||C>E+L?P.classList.add("good-rating-count"):P.classList.add("bad-rating-count");}let z=j.createElement("dt",{className:"script-list-version",innerHTML:`<span>${r.t("版本")}</span>`}),k=j.createElement("dd",{className:"script-list-version",innerHTML:`<span>${f.scriptVersion}</span>`},{"data-position":"right"}),w=j.createElement("dt",{className:"script-list-operation",innerHTML:`<span>${r.t("操作")}</span>`}),b=j.createElement("dd",{className:"script-list-operation",innerHTML:`
						<a
							target="_blank"
							class="install-link"
							data-install-format="js"
							data-script-name="${f.scriptName}"
							data-script-namespace=""
							data-script-version="${f.scriptVersion}"
							data-update-label="${r.t("更新到 {{version}} 版本",{version:f.scriptVersion})}"
							data-downgrade-label="${r.t("降级到 {{version}} 版本",{version:f.scriptVersion})}"
							data-reinstall-label="${r.t("重新安装 {{version}} 版本",{version:f.scriptVersion})}"
							href="${y}"></a>
						<button class="script-collect-btn">${r.t("收藏")}</button>
						`},{"data-position":"right",style:"gap:10px;display: flex;flex-wrap: wrap;align-items: center;"}),U=b.querySelector(".script-collect-btn"),B=b.querySelector(".install-link");if(B["data-script-info"]=f,j.addClass(B,t),f.scriptType==="library"&&B.remove(),j.on(U,"click",C=>{j.preventEvent(C),qe(f.scriptId);}),m.getValue("gf-scripts-filter-enable")){let C=j.createElement("button",{className:"script-filter-btn",innerHTML:r.t("过滤")}),E="data-filter-key",L="data-filter-value";j.on(C,"click",H=>{j.preventEvent(H);let G=Q.alert({title:{text:r.t("选择需要过滤的选项"),position:"center"},content:{text:`
									<button ${E}="scriptId" ${L}="^${f.scriptId}$">${r.t("脚本id：{{text}}",{text:f.scriptId})}</button>
									<button ${E}="scriptName" ${L}="^${h.toRegExpStr(f.scriptName)}$">${r.t("脚本名：{{text}}",{text:f.scriptName})}</button>
								...
								`}),Ae=G.$shadowRoot.querySelector(".pops-alert-content");f.scriptAuthors.forEach(W=>{let re=j.createElement("button",{innerHTML:r.t("作者id：{{text}}",{text:W.authorId})});re.setAttribute(E,"scriptAuthorId"),re.setAttribute(L,"^"+W.authorId+"$");let Y=j.createElement("button",{innerHTML:r.t("作者名：{{text}}",{text:W.authorName})});Y.setAttribute(E,"scriptAuthorName"),Y.setAttribute(L,"^"+h.toRegExpStr(W.authorName)+"$"),Ae.appendChild(re),Ae.appendChild(Y);}),j.on(G.$shadowRoot,"click",`button[${E}]`,W=>{j.preventEvent(W);let re=W.target,Y=re.getAttribute(E),me=re.getAttribute(L);ae.addValue(Y,me),G.close(),ae.filter(),g.success(r.t("添加成功"));});}),b.appendChild(C);}v.appendChild(x),v.appendChild(P),v.appendChild(z),v.appendChild(k),v.appendChild(w),v.appendChild(b);});},100),l=new h.LockFunction(async()=>{let d=Array.from(X(".install-link[data-install-format=js]:not([gm-is-check-install-status])"));for(let u=0;u<d.length;u++){const f=d[u];f.setAttribute("gm-is-check-install-status","");let v=Reflect.get(f,"data-script-info");if(i){if(o){let y=await F.getScriptInfo(v.scriptId);y&&f.setAttribute("data-script-namespace",y.namespace);}Me.checkForUpdatesJS(f,true).then(y=>{j.removeClass(f,t),y||j.text(f,A);});}else j.removeClass(f,t),j.text(f,A);}});h.mutationObserver(document,{config:{subtree:true,childList:true},immediate:true,callback:()=>{a.run(),l.run();}});}),e}},Te={waitScritList(){return c.waitNode("#browse-script-list",1e4)},addFilterControls(e){m.getValue("beautifyCenterContent")&&D(`
        .sidebarred .sidebarred-main-content{
          width: 100%;
        }
      `);function t(){let l=N("#gm-script-filter-controls");if(!l)return;let d=l?.shadowRoot;if(!d)return;let u=d.querySelector(".pops-filter-controls_inner"),f=d.querySelector(".pops-filter-search-container");if(u&&f)return {$filterControl:u,$search:f}}let A=t();if(A)return A;let n=c.createElement("div",{id:"gm-script-filter-controls"}),i=n.attachShadow({mode:"open"});i.appendChild(c.createElement("style",{innerHTML:`
                    ${Q.config.cssText.index}

                    ${Q.config.cssText.common}

                    ${Q.config.cssText.panelCSS}
                `})),i.appendChild(c.createElement("style",{innerHTML:`
        .pops{
          display: flex;
          flex-direction: column;
          gap: 10px;
        ...
        .pops .pops-panel-item-left-main-text{
          ...
          padding: 0px;
				...
				.gm-script-search-input{
					...
					align-items: center;
					gap: 20px;
				}`}));const s=c.createElement("div",{className:"pops pops-filter-controls-container",innerHTML:`
				<div class="pops-filter-search-container"></div>
				<div class="pops-filter-controls_inner"></div>
			`}),o=s.querySelector(".pops-filter-controls_inner"),a=s.querySelector(".pops-filter-search-container");return i.appendChild(s),c.before(e,n),{$filterControl:o,$search:a}}},zt={init(){c.onReady(()=>{Te.waitScritList().then(e=>{if(!e){p.error("未找到脚本列表节点，无法继续执行");return}m.onceExec("GreasyforkScriptsSearch",()=>{let{$filterControl:t,$search:A}=Te.addFilterControls(e);this.addFilterControlsItem(A,t);});});});},addFilterControlsItem(e,t){let A=Q.config.PanelHandlerComponents(),n=[{name:r.t("名称"),desc:r.t("全词匹配"),ENABLE_KEY:"gf-script-search-filterScriptTitleWholeWordMatching",STORAGE_KEY:"gf-script-search-filterScriptTitleWholeWordMatching-enable",callback:(o,a)=>{try{return !a.scriptName.match(new RegExp(o,"i"))}catch(l){return p.error(l),!a.scriptName.match(o)}},mutualExclusionSwitch:['.gm-script-control-item[data-storage-key="gf-script-search-filterScriptTitleOrDescWholeWordMatching-enable"]']},{name:r.t("描述"),desc:r.t("全词匹配"),ENABLE_KEY:"gf-script-search-filterScriptDescWholeWordMatching",STORAGE_KEY:"gf-script-search-filterScriptDescWholeWordMatching-enable",callback:(o,a)=>{try{return !a.scriptDescription.match(new RegExp(o,"i"))}catch{return !a.scriptDescription.match(o)}},mutualExclusionSwitch:['.gm-script-control-item[data-storage-key="gf-script-search-filterScriptTitleOrDescWholeWordMatching-enable"]']},{name:r.t("名称/描述"),desc:r.t("任一全词匹配"),ENABLE_KEY:"gf-script-search-filterScriptTitleOrDescWholeWordMatching",STORAGE_KEY:"gf-script-search-filterScriptTitleOrDescWholeWordMatching-enable",callback:(o,a)=>{let l=n.find(u=>u.ENABLE_KEY==="gf-script-search-filterScriptTitleWholeWordMatching").callback(o,a),d=n.find(u=>u.ENABLE_KEY==="gf-script-search-filterScriptDescWholeWordMatching").callback(o,a);return l&&d},mutualExclusionSwitch:['.gm-script-control-item[data-storage-key="gf-script-search-filterScriptTitleWholeWordMatching-enable"]','.gm-script-control-item[data-storage-key="gf-script-search-filterScriptDescWholeWordMatching-enable"]']},{name:r.t("作者名称"),desc:r.t("全词匹配"),ENABLE_KEY:"gf-script-search-filterScriptAuthorNameWholeWordMatching",STORAGE_KEY:"gf-script-search-filterScriptAuthorNameWholeWordMatching-enable",callback:(o,a)=>{try{return !a.scriptAuthorName.match(new RegExp(o,"i"))}catch{return !a.scriptAuthorName.match(o)}}}],i=()=>{let o=e.querySelector("input");if(o)return o.value;let a=new URLSearchParams(window.location.search);return a.get("q")?.trim()||a.get("c")?.trim()||""},s=()=>{let o=i();ae.getElementList().forEach(l=>{let d=le.parseScriptListInfo(l),u=n.map(f=>{if(o=="")return  false;if(ee(f.STORAGE_KEY))return f.callback(o,d)}).filter(f=>typeof f=="boolean");if(u.length!==0){let f=false;u.forEach(v=>{f=f||v;}),f?c.hide(l,false):c.show(l,false);}else c.show(l,false);});};if(m.getValue("gf-script-search-addFilterSearchInput")){let o=A.createSectionContainerItem_input({type:"input",className:"gm-script-search-input",getValue(){let a=i();return m.getValue("gf-script-search-addFilterSearchInput-text")||a||""},description:r.t("自动执行过滤"),placeholder:r.t("过滤的关键词"),text:r.t("关键词"),callback:h.debounce((a,l)=>{m.setValue("gf-script-search-addFilterSearchInput-text",l.toString()),s();},500)}).$el;c.append(e,o);}n.forEach(o=>{if(!m.getValue(o.ENABLE_KEY))return;p.info(`添加按钮${o.name}`);let a=A.createSectionContainerItem_switch({type:"switch",className:"gm-script-control-item",text:o.name,description:o.desc,attributes:{"data-storage-key":o.STORAGE_KEY},getValue(){let l=ee(o.STORAGE_KEY,false);return l=!!l,s(),l},callback(l,d){if($(o.STORAGE_KEY,d),s(),d&&Array.isArray(o.mutualExclusionSwitch)){let u=a.closest(".pops");o.mutualExclusionSwitch.forEach(f=>{let v=u.querySelector(f);v&&v.querySelector("input")?.checked&&v.querySelector(".pops-panel-switch__core")?.click();});}}}).$el;c.append(t,a);});}},Mt=`section.text-content {\r
...
`,kt={init(){m.execMenuOnce("conversations-beautifyDialogBox",()=>this.beautifyDialogBox()),c.onReady(()=>{m.execMenuOnce("conversations-beautifyPrivateMessageList",()=>{this.beautifyPrivateMessageList();});});},beautifyDialogBox(){return p.info("美化对话框"),D(Mt)},beautifyPrivateMessageList(){p.info("美化私信列表"),D(`
		...
		`),X("section.text-content ul li").forEach(e=>{let t=e.querySelector('a[href*="conversations"]'),A=t.href,n=t.textContent?.split(" ")[1],i=e.querySelector("a.user-link"),s=null,o=null,a=null;if(i){s=i.textContent,o=i.href;let l=e.querySelector("relative-time");new Date(l.getAttribute("datetime")),a=l.shadowRoot.lastChild.textContent;}e.classList.add("user-conversations-item"),O.getUserId(),e.innerHTML=`
			<div class="user-link-container">
				<a class="user-link" href="${A}">${n}</a>
				<span class="user-latest-send-time">${a}</span>
			</div>
			...
			`;});}},St={init(){m.execMenuOnce("users-changeConsoleToTopNavigator",()=>{this.changeConsoleToTopNavigator();}),m.execMenuOnce("gf-scripts-filter-enable",()=>{ae.init();}),m.execMenuOnce("beautifyCenterContent",()=>Xe.beautifyCenterContent());},changeConsoleToTopNavigator(){p.info("迁移【控制台】到顶部导航栏"),_.addBlockCSS("#about-user"),c.onReady(()=>{let e=N("#about-user");if(!e){p.error("#about-user元素不存在");return}e=e.cloneNode(true),le.registerTopNavMenu({name:r.t("控制台"),className:"scripts-console",clickEvent(t){Q.alert({title:{text:r.t("控制台"),position:"center"},content:{text:"",html:true},btn:{ok:{enable:false}},mask:{enable:true,clickEvent:{toClose:true}},drag:true,useShadowRoot:true,width:Z.setting.width,height:Z.setting.height,zIndex:h.getMaxZIndex(100),style:`
						#about-user{
							border: 0;
							box-shadow: none;
						}
						...
						`}).$shadowRoot.querySelector(".pops-alert-content").appendChild(e);}});});}},Et={init(){if(K.isImageSource()){p.info("Router: 资源界面，不执行脚本功能");return}m.execMenu("checkPage",()=>{this.checkPage();}),vt.init(),Re.init(),K.isScript()&&Lt.init(),(K.isScriptList()||K.isScriptLibraryList()||K.isScriptCodeSearch()||K.isScriptsBySite())&&Xe.init(),K.isDiscuessions()?(p.info("Router: 讨论页面"),wt.init()):K.isUsers()?(p.info("Router: 用户页面"),St.init(),K.isUsersConversations()&&(p.info("Router-next: 私聊用户页面"),kt.init())):(K.isScriptSearch()||K.isScriptsBySite()||K.isScriptCodeSearch()||K.isScriptLibraryListSearch())&&(p.info("Router: 脚本搜索页面"),zt.init()),m.execMenuOnce("scripts-addOperationPanelBtnWithNavigator",()=>{this.addOperationPanelBtnWithNavigator();}),c.onReady(()=>{q.initEnv(),mt.init(),Se.init(),q.handleLocalGotoCallBack(),m.execMenuOnce("fixImageWidth",()=>this.fixImageWidth()),this.languageSelectorLocale(),m.execMenuOnce("optimizeImageBrowsing",()=>{this.optimizeImageBrowsing();}),m.execMenuOnce("overlayBedImageClickEvent",()=>{this.overlayBedImageClickEvent();}),K.isCodeStrict()||m.execMenuOnce("addMarkdownCopyButton",()=>{this.addMarkdownCopyButton();}),m.execMenuOnce("queryUserRegisterTime",()=>this.queryUserRegisterTime());});},fixImageWidth(){if(window.innerWidth<window.innerHeight)return p.info("修复图片显示问题"),D(`
            img.lum-img{
                width: 100% !important;
                height: 100% !important;
            }
          `)},optimizeImageBrowsing(){p.info("优化图片浏览"),D(ke("ViewerCSS")),D(`
        ...
        `);function e(A=[],n=0){let i="";A.forEach(a=>{i+=`<li><img data-src="${a}" loading="lazy"></li>`;});let s=c.createElement("ul",{innerHTML:i}),o=new Ve(s,{inline:false,url:"data-src",zIndex:h.getMaxZIndex()+100,hidden:()=>{o.destroy();}});n=n<0?0:n,o.view(n),o.zoomTo(1),o.show();}function t(A){return A.getAttribute("data-src")||A.getAttribute("src")||A.getAttribute("alt")}c.on(document,"click","img",function(A){let n=A.target;if(n.parentElement?.localName==="a"&&n.hasAttribute("data-screenshots")||n.closest(".viewer-container")||n.closest(".lum-lightbox-position-helper"))return;let i=n.closest(".user-content"),s=[],o=0,a=[],l=t(n);if(l){if(l.startsWith("https://img.shields.io"))return;if(l.startsWith("/vite/assets/"))return}i?(i.querySelectorAll("img").forEach(d=>{a.push(d);let u=t(d),f=d.parentElement;f?.localName==="a"&&(u=f.getAttribute("data-href")||f.href),s.push(u);}),o=a.indexOf(n),o===-1&&(o=0)):(s.push(l),o=0),p.success("点击浏览图片👉",s,o),e(s,o);}),X(".user-screenshots").forEach(A=>{let n=A.querySelector("a");if(!n)return;let i=n.getAttribute("data-href")||n.getAttribute("href"),s=A.querySelector("img");s&&(s.setAttribute("data-screenshots","true"),s.setAttribute("data-src",i),n.setAttribute("href","javascript:;"),c.after(n,s),n.remove());});},overlayBedImageClickEvent(){p.info("覆盖图床图片的parentElement的a标签"),X(".user-content a>img").forEach(e=>{const t=e.parentElement,A=t.getAttribute("href");if(t.setAttribute("data-href",A),t.removeAttribute("href"),A.startsWith("/rails/active_storage/blobs/redirect")){p.info("该图片是上传到Greasyfork的图片，拦截默认行为，不做提示");return}c.on(t,"click",()=>{g.warning(`<div style="overflow-wrap: anywhere;">${r.t("拦截跳转：")}<a href="${A}" target="_blank">${A}</a></div>`,{isHTML:true,zIndex:h.getMaxZIndex()+105});});});},addMarkdownCopyButton(){p.info("在Markdown右上角添加复制按钮"),D(`
        pre{
          position: relative;
        ...
        `);function e(){let t=c.createElement("div",{className:"zeroclipboard-container",innerHTML:`
            ...
            `}),A=t.querySelector(".js-clipboard-copy"),n=t.querySelector(".octicon-copy"),i=t.querySelector(".octicon-check-copy");return c.on(t,"click",()=>{let s=c.parent(t),o=s.querySelector("code");if(!o&&s.className.includes("prettyprinted")&&(o=t.parentElement),o||(o=s.querySelector("pre")),!o){g.error(r.t("未找到{{selector}}元素",{selector:"code"}));return}h.copy(c.text(o)),A.setAttribute("success","true"),n.setAttribute("aria-hidden","true"),i.removeAttribute("aria-hidden");let a=Q.tooltip({$target:A,content:r.t("✅ 复制成功!"),position:"left",className:"github-tooltip",alwaysShow:true});a.toolTip.onToolTipAnimationFinishEvent(),setTimeout(()=>{A.removeAttribute("success"),i.setAttribute("aria-hidden","true"),n.removeAttribute("aria-hidden"),a.toolTip.close();},2e3);}),t}X("pre").forEach(t=>{if(t.querySelector("div.zeroclipboard-container"))return;const n=e(),i=c.createElement("div",{className:"snippet-clipboard-content"});c.before(t,i),i.appendChild(t),i.appendChild(n);});},languageSelectorLocale(){const e=m.getValue("language-selector-locale"),t=window.location.pathname.split("/").filter(A=>!!A)[0];if(p.success("选择语言："+e),p.success("当前语言："+t),!h.isNull(e)&&e!==t){let A=null,n=O.getSwitchLanguageUrl(e);F.switchLanguage(n),p.success("新Url："+n),g.loading(r.t("当前语言：{{currentLocaleLanguage}}，，3秒后切换至：{{localeLanguage}}",{currentLocaleLanguage:t,localeLanguage:e}),{timeout:3e3,showClose:true,onClose(){clearTimeout(A);}}),g.info(r.t("导航至：")+n,{timeout:3e3}),A=setTimeout(()=>{window.location.href=n;},3e3);}},checkPage(){p.info("检测gf页面是否正确加载，有时候会出现"),c.onReady(()=>{if(document.body.firstElementChild&&document.body.firstElementChild.localName==="p"&&document.body.firstElementChild.innerText.includes("We're down for maintenance. Check back again soon.")){let e=parseInt(ee("greasyfork-check-page-time",0)),t=m.getValue("greasyfork-check-page-timeout",5),A=t*1e3;if(e&&Date.now()-e<A){g.warning(r.t("上次重载时间 {{time}}，{{timeout}}秒内拒绝反复重载",{time:h.formatTime(e,"yyyy-MM-dd HH:mm:ss"),timeout:t}));return}$("greasyfork-check-page-time",Date.now()),window.location.reload();}});},addOperationPanelBtnWithNavigator(){p.info("添加【操作面板】按钮"),_.addBlockCSS(".sidebarred .sidebar",".sidebarred-main-content .open-sidebar"),D(`
		.sidebarred .sidebarred-main-content{
			max-width: 100%;
		}	
		`),c.onReady(()=>{N("#site-nav nav"),N("#site-nav .with-submenu nav");let e=N("#script-list-option-groups")||N(".list-option-groups");if(!e){p.warn("不存在右侧面板元素#script-list-option-groups");return}e=e.cloneNode(true),e.classList.add("option-panel-groups"),le.registerTopNavMenu({name:r.t("操作面板"),className:"filter-scripts",clickEvent(t){Q.alert({title:{text:r.t("操作面板"),position:"center"},content:{text:"",html:true},btn:{ok:{enable:false}},mask:{enable:true,clickEvent:{toClose:true}},drag:true,useShadowRoot:true,width:Z.setting.width,height:Z.setting.height,zIndex:h.getMaxZIndex(100),style:`
						...
						`}).$shadowRoot.querySelector(".pops-alert-content").appendChild(e);}});});},queryUserRegisterTime(){const e=new Map,t=s=>{X(`.query-wrapper[data-user-id='${s.userId}']`).forEach(o=>{const a=c.prev(o);o.remove(),c.text(a,r.t("{{name}} {{leftBracket}}{{created_at}}{{rightBracket}}",{name:s.name,created_at:s.formatTime,leftBracket:r.t("（"),rightBracket:r.t("）")}));});},A=new h.LockFunction(()=>{X("a[href*='/users/']:not(:has(+.query-wrapper))").forEach(s=>{if(s.closest("#nav-user-info"))return;const o=s.href,a=s.classList.contains("user-link");if(o.match(/\/users\/.+\/.+/)&&!a||o.match(/\#message-[\d]+$/))return;const l=O.getUserId(o);if(l==null)return;if(e.has(l)){const u=e.get(l);t({userId:l,...u});return}const d=c.createElement("div",{className:"query-wrapper",innerHTML:`
            ...
            `},{"data-user-id":l});c.after(s,d);});}),n=h.mutationObserver(document,{config:{subtree:true,childList:true},immediate:true,callback:()=>{A.run();}}),i=c.on(document,"click",".query-badge-control",async(s,o)=>{c.preventEvent(s);const a=c.parent(o),l=a.getAttribute("data-user-id");if(!l){g.error(r.t("获取用户ID失败"));return}const d=a.querySelector(".query-msg");c.text(d,r.t("查询中..."));const u=await F.getUserInfo(l);if(!u){c.text(d,r.t("查询失败"));return}c.empty(d);const{name:f,created_at:v}=u,y=h.formatTime(v,r.t("yyyy年MM月dd日 HH:mm:ss"));e.set(l,{name:f,formatTime:y}),t({userId:l,name:f,formatTime:y});});return [()=>{n.disconnect(),i.off(),c.remove(".query-wrapper"),e.clear();},D(`
        ...
        /* ↑查询按钮 */
      `)]}},ue=function(e,t,A,n,i,s,o,a,l,d){const u={text:e,type:"button",attributes:{},props:{},description:t,buttonIcon:n,buttonIsRightIcon:i,buttonIconIsLoading:s,buttonType:o,buttonText:A,callback(f){typeof a=="function"&&a(f);},afterAddToUListCallBack:l};return Reflect.set(u.attributes,Le,()=>{u.disable=false;}),u},Dt=function(e,t,A,n,i,s="default",o){const a=typeof i=="function"?i():i;typeof n=="object"&&n!=null&&o.initConfig(A,n);const l=()=>o.getShowText(A,a),d=ue(e,t,l,"keyboard",false,false,s,async u=>{const v=u.target.closest(".pops-panel-button")?.querySelector("span");if(o.isWaitKeyboardPress()){g.warning("请先执行当前的录入操作");return}if(o.hasOptionValue(A))o.emptyOption(A),g.success("清空快捷键");else {const y=g.loading("请按下快捷键...",{showClose:true,onClose(){o.cancelEnterShortcutKeys();}}),{status:x,option:P,key:I}=await o.enterShortcutKeys(A);y.close(),x?(p.success("录入快捷键",P),g.success("录入成功")):g.error(`快捷键 ${o.translateKeyboardValueToButtonText(P)} 已被 ${I} 占用`);}c.html(v,l());});return d.attributes={},Reflect.set(d.attributes,Le,()=>false),d},Be=function(e,t,A,n,i,s="",o,a){const l={text:e,type:"input",inputType:"password",attributes:{},props:{},description:n,placeholder:s,afterAddToUListCallBack:o,getValue(){return this.props[ne].get(t,A)},callback(d,u){this.props[ne].set(t,u);}};return Reflect.set(l.attributes,pe,t),Reflect.set(l.attributes,fe,A),Pe.initComponentsStorageApi("input",l,{get(d,u){return m.getValue(d,u)},set(d,u){m.setValue(d,u);}}),l},Ee=function(e,t,A,n,i,s){const o={type:"own",attributes:{},props:{},createLIElement:e,afterAddToUListCallBack:s};return typeof t=="object"&&t!==null&&Object.keys(t).length>0?Reflect.set(o.attributes,Qe,t):Reflect.set(o.attributes,Le,()=>false),typeof A=="object"&&A!==null&&Reflect.set(o.attributes,Ue,A),o},de=function(e,t,A,n,i,s,o){const a={text:e,type:"select",description:s,attributes:{},props:{},getValue(){return this.props[ne].get(t,A)},callback(l){if(l==null)return;const d=l.value;if(p.info(`选择：${l.text}`),typeof i=="function"&&i(l))return;this.props[ne].set(t,d);},data:n};return Reflect.set(a.attributes,pe,t),Reflect.set(a.attributes,fe,A),Pe.initComponentsStorageApi("select",a,{get(l,d){return m.getValue(l,d)},set(l,d){m.setValue(l,d);}}),a},S=function(e,t,A,n,i,s,o,a,l){const d={text:e,type:"switch",description:i,disabled:o,attributes:{},props:{},getValue(){return this.props[ne].get(t,A)},callback(u,f){const v=!!f;p.success(`${v?"开启":"关闭"} ${e}`),this.props[ne].set(t,v);},afterAddToUListCallBack:(...u)=>{}};return Reflect.set(d.attributes,pe,t),Reflect.set(d.attributes,fe,A),Pe.initComponentsStorageApi("switch",d,{get(u,f){return m.getValue(u,f)},set(u,f){m.setValue(u,f);}}),d},Pe={$data:{__storeApiFn:null,get storeApiValue(){return this.__storeApiFn||(this.__storeApiFn=new se.Dictionary),this.__storeApiFn}},getStorageApi(e){if(this.hasStorageApi(e))return this.$data.storeApiValue.get(e)},hasStorageApi(e){return this.$data.storeApiValue.has(e)},setStorageApi(e,t){this.$data.storeApiValue.set(e,t);},initComponentsStorageApi(e,t,A){let n;this.hasStorageApi(e)?n=this.getStorageApi(e):n=A,this.setComponentsStorageApiProperty(t,n);},setComponentsStorageApiProperty(e,t){Reflect.set(e.props,ne,t);}},It=function(e,t,A,n,i,s="",o="text",a,l){const d={text:e,type:"input",inputType:o,attributes:{},props:{},description:n,placeholder:s,afterAddToUListCallBack:a,getValue(){return this.props[ne].get(t,A)},callback(u,f){u.target.validity.valid,this.props[ne].set(t,f);}};return Reflect.set(d.attributes,pe,t),Reflect.set(d.attributes,fe,A),Pe.initComponentsStorageApi("input",d,{get(u,f){return m.getValue(u,f)},set(u,f){m.setValue(u,f);}}),d},je={async UIScriptList(e,t){if(!q.isLogin){g.error(r.t("请先登录账号！"));return}const i=q.getUserLinkElement().href?.split("/")?.pop()?.match(/([0-9]+)/)?.[0],s=Q.loading({mask:{enable:true},$parent:t,content:{text:r.t("获取信息中，请稍后...")},addIndexCSS:false}),o=await F.getUserAllInfo(i);if(s.close(),!o)return;p.info(o);const a=e==="script-list"?o.scriptList:o.scriptLibraryList;g.success(r.t("获取成功，共 {{count}} 个",{count:a.length}));for(const l of a){const d=c.createElement("li",{className:"w-script-list-item",innerHTML:`
            ...
            `}),u=d.querySelector(".w-script-info"),f=c.createElement("div",{className:"pops-panel-button",innerHTML:`
				<button type="button" data-type="primary" data-icon="" data-righticon="false">
				  <span>${r.t("同步代码")}</span>
				</button>
				`});l.deleted&&(d.classList.add("w-script-deleted"),f.querySelector("button").setAttribute("disabled","true")),c.on(f,"click",async function(){p.success("同步",l);const v=f.querySelector("button"),y=f.querySelector("button span"),x=c.createElement("i",{className:"pops-bottom-icon",innerHTML:Q.config.iconSVG.loading},{"is-loading":true});v.setAttribute("disabled","true"),v.setAttribute("data-icon","true"),y.innerText=r.t("同步中..."),c.before(y,x);const P=l?.id,I=await F.getSourceCodeSyncFormDataInfo(P.toString());if(I){const{formData:M,url:T}=I,z="script[script_sync_type_id]",k="script[sync_type]";if(M.has(z)||M.has(k)){let w="";if(M.has(z)){let B=M.get(z);B.toString()==="1"?w=r.t("手动"):B.toString()==="2"?w=r.t("自动"):B.toString()==="3"&&(w="webhook");}else M.has(k)&&(w=M.get(k));const b=d.querySelector(".w-script-sync-type");b?b.querySelector("p").innerText=r.t("同步方式：{{syncMode}}",{syncMode:w}):c.append(u,`
								<div class="w-script-sync-type">
									<p>${r.t("同步方式：{{syncMode}}",{syncMode:w})}
									</p>
								</div>`),await F.sourceCodeSync(l.id.toString(),M,T)?g.success(r.t("同步成功")):g.error(r.t("同步失败"));}else g.error(r.t("该脚本未设置同步信息"));}v.removeAttribute("disabled"),v.removeAttribute("data-icon"),y.innerText=r.t("同步代码"),x.remove();}),d.appendChild(f),t.appendChild(d);}}},Ot={id:"greasy-fork-panel-config-account",title:r.t("通用"),views:[{text:"",type:"container",views:[{text:r.t("Toast配置"),type:"deepMenu",views:[{text:"",type:"container",views:[de(r.t("Toast位置"),"qmsg-config-position","bottom",[{value:"topleft",text:r.t("左上角")},{value:"top",text:r.t("顶部")},{value:"topright",text:r.t("右上角")},{value:"left",text:r.t("左边")},{value:"center",text:r.t("中间")},{value:"right",text:r.t("右边")},{value:"bottomleft",text:r.t("左下角")},{value:"bottom",text:r.t("底部")},{value:"bottomright",text:r.t("右下角")}],e=>{p.info("设置当前Qmsg弹出位置"+e.text);},r.t("Toast显示在页面九宫格的位置")),de(r.t("最多显示的数量"),"qmsg-config-maxnums",3,[{value:1,text:"1"},{value:2,text:"2"},{value:3,text:"3"},{value:4,text:"4"},{value:5,text:"5"}],void 0,r.t("限制Toast显示的数量")),S(r.t("逆序弹出"),"qmsg-config-showreverse",false,void 0,r.t("修改Toast弹出的顺序"))]}]},de(r.t("语言"),"setting-language","zh-CN",[{value:"zh-CN",text:"中文"},{value:"en-US",text:"English"}],e=>{p.info("改变语言："+e.text),r.changeLanguage(e.value);})]},{text:"",type:"container",views:[{text:r.t("账号/密码"),type:"deepMenu",views:[{text:"",type:"container",views:[It(r.t("账号"),"user","",void 0,void 0,r.t("请输入账号")),Be(r.t("密码"),"pwd","",void 0,void 0,r.t("请输入密码")),Be(r.t("secret"),"secret","",r.t("两步验证（2FA）"),void 0,r.t("请输入secret"))]},{text:"",type:"container",views:[S(r.t("自动登录"),"autoLogin",true,void 0,r.t("自动登录当前保存的账号")),ue(r.t("清空账号/密码"),void 0,r.t("点击清空"),void 0,void 0,false,"default",e=>{if(confirm(r.t("确定清空账号和密码？"))){m.deleteValue("user"),m.deleteValue("pwd"),g.success(r.t("已清空账号/密码"));let t=e.target.getRootNode();t.querySelector('li[data-key="user"] .pops-panel-input input').value="",t.querySelector('li[data-key="pwd"] .pops-panel-input input').value="";}})]}]},{text:r.t("功能"),type:"deepMenu",views:[{text:r.t("功能"),type:"container",views:[de(r.t("固定当前语言"),"language-selector-locale","",(function(){const e=[{value:"",text:r.t("无")}];return c.onReady(()=>{X("select.language-selector-locale option").forEach(t=>{const A=t.getAttribute("value");if(A==="help")return;const n=c.text(t).trim();e.push({value:A,text:n});});}),e})()),S(r.t("修复图片宽度显示问题"),"fixImageWidth",true,void 0,r.t("修复图片在移动端宽度超出浏览器宽度问题")),S(r.t("优化图片浏览"),"optimizeImageBrowsing",true,void 0,r.t("使用Viewer浏览图片")),S(r.t("覆盖图床图片跳转"),"overlayBedImageClickEvent",true,void 0,r.t("配合上面的【优化图片浏览】更优雅浏览图片")),S(r.t("添加【操作面板】按钮"),"scripts-addOperationPanelBtnWithNavigator",true,void 0,r.t("在脚本列表页面时为顶部导航栏添加【操作面板】按钮")),S(r.t("给Markdown添加【复制】按钮"),"addMarkdownCopyButton",true,void 0,r.t("在Markdown内容右上角添加【复制】按钮，点击一键复制Markdown内容")),S(r.t("查询用户注册时间"),"queryUserRegisterTime",true,void 0,r.t("在用户名称后面添加查询按钮，点击查询用户注册时间"))]},{text:r.t("检测页面加载"),type:"container",views:[S(r.t("启用"),"checkPage",true,void 0,r.t("检测Greasyfork页面是否正常加载，如加载失败则自动刷新页面")),de(r.t("检测间隔"),"greasyfork-check-page-timeout",5,(()=>{let e=[];for(let t=0;t<5;t++)e.push({value:t+1,text:t+1+"s"});return e})(),void 0,r.t("设置检测上次刷新页面的间隔时间，当距离上次刷新页面的时间超过设置的值，将不再刷新页面"))]}]},{type:"deepMenu",text:r.t("表单"),views:[{type:"container",text:"",views:[S(r.t("记住回复内容"),"rememberReplyContent",true,void 0,r.t("监听表单内的textarea内容改变并存储到indexDB中，提交表单将清除保存的数据，误刷新页面时可动态恢复")),de(r.t("自动清理空间"),"gf-autoClearRememberReplayContent",7,[{text:r.t("不清理"),value:-1},{text:r.t("{{value}} 天",{value:1}),value:1},{text:r.t("{{value}} 周",{value:1}),value:7},{text:r.t("{{value}} 个月",{value:1}),value:30},{text:r.t("{{value}} 个月",{value:2}),value:60},{text:r.t("{{value}} 个月",{value:3}),value:90},{text:r.t("半年"),value:180}],void 0,r.t("根据设置的间隔时间自动清理保存的回复内容")),ue(r.t("数据占用空间：{{size}}",{size:r.t("计算中")}),r.t("当前存储的数据所占用的空间大小"),r.t("清空"),void 0,void 0,void 0,"default",async()=>{await Se.clearAllRememberReplyContent()?g.success(r.t("清理成功")):g.error(r.t("清理失败"));},async(e,t)=>{let A=t.ulElement.querySelector('li[data-key="gf-autoClearRememberReplayContent"]+li .pops-panel-item-left-main-text'),n=await Se.getAllRememberReplyContent(),i="";n.length?i=h.getTextStorageSize(JSON.stringify(n)):i=h.getTextStorageSize(""),A.innerText=r.t("数据占用空间：{{size}}",{size:i});})]}]},{text:r.t("美化"),type:"deepMenu",views:[{text:r.t("全局"),type:"container",views:[S(r.t("美化页面元素"),"beautifyPage",true,void 0,r.t("如button、input、textarea")),S(r.t("美化上传图片按钮"),"beautifyUploadImage",true,void 0,r.t("放大上传区域")),S(r.t("美化顶部导航栏"),"beautifyTopNavigationBar",true,void 0,r.t("可能会跟Greasyfork Beautify脚本有冲突")),S(r.t("美化Greasyfork Beautify脚本"),"beautifyGreasyforkBeautify",false,void 0,r.t('需安装Greasyfork Beautify脚本，<a href="https://greasyfork.org/zh-CN/scripts/446849-greasyfork-beautify" target="_blank">🖐点我安装</a>'))]},{type:"container",text:r.t("脚本列表"),views:[S(r.t("美化脚本列表"),"beautifyCenterContent",true,void 0,r.t("双列显示且添加脚本卡片操作项（安装、收藏）")),S("↑"+r.t("使用namespace查询脚本信息"),"beautifyCenterContent-queryNameSpace",true,void 0,r.t("开启后检测已安装的脚本信息更准确，但是速度会更慢"))]}]},{type:"deepMenu",text:r.t("自定义快捷键"),views:[{type:"container",text:"",views:[Dt(r.t("快捷键发表回复"),r.t("在输入框内按下快捷发表回复，例如：{{key}}",{key:"Ctrl + Enter"}),"gf-quickReply",{keyName:"Enter",keyValue:"13",ohterCodeList:["ctrl"]},r.t("点击录入快捷键"),void 0,Re.shortCut)]}]},{text:r.t("过滤"),type:"deepMenu",views:[{text:`<a target="_blank" href="https://greasyfork.org/zh-CN/scripts/475722-greasyfork%E4%BC%98%E5%8C%96#:~:text=%E8%84%9A%E6%9C%AC%E8%BF%87%E6%BB%A4%E8%A7%84%E5%88%99">${r.t("帮助文档")}</a>`,type:"container",views:[S(r.t("启用"),"gf-scripts-filter-enable",true,void 0,r.t("作用域：脚本、脚本搜索、用户主页")),Ee(e=>{const t=c.createElement("div",{className:"pops-panel-textarea",innerHTML:`
												<textarea placeholder="${r.t("请输入规则，每行一个")}" style="height:200px;"></textarea>`},{style:"width: 100%;"}),A=t.querySelector("textarea");return A.value=ae.getValue(),c.on(A,["input","propertychange"],void 0,h.debounce(function(n){ae.setValue(A.value);},200)),e.appendChild(t),e})]}]}]},{type:"container",text:r.t("脚本管理"),views:[{type:"deepMenu",text:r.t("代码同步"),views:[{text:r.t("代码同步"),type:"container",views:[ue(r.t("源代码同步【脚本列表】"),void 0,r.t("一键同步"),void 0,void 0,false,"primary",e=>{if(!K.isUsers()){m.setValue("goto_updateSettingsAndSynchronize_scriptList",true),q.getUserLinkElement()?(g.success(r.t("前往用户主页")),window.location.href=q.getUserLinkElement().href):g.error(r.t("获取当前已登录的用户主页失败"));return}let t=[];X("#user-script-list-section li a.script-link").forEach(A=>{t=t.concat(O.getAdminUrl(A.href));}),q.updateScript(t);}),ue(r.t("源代码同步【未上架的脚本】"),void 0,r.t("一键同步"),void 0,void 0,false,"primary",e=>{if(!K.isUsers()){m.setValue("goto_updateSettingsAndSynchronize_unlistedScriptList",true),q.getUserLinkElement()?(g.success(r.t("前往用户主页")),window.location.href=q.getUserLinkElement().href):g.error(r.t("获取当前已登录的用户主页失败"));return}let t=[];X("#user-unlisted-script-list li a.script-link").forEach(A=>{t=t.concat(O.getAdminUrl(A.href));}),q.updateScript(t);}),ue(r.t("源代码同步【库】"),void 0,r.t("一键同步"),void 0,void 0,false,"primary",e=>{if(!K.isUsers()){m.setValue("goto_updateSettingsAndSynchronize_libraryScriptList",true),q.getUserLinkElement()?(g.success(r.t("前往用户主页")),window.location.href=q.getUserLinkElement().href):g.error(r.t("获取当前已登录的用户主页失败"));return}let t=[];X("#user-library-script-list li a.script-link").forEach(A=>{t=t.concat(O.getAdminUrl(A.href));}),q.updateScript(t);})]}]},{type:"deepMenu",text:r.t("脚本列表"),views:[],afterEnterDeepMenuCallBack(e,t){je.UIScriptList("script-list",t.$sectionBodyContainer);}},{type:"deepMenu",text:r.t("库"),views:[],afterEnterDeepMenuCallBack(e,t){je.UIScriptList("script-library",t.$sectionBodyContainer);}}]}]},Tt={id:"greasy-fork-panel-config-discussions",title:r.t("论坛"),views:[{text:"",type:"container",views:[{text:r.t("功能"),type:"deepMenu",views:[{text:"",type:"container",views:[Ee(e=>{const t="discussions-readBgColor",A=c.createElement("div",{className:"pops-panel-item-left-text",innerHTML:`
											...
											`}),i=n.querySelector(".pops-color-choose");i.value=m.getValue(t);const s=c.createElement("style");return c.append(document.head,s),c.on(i,["input","propertychange"],o=>{p.info("选择颜色："+i.value),s.innerHTML=`
												.discussion-read{
													background: ${i.value} !important;
												}
												`,m.setValue(t,i.value);}),e.appendChild(A),e.appendChild(n),e},{"discussions-readBgColor":"#e5e5e5"},{text:r.t("自定义已读颜色"),desc:r.t("在讨论内生效")}),S(r.t("添加【过滤】按钮"),"discussions-addShortcutOperationButton",true,void 0,r.t("在每一行讨论的最后面添加【过滤】按钮，需开启过滤功能才会生效")),S(r.t("添加【举报】按钮"),"discussions-addReportButton",true,void 0,r.t("在每一行讨论的最后面添加【举报】按钮"))]}]},{text:r.t("过滤"),type:"deepMenu",views:[{text:`<a target="_blank" href="https://greasyfork.org/zh-CN/scripts/475722-greasyfork%E4%BC%98%E5%8C%96#:~:text=%E8%AE%BA%E5%9D%9B%E8%BF%87%E6%BB%A4%E8%A7%84%E5%88%99">${r.t("帮助文档")}</a>`,type:"container",views:[S(r.t("启用"),"greasyfork-discussions-filter-enable",true,void 0,r.t("开启后下面的过滤功能才会生效")),S(r.t("过滤重复的评论"),"greasyfork-discussions-filter-duplicate-comments",true,void 0,r.t("过滤掉重复的评论数量(≥2)")),Ee(e=>{const t=c.createElement("div",{className:"pops-panel-textarea",innerHTML:`<textarea placeholder="${r.t("请输入规则，每行一个")}" style="height:200px;"></textarea>`},{style:"width: 100%;"}),A=t.querySelector("textarea");return A.value=te.getValue(),c.on(A,["input","propertychange"],void 0,h.debounce(function(n){te.setValue(A.value);},200)),e.appendChild(t),e})]}]}]}]},Bt={id:"greasy-fork-panel-config-scripts",title:r.t("脚本"),views:[{text:"",type:"container",views:[{text:r.t("代码"),type:"deepMenu",views:[{text:"",type:"container",views:[S(r.t("添加复制代码按钮"),"addCopyCodeButton",true,void 0,r.t("更优雅的复制")),S(r.t("快捷键"),"fullScreenOptimization",true,void 0,r.t("【F】键全屏、【Alt+Shift+F】键宽屏")),S(r.t("修复代码行号显示"),"code-repairCodeLineNumber",false,void 0,r.t("修复代码行数超过1k行号显示不全问题")),S("monacoEditor","code-use-monaco-editor",true,void 0,r.t("使用Monaco编辑器"))]}]},{text:r.t("历史版本"),type:"deepMenu",views:[{text:r.t("功能"),type:"container",views:[S(r.t("添加额外的标签按钮"),"scripts-versions-addExtraTagButton",true,void 0,r.t("在版本下面添加【安装】、【查看代码】按钮")),S(r.t("添加代码对比按钮"),"scripts-versions-addCompareCodeButton",true,void 0,r.t("monacoEditor"))]},{text:r.t("美化"),type:"container",views:[S(r.t("美化历史版本页面"),"beautifyHistoryVersionPage",true,void 0,r.t("更直观的查看版本迭代"))]}]}]},{text:"",type:"container",views:[{text:r.t("功能"),type:"deepMenu",views:[{text:"",type:"container",views:[S(r.t("添加【寻找引用】按钮"),"addFindReferenceButton",true,void 0,r.t("在脚本栏添加按钮，一般用于搜索引用该库的相关脚本")),S(r.t("添加【收藏】按钮"),"addCollectionButton",true,void 0,r.t("在脚本栏添加按钮，一般用于快捷收藏该脚本/库")),S(r.t("添加【今日检查】信息块"),"scriptHomepageAddedTodaySUpdate",true,void 0,r.t("在脚本信息栏添加【今日检查】信息块"))]}]}]}]},jt={id:"greasy-fork-panel-config-script-search",title:r.t("搜索"),views:[{type:"container",text:"搜素结果过滤",views:[S(r.t("新增【关键词】搜索框"),"gf-script-search-addFilterSearchInput",true,void 0,r.t("输入自定义关键词后自动执行过滤")),S(r.t("新增【{{buttonText}}】按钮",{buttonText:r.t("名称")}),"gf-script-search-filterScriptTitleWholeWordMatching",true,void 0,r.t("该Checkbox按钮开启后，自动过滤出包含搜索关键词的脚本")),S(r.t("新增【{{buttonText}}】按钮",{buttonText:r.t("描述")}),"gf-script-search-filterScriptDescWholeWordMatching",true,void 0,r.t("该Checkbox按钮开启后，自动过滤出包含搜索关键词的脚本")),S(r.t("新增【{{buttonText}}】按钮",{buttonText:r.t("名称/描述")}),"gf-script-search-filterScriptTitleOrDescWholeWordMatching",true,void 0,r.t("该Checkbox按钮开启后，自动过滤出包含搜索关键词的脚本")),S(r.t("新增【{{buttonText}}】按钮",{buttonText:r.t("作者名称")}),"gf-script-search-filterScriptAuthorNameWholeWordMatching",true,void 0,r.t("该Checkbox按钮开启后，自动过滤出包含搜索关键词的脚本"))]}]},Nt={id:"greasy-fork-panel-config-account",title:r.t("用户"),views:[{text:"",type:"container",views:[{text:r.t("功能"),type:"deepMenu",views:[{text:"",type:"container",views:[S(r.t("迁移【控制台】到顶部导航栏"),"users-changeConsoleToTopNavigator",true,void 0,r.t("将【控制台】按钮移动到顶部导航栏，节省空间"))]}]},{text:r.t("美化"),type:"deepMenu",views:[{text:"",type:"container",views:[S(r.t("美化私信页面"),"conversations-beautifyDialogBox",true,void 0,r.t("美化为左右对话模式")),S(r.t("美化私信列表"),"conversations-beautifyPrivateMessageList",true)]}]}]}]};Ke.addMenuOption({key:"githubUrl2webhookUrl",text:"⚙ "+r.t("Url To WebhookUrl"),autoReload:false,isStoreValue:false,showText(e){return e},callback:()=>{ot.showView();}});m.$data.panelConfig={style:Ye};xe.addContentConfig([Ot,Bt,jt,Tt,Nt]);m.init();Et.init();
