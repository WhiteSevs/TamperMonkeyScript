...
`,he={init(){v.execMenuOnce("beautifyHistoryVersionPage",()=>this.beautifyHistoryVersionPage()),v.execMenuOnce("scripts-versions-addExtraTagButton",()=>{this.addExtraTagButton();}),v.execMenuOnce("scripts-versions-addCompareCodeButton",()=>{this.sourceDiffMonacoEditor();});},beautifyHistoryVersionPage(){u.info("美化 历史版本 页面");let t=[];return t.push(P(ge)),t.push(tt.addBlockCSS(".version-number",".version-date",".version-changelog")),d.ready(function(){let e=z("ul.history_versions");if(!e){h.error(r.t("未找到history_versions元素列表"));return}Array.from(e.children).forEach(A=>{var p,f;let n=A.querySelector(".version-number a").href,i=A.querySelector(".version-number a").innerText,a=(p=A.querySelector(".version-date"))==null?void 0:p.getAttribute("datetime"),o=((f=A.querySelector(".version-changelog"))==null?void 0:f.innerHTML)||"",s=d.createElement("span",{className:"script-version-date",innerHTML:g.formatTime(a,r.t("yyyy年MM月dd日 HH:mm:ss"))}),l=d.createElement("div",{className:"script-tag",innerHTML:`
		...
		`});d.on(i.$shadowRoot,"click",".collect-add-script-id",async function(a){let o=a.target.closest(".user-collect-item"),s=o.dataset.id;o.dataset.name;let l=h.loading(r.t("添加中..."));try{let c=await j.getUserCollectionInfo(e,s);if(!c)return;if(n(c,t)){h.warning(r.t("该脚本已经在该收藏集中"));return}let p=g.cloneFormData(c),f=g.cloneFormData(c);p.set("add-script",t.toString()),p.set("script-action","i"),f.append("scripts-included[]",t.toString()),f.set("save","1");let m=new URLSearchParams(p),w=new URLSearchParams(f),x=Array.from(m).map(([U,q])=>`${encodeURIComponent(U)}=${encodeURIComponent(q)}`).join("&"),b=Array.from(w).map(([U,q])=>`${encodeURIComponent(U)}=${encodeURIComponent(q)}`).join("&");u.info("添加的数据",x),u.info("保存的数据",b);let M=await j.updateUserSetsInfo(e,s,x);if(!M)return;let k=M.querySelector(".change-script-set");if(!k){h.error(r.t("添加失败，{{selector}}元素不存在",{selector:".change-script-set"}));return}let N=k.querySelector("section");if(!N){h.error(r.t("添加失败，{{selector}}元素不存在",{selector:"section"}));return}let Q=N.querySelector(".alert");if(Q){D.alert({title:{text:r.t("添加失败"),position:"center"},content:{text:Q.innerHTML,html:!0},mask:{enable:!0,clickEvent:{toClose:!0}},style:`
						...
						`,drag:!0,dragLimit:!0,width:$.info.width,height:$.info.height});return}let I=new FormData(k);if(!n(I,t)){u.error("添加失败，提交的添加请求中不包含该脚本id"),h.error(r.t("添加失败，表单数据中不包含该脚本"));return}await j.updateUserSetsInfo(e,s,b),h.success(r.t("添加成功"));}catch(c){console.error(c);}finally{l.close();}}),d.on(i.$shadowRoot,"click",".collect-delete-script-id",async function(a){let o=a.target.closest(".user-collect-item"),s=o.dataset.id;o.dataset.name;let l=h.loading(r.t("删除中..."));try{let c=await j.getUserCollectionInfo(e,s);if(!c)return;if(!n(c,t)){h.info(r.t("已删除：{{scriptId}}",{scriptId:t}));return}let p=g.cloneFormData(c,(I,X)=>I==="scripts-included[]"&&typeof X=="string"&&X.toString().trim()===t.toString().trim()),f=g.cloneFormData(p);p.set("remove-scripts-included[]",t.toString()),p.set("remove-selected-scripts","i"),p.delete("script-action"),f.set("save","1");let m=new URLSearchParams(p),w=new URLSearchParams(f),x=Array.from(m).map(([I,X])=>`${encodeURIComponent(I)}=${encodeURIComponent(X)}`).join("&"),b=Array.from(w).map(([I,X])=>`${encodeURIComponent(I)}=${encodeURIComponent(X)}`).join("&");u.info("删除的数据",x),u.info("保存的数据",b);let M=await j.updateUserSetsInfo(e,s,x);if(!M)return;let k=M.querySelector(".change-script-set");if(!k){h.error(r.t("删除失败，{{selector}}元素不存在",{selector:".change-script-set"}));return}let N=new FormData(k);if(n(N,t)){u.error("删除失败，提交的删除请求中包含该脚本id"),h.error(r.t("删除失败，表单数据中仍包含该脚本"));return}await j.updateUserSetsInfo(e,s,b),h.success(r.t("删除成功"));}catch(c){console.error(c);}finally{l.close();}});},me={init(){C.isCode()?ve.init():C.isVersion()&&he.init(),C.isCodeStrict()&&(v.execMenuOnce("fullScreenOptimization",()=>{this.fullScreenOptimization();}),v.execMenuOnce("addCopyCodeButton",()=>{this.addCopyCodeButton();})),v.execMenuOnce("addCollectionButton",()=>{this.addCollectionButton();}),v.execMenuOnce("addFindReferenceButton",()=>{this.setFindCodeSearchBtn();}),d.ready(()=>{v.execMenuOnce("scriptHomepageAddedTodaySUpdate",()=>{this.scriptHomepageAddedTodaySUpdate();});});},addCollectionButton(){u.info("添加收藏按钮"),g.waitNode("ul#script-links li.current span").then(()=>{let t=d.createElement("li",{innerHTML:`
					<a href="javascript:;">
						<span>${r.t("收藏")}</span>
