...
(function (v, O, at, wt, r, Xt, Ht) {
	...
	const Xt__namespace = /*#__PURE__*/_interopNamespaceDefault(Xt);
	...
	var Pt=typeof GM_deleteValue<"u"?GM_deleteValue:void 0,Ct=typeof GM_getResourceText<"u"?GM_getResourceText:void 0,tt=typeof GM_getValue<"u"?GM_getValue:void 0,pt=typeof GM_info<"u"?GM_info:void 0,qt=typeof GM_registerMenuCommand<"u"?GM_registerMenuCommand:void 0,W=typeof GM_setValue<"u"?GM_setValue:void 0,Gt=typeof GM_unregisterMenuCommand<"u"?GM_unregisterMenuCommand:void 0,Vt=typeof GM_xmlhttpRequest<"u"?GM_xmlhttpRequest:void 0,X=typeof unsafeWindow<"u"?unsafeWindow:void 0,Ft=window;const it={waitRemove(...t){t.forEach(e=>{typeof e=="string"&&g.waitNodeList(e).then(A=>{A.forEach(n=>n.remove());});});},createBlockCSSNode(...t){let e=[];if(t.length!==0&&!(t.length===1&&typeof t[0]=="string"&&t[0].trim()===""))return t.forEach(A=>{Array.isArray(A)?e=e.concat(A):e.push(A);}),O.createElement("style",{type:"text/css",innerHTML:`${e.join(`,
`)}{display: none !important;}`})},addBlockCSS(...t){let e=[];if(t.length!==0&&!(t.length===1&&typeof t[0]=="string"&&t[0].trim()===""))return t.forEach(A=>{Array.isArray(A)?e=e.concat(A):e.push(A);}),M(`${e.join(`,
`)}{display: none !important;}`)},setGMResourceCSS(t){let e=typeof Ct=="function"?Ct(t.keyName):null;typeof e=="string"&&e?M(e):it.loadStyleLink(t.url);},async loadStyleLink(t){let e=document.createElement("link");e.rel="stylesheet",e.type="text/css",e.href=t,O.ready(()=>{document.head.appendChild(e);});},async loadScript(t){let e=document.createElement("script");return e.src=t,new Promise(A=>{e.onload=()=>{A(null);},(document.head||document.documentElement).appendChild(e);})},fixUrl(t){return t=t.trim(),t.match(/^http(s|):\/\//i)?t:t.startsWith("//")?(t.startsWith("///")||(t=window.location.protocol+t),t):(t.startsWith("/")||(t+="/"),t=window.location.origin+t,t)},fixHttps(t){if(t.startsWith("https://")||!t.startsWith("http://"))return t;let e=new URL(t);return e.protocol="https:",e.toString()},lockScroll(...t){let e=document.createElement("style");e.innerHTML=`
			.pops-overflow-hidden-important {
				overflow: hidden !important;
			}
		`;let A=[document.documentElement,document.body].concat(...t||[]);return A.forEach(n=>{n.classList.add("pops-overflow-hidden-important");}),(document.head||document.documentElement).appendChild(e),{recovery(){A.forEach(n=>{n.classList.remove("pops-overflow-hidden-important");}),e.remove();}}},async getClipboardText(){function t(n){navigator.clipboard.readText().then(i=>{n(i);}).catch(i=>{u.error("读取剪贴板内容失败👉",i),n("");});}function e(n){navigator.permissions.query({name:"clipboard-read"}).then(i=>{t(n);}).catch(i=>{u.error("申请剪贴板权限失败，尝试直接读取👉",i.message??i.name??i.stack),t(n);});}function A(){return !(typeof navigator?.clipboard?.readText!="function"||typeof navigator?.permissions?.query!="function")}return new Promise(n=>{if(!A()){n("");return}document.hasFocus()?e(n):window.addEventListener("focus",()=>{e(n);},{once:true});})},escapeHtml(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;").replace(/©/g,"&copy;").replace(/®/g,"&reg;").replace(/™/g,"&trade;").replace(/→/g,"&rarr;").replace(/←/g,"&larr;").replace(/↑/g,"&uarr;").replace(/↓/g,"&darr;").replace(/—/g,"&mdash;").replace(/–/g,"&ndash;").replace(/…/g,"&hellip;").replace(/ /g,"&nbsp;").replace(/\r\n/g,"<br>").replace(/\r/g,"<br>").replace(/\n/g,"<br>").replace(/\t/g,"&nbsp;&nbsp;&nbsp;&nbsp;")},interval(t,e,A=5e3){let n,i=A-e,a=e,o=async l=>{let s=await t(l);if(typeof s=="boolean"&&!s||l){g.workerClearTimeout(n);return}if(a+=e,a>i){o(true);return}n=g.workerSetTimeout(()=>{o(false);},e);};o(false);},findParentNode(t,e,A){if(A){let n=O.closest(t,A);if(n)return n.querySelector(e)}else return O.matches(t,e)?t:O.closest(t,e)}},ct={qmsg_config_position:{key:"qmsg-config-position",defaultValue:"bottom"},qmsg_config_maxnums:{key:"qmsg-config-maxnums",defaultValue:3},qmsg_config_showreverse:{key:"qmsg-config-showreverse",defaultValue:false}},g=at.noConflict(),c=O.noConflict(),R=wt,u=new g.Log(pt,X.console||Ft.console);let zt=pt?.script?.name||void 0;wt.config.Utils.AnyTouch();const Tt=false;u.config({debug:Tt,logMaxCount:1e3,autoClearConsole:true,tag:true});v.config({isHTML:true,autoClose:true,showClose:false,consoleLogContent(t){const e=t.getSetting().type;if(e==="loading")return  false;const A=t.getSetting().content;return e==="warning"?u.warn(A):e==="error"?u.error(A):u.info(A),true},get position(){return h.getValue(ct.qmsg_config_position.key,ct.qmsg_config_position.defaultValue)},get maxNums(){return h.getValue(ct.qmsg_config_maxnums.key,ct.qmsg_config_maxnums.defaultValue)},get showReverse(){return h.getValue(ct.qmsg_config_showreverse.key,ct.qmsg_config_showreverse.defaultValue)},get zIndex(){let t=at.getMaxZIndex(),e=wt.config.InstanceUtils.getPopsMaxZIndex().zIndex;return at.getMaxValue(t,e)+100}});R.GlobalConfig.setGlobalConfig({zIndex:()=>{let t=at.getMaxZIndex(void 0,void 0,A=>{if(A?.classList?.contains("qmsg-shadow-container")||A?.closest("qmsg")&&A.getRootNode()instanceof ShadowRoot)return  false}),e=wt.config.InstanceUtils.getPopsMaxZIndex().zIndex;return at.getMaxValue(t,e)+100},mask:{enable:true,clickEvent:{toClose:false,toHide:false}},drag:true});const Bt=new g.GM_Menu({GM_getValue:tt,GM_setValue:W,GM_registerMenuCommand:qt,GM_unregisterMenuCommand:Gt}),q=new g.Httpx({xmlHttpRequest:Vt,logDetails:Tt});q.interceptors.request.use(t=>t);q.interceptors.response.use(void 0,t=>(u.error("拦截器-请求错误",t),t.type==="onabort"?v.warning("请求取消",{consoleLogContent:true}):t.type==="onerror"?v.error("请求异常",{consoleLogContent:true}):t.type==="ontimeout"?v.error("请求超时",{consoleLogContent:true}):v.error("其它错误",{consoleLogContent:true}),t));X.Object.defineProperty,X.Function.prototype.apply,X.Function.prototype.call,X.Element.prototype.appendChild,X.setTimeout;const M=g.addStyle.bind(g),T=O.selector.bind(O),K=O.selectorAll.bind(O);new g.GM_Cookie;const kt="GM_Panel",St="data-init",gt="data-key",vt="data-default-value",Yt="data-init-more-value",st="data-storage-api",Z={get width(){return globalThis.innerWidth},get height(){return globalThis.innerHeight}},_={setting:{get width(){return Z.width<550?"88vw":Z.width<700?"550px":"700px"},get height(){return Z.height<450?"70vh":Z.height<550?"450px":"550px"}},settingMiddle:{get width(){return Z.width<350?"88vw":"350px"},get height(){return Z.height<450?"88vh":"450px"}},settingBig:{get width(){return Z.width<800?"92vw":"800px"},get height(){return Z.height<600?"80vh":"600px"}},info:{get width(){return Z.width<350?"88vw":"350px"},get height(){return Z.height<250?"88vh":"250px"}}};class Zt{storageKey;listenerData;constructor(e){if(typeof e=="string"){let A=e.trim();if(A=="")throw new Error("key参数不能为空字符串");this.storageKey=A;}else throw new Error("key参数类型错误，必须是字符串");this.listenerData=new at.Dictionary;}getLocalValue(){let e=tt(this.storageKey);return e==null&&(e={},this.setLocalValue(e)),e}setLocalValue(e){W(this.storageKey,e);}set(e,A){let n=this.get(e),i=this.getLocalValue();Reflect.set(i,e,A),this.setLocalValue(i),this.triggerValueChangeListener(e,n,A);}get(e,A){let n=this.getLocalValue();return Reflect.get(n,e)??A}getAll(){return this.getLocalValue()}delete(e){let A=this.get(e),n=this.getLocalValue();Reflect.deleteProperty(n,e),this.setLocalValue(n),this.triggerValueChangeListener(e,A,void 0);}has(e){let A=this.getLocalValue();return Reflect.has(A,e)}keys(){let e=this.getLocalValue();return Reflect.ownKeys(e)}values(){let e=this.getLocalValue();return Reflect.ownKeys(e).map(A=>Reflect.get(e,A))}clear(){Pt(this.storageKey);}addValueChangeListener(e,A){let n=Math.random(),i=this.listenerData.get(e)||[];return i.push({id:n,key:e,callback:A}),this.listenerData.set(e,i),n}removeValueChangeListener(e){let A=false;for(const[n,i]of this.listenerData.entries()){for(let a=0;a<i.length;a++){const o=i[a];(typeof e=="string"&&o.key===e||typeof e=="number"&&o.id===e)&&(i.splice(a,1),a--,A=true);}this.listenerData.set(n,i);}return A}triggerValueChangeListener(e,A,n){if(!this.listenerData.has(e))return;let i=this.listenerData.get(e);for(let a=0;a<i.length;a++){const o=i[a];if(typeof o.callback=="function"){let l=this.get(e),s,d;typeof A<"u"&&arguments.length>=2?d=A:d=l,typeof n<"u"&&arguments.length>2?s=n:s=l,o.callback(e,d,s);}}}}const nt=new Zt(kt),yt={$data:{__contentConfig:null,get contentConfig(){return this.__contentConfig==null&&(this.__contentConfig=new g.Dictionary),this.__contentConfig}},addContentConfig(t){Array.isArray(t)||(t=[t]);let e=this.$data.contentConfig.keys().length;this.$data.contentConfig.set(e,t);},getAllContentConfig(){return this.$data.contentConfig.values().flat()},getConfig(t=0){return this.$data.contentConfig.get(t)??[]},getDefaultBottomContentConfig(){return [{id:"script-version",title:`版本：${pt?.script?.version||"未知"}`,isBottom:true,forms:[],clickFirstCallback(t,e,A){let n=pt?.script?.supportURL||pt?.script?.namespace;return typeof n=="string"&&g.isNotNull(n)&&window.open(n,"_blank"),false}}]}},jt={$data:{__menuOption:[{key:"show_pops_panel_setting",text:"⚙ 设置",autoReload:false,isStoreValue:false,showText(t){return t},callback:()=>{h.showPanel(yt.getConfig(0));}}],get menuOption(){return this.__menuOption}},init(){this.initExtensionsMenu();},initExtensionsMenu(){h.isTopWindow()&&Bt.add(this.$data.menuOption);},addMenuOption(t){Array.isArray(t)||(t=[t]),this.$data.menuOption.push(...t);},updateMenuOption(t){Array.isArray(t)||(t=[t]),t.forEach(e=>{let A=this.$data.menuOption.findIndex(n=>n.key===e.key);A!==-1&&(this.$data.menuOption[A]=e);});},getMenuOption(t=0){return this.$data.menuOption[t]},deleteMenuOption(t=0){this.$data.menuOption.splice(t,1);}},h={$data:{__contentConfigInitDefaultValue:null,__onceExecMenuData:null,__urlChangeReloadMenuExecOnce:null,__onceExecData:null,__panelConfig:{},$panel:null,panelContent:[],get contentConfigInitDefaultValue(){return this.__contentConfigInitDefaultValue==null&&(this.__contentConfigInitDefaultValue=new g.Dictionary),this.__contentConfigInitDefaultValue},contentConfigInitDisabledKeys:[],get onceExecMenuData(){return this.__onceExecMenuData==null&&(this.__onceExecMenuData=new g.Dictionary),this.__onceExecMenuData},get urlChangeReloadMenuExecOnce(){return this.__urlChangeReloadMenuExecOnce==null&&(this.__urlChangeReloadMenuExecOnce=new g.Dictionary),this.__urlChangeReloadMenuExecOnce},get onceExecData(){return this.__onceExecData==null&&(this.__onceExecData=new g.Dictionary),this.__onceExecData},get scriptName(){return zt},get panelConfig(){return this.__panelConfig},set panelConfig(t){this.__panelConfig=t;},key:kt,attributeKeyName:gt,attributeDefaultValueName:vt},init(){this.initContentDefaultValue(),jt.init();},isTopWindow(){return X.top===X.self},initContentDefaultValue(){const t=n=>{if(!n.attributes||n.type==="button"||n.type==="forms"||n.type==="deepMenu")return;let i=n.attributes[St];if(typeof i=="function"){let s=i();if(typeof s=="boolean"&&!s)return}let a=new Map,o=n.attributes[gt];if(o!=null){const s=n.attributes[vt];a.set(o,s);}let l=n.attributes[Yt];if(typeof l=="object"&&l&&Object.keys(l).forEach(s=>{a.set(s,l[s]);}),!a.size){u.warn(["请先配置键",n]);return}if(n.type==="switch"){let s=typeof n.disabled=="function"?n.disabled():n.disabled;typeof s=="boolean"&&s&&this.$data.contentConfigInitDisabledKeys.push(...a.keys());}for(const[s,d]of a.entries())this.setDefaultValue(s,d);},e=n=>{for(let i=0;i<n.length;i++){let a=n[i];t(a);let o=a.forms;o&&Array.isArray(o)&&e(o);}},A=[...yt.getAllContentConfig()];for(let n=0;n<A.length;n++){let i=A[n];if(!i.forms)continue;const a=i.forms;a&&Array.isArray(a)&&e(a);}this.$data.contentConfigInitDisabledKeys=[...new Set(this.$data.contentConfigInitDisabledKeys)];},setDefaultValue(t,e){this.$data.contentConfigInitDefaultValue.has(t)&&u.warn("请检查该key(已存在): "+t),this.$data.contentConfigInitDefaultValue.set(t,e);},setValue(t,e){nt.set(t,e);},getValue(t,e){let A=nt.get(t);return A??(this.$data.contentConfigInitDefaultValue.has(t)?this.$data.contentConfigInitDefaultValue.get(t):e)},deleteValue(t){nt.delete(t);},hasKey(t){return nt.has(t)},addValueChangeListener(t,e){return nt.addValueChangeListener(t,(n,i,a)=>{e(t,a,i);})},removeValueChangeListener(t){nt.removeValueChangeListener(t);},triggerMenuValueChange(t,e,A){nt.triggerValueChangeListener(t,A,e);},exec(t,e,A,n=true){const i=this;let a;typeof t=="string"||Array.isArray(t)?a=()=>t:a=t;let o=false,l=a(),s=[];Array.isArray(l)?(o=true,s=l):s.push(l);let d=s.find(w=>!this.$data.contentConfigInitDefaultValue.has(w));if(d){u.warn(`${d} 键不存在`);return}let f=JSON.stringify(s);if(n&&this.$data.onceExecMenuData.has(f))return this.$data.onceExecMenuData.get(f);let p=[],m=[],y=(w,C)=>{let z=[];Array.isArray(C)||(C=[C]),C.forEach(N=>{if(N!=null&&N instanceof HTMLStyleElement){z.push(N);return}}),p=p.concat(z);},P=w=>this.getValue(w),b=()=>{for(let w=0;w<p.length;w++)p[w].remove(),p.splice(w,1),w--;},Q=()=>{let w=false;return typeof A=="function"?w=A(s):w=s.every(C=>P(C)),w},k=w=>{let C=Q(),z=[];if(C){let N=s.map(L=>this.getValue(L)),D=e({value:o?N:N[0],addStyleElement:(...L)=>y(true,...L)});Array.isArray(D)||(D=[D]),D.forEach(L=>{if(L!=null&&L instanceof HTMLStyleElement){z.push(L);return}});}b(),p=[...z];};n&&s.forEach(w=>{let C=this.addValueChangeListener(w,(z,N,D)=>{k();});m.push(C);}),k();let j={reload(){k();},clear(){this.clearStoreStyleElements(),this.removeValueChangeListener(),n&&i.$data.onceExecMenuData.delete(f);},clearStoreStyleElements:()=>b(),removeValueChangeListener:()=>{m.forEach(w=>{this.removeValueChangeListener(w);});}};return this.$data.onceExecMenuData.set(f,j),j},execMenu(t,e,A=false,n=false){return this.exec(t,i=>e(i),i=>i.every(o=>{let l=!!this.getValue(o);return h.$data.contentConfigInitDisabledKeys.includes(o)&&(l=false,u.warn(`.execMenu${n?"Once":""} ${o} 被禁用`)),A&&(l=!l),l}),n)},execMenuOnce(t,e,A=false,n=false){const i=this.execMenu(t,e,A,true);if(n&&i){const a=()=>{i.reload();};this.removeUrlChangeWithExecMenuOnceListener(t),this.addUrlChangeWithExecMenuOnceListener(t,a);const o=i.clear;i.clear=()=>{o(),this.removeUrlChangeWithExecMenuOnceListener(t);};}return i},deleteExecMenuOnce(t){return t=this.transformKey(t),this.$data.onceExecMenuData.delete(t),this.$data.urlChangeReloadMenuExecOnce.delete(t),nt.removeValueChangeListener(t)},onceExec(t,e){if(t=this.transformKey(t),typeof t!="string")throw new TypeError("key 必须是字符串");this.$data.onceExecData.has(t)||(e(),this.$data.onceExecData.set(t,1));},deleteOnceExec(t){t=this.transformKey(t),this.$data.onceExecData.delete(t);},addUrlChangeWithExecMenuOnceListener(t,e){t=this.transformKey(t),this.$data.urlChangeReloadMenuExecOnce.set(t,e);},removeUrlChangeWithExecMenuOnceListener(t){t=this.transformKey(t),this.$data.urlChangeReloadMenuExecOnce.delete(t);},triggerUrlChangeWithExecMenuOnceEvent(t){this.$data.urlChangeReloadMenuExecOnce.forEach((e,A)=>{e(t);});},showPanel(t,e=`${zt}-设置`,A=false,n=false){this.$data.$panel=null,this.$data.panelContent=[];let i=t.findIndex(o=>(typeof o.isBottom=="function"?o.isBottom():!!o.isBottom)&&o.id==="script-version")!==-1;!A&&!i&&t.push(...yt.getDefaultBottomContentConfig());let a=R.panel({title:{text:e,position:"center",html:false,style:""},content:t,btn:{close:{enable:true,callback:(o,l)=>{o.close(),this.$data.$panel=null;}}},mask:{enable:true,clickEvent:{toClose:true,toHide:false},clickCallBack:(o,l)=>{o(),this.$data.$panel=null;}},width:_.setting.width,height:_.setting.height,drag:true,only:true,...this.$data.panelConfig});this.$data.$panel=a,this.$data.panelContent=t,n||this.registerConfigSearch({$panel:a,content:t});},registerConfigSearch(t){const{$panel:e,content:A}=t;let n=async(f,p)=>{if(f==null)return;let m=await p(f);return m&&typeof m.isFind=="boolean"&&m.isFind?m.data:await n(m.data,p)},i=(f,p)=>{const m=new IntersectionObserver(y=>{y.forEach(P=>{P.isIntersecting&&(p?.(),m.disconnect());});},{root:null,threshold:1});m.observe(f),f.scrollIntoView({behavior:"smooth",block:"center"});},a=f=>{const p="pops-flashing";c.animationend(f,()=>{f.classList.remove(p);}),f.classList.add(p);},o=(f,p)=>{g.preventEvent(f);let m=R.alert({title:{text:"搜索配置",position:"center"},content:{text:`
					...
					${t.searchDialogStyle??""}
				`});m.$shadowRoot.querySelector(".search-wrapper");let y=m.$shadowRoot.querySelector(".search-config-text"),P=m.$shadowRoot.querySelector(".search-result-wrapper");y.focus();let b=()=>{c.empty(P);},Q=j=>{const w=g.queryProperty(j,z=>z?.next?{isFind:false,data:z.next}:{isFind:true,data:z});let C=c.createElement("div",{className:"search-result-item",innerHTML:`
						...
						`});return c.on(C,"click",z=>{let D=e.$shadowRoot.querySelectorAll("aside.pops-panel-aside .pops-panel-aside-top-container li")[j.index];if(!D){v.error(`左侧项下标${j.index}不存在`);return}D.scrollIntoView({behavior:"smooth",block:"center"}),D.click(),n(j.next,async L=>{if(L?.next){let E=await g.waitNode(()=>Array.from(e.$shadowRoot.querySelectorAll(".pops-panel-deepMenu-nav-item")).find(I=>{const H=Reflect.get(I,"__formConfig__");return typeof H=="object"&&H!=null&&H.text===L.name}),2500);if(E)E.click();else return v.error("未找到对应的二级菜单"),{isFind:true,data:L};return {isFind:false,data:L.next}}else {let E=await g.waitNode(()=>Array.from(e.$shadowRoot.querySelectorAll("li:not(.pops-panel-deepMenu-nav-item)")).find(I=>Reflect.get(I,"__formConfig__")===L.matchedData?.formConfig),2500);if(E){i(E);let I=E.closest(".pops-panel-forms-fold[data-fold-enable]");I&&(I.querySelector(".pops-panel-forms-fold-container").click(),await g.sleep(500)),i(E,()=>{a(E);});}else v.error("未找到对应的菜单项");return {isFind:true,data:L}}});}),C},k=j=>{const w=new RegExp(j,"i"),C=[],z=(D,L)=>{for(let E=0;E<D.length;E++){const I=D[E];let H=I.forms;if(H&&Array.isArray(H)){const F=g.deepClone(L);if(I.type==="deepMenu"){const et=g.queryProperty(F,J=>J?.next?{isFind:false,data:J.next}:{isFind:true,data:J});et.next={name:I.text};}z(H,F);}else {let F=Reflect.get(I,"text"),et=Reflect.get(I,"description");const J=[F,et];let Y=J.findIndex(V=>{if(typeof V=="string")return V.match(w)});if(Y!==-1){const V=g.deepClone(L),At=g.queryProperty(V,rt=>rt?.next?{isFind:false,data:rt.next}:{isFind:true,data:rt});At.next={name:F,matchedData:{path:"",formConfig:I,matchedText:J[Y],description:et}};const ht=[];g.queryProperty(V,rt=>{const bt=rt?.name;return typeof bt=="string"&&bt.trim()!==""&&ht.push(bt),rt?.next?{isFind:false,data:rt.next}:{isFind:true,data:rt}});const Kt=ht.join(it.escapeHtml(" - "));At.next.matchedData.path=Kt,C.push(V);}}}};for(let D=0;D<A.length;D++){const L=A[D];if(!L.forms||L.isBottom&&L.id==="script-version")continue;const E=L.forms;if(E&&Array.isArray(E)){let I=L.title;typeof I=="function"&&(I=I()),z(E,{index:D,name:I});}}let N=document.createDocumentFragment();for(const D of C){let L=Q(D);N.appendChild(L);}b(),P.append(N);};c.on(y,"input",g.debounce(j=>{g.preventEvent(j);let w=c.val(y).trim();if(w===""){b();return}k(w);},200));},l=null,s=false,d;c.on(e.$shadowRoot,"dblclick","aside.pops-panel-aside .pops-panel-aside-item:not(#script-version)",o),c.on(e.$shadowRoot,"touchend","aside.pops-panel-aside .pops-panel-aside-item:not(#script-version)",(f,p)=>{clearTimeout(d),d=void 0,s&&l===p?(s=false,l=null,o(f)):(d=setTimeout(()=>{s=false;},200),s=true,l=p);},{capture:true}),e.$shadowRoot.appendChild(c.createElement("style",{type:"text/css",textContent:`
					.pops-flashing{
...
`,le="",B={isHome(){return window.location.pathname.split("/").length<=2&&window.location.search===""},isCode(){return !!window.location.pathname.split("/")?.includes("code")},isCodeStrict(){return !!window.location.pathname.match(/\/code(\/|)$/)},isVersion(){return !!window.location.pathname.match(/\/versions(\/|)$/)},isUsers(){return !!window.location.pathname.match(/\/.+\/users\/.+/gi)},isUsersConversations(){return this.isUsers()&&!!window.location.pathname.includes("/conversations")},isUsersConversationsWithSomeUser(){return this.isUsersConversations()&&!!window.location.pathname.match(/\/conversations\/[\d]+/)},isScript(){return !!window.location.pathname.match(/\/scripts\/[\d+]/)},isScriptAdmin(){return !!window.location.pathname.endsWith("/admin")},isScriptList(){return !!window.location.pathname.match(/\/scripts(\/|)$/)},isScriptsBySite(){return !!window.location.pathname.match("/scripts/by-site")},isScriptsFeedback(){return this.isScript()&&window.location.pathname.match(/\/feedback(\/|)$/i)},isScriptLibraryList(){return !!window.location.pathname.match(/\/libraries(\/|)$/)},isScriptSearch(){let t=new URLSearchParams(window.location.search);return this.isScriptList()&&t.has("q")},isScriptCodeSearch(){return !!window.location.pathname.match(/\/code-search(\/|)$/)},isScriptLibraryListSearch(){let t=new URLSearchParams(window.location.search);return this.isScriptLibraryList()&&t.has("q")},isDiscuessions(){return !!window.location.pathname.match(/\/discussions(\/|)$/)},isImageSource(){return window.location.pathname.startsWith("/vite/assets")}},ce={init(){h.execMenuOnce("beautifyPage",()=>this.beautifyPageElement()),h.execMenuOnce("beautifyGreasyforkBeautify",()=>this.beautifyGreasyforkBeautify()),h.execMenuOnce("beautifyUploadImage",()=>this.beautifyUploadImage()),h.execMenuOnce("beautifyTopNavigationBar",()=>this.beautifyTopNavigationBar());},beautifyPageElement(){u.info("美化页面元素");let t=[];return t.push(M(te)),t.push(M(ee)),t.push(M(Ae)),t.push(M(re)),t.push(M(ne)),t.push(M(`
			p:has(input[type="submit"][name="update-and-sync"]){
			  margin-top: 10px;
			}
			`)),c.ready(()=>{let e=T('a[target="markup_choice"][href*="daringfireball.net"]');e&&e.parentElement.replaceChild(c.createElement("span",{textContent:"Markdown"}),e),K(".user-content ul li").forEach(i=>{let a=i.firstChild;a?.nodeName==="#text"&&(a.textContent?.startsWith("[x] ")?(a.textContent=a.textContent.replace("[x] ",""),c.prepend(i,`
							<input type="checkbox" disabled="" class="task-list-item-checkbox" checked="">
						`)):a.textContent?.startsWith("[ ] ")&&(a.textContent=a.textContent.replace("[ ] ",""),c.prepend(i,`
				...
				`},n=Object.keys(A);if(K(".user-content blockquote").forEach(i=>{Array.from(i.children).forEach(a=>{let o=c.html(a);if(a.nodeName==="P"){let l=n.find(s=>o.toLowerCase().startsWith(s.toLowerCase()));l&&(c.html(a,o.replace(l,"")),c.before(a,A[l]),c.html(a)===""&&a.remove());}});}),K(".user-content > blockquote").forEach(i=>{Array.from(i.querySelectorAll(".markdown-alert-title")).forEach(a=>{let o=c.createElement("div",{className:"markdown-alert markdown-alert-tip"},{"data-type":a.getAttribute("data-type")});for(;a.nextSibling&&!a.nextSibling?.classList?.contains("markdown-alert-title");)c.append(o,a.nextSibling);c.prepend(o,a),c.before(i,o);});}),B.isHome())t.push(M(se));else if(B.isScriptsFeedback()){t.push(M(le));let i=T('.radio-label[for*="discussion_rating_0"]');i&&(i.innerHTML=i.innerHTML.replace("不评分",'<span class="rating-icon rating-icon-none">不评分</span>'));let a=T('.radio-label[for*="discussion_rating_2"]');a&&(a.innerHTML=a.innerHTML.replace("差评",'<span class="rating-icon rating-icon-bad">差评</span>'));let o=T('.radio-label[for*="discussion_rating_3"]');o&&(o.innerHTML=o.innerHTML.replace("一般",'<span class="rating-icon rating-icon-ok">一般</span>'));let l=T('.radio-label[for*="discussion_rating_4"]');l&&(l.innerHTML=l.innerHTML.replace("好评",'<span class="rating-icon rating-icon-good">好评</span>'));}else B.isScriptAdmin()&&(T('input[type="submit"][name="update-only"]')||t.push(M(`
					.indented{
						padding-left: unset;
					}
					`)));}),t},beautifyGreasyforkBeautify(){u.info("美化 Greasyfork Beautify脚本");let t=[];return t.push(M(ie)),g.isPhone()?t.push(M(`
				section#script-info,
				...
				section#script-info{
					margin-top: 10px;
				}`)),t},beautifyUploadImage(){u.info("美化上传图片");let t=[];return t.push(M(oe)),c.ready(()=>{function e(i){for(;i.nextElementSibling;)i.parentElement.removeChild(i.nextElementSibling);}let A=K('input[type="file"]');A.forEach(i=>{i.getAttribute("name")!=="code_upload"&&(i.hasAttribute("accept")&&i.getAttribute("accept").includes("javascript")||c.on(i,["propertychange","input"],function(a){e(a.target);let o=a.currentTarget.files;if(!o||o.length===0)return;u.info("选择的图片",o),o.length>5&&c.after(i,c.createElement("p",{textContent:r.t("❌ 最多同时长传5张图片")}));let l=[];Array.from(o).forEach(s=>{(s.size>204800||!s.type.match(/png|jpg|jpeg|gif|apng|webp/i))&&l.push(s);}),l.length!==0&&l.forEach(s=>{c.after(i,c.createElement("p",{textContent:r.t("❌ 图片：{{name}} 大小：{{size}}",{name:s.name,size:s.size})}));});}));}),["textarea#comment_text","textarea.comment-entry"].forEach(i=>{c.on(i,"paste",a=>{u.info("触发粘贴事件",a),setTimeout(()=>{c.trigger(A,"input");},100);});});}),t},beautifyTopNavigationBar(){u.info("美化顶部导航栏");let t=[];return t.push(M(ae)),window.outerWidth>550&&(t.push(it.addBlockCSS(".with-submenu")),c.ready(()=>{let A=T("#site-nav").querySelector("nav");K(".with-submenu nav li").forEach(n=>{A.appendChild(n);});})),t}},U={menu:Bt,isLogin:false,initEnv(){let t=this.getUserLinkElement();this.isLogin=!!t;},getUserLinkElement(){return T("#nav-user-info span.user-profile-link a")},async updateScript(t){let e=function(A,n=1){return `
			<div style="display: flex;flex-direction: column;align-items: flex-start;">
				<div style="height: 30px;line-height: 30px;">${r.t("名称：")}${A}</div>
				<div style="height: 30px;line-height: 30px;">${r.t("进度：")}${n}/${t.length}</div>
			</div>`};if(g.isNull(t))v.error(r.t("未获取到【脚本列表】"));else {let A=v.loading(e(S.getScriptName(t[0])),{isHTML:true}),n=0,i=0;for(let a=0;a<t.length;a++){let o=t[a],l=S.getScriptId(o);u.success("更新："+o);let s=S.getScriptName(o);A.setHTML(e(s,a+1));let d=await G.getSourceCodeSyncFormDataInfo(l);if(d){const{formData:f,url:p}=d;await G.sourceCodeSync(l,f,p)?(v.success(r.t("源代码同步成功，3秒后更新下一个")),await g.sleep(3e3),n++):(v.error(r.t("源代码同步失败")),i++);}else v.error(r.t("源代码同步失败")),i++;}A.close(),n===0?v.error(r.t("全部更新失败")):v.success(r.t("全部更新完毕<br >成功：{{successNums}}<br >失败：{{failedNums}}<br >总计：{{scriptUrlListLength}}",{successNums:n,failedNums:i,scriptUrlListLength:t.length}),{isHTML:true});}},handleLocalGotoCallBack(){if(h.getValue("goto_updateSettingsAndSynchronize_scriptList")){if(h.deleteValue("goto_updateSettingsAndSynchronize_scriptList"),!B.isUsers()){h.setValue("goto_updateSettingsAndSynchronize_scriptList",true),U.getUserLinkElement()?(v.success(r.t("前往用户主页")),window.location.href=U.getUserLinkElement().href):v.error(r.t("获取当前已登录的用户主页失败"));return}let t=[];K("#user-script-list-section li a.script-link").forEach(e=>{t=t.concat(S.getAdminUrl(e.href));}),U.updateScript(t);}else if(h.getValue("goto_updateSettingsAndSynchronize_unlistedScriptList")){if(h.deleteValue("goto_updateSettingsAndSynchronize_unlistedScriptList"),!B.isUsers()){h.setValue("goto_updateSettingsAndSynchronize_unlistedScriptList",true),U.getUserLinkElement()?(v.success(r.t("前往用户主页")),window.location.href=U.getUserLinkElement().href):v.error(r.t("获取当前已登录的用户主页失败"));return}let t=[];K("#user-unlisted-script-list li a.script-link").forEach(e=>{t=t.concat(S.getAdminUrl(e.href));}),U.updateScript(t);}else if(h.getValue("goto_updateSettingsAndSynchronize_libraryScriptList")){if(h.deleteValue("goto_updateSettingsAndSynchronize_libraryScriptList"),!B.isUsers()){h.setValue("goto_updateSettingsAndSynchronize_libraryScriptList",true),U.getUserLinkElement()?(v.success(r.t("前往用户主页")),window.location.href=U.getUserLinkElement().href):v.error(r.t("获取当前已登录的用户主页失败"));return}let t=[];K("#user-library-script-list li a.script-link").forEach(e=>{t=t.concat(S.getAdminUrl(e.href));}),U.updateScript(t);}}},de={init(){h.execMenu("autoLogin",()=>{this.autoLogin();});},autoLogin(){g.waitNode("span.sign-in-link a[rel=nofollow]").then(async()=>{let t=h.getValue("user"),e=h.getValue("pwd"),A=h.getValue("secret");if(g.isNull(t)){v.error(r.t("请先在菜单中录入账号"));return}if(g.isNull(e)){v.error(r.t("请先在菜单中录入密码"));return}if(g.isNull(A)){v.error(r.t("请先在菜单中录入secret"));return}let n=T("meta[name='csrf-token']");if(!n){v.error(r.t("获取csrf-token失败"));return}let a=new Xt__namespace.TOTP({secret:A}).generate(),o=v.loading(r.t("正在登录中...")),l=await q.post("/zh-CN/users/sign_in",{fetch:true,data:encodeURI(`authenticity_token=${n.getAttribute("content")}&user[email]=${t}&user[password]=${e}&user[remember_me]=1&user[otp_attempt]=${a}&commit=登录`),headers:{"Content-Type":"application/x-www-form-urlencoded"}});if(o.destroy(),!l.status){u.error(l),v.error(r.t("登录失败，请求异常"));return}let s=l.data.responseText,d=c.parseHTML(s,true,true),f=d.querySelector(".width-constraint .alert");d.querySelectorAll(".sign-out-link a[rel=nofollow][data-method='delete']").length?(v.success(r.t("登录成功，1s后自动跳转")),setTimeout(()=>{window.location.reload();},1e3)):f?(u.error(f),v.error(c.text(f))):(v.error(r.t("登录失败，可能是账号/密码错误，请在控制台查看原因"),{consoleLogContent:true}),u.error(l),u.error(`当前账号:${t}`),u.error(`当前密码:${e}`));});}},$={key:"gf-discuessions-filter-rule",$data:{FILTER_SCRIPT_KEY:"greasyfork-discussions-filter-script",FILTER_POST_USER_KEY:"greasyfork-discussions-filter-post-user",FILTER_REPLY_USER_KEY:"greasyfork-discussions-filter-reply-user"},init(){u.info("论坛-过滤"),M(`
        ...
        `);let t=new g.LockFunction(()=>{this.filter();},50);g.mutationObserver(document.body,{config:{subtree:true,childList:true},immediate:true,callback:()=>{t.run();}}),t.run();},getElementList(){let t=[];return t=t.concat(Array.from(K(".discussion-list-container"))),t},filter(){const t=new Map;this.getElementList().forEach((e,A)=>{const n=this.parseDiscuessionListContainerInfo(e);if(!n)return;let i=this.getValue().split(`
`);if(t.has(n.snippet)&&h.getValue("greasyfork-discussions-filter-duplicate-comments")){let a=t.get(n.snippet).querySelector("a.discussion-title");a.setAttribute("data-repeat-tip-show","true");let o=0;a.hasAttribute("data-repeat-count")&&(o=parseInt(a.getAttribute("data-repeat-count"))),o++,a.setAttribute("data-repeat-count",o.toString()),a.setAttribute("data-repeat-tip-show",r.t("已过滤：{{oldCount}}",{oldCount:o})),u.success([`过滤重复内容：${n.snippet}`,n]),e.remove();return}t.set(n.snippet,e);for(let a=0;a<i.length;a++){let o=i[a],l=o.split("##"),s=l[0],d=l[1];if(s in n){let f=new RegExp(d,"ig");if(n[s]!=null&&String(n[s]).match(f)){u.info("触发论坛过滤规则",o,n),e.remove();return}}}});},parseDiscuessionListContainerInfo(t){let e=t.querySelector("a.discussion-title");if(!e)return;let A=e.href,n=A.match(/\/discussions(|\/greasyfork)\/([\d]+)/);if(!n)return;let i=n[n.length-1];const a=t.querySelector(".discussion-meta-item-script-name"),o=a?.querySelector("a"),l=t.querySelector("a.user-link"),s={scriptName:a.innerText,scriptUrl:o?.href,scriptId:S.getScriptId(o?.href),postUserName:l?.innerText,postUserHomeUrl:l?.href,postUserId:S.getUserId(l?.href),postTimeStamp:new Date(t.querySelector("relative-time").getAttribute("datetime")),snippetId:i,snippetUrl:A,snippet:t.querySelector("span.discussion-snippet")?.innerText||"",replyUserName:void 0,replyUserHomeUrl:void 0,replyUserId:void 0,replyTimeStamp:void 0};let d=t.querySelector(".discussion-meta-item .discussion-meta-item");if(d){const f=d.querySelector("a.user-link");s.replyUserName=f.innerText,s.replyUserHomeUrl=f.href,s.replyUserId=S.getUserId(s.replyUserHomeUrl),s.replyTimeStamp=new Date(d.querySelector("relative-time").getAttribute("datetime"));}return s},setValue(t){h.setValue(this.key,t);},addValue(t,e){let A=this.getValue();if(A.trim()!==""&&(A+=`
`),g.isNull(t))return;t=t.toString().trim();let n=t+"##"+e;A+=n,this.setValue(A);},getValue(){return h.getValue(this.key,"")}},ue={init(){this.readBgColor(),c.ready(()=>{h.execMenuOnce("greasyfork-discussions-filter-enable",()=>{this.filterEnable();});let t=new g.LockFunction(()=>{h.execMenu("discussions-addShortcutOperationButton",()=>{this.addShortcutOperationButton();}),h.execMenu("discussions-addReportButton",()=>{this.addReportButton();});});g.mutationObserver(document.body,{config:{subtree:true,childList:true},immediate:true,callback:()=>{t.run();}});});},filterEnable(){u.info("启用Greasyfork论坛过滤器"),$.init();},readBgColor(){u.info("设置已读背景颜色");let t=h.getValue("discussions-readBgColor");return M(`
        .discussion-read{
            background: ${t} !important;
        }
        `)},addShortcutOperationButton(){u.info("添加【过滤】按钮"),$.getElementList().forEach(t=>{if(t.querySelector(".discussion-filter-button"))return;let A=t.querySelector(".discussion-list-item").querySelector(".discussion-meta"),n=c.createElement("div",{className:"discussion-meta-item",innerHTML:`
					<button class="discussion-filter-button">${r.t("过滤")}</button>
					`},{style:"flex: 0;"}),i=n.querySelector(".discussion-filter-button");A.appendChild(n),c.on(i,"click",a=>{g.preventEvent(a);const o=$.parseDiscuessionListContainerInfo(t);if(!o)return;let l="data-filter-key",s="data-filter-value",d=R.alert({title:{text:r.t("选择需要过滤的选项"),position:"center",html:false},content:{text:`
								<button ${l}="scriptId" ${s}="^${o.scriptId}$">${r.t("脚本id：{{text}}",{text:o.scriptId})}</button>
								<button ${l}="scriptName" ${s}="^${g.parseStringToRegExpString(o.scriptName)}$">${r.t("脚本名：{{text}}",{text:o.scriptName})}</button>
								<button ${l}="postUserId" ${s}="^${g.parseStringToRegExpString(o.postUserId)}$">${r.t("发布的用户id：{{text}}",{text:o.postUserId})}</button>
						...
						`}),f=d.$shadowRoot.querySelector(".pops-alert-content");if(o.scriptId==null&&f.querySelector(`button[${l}="scriptId"]`)?.remove(),o.scriptName==null&&f.querySelector(`button[${l}="scriptName"]`)?.remove(),o.postUserId==null&&f.querySelector(`button[${l}="postUserId"]`)?.remove(),o.replyUserId!=null){let p=c.createElement("button",{innerHTML:r.t("作者id：{{text}}",{text:o.replyUserId})});p.setAttribute(l,"scriptAuthorId"),p.setAttribute(s,"^"+o.replyUserId+"$"),f.appendChild(p);}c.on(d.$shadowRoot,"click",`button[${l}]`,p=>{g.preventEvent(p);let m=p.target,y=m.getAttribute(l),P=m.getAttribute(s);$.addValue(y,P),d.close(),$.filter(),v.success(r.t("添加成功"));});});});},addReportButton(){u.info("添加【举报】按钮"),$.getElementList().forEach(t=>{if(t.querySelector(".discussion-report-button"))return;let A=t.querySelector(".discussion-list-item").querySelector(".discussion-meta"),n=c.createElement("div",{className:"discussion-meta-item",innerHTML:`
					<button class="discussion-report-button pops-button--danger">${r.t("举报")}</button>
					`},{style:"flex: 0;"}),i=n.querySelector(".discussion-report-button");A.appendChild(n),c.on(i,"click",a=>{g.preventEvent(a);const o=$.parseDiscuessionListContainerInfo(t);o&&R.alert({title:{text:r.t("举报"),position:"center",html:false},content:{text:`
						<div class="report-item">
							${r.t("举报讨论：")}
							<a href="${S.getReportUrl("discussion",o.snippetId)}" target="_blank">${o.snippet}</a>
						</div>
						${o.scriptId?`
							<div class="report-item">
							${r.t("举报脚本：")}
							<a href="${S.getReportUrl("script",o.scriptId)}" target="_blank">${o.scriptName}</a>
						</div>
						`:""}
						
						<div class="report-item">
							${r.t("举报用户：")}
							<a href="${S.getReportUrl("user",o.postUserId)}" target="_blank">${o.postUserName}</a>
						</div>
						${o.replyUserId&&o.replyUserId!=o.postUserId?`
								<div class="report-item">
									${r.t("举报用户：")}
									<a href="${S.getReportUrl("user",o.replyUserId)}" target="_blank">${o.replyUserName}</a>
								</div>
								`:""}
							...
							`});});});}},Dt={isCurrentLoginUserHome(){let t=lt.getCurrentLoginUserId();return !!(t!=null&&B.isUsers()&&window.location.pathname.includes("/"+t))},monacoEditor(){return new Promise(t=>{const e="0.52.2",A="monaco-editor-ready";if(X.monaco){t(X.monaco);return}let n=v.loading(r.t("monaco-editor加载中...")),i=c.createElement("script",{type:"module",defer:true,innerHTML:`
					// import * as monaco from "https://fastly.jsdelivr.net/npm/monaco-editor@${e}/+esm";
				...
				`});c.append(document.head||document.documentElement,i),c.on(X,A,()=>{let a=X.monaco;u.success("网络加载monaco编辑器成功"),n.close(),t(a);},{once:true});})}},fe={init(){h.execMenuOnce("code-repairCodeLineNumber",()=>{this.repairCodeLineNumber();}),h.execMenuOnce("code-use-monaco-editor",()=>{this.coverEditorWithMonaco();});},repairCodeLineNumber(){u.info("修复代码的行号显示不够问题"),h.execMenuOnce("beautifyGreasyforkBeautify",()=>{M(`
				.code-container pre code .marker{
					padding-left: 6px;
				}	
				`);}),g.waitNode("#script-content div.code-container pre.prettyprint ol").then(t=>{t.childElementCount>=1e3&&(u.success(`当前代码行数${t.childElementCount}行，超过1000行，优化行号显示问题`),M(`
                    pre.prettyprint{
                        padding-left: 26px;
                    }
					`));});},coverEditorWithMonaco(){u.info("使用monacoEditor替换编辑器"),M(`
			.monaco-editor{
		...
		`),it.addBlockCSS("#script-content .code-container > pre");let t=S.getScriptId(window.location.href);if(!t){v.error("未解析出当前脚本ID",{consoleLogContent:true});return}Promise.all([Dt.monacoEditor(),G.getScriptInfo(t)]).then(async([e,A])=>{let n=A?.code_url;if(!n){v.error("请求结果中未解析出脚本代码URL",{consoleLogContent:true});return}let i=new URLSearchParams(window.location.search);if(i.has("version")){let l=i.get("version");n=n.replace(new RegExp(`/${t}(/[\\d]+|)`),`/${t}/${l}`),u.info(`当前是历史代码页面，请求的脚本代码URL为${n}`);}let a=await q.get(n,{timeout:2e4});if(!a.status)return;let o=a.data.responseText;c.ready(async()=>{let l=await g.waitNode("#script-content .code-container > pre",1e4);if(!l)return;let s=c.createElement("div",{className:"monaco-editor"});c.after(l,s),e.editor.create(s,{value:o,minimap:{enabled:true},automaticLayout:true,codeLens:true,colorDecorators:true,contextmenu:true,readOnly:true,formatOnPaste:true,overviewRulerBorder:true,scrollBeyondLastLine:false,theme:"vs-dark",fontSize:window.innerWidth>600?14:12,wordWrap:"off",language:"javascript",folding:true,foldingStrategy:"indentation"});});});}},pe=`ul.history_versions,\r
...
`,ge={init(){h.execMenuOnce("beautifyHistoryVersionPage",()=>this.beautifyHistoryVersionPage()),h.execMenuOnce("scripts-versions-addExtraTagButton",()=>{this.addExtraTagButton();}),h.execMenuOnce("scripts-versions-addCompareCodeButton",()=>{this.sourceDiffMonacoEditor();});},beautifyHistoryVersionPage(){u.info("美化 历史版本 页面");let t=[];return t.push(M(pe)),t.push(it.addBlockCSS(".version-number",".version-date",".version-changelog")),c.ready(function(){let e=T("ul.history_versions");if(!e){v.error(r.t("未找到history_versions元素列表"));return}Array.from(e.children).forEach(A=>{let n=A.querySelector(".version-number a").href,i=A.querySelector(".version-number a").innerText,a=A.querySelector(".version-date")?.getAttribute("datetime"),o=A.querySelector(".version-changelog")?.innerHTML||"",l=c.createElement("span",{className:"script-version-date",innerHTML:g.formatTime(a,r.t("yyyy年MM月dd日 HH:mm:ss"))}),s=c.createElement("div",{className:"script-tag",innerHTML:`
                    <div class="script-tag-version">
                    ...
                    </div>`}),d=c.createElement("div",{className:"script-note-box-body",innerHTML:o});A.appendChild(l),A.appendChild(s),A.appendChild(d);});}),t},addExtraTagButton(){u.info("添加额外的标签按钮"),c.ready(()=>{K(".script-tag-version").forEach(t=>{let e=t.querySelector("a");if(!e)return;let A=new URL(e.href),n=A.pathname.match(/\/scripts\/([\d]+)/)?.[1],i=A.searchParams.get("version"),a=A.pathname.match(/\/scripts\/[\d]+-(.+)/)?.[1],o=S.getInstallUrl(n,i,a),l=S.getCodeUrl(n,i),s=c.createElement("div",{className:"scripts-tag-install",innerHTML:`
						...
						`});c.after(t,s);});});},sourceDiffMonacoEditor(){u.info("源码对比（monacoEditor）"),Dt.monacoEditor().then(t=>{c.ready(()=>{K('#script-content form[action*="/diff"] input[type="submit"]').forEach(e=>{let A=c.createElement("input",{type:"button",value:r.t("对比选中版本差异（monacoEditor）")},{style:"margin-left: 10px;"});c.after(e,A),c.on(A,"click",async n=>{g.preventEvent(n);let i=e.closest("form"),a=new FormData(i),o=a.get("v1"),l=a.get("v2");if(o===l){v.warning(r.t("版本号相同，不需要比较源码"));return}let s=v.loading(r.t("正在获取对比文本中...")),d=S.getScriptId(),f=await G.getScriptInfo(d);if(!f){s.close();return}let p=f.code_url,m=p.replace(new RegExp(`/${d}(/[\\d]+|)`),`/${d}/${o}`),y=p.replace(new RegExp(`/${d}(/[\\d]+|)`),`/${d}/${l}`),P="",b="";const[Q,k]=await Promise.all([q.get(m,{timeout:2e4}),q.get(y,{timeout:2e4})]);if(s.close(),!Q.status||!k.status)return;P=Q.data.responseText,b=k.data.responseText;let{recovery:j}=it.lockScroll(),w=R.alert({title:{text:r.t("代码对比"),html:false,position:"center"},content:{html:true,text:`
								<div class="monaco-editor-diff-container">
									<div class="monaco-editor-diff"></div>
								</div>
								`},mask:{enable:true,clickEvent:{toClose:false,toHide:false}},btn:{ok:{enable:false},close:{callback(L,E){L.close(),j();}}},zIndex(){let L=g.getMaxZIndex(),E=R.config.InstanceUtils.getPopsMaxZIndex().zIndex;return g.getMaxValue(L,E)+100},useShadowRoot:false,width:"90vw",height:"90vh",drag:true,style:`
						...
						`});w.$shadowRoot.querySelector(".monaco-editor-diff-container");let C=w.$shadowRoot.querySelector(".monaco-editor-diff"),z=t.editor.createDiffEditor(C,{hideUnchangedRegions:{enabled:true},minimap:{enabled:true},automaticLayout:true,codeLens:true,colorDecorators:true,contextmenu:false,readOnly:true,formatOnPaste:true,overviewRulerBorder:true,scrollBeyondLastLine:false,theme:"vs-dark",fontSize:window.innerWidth>600?14:12,wordWrap:"off",language:"javascript"});const N=t.editor.createModel(b,"javascript"),D=t.editor.createModel(P,"javascript");z.setModel({original:N,modified:D});});});});});}};let mt=[];const Nt=async function(t){if(u.info("当前脚本id："+t),!U.isLogin){u.error("请先登录账号"),v.error(r.t("请先登录账号"));return}let e=S.getUserId(U.getUserLinkElement().href);if(e==null){u.error("获取用户id失败"),v.error(r.t("获取用户id失败"));return}if(!mt.length){let a=v.loading(r.t("获取收藏夹中..."));if(mt=await G.getUserCollection(e)||[],a.close(),!mt.length)return}let A="";const n=(a,o)=>{let l=false;o=o.toString().trim();for(const[s,d]of a.entries())if(s==="scripts-included[]"&&d.toString().trim()===o){l=true;break}return l};mt.forEach(a=>{A+=`
		<li class="user-collect-item" data-id="${a.id}" data-name="${a.name}">
		  ...
		  `;});let i=R.alert({title:{text:r.t("收藏集"),position:"center"},content:{html:true,text:`<ul>${A}</ul>`},mask:{enable:true,clickEvent:{toClose:true}},btn:{ok:{enable:false}},width:R.isPhone()?"92vw":"500px",height:"auto",drag:true,only:true,style:`
		.pops{
		...
		`});c.on(i.$shadowRoot,"click",".collect-add-script-id",async function(a){let o=a.target.closest(".user-collect-item"),l=o.dataset.id;o.dataset.name;let s=v.loading(r.t("添加中..."));try{let d=await G.getUserCollectionInfo(e,l);if(!d)return;if(n(d,t)){v.warning(r.t("该脚本已经在该收藏集中"));return}let f=g.cloneFormData(d),p=g.cloneFormData(d);f.set("add-script",t.toString()),f.set("script-action","i"),p.append("scripts-included[]",t.toString()),p.set("save","1");let m=new URLSearchParams(f),y=new URLSearchParams(p),P=Array.from(m).map(([N,D])=>`${encodeURIComponent(N)}=${encodeURIComponent(D)}`).join("&"),b=Array.from(y).map(([N,D])=>`${encodeURIComponent(N)}=${encodeURIComponent(D)}`).join("&");u.info("添加的数据",P),u.info("保存的数据",b);let Q=await G.updateUserSetsInfo(e,l,P);if(!Q)return;let k=Q.querySelector(".change-script-set");if(!k){v.error(r.t("添加失败，{{selector}}元素不存在",{selector:".change-script-set"}));return}let j=k.querySelector("section");if(!j){v.error(r.t("添加失败，{{selector}}元素不存在",{selector:"section"}));return}let w=j.querySelector(".alert");if(w){R.alert({title:{text:r.t("添加失败"),position:"center"},content:{text:w.innerHTML,html:!0},mask:{enable:!0,clickEvent:{toClose:!0}},style:`
						.pops-alert-content{
							font-style: italic;
						...
						`,drag:!0,dragLimit:!0,width:_.info.width,height:_.info.height});return}let C=new FormData(k);if(!n(C,t)){u.error("添加失败，提交的添加请求中不包含该脚本id"),v.error(r.t("添加失败，表单数据中不包含该脚本"));return}await G.updateUserSetsInfo(e,l,b),v.success(r.t("添加成功"));}catch(d){console.error(d);}finally{s.close();}}),c.on(i.$shadowRoot,"click",".collect-delete-script-id",async function(a){let o=a.target.closest(".user-collect-item"),l=o.dataset.id;o.dataset.name;let s=v.loading(r.t("删除中..."));try{let d=await G.getUserCollectionInfo(e,l);if(!d)return;if(!n(d,t)){v.info(r.t("已删除：{{scriptId}}",{scriptId:t}));return}let f=g.cloneFormData(d,(C,z)=>C==="scripts-included[]"&&typeof z=="string"&&z.toString().trim()===t.toString().trim()),p=g.cloneFormData(f);f.set("remove-scripts-included[]",t.toString()),f.set("remove-selected-scripts","i"),f.delete("script-action"),p.set("save","1");let m=new URLSearchParams(f),y=new URLSearchParams(p),P=Array.from(m).map(([C,z])=>`${encodeURIComponent(C)}=${encodeURIComponent(z)}`).join("&"),b=Array.from(y).map(([C,z])=>`${encodeURIComponent(C)}=${encodeURIComponent(z)}`).join("&");u.info("删除的数据",P),u.info("保存的数据",b);let Q=await G.updateUserSetsInfo(e,l,P);if(!Q)return;let k=Q.querySelector(".change-script-set");if(!k){v.error(r.t("删除失败，{{selector}}元素不存在",{selector:".change-script-set"}));return}let j=new FormData(k);if(n(j,t)){u.error("删除失败，提交的删除请求中包含该脚本id"),v.error(r.t("删除失败，表单数据中仍包含该脚本"));return}await G.updateUserSetsInfo(e,l,b),v.success(r.t("删除成功"));}catch(d){console.error(d);}finally{s.close();}});},ve={init(){B.isCode()?fe.init():B.isVersion()&&ge.init(),B.isCodeStrict()&&(h.execMenuOnce("fullScreenOptimization",()=>{this.fullScreenOptimization();}),h.execMenuOnce("addCopyCodeButton",()=>{this.addCopyCodeButton();})),h.execMenuOnce("addCollectionButton",()=>{this.addCollectionButton();}),h.execMenuOnce("addFindReferenceButton",()=>{this.setFindCodeSearchBtn();}),c.ready(()=>{h.execMenuOnce("scriptHomepageAddedTodaySUpdate",()=>{this.scriptHomepageAddedTodaySUpdate();});});},addCollectionButton(){u.info("添加收藏按钮"),g.waitNode("ul#script-links li.current span").then(()=>{let t=c.createElement("li",{innerHTML:`
					<a href="javascript:;">
						<span>${r.t("收藏")}</span>
					</a>`});c.append(T("ul#script-links"),t),c.on(t,"click",()=>{let e=window.location.pathname.match(/scripts\/([\d]+)/i);if(!e){u.error(e,window.location.pathname),v.error(r.t("获取脚本id失败"));return}let A=e[e.length-1];Nt(A);});});},fullScreenOptimization(){u.info("F11全屏，F键代码全屏"),M(`
        ...
        `);let t=false;c.keydown(X,function(e){if(e.key.toLowerCase()==="f"){let A=T(".monaco-editor")||T("#script-content div.code-container code");e.altKey&&e.shiftKey?(u.info("宽屏"),g.preventEvent(e),A.classList.contains("code-wide-screen")?A.classList.remove("code-wide-screen"):A.classList.add("code-wide-screen")):!e.altKey&&!e.ctrlKey&&!e.shiftKey&&!e.metaKey&&(u.info("全屏"),g.preventEvent(e),t?(g.exitFullScreen(A),t=false):(g.enterFullScreen(A),t=true));}},{capture:true});},setFindCodeSearchBtn(){u.info("设置代码搜索按钮(对于库)"),g.waitNode("ul#script-links li.current span").then(()=>{let t=c.createElement("li",{innerHTML:`
					<a href="javascript:;">
						<span>${r.t("寻找引用")}</span>
					</a>`});c.append(T("ul#script-links"),t),c.on(t,"click",async function(){let e=window.location.pathname.match(/scripts\/([\d]+)/i);if(!e){u.error(e,window.location.pathname),v.error(r.t("获取脚本id失败"));return}let A=e[e.length-1];window.location.href=S.getCodeSearchUrl(`/scripts/${A}`);});});},async scriptHomepageAddedTodaySUpdate(){if(!T("#install-area"))return;u.info("脚本首页新增【今日检查】");let t=await G.getScriptStats(S.getScriptId());if(!t)return;u.info("统计信息",t);let e=t[g.formatTime(void 0,"yyyy-MM-dd")];if(!e){u.error("今日份的统计信息不存在");return}let A=e.update_checks;u.info("今日统计信息",e),c.after("dd.script-show-daily-installs",c.createElement("dt",{className:"script-show-daily-update_checks",innerHTML:`<span>${r.t("今日检查")}</span>`})),c.after("dt.script-show-daily-update_checks",c.createElement("dd",{className:"script-show-daily-update_checks",innerHTML:"<span>"+A+"</span>"}));},addCopyCodeButton(){u.info("添加复制代码按钮"),g.waitNode("div#script-content div.code-container").then(t=>{let e=c.createElement("button",{textContent:r.t("复制代码")},{style:"margin-bottom: 1em;"});c.on(e,"click",async function(){let A=v.loading(r.t("加载文件中...")),n=S.getScriptId(),i=await G.getScriptInfo(n);if(!i){A.close();return}let a=i.code_url;u.success("代码地址：",a);let o=await q.get(a);if(!o.status){A.close();return}A.close(),g.setClip(o.data.responseText),v.success(r.t("复制成功"));}),c.before(t,e);});}},ot={key:"gf-shield-rule",init(){u.info("脚本过滤");let t=new g.LockFunction(()=>{this.filter();},50);c.ready(()=>{if(Dt.isCurrentLoginUserHome()){u.warn("当前在已登录的账户主页下，禁用脚本过滤");return}g.mutationObserver(document.body,{config:{subtree:true,childList:true},immediate:true,callback:()=>{t.run();}}),t.run();});},getElementList(){return [...Array.from(K("ol.script-list li[data-script-name]"))]},filter(){this.getElementList().forEach(t=>{let e=lt.parseScriptListInfo(t),A=this.getValue().split(`
`);for(let n=0;n<A.length;n++){let i=A[n],a=i.split("##"),o=a[0],l=a[1];if(o==="scriptRatingScore"){let s=parseFloat(l.slice(1));if(l.startsWith(">")){if(e.scriptRatingScore>s){u.info("触发脚本过滤规则",[i,e]),t.remove();break}}else if(l.startsWith("<")&&e.scriptRatingScore<s){u.info("触发脚本过滤规则",[i,e]),t.remove();break}}else if(o in e||o==="scriptDescription"){if(typeof l!="string")continue;let s=new RegExp(l,"ig");if(String(e[o]).match(s)){u.info("触发脚本过滤规则",i,e),t.remove();break}}}});},setValue(t){h.setValue(this.key,t);},addValue(t,e){let A=this.getValue();if(A.trim()!==""&&(A+=`
`),g.isNull(t))return;t=t.toString().trim();let n=t+"##"+e;A+=n,this.setValue(A);},getValue(){return h.getValue(this.key,"")}},he=`.sidebarred-main-content {\r
	max-width: unset;\r
	flex: unset;\r
}\r
ol.script-list {\r
...
`,xt={getTampermonkey:()=>X.external?.Tampermonkey,getViolentmonkey:()=>X.external?.Violentmonkey,getScriptCat:()=>X.external?.Scriptcat,getScriptContainerStatus(){let t={Tampermonkey:false,Violentmonkey:false,ScriptCat:false};return X.external?.Tampermonkey&&(t.Tampermonkey=true),X.external?.Violentmonkey&&(t.Violentmonkey=true),X.external?.Scriptcat&&(t.ScriptCat=true),t},getRegisterScriptContainerNameList(){let e=this.getScriptContainerStatus(),A=[];return Object.keys(e).forEach(n=>{Reflect.get(e,n)&&A.push(n);}),A},getInstalledVersion(t,e){return new Promise((A,n)=>{const i=this.getTampermonkey();if(i){i.isInstalled(t,e,function(l){l.installed?A(l.version):A(null);});return}const a=this.getViolentmonkey();if(a){a.isInstalled(t,e).then(A);return}const o=this.getScriptCat();if(o){o.isInstalled(t,e,function(l){l.installed?A(l.version):A(null);});return}n(new TypeError("获取脚本容器暴露的external信息失败"));})},compareVersions(t,e){if(t===e)return 0;const A=t.split("."),n=e.split(".");for(let i=0;i<A.length;i++){const a=this.compareVersionPart(A[i],n[i]);if(a!==0)return a}return 0},compareVersionPart(t,e){const A=this.parseVersionPart(t),n=this.parseVersionPart(e);for(let i=0;i<A.length;i++){if(A[i].length>0&&n[i].length===0)return  -1;if(A[i].length===0&&n[i].length>0||A[i]>n[i])return 1;if(A[i]<n[i])return  -1}return 0},parseVersionPart(t){if(!t)return [0,"",0,""];const e=/([0-9]*)([^0-9]*)([0-9]*)([^0-9]*)/.exec(t);return [e[1]?parseInt(e[1]):0,e[2],e[3]?parseInt(e[3]):0,e[4]]},handleInstallResult(t,e,A){if(e!=null)switch(t.removeAttribute("data-ping-url"),this.compareVersions(e,A)){case  -1:t.textContent=t.getAttribute("data-update-label");break;case 1:t.textContent=t.getAttribute("data-downgrade-label");break;case 0:t.textContent=t.getAttribute("data-reinstall-label");break}},async checkForUpdatesJS(t,e){const A=t.getAttribute("data-script-name"),n=t.getAttribute("data-script-namespace"),i=t.getAttribute("data-script-version");try{let a=await this.getInstalledVersion(A,n);return a==null?!1:(this.handleInstallResult(t,a,i),!0)}catch{if(e){await g.sleep(1e3);try{return await this.checkForUpdatesJS(t,!1)}catch{}}return  false}},checkForUpdatesCSS(t){const e=t.getAttribute("data-script-name"),A=t.getAttribute("data-script-namespace");postMessage({type:"style-version-query",name:e,namespace:A,url:location.href},location.origin);}},Qt={init(){h.execMenuOnce("gf-scripts-filter-enable",()=>{ot.init();}),h.execMenuOnce("beautifyCenterContent",()=>this.beautifyCenterContent());},beautifyCenterContent(){u.info("美化脚本列表-双列");let t=[];t.push(M(he));const e="lum-lightbox-loader",A=r.t("安装此脚本");return O.ready(()=>{let n=xt.getScriptContainerStatus(),i=Object.values(n).find(d=>d),a=xt.getRegisterScriptContainerNameList(),o=h.getValue("beautifyCenterContent-queryNameSpace");i?u.info("当前暴露的external信息："+a.map(d=>`【${d}】`).join("、")):u.error("脚本容器未暴露external信息",window.external);let l=new g.LockFunction(()=>{ot.getElementList().forEach(f=>{if(f.querySelector(".script-list-operation"))return;let p=lt.parseScriptListInfo(f),m=f.querySelector(".inline-script-stats");if(!m)return;let y=p.codeUrl,P=O.createElement("dt",{className:"script-list-rating-score",innerHTML:`<span>${r.t("评分")}</span>`}),b=O.createElement("dd",{className:"script-list-rating-score",innerHTML:`<span>${p.scriptRatingScore}</span>`},{"data-position":"right"}),Q=f.querySelector("dd.script-list-ratings .good-rating-count"),k=f.querySelector("dd.script-list-ratings .ok-rating-count"),j=f.querySelector("dd.script-list-ratings .bad-rating-count");if(Q&&k&&j){let E=parseInt(Q.innerText),I=parseInt(k.innerText),H=parseInt(j.innerText),F=E+I+H;F>=10?E/F>=.6?b.classList.add("good-rating-count"):b.classList.add("bad-rating-count"):F==0||E>I+H?b.classList.add("good-rating-count"):b.classList.add("bad-rating-count");}let w=O.createElement("dt",{className:"script-list-version",innerHTML:`<span>${r.t("版本")}</span>`}),C=O.createElement("dd",{className:"script-list-version",innerHTML:`<span>${p.scriptVersion}</span>`},{"data-position":"right"}),z=O.createElement("dt",{className:"script-list-operation",innerHTML:`<span>${r.t("操作")}</span>`}),N=O.createElement("dd",{className:"script-list-operation",innerHTML:`
						<a
							target="_blank"
							class="install-link"
						...
						`},{"data-position":"right",style:"gap:10px;display: flex;flex-wrap: wrap;align-items: center;"}),D=N.querySelector(".script-collect-btn"),L=N.querySelector(".install-link");if(L["data-script-info"]=p,O.addClass(L,e),p.scriptType==="library"&&L.remove(),O.on(D,"click",E=>{g.preventEvent(E),Nt(p.scriptId);}),h.getValue("gf-scripts-filter-enable")){let E=O.createElement("button",{className:"script-filter-btn",innerHTML:r.t("过滤")}),I="data-filter-key",H="data-filter-value";O.on(E,"click",F=>{g.preventEvent(F);let et=R.alert({title:{text:r.t("选择需要过滤的选项"),position:"center"},content:{text:`
									<button ${I}="scriptId" ${H}="^${p.scriptId}$">${r.t("脚本id：{{text}}",{text:p.scriptId})}</button>
									<button ${I}="scriptName" ${H}="^${g.parseStringToRegExpString(p.scriptName)}$">${r.t("脚本名：{{text}}",{text:p.scriptName})}</button>
								...
								`}),J=et.$shadowRoot.querySelector(".pops-alert-content");p.scriptAuthors.forEach(Y=>{let V=O.createElement("button",{innerHTML:r.t("作者id：{{text}}",{text:Y.authorId})});V.setAttribute(I,"scriptAuthorId"),V.setAttribute(H,"^"+Y.authorId+"$");let At=O.createElement("button",{innerHTML:r.t("作者名：{{text}}",{text:Y.authorName})});At.setAttribute(I,"scriptAuthorName"),At.setAttribute(H,"^"+g.parseStringToRegExpString(Y.authorName)+"$"),J.appendChild(V),J.appendChild(At);}),O.on(et.$shadowRoot,"click",`button[${I}]`,Y=>{g.preventEvent(Y);let V=Y.target,At=V.getAttribute(I),ht=V.getAttribute(H);ot.addValue(At,ht),et.close(),ot.filter(),v.success(r.t("添加成功"));});}),N.appendChild(E);}m.appendChild(P),m.appendChild(b),m.appendChild(w),m.appendChild(C),m.appendChild(z),m.appendChild(N);});},100),s=new g.LockFunction(async()=>{let d=Array.from(K(".install-link[data-install-format=js]:not([gm-is-check-install-status])"));for(let f=0;f<d.length;f++){const p=d[f];p.setAttribute("gm-is-check-install-status","");let m=Reflect.get(p,"data-script-info");if(i){if(o){let y=await G.getScriptInfo(m.scriptId);y&&p.setAttribute("data-script-namespace",y.namespace);}xt.checkForUpdatesJS(p,true).then(y=>{O.removeClass(p,e),y||O.text(p,A);});}else O.removeClass(p,e),O.text(p,A);}});g.mutationObserver(document,{config:{subtree:true,childList:true},immediate:true,callback:()=>{l.run(),s.run();}});}),t}},me={init(){h.execMenuOnce("users-changeConsoleToTopNavigator",()=>{this.changeConsoleToTopNavigator();}),h.execMenuOnce("gf-scripts-filter-enable",()=>{ot.init();}),h.execMenuOnce("beautifyCenterContent",()=>Qt.beautifyCenterContent());},changeConsoleToTopNavigator(){u.info("迁移【控制台】到顶部导航栏"),it.addBlockCSS("#about-user"),c.ready(()=>{let t=T("#about-user");if(!t){u.error("#about-user元素不存在");return}t=t.cloneNode(true),lt.registerTopNavMenu({name:r.t("控制台"),className:"scripts-console",clickEvent(e){R.alert({title:{text:r.t("控制台"),position:"center"},content:{text:"",html:true},btn:{ok:{enable:false}},mask:{enable:true,clickEvent:{toClose:true}},drag:true,useShadowRoot:true,width:_.setting.width,height:_.setting.height,zIndex:g.getMaxZIndex(100),style:`
						#about-user{
							border: 0;
							box-shadow: none;
						}
						...
						`}).$shadowRoot.querySelector(".pops-alert-content").appendChild(t);}});});}};class ye{key="short-cut";$data;isWaitPress=false;currentWaitEnterPressInstanceHandler=null;constructor(e){typeof e=="string"&&(this.key=e),this.$data={otherShortCutOptions:[]};}initConfig(e,A){this.hasOption(e)||this.setOption(e,A);}getStorageKey(){return this.key}getLocalAllOptions(){return tt(this.key,[])}hasOption(e){return !!this.getLocalAllOptions().find(i=>i.key===e)}hasOptionValue(e){return this.hasOption(e)?this.getOption(e)?.value!=null:false}getOption(e,A){return this.getLocalAllOptions().find(a=>a.key===e)??A}setOption(e,A){let n=this.getLocalAllOptions(),i=n.findIndex(a=>a.key===e);i==-1?n.push({key:e,value:A}):Reflect.set(n[i],"value",A),W(this.key,n);}emptyOption(e){let A=false,n=this.getLocalAllOptions(),i=n.findIndex(a=>a.key===e);return i!==-1&&(n[i].value=null,A=true),W(this.key,n),A}deleteOption(e){let A=false,n=this.getLocalAllOptions(),i=n.findIndex(a=>a.key===e);return i!==-1&&(n.splice(i,1),A=true),W(this.key,n),A}translateKeyboardValueToButtonText(e){let A="";return e.ohterCodeList.forEach(n=>{A+=g.stringTitleToUpperCase(n,true)+" + ";}),A+=g.stringTitleToUpperCase(e.keyName),A}getShowText(e,A){if(this.hasOption(e)){let n=this.getOption(e);return n.value==null?A:this.translateKeyboardValueToButtonText(n.value)}else return A}async enterShortcutKeys(e){const A=this;return new Promise(n=>{this.isWaitPress=true;let i=c.listenKeyboard(window,"keyup",(a,o,l)=>{const s={keyName:a,keyValue:o,ohterCodeList:l};let d={};try{const f=JSON.stringify(s),p=this.getLocalAllOptions();Array.isArray(this.$data.otherShortCutOptions)&&p.push(...this.$data.otherShortCutOptions);for(let m=0;m<p.length;m++){let y=p[m];if(y.key===e)continue;const P=JSON.stringify(y.value);let b=!1;if(y.value!=null&&f===P&&(b=!0),b){d={status:!1,key:y.key,option:s};return}}this.setOption(e,s),d={status:!0,key:e,option:s};}catch(f){console.log(f),d={status:false,key:e,option:s};}finally{A.isWaitPress=false,i.removeListen(),A.currentWaitEnterPressInstanceHandler=null,n(d);}});A.currentWaitEnterPressInstanceHandler=null,A.currentWaitEnterPressInstanceHandler=()=>{A.isWaitPress=false,i.removeListen();};})}cancelEnterShortcutKeys(){typeof this.currentWaitEnterPressInstanceHandler=="function"&&this.currentWaitEnterPressInstanceHandler();}initGlobalKeyboardListener(e,A){let n=this.getLocalAllOptions();if(!n.length){u.warn("没有设置快捷键");return}const i=this;function a(s,d){c.listenKeyboard(s,"keydown",(f,p,m,y)=>{if(i.isWaitPress)return;A?.isPrevent&&g.preventEvent(y),n=i.getLocalAllOptions();let P=n.findIndex(b=>{let Q=b.value,k={keyName:f,keyValue:p,ohterCodeList:m};if(JSON.stringify(Q)===JSON.stringify(k))return b});if(P!=-1){let b=n[P];b.key in d&&(u.info(["调用快捷键",b]),d[b.key].callback());}},{capture:!!A?.capture});}let o={},l={};Object.keys(e).forEach(s=>{let d=e[s];(d.target==null||typeof d.target=="string"&&d.target==="")&&(d.target="window"),d.target==="window"?Reflect.set(o,s,d):Reflect.set(l,s,d);}),a(window,o),c.ready(()=>{Object.keys(l).forEach(async s=>{let d=l[s];if(typeof d.target=="string")g.waitNode(d.target,1e4).then(f=>{if(!f)return;let p={};Reflect.set(p,s,d),a(f,p);});else if(typeof d.target=="function"){let f=await d.target();if(f==null)return;let p={};Reflect.set(p,s,d),a(f,p);}else {let f={};Reflect.set(f,s,d),a(d.target,f);}});});}}const Ut={shortCut:new ye,shortOption:{"gf-quickReply":{target:()=>{let t=T("form textarea"),e=T('input[name="commit"][type="submit"]');if(t){if(!e){u.error("页面不存在【发表回复】按钮");return}}else {u.error("页面不存在输入框");return}return u.success("监听快捷键：gf-quickReply"),t},callback(){if(document.activeElement){let t=document.activeElement.closest("form");if(!t){u.error("当前activeElement不在表单内，无法触发快捷键");return}let e=t.querySelectorAll('input[name="commit"][type="submit"]');if(!e.length){u.error("表单内不存在【发表回复】按钮");return}e.length>1&&u.warn("表单内存在多个【发表回复】按钮，只触发第一个"),e[0].click();}else u.error("当前页面没有激活元素，无法触发快捷键");}}},init(){this.shortCut.initGlobalKeyboardListener(this.shortOption);}},we=`section.text-content {\r
...
.comment:not(:has(.report-link)) .comment-meta-item {\r
	margin-left: 0px;\r
	margin-right: 15px;\r
}`,be={init(){h.execMenuOnce("conversations-beautifyDialogBox",()=>this.beautifyDialogBox()),c.ready(()=>{h.execMenuOnce("conversations-beautifyPrivateMessageList",()=>{this.beautifyPrivateMessageList();});});},beautifyDialogBox(){return u.info("美化对话框"),M(we)},beautifyPrivateMessageList(){u.info("美化私信列表"),M(`
		...
		`),K("section.text-content ul li").forEach(t=>{let e=t.querySelector('a[href*="conversations"]'),A=e.href,n=e.textContent?.split(" ")[1],i=t.querySelector("a.user-link"),a=null,o=null,l=null;if(i){a=i.textContent,o=i.href;let s=t.querySelector("relative-time");new Date(s.getAttribute("datetime")),l=s.shadowRoot.lastChild.textContent;}t.classList.add("user-conversations-item"),S.getUserId(),t.innerHTML=`
			...
			`;});}},Mt={$key:{DB_KEY:"data"},$data:{db:null},init(){this.$data.db=this.getDB(),h.execMenuOnce("rememberReplyContent",()=>{this.rememberReplyContent();}),h.execMenu("gf-autoClearRememberReplayContent",t=>{this.autoClearRememberReplayContent(t.value);});},getDB(){const t="reply_record",e="textarea_text";return new g.indexedDB(t,e,2)},async rememberReplyContent(){const t="记住回复内容 -- ";let e=K("form");if(!e.length){u.warn(t+"不存在表单");return}try{await this.clearRelayHistoryRememberContentText();}catch(A){u.error(A);}e.forEach(async A=>{let n=A.querySelector("textarea"),i=A.querySelector('input[type="submit"]');n&&i&&(u.success("开始监听form --- 记住回复内容",A),this.$data.db.get(this.$key.DB_KEY).then(a=>{if(!a.data)return;let o=a.data.findIndex(s=>this.checkUrlIsSame(window.location.href,s.url));if(o==-1)return;let l=a.data[o].text;u.success("填入历史输入内容："+l),n.value=l;}),c.on(n,["propertychange","input"],g.debounce(a=>{let o={url:window.location.href,text:n.value,time:Date.now()};this.$data.db.get(this.$key.DB_KEY).then(l=>{if(!l.success&&l.event&&l.event.type!=="success"){u.warn(l);return}l.data==null&&(l.data=[]);let s=l.data.findIndex(d=>this.checkUrlIsSame(window.location.href,d.url));s!==-1?g.isNull(o.text)?l.data.splice(s,1):l.data[s]=g.assign(l.data[s],o):l.data=l.data.concat(o),this.$data.db.save(this.$key.DB_KEY,l.data).then(d=>{d.success||u.error("保存失败",d);});});},25)),c.on(A,"submit",a=>{u.info("表单提交，刷新页面后清理内容："+window.location.href),W("delyClear_rememberReplyContent_url",window.location.href);},{capture:true}));});},async clearRelayHistoryRememberContentText(){const t="delyClear_rememberReplyContent_url";let e=tt(t);if(e){let A=await this.$data.db.get(this.$key.DB_KEY);if(!A.data){u.info("表单记录：数据库是空的");return}let n=A.data.findIndex(a=>this.checkUrlIsSame(e,a.url));if(n==-1){u.info("表单记录：已不存在该数据"),Pt(t);return}A.data.splice(n,1),(await this.$data.db.save(this.$key.DB_KEY,A.data)).success?(u.success("表单记录：成功清除"),Pt(t)):u.error("表单记录：清除失败",A);}},checkUrlIsSame(t,e){let A=new URL(t),n=new URL(e);return A.origin===n.origin&&A.pathname===n.pathname},autoClearRememberReplayContent(t){const e="gf-last-time-autoClearRememberReplayContent";let A=tt(e),n=t*24*60*60*1e3;if(A)if(Date.now()-A>n)W(e,Date.now());else return;W(e,Date.now());},async getAllRememberReplyContent(){return (await this.$data.db.get(this.$key.DB_KEY)).data??[]},async clearAllRememberReplyContent(){return (await this.$data.db.delete(this.$key.DB_KEY)).success}},It={waitScritList(){return g.waitNode("#browse-script-list",1e4)},addFilterControls(t){function e(){let s=T("#gm-script-filter-controls");if(!s)return;let d=s?.shadowRoot;if(!d)return;let f=d.querySelector(".pops-filter-controls_inner"),p=d.querySelector(".pops-filter-search-container");if(f&&p)return {$filterControl:f,$search:p}}let A=e();if(A)return A;let n=c.createElement("div",{id:"gm-script-filter-controls"}),i=n.attachShadow({mode:"open"});i.appendChild(c.createElement("style",{innerHTML:`
                    ${R.config.cssText.index}

                    ${R.config.cssText.common}
			...
			`}),o=a.querySelector(".pops-filter-controls_inner"),l=a.querySelector(".pops-filter-search-container");return i.appendChild(a),c.before(t,n),{$filterControl:o,$search:l}}},xe={init(){c.ready(()=>{It.waitScritList().then(t=>{if(!t){u.error("未找到脚本列表节点，无法继续执行");return}h.onceExec("GreasyforkScriptsSearch",()=>{let{$filterControl:e,$search:A}=It.addFilterControls(t);this.addFilterControlsItem(A,e);});});});},addFilterControlsItem(t,e){let A=R.config.PanelHandlerComponents(),n=[{name:r.t("名称"),desc:r.t("全词匹配"),ENABLE_KEY:"gf-script-search-filterScriptTitleWholeWordMatching",STORAGE_KEY:"gf-script-search-filterScriptTitleWholeWordMatching-enable",callback:(o,l)=>{try{return !l.scriptName.match(new RegExp(o,"i"))}catch(s){return u.error(s),!l.scriptName.match(o)}},mutualExclusionSwitch:['.gm-script-control-item[data-storage-key="gf-script-search-filterScriptTitleOrDescWholeWordMatching-enable"]']},{name:r.t("描述"),desc:r.t("全词匹配"),ENABLE_KEY:"gf-script-search-filterScriptDescWholeWordMatching",STORAGE_KEY:"gf-script-search-filterScriptDescWholeWordMatching-enable",callback:(o,l)=>{try{return !l.scriptDescription.match(new RegExp(o,"i"))}catch{return !l.scriptDescription.match(o)}},mutualExclusionSwitch:['.gm-script-control-item[data-storage-key="gf-script-search-filterScriptTitleOrDescWholeWordMatching-enable"]']},{name:r.t("名称/描述"),desc:r.t("任一全词匹配"),ENABLE_KEY:"gf-script-search-filterScriptTitleOrDescWholeWordMatching",STORAGE_KEY:"gf-script-search-filterScriptTitleOrDescWholeWordMatching-enable",callback:(o,l)=>{let s=n.find(f=>f.ENABLE_KEY==="gf-script-search-filterScriptTitleWholeWordMatching").callback(o,l),d=n.find(f=>f.ENABLE_KEY==="gf-script-search-filterScriptDescWholeWordMatching").callback(o,l);return s&&d},mutualExclusionSwitch:['.gm-script-control-item[data-storage-key="gf-script-search-filterScriptTitleWholeWordMatching-enable"]','.gm-script-control-item[data-storage-key="gf-script-search-filterScriptDescWholeWordMatching-enable"]']},{name:r.t("作者名称"),desc:r.t("全词匹配"),ENABLE_KEY:"gf-script-search-filterScriptAuthorNameWholeWordMatching",STORAGE_KEY:"gf-script-search-filterScriptAuthorNameWholeWordMatching-enable",callback:(o,l)=>{try{return !l.scriptAuthorName.match(new RegExp(o,"i"))}catch{return !l.scriptAuthorName.match(o)}}}],i=()=>{let o=t.querySelector("input");if(o)return o.value;let l=new URLSearchParams(window.location.search);return l.get("q")?.trim()||l.get("c")?.trim()||""},a=()=>{let o=i();ot.getElementList().forEach(s=>{let d=lt.parseScriptListInfo(s),f=n.map(p=>{if(o=="")return  false;if(tt(p.STORAGE_KEY))return p.callback(o,d)}).filter(p=>typeof p=="boolean");if(f.length!==0){let p=false;f.forEach(m=>{p=p||m;}),p?c.hide(s,false):c.show(s,false);}else c.show(s,false);});};if(h.getValue("gf-script-search-addFilterSearchInput")){let o=A.createSectionContainerItem_input({type:"input",className:"gm-script-search-input",getValue(){let l=i();return h.getValue("gf-script-search-addFilterSearchInput-text")||l||""},description:r.t("自动执行过滤"),placeholder:r.t("过滤的关键词"),text:r.t("关键词"),callback:g.debounce((l,s)=>{h.setValue("gf-script-search-addFilterSearchInput-text",s.toString()),a();},500)});c.append(t,o);}n.forEach(o=>{if(!h.getValue(o.ENABLE_KEY))return;u.info(`添加按钮${o.name}`);let l=A.createSectionContainerItem_switch({type:"switch",className:"gm-script-control-item",text:o.name,description:o.desc,attributes:{"data-storage-key":o.STORAGE_KEY},getValue(){let s=tt(o.STORAGE_KEY,false);return s=!!s,a(),s},callback(s,d){if(W(o.STORAGE_KEY,d),a(),d&&Array.isArray(o.mutualExclusionSwitch)){let f=l.closest(".pops");o.mutualExclusionSwitch.forEach(p=>{let m=f.querySelector(p);m&&m.querySelector("input")?.checked&&m.querySelector(".pops-panel-switch__core")?.click();});}}});c.append(e,l);});}},ut={init(){if(B.isImageSource()){u.info("Router: 资源界面，不执行脚本功能");return}h.execMenu("checkPage",()=>{this.checkPage();}),ce.init(),Ut.init(),B.isScript()&&ve.init(),(B.isScriptList()||B.isScriptLibraryList()||B.isScriptCodeSearch()||B.isScriptsBySite())&&Qt.init(),B.isDiscuessions()?(u.info("Router: 讨论页面"),ue.init()):B.isUsers()?(u.info("Router: 用户页面"),me.init(),B.isUsersConversations()&&(u.info("Router-next: 私聊用户页面"),be.init())):(B.isScriptSearch()||B.isScriptsBySite()||B.isScriptCodeSearch()||B.isScriptLibraryListSearch())&&(u.info("Router: 脚本搜索页面"),xe.init()),h.execMenuOnce("scripts-addOperationPanelBtnWithNavigator",()=>{this.addOperationPanelBtnWithNavigator();}),c.ready(()=>{U.initEnv(),de.init(),Mt.init(),U.handleLocalGotoCallBack(),h.execMenuOnce("fixImageWidth",()=>{ut.fixImageWidth();}),ut.languageSelectorLocale(),h.execMenuOnce("optimizeImageBrowsing",()=>{ut.optimizeImageBrowsing();}),h.execMenuOnce("overlayBedImageClickEvent",()=>{ut.overlayBedImageClickEvent();}),B.isCodeStrict()||h.execMenuOnce("addMarkdownCopyButton",()=>{ut.addMarkdownCopyButton();});});},fixImageWidth(){window.innerWidth<window.innerHeight&&(u.info("修复图片显示问题"),M(`
            img.lum-img{
                width: 100% !important;
                height: 100% !important;
            }
          `));},optimizeImageBrowsing(){u.info("优化图片浏览"),M(Ct("ViewerCSS")),M(`
        ...
        `);function t(A=[],n=0){let i="";A.forEach(l=>{i+=`<li><img data-src="${l}" loading="lazy"></li>`;});let a=c.createElement("ul",{innerHTML:i}),o=new Ht(a,{inline:false,url:"data-src",zIndex:g.getMaxZIndex()+100,hidden:()=>{o.destroy();}});n=n<0?0:n,o.view(n),o.zoomTo(1),o.show();}function e(A){return A.getAttribute("data-src")||A.getAttribute("src")||A.getAttribute("alt")}c.on(document,"click","img",function(A){let n=A.target;if(n.parentElement?.localName==="a"&&n.hasAttribute("data-screenshots")||n.closest(".viewer-container")||n.closest(".lum-lightbox-position-helper"))return;let i=n.closest(".user-content"),a=[],o=0,l=[],s=e(n);if(s){if(s.startsWith("https://img.shields.io"))return;if(s.startsWith("/vite/assets/"))return}i?(i.querySelectorAll("img").forEach(d=>{l.push(d);let f=e(d),p=d.parentElement;p?.localName==="a"&&(f=p.getAttribute("data-href")||p.href),a.push(f);}),o=l.indexOf(n),o===-1&&(o=0)):(a.push(s),o=0),u.success("点击浏览图片👉",a,o),t(a,o);}),K(".user-screenshots").forEach(A=>{let n=A.querySelector("a");if(!n)return;let i=n.getAttribute("data-href")||n.getAttribute("href"),a=A.querySelector("img");a&&(a.setAttribute("data-screenshots","true"),a.setAttribute("data-src",i),n.setAttribute("href","javascript:;"),c.after(n,a),n.remove());});},overlayBedImageClickEvent(){u.info("覆盖图床图片的parentElement的a标签"),K(".user-content a>img").forEach(t=>{let e=t.parentElement,A=e.getAttribute("href");if(e.setAttribute("data-href",A),e.removeAttribute("href"),A.startsWith("/rails/active_storage/blobs/redirect")){u.info("该图片是上传到Greasyfork的图片，拦截默认行为，不做提示");return}c.on(e,"click",()=>{v.warning(`<div style="overflow-wrap: anywhere;">${r.t("拦截跳转：")}<a href="${A}" target="_blank">${A}</a></div>`,{isHTML:true,zIndex:g.getMaxZIndex()+105});});});},addMarkdownCopyButton(){u.info("在Markdown右上角添加复制按钮"),M(`
        pre{
          position: relative;
        ...
        `);function t(){let e=c.createElement("div",{className:"zeroclipboard-container",innerHTML:`
            ...
            `}),A=e.querySelector(".js-clipboard-copy"),n=e.querySelector(".octicon-copy"),i=e.querySelector(".octicon-check-copy");return c.on(e,"click",()=>{let a=c.parent(e),o=a.querySelector("code");if(!o&&a.className.includes("prettyprinted")&&(o=e.parentElement),o||(o=a.querySelector("pre")),!o){v.error(r.t("未找到{{selector}}元素",{selector:"code"}));return}g.setClip(c.text(o)),A.setAttribute("success","true"),n.setAttribute("aria-hidden","true"),i.removeAttribute("aria-hidden");let l=R.tooltip({target:A,content:r.t("✅ 复制成功!"),position:"left",className:"github-tooltip",alwaysShow:true});l.toolTip.onToolTipAnimationFinishEvent(),setTimeout(()=>{A.removeAttribute("success"),i.setAttribute("aria-hidden","true"),n.removeAttribute("aria-hidden"),l.toolTip.close();},2e3);}),e}K("pre").forEach(e=>{if(e.querySelector("div.zeroclipboard-container"))return;let n=t(),i=c.createElement("div",{className:"snippet-clipboard-content"});c.before(e,i),i.appendChild(e),i.appendChild(n);});},languageSelectorLocale(){let t=h.getValue("language-selector-locale"),e=window.location.pathname.split("/").filter(A=>!!A)[0];if(u.success("选择语言："+t),u.success("当前语言："+e),!g.isNull(t)&&t!==e){let A=null,n=S.getSwitchLanguageUrl(t);G.switchLanguage(n),u.success("新Url："+n),v.loading(r.t("当前语言：{{currentLocaleLanguage}}，，3秒后切换至：{{localeLanguage}}",{currentLocaleLanguage:e,localeLanguage:t}),{timeout:3e3,showClose:true,onClose(){clearTimeout(A);}}),v.info(r.t("导航至：")+n,{timeout:3e3}),A=setTimeout(()=>{window.location.href=n;},3e3);}},checkPage(){u.info("检测gf页面是否正确加载，有时候会出现"),c.ready(()=>{if(document.body.firstElementChild&&document.body.firstElementChild.localName==="p"&&document.body.firstElementChild.innerText.includes("We're down for maintenance. Check back again soon.")){let t=parseInt(tt("greasyfork-check-page-time",0)),e=h.getValue("greasyfork-check-page-timeout",5),A=e*1e3;if(t&&Date.now()-t<A){v.warning(r.t("上次重载时间 {{time}}，{{timeout}}秒内拒绝反复重载",{time:g.formatTime(t,"yyyy-MM-dd HH:mm:ss"),timeout:e}));return}W("greasyfork-check-page-time",Date.now()),window.location.reload();}});},addOperationPanelBtnWithNavigator(){u.info("添加【操作面板】按钮"),it.addBlockCSS(".sidebarred .sidebar",".sidebarred-main-content .open-sidebar"),M(`
		.sidebarred .sidebarred-main-content{
			max-width: 100%;
		}	
		`),c.ready(()=>{T("#site-nav nav"),T("#site-nav .with-submenu nav");let t=T("#script-list-option-groups")||T(".list-option-groups");if(!t){u.warn("不存在右侧面板元素#script-list-option-groups");return}t=t.cloneNode(true),t.classList.add("option-panel-groups"),lt.registerTopNavMenu({name:r.t("操作面板"),className:"filter-scripts",clickEvent(e){R.alert({title:{text:r.t("操作面板"),position:"center"},content:{text:"",html:true},btn:{ok:{enable:false}},mask:{enable:true,clickEvent:{toClose:true}},drag:true,useShadowRoot:true,width:_.setting.width,height:_.setting.height,zIndex:g.getMaxZIndex(100),style:`
						...
						`}).$shadowRoot.querySelector(".pops-alert-content").appendChild(t);}});});}},Le={init(){},showView(){let t=R.alert({title:{text:r.t("Url To WebhookUrl"),position:"center"},content:{text:`
            ...
            `}),e=t.$shadowRoot.querySelector("#github"),A=t.$shadowRoot.querySelector("#webhook");c.on(e,["input","propertychange"],n=>{let i=e.value.trim(),a=[];i.split(`
`).forEach(o=>{try{if(o=o.trim(),g.isNull(o))return;let s=new URL(o).pathname.split("/"),{1:d,2:f,3:p,4:m}=s,y="";if(s.length>=6&&p==="blob")y=s.slice(5,s.length).join("/");else if(s.length>=8&&p==="raw"&&m==="refs")m=s[6],y=s.slice(7,s.length).join("/");else return;if(y==="")return;a.push(`https://raw.githubusercontent.com/${d}/${f}/${m}/${y}`);}catch{}}),A.value=a.join(`
`);});}},ft=function(t,e,A,n,i,a,o,l,s,d){let f={text:t,type:"button",attributes:{},props:{},description:e,buttonIcon:n,buttonIsRightIcon:i,buttonIconIsLoading:a,buttonType:o,buttonText:A,callback(p){typeof l=="function"&&l(p);},afterAddToUListCallBack:s};return Reflect.set(f.attributes,St,()=>{f.disable=false;}),f},Et={$data:{__storeApiFn:null,get storeApiValue(){return this.__storeApiFn||(this.__storeApiFn=new at.Dictionary),this.__storeApiFn}},getStorageApi(t){if(this.hasStorageApi(t))return this.$data.storeApiValue.get(t)},hasStorageApi(t){return this.$data.storeApiValue.has(t)},setStorageApi(t,e){this.$data.storeApiValue.set(t,e);},initComponentsStorageApi(t,e,A){let n;this.hasStorageApi(t)?n=this.getStorageApi(t):n=A,this.setComponentsStorageApiProperty(e,n);},setComponentsStorageApiProperty(t,e){Reflect.set(t.props,st,e);}},Lt=function(t,e,A,n,i,a="",o,l,s,d){let f={text:t,type:"input",isNumber:!!o,isPassword:!!l,attributes:{},props:{},description:n,afterAddToUListCallBack:s,getValue(){return this.props[st].get(e,A)},callback(p,m,y){this.props[st].set(e,m);},placeholder:a};return Reflect.set(f.attributes,gt,e),Reflect.set(f.attributes,vt,A),Et.initComponentsStorageApi("input",f,{get(p,m){return h.getValue(p,m)},set(p,m){h.setValue(p,m);}}),f},x=function(t,e,A,n,i,a,o,l){let s={text:t,type:"switch",description:i,disabled:o,attributes:{},props:{},getValue(){return this.props[st].get(e,A)},callback(d,f){let p=!!f;u.success(`${p?"开启":"关闭"} ${t}`),this.props[st].set(e,p);},afterAddToUListCallBack:a};return Reflect.set(s.attributes,gt,e),Reflect.set(s.attributes,vt,A),Et.initComponentsStorageApi("switch",s,{get(d,f){return h.getValue(d,f)},set(d,f){h.setValue(d,f);}}),s},dt=function(t,e,A,n,i,a,o){let l=[];typeof n=="function"?l=n():l=n;let s={text:t,type:"select",description:a,attributes:{},props:{},getValue(){return this.props[st].get(e,A)},callback(d,f,p){let m=f;if(u.info(`选择：${p}`),typeof i=="function"&&i(d,m,p))return;this.props[st].set(e,m);},data:l};return Reflect.set(s.attributes,gt,e),Reflect.set(s.attributes,vt,A),Et.initComponentsStorageApi("select",s,{get(d,f){return h.getValue(d,f)},set(d,f){h.setValue(d,f);}}),s},Pe=function(t,e,A,n,i,a="default",o){let l=typeof i=="function"?i():i;typeof n=="object"&&o.initConfig(A,n);let s=()=>o.getShowText(A,l),d=ft(t,e,s,"keyboard",false,false,a,async f=>{let m=f.target.closest(".pops-panel-button")?.querySelector("span");if(o.isWaitPress){v.warning("请先执行当前的录入操作");return}if(o.hasOptionValue(A))o.emptyOption(A),v.success("清空快捷键");else {let y=v.loading("请按下快捷键...",{showClose:true,onClose(){o.cancelEnterShortcutKeys();}}),{status:P,option:b,key:Q}=await o.enterShortcutKeys(A);y.close(),P?(u.success(["成功录入快捷键",b]),v.success("成功录入")):v.error(`快捷键 ${o.translateKeyboardValueToButtonText(b)} 已被 ${Q} 占用`);}m.innerHTML=s();});return d.attributes={},Reflect.set(d.attributes,St,()=>false),d},Ot={async UIScriptList(t,e){if(!U.isLogin){v.error(r.t("请先登录账号！"));return}let i=U.getUserLinkElement().href?.split("/")?.pop()?.match(/([0-9]+)/)?.[0],a=R.loading({mask:{enable:true},parent:e,content:{text:r.t("获取信息中，请稍后...")},addIndexCSS:false}),o=await G.getUserInfo(i);if(a.close(),!o)return;u.info(o);let l=t==="script-list"?o.scriptList:o.scriptLibraryList;v.success(r.t("获取成功，共 {{count}} 个",{count:l.length}));for(const s of l){let d=c.createElement("li",{className:"w-script-list-item",innerHTML:`
				...
				<div class="w-script-update-time">
					<p>${r.t("更新：")}${g.getDaysDifference(new Date(s.code_updated_at).getTime(),void 0,"auto")}前</p>
				</div>
				</div>
            `}),f=d.querySelector(".w-script-info"),p=c.createElement("div",{className:"pops-panel-button",innerHTML:`
				<button type="primary" data-icon="" data-righticon="false">
				<span>${r.t("同步代码")}</span>
				</button>
				`});s.deleted&&(d.classList.add("w-script-deleted"),p.querySelector("button").setAttribute("disabled","true")),c.on(p,"click",async function(){u.success("同步",s);let m=p.querySelector("button"),y=p.querySelector("button span"),P=c.createElement("i",{className:"pops-bottom-icon",innerHTML:R.config.iconSVG.loading},{"is-loading":true});m.setAttribute("disabled","true"),m.setAttribute("data-icon","true"),y.innerText=r.t("同步中..."),c.before(y,P);let b=s?.id,Q=await G.getSourceCodeSyncFormDataInfo(b.toString());if(Q){const{formData:k,url:j}=Q,w="script[script_sync_type_id]",C="script[sync_type]";if(k.has(w)||k.has(C)){let z="";if(k.has(w)){let L=k.get(w);L.toString()==="1"?z=r.t("手动"):L.toString()==="2"?z=r.t("自动"):L.toString()==="3"&&(z="webhook");}else k.has(C)&&(z=k.get(C));let N=d.querySelector(".w-script-sync-type");N?N.querySelector("p").innerText=r.t("同步方式：{{syncMode}}",{syncMode:z}):c.append(f,`
								<div class="w-script-sync-type">
									<p>${r.t("同步方式：{{syncMode}}",{syncMode:z})}
									</p>
								</div>`),await G.sourceCodeSync(s.id.toString(),k,j)?v.success(r.t("同步成功")):v.error(r.t("同步失败"));}else v.error(r.t("该脚本未设置同步信息"));}m.removeAttribute("disabled"),m.removeAttribute("data-icon"),y.innerText=r.t("同步代码"),P.remove();}),d.appendChild(p),e.appendChild(d);}}},Ce={id:"greasy-fork-panel-config-account",title:r.t("通用"),forms:[{text:"",type:"forms",forms:[{text:r.t("Toast配置"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[dt(r.t("Toast位置"),"qmsg-config-position","bottom",[{value:"topleft",text:r.t("左上角")},{value:"top",text:r.t("顶部")},{value:"topright",text:r.t("右上角")},{value:"left",text:r.t("左边")},{value:"center",text:r.t("中间")},{value:"right",text:r.t("右边")},{value:"bottomleft",text:r.t("左下角")},{value:"bottom",text:r.t("底部")},{value:"bottomright",text:r.t("右下角")}],(t,e,A)=>{u.info("设置当前Qmsg弹出位置"+A);},r.t("Toast显示在页面九宫格的位置")),dt(r.t("最多显示的数量"),"qmsg-config-maxnums",3,[{value:1,text:"1"},{value:2,text:"2"},{value:3,text:"3"},{value:4,text:"4"},{value:5,text:"5"}],void 0,r.t("限制Toast显示的数量")),x(r.t("逆序弹出"),"qmsg-config-showreverse",false,void 0,r.t("修改Toast弹出的顺序"))]}]},dt(r.t("语言"),"setting-language","zh-CN",[{value:"zh-CN",text:"中文"},{value:"en-US",text:"English"}],(t,e,A)=>{u.info("改变语言："+A),r.changeLanguage(e);})]},{text:"",type:"forms",forms:[{text:r.t("账号/密码"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[Lt(r.t("账号"),"user","",void 0,void 0,r.t("请输入账号")),Lt(r.t("密码"),"pwd","",void 0,void 0,r.t("请输入密码"),false,true),Lt(r.t("secret"),"secret","","两步验证（2FA）",void 0,r.t("请输入secret"),false,true)]},{text:"",type:"forms",forms:[x(r.t("自动登录"),"autoLogin",true,void 0,r.t("自动登录当前保存的账号")),ft(r.t("清空账号/密码"),void 0,r.t("点击清空"),void 0,void 0,false,"default",t=>{if(confirm(r.t("确定清空账号和密码？"))){h.deleteValue("user"),h.deleteValue("pwd"),v.success(r.t("已清空账号/密码"));let e=t.target.getRootNode();e.querySelector('li[data-key="user"] .pops-panel-input input').value="",e.querySelector('li[data-key="pwd"] .pops-panel-input input').value="";}})]}]},{text:r.t("功能"),type:"deepMenu",forms:[{text:r.t("功能"),type:"forms",forms:[dt(r.t("固定当前语言"),"language-selector-locale","",(function(){let t=[{value:"",text:r.t("无")}];return c.ready(()=>{K("select.language-selector-locale option").forEach(e=>{let A=e.getAttribute("value");if(A==="help")return;let n=c.text(e).trim();t.push({value:A,text:n});});}),t})()),x(r.t("修复图片宽度显示问题"),"fixImageWidth",true,void 0,r.t("修复图片在移动端宽度超出浏览器宽度问题")),x(r.t("优化图片浏览"),"optimizeImageBrowsing",true,void 0,r.t("使用Viewer浏览图片")),x(r.t("覆盖图床图片跳转"),"overlayBedImageClickEvent",true,void 0,r.t("配合上面的【优化图片浏览】更优雅浏览图片")),x(r.t("添加【操作面板】按钮"),"scripts-addOperationPanelBtnWithNavigator",true,void 0,r.t("在脚本列表页面时为顶部导航栏添加【操作面板】按钮")),x(r.t("给Markdown添加【复制】按钮"),"addMarkdownCopyButton",true,void 0,r.t("在Markdown内容右上角添加【复制】按钮，点击一键复制Markdown内容"))]},{text:r.t("检测页面加载"),type:"forms",forms:[x(r.t("启用"),"checkPage",true,void 0,r.t("检测Greasyfork页面是否正常加载，如加载失败则自动刷新页面")),dt(r.t("检测间隔"),"greasyfork-check-page-timeout",5,(()=>{let t=[];for(let e=0;e<5;e++)t.push({value:e+1,text:e+1+"s"});return t})(),void 0,r.t("设置检测上次刷新页面的间隔时间，当距离上次刷新页面的时间超过设置的值，将不再刷新页面"))]}]},{type:"deepMenu",text:r.t("表单"),forms:[{type:"forms",text:"",forms:[x(r.t("记住回复内容"),"rememberReplyContent",true,void 0,r.t("监听表单内的textarea内容改变并存储到indexDB中，提交表单将清除保存的数据，误刷新页面时可动态恢复")),dt(r.t("自动清理空间"),"gf-autoClearRememberReplayContent",7,[{text:r.t("不清理"),value:-1},{text:r.t("{{value}} 天",{value:1}),value:1},{text:r.t("{{value}} 周",{value:1}),value:7},{text:r.t("{{value}} 个月",{value:1}),value:30},{text:r.t("{{value}} 个月",{value:2}),value:60},{text:r.t("{{value}} 个月",{value:3}),value:90},{text:r.t("半年"),value:180}],void 0,r.t("根据设置的间隔时间自动清理保存的回复内容")),ft(r.t("数据占用空间：{{size}}",{size:r.t("计算中")}),r.t("当前存储的数据所占用的空间大小"),r.t("清空"),void 0,void 0,void 0,"default",async()=>{await Mt.clearAllRememberReplyContent()?v.success(r.t("清理成功")):v.error(r.t("清理失败"));},async(t,e)=>{let A=e.ulElement.querySelector('li[data-key="gf-autoClearRememberReplayContent"]+li .pops-panel-item-left-main-text'),n=await Mt.getAllRememberReplyContent(),i="";n.length?i=g.getTextStorageSize(JSON.stringify(n)):i=g.getTextStorageSize(""),A.innerText=r.t("数据占用空间：{{size}}",{size:i});})]}]},{text:r.t("美化"),type:"deepMenu",forms:[{text:r.t("全局"),type:"forms",forms:[x(r.t("美化页面元素"),"beautifyPage",true,void 0,r.t("如button、input、textarea")),x(r.t("美化上传图片按钮"),"beautifyUploadImage",true,void 0,r.t("放大上传区域")),x(r.t("美化顶部导航栏"),"beautifyTopNavigationBar",true,void 0,r.t("可能会跟Greasyfork Beautify脚本有冲突")),x(r.t("美化Greasyfork Beautify脚本"),"beautifyGreasyforkBeautify",false,void 0,r.t('需安装Greasyfork Beautify脚本，<a href="https://greasyfork.org/zh-CN/scripts/446849-greasyfork-beautify" target="_blank">🖐点我安装</a>'))]},{type:"forms",text:r.t("脚本列表"),forms:[x(r.t("美化脚本列表"),"beautifyCenterContent",true,void 0,r.t("双列显示且添加脚本卡片操作项（安装、收藏）")),x("↑"+r.t("使用namespace查询脚本信息"),"beautifyCenterContent-queryNameSpace",true,void 0,r.t("开启后检测已安装的脚本信息更准确，但是速度会更慢"))]}]},{type:"deepMenu",text:r.t("自定义快捷键"),forms:[{type:"forms",text:"",forms:[Pe(r.t("快捷键发表回复"),r.t("在输入框内按下快捷发表回复，例如：{{key}}",{key:"Ctrl + Enter"}),"gf-quickReply",{keyName:"Enter",keyValue:"13",ohterCodeList:["ctrl"]},r.t("点击录入快捷键"),void 0,Ut.shortCut)]}]},{text:r.t("过滤"),type:"deepMenu",forms:[{text:`<a target="_blank" href="https://greasyfork.org/zh-CN/scripts/475722-greasyfork%E4%BC%98%E5%8C%96#:~:text=%E8%84%9A%E6%9C%AC%E8%BF%87%E6%BB%A4%E8%A7%84%E5%88%99">${r.t("帮助文档")}</a>`,type:"forms",forms:[x(r.t("启用"),"gf-scripts-filter-enable",true,void 0,r.t("作用域：脚本、脚本搜索、用户主页")),{type:"own",getLiElementCallBack(t){let e=c.createElement("div",{className:"pops-panel-textarea",innerHTML:`
												<textarea placeholder="${r.t("请输入规则，每行一个")}" style="height:200px;"></textarea>`},{style:"width: 100%;"}),A=e.querySelector("textarea");return A.value=ot.getValue(),c.on(A,["input","propertychange"],void 0,g.debounce(function(n){ot.setValue(A.value);},200)),t.appendChild(e),t}}]}]}]},{type:"forms",text:r.t("脚本管理"),forms:[{type:"deepMenu",text:r.t("代码同步"),forms:[{text:r.t("代码同步"),type:"forms",forms:[ft(r.t("源代码同步【脚本列表】"),void 0,r.t("一键同步"),void 0,void 0,false,"primary",t=>{if(!B.isUsers()){h.setValue("goto_updateSettingsAndSynchronize_scriptList",true),U.getUserLinkElement()?(v.success(r.t("前往用户主页")),window.location.href=U.getUserLinkElement().href):v.error(r.t("获取当前已登录的用户主页失败"));return}let e=[];K("#user-script-list-section li a.script-link").forEach(A=>{e=e.concat(S.getAdminUrl(A.href));}),U.updateScript(e);}),ft(r.t("源代码同步【未上架的脚本】"),void 0,r.t("一键同步"),void 0,void 0,false,"primary",t=>{if(!B.isUsers()){h.setValue("goto_updateSettingsAndSynchronize_unlistedScriptList",true),U.getUserLinkElement()?(v.success(r.t("前往用户主页")),window.location.href=U.getUserLinkElement().href):v.error(r.t("获取当前已登录的用户主页失败"));return}let e=[];K("#user-unlisted-script-list li a.script-link").forEach(A=>{e=e.concat(S.getAdminUrl(A.href));}),U.updateScript(e);}),ft(r.t("源代码同步【库】"),void 0,r.t("一键同步"),void 0,void 0,false,"primary",t=>{if(!B.isUsers()){h.setValue("goto_updateSettingsAndSynchronize_libraryScriptList",true),U.getUserLinkElement()?(v.success(r.t("前往用户主页")),window.location.href=U.getUserLinkElement().href):v.error(r.t("获取当前已登录的用户主页失败"));return}let e=[];K("#user-library-script-list li a.script-link").forEach(A=>{e=e.concat(S.getAdminUrl(A.href));}),U.updateScript(e);})]}]},{type:"deepMenu",text:r.t("脚本列表"),forms:[],afterEnterDeepMenuCallBack(t,e){Ot.UIScriptList("script-list",e.sectionBodyContainer);}},{type:"deepMenu",text:r.t("库"),forms:[],afterEnterDeepMenuCallBack(t,e){Ot.UIScriptList("script-library",e.sectionBodyContainer);}}]}]},ze={id:"greasy-fork-panel-config-scripts",title:r.t("脚本"),forms:[{text:"",type:"forms",forms:[{text:r.t("代码"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[x(r.t("添加复制代码按钮"),"addCopyCodeButton",true,void 0,r.t("更优雅的复制")),x(r.t("快捷键"),"fullScreenOptimization",true,void 0,r.t("【F】键全屏、【Alt+Shift+F】键宽屏")),x(r.t("修复代码行号显示"),"code-repairCodeLineNumber",false,void 0,r.t("修复代码行数超过1k行号显示不全问题")),x("monacoEditor","code-use-monaco-editor",true,void 0,r.t("使用Monaco编辑器"))]}]},{text:r.t("历史版本"),type:"deepMenu",forms:[{text:r.t("功能"),type:"forms",forms:[x(r.t("添加额外的标签按钮"),"scripts-versions-addExtraTagButton",true,void 0,r.t("在版本下面添加【安装】、【查看代码】按钮")),x(r.t("添加代码对比按钮"),"scripts-versions-addCompareCodeButton",true,void 0,r.t("monacoEditor"))]},{text:r.t("美化"),type:"forms",forms:[x(r.t("美化历史版本页面"),"beautifyHistoryVersionPage",true,void 0,r.t("更直观的查看版本迭代"))]}]}]},{text:"",type:"forms",forms:[{text:r.t("功能"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[x(r.t("添加【寻找引用】按钮"),"addFindReferenceButton",true,void 0,r.t("在脚本栏添加按钮，一般用于搜索引用该库的相关脚本")),x(r.t("添加【收藏】按钮"),"addCollectionButton",true,void 0,r.t("在脚本栏添加按钮，一般用于快捷收藏该脚本/库")),x(r.t("添加【今日检查】信息块"),"scriptHomepageAddedTodaySUpdate",true,void 0,r.t("在脚本信息栏添加【今日检查】信息块"))]}]}]}]},Me={id:"greasy-fork-panel-config-script-search",title:r.t("搜索"),forms:[{type:"forms",text:"搜素结果过滤",forms:[x(r.t("新增【关键词】搜索框"),"gf-script-search-addFilterSearchInput",true,void 0,r.t("输入自定义关键词后自动执行过滤")),x(r.t("新增【{{buttonText}}】按钮",{buttonText:r.t("名称")}),"gf-script-search-filterScriptTitleWholeWordMatching",true,void 0,r.t("该Checkbox按钮开启后，自动过滤出包含搜索关键词的脚本")),x(r.t("新增【{{buttonText}}】按钮",{buttonText:r.t("描述")}),"gf-script-search-filterScriptDescWholeWordMatching",true,void 0,r.t("该Checkbox按钮开启后，自动过滤出包含搜索关键词的脚本")),x(r.t("新增【{{buttonText}}】按钮",{buttonText:r.t("名称/描述")}),"gf-script-search-filterScriptTitleOrDescWholeWordMatching",true,void 0,r.t("该Checkbox按钮开启后，自动过滤出包含搜索关键词的脚本")),x(r.t("新增【{{buttonText}}】按钮",{buttonText:r.t("作者名称")}),"gf-script-search-filterScriptAuthorNameWholeWordMatching",true,void 0,r.t("该Checkbox按钮开启后，自动过滤出包含搜索关键词的脚本"))]}]},ke={id:"greasy-fork-panel-config-discussions",title:r.t("论坛"),forms:[{text:"",type:"forms",forms:[{text:r.t("功能"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[{type:"own",attributes:{"data-key":"discussions-readBgColor","data-default-value":"#e5e5e5"},getLiElementCallBack(t){let e="discussions-readBgColor",A=c.createElement("div",{className:"pops-panel-item-left-text",innerHTML:`
											...
											`}),i=n.querySelector(".pops-color-choose");i.value=h.getValue(e);let a=c.createElement("style");return c.append(document.head,a),c.on(i,["input","propertychange"],o=>{u.info("选择颜色："+i.value),a.innerHTML=`
												.discussion-read{
													background: ${i.value} !important;
												}
												`,h.setValue(e,i.value);}),t.appendChild(A),t.appendChild(n),t}},x(r.t("添加【过滤】按钮"),"discussions-addShortcutOperationButton",true,void 0,r.t("在每一行讨论的最后面添加【过滤】按钮，需开启过滤功能才会生效")),x(r.t("添加【举报】按钮"),"discussions-addReportButton",true,void 0,r.t("在每一行讨论的最后面添加【举报】按钮"))]}]},{text:r.t("过滤"),type:"deepMenu",forms:[{text:`<a target="_blank" href="https://greasyfork.org/zh-CN/scripts/475722-greasyfork%E4%BC%98%E5%8C%96#:~:text=%E8%AE%BA%E5%9D%9B%E8%BF%87%E6%BB%A4%E8%A7%84%E5%88%99">${r.t("帮助文档")}</a>`,type:"forms",forms:[x(r.t("启用"),"greasyfork-discussions-filter-enable",true,void 0,r.t("开启后下面的过滤功能才会生效")),x(r.t("过滤重复的评论"),"greasyfork-discussions-filter-duplicate-comments",true,void 0,r.t("过滤掉重复的评论数量(≥2)")),{type:"own",getLiElementCallBack(t){let e=c.createElement("div",{className:"pops-panel-textarea",innerHTML:`
										<textarea placeholder="${r.t("请输入规则，每行一个")}" style="height:200px;"></textarea>`},{style:"width: 100%;"}),A=e.querySelector("textarea");return A.value=$.getValue(),c.on(A,["input","propertychange"],void 0,g.debounce(function(n){$.setValue(A.value);},200)),t.appendChild(e),t}}]}]}]}]},Se={id:"greasy-fork-panel-config-account",title:r.t("用户"),forms:[{text:"",type:"forms",forms:[{text:r.t("功能"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[x(r.t("迁移【控制台】到顶部导航栏"),"users-changeConsoleToTopNavigator",true,void 0,r.t("将【控制台】按钮移动到顶部导航栏，节省空间"))]}]},{text:r.t("美化"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[x(r.t("美化私信页面"),"conversations-beautifyDialogBox",true,void 0,r.t("美化为左右对话模式")),x(r.t("美化私信列表"),"conversations-beautifyPrivateMessageList",true)]}]}]}]},De=`.w-script-list-item {\r
	padding: 10px;\r
...
`;jt.addMenuOption({key:"githubUrl2webhookUrl",text:"⚙ "+r.t("Url To WebhookUrl"),autoReload:false,isStoreValue:false,showText(t){return t},callback:()=>{Le.showView();}});h.$data.panelConfig={style:De};yt.addContentConfig([Ce,ze,Me,ke,Se]);h.init();ut.init();
