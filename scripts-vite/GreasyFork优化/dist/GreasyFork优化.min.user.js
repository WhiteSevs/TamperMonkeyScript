...
`,at={init(){p.execMenuOnce("beautifyPage",()=>this.beautifyPageElement()),p.execMenuOnce("beautifyGreasyforkBeautify",()=>this.beautifyGreasyforkBeautify()),p.execMenuOnce("beautifyUploadImage",()=>this.beautifyUploadImage()),p.execMenuOnce("beautifyTopNavigationBar",()=>this.beautifyTopNavigationBar());},beautifyPageElement(){d.info("美化页面元素");let e=[];return e.push(v(Qe)),e.push(v(Xe)),e.push(v(et)),e.push(v(tt)),e.push(v(rt)),e.push(v(`
			...
			`)),f.ready(function(){let t=document.querySelector('a[target="markup_choice"][href*="daringfireball.net"]');t&&t.parentElement.replaceChild(f.createElement("span",{textContent:"Markdown"}),t),globalThis.location.pathname.endsWith("/admin")&&!document.querySelector('input[type="submit"][name="update-only"]')&&e.push(v(`
					...
					`));}),e},beautifyGreasyforkBeautify(){d.info("美化 Greasyfork Beautify脚本");let e=[];return e.push(v(it)),g.isPhone()?e.push(v(`
				section#script-info,
				...
				section#script-info{
					margin-top: 10px;
				}`)),e},beautifyUploadImage(){d.info("美化上传图片");let e=[];return e.push(v(nt)),f.ready(()=>{function t(o){for(;o.nextElementSibling;)o.parentElement.removeChild(o.nextElementSibling);}let r=document.querySelectorAll('input[type="file"]');r.forEach(o=>{o.getAttribute("name")!=="code_upload"&&(o.hasAttribute("accept")&&o.getAttribute("accept").includes("javascript")||f.on(o,["propertychange","input"],function(a){t(a.target);let s=a.currentTarget.files;if(!s||s.length===0)return;d.info(["选择的图片",s]),s.length>5&&f.after(o,f.createElement("p",{textContent:n.t("❌ 最多同时长传5张图片")}));let l=[];Array.from(s).forEach(c=>{(c.size>204800||!c.type.match(/png|jpg|jpeg|gif|apng|webp/i))&&l.push(c);}),l.length!==0&&l.forEach(c=>{f.after(o,f.createElement("p",{textContent:n.t("❌ 图片：{{name}} 大小：{{size}}",{name:c.name,size:c.size})}));});}));}),["textarea#comment_text","textarea.comment-entry"].forEach(o=>{f.on(o,"paste",a=>{d.info(["触发粘贴事件",a]),setTimeout(()=>{f.trigger(r,"input");},100);});});}),e},beautifyTopNavigationBar(){d.info("美化顶部导航栏");let e=[];return e.push(v(ot)),window.outerWidth>550&&(e.push(re.addBlockCSS(".with-submenu")),f.ready(()=>{let r=document.querySelector("#site-nav").querySelector("nav");document.querySelectorAll(".with-submenu nav li").forEach(i=>{r.appendChild(i);});})),e}},st={init(){p.execMenu("autoLogin",()=>{this.autoLogin();});},autoLogin(){g.waitNode("span.sign-in-link a[rel=nofollow]").then(async()=>{let e=p.getValue("user"),t=p.getValue("pwd");if(g.isNull(e)){m.error(n.t("请先在菜单中录入账号"));return}if(g.isNull(t)){m.error(n.t("请先在菜单中录入密码"));return}let r=document.querySelector("meta[name='csrf-token']");if(!r){m.error(n.t("获取csrf-token失败"));return}let i=m.loading(n.t("正在登录中...")),o=await B.post("https://greasyfork.org/zh-CN/users/sign_in",{fetch:!0,data:encodeURI(`authenticity_token=${r.getAttribute("content")}&user[email]=${e}&user[password]=${t}&user[remember_me]=1&commit=登录`),headers:{"Content-Type":"application/x-www-form-urlencoded"}});if(i.destroy(),!o.status){d.error(o),m.error(n.t("登录失败，请在控制台查看原因"));return}let a=o.data.responseText;f.parseHTML(a,!0,!0).querySelectorAll(".sign-out-link a[rel=nofollow][data-method='delete']").length?(m.success(n.t("登录成功，1s后自动跳转")),setTimeout(()=>{window.location.reload();},1e3)):(d.error(o),d.error(`当前账号:${e}`),d.error(`当前密码:${t}`),m.error(n.t("登录失败，可能是账号/密码错误，请在控制台查看原因")));});}},lt={init(){this.readBgColor(),f.ready(()=>{p.execMenuOnce("greasyfork-discussions-filter-enable",()=>{this.filterEnable();}),p.execMenuOnce("discussions-addShortcutOperationButton",()=>{this.addShortcutOperationButton();});});},filterEnable(){d.info("启用Greasyfork论坛过滤器"),H.init();},readBgColor(){d.info("设置已读背景颜色");let e=p.getValue("discussions-readBgColor");v(`
        .discussion-read{
        ...
        `);},addShortcutOperationButton(){d.info("添加快捷操作按钮"),H.getElementList().forEach(e=>{if(e.querySelector(".discussion-filter-button"))return;let r=e.querySelector(".discussion-list-item").querySelector(".discussion-meta"),i=f.createElement("div",{className:"discussion-meta-item",innerHTML:`
					...
					`}),o=i.querySelector(".discussion-filter-button");r.appendChild(i),f.on(o,"click",a=>{var y,b,x;g.preventEvent(a);const s=H.parseDiscuessionListContainerInfo(e);let l="data-filter-key",c="data-filter-value",u=R.alert({title:{text:n.t("选择需要过滤的选项"),position:"center",html:!1},content:{text:`
                    ...
                    `}).$shadowRoot.querySelector(".pops-drawer-content").appendChild(e);}),t.appendChild(r);});}},dt=`section.text-content {\r
...
.comment:not(:has(.report-link)) .comment-meta-item {\r
	margin-left: 0px;\r
	margin-right: 15px;\r
}`,ut={init(){p.execMenuOnce("conversations-beautifyDialogBox",()=>this.beautifyDialogBox()),f.ready(()=>{p.execMenuOnce("conversations-beautifyPrivateMessageList",()=>{this.beautifyPrivateMessageList();});});},beautifyDialogBox(){d.info("美化对话框"),[].push(te(dt));},beautifyPrivateMessageList(){d.info("美化私信列表"),te(`
		...
		`),document.querySelectorAll("section.text-content ul li").forEach(e=>{var c;let t=e.querySelector('a[href*="conversations"]'),r=t.href,i=(c=t.textContent)==null?void 0:c.split(" ")[1],o=e.querySelector("a.user-link"),a=null,s=null,l=null;if(o){a=o.textContent,s=o.href;let u=e.querySelector("relative-time");new Date(u.getAttribute("datetime")),l=u.shadowRoot.lastChild.textContent;}e.classList.add("user-conversations-item"),E.getUserId(),e.innerHTML=`
			...
			`;});}},Y={init(){p.execMenu("checkPage",()=>{this.checkPage();}),at.init(),be.init(),T.isScript()&&Pe.init(),(T.isScriptList()||T.isScriptLibraryList()||T.isScriptCodeSearch()||T.isScriptsBySite())&&ve.init(),T.isDiscuessions()&&lt.init(),T.isUsers()&&(ct.init(),T.isUsersConversations()&&ut.init()),p.execMenuOnce("scripts-addOperationPanelBtnWithNavigator",()=>{this.addOperationPanelBtnWithNavigator();}),f.ready(()=>{C.initEnv(),st.init(),ce.init(),C.handleLocalGotoCallBack(),p.execMenuOnce("fixImageWidth",()=>{Y.fixImageWidth();}),Y.languageSelectorLocale(),p.execMenuOnce("optimizeImageBrowsing",()=>{Y.optimizeImageBrowsing();}),p.execMenuOnce("overlayBedImageClickEvent",()=>{Y.overlayBedImageClickEvent();}),T.isCodeStrict()||p.execMenuOnce("addMarkdownCopyButton",()=>{Y.addMarkdownCopyButton();});});},fixImageWidth(){window.innerWidth<window.innerHeight&&(d.info("修复图片显示问题"),v(`
            img.lum-img{
                width: 100% !important;
		...
		`),f.ready(()=>{let e=document.querySelector("#site-nav nav"),t=document.querySelector("#site-nav .with-submenu nav"),r=document.querySelector("#script-list-option-groups")||document.querySelector(".list-option-groups");if(!r){d.warn("不存在右侧面板元素#script-list-option-groups");return}if(r=r.cloneNode(!0),r.classList.add("option-panel-groups"),!e){d.error("元素#site-nav nav不存在");return}let i=f.createElement("li",{className:"filter-scripts",innerHTML:`
                ...
                `});f.on(i,"click",o=>{g.preventEvent(o),R.drawer({title:{enable:!1},content:{text:"",html:!0},direction:"top",size:"80%",zIndex:g.getMaxZIndex(100),style:`
