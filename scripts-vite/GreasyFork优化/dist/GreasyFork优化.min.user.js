...
`)}{display: none !important;}`)},setGMResourceCSS(e){let t=typeof ce=="function"?ce(e.keyName):"";typeof t=="string"&&t?te(t):re.addLinkNode(e.url);},async addLinkNode(e){let t=document.createElement("link");return t.rel="stylesheet",t.type="text/css",t.href=e,f.ready(()=>{document.head.appendChild(t);}),t},fixUrl(e){return e=e.trim(),e.match(/^http(s|):\/\//i)||(e.startsWith("/")||(e+="/"),e=window.location.origin+e),e}},Oe={init(){p.execMenuOnce("beautifyHistoryVersionPage",()=>this.beautifyHistoryVersionPage()),p.execMenuOnce("scripts-versions-addExtraTagButton",()=>{this.addExtraTagButton();});},beautifyHistoryVersionPage(){d.info("美化 历史版本 页面");let e=[];return e.push(v(De)),e.push(re.addBlockCSS(".version-number",".version-date",".version-changelog")),f.ready(function(){let t=document.querySelector("ul.history_versions");if(!t){m.error(n.t("未找到history_versions元素列表"));return}Array.from(t.children).forEach(r=>{var h,y;let i=r.querySelector(".version-number a").href,o=r.querySelector(".version-number a").innerText,a=(h=r.querySelector(".version-date"))==null?void 0:h.getAttribute("datetime"),s=((y=r.querySelector(".version-changelog"))==null?void 0:y.innerHTML)||"",l=f.createElement("span",{className:"script-version-date",innerHTML:g.formatTime(a,n.t("yyyy年MM月dd日 HH:mm:ss"))}),c=f.createElement("div",{className:"script-tag",innerHTML:`
		...
		`});f.on(i.$shadowRoot,"click",".collect-add-script-id",async function(o){let a=o.target.closest(".user-collect-item"),s=a.dataset.id;a.dataset.name;let l=m.loading(n.t("添加中...")),c=await $.getUserCollectionInfo(t,s);if(!c){l.close();return}let u=g.cloneFormData(c),h=g.cloneFormData(c),y=!1;for(const[T,L]of c.entries())if(T==="scripts-included[]"&&String(L).trim()===String(e).trim()){y=!0;break}else h.append(T,L),u.append(T,L);if(y){m.warning(n.t("该脚本已经在该收藏集中")),l.close();return}u.set("add-script",e.toString()),u.set("script-action","i"),h.append("scripts-included[]",e.toString()),h.set("save","1");let b=new URLSearchParams(u),x=new URLSearchParams(h),S=Array.from(b).map(([T,L])=>`${encodeURIComponent(T)}=${encodeURIComponent(L)}`).join("&"),_=Array.from(x).map(([T,L])=>`${encodeURIComponent(T)}=${encodeURIComponent(L)}`).join("&");d.info(["添加的数据",S]),d.info(["保存的数据",_]);let I=await $.updateUserSetsInfo(t,s,S);if(!I){l.close();return}let k=I.querySelector(".change-script-set");if(!k){m.error(n.t("添加失败，{{selector}}元素不存在",{selector:".change-script-set"})),l.close();return}let A=k.querySelector("section");if(!A){m.error(n.t("添加失败，{{selector}}元素不存在",{selector:"section"})),l.close();return}let D=A.querySelector(".alert");D?B.alert({title:{text:n.t("添加失败"),position:"center"},content:{text:D.innerHTML,html:!0},mask:{enable:!0,clickEvent:{toClose:!0}},style:`
					.pops-alert-content{
						font-style: italic;
...
`),g.isNull(e))return;e=e.toString().trim();let i=e+"##"+t;r+=i,this.setValue(r);},getValue(){return p.getValue(this.key,"")}},We={id:"greasy-fork-panel-config-discussions",title:n.t("论坛"),forms:[{text:"",type:"forms",forms:[{text:n.t("功能"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[{type:"own",attributes:{"data-key":"discussions-readBgColor","data-default-value":"#e5e5e5"},getLiElementCallBack(e){let t="discussions-readBgColor",r=f.createElement("div",{className:"pops-panel-item-left-text",innerHTML:`
            ...
            `}),_=S.querySelector(".w-script-info"),I=f.createElement("div",{className:"pops-panel-button",innerHTML:`
				<button type="primary" data-icon="" data-righticon="false">
				<span>${n.t("同步代码")}</span>
				</button>
				`});x.deleted&&(S.classList.add("w-script-deleted"),I.querySelector("button").setAttribute("disabled","true")),f.on(I,"click",void 0,async function(){d.success(["同步",x]);let k=I.querySelector("button"),A=I.querySelector("button span"),D=f.createElement("i",{className:"pops-bottom-icon",innerHTML:B.config.iconSVG.loading},{"is-loading":!0});k.setAttribute("disabled","true"),k.setAttribute("data-icon","true"),A.innerText=n.t("同步中..."),f.before(A,D);let T=x==null?void 0:x.id,L=await $.getSourceCodeSyncFormData(T.toString());if(L){const V="script[script_sync_type_id]";if(L.has(V)){let O=L.get(V),P="";O.toString()==="1"?P=n.t("手动"):O.toString()==="2"?P=n.t("自动"):O.toString()==="3"&&(P="webhook");let Z=S.querySelector(".w-script-sync-type");Z?Z.querySelector("p").innerText=n.t("同步方式：{{syncMode}}",{syncMode:P}):f.append(_,`
								<div class="w-script-sync-type">
									<p>${n.t("同步方式：{{syncMode}}",{syncMode:P})}
									</p>
								</div>`),await $.sourceCodeSync(x.id.toString(),L)?m.success(n.t("同步成功")):m.error(n.t("同步失败"));}else m.error(n.t("该脚本未设置同步信息"));}k.removeAttribute("disabled"),k.removeAttribute("data-icon"),A.innerText=n.t("同步代码"),D.remove();}),S.appendChild(I),i.appendChild(S);}}},Ke={id:"greasy-fork-panel-config-script-list",title:n.t("脚本列表"),callback(e,t,r){Se.UIScriptList("script-list",e,t,r);},forms:[]},Je={id:"greasy-fork-panel-config-library",title:n.t("库"),callback(e,t,r){Se.UIScriptList("script-library",e,t,r);},forms:[]},Ye=`.w-script-list-item {\r
	padding: 10px 0;\r
...
.comment:not(:has(.report-link)) .comment-meta-item {\r
	...
	margin-right: 15px;\r
		...
		`),document.querySelectorAll("section.text-content ul li").forEach(e=>{var c;let t=e.querySelector('a[href*="conversations"]'),r=t.href,i=(c=t.textContent)==null?void 0:c.split(" ")[1],o=e.querySelector("a.user-link"),a=null,s=null,l=null;if(o){a=o.textContent,s=o.href;let u=e.querySelector("relative-time");new Date(u.getAttribute("datetime")),l=u.shadowRoot.lastChild.textContent;}e.classList.add("user-conversations-item"),E.getUserId(),e.innerHTML=`
			...
			`;});}},Y={init(){p.execMenu("checkPage",()=>{this.checkPage();}),at.init(),be.init(),M.isScript()&&Pe.init(),(M.isScriptList()||M.isScriptLibraryList()||M.isScriptCodeSearch())&&ve.init(),M.isDiscuessions()&&lt.init(),M.isUsers()&&(ct.init(),M.isUsersConversations()&&ut.init()),p.execMenuOnce("scripts-addOperationPanelBtnWithNavigator",()=>{this.addOperationPanelBtnWithNavigator();}),f.ready(()=>{C.initEnv(),st.init(),de.init(),C.handleLocalGotoCallBack(),p.execMenuOnce("fixImageWidth",()=>{Y.fixImageWidth();}),Y.languageSelectorLocale(),p.execMenuOnce("optimizeImageBrowsing",()=>{Y.optimizeImageBrowsing();}),p.execMenuOnce("overlayBedImageClickEvent",()=>{Y.overlayBedImageClickEvent();}),M.isCodeStrict()||p.execMenuOnce("addMarkdownCopyButton",()=>{Y.addMarkdownCopyButton();});});},fixImageWidth(){window.innerWidth<window.innerHeight&&(d.info("修复图片显示问题"),v(`
            img.lum-img{
                width: 100% !important;
                height: 100% !important;
            }
          `));},optimizeImageBrowsing(){d.info("优化图片浏览"),v(ce("ViewerCSS")),v(`
