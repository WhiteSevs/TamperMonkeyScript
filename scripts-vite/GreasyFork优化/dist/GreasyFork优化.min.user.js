...
.comment:not(:has(.report-link)) .comment-meta-item {\r
	...
	margin-right: 15px;\r
        ...
        `);function e(r=[],i=0){let o="";r.forEach(l=>{o+=`<li><img data-src="${l}" loading="lazy"></li>`;});let s=p.createElement("ul",{innerHTML:o}),a=new Ne(s,{inline:!1,url:"data-src",zIndex:m.getMaxZIndex()+100,hidden:()=>{a.destroy();}});i=i<0?0:i,a.view(i),a.zoomTo(1),a.show();}function t(r){return r.getAttribute("data-src")||r.getAttribute("src")||r.getAttribute("alt")}p.on(document,"click","img",function(r){var d;let i=r.target;if(((d=i.parentElement)==null?void 0:d.localName)==="a"&&i.hasAttribute("data-screenshots")||i.closest(".viewer-container")||i.closest(".lum-lightbox-position-helper"))return;let o=i.closest(".user-content"),s=[],a=0,l=[],c=t(i);if(c){if(c.startsWith("https://img.shields.io"))return;if(c.startsWith("/vite/assets/"))return}o?(o.querySelectorAll("img").forEach(f=>{l.push(f);let y=t(f),b=f.parentElement;(b==null?void 0:b.localName)==="a"&&(y=b.getAttribute("data-href")||b.href),s.push(y);}),a=l.indexOf(i),a===-1&&(a=0)):(s.push(c),a=0),u.success("点击浏览图片👉",s,a),e(s,a);}),document.querySelectorAll(".user-screenshots").forEach(r=>{let i=r.querySelector("a");if(!i)return;let o=i.getAttribute("data-href")||i.getAttribute("href"),s=r.querySelector("img");s&&(s.setAttribute("data-screenshots","true"),s.setAttribute("data-src",o),i.setAttribute("href","javascript:;"),p.after(i,s),i.remove());});},overlayBedImageClickEvent(){u.info("覆盖图床图片的parentElement的a标签"),document.querySelectorAll(".user-content a>img").forEach(e=>{let t=e.parentElement,r=t.getAttribute("href");if(t.setAttribute("data-href",r),t.removeAttribute("href"),r.startsWith("/rails/active_storage/blobs/redirect")){u.info("该图片是上传到Greasyfork的图片，拦截默认行为，不做提示");return}p.on(t,"click",()=>{g.warning(`<div style="overflow-wrap: anywhere;">${n.t("拦截跳转：")}<a href="${r}" target="_blank">${r}</a></div>`,{html:!0,zIndex:m.getMaxZIndex()+105});});});},addMarkdownCopyButton(){u.info("在Markdown右上角添加复制按钮"),k(`
        pre{
          position: relative;
