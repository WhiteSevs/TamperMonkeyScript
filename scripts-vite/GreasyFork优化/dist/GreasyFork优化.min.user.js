...
`)}{display: none !important;}`)},setGMResourceCSS(t){let e=typeof ht=="function"?ht(t.keyName):"";typeof e=="string"&&e?P(e):tt.loadStyleLink(t.url);},async loadStyleLink(t){let e=document.createElement("link");e.rel="stylesheet",e.type="text/css",e.href=t,d.ready(()=>{document.head.appendChild(e);});},async loadScript(t){let e=document.createElement("script");return e.src=t,new Promise(A=>{e.onload=()=>{A(null);},(document.head||document.documentElement).appendChild(e);})},fixUrl(t){return t=t.trim(),t.match(/^http(s|):\/\//i)||(t.startsWith("/")||(t+="/"),t=window.location.origin+t),t},fixHttps(t){if(t.startsWith("https://")||!t.startsWith("http://"))return t;let e=new URL(t);return e.protocol="https:",e.toString()},lockScroll(...t){let e=document.createElement("style");e.innerHTML=`
		...
		`;let A=[document.documentElement,document.body].concat(...t||[]);return A.forEach(n=>{n.classList.add("pops-overflow-hidden-important");}),(document.head||document.documentElement).appendChild(e),{recovery(){A.forEach(n=>{n.classList.remove("pops-overflow-hidden-important");}),e.remove();}}}};Vt();R(Y,{});const Ft=r.t("GreasyFork优化"),g=lt.noConflict(),d=T.noConflict(),D=Mt,u=new g.Log(ft,O.console||Xt.console);var Ct;const xt=((Ct=ft==null?void 0:ft.script)==null?void 0:Ct.name)||Ft,kt=false;u.config({debug:kt,logMaxCount:1e3,autoClearConsole:true,tag:true});h.config(Object.defineProperties({html:true,autoClose:true,showClose:false},{position:{get(){return v.getValue("qmsg-config-position","bottom")}},maxNums:{get(){return v.getValue("qmsg-config-maxnums",5)}},showReverse:{get(){return v.getValue("qmsg-config-showreverse",true)}},zIndex:{get(){let t=lt.getMaxZIndex(),e=Mt.config.InstanceUtils.getPopsMaxZIndex().zIndex;return lt.getMaxValue(t,e)+100}}}));const Et=new g.GM_Menu({GM_getValue:R,GM_setValue:V,GM_registerMenuCommand:Ut,GM_unregisterMenuCommand:Kt}),B=new g.Httpx(Rt);B.interceptors.response.use(void 0,t=>(u.error("拦截器-请求错误",t),t.type==="onabort"?h.warning(r.t("请求取消")):t.type==="onerror"?h.error(r.t("请求异常")):t.type==="ontimeout"?h.error(r.t("请求超时")):h.error(r.t("其它错误")),t));B.config({logDetails:kt});O.Object.defineProperty,O.Function.prototype.apply,O.Function.prototype.call,O.Element.prototype.appendChild,O.setTimeout;const P=g.addStyle.bind(g),z=document.querySelector.bind(document),E=document.querySelectorAll.bind(document),nt=function(t,e,A,n,i,a,o,s,l){return {text:t,type:"button",description:e,buttonIcon:n,buttonIsRightIcon:i,buttonIconIsLoading:a,buttonType:o,buttonText:A,callback(p){typeof s=="function"&&s(p);},afterAddToUListCallBack:l}},vt=function(t,e,A,n,i,a="",o,s){let l={text:t,type:"input",isNumber:!!o,isPassword:!!s,props:{},attributes:{},description:n,getValue(){return this.props[W].get(e,A)},callback(c,p){this.props[W].set(e,p);},placeholder:a};return Reflect.set(l.attributes,ct,e),Reflect.set(l.attributes,dt,A),Reflect.set(l.props,W,{get(c,p){return v.getValue(c,p)},set(c,p){v.setValue(c,p);}}),l},y=function(t,e,A,n,i,a){let o={text:t,type:"switch",description:i,attributes:{},props:{},getValue(){return !!this.props[W].get(e,A)},callback(s,l){let c=!!l;u.success(`${c?"开启":"关闭"} ${t}`),this.props[W].set(e,c);},afterAddToUListCallBack:a};return Reflect.set(o.attributes,ct,e),Reflect.set(o.attributes,dt,A),Reflect.set(o.props,W,{get(s,l){return v.getValue(s,l)},set(s,l){v.setValue(s,l);}}),o},L={getInstallUrl(t,e,A){return g.isNotNull(A)?A="/"+A:A="",`https://update.${window.location.hostname}/scripts/${t}/${e}${A}.user.js`},getCodeUrl(t,e){return g.isNull(e)&&(e=""),`/scripts/${t}/code?version=${e}`},getCodeSearchUrl(t){return "/zh-CN/scripts/code-search?c="+encodeURIComponent(t)},getScriptInfoUrl(t){return `/scripts/${t}.json`},getScriptHomeUrl(t){return `/scripts/${t}`},getAdminUrl(t){return t+"/admin"},getScriptId(t){var e,A;return (A=(e=t||window.location.pathname)==null?void 0:e.match(/\/scripts\/([\d]+)/i))==null?void 0:A[1]},getUserId(t){var e;return (e=(t||window.location.pathname).match(/\/users\/([\d]+)/i))==null?void 0:e[1]},getReportUrl(t,e){return `${window.location.origin}/reports/new?item_class=${t}&item_id=${e}`},getScriptName(t){let e=window.location.pathname;t!=null&&(e=new URL(t).pathname),e=decodeURIComponent(e);let A=e.split("/");for(const n of A){let i=n.match(/[\d]+/);if(i&&i.length)return i[1]}},getSwitchLanguageUrl(t="zh-CN"){let e=window.location.origin,A=window.location.pathname.split("/");return A[1]=t,e=e+A.join("/"),e+=window.location.search,window.location.search===""?e+="?locale_override=1":window.location.search.includes("locale_override=1")||(e+="&locale_override=1"),e}},et={getCurrentLoginUserId(){let t=z("#nav-user-info .user-profile-link a");if(!t)return;let e=L.getUserId(t.href);if(e!=null)return e},parseScriptListInfo(t){let e=t.dataset;const A={scriptId:parseInt(e.scriptId),scriptName:e.scriptName,scriptAuthors:[],scriptDailyInstalls:parseInt(e.scriptDailyInstalls),scriptTotalInstalls:parseInt(e.scriptTotalInstalls),scriptRatingScore:parseFloat(e.scriptRatingScore),scriptCreatedDate:new Date(e.scriptCreatedDate),scriptUpdatedDate:new Date(e.scriptUpdatedDate),scriptType:e.scriptType,scriptVersion:e.scriptVersion,sensitive:e.sensitive==="true",scriptLanguage:e.scriptLanguage,cssAvailableAsJs:e.cssAvailableAsJs==="true",codeUrl:e.codeUrl,scriptDescription:e.scriptDescription,scriptAuthorId:parseInt(e.scriptAuthorId),scriptAuthorName:e.scriptAuthorName};let n=g.toJSON(e.scriptAuthors);if(Object.keys(n).forEach(i=>{let a=n[i];A.scriptAuthors.push({authorId:parseInt(i),authorName:a});}),(A.scriptAuthorName==null||isNaN(A.scriptAuthorId))&&A.scriptAuthors.length&&(A.scriptAuthorName=A.scriptAuthors[0].authorName,A.scriptAuthorId=A.scriptAuthors[0].authorId),A.scriptDescription==null){let i=t.querySelector(".script-description")||t.querySelector(".description");i&&(A.scriptDescription=i.innerText||i.textContent);}return A},registerTopNavMenu(t){d.ready(()=>{let e=z("#site-nav nav"),A=z("#site-nav .with-submenu nav");if(!e){u.error("元素#site-nav nav不存在");return}let n=d.createElement("li",{className:t.className,innerHTML:`
                ...
                <a href="javascript:;">${t.name}</a>
                `});d.on(i,"click",s=>{g.preventEvent(s),t.clickEvent(s);});let a=z("#mobile-nav nav"),o=z("#mobile-nav nav .multi-link-nav");o?d.before(o,i):a?d.append(a,i):u.error("元素#mobile-nav nav不存在");});}},j={async getScriptInfo(t){let e=L.getScriptInfoUrl(t),A=await B.get(e,{allowInterceptConfig:false,responseType:"json"});if(!A.status){let i=L.getScriptHomeUrl(t),a=await B.get(i,{fetch:true});if(!a.status)return;let o=d.parseHTML(a.data.responseText,true,true),s=o.querySelector(".install-link"),l=o.querySelector("dd.script-show-created-date relative-time[datetime]"),c=o.querySelector("dd.script-show-daily-installs"),p=o.querySelector("dd.script-show-total-installs"),f=o.querySelector("dd.script-show-updated-date relative-time[datetime]"),m=o.querySelector("#script-description"),w=o.querySelector(".good-rating-count"),x=o.querySelector(".ok-rating-count"),b=o.querySelector(".bad-rating-count"),M=o.querySelector("dd.script-show-license");return {id:Number(t),created_at:l==null?void 0:l.getAttribute("datetime"),daily_installs:Number(d.text(c)||"0"),total_installs:Number(d.text(p)||"0"),code_updated_at:f==null?void 0:f.getAttribute("datetime"),support_url:"",fan_score:"",namespace:s.getAttribute("data-script-namespace"),contribution_url:null,contribution_amount:null,good_ratings:Number(d.text(w)||"0"),ok_ratings:Number(d.text(x)||"0"),bad_ratings:Number(d.text(b)||"0"),users:[],name:s.getAttribute("data-script-name"),description:d.text(m),url:e,code_url:s.getAttribute("href"),license:d.text(M)||null,version:s.getAttribute("data-script-version"),locale:"",deleted:false}}return g.toJSON(A.data.responseText)},async getScriptStats(t){let e=await B.get(`/scripts/${t}/stats.json`,{allowInterceptConfig:false});if(u.info(e),!e.status){u.error(r.t("获取脚本统计数据失败"));return}return g.toJSON(e.data.responseText)},async getSourceCodeSyncFormDataInfo(t){let e=await B.get(`/scripts/${t}/admin`,{fetch:true,allowInterceptConfig:false});if(u.info(e),!e.status){h.error(r.t("请求admin内容失败"));return}let A=e.data.responseText,i=d.parseHTML(A,false,true).querySelector("form.edit_script[action*='sync_update']");if(!i){h.error(r.t("解析admin的源代码同步表单失败"));return}let a=new FormData(i),o=i.querySelector("input[type='submit'][name='update-and-sync']");return o&&a.append(o.name,o.value),{url:i.action,formData:a}},async sourceCodeSync(t,e,A){let n=await B.post(A||`/scripts/${t}/sync_update`,{fetch:true,data:e,allowInterceptConfig:false,headers:{Accept:"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7","Content-Type":"application/x-www-form-urlencoded",Origin:window.location.origin,Referer:window.location.href,"User-Agent":g.getRandomPCUA()}});if(u.info(n),!n.status){h.error(r.t("源代码同步失败"));return}return n},async getUserInfo(t){let e=await B.get(`/users/${t}.json`,{allowInterceptConfig:false});if(u.success(e),!e.status){h.error(r.t("获取用户信息失败"));return}let A=g.toJSON(e.data.responseText);if(A.scriptList=[],A.scriptLibraryList=[],A.scripts.forEach(n=>{n.code_url.endsWith(".user.js")?A.scriptList.push(n):A.scriptLibraryList.push(n);}),!A.scriptLibraryList.length){let n=await B.get(`/users/${t}`,{fetch:true,allowInterceptConfig:false});if(!n.status){h.error(r.t("获取用户主页信息失败"));return}let i=n.data.responseText,a=d.parseHTML(i,true,true),o=a.querySelector("#user-library-script-list");o?o.querySelectorAll("li").forEach(s=>{let l=et.parseScriptListInfo(s),c=s.querySelector("a.script-link").href;A.scriptLibraryList.push({id:l.scriptId,created_at:l.scriptCreatedDate.toISOString(),daily_installs:l.scriptDailyInstalls,total_installs:l.scriptTotalInstalls,code_updated_at:l.scriptUpdatedDate.toISOString(),support_url:null,fan_score:l.scriptRatingScore.toString(),namespace:null,contribution_url:null,contribution_amount:null,good_ratings:0,ok_ratings:0,bad_ratings:0,name:l.scriptName,description:l.scriptDescription,url:c,code_url:l.codeUrl,license:null,version:l.scriptVersion,locale:l.scriptLanguage,deleted:false});}):u.error("解析用户主页的库列表失败",a);}return A},async getUserCollection(t){let e=await B.get(`/users/${t}`,{fetch:true,allowInterceptConfig:false});if(u.info("获取用户的收藏集",e),!e.status){h.error(r.t("获取用户的收藏集失败"));return}let A=e.data.responseText,i=d.parseHTML(A,true,true).querySelector("#user-script-sets");if(!i){u.error("解析Script Sets失败");return}let a=[];return i.querySelectorAll("li").forEach(o=>{var f;let s=o.querySelector("a:last-child");if(!s)return;let l=s.href;if(l.includes("?fav=1"))return;let c=o.querySelector("a").innerText,p=(f=l.match(/\/sets\/([\d]+)\//))==null?void 0:f[1];a.push({id:p,name:c});}),a},async getUserCollectionInfo(t,e){let A=await B.get(`/users/${t}/sets/${e}/edit`,{fetch:true,allowInterceptConfig:false});if(u.info(A),!A.status){h.error(r.t("获取收藏集{{setsId}}失败",{setsId:e}));return}let n=A.data.responseText,i=d.parseHTML(n,true,true),a=i.querySelector('form[id^="edit_script_set"]');if(!a){h.error(r.t("获取表单元素#edit_script_set失败"));return}let o=new FormData(a),s=i.querySelector('meta[name="csrf-token"]');if(!s){h.error(r.t("获取表单csrfToken失败"));return}if(s.hasAttribute("content")){let l=s.getAttribute("content");l&&o.set("authenticity_token",l);}return o},async updateUserSetsInfo(t,e,A){let n=await B.post(`/users/${t}/sets/${e}`,{fetch:true,allowInterceptConfig:false,headers:{Accept:"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7","Accept-Language":"zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6","Cache-Control":"no-cache","Content-Type":"application/x-www-form-urlencoded",Pragma:"no-cache"},fetchInit:{referrerPolicy:"strict-origin-when-cross-origin"},data:A});if(u.info(n),!n.status){h.error(r.t("更新收藏集表单请求失败"));return}let i=n.data.responseText;return d.parseHTML(i,true,true)},async switchLanguage(t){let e=await B.get(t,{fetch:true,headers:{"Upgrade-Insecure-Requests":"1"}});u.info(e),e.status;}},C={isHome(){return window.location.pathname.split("/").length<=2&&window.location.search===""},isCode(){var t;return !!((t=window.location.pathname.split("/"))!=null&&t.includes("code"))},isCodeStrict(){return !!window.location.pathname.match(/\/code(\/|)$/)},isVersion(){return !!window.location.pathname.match(/\/versions(\/|)$/)},isUsers(){return !!window.location.pathname.match(/\/.+\/users\/.+/gi)},isUsersConversations(){return this.isUsers()&&!!window.location.pathname.includes("/conversations")},isUsersConversationsWithSomeUser(){return this.isUsersConversations()&&!!window.location.pathname.match(/\/conversations\/[\d]+/)},isScript(){return !!window.location.pathname.match(/\/scripts\/[\d+]/)},isScriptAdmin(){return !!window.location.pathname.endsWith("/admin")},isScriptList(){return !!window.location.pathname.match(/\/scripts(\/|)$/)},isScriptsBySite(){return !!window.location.pathname.match("/scripts/by-site")},isScriptsFeedback(){return this.isScript()&&window.location.pathname.match(/\/feedback(\/|)$/i)},isScriptLibraryList(){return !!window.location.pathname.match(/\/libraries(\/|)$/)},isScriptSearch(){let t=new URLSearchParams(window.location.search);return this.isScriptList()&&t.has("q")},isScriptCodeSearch(){return !!window.location.pathname.match(/\/code-search(\/|)$/)},isScriptLibraryListSearch(){let t=new URLSearchParams(window.location.search);return this.isScriptLibraryList()&&t.has("q")},isDiscuessions(){return !!window.location.pathname.match(/\/discussions(\/|)$/)},isImageSource(){return window.location.pathname.startsWith("/vite/assets")}},S={menu:Et,isLogin:false,initEnv(){let t=this.getUserLinkElement();this.isLogin=!!t;},getUserLinkElement(){return z("#nav-user-info span.user-profile-link a")},async updateScript(t){let e=function(A,n=1){return `
			...
			</div>`};if(g.isNull(t))h.error(r.t("未获取到【脚本列表】"));else {let A=h.loading(e(L.getScriptName(t[0])),{html:true}),n=0,i=0;for(let a=0;a<t.length;a++){let o=t[a],s=L.getScriptId(o);u.success("更新："+o);let l=L.getScriptName(o);A.setHTML(e(l,a+1));let c=await j.getSourceCodeSyncFormDataInfo(s);if(c){const{formData:p,url:f}=c;await j.sourceCodeSync(s,p,f)?(h.success(r.t("源代码同步成功，3秒后更新下一个")),await g.sleep(3e3),n++):(h.error(r.t("源代码同步失败")),i++);}else h.error(r.t("源代码同步失败")),i++;}A.close(),n===0?h.error(r.t("全部更新失败")):h.success(r.t("全部更新完毕<br >成功：{{successNums}}<br >失败：{{failedNums}}<br >总计：{{scriptUrlListLength}}",{successNums:n,failedNums:i,scriptUrlListLength:t.length}),{html:true});}},handleLocalGotoCallBack(){if(v.getValue("goto_updateSettingsAndSynchronize_scriptList")){if(v.deleteValue("goto_updateSettingsAndSynchronize_scriptList"),!C.isUsers()){v.setValue("goto_updateSettingsAndSynchronize_scriptList",true),S.getUserLinkElement()?(h.success(r.t("前往用户主页")),window.location.href=S.getUserLinkElement().href):h.error(r.t("获取当前已登录的用户主页失败"));return}let t=[];E("#user-script-list-section li a.script-link").forEach(e=>{t=t.concat(L.getAdminUrl(e.href));}),S.updateScript(t);}else if(v.getValue("goto_updateSettingsAndSynchronize_unlistedScriptList")){if(v.deleteValue("goto_updateSettingsAndSynchronize_unlistedScriptList"),!C.isUsers()){v.setValue("goto_updateSettingsAndSynchronize_unlistedScriptList",true),S.getUserLinkElement()?(h.success(r.t("前往用户主页")),window.location.href=S.getUserLinkElement().href):h.error(r.t("获取当前已登录的用户主页失败"));return}let t=[];E("#user-unlisted-script-list li a.script-link").forEach(e=>{t=t.concat(L.getAdminUrl(e.href));}),S.updateScript(t);}else if(v.getValue("goto_updateSettingsAndSynchronize_libraryScriptList")){if(v.deleteValue("goto_updateSettingsAndSynchronize_libraryScriptList"),!C.isUsers()){v.setValue("goto_updateSettingsAndSynchronize_libraryScriptList",true),S.getUserLinkElement()?(h.success(r.t("前往用户主页")),window.location.href=S.getUserLinkElement().href):h.error(r.t("获取当前已登录的用户主页失败"));return}let t=[];E("#user-library-script-list li a.script-link").forEach(e=>{t=t.concat(L.getAdminUrl(e.href));}),S.updateScript(t);}}},rt=function(t,e,A,n,i,a){let o=[];typeof n=="function"?o=n():o=n;let s={text:t,type:"select",description:a,attributes:{},props:{},getValue(){return this.props[W].get(e,A)},callback(l,c,p){let f=c;u.info(`选择：${p}`),this.props[W].set(e,f),typeof i=="function"&&i(l,f,p);},data:o};return Reflect.set(s.attributes,ct,e),Reflect.set(s.attributes,dt,A),Reflect.set(s.props,W,{get(l,c){return v.getValue(l,c)},set(l,c){v.setValue(l,c);}}),s},Yt=function(t,e,A,n,i,a="default",o){let s=typeof i=="function"?i():i;typeof n=="object"&&o.initConfig(A,n);let l=()=>o.getShowText(A,s),c=nt(t,e,l,"keyboard",false,false,a,async p=>{var w;let m=(w=p.target.closest(".pops-panel-button"))==null?void 0:w.querySelector("span");if(o.isWaitPress){h.warning("请先执行当前的录入操作");return}if(o.hasOptionValue(A))o.emptyOption(A),h.success("清空快捷键");else {let x=h.loading("请按下快捷键...",{showClose:true,onClose(){o.cancelEnterShortcutKeys();}}),{status:b,option:M,key:k}=await o.enterShortcutKeys(A);x.close(),b?(u.success(["成功录入快捷键",M]),h.success("成功录入")):h.error(`快捷键 ${o.translateKeyboardValueToButtonText(M)} 已被 ${k} 占用`);}m.innerHTML=l();});return c.attributes={},Reflect.set(c.attributes,St,()=>false),c};class Zt{constructor(e){st(this,"key","short-cut");st(this,"$data");st(this,"isWaitPress",false);st(this,"currentWaitEnterPressInstanceHandler",null);typeof e=="string"&&(this.key=e),this.$data={otherShortCutOptions:[]};}initConfig(e,A){this.hasOption(e)||this.setOption(e,A);}getStorageKey(){return this.key}getLocalAllOptions(){return R(this.key,[])}hasOption(e){return !!this.getLocalAllOptions().find(i=>i.key===e)}hasOptionValue(e){if(this.hasOption(e)){let A=this.getOption(e);return (A==null?void 0:A.value)!=null}else return  false}getOption(e,A){return this.getLocalAllOptions().find(a=>a.key===e)??A}setOption(e,A){let n=this.getLocalAllOptions(),i=n.findIndex(a=>a.key===e);i==-1?n.push({key:e,value:A}):Reflect.set(n[i],"value",A),V(this.key,n);}emptyOption(e){let A=false,n=this.getLocalAllOptions(),i=n.findIndex(a=>a.key===e);return i!==-1&&(n[i].value=null,A=true),V(this.key,n),A}deleteOption(e){let A=false,n=this.getLocalAllOptions(),i=n.findIndex(a=>a.key===e);return i!==-1&&(n.splice(i,1),A=true),V(this.key,n),A}translateKeyboardValueToButtonText(e){let A="";return e.ohterCodeList.forEach(n=>{A+=g.stringTitleToUpperCase(n,true)+" + ";}),A+=g.stringTitleToUpperCase(e.keyName),A}getShowText(e,A){if(this.hasOption(e)){let n=this.getOption(e);return n.value==null?A:this.translateKeyboardValueToButtonText(n.value)}else return A}async enterShortcutKeys(e){const A=this;return new Promise(n=>{this.isWaitPress=true;let i=d.listenKeyboard(window,"keyup",(a,o,s)=>{const l={keyName:a,keyValue:o,ohterCodeList:s};let c={};try{const p=JSON.stringify(l),f=this.getLocalAllOptions();Array.isArray(this.$data.otherShortCutOptions)&&f.push(...this.$data.otherShortCutOptions);for(let m=0;m<f.length;m++){let w=f[m];if(w.key===e)continue;const x=JSON.stringify(w.value);let b=!1;if(w.value!=null&&p===x&&(b=!0),b){c={status:!1,key:w.key,option:l};return}}this.setOption(e,l),c={status:!0,key:e,option:l};}catch(p){console.log(p),c={status:false,key:e,option:l};}finally{A.isWaitPress=false,i.removeListen(),A.currentWaitEnterPressInstanceHandler=null,n(c);}});A.currentWaitEnterPressInstanceHandler=null,A.currentWaitEnterPressInstanceHandler=()=>{A.isWaitPress=false,i.removeListen();};})}cancelEnterShortcutKeys(){typeof this.currentWaitEnterPressInstanceHandler=="function"&&this.currentWaitEnterPressInstanceHandler();}initGlobalKeyboardListener(e,A){let n=this.getLocalAllOptions();if(!n.length){u.warn("没有设置快捷键");return}const i=this;function a(l,c){d.listenKeyboard(l,"keydown",(p,f,m,w)=>{if(i.isWaitPress)return;A!=null&&A.isPrevent&&g.preventEvent(w),n=i.getLocalAllOptions();let x=n.findIndex(b=>{let M=b.value,k={keyName:p,keyValue:f,ohterCodeList:m};if(JSON.stringify(M)===JSON.stringify(k))return b});if(x!=-1){let b=n[x];b.key in c&&(u.info(["调用快捷键",b]),c[b.key].callback());}},{capture:!!(A!=null&&A.capture)});}let o={},s={};Object.keys(e).forEach(l=>{let c=e[l];(c.target==null||typeof c.target=="string"&&c.target==="")&&(c.target="window"),c.target==="window"?Reflect.set(o,l,c):Reflect.set(s,l,c);}),a(window,o),d.ready(()=>{Object.keys(s).forEach(async l=>{let c=s[l];if(typeof c.target=="string")g.waitNode(c.target,1e4).then(p=>{if(!p)return;let f={};Reflect.set(f,l,c),a(p,f);});else if(typeof c.target=="function"){let p=await c.target();if(p==null)return;let f={};Reflect.set(f,l,c),a(p,f);}else {let p={};Reflect.set(p,l,c),a(c.target,p);}});});}}const Dt={shortCut:new Zt,shortOption:{"gf-quickReply":{target:()=>{let t=z("form textarea"),e=z('input[name="commit"][type="submit"]');if(t){if(!e){u.error("页面不存在【发表回复】按钮");return}}else {u.error("页面不存在输入框");return}return u.success("监听快捷键：gf-quickReply"),t},callback(){if(document.activeElement){let t=document.activeElement.closest("form");if(!t){u.error("当前activeElement不在表单内，无法触发快捷键");return}let e=t.querySelectorAll('input[name="commit"][type="submit"]');if(!e.length){u.error("表单内不存在【发表回复】按钮");return}e.length>1&&u.warn("表单内存在多个【发表回复】按钮，只触发第一个"),e[0].click();}else u.error("当前页面没有激活元素，无法触发快捷键");}}},init(){this.shortCut.initGlobalKeyboardListener(this.shortOption);}},mt={$key:{DB_KEY:"data"},$data:{db:null},init(){this.$data.db=this.getDB(),v.execMenuOnce("rememberReplyContent",()=>{this.rememberReplyContent();}),v.execMenu("gf-autoClearRememberReplayContent",t=>{this.autoClearRememberReplayContent(t);});},getDB(){const t="reply_record",e="textarea_text";return new g.indexedDB(t,e,2)},async rememberReplyContent(){const t="记住回复内容 -- ";let e=E("form");if(!e.length){u.warn(t+"不存在表单");return}try{await this.clearRelayHistoryRememberContentText();}catch(A){u.error(A);}e.forEach(async A=>{let n=A.querySelector("textarea"),i=A.querySelector('input[type="submit"]');n&&i&&(u.success("开始监听form --- 记住回复内容",A),this.$data.db.get(this.$key.DB_KEY).then(a=>{if(!a.data)return;let o=a.data.findIndex(l=>this.checkUrlIsSame(window.location.href,l.url));if(o==-1)return;let s=a.data[o].text;u.success("填入历史输入内容："+s),n.value=s;}),d.on(n,["propertychange","input"],g.debounce(a=>{let o={url:window.location.href,text:n.value,time:Date.now()};this.$data.db.get(this.$key.DB_KEY).then(s=>{if(!s.success&&s.event&&s.event.type!=="success"){u.warn(s);return}s.data==null&&(s.data=[]);let l=s.data.findIndex(c=>this.checkUrlIsSame(window.location.href,c.url));l!==-1?g.isNull(o.text)?s.data.splice(l,1):s.data[l]=g.assign(s.data[l],o):s.data=s.data.concat(o),this.$data.db.save(this.$key.DB_KEY,s.data).then(c=>{c.success||u.error("保存失败",c);});});},25)),d.on(A,"submit",a=>{u.info("表单提交，刷新页面后清理内容："+window.location.href),V("delyClear_rememberReplyContent_url",window.location.href);},{capture:true}));});},async clearRelayHistoryRememberContentText(){const t="delyClear_rememberReplyContent_url";let e=R(t);if(e){let A=await this.$data.db.get(this.$key.DB_KEY);if(!A.data){u.info("表单记录：数据库是空的");return}let n=A.data.findIndex(a=>this.checkUrlIsSame(e,a.url));if(n==-1){u.info("表单记录：已不存在该数据"),bt(t);return}A.data.splice(n,1),(await this.$data.db.save(this.$key.DB_KEY,A.data)).success?(u.success("表单记录：成功清除"),bt(t)):u.error("表单记录：清除失败",A);}},checkUrlIsSame(t,e){let A=new URL(t),n=new URL(e);return A.origin===n.origin&&A.pathname===n.pathname},autoClearRememberReplayContent(t){const e="gf-last-time-autoClearRememberReplayContent";let A=R(e),n=t*24*60*60*1e3;if(A)if(Date.now()-A>n)V(e,Date.now());else return;V(e,Date.now());},async getAllRememberReplyContent(){return (await this.$data.db.get(this.$key.DB_KEY)).data??[]},async clearAllRememberReplyContent(){return (await this.$data.db.delete(this.$key.DB_KEY)).success}},Pt={async UIScriptList(t,e){var l,c,p;if(!S.isLogin){h.error(r.t("请先登录账号！"));return}let n=S.getUserLinkElement().href,i=(p=(c=(l=n==null?void 0:n.split("/"))==null?void 0:l.pop())==null?void 0:c.match(/([0-9]+)/))==null?void 0:p[0],a=D.loading({mask:{enable:true},parent:e,content:{text:r.t("获取信息中，请稍后...")},addIndexCSS:false}),o=await j.getUserInfo(i);if(a.close(),!o)return;u.info(o);let s=t==="script-list"?o.scriptList:o.scriptLibraryList;h.success(r.t("获取成功，共 {{count}} 个",{count:s.length}));for(const f of s){let m=d.createElement("li",{className:"w-script-list-item",innerHTML:`
				<div class="w-script-info">
				<div class="w-script-name">
					<a href="${f.url}" target="_blank">${f.name}</a>
				</div>
				<div class="w-script-fan-score">
					<p>${r.t("评分：")}${f.fan_score}</p>
				</div>
				<div class="w-script-locale">
					<p>${r.t("语言：")}${f.locale}</p>
				</div>
				<div class="w-script-version">
					<p>${r.t("版本：")}${f.version}</p>
				</div>
				<div class="w-script-update-time">
					<p>${r.t("更新：")}${g.getDaysDifference(new Date(f.code_updated_at).getTime(),void 0,"auto")}前</p>
				</div>
				</div>
            `}),w=m.querySelector(".w-script-info"),x=d.createElement("div",{className:"pops-panel-button",innerHTML:`
				<button type="primary" data-icon="" data-righticon="false">
				<span>${r.t("同步代码")}</span>
				</button>
				`});f.deleted&&(m.classList.add("w-script-deleted"),x.querySelector("button").setAttribute("disabled","true")),d.on(x,"click",async function(){u.success("同步",f);let b=x.querySelector("button"),M=x.querySelector("button span"),k=d.createElement("i",{className:"pops-bottom-icon",innerHTML:D.config.iconSVG.loading},{"is-loading":true});b.setAttribute("disabled","true"),b.setAttribute("data-icon","true"),M.innerText=r.t("同步中..."),d.before(M,k);let H=f==null?void 0:f.id,G=await j.getSourceCodeSyncFormDataInfo(H.toString());if(G){const{formData:I,url:K}=G,N="script[script_sync_type_id]",q="script[sync_type]";if(I.has(N)||I.has(q)){let Q="";if(I.has(N)){let X=I.get(N);X.toString()==="1"?Q=r.t("手动"):X.toString()==="2"?Q=r.t("自动"):X.toString()==="3"&&(Q="webhook");}else I.has(q)&&(Q=I.get(q));let U=m.querySelector(".w-script-sync-type");U?U.querySelector("p").innerText=r.t("同步方式：{{syncMode}}",{syncMode:Q}):d.append(w,`
								<div class="w-script-sync-type">
									<p>${r.t("同步方式：{{syncMode}}",{syncMode:Q})}
									</p>
								</div>`),await j.sourceCodeSync(f.id.toString(),I,K)?h.success(r.t("同步成功")):h.error(r.t("同步失败"));}else h.error(r.t("该脚本未设置同步信息"));}b.removeAttribute("disabled"),b.removeAttribute("data-icon"),M.innerText=r.t("同步代码"),k.remove();}),m.appendChild(x),e.appendChild(m);}}};let Lt=false;const wt={isCurrentLoginUserHome(){let t=et.getCurrentLoginUserId();return !!(t!=null&&C.isUsers()&&window.location.pathname.includes("/"+t))},monacoEditor(){return new Promise(t=>{const e="0.52.2",A="monaco-editor-ready";if(Lt||(Lt=true,P(`
					@font-face {
						font-family: 'codicon';
						src: url('https://fastly.jsdelivr.net/npm/monaco-editor@${e}/min/vs/base/browser/ui/codicons/codicon/codicon.ttf') format('truetype');
					}
				`)),O.monaco){t(O.monaco);return}let n=h.loading(r.t("monaco-editor加载中...")),i=d.createElement("script",{type:"module",defer:true,innerHTML:`
					import * as monaco from "https://fastly.jsdelivr.net/npm/monaco-editor@${e}/+esm";
					window.monaco = monaco;
					window.dispatchEvent(new CustomEvent("${A}"));
				`});d.append(document.head||document.documentElement,i),d.on(O,A,()=>{let a=O.monaco;u.success("网络加载monaco编辑器成功"),n.close(),t(a);},{once:true});})}},J={key:"gf-shield-rule",init(){u.info("脚本过滤");let t=new g.LockFunction(()=>{this.filter();},50);d.ready(()=>{if(wt.isCurrentLoginUserHome()){u.warn("当前在已登录的账户主页下，禁用脚本过滤");return}g.mutationObserver(document.body,{config:{subtree:true,childList:true},immediate:true,callback:()=>{t.run();}}),t.run();});},getElementList(){return [...Array.from(E("ol.script-list li[data-script-name]"))]},filter(){this.getElementList().forEach(t=>{let e=et.parseScriptListInfo(t),A=this.getValue().split(`
`);for(let n=0;n<A.length;n++){let i=A[n],a=i.split("##"),o=a[0],s=a[1];if(o==="scriptRatingScore"){let l=parseFloat(s.slice(1));if(s.startsWith(">")){if(e.scriptRatingScore>l){u.info("触发脚本过滤规则",[i,e]),t.remove();break}}else if(s.startsWith("<")&&e.scriptRatingScore<l){u.info("触发脚本过滤规则",[i,e]),t.remove();break}}else if(o in e||o==="scriptDescription"){if(typeof s!="string")continue;let l=new RegExp(s,"ig");if(String(e[o]).match(l)){u.info("触发脚本过滤规则",i,e),t.remove();break}}}});},setValue(t){v.setValue(this.key,t);},addValue(t,e){let A=this.getValue();if(A.trim()!==""&&(A+=`
`),g.isNull(t))return;t=t.toString().trim();let n=t+"##"+e;A+=n,this.setValue(A);},getValue(){return v.getValue(this.key,"")}},Wt={id:"greasy-fork-panel-config-account",title:r.t("通用"),forms:[{text:"",type:"forms",forms:[{text:r.t("Toast配置"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[rt(r.t("Toast位置"),"qmsg-config-position","bottom",[{value:"topleft",text:r.t("左上角")},{value:"top",text:r.t("顶部")},{value:"topright",text:r.t("右上角")},{value:"left",text:r.t("左边")},{value:"center",text:r.t("中间")},{value:"right",text:r.t("右边")},{value:"bottomleft",text:r.t("左下角")},{value:"bottom",text:r.t("底部")},{value:"bottomright",text:r.t("右下角")}],(t,e,A)=>{u.info("设置当前Qmsg弹出位置"+A);},r.t("Toast显示在页面九宫格的位置")),rt(r.t("最多显示的数量"),"qmsg-config-maxnums",3,[{value:1,text:"1"},{value:2,text:"2"},{value:3,text:"3"},{value:4,text:"4"},{value:5,text:"5"}],void 0,r.t("限制Toast显示的数量")),y(r.t("逆序弹出"),"qmsg-config-showreverse",false,void 0,r.t("修改Toast弹出的顺序"))]}]},rt(r.t("语言"),"setting-language","zh-CN",[{value:"zh-CN",text:"中文"},{value:"en-US",text:"English"}],(t,e,A)=>{u.info("改变语言："+A),r.changeLanguage(e);})]},{text:"",type:"forms",forms:[{text:r.t("账号/密码"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[vt(r.t("账号"),"user","",void 0,void 0,r.t("请输入账号")),vt(r.t("密码"),"pwd","",void 0,void 0,r.t("请输入密码"),false,true),vt(r.t("secret"),"secret","","两步验证（2FA）",void 0,r.t("请输入secret"),false,true)]},{text:"",type:"forms",forms:[y(r.t("自动登录"),"autoLogin",true,void 0,r.t("自动登录当前保存的账号")),nt(r.t("清空账号/密码"),void 0,r.t("点击清空"),void 0,void 0,false,"default",t=>{if(confirm(r.t("确定清空账号和密码？"))){v.deleteValue("user"),v.deleteValue("pwd"),h.success(r.t("已清空账号/密码"));let e=t.target.getRootNode();e.querySelector('li[data-key="user"] .pops-panel-input input').value="",e.querySelector('li[data-key="pwd"] .pops-panel-input input').value="";}})]}]},{text:r.t("功能"),type:"deepMenu",forms:[{text:r.t("功能"),type:"forms",forms:[rt(r.t("固定当前语言"),"language-selector-locale","",function(){let t=[{value:"",text:r.t("无")}];return d.ready(()=>{E("select.language-selector-locale option").forEach(e=>{let A=e.getAttribute("value");if(A==="help")return;let n=d.text(e).trim();t.push({value:A,text:n});});}),t}()),y(r.t("修复图片宽度显示问题"),"fixImageWidth",true,void 0,r.t("修复图片在移动端宽度超出浏览器宽度问题")),y(r.t("优化图片浏览"),"optimizeImageBrowsing",true,void 0,r.t("使用Viewer浏览图片")),y(r.t("覆盖图床图片跳转"),"overlayBedImageClickEvent",true,void 0,r.t("配合上面的【优化图片浏览】更优雅浏览图片")),y(r.t("添加【操作面板】按钮"),"scripts-addOperationPanelBtnWithNavigator",true,void 0,r.t("在脚本列表页面时为顶部导航栏添加【操作面板】按钮")),y(r.t("给Markdown添加【复制】按钮"),"addMarkdownCopyButton",true,void 0,r.t("在Markdown内容右上角添加【复制】按钮，点击一键复制Markdown内容"))]},{text:r.t("检测页面加载"),type:"forms",forms:[y(r.t("启用"),"checkPage",true,void 0,r.t("检测Greasyfork页面是否正常加载，如加载失败则自动刷新页面")),rt(r.t("检测间隔"),"greasyfork-check-page-timeout",5,(()=>{let t=[];for(let e=0;e<5;e++)t.push({value:e+1,text:e+1+"s"});return t})(),void 0,r.t("设置检测上次刷新页面的间隔时间，当距离上次刷新页面的时间超过设置的值，将不再刷新页面"))]}]},{type:"deepMenu",text:r.t("表单"),forms:[{type:"forms",text:"",forms:[y(r.t("记住回复内容"),"rememberReplyContent",true,void 0,r.t("监听表单内的textarea内容改变并存储到indexDB中，提交表单将清除保存的数据，误刷新页面时可动态恢复")),rt(r.t("自动清理空间"),"gf-autoClearRememberReplayContent",7,[{text:r.t("不清理"),value:-1},{text:r.t("{{value}} 天",{value:1}),value:1},{text:r.t("{{value}} 周",{value:1}),value:7},{text:r.t("{{value}} 个月",{value:1}),value:30},{text:r.t("{{value}} 个月",{value:2}),value:60},{text:r.t("{{value}} 个月",{value:3}),value:90},{text:r.t("半年"),value:180}],void 0,r.t("根据设置的间隔时间自动清理保存的回复内容")),nt(r.t("数据占用空间：{{size}}",{size:r.t("计算中")}),r.t("当前存储的数据所占用的空间大小"),r.t("清空"),void 0,void 0,void 0,"default",async()=>{await mt.clearAllRememberReplyContent()?h.success(r.t("清理成功")):h.error(r.t("清理失败"));},async(t,e)=>{let A=e.ulElement.querySelector('li[data-key="gf-autoClearRememberReplayContent"]+li .pops-panel-item-left-main-text'),n=await mt.getAllRememberReplyContent(),i="";n.length?i=g.getTextStorageSize(JSON.stringify(n)):i=g.getTextStorageSize(""),A.innerText=r.t("数据占用空间：{{size}}",{size:i});})]}]},{text:r.t("美化"),type:"deepMenu",forms:[{text:r.t("全局"),type:"forms",forms:[y(r.t("美化页面元素"),"beautifyPage",true,void 0,r.t("如button、input、textarea")),y(r.t("美化上传图片按钮"),"beautifyUploadImage",true,void 0,r.t("放大上传区域")),y(r.t("美化顶部导航栏"),"beautifyTopNavigationBar",true,void 0,r.t("可能会跟Greasyfork Beautify脚本有冲突")),y(r.t("美化Greasyfork Beautify脚本"),"beautifyGreasyforkBeautify",false,void 0,r.t('需安装Greasyfork Beautify脚本，<a href="https://greasyfork.org/zh-CN/scripts/446849-greasyfork-beautify" target="_blank">🖐点我安装</a>'))]},{type:"forms",text:r.t("脚本列表"),forms:[y(r.t("美化脚本列表"),"beautifyCenterContent",true,void 0,r.t("双列显示且添加脚本卡片操作项（安装、收藏）")),y("↑"+r.t("使用namespace查询脚本信息"),"beautifyCenterContent-queryNameSpace",true,void 0,r.t("开启后检测已安装的脚本信息更准确，但是速度会更慢"))]}]},{type:"deepMenu",text:r.t("自定义快捷键"),forms:[{type:"forms",text:"",forms:[Yt(r.t("快捷键发表回复"),r.t("在输入框内按下快捷发表回复，例如：{{key}}",{key:"Ctrl + Enter"}),"gf-quickReply",{keyName:"Enter",keyValue:"13",ohterCodeList:["ctrl"]},r.t("点击录入快捷键"),void 0,Dt.shortCut)]}]},{text:r.t("过滤"),type:"deepMenu",forms:[{text:`<a target="_blank" href="https://greasyfork.org/zh-CN/scripts/475722-greasyfork%E4%BC%98%E5%8C%96#:~:text=%E8%84%9A%E6%9C%AC%E8%BF%87%E6%BB%A4%E8%A7%84%E5%88%99">${r.t("帮助文档")}</a>`,type:"forms",forms:[y(r.t("启用"),"gf-scripts-filter-enable",true,void 0,r.t("作用域：脚本、脚本搜索、用户主页")),{type:"own",getLiElementCallBack(t){let e=d.createElement("div",{className:"pops-panel-textarea",innerHTML:`
												<textarea placeholder="${r.t("请输入规则，每行一个")}" style="height:200px;"></textarea>`},{style:"width: 100%;"}),A=e.querySelector("textarea");return A.value=J.getValue(),d.on(A,["input","propertychange"],void 0,g.debounce(function(n){J.setValue(A.value);},200)),t.appendChild(e),t}}]}]}]},{type:"forms",text:r.t("脚本管理"),forms:[{type:"deepMenu",text:r.t("代码同步"),forms:[{text:r.t("代码同步"),type:"forms",forms:[nt(r.t("源代码同步【脚本列表】"),void 0,r.t("一键同步"),void 0,void 0,false,"primary",t=>{if(!C.isUsers()){v.setValue("goto_updateSettingsAndSynchronize_scriptList",true),S.getUserLinkElement()?(h.success(r.t("前往用户主页")),window.location.href=S.getUserLinkElement().href):h.error(r.t("获取当前已登录的用户主页失败"));return}let e=[];E("#user-script-list-section li a.script-link").forEach(A=>{e=e.concat(L.getAdminUrl(A.href));}),S.updateScript(e);}),nt(r.t("源代码同步【未上架的脚本】"),void 0,r.t("一键同步"),void 0,void 0,false,"primary",t=>{if(!C.isUsers()){v.setValue("goto_updateSettingsAndSynchronize_unlistedScriptList",true),S.getUserLinkElement()?(h.success(r.t("前往用户主页")),window.location.href=S.getUserLinkElement().href):h.error(r.t("获取当前已登录的用户主页失败"));return}let e=[];E("#user-unlisted-script-list li a.script-link").forEach(A=>{e=e.concat(L.getAdminUrl(A.href));}),S.updateScript(e);}),nt(r.t("源代码同步【库】"),void 0,r.t("一键同步"),void 0,void 0,false,"primary",t=>{if(!C.isUsers()){v.setValue("goto_updateSettingsAndSynchronize_libraryScriptList",true),S.getUserLinkElement()?(h.success(r.t("前往用户主页")),window.location.href=S.getUserLinkElement().href):h.error(r.t("获取当前已登录的用户主页失败"));return}let e=[];E("#user-library-script-list li a.script-link").forEach(A=>{e=e.concat(L.getAdminUrl(A.href));}),S.updateScript(e);})]}]},{type:"deepMenu",text:r.t("脚本列表"),forms:[],afterEnterDeepMenuCallBack(t,e){Pt.UIScriptList("script-list",e.sectionBodyContainer);}},{type:"deepMenu",text:r.t("库"),forms:[],afterEnterDeepMenuCallBack(t,e){Pt.UIScriptList("script-library",e.sectionBodyContainer);}}]}]},Jt={id:"greasy-fork-panel-config-scripts",title:r.t("脚本"),forms:[{text:"",type:"forms",forms:[{text:r.t("代码"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[y(r.t("添加复制代码按钮"),"addCopyCodeButton",true,void 0,r.t("更优雅的复制")),y(r.t("快捷键"),"fullScreenOptimization",true,void 0,r.t("【F】键全屏、【Alt+Shift+F】键宽屏")),y(r.t("修复代码行号显示"),"code-repairCodeLineNumber",false,void 0,r.t("修复代码行数超过1k行号显示不全问题")),y("monacoEditor","code-use-monaco-editor",true,void 0,r.t("使用Monaco编辑器"))]}]},{text:r.t("历史版本"),type:"deepMenu",forms:[{text:r.t("功能"),type:"forms",forms:[y(r.t("添加额外的标签按钮"),"scripts-versions-addExtraTagButton",true,void 0,r.t("在版本下面添加【安装】、【查看代码】按钮")),y(r.t("添加代码对比按钮"),"scripts-versions-addCompareCodeButton",true,void 0,r.t("monacoEditor"))]},{text:r.t("美化"),type:"forms",forms:[y(r.t("美化历史版本页面"),"beautifyHistoryVersionPage",true,void 0,r.t("更直观的查看版本迭代"))]}]}]},{text:"",type:"forms",forms:[{text:r.t("功能"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[y(r.t("添加【寻找引用】按钮"),"addFindReferenceButton",true,void 0,r.t("在脚本栏添加按钮，一般用于搜索引用该库的相关脚本")),y(r.t("添加【收藏】按钮"),"addCollectionButton",true,void 0,r.t("在脚本栏添加按钮，一般用于快捷收藏该脚本/库")),y(r.t("添加【今日检查】信息块"),"scriptHomepageAddedTodaySUpdate",true,void 0,r.t("在脚本信息栏添加【今日检查】信息块"))]}]}]}]},Z={key:"gf-discuessions-filter-rule",$data:{FILTER_SCRIPT_KEY:"greasyfork-discussions-filter-script",FILTER_POST_USER_KEY:"greasyfork-discussions-filter-post-user",FILTER_REPLY_USER_KEY:"greasyfork-discussions-filter-reply-user"},init(){u.info("论坛-过滤"),P(`
        ...
        `);let t=new g.LockFunction(()=>{this.filter();},50);g.mutationObserver(document.body,{config:{subtree:true,childList:true},immediate:true,callback:()=>{t.run();}}),t.run();},getElementList(){let t=[];return t=t.concat(Array.from(E(".discussion-list-container"))),t},filter(){this.transformOldRule();const t=new Map;this.getElementList().forEach((e,A)=>{const n=this.parseDiscuessionListContainerInfo(e);let i=this.getValue().split(`
`);if(t.has(n.snippet)&&v.getValue("greasyfork-discussions-filter-duplicate-comments")){let a=t.get(n.snippet).querySelector("a.discussion-title");a.setAttribute("data-repeat-tip-show","true");let o=0;a.hasAttribute("data-repeat-count")&&(o=parseInt(a.getAttribute("data-repeat-count"))),o++,a.setAttribute("data-repeat-count",o.toString()),a.setAttribute("data-repeat-tip-show",r.t("已过滤：{{oldCount}}",{oldCount:o})),u.success([`过滤重复内容：${n.snippet}`,n]),e.remove();return}t.set(n.snippet,e);for(let a=0;a<i.length;a++){let o=i[a],s=o.split("##"),l=s[0],c=s[1];if(l in n){let p=new RegExp(c,"ig");if(n[l]!=null&&String(n[l]).match(p)){u.info("触发论坛过滤规则",o,n),e.remove();return}}}});},parseDiscuessionListContainerInfo(t){var a,o,s,l;let e=t.querySelector("a.discussion-title").href,A=e.match(/\/discussions(|\/greasyfork)\/([\d]+)/),n=A[A.length-1];const i={scriptName:t.querySelector(".discussion-meta-item-script-name").innerText,scriptUrl:(a=t.querySelector(".discussion-meta-item-script-name a"))==null?void 0:a.href,scriptId:L.getScriptId((o=t.querySelector(".discussion-meta-item-script-name a"))==null?void 0:o.href),postUserName:t.querySelector("a.user-link").innerText,postUserHomeUrl:t.querySelector("a.user-link").href,postUserId:L.getUserId(t.querySelector("a.user-link").href),postTimeStamp:new Date(t.querySelector("relative-time").getAttribute("datetime")),snippetId:n,snippetUrl:e,snippet:((s=t.querySelector("span.discussion-snippet"))==null?void 0:s.innerText)||"",replyUserName:void 0,replyUserHomeUrl:void 0,replyUserId:void 0,replyTimeStamp:void 0};return t.querySelector(".discussion-meta-item .discussion-meta-item")&&(i.replyUserName=t.querySelector(".discussion-meta-item .discussion-meta-item a.user-link").innerText,i.replyUserHomeUrl=t.querySelector(".discussion-meta-item .discussion-meta-item a.user-link").href,i.replyUserId=L.getUserId(i.replyUserHomeUrl),i.replyTimeStamp=new Date((l=t.querySelector(".discussion-meta-item .discussion-meta-item relative-time"))==null?void 0:l.getAttribute("datetime"))),i},transformOldRule(){if(Date.now()>new Date("2024-8-19").getTime())return;const t="greasyfork-discussions-filter-script",e="greasyfork-discussions-filter-post-user",A="greasyfork-discussions-filter-reply-user",n=v.getValue(t,""),i=v.getValue(e,""),a=v.getValue(A,""),o=n.trim()===""?[]:n.split(`
`),s=i.trim()===""?[]:i.split(`
`),l=a.trim()===""?[]:a.split(`
`);o.forEach(c=>{this.addValue("scriptId",g.parseStringToRegExpString("^"+c+"$"));}),s.forEach(c=>{this.addValue("postUserId",g.parseStringToRegExpString("^"+c+"$"));}),l.forEach(c=>{this.addValue("replyUserId",g.parseStringToRegExpString("^"+c+"$"));}),v.deleteValue(t),v.deleteValue(e),v.deleteValue(A);},setValue(t){v.setValue(this.key,t);},addValue(t,e){let A=this.getValue();if(A.trim()!==""&&(A+=`
`),g.isNull(t))return;t=t.toString().trim();let n=t+"##"+e;A+=n,this.setValue(A);},getValue(){return v.getValue(this.key,"")}},$t={id:"greasy-fork-panel-config-discussions",title:r.t("论坛"),forms:[{text:"",type:"forms",forms:[{text:r.t("功能"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[{type:"own",attributes:{"data-key":"discussions-readBgColor","data-default-value":"#e5e5e5"},getLiElementCallBack(t){let e="discussions-readBgColor",A=d.createElement("div",{className:"pops-panel-item-left-text",innerHTML:`
...
`);});}},$={setting:{get width(){return window.innerWidth<550?"88vw":"550px"},get height(){return window.innerHeight<450?"70vh":"450px"}},settingBig:{get width(){return window.innerWidth<800?"92vw":"800px"},get height(){return window.innerHeight<600?"80vh":"600px"}},info:{get width(){return "350px"},get height(){return "250px"}}},Ae={id:"greasy-fork-panel-config-script-search",title:r.t("搜索"),forms:[{type:"forms",text:"搜素结果过滤",forms:[y(r.t("新增【关键词】搜索框"),"gf-script-search-addFilterSearchInput",true,void 0,r.t("输入自定义关键词后自动执行过滤")),y(r.t("新增【{{buttonText}}】按钮",{buttonText:r.t("名称")}),"gf-script-search-filterScriptTitleWholeWordMatching",true,void 0,r.t("该Checkbox按钮开启后，自动过滤出包含搜索关键词的脚本")),y(r.t("新增【{{buttonText}}】按钮",{buttonText:r.t("描述")}),"gf-script-search-filterScriptDescWholeWordMatching",true,void 0,r.t("该Checkbox按钮开启后，自动过滤出包含搜索关键词的脚本")),y(r.t("新增【{{buttonText}}】按钮",{buttonText:r.t("名称/描述")}),"gf-script-search-filterScriptTitleOrDescWholeWordMatching",true,void 0,r.t("该Checkbox按钮开启后，自动过滤出包含搜索关键词的脚本")),y(r.t("新增【{{buttonText}}】按钮",{buttonText:r.t("作者名称")}),"gf-script-search-filterScriptAuthorNameWholeWordMatching",true,void 0,r.t("该Checkbox按钮开启后，自动过滤出包含搜索关键词的脚本"))]}]},v={$data:{__data:null,__oneSuccessExecMenu:null,__onceExec:null,__listenData:null,get data(){return v.$data.__data==null&&(v.$data.__data=new g.Dictionary),v.$data.__data},get oneSuccessExecMenu(){return v.$data.__oneSuccessExecMenu==null&&(v.$data.__oneSuccessExecMenu=new g.Dictionary),v.$data.__oneSuccessExecMenu},get onceExec(){return v.$data.__onceExec==null&&(v.$data.__onceExec=new g.Dictionary),v.$data.__onceExec},get scriptName(){return xt},key:Y,attributeKeyName:ct,attributeDefaultValueName:dt},$listener:{get listenData(){return v.$data.__listenData==null&&(v.$data.__listenData=new g.Dictionary),v.$data.__listenData}},init(){this.initPanelDefaultValue(),this.initExtensionsMenu();},isTopWindow(){return O.top===O.self},initExtensionsMenu(){this.isTopWindow()&&Et.add([{key:"show_pops_panel_setting",text:r.t("⚙ 设置"),autoReload:false,isStoreValue:false,showText(t){return t},callback:()=>{this.showPanel();}},{key:"githubUrl2webhookUrl",text:"⚙ "+r.t("Url To WebhookUrl"),autoReload:false,isStoreValue:false,showText(t){return t},callback:()=>{ee.showView();}}]);},initPanelDefaultValue(){let t=this;function e(i){if(!i.attributes)return;let a={},o=i.attributes[ct];o!=null&&(a[o]=i.attributes[dt]);let s=i.attributes[St];if(typeof s=="function"){let p=s();if(typeof p=="boolean"&&!p)return}let l=i.attributes[qt];l&&typeof l=="object"&&Object.assign(a,l);let c=Object.keys(a);if(!c.length){u.warn("请先配置键",i);return}c.forEach(p=>{let f=a[p];t.$data.data.has(p)&&u.warn("请检查该key(已存在): "+p),t.$data.data.set(p,f);});}function A(i){for(let a=0;a<i.length;a++){let o=i[a];e(o);let s=o.forms;s&&Array.isArray(s)&&A(s);}}let n=this.getPanelContentConfig();for(let i=0;i<n.length;i++){let a=n[i];if(!a.forms)continue;let o=a.forms;o&&Array.isArray(o)&&A(o);}},setValue(t,e){let A=R(Y,{}),n=A[t];A[t]=e,V(Y,A),this.$listener.listenData.has(t)&&this.$listener.listenData.get(t).callback(t,n,e);},getValue(t,e){let n=R(Y,{})[t];return n??(this.$data.data.has(t)?this.$data.data.get(t):e)},deleteValue(t){let e=R(Y,{}),A=e[t];Reflect.deleteProperty(e,t),V(Y,e),this.$listener.listenData.has(t)&&this.$listener.listenData.get(t).callback(t,A,void 0);},addValueChangeListener(t,e,A){let n=Math.random();return this.$listener.listenData.set(t,{id:n,key:t,callback:e}),A&&A.immediate&&e(t,this.getValue(t),this.getValue(t)),n},removeValueChangeListener(t){let e=null;for(const[A,n]of this.$listener.listenData.entries())if(n.id===t){e=A;break}typeof e=="string"?this.$listener.listenData.delete(e):console.warn("没有找到对应的监听器");},triggerMenuValueChange(t,e,A){if(this.$listener.listenData.has(t)){let n=this.$listener.listenData.get(t);if(typeof n.callback=="function"){let i=this.getValue(t),a=i,o=i;typeof e<"u"&&arguments.length>1&&(a=e),typeof A<"u"&&arguments.length>2&&(o=A),n.callback(t,o,a);}}},hasKey(t){let e=R(Y,{});return t in e},execMenu(t,e,A=false){if(!(typeof t=="string"||typeof t=="object"&&Array.isArray(t)))throw new TypeError("key 必须是字符串或者字符串数组");let n=[];typeof t=="object"&&Array.isArray(t)?n=[...t]:n.push(t);let i;for(let a=0;a<n.length;a++){const o=n[a];if(!this.$data.data.has(o)){u.warn(`${t} 键不存在`);return}let s=v.getValue(o);if(A&&(s=!s),!s)break;i=s;}i&&e(i);},execMenuOnce(t,e,A,n){if(typeof t!="string")throw new TypeError("key 必须是字符串");if(!this.$data.data.has(t)){u.warn(`${t} 键不存在`);return}if(this.$data.oneSuccessExecMenu.has(t))return;this.$data.oneSuccessExecMenu.set(t,1);let i=()=>{let c=v.getValue(t);return typeof A=="function"?A(t,c):c},a=[],o=c=>{let p=i(),f=[];if(c instanceof HTMLStyleElement?f=[c]:Array.isArray(c)&&(f=[...c.filter(m=>m!=null&&m instanceof HTMLStyleElement)]),p)a=a.concat(f);else for(let m=0;m<f.length;m++)f[m].remove(),f.splice(m,1),m--;},s=c=>{let p=[];if(c){let f=e(c,o);f instanceof HTMLStyleElement?p=[f]:Array.isArray(f)&&(p=[...f.filter(m=>m!=null&&m instanceof HTMLStyleElement)]);}for(let f=0;f<a.length;f++)a[f].remove(),a.splice(f,1),f--;a=[...p];};this.addValueChangeListener(t,(c,p,f)=>{let m=f;typeof n=="function"&&(m=n(c,f,p)),s(m);});let l=i();l&&s(l);},execInheritMenuOnce(t,e,A,n){let i=this;const a=(o,s)=>{let l=i.getValue(o),c=i.getValue(s);if(typeof n=="function"){let p=n(l,c);if(p!==void 0)return p}return l};this.execMenuOnce(t,A,()=>a(t,e),()=>a(t,e)),this.execMenuOnce(e,()=>{},()=>false,()=>(this.triggerMenuValueChange(t),false));},onceExec(t,e){if(typeof t!="string")throw new TypeError("key 必须是字符串");this.$data.onceExec.has(t)||(e(),this.$data.onceExec.set(t,1));},showPanel(){D.panel({title:{text:r.t("{{SCRIPT_NAME}}-设置",{SCRIPT_NAME:xt}),position:"center",html:false,style:""},content:this.getPanelContentConfig(),mask:{enable:true,clickEvent:{toClose:true,toHide:false}},zIndex(){let t=lt.getMaxZIndex(),e=D.config.InstanceUtils.getPopsMaxZIndex().zIndex;return lt.getMaxValue(t,e)+100},width:$.setting.width,height:$.setting.height,drag:true,only:true,style:`
			${_t}
			`});},getPanelContentConfig(){return [Wt,Jt,Ae,$t,te]}},re=`:root {\r
...
/* 狐猴浏览器 */\r
#android-browser-list li:nth-child(10)::before {\r
...
`,ue="",fe={init(){v.execMenuOnce("beautifyPage",()=>this.beautifyPageElement()),v.execMenuOnce("beautifyGreasyforkBeautify",()=>this.beautifyGreasyforkBeautify()),v.execMenuOnce("beautifyUploadImage",()=>this.beautifyUploadImage()),v.execMenuOnce("beautifyTopNavigationBar",()=>this.beautifyTopNavigationBar());},beautifyPageElement(){u.info("美化页面元素");let t=[];return t.push(P(re)),t.push(P(ne)),t.push(P(oe)),t.push(P(ie)),t.push(P(ae)),t.push(P(`
			p:has(input[type="submit"][name="update-and-sync"]){
			  margin-top: 10px;
			}
			`)),d.ready(()=>{let e=z('a[target="markup_choice"][href*="daringfireball.net"]');e&&e.parentElement.replaceChild(d.createElement("span",{textContent:"Markdown"}),e),E(".user-content ul li").forEach(i=>{var o,s;let a=i.firstChild;(a==null?void 0:a.nodeName)==="#text"&&((o=a.textContent)!=null&&o.startsWith("[x] ")?(a.textContent=a.textContent.replace("[x] ",""),d.prepend(i,`
							<input type="checkbox" disabled="" class="task-list-item-checkbox" checked="">
						`)):(s=a.textContent)!=null&&s.startsWith("[ ] ")&&(a.textContent=a.textContent.replace("[ ] ",""),d.prepend(i,`
				...
				`},n=Object.keys(A);if(E(".user-content blockquote").forEach(i=>{Array.from(i.children).forEach(a=>{let o=d.html(a);if(a.nodeName==="P"){let s=n.find(l=>o.toLowerCase().startsWith(l.toLowerCase()));s&&(d.html(a,o.replace(s,"")),d.before(a,A[s]),d.html(a)===""&&a.remove());}});}),E(".user-content > blockquote").forEach(i=>{Array.from(i.querySelectorAll(".markdown-alert-title")).forEach(a=>{var s,l;let o=d.createElement("div",{className:"markdown-alert markdown-alert-tip"},{"data-type":a.getAttribute("data-type")});for(;a.nextSibling&&!((l=(s=a.nextSibling)==null?void 0:s.classList)!=null&&l.contains("markdown-alert-title"));)d.append(o,a.nextSibling);d.prepend(o,a),d.before(i,o);});}),C.isHome())t.push(P(de));else if(C.isScriptsFeedback()){t.push(P(ue));let i=z('.radio-label[for*="discussion_rating_0"]');i&&(i.innerHTML=i.innerHTML.replace("不评分",'<span class="rating-icon rating-icon-none">不评分</span>'));let a=z('.radio-label[for*="discussion_rating_2"]');a&&(a.innerHTML=a.innerHTML.replace("差评",'<span class="rating-icon rating-icon-bad">差评</span>'));let o=z('.radio-label[for*="discussion_rating_3"]');o&&(o.innerHTML=o.innerHTML.replace("一般",'<span class="rating-icon rating-icon-ok">一般</span>'));let s=z('.radio-label[for*="discussion_rating_4"]');s&&(s.innerHTML=s.innerHTML.replace("好评",'<span class="rating-icon rating-icon-good">好评</span>'));}else C.isScriptAdmin()&&(z('input[type="submit"][name="update-only"]')||t.push(P(`
					.indented{
						padding-left: unset;
					}
					`)));}),t},beautifyGreasyforkBeautify(){u.info("美化 Greasyfork Beautify脚本");let t=[];return t.push(P(le)),g.isPhone()?t.push(P(`
				section#script-info,
				...
				section#script-info{
					margin-top: 10px;
				}`)),t},beautifyUploadImage(){u.info("美化上传图片");let t=[];return t.push(P(se)),d.ready(()=>{function e(i){for(;i.nextElementSibling;)i.parentElement.removeChild(i.nextElementSibling);}let A=E('input[type="file"]');A.forEach(i=>{i.getAttribute("name")!=="code_upload"&&(i.hasAttribute("accept")&&i.getAttribute("accept").includes("javascript")||d.on(i,["propertychange","input"],function(a){e(a.target);let o=a.currentTarget.files;if(!o||o.length===0)return;u.info("选择的图片",o),o.length>5&&d.after(i,d.createElement("p",{textContent:r.t("❌ 最多同时长传5张图片")}));let s=[];Array.from(o).forEach(l=>{(l.size>204800||!l.type.match(/png|jpg|jpeg|gif|apng|webp/i))&&s.push(l);}),s.length!==0&&s.forEach(l=>{d.after(i,d.createElement("p",{textContent:r.t("❌ 图片：{{name}} 大小：{{size}}",{name:l.name,size:l.size})}));});}));}),["textarea#comment_text","textarea.comment-entry"].forEach(i=>{d.on(i,"paste",a=>{u.info("触发粘贴事件",a),setTimeout(()=>{d.trigger(A,"input");},100);});});}),t},beautifyTopNavigationBar(){u.info("美化顶部导航栏");let t=[];return t.push(P(ce)),window.outerWidth>550&&(t.push(tt.addBlockCSS(".with-submenu")),d.ready(()=>{let A=z("#site-nav").querySelector("nav");E(".with-submenu nav li").forEach(n=>{A.appendChild(n);});})),t}},pe={init(){v.execMenu("autoLogin",()=>{this.autoLogin();});},autoLogin(){g.waitNode("span.sign-in-link a[rel=nofollow]").then(async()=>{let t=v.getValue("user"),e=v.getValue("pwd"),A=v.getValue("secret");if(g.isNull(t)){h.error(r.t("请先在菜单中录入账号"));return}if(g.isNull(e)){h.error(r.t("请先在菜单中录入密码"));return}if(g.isNull(A)){h.error(r.t("请先在菜单中录入secret"));return}let n=z("meta[name='csrf-token']");if(!n){h.error(r.t("获取csrf-token失败"));return}let a=new Nt__namespace.TOTP({secret:A}).generate(),o=h.loading(r.t("正在登录中...")),s=await B.post("/zh-CN/users/sign_in",{fetch:true,data:encodeURI(`authenticity_token=${n.getAttribute("content")}&user[email]=${t}&user[password]=${e}&user[remember_me]=1&user[otp_attempt]=${a}&commit=登录`),headers:{"Content-Type":"application/x-www-form-urlencoded"}});if(o.destroy(),!s.status){u.error(s),h.error(r.t("登录失败，请求异常"));return}let l=s.data.responseText,c=d.parseHTML(l,true,true),p=c.querySelector(".width-constraint .alert");c.querySelectorAll(".sign-out-link a[rel=nofollow][data-method='delete']").length?(h.success(r.t("登录成功，1s后自动跳转")),setTimeout(()=>{window.location.reload();},1e3)):p?(u.error(p),h.error(d.text(p))):(h.error(r.t("登录失败，可能是账号/密码错误，请在控制台查看原因"),{consoleLogContent:true}),u.error(s),u.error(`当前账号:${t}`),u.error(`当前密码:${e}`));});}},ve={init(){this.readBgColor(),d.ready(()=>{v.execMenuOnce("greasyfork-discussions-filter-enable",()=>{this.filterEnable();});let t=new g.LockFunction(()=>{v.execMenu("discussions-addShortcutOperationButton",()=>{this.addShortcutOperationButton();}),v.execMenu("discussions-addReportButton",()=>{this.addReportButton();});});g.mutationObserver(document.body,{config:{subtree:true,childList:true},immediate:true,callback:()=>{t.run();}});});},filterEnable(){u.info("启用Greasyfork论坛过滤器"),Z.init();},readBgColor(){u.info("设置已读背景颜色");let t=v.getValue("discussions-readBgColor");return P(`
        .discussion-read{
            background: ${t} !important;
        }
        `)},addShortcutOperationButton(){u.info("添加【过滤】按钮"),Z.getElementList().forEach(t=>{if(t.querySelector(".discussion-filter-button"))return;let A=t.querySelector(".discussion-list-item").querySelector(".discussion-meta"),n=d.createElement("div",{className:"discussion-meta-item",innerHTML:`
					<button class="discussion-filter-button">${r.t("过滤")}</button>
					`},{style:"flex: 0;"}),i=n.querySelector(".discussion-filter-button");A.appendChild(n),d.on(i,"click",a=>{var f,m,w;g.preventEvent(a);const o=Z.parseDiscuessionListContainerInfo(t);let s="data-filter-key",l="data-filter-value",c=D.alert({title:{text:r.t("选择需要过滤的选项"),position:"center",html:false},content:{text:`
						...
						`}),p=c.$shadowRoot.querySelector(".pops-alert-content");if(o.scriptId==null&&((f=p.querySelector(`button[${s}="scriptId"]`))==null||f.remove()),o.scriptName==null&&((m=p.querySelector(`button[${s}="scriptName"]`))==null||m.remove()),o.postUserId==null&&((w=p.querySelector(`button[${s}="postUserId"]`))==null||w.remove()),o.replyUserId!=null){let x=d.createElement("button",{innerHTML:r.t("作者id：{{text}}",{text:o.replyUserId})});x.setAttribute(s,"scriptAuthorId"),x.setAttribute(l,"^"+o.replyUserId+"$"),p.appendChild(x);}d.on(c.$shadowRoot,"click",`button[${s}]`,x=>{g.preventEvent(x);let b=x.target,M=b.getAttribute(s),k=b.getAttribute(l);Z.addValue(M,k),c.close(),Z.filter(),h.success(r.t("添加成功"));});});});},addReportButton(){u.info("添加【举报】按钮"),Z.getElementList().forEach(t=>{if(t.querySelector(".discussion-report-button"))return;let A=t.querySelector(".discussion-list-item").querySelector(".discussion-meta"),n=d.createElement("div",{className:"discussion-meta-item",innerHTML:`
					<button class="discussion-report-button" style="border-color: #ff4d4d;background-color: #ffe6e6;color: red;">${r.t("举报")}</button>
					`},{style:"flex: 0;"}),i=n.querySelector(".discussion-report-button");A.appendChild(n),d.on(i,"click",a=>{g.preventEvent(a);const o=Z.parseDiscuessionListContainerInfo(t);D.alert({title:{text:r.t("举报"),position:"center",html:false},content:{text:`
						<div class="report-item">
							${r.t("举报讨论：")}
							<a href="${L.getReportUrl("discussion",o.snippetId)}" target="_blank">${o.snippet}</a>
						</div>
						${o.scriptId?`
							<div class="report-item">
							${r.t("举报脚本：")}
							<a href="${L.getReportUrl("script",o.scriptId)}" target="_blank">${o.scriptName}</a>
						</div>
						`:""}
						
						<div class="report-item">
							${r.t("举报用户：")}
							<a href="${L.getReportUrl("user",o.postUserId)}" target="_blank">${o.postUserName}</a>
						</div>
						${o.replyUserId&&o.replyUserId!=o.postUserId?`
								<div class="report-item">
									${r.t("举报用户：")}
									<a href="${L.getReportUrl("user",o.replyUserId)}" target="_blank">${o.replyUserName}</a>
								</div>
								`:""}
							...
							`});});});}},ge={init(){v.execMenuOnce("code-repairCodeLineNumber",()=>{this.repairCodeLineNumber();}),v.execMenuOnce("code-use-monaco-editor",()=>{this.coverEditorWithMonaco();});},repairCodeLineNumber(){u.info("修复代码的行号显示不够问题"),v.execMenuOnce("beautifyGreasyforkBeautify",()=>{P(`
				.code-container pre code .marker{
					padding-left: 6px;
				}	
				`);}),g.waitNode("#script-content div.code-container pre.prettyprint ol").then(t=>{t.childElementCount>=1e3&&(u.success(`当前代码行数${t.childElementCount}行，超过1000行，优化行号显示问题`),P(`
                    pre.prettyprint{
                        padding-left: 26px;
                    }
					`));});},coverEditorWithMonaco(){u.info("使用monacoEditor替换编辑器"),P(`
			.monaco-editor{
		...
		`),tt.addBlockCSS("#script-content .code-container > pre");let t=L.getScriptId(window.location.href);if(!t){h.error("未解析出当前脚本ID",{consoleLogContent:true});return}Promise.all([wt.monacoEditor(),j.getScriptInfo(t)]).then(async([e,A])=>{let n=A==null?void 0:A.code_url;if(!n){h.error("请求结果中未解析出脚本代码URL",{consoleLogContent:true});return}let i=new URLSearchParams(window.location.search);if(i.has("version")){let s=i.get("version");n=n.replace(new RegExp(`/${t}(/[\\d]+|)`),`/${t}/${s}`),u.info(`当前是历史代码页面，请求的脚本代码URL为${n}`);}let a=await B.get(n,{timeout:2e4});if(!a.status)return;let o=a.data.responseText;d.ready(async()=>{let s=await g.waitNode("#script-content .code-container > pre",1e4);if(!s)return;let l=d.createElement("div",{className:"monaco-editor"});d.after(s,l),e.editor.create(l,{value:o,minimap:{enabled:true},automaticLayout:true,codeLens:true,colorDecorators:true,contextmenu:true,readOnly:true,formatOnPaste:true,overviewRulerBorder:true,scrollBeyondLastLine:true,theme:"vs-dark",fontSize:window.innerWidth>600?14:12,wordWrap:"off",language:"javascript",folding:true,foldingStrategy:"indentation"});});});}},he=`ul.history_versions,\r
...
`,me={init(){v.execMenuOnce("beautifyHistoryVersionPage",()=>this.beautifyHistoryVersionPage()),v.execMenuOnce("scripts-versions-addExtraTagButton",()=>{this.addExtraTagButton();}),v.execMenuOnce("scripts-versions-addCompareCodeButton",()=>{this.sourceDiffMonacoEditor();});},beautifyHistoryVersionPage(){u.info("美化 历史版本 页面");let t=[];return t.push(P(he)),t.push(tt.addBlockCSS(".version-number",".version-date",".version-changelog")),d.ready(function(){let e=z("ul.history_versions");if(!e){h.error(r.t("未找到history_versions元素列表"));return}Array.from(e.children).forEach(A=>{var p,f;let n=A.querySelector(".version-number a").href,i=A.querySelector(".version-number a").innerText,a=(p=A.querySelector(".version-date"))==null?void 0:p.getAttribute("datetime"),o=((f=A.querySelector(".version-changelog"))==null?void 0:f.innerHTML)||"",s=d.createElement("span",{className:"script-version-date",innerHTML:g.formatTime(a,r.t("yyyy年MM月dd日 HH:mm:ss"))}),l=d.createElement("div",{className:"script-tag",innerHTML:`
                    <div class="script-tag-version">
                    ...
                    </div>`}),c=d.createElement("div",{className:"script-note-box-body",innerHTML:o});A.appendChild(s),A.appendChild(l),A.appendChild(c);});}),t},addExtraTagButton(){u.info("添加额外的标签按钮"),d.ready(()=>{E(".script-tag-version").forEach(t=>{var c,p;let e=t.querySelector("a");if(!e)return;let A=new URL(e.href),n=(c=A.pathname.match(/\/scripts\/([\d]+)/))==null?void 0:c[1],i=A.searchParams.get("version"),a=(p=A.pathname.match(/\/scripts\/[\d]+-(.+)/))==null?void 0:p[1],o=L.getInstallUrl(n,i,a),s=L.getCodeUrl(n,i),l=d.createElement("div",{className:"scripts-tag-install",innerHTML:`
						...
						`});d.after(t,l);});});},sourceDiffMonacoEditor(){u.info("源码对比（monacoEditor）"),wt.monacoEditor().then(t=>{d.ready(()=>{E('#script-content form[action*="/diff"] input[type="submit"]').forEach(e=>{let A=d.createElement("input",{type:"button",value:r.t("对比选中版本差异（monacoEditor）")},{style:"margin-left: 10px;"});d.after(e,A),d.on(A,"click",async n=>{g.preventEvent(n);let i=e.closest("form"),a=new FormData(i),o=a.get("v1"),s=a.get("v2");if(o===s){h.warning(r.t("版本号相同，不需要比较源码"));return}let l=h.loading(r.t("正在获取对比文本中...")),c=L.getScriptId(),p=await j.getScriptInfo(c);if(!p){l.close();return}let f=p.code_url,m=f.replace(new RegExp(`/${c}(/[\\d]+|)`),`/${c}/${o}`),w=f.replace(new RegExp(`/${c}(/[\\d]+|)`),`/${c}/${s}`),x="",b="";const[M,k]=await Promise.all([B.get(m,{timeout:2e4}),B.get(w,{timeout:2e4})]);if(l.close(),!M.status||!k.status)return;x=M.data.responseText,b=k.data.responseText;let{recovery:H}=tt.lockScroll(),G=D.alert({title:{text:r.t("代码对比"),html:false,position:"center"},content:{html:true,text:`
								<div class="monaco-editor-diff-container">
									<div class="monaco-editor-diff"></div>
								</div>
								`},mask:{enable:true,clickEvent:{toClose:false,toHide:false}},btn:{ok:{enable:false},close:{callback(Q,U){Q.close(),H();}}},zIndex(){let Q=g.getMaxZIndex(),U=D.config.InstanceUtils.getPopsMaxZIndex().zIndex;return g.getMaxValue(Q,U)+100},useShadowRoot:false,width:"90vw",height:"90vh",drag:true,style:`
						...
						`});G.$shadowRoot.querySelector(".monaco-editor-diff-container");let I=G.$shadowRoot.querySelector(".monaco-editor-diff"),K=t.editor.createDiffEditor(I,{hideUnchangedRegions:{enabled:true},minimap:{enabled:true},automaticLayout:true,codeLens:true,colorDecorators:true,contextmenu:false,readOnly:true,formatOnPaste:true,overviewRulerBorder:true,scrollBeyondLastLine:true,theme:"vs-dark",fontSize:window.innerWidth>600?14:12,wordWrap:"off",language:"javascript"});const N=t.editor.createModel(b,"javascript"),q=t.editor.createModel(x,"javascript");K.setModel({original:N,modified:q});});});});});}};let ut=[];const It=async function(t){if(u.info("当前脚本id："+t),!S.isLogin){u.error("请先登录账号"),h.error(r.t("请先登录账号"));return}let e=L.getUserId(S.getUserLinkElement().href);if(e==null){u.error("获取用户id失败"),h.error(r.t("获取用户id失败"));return}if(!ut.length){let a=h.loading(r.t("获取收藏夹中..."));if(ut=await j.getUserCollection(e)||[],a.close(),!ut.length)return}let A="";const n=(a,o)=>{let s=false;o=o.toString().trim();for(const[l,c]of a.entries())if(l==="scripts-included[]"&&c.toString().trim()===o){s=true;break}return s};ut.forEach(a=>{A+=`
		<li class="user-collect-item" data-id="${a.id}" data-name="${a.name}">
		  ...
		  `;});let i=D.alert({title:{text:r.t("收藏集"),position:"center"},content:{html:true,text:`<ul>${A}</ul>`},mask:{enable:true,clickEvent:{toClose:true}},btn:{ok:{enable:false}},width:D.isPhone()?"92vw":"500px",height:"auto",drag:true,only:true,style:`
		.pops{
		...
		`});d.on(i.$shadowRoot,"click",".collect-add-script-id",async function(a){let o=a.target.closest(".user-collect-item"),s=o.dataset.id;o.dataset.name;let l=h.loading(r.t("添加中..."));try{let c=await j.getUserCollectionInfo(e,s);if(!c)return;if(n(c,t)){h.warning(r.t("该脚本已经在该收藏集中"));return}let p=g.cloneFormData(c),f=g.cloneFormData(c);p.set("add-script",t.toString()),p.set("script-action","i"),f.append("scripts-included[]",t.toString()),f.set("save","1");let m=new URLSearchParams(p),w=new URLSearchParams(f),x=Array.from(m).map(([N,q])=>`${encodeURIComponent(N)}=${encodeURIComponent(q)}`).join("&"),b=Array.from(w).map(([N,q])=>`${encodeURIComponent(N)}=${encodeURIComponent(q)}`).join("&");u.info("添加的数据",x),u.info("保存的数据",b);let M=await j.updateUserSetsInfo(e,s,x);if(!M)return;let k=M.querySelector(".change-script-set");if(!k){h.error(r.t("添加失败，{{selector}}元素不存在",{selector:".change-script-set"}));return}let H=k.querySelector("section");if(!H){h.error(r.t("添加失败，{{selector}}元素不存在",{selector:"section"}));return}let G=H.querySelector(".alert");if(G){D.alert({title:{text:r.t("添加失败"),position:"center"},content:{text:G.innerHTML,html:!0},mask:{enable:!0,clickEvent:{toClose:!0}},style:`
						.pops-alert-content{
							font-style: italic;
						...
						`,drag:!0,dragLimit:!0,width:$.info.width,height:$.info.height});return}let I=new FormData(k);if(!n(I,t)){u.error("添加失败，提交的添加请求中不包含该脚本id"),h.error(r.t("添加失败，表单数据中不包含该脚本"));return}await j.updateUserSetsInfo(e,s,b),h.success(r.t("添加成功"));}catch(c){console.error(c);}finally{l.close();}}),d.on(i.$shadowRoot,"click",".collect-delete-script-id",async function(a){let o=a.target.closest(".user-collect-item"),s=o.dataset.id;o.dataset.name;let l=h.loading(r.t("删除中..."));try{let c=await j.getUserCollectionInfo(e,s);if(!c)return;if(!n(c,t)){h.info(r.t("已删除：{{scriptId}}",{scriptId:t}));return}let p=g.cloneFormData(c,(I,K)=>I==="scripts-included[]"&&typeof K=="string"&&K.toString().trim()===t.toString().trim()),f=g.cloneFormData(p);p.set("remove-scripts-included[]",t.toString()),p.set("remove-selected-scripts","i"),p.delete("script-action"),f.set("save","1");let m=new URLSearchParams(p),w=new URLSearchParams(f),x=Array.from(m).map(([I,K])=>`${encodeURIComponent(I)}=${encodeURIComponent(K)}`).join("&"),b=Array.from(w).map(([I,K])=>`${encodeURIComponent(I)}=${encodeURIComponent(K)}`).join("&");u.info("删除的数据",x),u.info("保存的数据",b);let M=await j.updateUserSetsInfo(e,s,x);if(!M)return;let k=M.querySelector(".change-script-set");if(!k){h.error(r.t("删除失败，{{selector}}元素不存在",{selector:".change-script-set"}));return}let H=new FormData(k);if(n(H,t)){u.error("删除失败，提交的删除请求中包含该脚本id"),h.error(r.t("删除失败，表单数据中仍包含该脚本"));return}await j.updateUserSetsInfo(e,s,b),h.success(r.t("删除成功"));}catch(c){console.error(c);}finally{l.close();}});},we={init(){C.isCode()?ge.init():C.isVersion()&&me.init(),C.isCodeStrict()&&(v.execMenuOnce("fullScreenOptimization",()=>{this.fullScreenOptimization();}),v.execMenuOnce("addCopyCodeButton",()=>{this.addCopyCodeButton();})),v.execMenuOnce("addCollectionButton",()=>{this.addCollectionButton();}),v.execMenuOnce("addFindReferenceButton",()=>{this.setFindCodeSearchBtn();}),d.ready(()=>{v.execMenuOnce("scriptHomepageAddedTodaySUpdate",()=>{this.scriptHomepageAddedTodaySUpdate();});});},addCollectionButton(){u.info("添加收藏按钮"),g.waitNode("ul#script-links li.current span").then(()=>{let t=d.createElement("li",{innerHTML:`
					<a href="javascript:;">
						<span>${r.t("收藏")}</span>
					</a>`});d.append(z("ul#script-links"),t),d.on(t,"click",()=>{let e=window.location.pathname.match(/scripts\/([\d]+)/i);if(!e){u.error(e,window.location.pathname),h.error(r.t("获取脚本id失败"));return}let A=e[e.length-1];It(A);});});},fullScreenOptimization(){u.info("F11全屏，F键代码全屏"),P(`
...
`,gt={getTampermonkey:()=>{var t;return (t=O.external)==null?void 0:t.Tampermonkey},getViolentmonkey:()=>{var t;return (t=O.external)==null?void 0:t.Violentmonkey},getScriptCat:()=>{var t;return (t=O.external)==null?void 0:t.Scriptcat},getScriptContainerStatus(){var e,A,n;let t={Tampermonkey:false,Violentmonkey:false,ScriptCat:false};return (e=O.external)!=null&&e.Tampermonkey&&(t.Tampermonkey=true),(A=O.external)!=null&&A.Violentmonkey&&(t.Violentmonkey=true),(n=O.external)!=null&&n.Scriptcat&&(t.ScriptCat=true),t},getRegisterScriptContainerNameList(){let e=this.getScriptContainerStatus(),A=[];return Object.keys(e).forEach(n=>{Reflect.get(e,n)&&A.push(n);}),A},getInstalledVersion(t,e){return new Promise((A,n)=>{const i=this.getTampermonkey();if(i){i.isInstalled(t,e,function(s){s.installed?A(s.version):A(null);});return}const a=this.getViolentmonkey();if(a){a.isInstalled(t,e).then(A);return}const o=this.getScriptCat();if(o){o.isInstalled(t,e,function(s){s.installed?A(s.version):A(null);});return}n(new TypeError("获取脚本容器暴露的external信息失败"));})},compareVersions(t,e){if(t===e)return 0;const A=t.split("."),n=e.split(".");for(let i=0;i<A.length;i++){const a=this.compareVersionPart(A[i],n[i]);if(a!==0)return a}return 0},compareVersionPart(t,e){const A=this.parseVersionPart(t),n=this.parseVersionPart(e);for(let i=0;i<A.length;i++){if(A[i].length>0&&n[i].length===0)return  -1;if(A[i].length===0&&n[i].length>0||A[i]>n[i])return 1;if(A[i]<n[i])return  -1}return 0},parseVersionPart(t){if(!t)return [0,"",0,""];const e=/([0-9]*)([^0-9]*)([0-9]*)([^0-9]*)/.exec(t);return [e[1]?parseInt(e[1]):0,e[2],e[3]?parseInt(e[3]):0,e[4]]},handleInstallResult(t,e,A){if(e!=null)switch(t.removeAttribute("data-ping-url"),this.compareVersions(e,A)){case  -1:t.textContent=t.getAttribute("data-update-label");break;case 1:t.textContent=t.getAttribute("data-downgrade-label");break;case 0:t.textContent=t.getAttribute("data-reinstall-label");break}},async checkForUpdatesJS(t,e){const A=t.getAttribute("data-script-name"),n=t.getAttribute("data-script-namespace"),i=t.getAttribute("data-script-version");try{let a=await this.getInstalledVersion(A,n);return a==null?!1:(this.handleInstallResult(t,a,i),!0)}catch{if(e){await g.sleep(1e3);try{return await this.checkForUpdatesJS(t,!1)}catch{}}return  false}},checkForUpdatesCSS(t){const e=t.getAttribute("data-script-name"),A=t.getAttribute("data-script-namespace");postMessage({type:"style-version-query",name:e,namespace:A,url:location.href},location.origin);}},Tt={init(){v.execMenuOnce("gf-scripts-filter-enable",()=>{J.init();}),v.execMenuOnce("beautifyCenterContent",()=>this.beautifyCenterContent());},beautifyCenterContent(){u.info("美化脚本列表-双列");let t=[];t.push(P(ye));const e="lum-lightbox-loader",A=r.t("安装此脚本");return T.ready(()=>{let n=gt.getScriptContainerStatus(),i=Object.values(n).find(c=>c),a=gt.getRegisterScriptContainerNameList(),o=v.getValue("beautifyCenterContent-queryNameSpace");i?u.info("当前暴露的external信息："+a.map(c=>`【${c}】`).join("、")):u.error("脚本容器未暴露external信息",window.external);let s=new g.LockFunction(()=>{J.getElementList().forEach(p=>{if(p.querySelector(".script-list-operation"))return;let f=et.parseScriptListInfo(p),m=p.querySelector(".inline-script-stats");if(!m)return;let w=f.codeUrl,x=T.createElement("dt",{className:"script-list-rating-score",innerHTML:`<span>${r.t("评分")}</span>`}),b=T.createElement("dd",{className:"script-list-rating-score",innerHTML:`<span>${f.scriptRatingScore}</span>`},{"data-position":"right"}),M=p.querySelector("dd.script-list-ratings .good-rating-count"),k=p.querySelector("dd.script-list-ratings .ok-rating-count"),H=p.querySelector("dd.script-list-ratings .bad-rating-count");if(M&&k&&H){let U=parseInt(M.innerText),F=parseInt(k.innerText),X=parseInt(H.innerText),it=U+F+X;it>=10?U/it>=.6?b.classList.add("good-rating-count"):b.classList.add("bad-rating-count"):it==0||U>F+X?b.classList.add("good-rating-count"):b.classList.add("bad-rating-count");}let G=T.createElement("dt",{className:"script-list-version",innerHTML:`<span>${r.t("版本")}</span>`}),I=T.createElement("dd",{className:"script-list-version",innerHTML:`<span>${f.scriptVersion}</span>`},{"data-position":"right"}),K=T.createElement("dt",{className:"script-list-operation",innerHTML:`<span>${r.t("操作")}</span>`}),N=T.createElement("dd",{className:"script-list-operation",innerHTML:`
						<a
							target="_blank"
							class="install-link"
						...
						`},{"data-position":"right",style:"gap:10px;display: flex;flex-wrap: wrap;align-items: center;"}),q=N.querySelector(".script-collect-btn"),Q=N.querySelector(".install-link");if(Q["data-script-info"]=f,T.addClass(Q,e),f.scriptType==="library"&&Q.remove(),T.on(q,"click",U=>{g.preventEvent(U),It(f.scriptId);}),v.getValue("gf-scripts-filter-enable")){let U=T.createElement("button",{className:"script-filter-btn",innerHTML:r.t("过滤")}),F="data-filter-key",X="data-filter-value";T.on(U,"click",it=>{g.preventEvent(it);let pt=D.alert({title:{text:r.t("选择需要过滤的选项"),position:"center"},content:{text:`
									<button ${F}="scriptId" ${X}="^${f.scriptId}$">${r.t("脚本id：{{text}}",{text:f.scriptId})}</button>
									<button ${F}="scriptName" ${X}="^${g.parseStringToRegExpString(f.scriptName)}$">${r.t("脚本名：{{text}}",{text:f.scriptName})}</button>
								...
								`}),yt=pt.$shadowRoot.querySelector(".pops-alert-content");f.scriptAuthors.forEach(_=>{let At=T.createElement("button",{innerHTML:r.t("作者id：{{text}}",{text:_.authorId})});At.setAttribute(F,"scriptAuthorId"),At.setAttribute(X,"^"+_.authorId+"$");let at=T.createElement("button",{innerHTML:r.t("作者名：{{text}}",{text:_.authorName})});at.setAttribute(F,"scriptAuthorName"),at.setAttribute(X,"^"+g.parseStringToRegExpString(_.authorName)+"$"),yt.appendChild(At),yt.appendChild(at);}),T.on(pt.$shadowRoot,"click",`button[${F}]`,_=>{g.preventEvent(_);let At=_.target,at=At.getAttribute(F),Ot=At.getAttribute(X);J.addValue(at,Ot),pt.close(),J.filter(),h.success(r.t("添加成功"));});}),N.appendChild(U);}m.appendChild(x),m.appendChild(b),m.appendChild(G),m.appendChild(I),m.appendChild(K),m.appendChild(N);});},100),l=new g.LockFunction(async()=>{let c=Array.from(E(".install-link[data-install-format=js]:not([gm-is-check-install-status])"));for(let p=0;p<c.length;p++){const f=c[p];f.setAttribute("gm-is-check-install-status","");let m=Reflect.get(f,"data-script-info");if(i){if(o){let w=await j.getScriptInfo(m.scriptId);w&&f.setAttribute("data-script-namespace",w.namespace);}gt.checkForUpdatesJS(f,true).then(w=>{T.removeClass(f,e),w||T.text(f,A);});}else T.removeClass(f,e),T.text(f,A);}});g.mutationObserver(document,{config:{subtree:true,childList:true},immediate:true,callback:()=>{s.run(),l.run();}});}),t}},be={init(){v.execMenuOnce("users-changeConsoleToTopNavigator",()=>{this.changeConsoleToTopNavigator();}),v.execMenuOnce("gf-scripts-filter-enable",()=>{J.init();}),v.execMenuOnce("beautifyCenterContent",()=>Tt.beautifyCenterContent());},changeConsoleToTopNavigator(){u.info("迁移【控制台】到顶部导航栏"),tt.addBlockCSS("#about-user"),d.ready(()=>{let t=z("#about-user");if(!t){u.error("#about-user元素不存在");return}t=t.cloneNode(true),et.registerTopNavMenu({name:r.t("控制台"),className:"scripts-console",clickEvent(e){D.alert({title:{text:r.t("控制台"),position:"center"},content:{text:"",html:true},btn:{ok:{enable:false}},mask:{enable:true,clickEvent:{toClose:true}},drag:true,useShadowRoot:true,width:$.setting.width,height:$.setting.height,zIndex:g.getMaxZIndex(100),style:`
						#about-user{
							border: 0;
							box-shadow: none;
						}
						...
						`}).$shadowRoot.querySelector(".pops-alert-content").appendChild(t);}});});}},xe=`section.text-content {\r
...
.comment:not(:has(.report-link)) .comment-meta-item {\r
	margin-left: 0px;\r
	margin-right: 15px;\r
}`,Pe={init(){v.execMenuOnce("conversations-beautifyDialogBox",()=>this.beautifyDialogBox()),d.ready(()=>{v.execMenuOnce("conversations-beautifyPrivateMessageList",()=>{this.beautifyPrivateMessageList();});});},beautifyDialogBox(){return u.info("美化对话框"),P(xe)},beautifyPrivateMessageList(){u.info("美化私信列表"),P(`
		...
		`),E("section.text-content ul li").forEach(t=>{var l;let e=t.querySelector('a[href*="conversations"]'),A=e.href,n=(l=e.textContent)==null?void 0:l.split(" ")[1],i=t.querySelector("a.user-link"),a=null,o=null,s=null;if(i){a=i.textContent,o=i.href;let c=t.querySelector("relative-time");new Date(c.getAttribute("datetime")),s=c.shadowRoot.lastChild.textContent;}t.classList.add("user-conversations-item"),L.getUserId(),t.innerHTML=`
			...
			`;});}},zt={waitScritList(){return g.waitNode("#browse-script-list",1e4)},addFilterControls(t){function e(){let l=z("#gm-script-filter-controls");if(!l)return;let c=l==null?void 0:l.shadowRoot;if(!c)return;let p=c.querySelector(".pops-filter-controls_inner"),f=c.querySelector(".pops-filter-search-container");if(p&&f)return {$filterControl:p,$search:f}}let A=e();if(A)return A;let n=d.createElement("div",{id:"gm-script-filter-controls"}),i=n.attachShadow({mode:"open"});i.appendChild(d.createElement("style",{innerHTML:`
                    ${D.config.cssText.index}

                    ${D.config.cssText.common}
			...
			`}),o=a.querySelector(".pops-filter-controls_inner"),s=a.querySelector(".pops-filter-search-container");return i.appendChild(a),d.before(t,n),{$filterControl:o,$search:s}}},Le={init(){d.ready(()=>{zt.waitScritList().then(t=>{if(!t){u.error("未找到脚本列表节点，无法继续执行");return}v.onceExec("GreasyforkScriptsSearch",()=>{let{$filterControl:e,$search:A}=zt.addFilterControls(t);this.addFilterControlsItem(A,e);});});});},addFilterControlsItem(t,e){let A=D.config.panelHandleContentUtils(),n=[{name:r.t("名称"),desc:r.t("全词匹配"),ENABLE_KEY:"gf-script-search-filterScriptTitleWholeWordMatching",STORAGE_KEY:"gf-script-search-filterScriptTitleWholeWordMatching-enable",callback:(o,s)=>{try{return !s.scriptName.match(new RegExp(o,"i"))}catch(l){return u.error(l),!s.scriptName.match(o)}},mutualExclusionSwitch:['.gm-script-control-item[data-storage-key="gf-script-search-filterScriptTitleOrDescWholeWordMatching-enable"]']},{name:r.t("描述"),desc:r.t("全词匹配"),ENABLE_KEY:"gf-script-search-filterScriptDescWholeWordMatching",STORAGE_KEY:"gf-script-search-filterScriptDescWholeWordMatching-enable",callback:(o,s)=>{try{return !s.scriptDescription.match(new RegExp(o,"i"))}catch{return !s.scriptDescription.match(o)}},mutualExclusionSwitch:['.gm-script-control-item[data-storage-key="gf-script-search-filterScriptTitleOrDescWholeWordMatching-enable"]']},{name:r.t("名称/描述"),desc:r.t("任一全词匹配"),ENABLE_KEY:"gf-script-search-filterScriptTitleOrDescWholeWordMatching",STORAGE_KEY:"gf-script-search-filterScriptTitleOrDescWholeWordMatching-enable",callback:(o,s)=>{let l=n.find(p=>p.ENABLE_KEY==="gf-script-search-filterScriptTitleWholeWordMatching").callback(o,s),c=n.find(p=>p.ENABLE_KEY==="gf-script-search-filterScriptDescWholeWordMatching").callback(o,s);return l&&c},mutualExclusionSwitch:['.gm-script-control-item[data-storage-key="gf-script-search-filterScriptTitleWholeWordMatching-enable"]','.gm-script-control-item[data-storage-key="gf-script-search-filterScriptDescWholeWordMatching-enable"]']},{name:r.t("作者名称"),desc:r.t("全词匹配"),ENABLE_KEY:"gf-script-search-filterScriptAuthorNameWholeWordMatching",STORAGE_KEY:"gf-script-search-filterScriptAuthorNameWholeWordMatching-enable",callback:(o,s)=>{try{return !s.scriptAuthorName.match(new RegExp(o,"i"))}catch{return !s.scriptAuthorName.match(o)}}}],i=()=>{var c,p;let o=t.querySelector("input");if(o)return o.value;let s=new URLSearchParams(window.location.search);return ((c=s.get("q"))==null?void 0:c.trim())||((p=s.get("c"))==null?void 0:p.trim())||""},a=()=>{let o=i();J.getElementList().forEach(l=>{let c=et.parseScriptListInfo(l),p=n.map(f=>{if(o=="")return  false;if(R(f.STORAGE_KEY))return f.callback(o,c)}).filter(f=>typeof f=="boolean");if(p.length!==0){let f=false;p.forEach(m=>{f=f||m;}),f?d.hide(l,false):d.show(l,false);}else d.show(l,false);});};if(v.getValue("gf-script-search-addFilterSearchInput")){let o=A.createSectionContainerItem_input({type:"input",className:"gm-script-search-input",getValue(){let s=i();return v.getValue("gf-script-search-addFilterSearchInput-text")||s||""},description:r.t("自动执行过滤"),placeholder:r.t("过滤的关键词"),text:r.t("关键词"),callback:g.debounce((s,l)=>{v.setValue("gf-script-search-addFilterSearchInput-text",l.toString()),a();},500)});d.append(t,o);}n.forEach(o=>{if(!v.getValue(o.ENABLE_KEY))return;u.info(`添加按钮${o.name}`);let s=A.createSectionContainerItem_switch({type:"switch",className:"gm-script-control-item",text:o.name,description:o.desc,attributes:{"data-storage-key":o.STORAGE_KEY},getValue(){let l=R(o.STORAGE_KEY,false);return l=!!l,a(),l},callback(l,c){if(V(o.STORAGE_KEY,c),a(),c&&Array.isArray(o.mutualExclusionSwitch)){let p=s.closest(".pops");o.mutualExclusionSwitch.forEach(f=>{let m=p.querySelector(f);if(m){let w=m.querySelector("input");if(w!=null&&w.checked){let x=m.querySelector(".pops-panel-switch__core");x==null||x.click();}}});}}});d.append(e,s);});}},ot={init(){if(C.isImageSource()){u.info("Router: 资源界面，不执行脚本功能");return}v.execMenu("checkPage",()=>{this.checkPage();}),fe.init(),Dt.init(),C.isScript()&&we.init(),(C.isScriptList()||C.isScriptLibraryList()||C.isScriptCodeSearch()||C.isScriptsBySite())&&Tt.init(),C.isDiscuessions()?(u.info("Router: 讨论页面"),ve.init()):C.isUsers()?(u.info("Router: 用户页面"),be.init(),C.isUsersConversations()&&(u.info("Router-next: 私聊用户页面"),Pe.init())):(C.isScriptSearch()||C.isScriptsBySite()||C.isScriptCodeSearch()||C.isScriptLibraryListSearch())&&(u.info("Router: 脚本搜索页面"),Le.init()),v.execMenuOnce("scripts-addOperationPanelBtnWithNavigator",()=>{this.addOperationPanelBtnWithNavigator();}),d.ready(()=>{S.initEnv(),pe.init(),mt.init(),S.handleLocalGotoCallBack(),v.execMenuOnce("fixImageWidth",()=>{ot.fixImageWidth();}),ot.languageSelectorLocale(),v.execMenuOnce("optimizeImageBrowsing",()=>{ot.optimizeImageBrowsing();}),v.execMenuOnce("overlayBedImageClickEvent",()=>{ot.overlayBedImageClickEvent();}),C.isCodeStrict()||v.execMenuOnce("addMarkdownCopyButton",()=>{ot.addMarkdownCopyButton();});});},fixImageWidth(){window.innerWidth<window.innerHeight&&(u.info("修复图片显示问题"),P(`
            img.lum-img{
                width: 100% !important;
                height: 100% !important;
            }
          `));},optimizeImageBrowsing(){u.info("优化图片浏览"),P(ht("ViewerCSS")),P(`
        ...
        `);function t(A=[],n=0){let i="";A.forEach(s=>{i+=`<li><img data-src="${s}" loading="lazy"></li>`;});let a=d.createElement("ul",{innerHTML:i}),o=new Qt(a,{inline:false,url:"data-src",zIndex:g.getMaxZIndex()+100,hidden:()=>{o.destroy();}});n=n<0?0:n,o.view(n),o.zoomTo(1),o.show();}function e(A){return A.getAttribute("data-src")||A.getAttribute("src")||A.getAttribute("alt")}d.on(document,"click","img",function(A){var c;let n=A.target;if(((c=n.parentElement)==null?void 0:c.localName)==="a"&&n.hasAttribute("data-screenshots")||n.closest(".viewer-container")||n.closest(".lum-lightbox-position-helper"))return;let i=n.closest(".user-content"),a=[],o=0,s=[],l=e(n);if(l){if(l.startsWith("https://img.shields.io"))return;if(l.startsWith("/vite/assets/"))return}i?(i.querySelectorAll("img").forEach(p=>{s.push(p);let f=e(p),m=p.parentElement;(m==null?void 0:m.localName)==="a"&&(f=m.getAttribute("data-href")||m.href),a.push(f);}),o=s.indexOf(n),o===-1&&(o=0)):(a.push(l),o=0),u.success("点击浏览图片👉",a,o),t(a,o);}),E(".user-screenshots").forEach(A=>{let n=A.querySelector("a");if(!n)return;let i=n.getAttribute("data-href")||n.getAttribute("href"),a=A.querySelector("img");a&&(a.setAttribute("data-screenshots","true"),a.setAttribute("data-src",i),n.setAttribute("href","javascript:;"),d.after(n,a),n.remove());});},overlayBedImageClickEvent(){u.info("覆盖图床图片的parentElement的a标签"),E(".user-content a>img").forEach(t=>{let e=t.parentElement,A=e.getAttribute("href");if(e.setAttribute("data-href",A),e.removeAttribute("href"),A.startsWith("/rails/active_storage/blobs/redirect")){u.info("该图片是上传到Greasyfork的图片，拦截默认行为，不做提示");return}d.on(e,"click",()=>{h.warning(`<div style="overflow-wrap: anywhere;">${r.t("拦截跳转：")}<a href="${A}" target="_blank">${A}</a></div>`,{html:true,zIndex:g.getMaxZIndex()+105});});});},addMarkdownCopyButton(){u.info("在Markdown右上角添加复制按钮"),P(`
        pre{
          position: relative;
        ...
        `);function t(){let e=d.createElement("div",{className:"zeroclipboard-container",innerHTML:`
            ...
            `}),A=e.querySelector(".js-clipboard-copy"),n=e.querySelector(".octicon-copy"),i=e.querySelector(".octicon-check-copy");return d.on(e,"click",()=>{let a=d.parent(e),o=a.querySelector("code");if(!o&&a.className.includes("prettyprinted")&&(o=e.parentElement),o||(o=a.querySelector("pre")),!o){h.error(r.t("未找到{{selector}}元素",{selector:"code"}));return}g.setClip(d.text(o)),A.setAttribute("success","true"),n.setAttribute("aria-hidden","true"),i.removeAttribute("aria-hidden");let s=D.tooltip({target:A,content:r.t("✅ 复制成功!"),position:"left",className:"github-tooltip",alwaysShow:true});s.toolTip.onToolTipAnimationFinishEvent(),setTimeout(()=>{A.removeAttribute("success"),i.setAttribute("aria-hidden","true"),n.removeAttribute("aria-hidden"),s.toolTip.close();},2e3);}),e}E("pre").forEach(e=>{if(e.querySelector("div.zeroclipboard-container"))return;let n=t(),i=d.createElement("div",{className:"snippet-clipboard-content"});d.before(e,i),i.appendChild(e),i.appendChild(n);});},languageSelectorLocale(){let t=v.getValue("language-selector-locale"),e=window.location.pathname.split("/").filter(A=>!!A)[0];if(u.success("选择语言："+t),u.success("当前语言："+e),!g.isNull(t)&&t!==e){let A=null,n=L.getSwitchLanguageUrl(t);j.switchLanguage(n),u.success("新Url："+n),h.loading(r.t("当前语言：{{currentLocaleLanguage}}，，3秒后切换至：{{localeLanguage}}",{currentLocaleLanguage:e,localeLanguage:t}),{timeout:3e3,showClose:true,onClose(){clearTimeout(A);}}),h.info(r.t("导航至：")+n,{timeout:3e3}),A=setTimeout(()=>{window.location.href=n;},3e3);}},checkPage(){u.info("检测gf页面是否正确加载，有时候会出现"),d.ready(()=>{if(document.body.firstElementChild&&document.body.firstElementChild.localName==="p"&&document.body.firstElementChild.innerText.includes("We're down for maintenance. Check back again soon.")){let t=parseInt(R("greasyfork-check-page-time",0)),e=v.getValue("greasyfork-check-page-timeout",5),A=e*1e3;if(t&&Date.now()-t<A){h.warning(r.t("上次重载时间 {{time}}，{{timeout}}秒内拒绝反复重载",{time:g.formatTime(t,"yyyy-MM-dd HH:mm:ss"),timeout:e}));return}V("greasyfork-check-page-time",Date.now()),window.location.reload();}});},addOperationPanelBtnWithNavigator(){u.info("添加【操作面板】按钮"),tt.addBlockCSS(".sidebarred .sidebar",".sidebarred-main-content .open-sidebar"),P(`
		.sidebarred .sidebarred-main-content{
			max-width: 100%;
		}	
		`),d.ready(()=>{z("#site-nav nav"),z("#site-nav .with-submenu nav");let t=z("#script-list-option-groups")||z(".list-option-groups");if(!t){u.warn("不存在右侧面板元素#script-list-option-groups");return}t=t.cloneNode(true),t.classList.add("option-panel-groups"),et.registerTopNavMenu({name:r.t("操作面板"),className:"filter-scripts",clickEvent(e){D.alert({title:{text:r.t("操作面板"),position:"center"},content:{text:"",html:true},btn:{ok:{enable:false}},mask:{enable:true,clickEvent:{toClose:true}},drag:true,useShadowRoot:true,width:$.setting.width,height:$.setting.height,zIndex:g.getMaxZIndex(100),style:`
						...
						`}).$shadowRoot.querySelector(".pops-alert-content").appendChild(t);}});});}};v.init();ot.init();
