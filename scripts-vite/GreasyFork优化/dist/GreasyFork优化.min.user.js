...
`)}{display: none !important;}`)},setGMResourceCSS(e){let t=typeof ae=="function"?ae(e.keyName):"";typeof t=="string"&&t?ce(t):X.addLinkNode(e.url);},async addLinkNode(e){let t=document.createElement("link");return t.rel="stylesheet",t.type="text/css",t.href=e,d.ready(()=>{document.head.appendChild(t);}),t},fixUrl(e){return e=e.trim(),e.match(/^http(s|):\/\//i)||(e.startsWith("/")||(e+="/"),e=window.location.origin+e),e}},se={getInstallUrl(e,t,r){return h.isNotNull(r)?r="/"+r:r="",`https://update.greasyfork.org/scripts/${e}/${t}${r}.user.js`},getCodeUrl(e,t){return h.isNull(t)&&(t=""),`https://greasyfork.org/scripts/${e}/code?version=${t}`},getScriptInfoUrl(e){return `https://greasyfork.org/scripts/${e}.json`}},Ue={init(){p.execMenuOnce("beautifyHistoryVersionPage",()=>this.beautifyHistoryVersionPage()),p.execMenuOnce("scripts-versions-addExtraTagButton",()=>{this.addExtraTagButton();});},beautifyHistoryVersionPage(){c.info("美化 历史版本 页面");let e=[];return e.push(w(Ae)),e.push(X.addBlockCSS(".version-number",".version-date",".version-changelog")),d.ready(function(){let t=document.querySelector("ul.history_versions");if(!t){f.error(n.t("未找到history_versions元素列表"));return}Array.from(t.children).forEach(r=>{var g,y;let i=r.querySelector(".version-number a").href,o=r.querySelector(".version-number a").innerText,a=(g=r.querySelector(".version-date"))==null?void 0:g.getAttribute("datetime"),s=((y=r.querySelector(".version-changelog"))==null?void 0:y.innerHTML)||"",l=d.createElement("span",{className:"script-version-date",innerHTML:h.formatTime(a,n.t("yyyy年MM月dd日 HH:mm:ss"))}),m=d.createElement("div",{className:"script-tag",innerHTML:`
                    <div class="script-tag-version">
                    ...
                    </div>`}),u=d.createElement("div",{className:"script-note-box-body",innerHTML:s});r.appendChild(l),r.appendChild(m),r.appendChild(u);});}),e},addExtraTagButton(){c.info("添加额外的标签按钮"),d.ready(()=>{document.querySelectorAll(".script-tag-version").forEach(e=>{var u,g;let t=e.querySelector("a");if(!t)return;let r=new URL(t.href),i=(u=r.pathname.match(/\/scripts\/([\d]+)/))==null?void 0:u[1],o=r.searchParams.get("version"),a=(g=r.pathname.match(/\/scripts\/[\d]+-(.+)/))==null?void 0:g[1],s=se.getInstallUrl(i,o,a),l=se.getCodeUrl(i,o),m=d.createElement("div",{className:"scripts-tag-install",innerHTML:`
						...
						`});d.after(e,m);});});}};let te=[];const he=async function(e){if(c.info("当前脚本id："+e),!v.isLogin){c.error("请先登录账号"),f.error(n.t("请先登录账号"));return}let t=b.getUserId(v.getUserLinkElement().href);if(t==null){c.error("获取用户id失败"),f.error(n.t("获取用户id失败"));return}if(!te.length){let o=f.loading(n.t("获取收藏夹中..."));if(te=await b.getUserCollection(t)||[],o.close(),!te.length)return}let r="";te.forEach(o=>{r+=`
		<li class="user-collect-item" data-id="${o.id}" data-name="${o.name}">
		  ...
		  `;});let i=$.alert({title:{text:n.t("收藏集"),position:"center"},content:{html:!0,text:`<ul>${r}</ul>`},mask:{enable:!0,clickEvent:{toClose:!0}},btn:{ok:{enable:!1}},width:$.isPhone()?"92dvw":"500px",height:"auto",drag:!0,only:!0,style:`
		.pops{
		...
		`});d.on(i.$shadowRoot,"click",".collect-add-script-id",async function(o){let a=o.target.closest(".user-collect-item"),s=a.dataset.id;a.dataset.name;let l=f.loading(n.t("添加中...")),m=await b.getUserCollectionInfo(t,s);if(!m){l.close();return}let u=h.cloneFormData(m),g=h.cloneFormData(m),y=!1;for(const[E,S]of m.entries())if(E==="scripts-included[]"&&String(S).trim()===String(e).trim()){y=!0;break}else g.append(E,S),u.append(E,S);if(y){f.warning(n.t("该脚本已经在该收藏集中")),l.close();return}u.set("add-script",e.toString()),u.set("script-action","i"),g.append("scripts-included[]",e.toString()),g.set("save","1");let M=new URLSearchParams(u),k=new URLSearchParams(g),L=Array.from(M).map(([E,S])=>`${encodeURIComponent(E)}=${encodeURIComponent(S)}`).join("&"),q=Array.from(k).map(([E,S])=>`${encodeURIComponent(E)}=${encodeURIComponent(S)}`).join("&");c.info(["添加的数据",L]),c.info(["保存的数据",q]);let V=await b.updateUserSetsInfo(t,s,L);if(!V){l.close();return}let C=V.querySelector(".change-script-set");if(!C){f.error(n.t("添加失败，{{selector}}元素不存在",{selector:".change-script-set"})),l.close();return}let T=C.querySelector("section");if(!T){f.error(n.t("添加失败，{{selector}}元素不存在",{selector:"section"})),l.close();return}let B=T.querySelector(".alert");B?$.alert({title:{text:n.t("添加失败"),position:"center"},content:{text:B.innerHTML,html:!0},mask:{enable:!0,clickEvent:{toClose:!0}},style:`
					.pops-alert-content{
						font-style: italic;
					...
					`,drag:!0,dragLimit:!0,width:$.isPhone()?"88vw":"400px",height:$.isPhone()?"50vh":"300px"}):(await b.updateUserSetsInfo(t,s,q),f.success(n.t("添加成功"))),l.close();}),d.on(i.$shadowRoot,"click",".collect-delete-script-id",async function(o){let a=o.target.closest(".user-collect-item"),s=a.dataset.id;a.dataset.name;let l=f.loading(n.t("删除中...")),m=await b.getUserCollectionInfo(t,s);if(!m){l.close();return}let u=new FormData,g=new FormData,y=!1;for(const[C,T]of m.entries()){if(String(C).trim()==="scripts-included[]"&&String(T).trim()===String(e).trim()){y=!0;continue}g.append(C,T),u.append(C,T);}if(!y){f.warning(n.t("该收藏集未包含：{{scriptId}}",{scriptId:e})),l.close();return}u.set("remove-scripts-included[]",e.toString()),u.set("remove-selected-scripts","i"),u.delete("script-action"),g.set("save","1");let M=new URLSearchParams(u),k=new URLSearchParams(g),L=Array.from(M).map(([C,T])=>`${encodeURIComponent(C)}=${encodeURIComponent(T)}`).join("&"),q=Array.from(k).map(([C,T])=>`${encodeURIComponent(C)}=${encodeURIComponent(T)}`).join("&");if(c.info(["删除的数据",L]),c.info(["保存的数据",q]),!await b.updateUserSetsInfo(t,s,L)){l.close();return}await b.updateUserSetsInfo(t,s,q),f.success(n.t("删除成功")),l.close();});},_e={init(){A.isCode()?Ie.init():A.isVersion()&&Ue.init(),A.isCodeStrict()&&(p.execMenuOnce("fullScreenOptimization",()=>{this.fullScreenOptimization();}),p.execMenuOnce("addCopyCodeButton",()=>{this.addCopyCodeButton();})),p.execMenuOnce("addCollectionButton",()=>{this.addCollectionButton();}),p.execMenuOnce("addFindReferenceButton",()=>{this.setFindCodeSearchBtn();}),p.execMenuOnce("scriptHomepageAddedTodaySUpdate",()=>{this.scriptHomepageAddedTodaySUpdate();});},addCollectionButton(){c.info("添加收藏按钮"),h.waitNode("ul#script-links li.current span").then(()=>{let e=d.createElement("li",{innerHTML:`
					<a href="javascript:;">
						<span>${n.t("收藏")}</span>
					</a>`});d.append(document.querySelector("ul#script-links"),e),d.on(e,"click",()=>{let t=window.location.pathname.match(/scripts\/([\d]+)/i);if(!t){c.error([t,window.location.pathname]),f.error(n.t("获取脚本id失败"));return}let r=t[t.length-1];he(r);});});},fullScreenOptimization(){c.info("F11全屏，F键代码全屏"),w(`
        .code-wide-screen{
          position: absolute;
          top: 0;
          left: 0;
        ...
        `);let e=!1;d.keydown(I,function(t){if(t.key.toLowerCase()==="f"){let r=document.querySelector("#script-content div.code-container code");t.altKey&&t.shiftKey?(h.preventEvent(t),r.classList.contains("code-wide-screen")?r.classList.remove("code-wide-screen"):r.classList.add("code-wide-screen")):!t.altKey&&!t.ctrlKey&&!t.shiftKey&&!t.metaKey&&(h.preventEvent(t),e?(h.exitFullScreen(r),e=!1):(h.enterFullScreen(r),e=!0));}},{capture:!0});},setFindCodeSearchBtn(){c.info("设置代码搜索按钮(对于库)"),h.waitNode("ul#script-links li.current span").then(()=>{let e=d.createElement("li",{innerHTML:`
					<a href="javascript:;">
						<span>${n.t("寻找引用")}</span>
					</a>`});d.append(document.querySelector("ul#script-links"),e),d.on(e,"click",async function(){let t=window.location.pathname.match(/scripts\/([\d]+)/i);if(!t){c.error([t,window.location.pathname]),f.error(n.t("获取脚本id失败"));return}let r=t[t.length-1];window.location.href=b.getCodeSearchUrl(`greasyfork.org/scripts/${r}`);});});},async scriptHomepageAddedTodaySUpdate(){if(!document.querySelector("#install-area"))return;c.info("脚本首页新增【今日检查】");let e=await b.getScriptStats(b.getScriptId());if(!e)return;let t=h.toJSON(e.responseText);c.info(["统计信息",t]);let r=t[h.formatTime(void 0,"yyyy-MM-dd")];if(!r){c.error("今日份的统计信息不存在");return}let i=r.update_checks;c.info(["今日统计信息",r]),d.after("dd.script-show-daily-installs",d.createElement("dt",{className:"script-show-daily-update_checks",innerHTML:`<span>${n.t("今日检查")}</span>`})),d.after("dt.script-show-daily-update_checks",d.createElement("dd",{className:"script-show-daily-update_checks",innerHTML:"<span>"+i+"</span>"}));},addCopyCodeButton(){c.info("添加复制代码按钮"),h.waitNode("div#script-content div.code-container").then(e=>{let t=d.createElement("button",{textContent:n.t("复制代码")},{style:"margin-bottom: 1em;"});d.on(t,"click",async function(){let r=f.loading(n.t("加载文件中...")),i=await _.get(`https://greasyfork.org/zh-CN/scripts/${b.getScriptId()}.json`,{fetch:!0,responseType:"json"});if(!i.status){r.close();return}let a=h.toJSON(i.data.responseText).code_url;c.success(["代码地址：",a]);let s=await _.get(a);if(!s.status){r.close();return}r.close(),h.setClip(s.data.responseText),f.success(n.t("复制成功"));}),d.before(e,t);});}},$e=`.sidebarred-main-content {\r
	max-width: unset;\r
	flex: unset;\r
}\r
ol.script-list {\r
...
`,ie={getTampermonkey:()=>{var e;return (e=I.external)==null?void 0:e.Tampermonkey},getViolentmonkey:()=>{var e;return (e=I.external)==null?void 0:e.Violentmonkey},getScriptCat:()=>{var e;return (e=I.external)==null?void 0:e.Scriptcat},getScriptContainerStatus(){var t,r,i;let e={Tampermonkey:!1,Violentmonkey:!1,ScriptCat:!1};return (t=I.external)!=null&&t.Tampermonkey&&(e.Tampermonkey=!0),(r=I.external)!=null&&r.Violentmonkey&&(e.Violentmonkey=!0),(i=I.external)!=null&&i.Scriptcat&&(e.ScriptCat=!0),e},getInstalledVersion(e,t){return new Promise((r,i)=>{const o=this.getTampermonkey();if(o){o.isInstalled(e,t,function(l){l.installed?r(l.version):r(null);});return}const a=this.getViolentmonkey();if(a){a.isInstalled(e,t).then(r);return}const s=this.getScriptCat();if(s){s.isInstalled(e,t,function(l){l.installed?r(l.version):r(null);});return}i(new TypeError("获取脚本容器暴露的external信息失败"));})},compareVersions(e,t){if(e===t)return 0;const r=e.split("."),i=t.split(".");for(let o=0;o<r.length;o++){const a=this.compareVersionPart(r[o],i[o]);if(a!==0)return a}return 0},compareVersionPart(e,t){const r=this.parseVersionPart(e),i=this.parseVersionPart(t);for(let o=0;o<r.length;o++){if(r[o].length>0&&i[o].length===0)return -1;if(r[o].length===0&&i[o].length>0||r[o]>i[o])return 1;if(r[o]<i[o])return -1}return 0},parseVersionPart(e){if(!e)return [0,"",0,""];const t=/([0-9]*)([^0-9]*)([0-9]*)([^0-9]*)/.exec(e);return [t[1]?parseInt(t[1]):0,t[2],t[3]?parseInt(t[3]):0,t[4]]},handleInstallResult(e,t,r){if(t!=null)switch(e.removeAttribute("data-ping-url"),this.compareVersions(t,r)){case-1:e.textContent=e.getAttribute("data-update-label");break;case 1:e.textContent=e.getAttribute("data-downgrade-label");break;case 0:e.textContent=e.getAttribute("data-reinstall-label");break}},async checkForUpdatesJS(e,t){const r=e.getAttribute("data-script-name"),i=e.getAttribute("data-script-namespace"),o=e.getAttribute("data-script-version");try{let a=await this.getInstalledVersion(r,i);return a==null?!1:(this.handleInstallResult(e,a,o),!0)}catch{if(t){await h.sleep(1e3);try{return await this.checkForUpdatesJS(e,!1)}catch{}}return !1}},checkForUpdatesCSS(e){const t=e.getAttribute("data-script-name"),r=e.getAttribute("data-script-namespace");postMessage({type:"style-version-query",name:t,namespace:r,url:location.href},location.origin);}},ge=e=>{let t=e.dataset;const r={scriptId:parseInt(t.scriptId),scriptName:t.scriptName,scriptAuthors:[],scriptDailyInstalls:parseInt(t.scriptDailyInstalls),scriptTotalInstalls:parseInt(t.scriptTotalInstalls),scriptRatingScore:parseFloat(t.scriptRatingScore),scriptCreatedDate:new Date(t.scriptCreatedDate),scriptUpdatedDate:new Date(t.scriptUpdatedDate),scriptType:t.scriptType,scriptVersion:t.scriptVersion,sensitive:t.sensitive==="true",scriptLanguage:t.scriptLanguage,cssAvailableAsJs:t.cssAvailableAsJs==="true",codeUrl:t.codeUrl,scriptDescription:t.scriptDescription,scriptAuthorId:parseInt(t.scriptAuthorId),scriptAuthorName:t.scriptAuthorName};let i=h.toJSON(t.scriptAuthors);if(Object.keys(i).forEach(o=>{let a=i[o];r.scriptAuthors.push({authorId:parseInt(o),authorName:a});}),(r.scriptAuthorName==null||isNaN(r.scriptAuthorId))&&r.scriptAuthors.length&&(r.scriptAuthorName=r.scriptAuthors[0].authorName,r.scriptAuthorId=r.scriptAuthors[0].authorId),r.scriptDescription==null){let o=e.querySelector(".script-description")||e.querySelector(".description");o&&(r.scriptDescription=o.innerText||o.textContent);}return r},ye={init(){p.execMenuOnce("gf-scripts-filter-enable",()=>{O.init();}),p.execMenuOnce("beautifyCenterContent",()=>this.beautifyCenterContent());},beautifyCenterContent(){c.info("美化脚本列表");let e=[];return e.push(w($e)),U.ready(async()=>{O.getScriptElementList().forEach(a=>{if(a.querySelector(".script-list-operation"))return;let s=ge(a),l=a.querySelector(".inline-script-stats"),m=s.codeUrl,u=U.createElement("dt",{className:"script-list-rating-score",innerHTML:`<span>${n.t("评分")}</span>`}),g=U.createElement("dd",{className:"script-list-rating-score",innerHTML:`<span>${s.scriptRatingScore}</span>`},{"data-position":"right"}),y=a.querySelector("dd.script-list-ratings .good-rating-count"),M=a.querySelector("dd.script-list-ratings .ok-rating-count"),k=a.querySelector("dd.script-list-ratings .bad-rating-count");if(y&&M&&k){let E=parseInt(y.innerText),S=parseInt(M.innerText),z=parseInt(k.innerText),R=E+S+z;R>=10?E/R>=.6?g.classList.add("good-rating-count"):g.classList.add("bad-rating-count"):R==0||E>S+z?g.classList.add("good-rating-count"):g.classList.add("bad-rating-count");}let L=U.createElement("dt",{className:"script-list-version",innerHTML:`<span>${n.t("版本")}</span>`}),q=U.createElement("dd",{className:"script-list-version",innerHTML:`<span>${s.scriptVersion}</span>`},{"data-position":"right"}),V=U.createElement("dt",{className:"script-list-operation",innerHTML:`<span>${n.t("操作")}</span>`}),C=U.createElement("dd",{className:"script-list-operation",innerHTML:`
						...
						`},{"data-position":"right",style:"gap:10px;display: flex;flex-wrap: wrap;align-items: center;"}),T=C.querySelector(".script-collect-btn"),B=C.querySelector(".install-link");if(B["data-script-info"]=s,B.classList.add("lum-lightbox-loader"),s.scriptType==="library"&&B.remove(),U.on(T,"click",E=>{h.preventEvent(E),he(s.scriptId);}),p.getValue("gf-scripts-filter-enable")){let E=U.createElement("button",{className:"script-filter-btn",innerHTML:n.t("过滤")}),S="data-filter-key",z="data-filter-value";U.on(E,"click",R=>{h.preventEvent(R);let D=$.alert({title:{text:n.t("选择过滤的选项"),position:"center"},content:{text:`
									<button ${S}="scriptId" ${z}="^${s.scriptId}$">${n.t("脚本id：{{text}}",{text:s.scriptId})}</button>
									<button ${S}="scriptName" ${z}="^${h.parseStringToRegExpString(s.scriptName)}$">${n.t("脚本名：{{text}}",{text:s.scriptName})}</button>
								...
								`}),K=D.$shadowRoot.querySelector(".pops-alert-content");s.scriptAuthors.forEach(F=>{let j=U.createElement("button",{innerHTML:n.t("作者id：{{text}}",{text:F.authorId})});j.setAttribute(S,"scriptAuthorId"),j.setAttribute(z,"^"+F.authorId+"$");let Y=U.createElement("button",{innerHTML:n.t("作者名：{{text}}",{text:F.authorName})});Y.setAttribute(S,"scriptAuthorName"),Y.setAttribute(z,"^"+h.parseStringToRegExpString(F.authorName)+"$"),K.appendChild(j),K.appendChild(Y);}),U.on(D.$shadowRoot,"click","button",F=>{h.preventEvent(F);let j=F.target,Y=j.getAttribute(S),be=j.getAttribute(z);O.addValue(`${Y}##${be}`),D.close(),O.filter(),f.success(n.t("添加成功"));});}),C.appendChild(E);}l.appendChild(u),l.appendChild(g),l.appendChild(L),l.appendChild(q),l.appendChild(V),l.appendChild(C);});let r=Array.from(document.querySelectorAll(".install-link[data-install-format=js]")),i=ie.getScriptContainerStatus(),o=Object.values(i).filter(Boolean);if(!i.Tampermonkey)r.forEach(async a=>{let s=await ie.checkForUpdatesJS(a,!0);a.classList.remove("lum-lightbox-loader"),s||(a.textContent=n.t("安装此脚本"));});else if(o.length)for(let a=0;a<r.length;a++){let s=r[a],l=s["data-script-info"],m=await _.get(se.getScriptInfoUrl(l.scriptId),{fetch:!0});if(!m.status){s.textContent=n.t("安装此脚本");continue}let u=h.toJSON(m.data.responseText);s.setAttribute("data-script-namespace",u.namespace);let g=await ie.checkForUpdatesJS(s,!0);s.classList.remove("lum-lightbox-loader"),g||(s.textContent=n.t("安装此脚本")),await h.sleep(150);}else c.error("未知的脚本容器");}),e}},O={key:"gf-shield-rule",init(){c.info("脚本过滤");let e=new h.LockFunction(()=>{this.filter();},50);d.ready(()=>{h.mutationObserver(document.body,{config:{subtree:!0,childList:!0},callback:()=>{e.run();}}),e.run();});},getScriptElementList(){let e=[];return e=e.concat(Array.from(document.querySelectorAll("ol.script-list li"))),e},filter(){this.getScriptElementList().forEach(e=>{let t=ge(e),r=this.getValue().split(`
`);for(let i=0;i<r.length;i++){let o=r[i],a=o.split("##"),s=a[0],l=a[1];if(s==="scriptRatingScore"){let m=parseFloat(l.slice(1));if(l.startsWith(">")){if(t.scriptRatingScore>m){c.info(["触发过滤规则",[o,t]]),e.remove();break}}else if(l.startsWith("<")&&t.scriptRatingScore<m){c.info(["触发过滤规则",[o,t]]),e.remove();break}}else if(s in t||s==="scriptDescription"){if(typeof l!="string")continue;let m=new RegExp(l,"ig");if(String(t[s]).match(m)){c.info(["触发过滤规则",o,t]),e.remove();break}}}});},setValue(e){p.setValue(this.key,e);},addValue(e){let t=this.getValue();return t.trim()!==""&&(t+=`
`),t+=e,this.setValue(t)},getValue(){return p.getValue(this.key,"")}},Ne={id:"greasy-fork-panel-config-scripts",title:n.t("脚本"),forms:[{text:"",type:"forms",forms:[{text:n.t("代码"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[x(n.t("添加复制代码按钮"),"addCopyCodeButton",!0,void 0,n.t("更优雅的复制")),x(n.t("快捷键"),"fullScreenOptimization",!0,void 0,n.t("【F】键全屏、【Alt+Shift+F】键宽屏")),x(n.t("修复代码行号显示"),"code-repairCodeLineNumber",!0,void 0,n.t("修复代码行数超过1k行号显示不全问题"))]}]},{text:n.t("历史版本"),type:"deepMenu",forms:[{text:n.t("功能"),type:"forms",forms:[x(n.t("添加额外的标签按钮"),"scripts-versions-addExtraTagButton",!0,void 0,n.t("在版本下面添加【安装】、【查看代码】按钮"))]},{text:n.t("美化"),type:"forms",forms:[x(n.t("美化历史版本页面"),"beautifyHistoryVersionPage",!0,void 0,n.t("更直观的查看版本迭代"))]}]}]},{text:"",type:"forms",forms:[{text:n.t("功能"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[x(n.t("添加【寻找引用】按钮"),"addFindReferenceButton",!0,void 0,n.t("在脚本栏添加按钮，一般用于搜索引用该库的相关脚本")),x(n.t("添加【收藏】按钮"),"addCollectionButton",!0,void 0,n.t("在脚本栏添加按钮，一般用于快捷收藏该脚本/库")),x(n.t("添加【今日检查】信息块"),"scriptHomepageAddedTodaySUpdate",!0,void 0,n.t("在脚本信息栏添加【今日检查】信息块"))]}]},{text:n.t("美化"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[x(n.t("美化脚本列表"),"beautifyCenterContent",!0,void 0,n.t("双列显示且添加脚本卡片操作项（安装、收藏）"))]}]},{text:n.t("过滤"),type:"deepMenu",forms:[{text:`<a target="_blank" href="https://greasyfork.org/zh-CN/scripts/475722-greasyfork%E4%BC%98%E5%8C%96#:~:text=%E5%B1%8F%E8%94%BD%E8%A7%84%E5%88%99">${n.t("帮助文档")}</a>`,type:"forms",forms:[x(n.t("启用"),"gf-scripts-filter-enable",!0,void 0,n.t("作用域：脚本、脚本搜索、用户主页")),{type:"own",getLiElementCallBack(e){let t=d.createElement("div",{className:"pops-panel-textarea",innerHTML:`
												<textarea placeholder="${n.t("请输入规则，每行一个")}" style="height:150px;"></textarea>`},{style:"width: 100%;"}),r=t.querySelector("textarea");return r.value=p.getValue(O.key,""),d.on(r,["input","propertychange"],void 0,h.debounce(function(i){p.setValue(O.key,r.value);},200)),e.appendChild(t),e}}]}]}]}]},Ve={id:"greasy-fork-panel-config-discussions",title:n.t("论坛"),forms:[{text:"",type:"forms",forms:[{text:n.t("功能"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[{type:"own",attributes:{"data-key":"discussions-readBgColor","data-default-value":"#e5e5e5"},getLiElementCallBack(e){let t="discussions-readBgColor",r=d.createElement("div",{className:"pops-panel-item-left-text",innerHTML:`
											...
											`}),o=i.querySelector(".pops-color-choose");o.value=p.getValue(t);let a=d.createElement("style");return d.append(document.head,a),d.on(o,["input","propertychange"],s=>{c.info("选择颜色："+o.value),a.innerHTML=`
												...
												`,p.setValue(t,o.value);}),e.appendChild(r),e.appendChild(i),e}},x(n.t("添加快捷操作按钮"),"discussions-addShortcutOperationButton",!0,void 0,n.t("在每一行讨论的最后面添加【过滤】按钮，需开启过滤功能才会生效"))]}]}]},{text:"",type:"forms",forms:[{text:n.t("过滤"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[x(n.t("启用"),"greasyfork-discussions-filter-enable",!0,void 0,n.t("开启后下面的过滤功能才会生效")),x(n.t("过滤重复的评论"),"greasyfork-discussions-filter-duplicate-comments",!1,void 0,n.t("过滤掉重复的评论数量(≥2)"))]},{text:"",type:"forms",forms:[{text:n.t("过滤脚本(id)"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[{type:"own",getLiElementCallBack(e){let t=d.createElement("div",{className:"pops-panel-textarea",innerHTML:`
														<textarea placeholder="${n.t("请输入脚本id，每行一个")}" style="height:150px;"></textarea>`},{style:"width: 100%;"}),r=t.querySelector("textarea");const i="greasyfork-discussions-filter-script";return r.value=p.getValue(i,""),d.on(r,["input","propertychange"],void 0,h.debounce(function(o){p.setValue(i,r.value);},200)),e.appendChild(t),e}}]}]},{text:n.t("过滤发布的用户(id)"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[{type:"own",getLiElementCallBack(e){let t=d.createElement("div",{className:"pops-panel-textarea",innerHTML:`
														<textarea placeholder="${n.t("请输入用户id，每行一个")}" style="height:150px;"></textarea>`},{style:"width: 100%;"}),r=t.querySelector("textarea");const i="greasyfork-discussions-filter-post-user";return r.value=p.getValue(i,""),d.on(r,["input","propertychange"],void 0,h.debounce(function(o){p.setValue(i,r.value);},200)),e.appendChild(t),e}}]}]},{text:n.t("过滤回复的用户(id)"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[{type:"own",getLiElementCallBack(e){let t=d.createElement("div",{className:"pops-panel-textarea",innerHTML:`
												<textarea placeholder="${n.t("请输入用户id，每行一个")}" style="height:150px;"></textarea>`},{style:"width: 100%;"}),r=t.querySelector("textarea");const i="greasyfork-discussions-filter-reply-user";return r.value=p.getValue(i,""),d.on(r,["input","propertychange"],void 0,h.debounce(function(o){p.setValue(i,r.value);},200)),e.appendChild(t),e}}]}]}]}]}]}]},ze=`code {\r
...
`,He={init(){p.execMenuOnce("beautifyPage",()=>this.beautifyPageElement()),p.execMenuOnce("beautifyGreasyforkBeautify",()=>this.beautifyGreasyforkBeautify()),p.execMenuOnce("beautifyUploadImage",()=>this.beautifyUploadImage()),p.execMenuOnce("beautifyTopNavigationBar",()=>this.beautifyTopNavigationBar());},beautifyPageElement(){c.info("美化页面元素");let e=[];return e.push(w(ze)),e.push(w(qe)),e.push(w(Be)),e.push(w(Re)),e.push(w(`
			p:has(input[type="submit"][name="update-and-sync"]){
			  margin-top: 10px;
			}
			`)),d.ready(function(){let t=document.querySelector('a[target="markup_choice"][href*="daringfireball.net"]');t&&t.parentElement.replaceChild(d.createElement("span",{textContent:"Markdown"}),t),globalThis.location.pathname.endsWith("/admin")&&!document.querySelector('input[type="submit"][name="update-only"]')&&e.push(w(`
					.indented{
						padding-left: unset;
					}
					`));}),e},beautifyGreasyforkBeautify(){c.info("美化 Greasyfork Beautify脚本");let e=[];return e.push(w(Pe)),h.isPhone()?e.push(w(`
				section#script-info,
				...
				section#script-info{
					margin-top: 10px;
				}`)),e},beautifyUploadImage(){c.info("美化上传图片");let e=[];return e.push(w(De)),d.ready(()=>{function t(o){for(;o.nextElementSibling;)o.parentElement.removeChild(o.nextElementSibling);}let r=document.querySelectorAll('input[type="file"]');r.forEach(o=>{o.getAttribute("name")!=="code_upload"&&(o.hasAttribute("accept")&&o.getAttribute("accept").includes("javascript")||d.on(o,["propertychange","input"],function(a){t(a.target);let s=a.currentTarget.files;if(!s||s.length===0)return;c.info(["选择的图片",s]),s.length>5&&d.after(o,d.createElement("p",{textContent:n.t("❌ 最多同时长传5张图片")}));let l=[];Array.from(s).forEach(m=>{(m.size>204800||!m.type.match(/png|jpg|jpeg|gif|apng|webp/i))&&l.push(m);}),l.length!==0&&l.forEach(m=>{d.after(o,d.createElement("p",{textContent:n.t("❌ 图片：{{name}} 大小：{{size}}",{name:m.name,size:m.size})}));});}));}),["textarea#comment_text","textarea.comment-entry"].forEach(o=>{d.on(o,"paste",a=>{c.info(["触发粘贴事件",a]),setTimeout(()=>{d.trigger(r,"input");},100);});});}),e},beautifyTopNavigationBar(){c.info("美化顶部导航栏");let e=[];return e.push(w(Fe)),window.outerWidth>550&&(e.push(X.addBlockCSS(".with-submenu")),d.ready(()=>{let r=document.querySelector("#site-nav").querySelector("nav");document.querySelectorAll(".with-submenu nav li").forEach(i=>{r.appendChild(i);});})),e}},Ge={init(){p.execMenu("autoLogin",()=>{this.autoLogin();});},autoLogin(){h.waitNode("span.sign-in-link a[rel=nofollow]").then(async()=>{let e=p.getValue("user"),t=p.getValue("pwd");if(h.isNull(e)){f.error(n.t("请先在菜单中录入账号"));return}if(h.isNull(t)){f.error(n.t("请先在菜单中录入密码"));return}let r=document.querySelector("meta[name='csrf-token']");if(!r){f.error(n.t("获取csrf-token失败"));return}let i=f.loading(n.t("正在登录中...")),o=await _.post("https://greasyfork.org/zh-CN/users/sign_in",{fetch:!0,data:encodeURI(`authenticity_token=${r.getAttribute("content")}&user[email]=${e}&user[password]=${t}&user[remember_me]=1&commit=登录`),headers:{"Content-Type":"application/x-www-form-urlencoded"}});if(i.destroy(),!o.status){c.error(o),f.error(n.t("登录失败，请在控制台查看原因"));return}let a=o.data.responseText;d.parseHTML(a,!0,!0).querySelectorAll(".sign-out-link a[rel=nofollow][data-method='delete']").length?(f.success(n.t("登录成功，1s后自动跳转")),setTimeout(()=>{window.location.reload();},1e3)):(c.error(o),c.error(`当前账号:${e}`),c.error(`当前密码:${t}`),f.error(n.t("登录失败，可能是账号/密码错误，请在控制台查看原因")));});}},W={$data:{FILTER_SCRIPT_KEY:"greasyfork-discussions-filter-script",FILTER_POST_USER_KEY:"greasyfork-discussions-filter-post-user",FILTER_REPLY_USER_KEY:"greasyfork-discussions-filter-reply-user"},init(){c.info("论坛-过滤"),w(`
        ...
        `);let e=new h.LockFunction(()=>{this.filterDiscussions();},50);h.mutationObserver(document.body,{config:{subtree:!0,childList:!0},callback:()=>{e.run();}}),e.run();},filterDiscussions(){const e=p.getValue(this.$data.FILTER_SCRIPT_KEY,""),t=p.getValue(this.$data.FILTER_POST_USER_KEY,""),r=p.getValue(this.$data.FILTER_REPLY_USER_KEY,""),i=e.trim()===""?[]:e.split(`
`),o=t.trim()===""?[]:t.split(`
`),a=r.trim()===""?[]:r.split(`
`),s=new Map;document.querySelectorAll(".discussion-list-container").forEach((l,m)=>{if(!l.querySelector("a.script-link"))return;const u=this.parseDiscussionListContainer(l);if(s.has(u.snippet)&&p.getValue("greasyfork-discussions-filter-duplicate-comments")){let g=s.get(u.snippet).querySelector("a.discussion-title");g.setAttribute("data-repeat-tip-show","true");let y=0;g.hasAttribute("data-repeat-count")&&(y=parseInt(g.getAttribute("data-repeat-count"))),y++,g.setAttribute("data-repeat-count",y.toString()),g.setAttribute("data-repeat-tip-show",n.t("已过滤：{{oldCount}}",{oldCount:y})),c.success([`过滤重复内容：${u.snippet}`,u]),l.remove();return}s.set(u.snippet,l);for(const g of i)if(u.scriptId===g){c.success([`过滤脚本id：${u.scriptId}`,u]),l.remove();return}for(const g of o)if(u.postUserId===g){c.success([`过滤发布用户id：${u.postUserId}`,u]),l.remove();return}if(u.replyUserName){for(const g of a)if(u.replyUserId===g){c.success([`过滤回复用户id：${u.replyUserId}`,u]),l.remove();return}}});},parseDiscussionListContainer(e){var r;const t={scriptName:e.querySelector("a.script-link").innerText,scriptUrl:e.querySelector("a.script-link").href,scriptId:b.getScriptId(e.querySelector("a.script-link").href),postUserName:e.querySelector("a.user-link").innerText,postUserHomeUrl:e.querySelector("a.user-link").href,postUserId:b.getUserId(e.querySelector("a.user-link").href),postTimeStamp:new Date(e.querySelector("relative-time").getAttribute("datetime")),snippetUrl:e.querySelector("a.discussion-title").href,snippet:e.querySelector("span.discussion-snippet").innerText,replyUserName:void 0,replyUserHomeUrl:void 0,replyUserId:void 0,replyTimeStamp:void 0};return e.querySelector(".discussion-meta-item .discussion-meta-item")&&(t.replyUserName=e.querySelector(".discussion-meta-item .discussion-meta-item a.user-link").innerText,t.replyUserHomeUrl=e.querySelector(".discussion-meta-item .discussion-meta-item a.user-link").href,t.replyUserId=b.getUserId(t.replyUserHomeUrl),t.replyTimeStamp=new Date((r=e.querySelector(".discussion-meta-item .discussion-meta-item relative-time"))==null?void 0:r.getAttribute("datetime"))),t}},Oe={init(){this.readBgColor(),d.ready(()=>{p.execMenuOnce("greasyfork-discussions-filter-enable",()=>{this.filterEnable();}),p.execMenuOnce("discussions-addShortcutOperationButton",()=>{this.addShortcutOperationButton();});});},filterEnable(){c.info("启用Greasyfork论坛过滤器"),W.init();},readBgColor(){c.info("设置已读背景颜色");let e=p.getValue("discussions-readBgColor");w(`
        .discussion-read{
            background: ${e} !important;
        }
        `);},addShortcutOperationButton(){c.info("添加快捷操作按钮"),document.querySelectorAll(".discussion-list-container").forEach(e=>{if(!e.querySelector("a.script-link"))return;let r=e.querySelector(".discussion-list-item").querySelector(".discussion-meta"),i=d.createElement("div",{className:"discussion-meta-item",innerHTML:`
					<button class="discussion-filter-button">${n.t("过滤")}</button>
					`}),o=i.querySelector(".discussion-filter-button");r.appendChild(i),d.on(o,"click",a=>{h.preventEvent(a);const s=W.parseDiscussionListContainer(e);let l=$.alert({title:{text:n.t("选择需要过滤的选项"),position:"center",html:!1},content:{text:`
							<div class="choose-list">
								...
								${s.replyUserId!=null?`<button class="choose-item" data-type="reply-user-id">${n.t("过滤回复的用户(id)")}</button>`:""}
							</div>
						...
						`});d.on(l.$shadowRoot,"click","button[data-type]",m=>{h.preventEvent(m);let u=m.target,g="",y="";u.dataset.type==="script-id"?(g=s.scriptId,y=W.$data.FILTER_SCRIPT_KEY):u.dataset.type==="user-id"?(g=s.postUserId,y=W.$data.FILTER_POST_USER_KEY):u.dataset.type==="reply-user-id"?(g=s.replyUserId,y=W.$data.FILTER_REPLY_USER_KEY):c.warn("未知data-type"),$.confirm({title:{text:n.t("提示"),position:"center"},content:{text:n.t("确定{{type}}：{{filterId}}？",{type:u.textContent,filterId:g}),html:!0},mask:{enable:!0,clickEvent:{toClose:!0}},btn:{ok:{callback(M,k){if(h.isNull(y)){c.error("存储的key是空的");return}let L=p.getValue(y,"").trim();L!==""&&(L+=`
`),L+=g,p.setValue(y,L),M.close(),l.close(),W.filterDiscussions();}}},drag:!0,dragLimit:!0,width:"300px",height:"200px"});});});});}},je={init(){p.execMenuOnce("users-changeConsoleToTopNavigator",()=>{this.changeConsoleToTopNavigator();}),p.execMenuOnce("gf-scripts-filter-enable",()=>{O.init();}),p.execMenuOnce("beautifyCenterContent",()=>ye.beautifyCenterContent());},changeConsoleToTopNavigator(){c.info("迁移【控制台】到顶部导航栏"),X.addBlockCSS("#about-user"),d.ready(()=>{let e=document.querySelector("#about-user"),t=document.querySelector("#site-nav nav");if(!e){c.error("#about-user元素不存在");return}if(!t){c.error("#site-nav nav元素不存在");return}e=e.cloneNode(!0);let r=d.createElement("li",{className:"scripts-console",innerHTML:`<a href="javascript:;">${n.t("控制台")}</a>`});d.on(r,"click",i=>{h.preventEvent(i),$.drawer({title:{enable:!1},content:{text:"",html:!0},size:"auto",direction:"top",zIndex:h.getMaxZIndex(100),style:`
                    .text-content{
                    ...
                    `}).$shadowRoot.querySelector(".pops-drawer-content").appendChild(e);}),t.appendChild(r);});}},H={init(){p.execMenu("checkPage",()=>{this.checkPage();}),He.init(),A.isScript()&&_e.init(),(A.isScriptList()||A.isScriptLibraryList())&&ye.init(),A.isDiscuessions()&&Oe.init(),A.isUserHome()&&je.init(),p.execMenuOnce("scripts-addOperationPanelBtnWithNavigator",()=>{this.addOperationPanelBtnWithNavigator();}),d.ready(()=>{v.initEnv(),Ge.init(),v.handleLocalGotoCallBack(),p.execMenuOnce("fixImageWidth",()=>{H.fixImageWidth();}),H.languageSelectorLocale(),p.execMenuOnce("optimizeImageBrowsing",()=>{H.optimizeImageBrowsing();}),p.execMenuOnce("overlayBedImageClickEvent",()=>{H.overlayBedImageClickEvent();}),A.isCodeStrict()||p.execMenuOnce("addMarkdownCopyButton",()=>{H.addMarkdownCopyButton();});});},fixImageWidth(){window.innerWidth<window.innerHeight&&(c.info("修复图片显示问题"),w(`
            img.lum-img{
                width: 100% !important;
                height: 100% !important;
            }
          `));},optimizeImageBrowsing(){c.info("优化图片浏览"),w(ae("ViewerCSS")),w(`
        ...
        `);function e(){let t=d.createElement("div",{className:"zeroclipboard-container",innerHTML:`
            ...
            `}),r=t.querySelector(".js-clipboard-copy"),i=t.querySelector(".octicon-copy"),o=t.querySelector(".octicon-check-copy");return d.on(t,"click",function(){let a=t.parentElement.querySelector("code");if(!a&&t.parentElement.className.includes("prettyprinted")&&(a=t.parentElement),!a){f.error(n.t("未找到{{selector}}元素",{selector:"code"}));return}h.setClip(a.innerText||a.textContent),r.setAttribute("success","true"),i.setAttribute("aria-hidden","true"),o.removeAttribute("aria-hidden");let s=$.tooltip({target:r,content:n.t("✅ 复制成功!"),position:"left",className:"github-tooltip",alwaysShow:!0});setTimeout(()=>{r.removeAttribute("success"),o.setAttribute("aria-hidden","true"),i.removeAttribute("aria-hidden"),s.close();},2e3);}),t}document.querySelectorAll("pre").forEach(t=>{if(t.querySelector("div.zeroclipboard-container"))return;let i=e(),o=d.createElement("div",{className:"snippet-clipboard-content"});d.before(t,o),o.appendChild(t),o.appendChild(i);});},languageSelectorLocale(){let e=p.getValue("language-selector-locale"),t=window.location.pathname.split("/").filter(r=>!!r)[0];if(c.success("选择语言："+e),c.success("当前语言："+t),!h.isNull(e)&&e!==t){let r=null,i=b.getSwitchLanguageUrl(e);b.switchLanguage(i),c.success("新Url："+i),f.loading(n.t("当前语言：{{currentLocaleLanguage}}，，3秒后切换至：{{localeLanguage}}",{currentLocaleLanguage:t,localeLanguage:e}),{timeout:3e3,showClose:!0,onClose(){clearTimeout(r);}}),f.info(n.t("导航至：")+i,{timeout:3e3}),r=setTimeout(()=>{window.location.href=i;},3e3);}},async UIScriptList(e,t,r,i){var g,y,M;if(!v.isLogin){f.error(n.t("请先登录账号！"));return}let a=v.getUserLinkElement().href,s=(M=(y=(g=a==null?void 0:a.split("/"))==null?void 0:g.pop())==null?void 0:y.match(/([0-9]+)/))==null?void 0:M[0],l=$.loading({mask:{enable:!0},parent:i,content:{text:n.t("获取信息中，请稍后...")},addIndexCSS:!1}),m=await b.getUserInfo(s);if(l.close(),!m)return;c.info(m);let u=e==="script-list"?m.scriptList:m.scriptLibraryList;f.success(n.t("获取成功，共 {{count}} 个",{count:u.length}));for(const k of u){let L=d.createElement("li",{className:"w-script-list-item",innerHTML:`
				<div class="w-script-info">
				<div class="w-script-name">
					<a href="${k.url}" target="_blank">${k.name}</a>
				</div>
				<div class="w-script-fan-score">
					<p>${n.t("评分：")}${k.fan_score}</p>
				</div>
				<div class="w-script-locale">
					<p>${n.t("语言：")}${k.locale}</p>
				</div>
				<div class="w-script-version">
					<p>${n.t("版本：")}${k.version}</p>
				</div>
				<div class="w-script-update-time">
					<p>${n.t("更新：")}${h.getDaysDifference(new Date(k.code_updated_at).getTime(),void 0,"auto")}前</p>
				</div>
				</div>
            `}),q=L.querySelector(".w-script-info"),V=d.createElement("div",{className:"pops-panel-button",innerHTML:`
				<button type="primary" data-icon="" data-righticon="false">
				<span>${n.t("同步代码")}</span>
				</button>
				`});k.deleted&&(L.classList.add("w-script-deleted"),V.querySelector("button").setAttribute("disabled","true")),d.on(V,"click",void 0,async function(){c.success(["同步",k]);let C=V.querySelector("button"),T=V.querySelector("button span"),B=d.createElement("i",{className:"pops-bottom-icon",innerHTML:$.config.iconSVG.loading},{"is-loading":!0});C.setAttribute("disabled","true"),C.setAttribute("data-icon","true"),T.innerText=n.t("同步中..."),d.before(T,B);let E=k==null?void 0:k.id,S=await b.getSourceCodeSyncFormData(E.toString());if(S){const z="script[script_sync_type_id]";if(S.has(z)){let R=S.get(z),D="";R.toString()==="1"?D=n.t("手动"):R.toString()==="2"?D=n.t("自动"):R.toString()==="3"&&(D="webhook");let K=L.querySelector(".w-script-sync-type");K?K.querySelector("p").innerText=n.t("同步方式：{{syncMode}}",{syncMode:D}):d.append(q,`
								<div class="w-script-sync-type">
									<p>${n.t("同步方式：{{syncMode}}",{syncMode:D})}
									</p>
								</div>`),await b.sourceCodeSync(k.id.toString(),S)?f.success(n.t("同步成功")):f.error(n.t("同步失败"));}else f.error(n.t("该脚本未设置同步信息"));}C.removeAttribute("disabled"),C.removeAttribute("data-icon"),T.innerText=n.t("同步代码"),B.remove();}),L.appendChild(V),i.appendChild(L);}},checkPage(){c.info("检测gf页面是否正确加载，有时候会出现"),d.ready(()=>{if(document.body.firstElementChild&&document.body.firstElementChild.localName==="p"&&document.body.firstElementChild.innerText.includes("We're down for maintenance. Check back again soon.")){let e=parseInt(G("greasyfork-check-page-time",0)),t=p.getValue("greasyfork-check-page-timeout",5),r=t*1e3;if(e&&Date.now()-e<r){f.warning(n.t("上次重载时间 {{time}}，{{timeout}}秒内拒绝反复重载",{time:h.formatTime(e,"yyyy-MM-dd HH:mm:ss"),timeout:t}));return}ne("greasyfork-check-page-time",Date.now()),window.location.reload();}});},addOperationPanelBtnWithNavigator(){c.info("添加【操作面板】按钮"),X.addBlockCSS(".sidebarred .sidebar",".sidebarred-main-content .open-sidebar"),w(`
		...
		`),d.ready(()=>{let e=document.querySelector("#site-nav nav"),t=document.querySelector("#site-nav .with-submenu nav"),r=document.querySelector("#script-list-option-groups")||document.querySelector(".list-option-groups");if(!r){c.warn("不存在右侧面板元素#script-list-option-groups");return}if(r=r.cloneNode(!0),r.classList.add("option-panel-groups"),!e){c.error("元素#site-nav nav不存在");return}let i=d.createElement("li",{className:"filter-scripts",innerHTML:`
                ...
                `});d.on(i,"click",o=>{h.preventEvent(o),$.drawer({title:{enable:!1},content:{text:"",html:!0},direction:"top",size:"80%",zIndex:h.getMaxZIndex(100),style:`
                    ...
                    `}).$shadowRoot.querySelector(".pops-drawer-content").appendChild(r);}),t&&t.children.length?t.appendChild(i):e.appendChild(i);});}},We={id:"greasy-fork-panel-config-script-list",title:n.t("脚本列表"),callback(e,t,r){H.UIScriptList("script-list",e,t,r);},forms:[]},Ke={id:"greasy-fork-panel-config-library",title:n.t("库"),callback(e,t,r){H.UIScriptList("script-library",e,t,r);},forms:[]},Ye=`.w-script-list-item {\r
	padding: 10px 0;\r
...
`,Je={id:"greasy-fork-panel-config-account",title:n.t("用户"),forms:[{text:"",type:"forms",forms:[{text:n.t("功能"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[x(n.t("迁移【控制台】到顶部导航栏"),"users-changeConsoleToTopNavigator",!0,void 0,n.t("将【控制台】按钮移动到顶部导航栏，节省空间"))]}]}]}]},N={data:null,oneSuccessExecMenu:null,onceExec:null,listenData:null},p={$data:{get data(){return N.data==null&&(N.data=new h.Dictionary),N.data},get oneSuccessExecMenu(){return N.oneSuccessExecMenu==null&&(N.oneSuccessExecMenu=new h.Dictionary),N.oneSuccessExecMenu},get onceExec(){return N.onceExec==null&&(N.onceExec=new h.Dictionary),N.onceExec},get scriptName(){return le},key:P,attributeKeyName:Z,attributeDefaultValueName:Q},$listener:{get listenData(){return N.listenData==null&&(N.listenData=new h.Dictionary),N.listenData}},init(){this.initPanelDefaultValue(),this.initExtensionsMenu();},initExtensionsMenu(){I.top===I.self&&me.add([{key:"show_pops_panel_setting",text:n.t("⚙ 设置"),autoReload:!1,isStoreValue:!1,showText(e){return e},callback:()=>{this.showPanel();}}]);},initPanelDefaultValue(){let e=this;function t(o){if(!o.attributes)return;let a=o.attributes[Z],s=o.attributes[Q];if(a==null){c.warn(["请先配置键",o]);return}e.$data.data.has(a)&&c.warn("请检查该key(已存在): "+a),e.$data.data.set(a,s);}function r(o){for(let a=0;a<o.length;a++){let s=o[a];t(s);let l=s.forms;l&&Array.isArray(l)&&r(l);}}let i=this.getPanelContentConfig();for(let o=0;o<i.length;o++){let a=i[o];if(!a.forms)continue;let s=a.forms;s&&Array.isArray(s)&&r(s);}},setValue(e,t){let r=G(P,{}),i=r[e];r[e]=t,ne(P,r),this.$listener.listenData.has(e)&&this.$listener.listenData.get(e).callback(e,i,t);},getValue(e,t){let i=G(P,{})[e];return i??(this.$data.data.has(e)?this.$data.data.get(e):t)},deleteValue(e){let t=G(P,{}),r=t[e];Reflect.deleteProperty(t,e),ne(P,t),this.$listener.listenData.has(e)&&this.$listener.listenData.get(e).callback(e,r,void 0);},addValueChangeListener(e,t,r){let i=Math.random();return this.$listener.listenData.set(e,{id:i,key:e,callback:t}),r&&r.immediate&&t(e,this.getValue(e),this.getValue(e)),i},removeValueChangeListener(e){let t=null;for(const[r,i]of this.$listener.listenData.entries())if(i.id===e){t=r;break}typeof t=="string"?this.$listener.listenData.delete(t):console.warn("没有找到对应的监听器");},hasKey(e){let t=G(P,{});return e in t},execMenu(e,t){if(typeof e!="string")throw new TypeError("key 必须是字符串");if(!this.$data.data.has(e)){c.warn(`${e} 键不存在`);return}let r=p.getValue(e);r&&t(r);},execMenuOnce(e,t){if(typeof e!="string")throw new TypeError("key 必须是字符串");if(!this.$data.data.has(e)){c.warn(`${e} 键不存在`);return}if(this.$data.oneSuccessExecMenu.has(e))return;this.$data.oneSuccessExecMenu.set(e,1);let r=[],i=s=>{let l=p.getValue(e);o(l,s);},o=(s,l)=>{let m=[];if(s){let u=l??t(s,i);u instanceof HTMLStyleElement?m=[u]:Array.isArray(u)&&(m=[...u.filter(g=>g!=null&&g instanceof HTMLStyleElement)]);}for(let u=0;u<r.length;u++)r[u].remove(),r.splice(u,1),u--;r=[...m];};this.addValueChangeListener(e,(s,l,m)=>{o(m);});let a=p.getValue(e);a&&o(a);},onceExec(e,t){if(typeof e!="string")throw new TypeError("key 必须是字符串");this.$data.onceExec.has(e)||(t(),this.$data.onceExec.set(e,1));},showPanel(){$.panel({title:{text:n.t("{{SCRIPT_NAME}}-设置",{SCRIPT_NAME:le}),position:"center",html:!1,style:""},content:this.getPanelContentConfig(),mask:{enable:!0,clickEvent:{toClose:!0,toHide:!1}},isMobile:this.isMobile(),width:this.getWidth(),height:this.getHeight(),drag:!0,only:!0,style:`
			${Ye}
			`});},isMobile(){return window.outerWidth<550},getWidth(){return window.outerWidth<550?"92dvw":"550px"},getHeight(){return window.outerHeight>450?"80dvh":"450px"},getPanelContentConfig(){return [Te,Ne,Ve,Je,We,Ke]}};p.init();H.init();
