...
`,gt="",K={isHome(){return window.location.pathname.split("/").length<=2&&window.location.search===""},isCode(){return !!window.location.pathname.split("/")?.includes("code")},isCodeStrict(){return !!window.location.pathname.match(/\/code(\/|)$/)},isVersion(){return !!window.location.pathname.match(/\/versions(\/|)$/)},isUsers(){return !!window.location.pathname.match(/\/.+\/users\/.+/gi)},isUsersConversations(){return this.isUsers()&&!!window.location.pathname.includes("/conversations")},isUsersConversationsWithSomeUser(){return this.isUsersConversations()&&!!window.location.pathname.match(/\/conversations\/[\d]+/)},isScript(){return !!window.location.pathname.match(/\/scripts\/[\d+]/)},isScriptAdmin(){return !!window.location.pathname.endsWith("/admin")},isScriptList(){return !!window.location.pathname.match(/\/scripts(\/|)$/)},isScriptsBySite(){return !!window.location.pathname.match("/scripts/by-site")},isScriptsFeedback(){return this.isScript()&&window.location.pathname.match(/\/feedback(\/|)$/i)},isScriptLibraryList(){return !!window.location.pathname.match(/\/libraries(\/|)$/)},isScriptSearch(){let e=new URLSearchParams(window.location.search);return this.isScriptList()&&e.has("q")},isScriptCodeSearch(){return !!window.location.pathname.match(/\/code-search(\/|)$/)},isScriptLibraryListSearch(){let e=new URLSearchParams(window.location.search);return this.isScriptLibraryList()&&e.has("q")},isDiscuessions(){return !!window.location.pathname.match(/\/discussions(\/|)$/)},isImageSource(){return window.location.pathname.startsWith("/vite/assets")}},vt={init(){m.execMenuOnce("beautifyPage",()=>this.beautifyPageElement()),m.execMenuOnce("beautifyGreasyforkBeautify",()=>this.beautifyGreasyforkBeautify()),m.execMenuOnce("beautifyUploadImage",()=>this.beautifyUploadImage()),m.execMenuOnce("beautifyTopNavigationBar",()=>this.beautifyTopNavigationBar());},beautifyPageElement(){u.info("美化页面元素");let e=[];return e.push(I(it)),e.push(I(at)),e.push(I(st)),e.push(I(lt)),e.push(I(ct)),e.push(I(`
			...
			`)),c.ready(()=>{let t=N('a[target="markup_choice"][href*="daringfireball.net"]');t&&t.parentElement.replaceChild(c.createElement("span",{textContent:"Markdown"}),t),X(".user-content ul li").forEach(o=>{let a=o.firstChild;a?.nodeName==="#text"&&(a.textContent?.startsWith("[x] ")?(a.textContent=a.textContent.replace("[x] ",""),c.prepend(o,`
				...
				section#script-info{
					margin-top: 10px;
				}`)),e},beautifyUploadImage(){u.info("美化上传图片");let e=[];return e.push(I(dt)),c.ready(()=>{function t(o){for(;o.nextElementSibling;)o.parentElement.removeChild(o.nextElementSibling);}let A=X('input[type="file"]');A.forEach(o=>{o.getAttribute("name")!=="code_upload"&&(o.hasAttribute("accept")&&o.getAttribute("accept").includes("javascript")||c.on(o,["propertychange","input"],function(a){t(a.target);let i=a.currentTarget.files;if(!i||i.length===0)return;u.info("选择的图片",i),i.length>5&&c.after(o,c.createElement("p",{textContent:r.t("❌ 最多同时长传5张图片")}));let s=[];Array.from(i).forEach(l=>{(l.size>204800||!l.type.match(/png|jpg|jpeg|gif|apng|webp/i))&&s.push(l);}),s.length!==0&&s.forEach(l=>{c.after(o,c.createElement("p",{textContent:r.t("❌ 图片：{{name}} 大小：{{size}}",{name:l.name,size:l.size})}));});}));}),["textarea#comment_text","textarea.comment-entry"].forEach(o=>{c.on(o,"paste",a=>{u.info("触发粘贴事件",a),setTimeout(()=>{c.trigger(A,"input");},100);});});}),e},beautifyTopNavigationBar(){u.info("美化顶部导航栏");let e=[];return e.push(I(pt)),window.outerWidth>550&&(e.push(_.addBlockCSS(".with-submenu")),c.ready(()=>{let A=N("#site-nav").querySelector("nav");X(".with-submenu nav li").forEach(n=>{A.appendChild(n);});})),e}},R={menu:Qe,isLogin:false,initEnv(){let e=this.getUserLinkElement();this.isLogin=!!e;},getUserLinkElement(){return N("#nav-user-info span.user-profile-link a")},async updateScript(e){let t=function(A,n=1){return `
...
`),h.isNull(e))return;e=e.toString().trim();let n=e+"##"+t;A+=n,this.setValue(A);},getValue(){return m.getValue(this.key,"")}},mt={init(){this.readBgColor(),c.ready(()=>{m.execMenuOnce("greasyfork-discussions-filter-enable",()=>{this.filterEnable();}),I(`
...
`,bt={init(){m.execMenuOnce("beautifyHistoryVersionPage",()=>this.beautifyHistoryVersionPage()),m.execMenuOnce("scripts-versions-addExtraTagButton",()=>{this.addExtraTagButton();}),m.execMenuOnce("scripts-versions-addCompareCodeButton",()=>{this.sourceDiffMonacoEditor();});},beautifyHistoryVersionPage(){u.info("美化 历史版本 页面");let e=[];return e.push(I(wt)),e.push(_.addBlockCSS(".version-number",".version-date",".version-changelog")),c.ready(function(){let t=N("ul.history_versions");if(!t){g.error(r.t("未找到history_versions元素列表"));return}Array.from(t.children).forEach(A=>{let n=A.querySelector(".version-number a").href,o=A.querySelector(".version-number a").innerText,a=A.querySelector(".version-date")?.getAttribute("datetime"),i=A.querySelector(".version-changelog")?.innerHTML||"",s=c.createElement("span",{className:"script-version-date",innerHTML:h.formatTime(a,r.t("yyyy年MM月dd日 HH:mm:ss"))}),l=c.createElement("div",{className:"script-tag",innerHTML:`
                    ...
                    </div>`}),d=c.createElement("div",{className:"script-note-box-body",innerHTML:i});A.appendChild(s),A.appendChild(l),A.appendChild(d);});}),e},addExtraTagButton(){u.info("添加额外的标签按钮"),c.ready(()=>{X(".script-tag-version").forEach(e=>{let t=e.querySelector("a");if(!t)return;let A=new URL(t.href),n=A.pathname.match(/\/scripts\/([\d]+)/)?.[1],o=A.searchParams.get("version"),a=A.pathname.match(/\/scripts\/[\d]+-(.+)/)?.[1],i=j.getInstallUrl(n,o,a),s=j.getCodeUrl(n,o),l=c.createElement("div",{className:"scripts-tag-install",innerHTML:`
...
`,Mt={init(){m.execMenuOnce("conversations-beautifyDialogBox",()=>this.beautifyDialogBox()),c.ready(()=>{m.execMenuOnce("conversations-beautifyPrivateMessageList",()=>{this.beautifyPrivateMessageList();});});},beautifyDialogBox(){return u.info("美化对话框"),I(zt)},beautifyPrivateMessageList(){u.info("美化私信列表"),I(`
