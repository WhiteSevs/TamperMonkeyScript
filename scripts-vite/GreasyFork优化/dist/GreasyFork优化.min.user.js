...
(function (g, B, ae, Ce, r, Fe, Ye) {
  ...
  var ge=typeof GM_deleteValue<"u"?GM_deleteValue:void 0,Se=typeof GM_getResourceText<"u"?GM_getResourceText:void 0,ee=typeof GM_getValue<"u"?GM_getValue:void 0,fe=typeof GM_info<"u"?GM_info:void 0,we=typeof GM_listValues<"u"?GM_listValues:void 0,Ze=typeof GM_registerMenuCommand<"u"?GM_registerMenuCommand:void 0,$=typeof GM_setValue<"u"?GM_setValue:void 0,Be=typeof GM_setValues<"u"?GM_setValues:void 0,We=typeof GM_unregisterMenuCommand<"u"?GM_unregisterMenuCommand:void 0,Je=typeof GM_xmlhttpRequest<"u"?GM_xmlhttpRequest:void 0,V=typeof unsafeWindow<"u"?unsafeWindow:void 0,$e=window;const _={waitRemove(...e){e.forEach(t=>{typeof t=="string"&&B.waitNodeList(t).then(A=>{A.forEach(n=>n.remove());});});},createBlockCSSNode(...e){let t=[];if(e.length!==0&&!(e.length===1&&typeof e[0]=="string"&&e[0].trim()===""))return e.forEach(A=>{Array.isArray(A)?t=t.concat(A):t.push(A);}),B.createElement("style",{type:"text/css",innerHTML:`${t.join(`,
...
`,Me={getTampermonkey:()=>V.external?.Tampermonkey,getViolentmonkey:()=>V.external?.Violentmonkey,getScriptCat:()=>V.external?.Scriptcat,getScriptContainerStatus(){let e={Tampermonkey:false,Violentmonkey:false,ScriptCat:false};return V.external?.Tampermonkey&&(e.Tampermonkey=true),V.external?.Violentmonkey&&(e.Violentmonkey=true),V.external?.Scriptcat&&(e.ScriptCat=true),e},getRegisterScriptContainerNameList(){let t=this.getScriptContainerStatus(),A=[];return Object.keys(t).forEach(n=>{Reflect.get(t,n)&&A.push(n);}),A},getInstalledVersion(e,t){return new Promise((A,n)=>{const o=this.getTampermonkey();if(o){o.isInstalled(e,t,function(s){s.installed?A(s.version):A(null);});return}const a=this.getViolentmonkey();if(a){a.isInstalled(e,t).then(A);return}const i=this.getScriptCat();if(i){i.isInstalled(e,t,function(s){s.installed?A(s.version):A(null);});return}n(new TypeError("获取脚本容器暴露的external信息失败"));})},compareVersions(e,t){if(e===t)return 0;const A=e.split("."),n=t.split(".");for(let o=0;o<A.length;o++){const a=this.compareVersionPart(A[o],n[o]);if(a!==0)return a}return 0},compareVersionPart(e,t){const A=this.parseVersionPart(e),n=this.parseVersionPart(t);for(let o=0;o<A.length;o++){if(A[o].length>0&&n[o].length===0)return  -1;if(A[o].length===0&&n[o].length>0||A[o]>n[o])return 1;if(A[o]<n[o])return  -1}return 0},parseVersionPart(e){if(!e)return [0,"",0,""];const t=/([0-9]*)([^0-9]*)([0-9]*)([^0-9]*)/.exec(e);return [t[1]?parseInt(t[1]):0,t[2],t[3]?parseInt(t[3]):0,t[4]]},handleInstallResult(e,t,A){if(t!=null)switch(e.removeAttribute("data-ping-url"),this.compareVersions(t,A)){case  -1:e.textContent=e.getAttribute("data-update-label");break;case 1:e.textContent=e.getAttribute("data-downgrade-label");break;case 0:e.textContent=e.getAttribute("data-reinstall-label");break}},async checkForUpdatesJS(e,t){const A=e.getAttribute("data-script-name"),n=e.getAttribute("data-script-namespace"),o=e.getAttribute("data-script-version");try{let a=await this.getInstalledVersion(A,n);return a==null?!1:(this.handleInstallResult(e,a,o),!0)}catch{if(t){await h.sleep(1e3);try{return await this.checkForUpdatesJS(e,!1)}catch{}}return  false}},checkForUpdatesCSS(e){const t=e.getAttribute("data-script-name"),A=e.getAttribute("data-script-namespace");postMessage({type:"style-version-query",name:t,namespace:A,url:location.href},location.origin);}},qe={init(){m.execMenuOnce("gf-scripts-filter-enable",()=>{ie.init();}),m.execMenuOnce("beautifyCenterContent",()=>this.beautifyCenterContent());},beautifyCenterContent(){u.info("美化脚本列表-双列");let e=[];e.push(I(Lt));const t="lum-lightbox-loader",A=r.t("安装此脚本");return B.ready(()=>{let n=Me.getScriptContainerStatus(),o=Object.values(n).find(d=>d),a=Me.getRegisterScriptContainerNameList(),i=m.getValue("beautifyCenterContent-queryNameSpace");o?u.info("当前暴露的external信息："+a.map(d=>`【${d}】`).join("、")):u.error("脚本容器未暴露external信息",window.external);let s=new h.LockFunction(()=>{ie.getElementList().forEach(p=>{if(p.querySelector(".script-list-operation"))return;let f=le.parseScriptListInfo(p),v=p.querySelector(".inline-script-stats");if(!v)return;let y=f.codeUrl,x=B.createElement("dt",{className:"script-list-rating-score",innerHTML:`<span>${r.t("评分")}</span>`}),L=B.createElement("dd",{className:"script-list-rating-score",innerHTML:`<span>${f.scriptRatingScore}</span>`},{"data-position":"right"}),D=p.querySelector("dd.script-list-ratings .good-rating-count"),M=p.querySelector("dd.script-list-ratings .ok-rating-count"),O=p.querySelector("dd.script-list-ratings .bad-rating-count");if(D&&M&&O){let C=parseInt(D.innerText),E=parseInt(M.innerText),P=parseInt(O.innerText),q=C+E+P;q>=10?C/q>=.6?L.classList.add("good-rating-count"):L.classList.add("bad-rating-count"):q==0||C>E+P?L.classList.add("good-rating-count"):L.classList.add("bad-rating-count");}let z=B.createElement("dt",{className:"script-list-version",innerHTML:`<span>${r.t("版本")}</span>`}),k=B.createElement("dd",{className:"script-list-version",innerHTML:`<span>${f.scriptVersion}</span>`},{"data-position":"right"}),w=B.createElement("dt",{className:"script-list-operation",innerHTML:`<span>${r.t("操作")}</span>`}),b=B.createElement("dd",{className:"script-list-operation",innerHTML:`
						...
						`},{"data-position":"right",style:"gap:10px;display: flex;flex-wrap: wrap;align-items: center;"}),U=b.querySelector(".script-collect-btn"),T=b.querySelector(".install-link");if(T["data-script-info"]=f,B.addClass(T,t),f.scriptType==="library"&&T.remove(),B.on(U,"click",C=>{B.preventEvent(C),Xe(f.scriptId);}),m.getValue("gf-scripts-filter-enable")){let C=B.createElement("button",{className:"script-filter-btn",innerHTML:r.t("过滤")}),E="data-filter-key",P="data-filter-value";B.on(C,"click",q=>{B.preventEvent(q);let H=Q.alert({title:{text:r.t("选择需要过滤的选项"),position:"center"},content:{text:`
								...
								`}),Ae=H.$shadowRoot.querySelector(".pops-alert-content");f.scriptAuthors.forEach(Z=>{let re=B.createElement("button",{innerHTML:r.t("作者id：{{text}}",{text:Z.authorId})});re.setAttribute(E,"scriptAuthorId"),re.setAttribute(P,"^"+Z.authorId+"$");let F=B.createElement("button",{innerHTML:r.t("作者名：{{text}}",{text:Z.authorName})});F.setAttribute(E,"scriptAuthorName"),F.setAttribute(P,"^"+h.toRegExpStr(Z.authorName)+"$"),Ae.appendChild(re),Ae.appendChild(F);}),B.on(H.$shadowRoot,"click",`button[${E}]`,Z=>{B.preventEvent(Z);let re=Z.target,F=re.getAttribute(E),ye=re.getAttribute(P);ie.addValue(F,ye),H.close(),ie.filter(),g.success(r.t("添加成功"));});}),b.appendChild(C);}v.appendChild(x),v.appendChild(L),v.appendChild(z),v.appendChild(k),v.appendChild(w),v.appendChild(b);});},100),l=new h.LockFunction(async()=>{let d=Array.from(X(".install-link[data-install-format=js]:not([gm-is-check-install-status])"));for(let p=0;p<d.length;p++){const f=d[p];f.setAttribute("gm-is-check-install-status","");let v=Reflect.get(f,"data-script-info");if(o){if(i){let y=await Y.getScriptInfo(v.scriptId);y&&f.setAttribute("data-script-namespace",y.namespace);}Me.checkForUpdatesJS(f,true).then(y=>{B.removeClass(f,t),y||B.text(f,A);});}else B.removeClass(f,t),B.text(f,A);}});h.mutationObserver(document,{config:{subtree:true,childList:true},immediate:true,callback:()=>{s.run(),l.run();}});}),e}},Ct={init(){m.execMenuOnce("users-changeConsoleToTopNavigator",()=>{this.changeConsoleToTopNavigator();}),m.execMenuOnce("gf-scripts-filter-enable",()=>{ie.init();}),m.execMenuOnce("beautifyCenterContent",()=>qe.beautifyCenterContent());},changeConsoleToTopNavigator(){u.info("迁移【控制台】到顶部导航栏"),_.addBlockCSS("#about-user"),c.ready(()=>{let e=N("#about-user");if(!e){u.error("#about-user元素不存在");return}e=e.cloneNode(true),le.registerTopNavMenu({name:r.t("控制台"),className:"scripts-console",clickEvent(t){Q.alert({title:{text:r.t("控制台"),position:"center"},content:{text:"",html:true},btn:{ok:{enable:false}},mask:{enable:true,clickEvent:{toClose:true}},drag:true,useShadowRoot:true,width:W.setting.width,height:W.setting.height,zIndex:h.getMaxZIndex(100),style:`
						#about-user{
							border: 0;
							box-shadow: none;
						}
...
`;Re.addMenuOption({key:"githubUrl2webhookUrl",text:"⚙ "+r.t("Url To WebhookUrl"),autoReload:false,isStoreValue:false,showText(e){return e},callback:()=>{St.showView();}});m.$data.panelConfig={style:jt};Le.addContentConfig([Dt,It,Ot,Tt,Bt]);m.init();ue.init();
