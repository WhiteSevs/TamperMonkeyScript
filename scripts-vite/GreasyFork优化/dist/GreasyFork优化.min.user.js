...
`,bt={init(){m.execMenuOnce("beautifyHistoryVersionPage",()=>this.beautifyHistoryVersionPage()),m.execMenuOnce("scripts-versions-addExtraTagButton",()=>{this.addExtraTagButton();}),m.execMenuOnce("scripts-versions-addCompareCodeButton",()=>{this.sourceDiffMonacoEditor();});},beautifyHistoryVersionPage(){u.info("美化 历史版本 页面");let e=[];return e.push(I(wt)),e.push(_.addBlockCSS(".version-number",".version-date",".version-changelog")),c.ready(function(){let t=N("ul.history_versions");if(!t){g.error(r.t("未找到history_versions元素列表"));return}Array.from(t.children).forEach(A=>{let n=A.querySelector(".version-number a").href,o=A.querySelector(".version-number a").innerText,a=A.querySelector(".version-date")?.getAttribute("datetime"),i=A.querySelector(".version-changelog")?.innerHTML||"",s=c.createElement("span",{className:"script-version-date",innerHTML:h.formatTime(a,r.t("yyyy年MM月dd日 HH:mm:ss"))}),l=c.createElement("div",{className:"script-tag",innerHTML:`
                    <div class="script-tag-version">
						...
						`});c.after(e,l);});});},sourceDiffMonacoEditor(){u.info("源码对比（monacoEditor）"),Ie.monacoEditor().then(e=>{c.ready(()=>{X('#script-content form[action*="/diff"] input[type="submit"]').forEach(t=>{let A=c.createElement("input",{type:"button",value:r.t("对比选中版本差异（monacoEditor）")},{style:"margin-left: 10px;"});c.after(t,A),c.on(A,"click",async n=>{c.preventEvent(n);let o=t.closest("form"),a=new FormData(o),i=a.get("v1"),s=a.get("v2");if(i===s){g.warning(r.t("版本号相同，不需要比较源码"));return}let l=g.loading(r.t("正在获取对比文本中...")),d=j.getScriptId(),f=await V.getScriptInfo(d);if(!f){l.close();return}let p=f.code_url,v=p.replace(new RegExp(`/${d}(/[\\d]+|)`),`/${d}/${i}`),y=p.replace(new RegExp(`/${d}(/[\\d]+|)`),`/${d}/${s}`),x="",C="";const[E,M]=await Promise.all([q.get(v,{timeout:2e4}),q.get(y,{timeout:2e4})]);if(l.close(),!E.status||!M.status)return;x=E.data.responseText,C=M.data.responseText;let{recovery:T}=_.lockScroll(),z=U.alert({title:{text:r.t("代码对比"),html:false,position:"center"},content:{html:true,text:`
