...
(function (g, j, Ce, se, r, Fe, Ye) {
  ...
  const Ye__namespace = /*#__PURE__*/_interopNamespaceDefault(Ye);
...
`,vt="",ht={init(){m.execMenuOnce("beautifyPage",()=>this.beautifyPageElement()),m.execMenuOnce("beautifyGreasyforkBeautify",()=>this.beautifyGreasyforkBeautify()),m.execMenuOnce("beautifyUploadImage",()=>this.beautifyUploadImage()),m.execMenuOnce("beautifyTopNavigationBar",()=>this.beautifyTopNavigationBar());},beautifyPageElement(){p.info("美化页面元素");const e=[];return e.push(D(at)),e.push(D(st)),e.push(D(lt)),e.push(D(ct)),e.push(D(dt)),e.push(D(`
			...
			`)),c.onReady(()=>{let t=N('a[target="markup_choice"][href*="daringfireball.net"]');t&&t.parentElement.replaceChild(c.createElement("span",{textContent:"Markdown"}),t),X(".user-content ul li").forEach(i=>{let s=i.firstChild;s?.nodeName==="#text"&&(s.textContent?.startsWith("[x] ")?(s.textContent=s.textContent.replace("[x] ",""),c.prepend(i,`
				...
				section#script-info{
					margin-top: 10px;
				}`)),e},beautifyUploadImage(){p.info("美化上传图片");const e=[];return e.push(D(ut)),c.onReady(()=>{function t(i){for(;i.nextElementSibling;)i.parentElement.removeChild(i.nextElementSibling);}const A=X('input[type="file"]');A.forEach(i=>{i.getAttribute("name")!=="code_upload"&&(i.hasAttribute("accept")&&i.getAttribute("accept").includes("javascript")||c.on(i,["propertychange","input"],function(s){t(s.target);const o=s.currentTarget.files;if(!o||o.length===0)return;p.info("选择的图片",o),o.length>5&&c.after(i,c.createElement("p",{textContent:r.t("❌ 最多同时长传5张图片")}));const a=[];Array.from(o).forEach(l=>{(l.size>204800||!l.type.match(/png|jpg|jpeg|gif|apng|webp/i))&&a.push(l);}),a.length!==0&&a.forEach(l=>{c.after(i,c.createElement("p",{textContent:r.t("❌ 图片：{{name}} 大小：{{size}}",{name:l.name,size:l.size})}));});}));}),["textarea#comment_text","textarea.comment-entry"].forEach(i=>{c.on(i,"paste",s=>{p.info("触发粘贴事件",s),setTimeout(()=>{c.emit(A,"input");},100);});});}),e},beautifyTopNavigationBar(){p.info("美化顶部导航栏");const e=[];return e.push(D(ft)),window.outerWidth>550&&(e.push(_.addBlockCSS(".with-submenu")),c.onReady(()=>{const A=N("#site-nav").querySelector("nav");X(".with-submenu nav li").forEach(n=>{A.appendChild(n);});})),e}},mt=".whitesev-hide{display:none}.whitesev-hide-important,.ad,.ad-entry{display:none!important}";Ve(mt);const yt={init(){m.execMenu("autoLogin",()=>{this.autoLogin();});},autoLogin(){c.waitNode("span.sign-in-link a[rel=nofollow]").then(async()=>{let e=m.getValue("user"),t=m.getValue("pwd"),A=m.getValue("secret");if(h.isNull(e)){g.error(r.t("请先在菜单中录入账号"));return}if(h.isNull(t)){g.error(r.t("请先在菜单中录入密码"));return}if(h.isNull(A)){g.error(r.t("请先在菜单中录入secret"));return}let n=N("meta[name='csrf-token']");if(!n){g.error(r.t("获取csrf-token失败"));return}let s=new Ye__namespace.TOTP({secret:A}).generate(),o=g.loading(r.t("正在登录中...")),a=await V.post("/zh-CN/users/sign_in",{fetch:true,data:encodeURI(`authenticity_token=${n.getAttribute("content")}&user[email]=${e}&user[password]=${t}&user[remember_me]=1&user[otp_attempt]=${s}&commit=登录`),headers:{"Content-Type":"application/x-www-form-urlencoded"}});if(o.destroy(),!a.status){p.error(a),g.error(r.t("登录失败，请求异常"));return}let l=a.data.responseText,d=c.toElement(l,true,true),u=d.querySelector(".width-constraint .alert");d.querySelectorAll(".sign-out-link a[rel=nofollow][data-method='delete']").length?(g.success(r.t("登录成功，1s后自动跳转")),setTimeout(()=>{window.location.reload();},1e3)):u?(p.error(u),g.error(c.text(u))):(g.error(r.t("登录失败，可能是账号/密码错误，请在控制台查看原因"),{consoleLogContent:true}),p.error(a),p.error(`当前账号:${e}`),p.error(`当前密码:${t}`));});}},q={menu:Qe,isLogin:false,initEnv(){let e=this.getUserLinkElement();this.isLogin=!!e;},getUserLinkElement(){return N("#nav-user-info span.user-profile-link a")},async updateScript(e){let t=function(A,n=1){return `
...
`),h.isNull(e))return;e=e.toString().trim();let n=e+"##"+t;A+=n,this.setValue(A);},getValue(){return m.getValue(this.key,"")}},bt={init(){this.readBgColor(),c.onReady(()=>{m.execMenuOnce("greasyfork-discussions-filter-enable",()=>{this.filterEnable();}),D(`
      ...
      `);const e=new h.LockFunction(()=>{const t=m.getValue("discussions-addShortcutOperationButton"),A=m.getValue("discussions-addReportButton");t&&this.addFilterButton(),A&&this.addReportButton(),(t||A)&&this.addMoreButton({addFilterButton:t,addReportButton:A});});h.mutationObserver(document.body,{config:{subtree:true,childList:true},immediate:true,callback:()=>{e.run();}});});},filterEnable(){p.info("启用Greasyfork论坛过滤器"),te.init();},readBgColor(){p.info("设置已读背景颜色");const e=m.getValue("discussions-readBgColor"),A=new h.ColorConversion().getDarkColor(e,.8);return D(`
        ...
        `)},addFilterButton(){const e="discussion-filter-button";te.getElementList().forEach(t=>{if(t.querySelector(`.${e}`))return;const n=t.querySelector(".discussion-list-item").querySelector(".discussion-meta"),i=c.createElement("div",{className:"discussion-meta-item",innerHTML:`
				...
				`});c.append(document.head||document.documentElement,i),c.on(R,A,()=>{const s=R.monaco;p.success("网络加载monaco编辑器成功"),n.close(),e(s);},{once:true});})}},xt={init(){m.execMenuOnce("code-repairCodeLineNumber",()=>{this.repairCodeLineNumber();}),m.execMenuOnce("code-use-monaco-editor",()=>{this.coverEditorWithMonaco();});},repairCodeLineNumber(){p.info("修复代码的行号显示不够问题"),m.execMenuOnce("beautifyGreasyforkBeautify",()=>{D(`
...
`),h.isNull(e))return;e=e.toString().trim();let n=e+"##"+t;A+=n,this.setValue(A);},getValue(){return m.getValue(this.key,"")}},zt=`.sidebarred-main-content {\r
  max-width: unset;\r
  flex: unset;\r
}\r
ol.script-list {\r
...
`,St={init(){m.execMenuOnce("conversations-beautifyDialogBox",()=>this.beautifyDialogBox()),c.onReady(()=>{m.execMenuOnce("conversations-beautifyPrivateMessageList",()=>{this.beautifyPrivateMessageList();});});},beautifyDialogBox(){return p.info("美化对话框"),D(kt)},beautifyPrivateMessageList(){p.info("美化私信列表"),D(`
		...
		`),X("section.text-content ul li").forEach(e=>{let t=e.querySelector('a[href*="conversations"]'),A=t.href,n=t.textContent?.split(" ")[1],i=e.querySelector("a.user-link"),s=null,o=null,a=null;if(i){s=i.textContent,o=i.href;let l=e.querySelector("relative-time");new Date(l.getAttribute("datetime")),a=l.shadowRoot.lastChild.textContent;}e.classList.add("user-conversations-item"),O.getUserId(),e.innerHTML=`
			...
			`;});}},Be={init(){m.execMenuOnce("users-changeConsoleToTopNavigator",()=>{this.changeConsoleToTopNavigator();}),m.execMenuOnce("gf-scripts-filter-enable",()=>{ae.init();}),m.execMenuOnce("beautifyCenterContent",()=>He.beautifyCenterContent());},changeConsoleToTopNavigator(){p.info("迁移【控制台】到顶部导航栏"),_.addBlockCSS("#about-user"),c.onReady(()=>{let e=N("#about-user");if(!e){p.error("#about-user元素不存在");return}e=e.cloneNode(true),le.registerTopNavMenu({name:r.t("控制台"),className:"scripts-console",clickEvent(t){Q.alert({title:{text:r.t("控制台"),position:"center"},content:{text:"",html:true},btn:{ok:{enable:false}},mask:{enable:true,clickEvent:{toClose:true}},drag:true,useShadowRoot:true,width:Z.setting.width,height:Z.setting.height,zIndex:h.getMaxZIndex(100),style:`
						#about-user{
							border: 0;
							box-shadow: none;
						}
						...
						`}).$shadowRoot.querySelector(".pops-alert-content").appendChild(e);}});});}},Et={init(){if(K.isImageSource()){p.info("Router: 资源界面，不执行脚本功能");return}m.execMenu("checkPage",()=>{this.checkPage();}),ht.init(),qe.init(),K.isScript()&&Pt.init(),(K.isScriptList()||K.isScriptLibraryList()||K.isScriptCodeSearch()||K.isScriptsBySite())&&He.init(),K.isDiscuessions()?(p.info("Router: 讨论页面"),bt.init()):K.isUsers()?(p.info("Router: 用户页面"),Be.init(),K.isUsersConversations()&&(p.info("Router-next: 私聊用户页面"),St.init())):(K.isScriptSearch()||K.isScriptsBySite()||K.isScriptCodeSearch()||K.isScriptLibraryListSearch())&&(p.info("Router: 脚本搜索页面"),Mt.init()),m.execMenuOnce("scripts-addOperationPanelBtnWithNavigator",()=>{this.addOperationPanelBtnWithNavigator();}),c.onReady(()=>{q.initEnv(),yt.init(),Se.init(),q.handleLocalGotoCallBack(),m.execMenuOnce("fixImageWidth",()=>this.fixImageWidth()),this.languageSelectorLocale(),m.execMenuOnce("optimizeImageBrowsing",()=>{this.optimizeImageBrowsing();}),m.execMenuOnce("overlayBedImageClickEvent",()=>{this.overlayBedImageClickEvent();}),K.isCodeStrict()||m.execMenuOnce("addMarkdownCopyButton",()=>{this.addMarkdownCopyButton();}),m.execMenuOnce("queryUserRegisterTime",()=>this.queryUserRegisterTime());});},fixImageWidth(){if(window.innerWidth<window.innerHeight)return p.info("修复图片显示问题"),D(`
            img.lum-img{
                width: 100% !important;
                height: 100% !important;
            }
