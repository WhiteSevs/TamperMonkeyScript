...
`)}{display: none !important;}`)},setGMResourceCSS(e){let t=typeof ae=="function"?ae(e.keyName):"";typeof t=="string"&&t?ce(t):X.addLinkNode(e.url);},async addLinkNode(e){let t=document.createElement("link");return t.rel="stylesheet",t.type="text/css",t.href=e,d.ready(()=>{document.head.appendChild(t);}),t},fixUrl(e){return e=e.trim(),e.match(/^http(s|):\/\//i)||(e.startsWith("/")||(e+="/"),e=window.location.origin+e),e}},se={getInstallUrl(e,t,r){return h.isNotNull(r)?r="/"+r:r="",`https://update.greasyfork.org/scripts/${e}/${t}${r}.user.js`},getCodeUrl(e,t){return h.isNull(t)&&(t=""),`https://greasyfork.org/scripts/${e}/code?version=${t}`},getScriptInfoUrl(e){return `https://greasyfork.org/scripts/${e}.json`}},Ue={init(){p.execMenuOnce("beautifyHistoryVersionPage",()=>this.beautifyHistoryVersionPage()),p.execMenuOnce("scripts-versions-addExtraTagButton",()=>{this.addExtraTagButton();});},beautifyHistoryVersionPage(){c.info("美化 历史版本 页面");let e=[];return e.push(w(Ae)),e.push(X.addBlockCSS(".version-number",".version-date",".version-changelog")),d.ready(function(){let t=document.querySelector("ul.history_versions");if(!t){f.error(n.t("未找到history_versions元素列表"));return}Array.from(t.children).forEach(r=>{var g,b;let i=r.querySelector(".version-number a").href,o=r.querySelector(".version-number a").innerText,a=(g=r.querySelector(".version-date"))==null?void 0:g.getAttribute("datetime"),s=((b=r.querySelector(".version-changelog"))==null?void 0:b.innerHTML)||"",l=d.createElement("span",{className:"script-version-date",innerHTML:h.formatTime(a,n.t("yyyy年MM月dd日 HH:mm:ss"))}),u=d.createElement("div",{className:"script-tag",innerHTML:`
                    ...
                    </div>`}),m=d.createElement("div",{className:"script-note-box-body",innerHTML:s});r.appendChild(l),r.appendChild(u),r.appendChild(m);});}),e},addExtraTagButton(){c.info("添加额外的标签按钮"),d.ready(()=>{document.querySelectorAll(".script-tag-version").forEach(e=>{var m,g;let t=e.querySelector("a");if(!t)return;let r=new URL(t.href),i=(m=r.pathname.match(/\/scripts\/([\d]+)/))==null?void 0:m[1],o=r.searchParams.get("version"),a=(g=r.pathname.match(/\/scripts\/[\d]+-(.+)/))==null?void 0:g[1],s=se.getInstallUrl(i,o,a),l=se.getCodeUrl(i,o),u=d.createElement("div",{className:"scripts-tag-install",innerHTML:`
		...
		`});d.on(i.$shadowRoot,"click",".collect-add-script-id",async function(o){let a=o.target.closest(".user-collect-item"),s=a.dataset.id;a.dataset.name;let l=f.loading(n.t("添加中...")),u=await y.getUserCollectionInfo(t,s);if(!u){l.close();return}let m=h.cloneFormData(u),g=h.cloneFormData(u),b=!1;for(const[L,S]of u.entries())if(L==="scripts-included[]"&&String(S).trim()===String(e).trim()){b=!0;break}else g.append(L,S),m.append(L,S);if(b){f.warning(n.t("该脚本已经在该收藏集中")),l.close();return}m.set("add-script",e.toString()),m.set("script-action","i"),g.append("scripts-included[]",e.toString()),g.set("save","1");let E=new URLSearchParams(m),k=new URLSearchParams(g),M=Array.from(E).map(([L,S])=>`${encodeURIComponent(L)}=${encodeURIComponent(S)}`).join("&"),B=Array.from(k).map(([L,S])=>`${encodeURIComponent(L)}=${encodeURIComponent(S)}`).join("&");c.info(["添加的数据",M]),c.info(["保存的数据",B]);let V=await y.updateUserSetsInfo(t,s,M);if(!V){l.close();return}let C=V.querySelector(".change-script-set");if(!C){f.error(n.t("添加失败，{{selector}}元素不存在",{selector:".change-script-set"})),l.close();return}let T=C.querySelector("section");if(!T){f.error(n.t("添加失败，{{selector}}元素不存在",{selector:"section"})),l.close();return}let q=T.querySelector(".alert");q?_.alert({title:{text:n.t("添加失败"),position:"center"},content:{text:q.innerHTML,html:!0},mask:{enable:!0,clickEvent:{toClose:!0}},style:`
					.pops-alert-content{
						font-style: italic;
					...
					`,drag:!0,dragLimit:!0,width:_.isPhone()?"88vw":"400px",height:_.isPhone()?"50vh":"300px"}):(await y.updateUserSetsInfo(t,s,B),f.success(n.t("添加成功"))),l.close();}),d.on(i.$shadowRoot,"click",".collect-delete-script-id",async function(o){let a=o.target.closest(".user-collect-item"),s=a.dataset.id;a.dataset.name;let l=f.loading(n.t("删除中...")),u=await y.getUserCollectionInfo(t,s);if(!u){l.close();return}let m=new FormData,g=new FormData,b=!1;for(const[C,T]of u.entries()){if(String(C).trim()==="scripts-included[]"&&String(T).trim()===String(e).trim()){b=!0;continue}g.append(C,T),m.append(C,T);}if(!b){f.warning(n.t("该收藏集未包含：{{scriptId}}",{scriptId:e})),l.close();return}m.set("remove-scripts-included[]",e.toString()),m.set("remove-selected-scripts","i"),m.delete("script-action"),g.set("save","1");let E=new URLSearchParams(m),k=new URLSearchParams(g),M=Array.from(E).map(([C,T])=>`${encodeURIComponent(C)}=${encodeURIComponent(T)}`).join("&"),B=Array.from(k).map(([C,T])=>`${encodeURIComponent(C)}=${encodeURIComponent(T)}`).join("&");if(c.info(["删除的数据",M]),c.info(["保存的数据",B]),!await y.updateUserSetsInfo(t,s,M)){l.close();return}await y.updateUserSetsInfo(t,s,B),f.success(n.t("删除成功")),l.close();});},$e={init(){A.isCode()?Ie.init():A.isVersion()&&Ue.init(),A.isCodeStrict()&&(p.execMenuOnce("fullScreenOptimization",()=>{this.fullScreenOptimization();}),p.execMenuOnce("addCopyCodeButton",()=>{this.addCopyCodeButton();})),p.execMenuOnce("addCollectionButton",()=>{this.addCollectionButton();}),p.execMenuOnce("addFindReferenceButton",()=>{this.setFindCodeSearchBtn();}),p.execMenuOnce("scriptHomepageAddedTodaySUpdate",()=>{this.scriptHomepageAddedTodaySUpdate();});},addCollectionButton(){c.info("添加收藏按钮"),h.waitNode("ul#script-links li.current span").then(()=>{let e=d.createElement("li",{innerHTML:`
					<a href="javascript:;">
						<span>${n.t("收藏")}</span>
					</a>`});d.append(document.querySelector("ul#script-links"),e),d.on(e,"click",()=>{let t=window.location.pathname.match(/scripts\/([\d]+)/i);if(!t){c.error([t,window.location.pathname]),f.error(n.t("获取脚本id失败"));return}let r=t[t.length-1];me(r);});});},fullScreenOptimization(){c.info("F11全屏，F键代码全屏"),w(`
        .code-wide-screen{
          position: absolute;
          top: 0;
          left: 0;
...
`,ie={getTampermonkey:()=>{var e;return (e=I.external)==null?void 0:e.Tampermonkey},getViolentmonkey:()=>{var e;return (e=I.external)==null?void 0:e.Violentmonkey},getScriptCat:()=>{var e;return (e=I.external)==null?void 0:e.Scriptcat},getScriptContainerStatus(){var t,r,i;let e={Tampermonkey:!1,Violentmonkey:!1,ScriptCat:!1};return (t=I.external)!=null&&t.Tampermonkey&&(e.Tampermonkey=!0),(r=I.external)!=null&&r.Violentmonkey&&(e.Violentmonkey=!0),(i=I.external)!=null&&i.Scriptcat&&(e.ScriptCat=!0),e},getInstalledVersion(e,t){return new Promise((r,i)=>{const o=this.getTampermonkey();if(o){o.isInstalled(e,t,function(l){l.installed?r(l.version):r(null);});return}const a=this.getViolentmonkey();if(a){a.isInstalled(e,t).then(r);return}const s=this.getScriptCat();if(s){s.isInstalled(e,t,function(l){l.installed?r(l.version):r(null);});return}i(new TypeError("获取脚本容器暴露的external信息失败"));})},compareVersions(e,t){if(e===t)return 0;const r=e.split("."),i=t.split(".");for(let o=0;o<r.length;o++){const a=this.compareVersionPart(r[o],i[o]);if(a!==0)return a}return 0},compareVersionPart(e,t){const r=this.parseVersionPart(e),i=this.parseVersionPart(t);for(let o=0;o<r.length;o++){if(r[o].length>0&&i[o].length===0)return -1;if(r[o].length===0&&i[o].length>0||r[o]>i[o])return 1;if(r[o]<i[o])return -1}return 0},parseVersionPart(e){if(!e)return [0,"",0,""];const t=/([0-9]*)([^0-9]*)([0-9]*)([^0-9]*)/.exec(e);return [t[1]?parseInt(t[1]):0,t[2],t[3]?parseInt(t[3]):0,t[4]]},handleInstallResult(e,t,r){if(t!=null)switch(e.removeAttribute("data-ping-url"),this.compareVersions(t,r)){case-1:e.textContent=e.getAttribute("data-update-label");break;case 1:e.textContent=e.getAttribute("data-downgrade-label");break;case 0:e.textContent=e.getAttribute("data-reinstall-label");break}},async checkForUpdatesJS(e,t){const r=e.getAttribute("data-script-name"),i=e.getAttribute("data-script-namespace"),o=e.getAttribute("data-script-version");try{let a=await this.getInstalledVersion(r,i);return a==null?!1:(this.handleInstallResult(e,a,o),!0)}catch{if(t){await h.sleep(1e3);try{return await this.checkForUpdatesJS(e,!1)}catch{}}return !1}},checkForUpdatesCSS(e){const t=e.getAttribute("data-script-name"),r=e.getAttribute("data-script-namespace");postMessage({type:"style-version-query",name:t,namespace:r,url:location.href},location.origin);}},ge=e=>{let t=e.dataset;const r={scriptId:parseInt(t.scriptId),scriptName:t.scriptName,scriptAuthors:[],scriptDailyInstalls:parseInt(t.scriptDailyInstalls),scriptTotalInstalls:parseInt(t.scriptTotalInstalls),scriptRatingScore:parseFloat(t.scriptRatingScore),scriptCreatedDate:new Date(t.scriptCreatedDate),scriptUpdatedDate:new Date(t.scriptUpdatedDate),scriptType:t.scriptType,scriptVersion:t.scriptVersion,sensitive:t.sensitive==="true",scriptLanguage:t.scriptLanguage,cssAvailableAsJs:t.cssAvailableAsJs==="true",codeUrl:t.codeUrl,scriptDescription:t.scriptDescription,scriptAuthorId:parseInt(t.scriptAuthorId),scriptAuthorName:t.scriptAuthorName};let i=h.toJSON(t.scriptAuthors);if(Object.keys(i).forEach(o=>{let a=i[o];r.scriptAuthors.push({authorId:parseInt(o),authorName:a});}),(r.scriptAuthorName==null||isNaN(r.scriptAuthorId))&&r.scriptAuthors.length&&(r.scriptAuthorName=r.scriptAuthors[0].authorName,r.scriptAuthorId=r.scriptAuthors[0].authorId),r.scriptDescription==null){let o=e.querySelector(".script-description")||e.querySelector(".description");o&&(r.scriptDescription=o.innerText||o.textContent);}return r},ye={init(){p.execMenuOnce("gf-scripts-filter-enable",()=>{O.init();}),p.execMenuOnce("beautifyCenterContent",()=>this.beautifyCenterContent());},beautifyCenterContent(){c.info("美化脚本列表");let e=[];return e.push(w(_e)),U.ready(async()=>{O.getElementList().forEach(a=>{if(a.querySelector(".script-list-operation"))return;let s=ge(a),l=a.querySelector(".inline-script-stats"),u=s.codeUrl,m=U.createElement("dt",{className:"script-list-rating-score",innerHTML:`<span>${n.t("评分")}</span>`}),g=U.createElement("dd",{className:"script-list-rating-score",innerHTML:`<span>${s.scriptRatingScore}</span>`},{"data-position":"right"}),b=a.querySelector("dd.script-list-ratings .good-rating-count"),E=a.querySelector("dd.script-list-ratings .ok-rating-count"),k=a.querySelector("dd.script-list-ratings .bad-rating-count");if(b&&E&&k){let L=parseInt(b.innerText),S=parseInt(E.innerText),z=parseInt(k.innerText),R=L+S+z;R>=10?L/R>=.6?g.classList.add("good-rating-count"):g.classList.add("bad-rating-count"):R==0||L>S+z?g.classList.add("good-rating-count"):g.classList.add("bad-rating-count");}let M=U.createElement("dt",{className:"script-list-version",innerHTML:`<span>${n.t("版本")}</span>`}),B=U.createElement("dd",{className:"script-list-version",innerHTML:`<span>${s.scriptVersion}</span>`},{"data-position":"right"}),V=U.createElement("dt",{className:"script-list-operation",innerHTML:`<span>${n.t("操作")}</span>`}),C=U.createElement("dd",{className:"script-list-operation",innerHTML:`
						...
						`},{"data-position":"right",style:"gap:10px;display: flex;flex-wrap: wrap;align-items: center;"}),T=C.querySelector(".script-collect-btn"),q=C.querySelector(".install-link");if(q["data-script-info"]=s,q.classList.add("lum-lightbox-loader"),s.scriptType==="library"&&q.remove(),U.on(T,"click",L=>{h.preventEvent(L),me(s.scriptId);}),p.getValue("gf-scripts-filter-enable")){let L=U.createElement("button",{className:"script-filter-btn",innerHTML:n.t("过滤")}),S="data-filter-key",z="data-filter-value";U.on(L,"click",R=>{h.preventEvent(R);let D=_.alert({title:{text:n.t("选择需要过滤的选项"),position:"center"},content:{text:`
								...
								`}),K=D.$shadowRoot.querySelector(".pops-alert-content");s.scriptAuthors.forEach(F=>{let j=U.createElement("button",{innerHTML:n.t("作者id：{{text}}",{text:F.authorId})});j.setAttribute(S,"scriptAuthorId"),j.setAttribute(z,"^"+F.authorId+"$");let J=U.createElement("button",{innerHTML:n.t("作者名：{{text}}",{text:F.authorName})});J.setAttribute(S,"scriptAuthorName"),J.setAttribute(z,"^"+h.parseStringToRegExpString(F.authorName)+"$"),K.appendChild(j),K.appendChild(J);}),U.on(D.$shadowRoot,"click",`button[${S}]`,F=>{h.preventEvent(F);let j=F.target,J=j.getAttribute(S),be=j.getAttribute(z);O.addValue(J,be),D.close(),O.filter(),f.success(n.t("添加成功"));});}),C.appendChild(L);}l.appendChild(m),l.appendChild(g),l.appendChild(M),l.appendChild(B),l.appendChild(V),l.appendChild(C);});let r=Array.from(document.querySelectorAll(".install-link[data-install-format=js]")),i=ie.getScriptContainerStatus(),o=Object.values(i).filter(Boolean);if(!i.Tampermonkey)r.forEach(async a=>{let s=await ie.checkForUpdatesJS(a,!0);a.classList.remove("lum-lightbox-loader"),s||(a.textContent=n.t("安装此脚本"));});else if(o.length)for(let a=0;a<r.length;a++){let s=r[a],l=s["data-script-info"],u=await $.get(se.getScriptInfoUrl(l.scriptId),{fetch:!0});if(!u.status){s.textContent=n.t("安装此脚本");continue}let m=h.toJSON(u.data.responseText);s.setAttribute("data-script-namespace",m.namespace);let g=await ie.checkForUpdatesJS(s,!0);s.classList.remove("lum-lightbox-loader"),g||(s.textContent=n.t("安装此脚本")),await h.sleep(150);}else c.error("未知的脚本容器");}),e}},O={key:"gf-shield-rule",init(){c.info("脚本过滤");let e=new h.LockFunction(()=>{this.filter();},50);d.ready(()=>{h.mutationObserver(document.body,{config:{subtree:!0,childList:!0},callback:()=>{e.run();}}),e.run();});},getElementList(){let e=[];return e=e.concat(Array.from(document.querySelectorAll("ol.script-list li"))),e},filter(){this.getElementList().forEach(e=>{let t=ge(e),r=this.getValue().split(`
...
`),h.isNull(e))return;e=e.toString().trim();let i=e+"##"+t;r+=i,this.setValue(r);},getValue(){return p.getValue(this.key,"")}},Ne={id:"greasy-fork-panel-config-scripts",title:n.t("脚本"),forms:[{text:"",type:"forms",forms:[{text:n.t("代码"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[x(n.t("添加复制代码按钮"),"addCopyCodeButton",!0,void 0,n.t("更优雅的复制")),x(n.t("快捷键"),"fullScreenOptimization",!0,void 0,n.t("【F】键全屏、【Alt+Shift+F】键宽屏")),x(n.t("修复代码行号显示"),"code-repairCodeLineNumber",!0,void 0,n.t("修复代码行数超过1k行号显示不全问题"))]}]},{text:n.t("历史版本"),type:"deepMenu",forms:[{text:n.t("功能"),type:"forms",forms:[x(n.t("添加额外的标签按钮"),"scripts-versions-addExtraTagButton",!0,void 0,n.t("在版本下面添加【安装】、【查看代码】按钮"))]},{text:n.t("美化"),type:"forms",forms:[x(n.t("美化历史版本页面"),"beautifyHistoryVersionPage",!0,void 0,n.t("更直观的查看版本迭代"))]}]}]},{text:"",type:"forms",forms:[{text:n.t("功能"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[x(n.t("添加【寻找引用】按钮"),"addFindReferenceButton",!0,void 0,n.t("在脚本栏添加按钮，一般用于搜索引用该库的相关脚本")),x(n.t("添加【收藏】按钮"),"addCollectionButton",!0,void 0,n.t("在脚本栏添加按钮，一般用于快捷收藏该脚本/库")),x(n.t("添加【今日检查】信息块"),"scriptHomepageAddedTodaySUpdate",!0,void 0,n.t("在脚本信息栏添加【今日检查】信息块"))]}]},{text:n.t("美化"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[x(n.t("美化脚本列表"),"beautifyCenterContent",!0,void 0,n.t("双列显示且添加脚本卡片操作项（安装、收藏）"))]}]},{text:n.t("过滤"),type:"deepMenu",forms:[{text:`<a target="_blank" href="https://greasyfork.org/zh-CN/scripts/475722-greasyfork%E4%BC%98%E5%8C%96#:~:text=%E8%84%9A%E6%9C%AC%E8%BF%87%E6%BB%A4%E8%A7%84%E5%88%99">${n.t("帮助文档")}</a>`,type:"forms",forms:[x(n.t("启用"),"gf-scripts-filter-enable",!0,void 0,n.t("作用域：脚本、脚本搜索、用户主页")),{type:"own",getLiElementCallBack(e){let t=d.createElement("div",{className:"pops-panel-textarea",innerHTML:`
        ...
        `);let e=new h.LockFunction(()=>{this.filter();},50);h.mutationObserver(document.body,{config:{subtree:!0,childList:!0},callback:()=>{e.run();}}),e.run();},getElementList(){let e=[];return e=e.concat(Array.from(document.querySelectorAll(".discussion-list-container"))),e},filter(){this.transformOldRule();const e=new Map;this.getElementList().forEach((t,r)=>{if(!t.querySelector("a.script-link"))return;const i=this.parseDiscuessionListContainerInfo(t);let o=this.getValue().split(`
`);if(e.has(i.snippet)&&p.getValue("greasyfork-discussions-filter-duplicate-comments")){let a=e.get(i.snippet).querySelector("a.discussion-title");a.setAttribute("data-repeat-tip-show","true");let s=0;a.hasAttribute("data-repeat-count")&&(s=parseInt(a.getAttribute("data-repeat-count"))),s++,a.setAttribute("data-repeat-count",s.toString()),a.setAttribute("data-repeat-tip-show",n.t("已过滤：{{oldCount}}",{oldCount:s})),c.success([`过滤重复内容：${i.snippet}`,i]),t.remove();return}e.set(i.snippet,t);for(let a=0;a<o.length;a++){let s=o[a],l=s.split("##"),u=l[0],m=l[1];if(u in i){let g=new RegExp(m,"ig");if(i[u]!=null&&String(i[u]).match(g)){c.info(["触发论坛过滤规则",s,i]),t.remove();return}}}});},parseDiscuessionListContainerInfo(e){var r;const t={scriptName:e.querySelector("a.script-link").innerText,scriptUrl:e.querySelector("a.script-link").href,scriptId:y.getScriptId(e.querySelector("a.script-link").href),postUserName:e.querySelector("a.user-link").innerText,postUserHomeUrl:e.querySelector("a.user-link").href,postUserId:y.getUserId(e.querySelector("a.user-link").href),postTimeStamp:new Date(e.querySelector("relative-time").getAttribute("datetime")),snippetUrl:e.querySelector("a.discussion-title").href,snippet:e.querySelector("span.discussion-snippet").innerText,replyUserName:void 0,replyUserHomeUrl:void 0,replyUserId:void 0,replyTimeStamp:void 0};return e.querySelector(".discussion-meta-item .discussion-meta-item")&&(t.replyUserName=e.querySelector(".discussion-meta-item .discussion-meta-item a.user-link").innerText,t.replyUserHomeUrl=e.querySelector(".discussion-meta-item .discussion-meta-item a.user-link").href,t.replyUserId=y.getUserId(t.replyUserHomeUrl),t.replyTimeStamp=new Date((r=e.querySelector(".discussion-meta-item .discussion-meta-item relative-time"))==null?void 0:r.getAttribute("datetime"))),t},transformOldRule(){if(Date.now()>new Date("2024-8-19").getTime())return;const e="greasyfork-discussions-filter-script",t="greasyfork-discussions-filter-post-user",r="greasyfork-discussions-filter-reply-user",i=p.getValue(e,""),o=p.getValue(t,""),a=p.getValue(r,""),s=i.trim()===""?[]:i.split(`
`),l=o.trim()===""?[]:o.split(`
`),u=a.trim()===""?[]:a.split(`
`);s.forEach(m=>{this.addValue("scriptId",h.parseStringToRegExpString("^"+m+"$"));}),l.forEach(m=>{this.addValue("postUserId",h.parseStringToRegExpString("^"+m+"$"));}),u.forEach(m=>{this.addValue("replyUserId",h.parseStringToRegExpString("^"+m+"$"));}),p.deleteValue(e),p.deleteValue(t),p.deleteValue(r);},setValue(e){p.setValue(this.key,e);},addValue(e,t){let r=this.getValue();if(r.trim()!==""&&(r+=`
`),h.isNull(e))return;e=e.toString().trim();let i=e+"##"+t;r+=i,this.setValue(r);},getValue(){return p.getValue(this.key,"")}},Ve={id:"greasy-fork-panel-config-discussions",title:n.t("论坛"),forms:[{text:"",type:"forms",forms:[{text:n.t("功能"),type:"deepMenu",forms:[{text:"",type:"forms",forms:[{type:"own",attributes:{"data-key":"discussions-readBgColor","data-default-value":"#e5e5e5"},getLiElementCallBack(e){let t="discussions-readBgColor",r=d.createElement("div",{className:"pops-panel-item-left-text",innerHTML:`
										...
										<textarea placeholder="${n.t("请输入规则，每行一个")}" style="height:200px;"></textarea>`},{style:"width: 100%;"}),r=t.querySelector("textarea");return r.value=W.getValue(),d.on(r,["input","propertychange"],void 0,h.debounce(function(i){W.setValue(r.value);},200)),e.appendChild(t),e}}]}]}]}]},ze=`code {\r
...
`,Ge={init(){p.execMenuOnce("beautifyPage",()=>this.beautifyPageElement()),p.execMenuOnce("beautifyGreasyforkBeautify",()=>this.beautifyGreasyforkBeautify()),p.execMenuOnce("beautifyUploadImage",()=>this.beautifyUploadImage()),p.execMenuOnce("beautifyTopNavigationBar",()=>this.beautifyTopNavigationBar());},beautifyPageElement(){c.info("美化页面元素");let e=[];return e.push(w(ze)),e.push(w(Be)),e.push(w(qe)),e.push(w(Re)),e.push(w(`
        ...
        `);},addShortcutOperationButton(){c.info("添加快捷操作按钮"),document.querySelectorAll(".discussion-list-container").forEach(e=>{if(!e.querySelector("a.script-link"))return;let r=e.querySelector(".discussion-list-item").querySelector(".discussion-meta"),i=d.createElement("div",{className:"discussion-meta-item",innerHTML:`
					...
					`}),o=i.querySelector(".discussion-filter-button");r.appendChild(i),d.on(o,"click",a=>{h.preventEvent(a);const s=W.parseDiscuessionListContainerInfo(e);let l="data-filter-key",u="data-filter-value",m=_.alert({title:{text:n.t("选择需要过滤的选项"),position:"center",html:!1},content:{text:`
								<button ${l}="scriptId" ${u}="^${s.scriptId}$">${n.t("脚本id：{{text}}",{text:s.scriptId})}</button>
								<button ${l}="scriptName" ${u}="^${h.parseStringToRegExpString(s.scriptName)}$">${n.t("脚本名：{{text}}",{text:s.scriptName})}</button>
								<button ${l}="postUserId" ${u}="^${h.parseStringToRegExpString(s.postUserId)}$">${n.t("发布的用户id：{{text}}",{text:s.postUserId})}</button>
        ...
        `);function e(){let t=d.createElement("div",{className:"zeroclipboard-container",innerHTML:`
            ...
            `}),B=M.querySelector(".w-script-info"),V=d.createElement("div",{className:"pops-panel-button",innerHTML:`
				<button type="primary" data-icon="" data-righticon="false">
				<span>${n.t("同步代码")}</span>
				</button>
				`});k.deleted&&(M.classList.add("w-script-deleted"),V.querySelector("button").setAttribute("disabled","true")),d.on(V,"click",void 0,async function(){c.success(["同步",k]);let C=V.querySelector("button"),T=V.querySelector("button span"),q=d.createElement("i",{className:"pops-bottom-icon",innerHTML:_.config.iconSVG.loading},{"is-loading":!0});C.setAttribute("disabled","true"),C.setAttribute("data-icon","true"),T.innerText=n.t("同步中..."),d.before(T,q);let L=k==null?void 0:k.id,S=await y.getSourceCodeSyncFormData(L.toString());if(S){const z="script[script_sync_type_id]";if(S.has(z)){let R=S.get(z),D="";R.toString()==="1"?D=n.t("手动"):R.toString()==="2"?D=n.t("自动"):R.toString()==="3"&&(D="webhook");let K=M.querySelector(".w-script-sync-type");K?K.querySelector("p").innerText=n.t("同步方式：{{syncMode}}",{syncMode:D}):d.append(B,`
								<div class="w-script-sync-type">
									<p>${n.t("同步方式：{{syncMode}}",{syncMode:D})}
									</p>
								</div>`),await y.sourceCodeSync(k.id.toString(),S)?f.success(n.t("同步成功")):f.error(n.t("同步失败"));}else f.error(n.t("该脚本未设置同步信息"));}C.removeAttribute("disabled"),C.removeAttribute("data-icon"),T.innerText=n.t("同步代码"),q.remove();}),M.appendChild(V),i.appendChild(M);}},checkPage(){c.info("检测gf页面是否正确加载，有时候会出现"),d.ready(()=>{if(document.body.firstElementChild&&document.body.firstElementChild.localName==="p"&&document.body.firstElementChild.innerText.includes("We're down for maintenance. Check back again soon.")){let e=parseInt(H("greasyfork-check-page-time",0)),t=p.getValue("greasyfork-check-page-timeout",5),r=t*1e3;if(e&&Date.now()-e<r){f.warning(n.t("上次重载时间 {{time}}，{{timeout}}秒内拒绝反复重载",{time:h.formatTime(e,"yyyy-MM-dd HH:mm:ss"),timeout:t}));return}ne("greasyfork-check-page-time",Date.now()),window.location.reload();}});},addOperationPanelBtnWithNavigator(){c.info("添加【操作面板】按钮"),X.addBlockCSS(".sidebarred .sidebar",".sidebarred-main-content .open-sidebar"),w(`
