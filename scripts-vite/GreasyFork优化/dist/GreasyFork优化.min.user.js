...
`),g.isNull(t))return;t=t.toString().trim();let n=t+"##"+e;A+=n,this.setValue(A);},getValue(){return h.getValue(this.key,"")}},pe=`.sidebarred-main-content {\r
	max-width: unset;\r
	flex: unset;\r
}\r
ol.script-list {\r
...
.comment:not(:has(.report-link)) .comment-meta-item {\r
	margin-left: 0px;\r
	margin-right: 15px;\r
}`,me={init(){h.execMenuOnce("conversations-beautifyDialogBox",()=>this.beautifyDialogBox()),c.ready(()=>{h.execMenuOnce("conversations-beautifyPrivateMessageList",()=>{this.beautifyPrivateMessageList();});});},beautifyDialogBox(){return u.info("美化对话框"),M(he)},beautifyPrivateMessageList(){u.info("美化私信列表"),M(`
		...
		`),c.ready(()=>{T("#site-nav nav"),T("#site-nav .with-submenu nav");let t=T("#script-list-option-groups")||T(".list-option-groups");if(!t){u.warn("不存在右侧面板元素#script-list-option-groups");return}t=t.cloneNode(true),t.classList.add("option-panel-groups"),st.registerTopNavMenu({name:r.t("操作面板"),className:"filter-scripts",clickEvent(e){R.alert({title:{text:r.t("操作面板"),position:"center"},content:{text:"",html:true},btn:{ok:{enable:false}},mask:{enable:true,clickEvent:{toClose:true}},drag:true,useShadowRoot:true,width:$.setting.width,height:$.setting.height,zIndex:g.getMaxZIndex(100),style:`
						...
						`}).$shadowRoot.querySelector(".pops-alert-content").appendChild(t);}});});}},we={init(){},showView(){let t=R.alert({title:{text:r.t("Url To WebhookUrl"),position:"center"},content:{text:`
...
`;Bt.addMenuOption({key:"githubUrl2webhookUrl",text:"⚙ "+r.t("Url To WebhookUrl"),autoReload:false,isStoreValue:false,showText(t){return t},callback:()=>{we.showView();}});h.$data.panelConfig={style:Me};mt.addContentConfig([xe,Le,Pe,Ce,ze]);h.init();dt.init();
