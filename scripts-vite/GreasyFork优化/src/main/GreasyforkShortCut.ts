  ...
  shortOption: {
    "gf-quickReply": {
      target: () => {
        ...
        let $replyBtn = $<HTMLButtonElement>('input[name="commit"][type="submit"]');
        if (!$commentText) {
          log.error("页面不存在输入框");
          return;
        } else if (!$replyBtn) {
      ...
      callback() {
        if (document.activeElement) {
          ...
          let $replyBtnList = $parentForm.querySelectorAll<HTMLButtonElement>('input[name="commit"][type="submit"]');
