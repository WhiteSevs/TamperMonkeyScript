...
export const GithubUrl2WebhookUrl = {
	...
	showView() {
		...
		let $githubUrlInput =
			$alert.$shadowRoot.querySelector<HTMLTextAreaElement>("#github")!;
		let $webhookUrlInput =
			$alert.$shadowRoot.querySelector<HTMLTextAreaElement>("#webhook")!;
			...
			githubUrl.split("\n").forEach((urlStr) => {
				try {
					...
					if (urlPathNameSplit.length >= 6 && blobStr === "blob") {
						// https://github.com/WhiteSevs/TamperMonkeyScript/blob/master/README.md
						filePath = urlPathNameSplit
							.slice(5, urlPathNameSplit.length)
							.join("/");
					} else if (
						urlPathNameSplit.length >= 8 &&
						blobStr === "raw" &&
						branchName === "refs"
					) {
						...
						filePath = urlPathNameSplit
							.slice(7, urlPathNameSplit.length)
							.join("/");
					} else {
						// 不符合要求的
						return;
