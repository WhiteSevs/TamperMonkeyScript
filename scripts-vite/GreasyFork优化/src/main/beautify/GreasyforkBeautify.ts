...
export const GreasyforkBeautify = {
  ...
  beautifyPageElement() {
    ...
    DOMUtils.ready(() => {
      ...
      $$<HTMLElement>(".user-content blockquote").forEach(($blockQuote) => {
        Array.from($blockQuote.children).forEach(($child) => {
          ...
          if ($child.nodeName === "P") {
            ...
            if (findValue) {
              DOMUtils.html($child as HTMLElement, htmlStr.replace(findValue, ""));
              DOMUtils.before($child as HTMLElement, warningListMap[findValue as keyof typeof warningListMap]);
      ...
      // 把警告列表的样式进行迁移合并
      $$<HTMLElement>(".user-content > blockquote").forEach(($blockQuote) => {
        Array.from($blockQuote.querySelectorAll<HTMLElement>(".markdown-alert-title")).forEach(
          ($markdownAlertTitle) => {
            ...
            while ($markdownAlertTitle.nextSibling) {
              if (($markdownAlertTitle.nextSibling as HTMLElement)?.classList?.contains("markdown-alert-title")) {
                break;
              }
              DOMUtils.append($markdownAlert, $markdownAlertTitle.nextSibling as HTMLElement);
            }
      ...
      if (GreasyforkRouter.isHome()) {
        ...
        let $noRating = $<HTMLElement>('.radio-label[for*="discussion_rating_0"]');
        if ($noRating) {
        ...
        let $badRating = $<HTMLElement>('.radio-label[for*="discussion_rating_2"]');
        if ($badRating) {
        ...
        let $okRating = $<HTMLElement>('.radio-label[for*="discussion_rating_3"]');
        if ($okRating) {
        ...
        let $goodRating = $<HTMLElement>('.radio-label[for*="discussion_rating_4"]');
        if ($goodRating) {
