...
export const GreasyforkScriptsList = {
  ...
  beautifyCenterContent() {
    ...
    DOMUtils.onReady(() => {
      ...
      /** 是否强制使用命名空间来查询信息 */
      let isForceUseNameSpace = Panel.getValue<boolean>("beautifyCenterContent-queryNameSpace");
      ...
      let lockFn = new utils.LockFunction(() => {
        ...
        allScriptsList.forEach(($scriptList) => {
          ...
          let $inlineStats = $scriptList.querySelector<HTMLElement>(".inline-script-stats");
          if (!$inlineStats) {
          ...
          // 中评
          let $okRatingCount = $scriptList.querySelector<HTMLSpanElement>("dd.script-list-ratings .ok-rating-count");
          // 差评
          let $badRatingCount = $scriptList.querySelector<HTMLSpanElement>("dd.script-list-ratings .bad-rating-count");
          ...
          let $collect = $operationRight.querySelector<HTMLButtonElement>(".script-collect-btn")!;

          let $installLink = $operationRight.querySelector<HTMLAnchorElement>(".install-link")!;
          // 存储数据
          ...
          if (Panel.getValue("gf-scripts-filter-enable")) {
            ...
            DOMUtils.on($filter, "click", (event) => {
              ...
              DOMUtils.on($dialog.$shadowRoot, "click", `button[${attr_filter_key}]`, (event) => {
                ...
                let key = $click.getAttribute(attr_filter_key)! as keyof ScriptFilterRule;
      ...
      let lockFn2 = new utils.LockFunction(async () => {
        ...
        let $installLinkList = Array.from(
          $$<HTMLElement>(".install-link[data-install-format=js]:not([gm-is-check-install-status])")
        );
        for (let index = 0; index < $installLinkList.length; index++) {
          ...
          let scriptLocalInfo = Reflect.get($installLink, "data-script-info") as GreasyforkScriptListInfo;
          if (hasScriptContainer) {
            // 得有脚本容器才可以获取版本号信息嘞
