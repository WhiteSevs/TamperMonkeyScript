...
const GreasyforkScriptsSearchElement = {
   ...
   */
  waitScritList() {
    return DOMUtils.waitNode<HTMLDivElement>("#browse-script-list", 10000);
  },
  /**
   * 添加控制区域
   */
  addFilterControls($scriptList: HTMLElement) {
    ...
    function getControls() {
      ...
      let $filterControl = shadowRoot.querySelector<HTMLElement>(".pops-filter-controls_inner");
      let $search = shadowRoot.querySelector<HTMLElement>(".pops-filter-search-container");
  ...
  addFilterControlsItem($search: HTMLElement, $filterControlsContainer: HTMLElement) {
    ...
    let controlsConfig: {
    ...
    }[] = [
      {
        ...
        callback: (searchText: string, scriptInfo: GreasyforkScriptListInfo) => {
          try {
    ...
    if (Panel.getValue("gf-script-search-addFilterSearchInput")) {
      let $searchInner = panelHandlerComponents.createSectionContainerItem_input({
        ...
        getValue() {
          ...
          return Panel.getValue<string>("gf-script-search-addFilterSearchInput-text") || queryText || "";
        },
          ...
          if (value && Array.isArray(controlConfig.mutualExclusionSwitch)) {
            ...
            controlConfig.mutualExclusionSwitch.forEach((selector) => {
              ...
              if ($switch) {
                ...
                if ($input?.checked) {
                  let $core = $switch.querySelector<HTMLElement>(".pops-panel-switch__core");
                  $core?.click();
                }
              }
            });
