...
import type { indexedDB } from "@whitesev/utils/dist/types/src/indexedDB.js";
...
export const GreasyforkRememberFormTextArea = {
  ...
  async rememberReplyContent() {
    ...
    $formList.forEach(async ($form) => {
      ...
      // 发表回复按钮
      let $replySubmit = $form.querySelector<HTMLButtonElement>(`input[type="submit"]`);
      ...
      this.$data.db.get<IndexedDBReplaceRecordData[]>(this.$key.DB_KEY).then((result) => {
        if (!result.data) {
          // 数据库是空的
          return;
        }
      ...
      DOMUtils.on(
        ...
        (event) => {
          ...
          GM_setValue("delyClear_rememberReplyContent_url", window.location.href);
        },
        {
          capture: true,
        }
  ...
  async clearRelayHistoryRememberContentText() {
    ...
    if (delyClear_rememberReplyContent_url) {
      // 清空数据
      let result = await this.$data.db.get<IndexedDBReplaceRecordData[]>(this.$key.DB_KEY);
      if (!result.data) {
        // 数据库是空的
