...
type ScriptCatInstalledData = ScriptCatInstalledDataTrue | ScriptCatInstalledDataFalse;
...
export const GreasyforkCheckVersion = {
  ...
  getTampermonkey: () => {
    return (unsafeWindow.external as any)?.Tampermonkey as
      | {
          /** 获取TamperMonkey版本号 */
          getVersion: (callback: (data: { version: string; id: string }) => void) => void;
          /** 获取脚本安装信息 */
          isInstalled: (
            name: string,
  ...
  getViolentmonkey: () => {
    return (unsafeWindow.external as any)?.Violentmonkey as
      | {
          ...
          version: string;
          /** 获取脚本安装信息，返回版本号 */
          isInstalled: (name: string, namespace: string) => Promise<null | string>;
        }
      | undefined;
  },
  ...
  compareVersionPart(partA: string, partB: string) {
    ...
    for (let i = 0; i < partAParts.length; i++) {
      ...
      if ((partAParts[i] as any).length > 0 && (partBParts[i] as any).length === 0) {
        return -1;
      }
      if ((partAParts[i] as any).length === 0 && (partBParts[i] as any).length > 0) {
        return 1;
   ...
   * @returns
   */
  handleInstallResult(installButton: HTMLElement, installedVersion: string, version: string) {
   ...
   * @param retry 重试
   */
  async checkForUpdatesJS(installButton: HTMLElement, retry: boolean): Promise<boolean> {
