...
export const NetDiskCheckLinkValidity = {
	 ...
	 */
	async check(
		checkInfoConfigList: NetDiskCheckLinkValidityInfoConfig | NetDiskCheckLinkValidityInfoConfig[]
	) {
	 ...
	 */
	setViewAgainCheckClickEvent($ele: HTMLElement, checkInfo: NetDiskCheckLinkValidityOption) {
		DOMUtils.on(
			$ele,
			"click",
			void 0,
	...
	getStatusName(statusInfo: NetDiskCheckLinkValidityStatusInstance) {
		for (const statusName of Object.keys(NetDiskCheckLinkValidityStatus)) {
			let statusNewInfo =
				NetDiskCheckLinkValidityStatus[statusName as keyof typeof NetDiskCheckLinkValidityStatus];
