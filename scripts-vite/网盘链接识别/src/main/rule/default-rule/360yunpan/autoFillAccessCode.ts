...
export const NetDiskAutoFillAccessCode_360yunpan = function (
	...
	if (window.location.hostname.endsWith(".link.yunpan.com")) {
		...
		utils
			.waitNode<HTMLInputElement>("#extract-bg-container input.pwd-input")
			.then(($el) => {
		...
		/* 移动端 */
		utils
			.waitNode<HTMLInputElement>("#extractForm input.pwd-input")
			.then(($el) => {
				if (!utils.isVisible($el)) {
					log.error("输入框不可见，不输入密码");
					return;
