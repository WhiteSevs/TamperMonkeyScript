...
export const NetDiskAutoFillAccessCode_xunlei = function (netDiskInfo: NetDiskAutoFillAccessCodeOption) {
  if (window.location.hostname === "pan.xunlei.com") {
    ...
    DOMUtils.waitNode<HTMLInputElement>("#__layout div.pass-input-wrap input.td-input__inner", 10000).then(($el) => {
      if (!$el) return;
    ...
    // 移动端
    DOMUtils.waitNode<HTMLInputElement>("#__layout div.pass-wrapper input.td-input__inner", 10000).then(($el) => {
      if (!$el) return;
      if (!utils.isVisible($el)) {
        log.error("输入框不可见，不输入密码");
        return;
