...
export const NetDiskAutoFillAccessCode_xunlei = function (netDiskInfo: NetDiskAutoFillAccessCodeOption) {
  if (window.location.hostname === "pan.xunlei.com") {
    ...
    utils.waitNode<HTMLInputElement>("#__layout div.pass-input-wrap input.td-input__inner").then((element) => {
    ...
    /* 移动端 */
    utils.waitNode<HTMLInputElement>("#__layout div.pass-wrapper input.td-input__inner").then((element) => {
      if (!utils.isVisible(element)) {
        log.error("输入框不可见，不输入密码");
        return;
      }
