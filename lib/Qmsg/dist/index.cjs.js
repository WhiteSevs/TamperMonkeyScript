            ...
            Object.defineProperty(__obj__, keyName, {
                get() {
                    let findIndex = other_objs.findIndex((other_obj) => {
                        // 判断其他对象中是否有该属性
                        return (typeof other_obj === "object" &&
                            other_obj != null &&
...
class QmsgMsg {
    ...
    constructor(config, uuid) {
        ...
        this.detectionType();
        this.init();
        let consoleLogContent = typeof this.setting.consoleLogContent === "function"
    ...
    init() {
        ...
        if (this.setting.autoClose && this.setting.listenEventToPauseAutoClose) {
            ...
            let leaveEvent = (event) => {
                if (this.timeId != null) {
                    // 似乎enterEvent函数未正确调用？
                    console.warn("QmsgInst timeId is not null，mouseenter may be not first trigger，timeId：" +
                        this.timeId);
                    return;
                }
